...
export default function PilotClient() {
  ...
  const pending = useRef(null), submitting = useRef(false), sourceDialog = useRef(null), shown = useRef('');
  ...
  async function loadConversation(id) {
    ...
    // A slower load for a conversation the user has already left never replaces the current view.
  ...
  async function submit(event) {
    ...
    try {
      ...
      // A failed intent is retried only for the same conversation, question and context. Another
      // conversation keeps its own stored key; a changed question here starts a new intent.
      const intent = { convId: id, question, contextMode, language: locale, ...(dialogue.enabled ? dialogue.selection : {}) };
      const signature = value => JSON.stringify(Object.entries(value).filter(([key, v]) => key !== 'clientTurnKey' && v !== undefined)
        .sort(([a], [b]) => a.localeCompare(b)));
      const same = pending.current && signature(pending.current) === signature(intent);
      if (pending.current && !same) {
        if (pending.current.convId === id) sessionStorage.removeItem(`m4-intent/${id}`);
      ...
      pending.current = pending.current || { ...intent, clientTurnKey: saved || crypto.randomUUID() };
      ...
      // A reply for a conversation the user has left does not pull the view back to it.
      if (shown.current !== id) return;
      if (dialogue.enabled) await dialogue.refresh({ accepted: !!value.context });
      await loadConversation(id);
    } catch (e) {
      if (convId && shown.current !== convId) return;
      setError(e.message); if (convId) { await loadConversation(convId).catch(() => {}); if (dialogue.enabled) await dialogue.refresh(); }
    }
  ...
  return <div className={styles.page}>
    <header><p className={styles.eyebrow}>{t('eyebrow')}</p><h1>{t('title')}</h1>
      ...
      <p>{t('scope')}</p></header>
    <nav aria-label={t('nav')}><button onClick={() => { shown.current = ''; setConvId(''); setTurns([]); setSource(null); pending.current = null; window.history.replaceState(null, '', '/rag-pilot'); }}>{t('newConversation')}</button>
      ...
      {convId && <button disabled={busy} onClick={async () => { try { await api('/api/chat/pilot', { action: 'delete', convId }); sessionStorage.removeItem(`m4-intent/${convId}`); shown.current = ''; setConvId(''); setTurns([]); setSource(null); pending.current = null; window.history.replaceState(null, '', '/rag-pilot'); } catch (e) { setError(e.message); } }}>{t('deleteConversation')}</button>}</nav>
