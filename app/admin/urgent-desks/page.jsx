...
import AdminUrgentDesksClient from "./AdminUrgentDesksClient";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";
export const revalidate = 0;
export const metadata = {
  title: "Urgent help desks - SotsiaalAI",
  robots: { index: false, follow: false, nocache: true }
};

export default async function AdminUrgentDesksPage() {
  noStore();
  const cookieStore = await cookies();
  const locale = getLocaleFromCookies(cookieStore);
  const session = await getServerSession(authConfig).catch(() => null);
  if (!session) {
    const params = new URLSearchParams({ callbackUrl: localizePath("/admin/urgent-desks", locale) });
