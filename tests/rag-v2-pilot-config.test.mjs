...
test('pilot switch, per-user grant, expiry, real-model config and approval gates fail closed despite key presence', async t => {
  ...
  await assert.rejects(readPilotConfig('tester'), { code: 'implementation_approval_mismatch' });
  // Historical answers stay readable. Even with the current code hash, their
  // signed prompt plans cannot authorize execution with the new instructions.
  for (const promptVersion of ['m4-grounded-answer-4', 'm4-grounded-answer-5']) {
    const historicalPlan = { ...real, promptVersion };
    const historicalConfig = { ...historicalPlan, approval: { ...approved.approval, planHash: digest(historicalPlan) } };
    await fs.writeFile(file, JSON.stringify(historicalConfig));
    assert.equal((await readPilotConfig('tester', { purpose: 'read' })).configHash, digest(historicalConfig));
    await assert.rejects(readPilotConfig('tester'), { code: 'implementation_approval_mismatch' });
  }
