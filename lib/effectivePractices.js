...
export async function syncEffectivePracticeSnapshot(publication, actor, dependencies = {}) {
  if (!dependencies.request && !String(process.env.RAG_SERVICE_API_KEY || "").trim()) {
  ...
  const rag = dependencies.request && dependencies.buildHeaders
    ? { ragServiceRequest: dependencies.request, buildRagHeaders: dependencies.buildHeaders }
    : await import("./documents/ragService.js");
  await rag.ragServiceRequest(
    "/ingest/text",
    {
      method: "POST",
      headers: rag.buildRagHeaders("application/json", {
      ...
      body: JSON.stringify({
        doc_id: docId,
        text: buildEffectivePracticeRagText(snapshot),
        metadata: {
