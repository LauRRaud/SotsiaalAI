...
export function createEffectivePracticeService(db = prisma, dependencies = {}) {
  ...
  async function processRagIngest(job) {
    ...
    try {
      ...
      if (ragSync?.status === "skipped") {
        ...
        await db.dataDeletionJob.update({
          where: { id: jobId },
          data: { action: "RAG_INGEST", lastErrorCode: "rag_key_missing", nextAttemptAt: new Date(now().getTime() + ragIngestBackoffMs(1)) }
        }).catch(() => null);
  ...
  async function runPracticeReviewSchedulerTick({
    now: nowArg = null, batchSize = 50, dryRun = false, overdueDays = 14, dueWithinDays = 0
  } = {}) {
    const at = nowArg || now();
    ...
    const execute = async (tx) => {
      ...
      do {
        const page = await tx.effectivePractice.findMany({
          ...
          select: { id: true, publicId: true, contentVersion: true, publishedVersion: true, publishedAt: true, nextReviewAt: true }
        });
      ...
      return { dryRun, reviewsDue: reviews.length, assignmentsOverdue: assignments.length, reviews, assignments };
    };
  ...
  async function repairAssignments(actorValue, { dryRun = false } = {}) {
    const actor = normalizeActor(actorValue);
    ...
    return db.$transaction(async (tx) => {
      ...
      for (const practice of fragileReadyPractices) {
        const reviewerPeople = new Set(latestReviewDecisions((practice.reviews || []).filter(
        ...
        flag({ type: "high_risk_chain", issue: "insufficient_reviewers", practiceId: practice.id, reviewerCount: reviewerPeople.size });
        if (!dryRun) {
          const updated = await tx.effectivePractice.updateMany({
          ...
          if (!updated || updated.count !== 1) continue;
          await assignReplacementReviewerTx(tx, practice, "REVIEWER", practice.contentVersion, currentTime, [...reviewerPeople]);
          await recordAudit(tx, practice, auditActorId, "HIGH_RISK_REVIEW_CHAIN_REPAIRED", "READY_TO_PUBLISH", "IN_REVIEW");
        }
        candidateRepairs += 1;
      }
