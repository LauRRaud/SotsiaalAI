...
} from "./privacy/serviceProfileRetrievalGuard.js";
...
export function buildEffectivePracticeRagDocId(practice = {}) {
  const publicId = normalizeText(practice.publicId, 120);
  const version = Number.isInteger(Number(practice.version)) && Number(practice.version) > 0
    ? `::v${Number(practice.version)}`
    : "";
  return publicId ? `${EFFECTIVE_PRACTICE_RAG_PREFIX}::${publicId}${version}` : "";
}
...
export function buildEffectivePracticeRagMetadata(practice = {}, docId = buildEffectivePracticeRagDocId(practice)) {
  return {
    ...
    practice_type: normalizeText(practice.practiceType, 120) || null,
    maturity_level: normalizeText(practice.maturityLevel, 80) || null,
    risk_level: normalizeText(practice.riskLevel, 40) || null,
    version: Number.isInteger(Number(practice.version)) ? Number(practice.version) : null,
...
/**
 * SOL-SPROF-02 — TEINE UKS. Kovisiooni teadmusotsing käib sama RAG-indeksi peal
 * ja ta EI PIIRA kollektsiooni, seega võib ta tagastada ka teenuseprofiili
 * dokumendi. Ilma väravata jõuaks tagasi võetud soovitusluba siit kovisiooni
 * lõuendile tagasi ja vestlusakna värav oleks dekoratsioon.
 *
 * Filtreeritakse TOORTULEMUSED, mitte normaliseeritud loend: normaliseerimine
 * lõikab `DEFAULT_TOP_K` peale, ja pärast lõikamist filtreerimine tähendaks, et
 * keelatud vaste võtaks lubatud vastelt koha ära.
 *
 * `prisma` tuleb dünaamiliselt ja alles siis, kui profiilivaste tõesti tuli:
 * see moodul on teadlikult serverikihita (sama põhjus, mis `ragService`-l).
 * Kui import kukub, jääb `db` nulliks ja värav on fail-closed.
 */
export async function filterCovisionKnowledgeConsent(results, { db = null } = {}) {
  const rawResults = Array.isArray(results) ? results : [];
  const hasProfileHit = rawResults.some((entry) =>
    Boolean(serviceProfileIdFromDocId(ragMatchDocId(entry)))
  );
  if (!hasProfileHit) return rawResults;

  const consentDb = db || (await import("@/lib/prisma")
    .then((mod) => mod.prisma || mod.default || null)
    .catch(() => null));
  return filterRevokedServiceProfileMatches(rawResults, { db: consentDb });
}

export async function fetchCovisionKnowledgeSupport() {
  return { ok: true, available: false, reason: "rag_retired", query: "", results: [] };
}
