...
export const ActionKind = Object.freeze({
  ...
  OPEN_WELLBEING_RECORD: "open_wellbeing_record",
  OPEN_LISTING: "open_listing",
  OPEN_INVITE: "open_invite",
  OPEN_PROFILE: "open_profile",
  OPEN_LIST: "open_list",
  OPEN_MENTORING: "open_mentoring",
  OPEN_MENTORING_RELATION: "open_mentoring_relation",
  OPEN_MENTOR_PROFILE: "open_mentor_profile",
  OPEN_ADMIN_MENTORING: "open_admin_mentoring",
  OPEN_FIELD_VISIT: "open_field_visit",
  OPEN_NETWORK_SHARE: "open_network_share"
});
...
export const ACTION_REGISTRY = Object.freeze({
  ...
  /* Kontrollpunkti teavitus peab viima KIRJE juurde, mitte Tööheaolu avalehele:
     „sinu kontrollpunkt saabus" + maandumine tühjal avalehel oleks sama V6 viga,
     mille kirjete lugemisrada äsja parandas. */
  [ActionKind.OPEN_WELLBEING_RECORD]: entry((target) => `/tooheaolu/minu-kirjed?record=${encodeURIComponent(targetId(target, "record"))}`),
  ...
  [ActionKind.OPEN_MENTORING]: entry(() => "/mentorlus"),
  [ActionKind.OPEN_MENTORING_RELATION]: entry((target) => `/mentorlus/suhe/${encodeURIComponent(targetId(target, "mentoring_relation"))}`),
  [ActionKind.OPEN_MENTOR_PROFILE]: entry(() => "/mentorlus/profiil"),
  [ActionKind.OPEN_ADMIN_MENTORING]: entry(() => "/admin/mentorlus"),
  [ActionKind.OPEN_FIELD_VISIT]: entry((target) => `/valitoo/${encodeURIComponent(targetId(target, "field_visit"))}`),
  [ActionKind.OPEN_NETWORK_SHARE]: entry((target) => `/eelpoordumised?networkShare=${encodeURIComponent(targetId(target, "network_share"))}`),
  [ActionKind.OPEN_WORKSPACE]: entry((target) => {
    ...
    if (kind === WorkspaceKind.MENTORING_PROCESS) return `/mentorlus/suhe/${encodeURIComponent(id)}`;
    if (kind === WorkspaceKind.FIELD_VISIT) return `/valitoo/${encodeURIComponent(id)}`;
    /* JTA-V1 (E6): `case_work` on olnud SUPPORTED juba JUHTUM-V1-st, aga temani
       ei viinud ükski tee — sündmus oleks visanud `UNSUPPORTED_WORKSPACE_ACTION`
       alles emiteerimise hetkel, mitte lepingu kirjutamise ajal.

       AADRESS ON PÄRINGUPARAMEETER, mitte tee: JUHTUM-V1 valis teadlikult ÜHE
       marsruudi (`/juhtumid`) ja detailvaade avaneb tema sees. `/juhtumid/<id>`
       oleks tuletatud aadress, mida ei ole olemas — sama viga, mille kuues
       audit laual juba üks kord leidis. */
    if (kind === WorkspaceKind.CASE_WORK) return `/juhtumid?juhtum=${encodeURIComponent(id)}`;
