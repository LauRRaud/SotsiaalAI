...
export async function getWorkspaceContinuity(
  ...
  const [
    drafts, received, memberships, activeSubscription, wellbeingDrafts, journeys, assignments, services,
    ...
    db.roomMember.findMany({
      ...
      select: {
        ...
        lastReadAt: true,
        billingSource: true,
        room: { select: { helpMatch: { select: { id: true } } } }
      },
  ...
  const accessibleMemberships = memberships.filter((membership) => hasRoomBillingAccess({
    userRole: String(role || "").trim().toUpperCase(),
    membership,
    hasActiveSubscription: Boolean(activeSubscription),
    room: membership.room
  }).ok);
  const roomUnread = await Promise.all(accessibleMemberships.map(async (membership) => ({
    ...membership,
