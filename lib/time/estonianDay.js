 ...
 *
 * KAKS SERVAJUHTU, MIS EI OLE VEAD VAID VALIKUD — ja mis on siin kirjas
 * seepärast, et funktsiooni nimi lubab ÜLDIST teisendust, mitte ainult
 * keskööd. Mõlemad on mõõdetud, mitte tuletatud, ja testid kinnitavad nad:
 *
 *   1. **OLEMATU kohalik aeg** (kevadine auk). Tallinnas ei eksisteeri
 *      `29.03.2026 03:30` — kell hüppab 03:00 → 04:00. Vastus on
 *      `01:30Z` = **04:30 kohalikku**, st hetk augu VÕRRA edasi.
 *      Funktsioon EI VISKA ja EI TEATA, et sisend oli olematu.
 *
 *   2. **MITMENE kohalik aeg** (sügisene kordus). `25.10.2026 03:30` esineb
 *      kaks korda; vastus on `01:30Z` — **HILISEM** esinemine, talveaeg.
 *
 * MIKS EI VALIDEERI. Selle mooduli kutsujad annavad ainult keskööd, ja
 * kalendripäeva ALGUSE jaoks on „augu võrra edasi" täpselt õige vastus:
 * vööndites, kus kell keeratakse keskööl (nt `America/Santiago`), ei
 * eksisteeri `00:00` ja päev algab päriselt `01:00`-st. Erind teeks seal
 * õigest vastusest vea. Kui keegi hakkab siit küsima suvalist kellaaega ja
 * tal on vaja teada, kas see aeg üldse oli — see kontroll tuleb KUTSUJA
 * poolele lisada, mitte siia, ja tema peab ütlema, mida ta olematu ajaga
 * teha tahab.
 */
