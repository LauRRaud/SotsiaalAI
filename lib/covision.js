...
export async function listCovisionWorkspace(auth, { db = prisma } = {}) {
  const cases = await db.covisionCase.findMany({
    where: workspaceCaseWhere(auth),
    orderBy: [{ lastActivityAt: "desc" }, { updatedAt: "desc" }, { id: "desc" }],
    select: workspaceCaseSelect(auth)
  });
  const serialized = cases.map((item) => serializeCovisionWorkspaceCase(item, auth)).filter(Boolean);
  serialized.sort((left, right) => Number(right.isInvitation) - Number(left.isInvitation)
    || new Date(right.lastActivityAt || right.updatedAt) - new Date(left.lastActivityAt || left.updatedAt)
    || String(right.id).localeCompare(String(left.id)));
  return {
    capabilities: { canCreate: canCreateCovision(auth) },
    counts: {
      total: serialized.length,
      invitations: serialized.filter((item) => item.isInvitation).length
    },
    cases: serialized
  };
}
...
export async function createCovisionCase(auth, input = {}, options = {}) {
  ...
  const covisionCase = await prisma.$transaction(async (tx) => {
    ...
    await Promise.all(createdParticipants
      .filter((participant) => participant.role !== "OWNER" && participant.email)
      .map((participant) => queueCovisionInviteDelivery(tx, {
        participantId: participant.id,
        email: participant.email
      })));
    await Promise.all(createdParticipants
      .filter((participant) => participant.role !== "OWNER")
      .map((participant) => appendCovisionAuditEvent(tx, {
        covisionCaseId: createdCase.id,
        participantId: participant.id,
        actorUserId: auth.userId,
        actorRoleSnapshot: auth.role,
        action: "INVITE_PARTICIPANT",
        entityType: "PARTICIPANT",
        entityId: participant.id,
        idempotencyKey: `${createdCase.id}:participant:${participant.id}:invited`,
        metadata: { invitedRole: participant.role }
      })));
...
export async function updateCovisionCase(auth, id, input = {}, { db = prisma } = {}) {
  const result = await withCovisionLegacyWriteLock(
    ...
    async (tx, existing) => {
      ...
      await Promise.all((covisionCase.participants || [])
        .filter((participant) => invitedEmails.includes(participant.email))
        .map((participant) => queueCovisionInviteDelivery(tx, {
          participantId: participant.id,
          email: participant.email
        })));
      await Promise.all((covisionCase.participants || [])
        ...
        .map((participant) => appendCovisionAuditEvent(tx, {
          ...
          entityType: "PARTICIPANT",
          entityId: participant.id,
          idempotencyKey: `${covisionCase.id}:participant:${participant.id}:invited`,
          metadata: { invitedRole: participant.role }
        })));
