...
const EVENT_SELECT = Object.freeze({
  ...
  fieldKeys: true,
  /* SOL-CW-16: sõrmejälg on TÕENDI osa ja ta peab vastuses olema — muidu ei saa
     keegi hiljem kontrollida, millise tekstiversiooniga rida seotud on. */
  contentHash: true,
  createdAt: true
});
...
/**
 * PLOKI SISU KANOONILINE SÕRMEJÄLG (SOL-CW-16).
 *
 * MIKS SEE OLEMAS ON. Audit tõendas seni teo AEGA ja VÄLJANIMESID, aga mitte
 * seda, milline tekst tegelikult lõikelauale läks. Negatiivkontrollis kopeeriti
 * „VERSIOON A", andmebaasis asendati see enne auditit „VERSIOON B"-ga ja audit
 * võeti vastu — append-only rida seisis vale sisuseisu küljes.
 *
 * SÕRMEJÄLG, MITTE SNAPSHOT. L8 keelab väljade VÄÄRTUSTE salvestamise:
 * täissnapshot elaks üle E7 sisu-purge'i ja oleks varju-register, ehitatud selle
 * mehhanismi sisse, mis pidi teda ära hoidma. Räsi tõendab identsust ilma sisu
 * hoidmata ja purge'i üle ta midagi ei kanna.
 *
 * KANOONILINE KUJU on `[[fieldKey, text], …]` võtme järgi sorditult, JSON-ina.
 * Sortimine on kohustuslik: päringu järjekord võib muutuda ja siis annaks sama
 * sisu kaks eri räsi. HOIATUSRIDA JA KEEL EI KÄI KAASA — nemad on serveri
 * kaunistus, mitte juhtumi sisu, ja `et` all laaditud plokk peab andma sama
 * tõendi mis `ru` all.
 */
export function star2ContentHash(fields) {
  const rows = (Array.isArray(fields) ? fields : [])
    .map((field) => [String(field?.fieldKey ?? ""), String(field?.text ?? "")])
...
function normalizeContentHash(value) {
  const hash = normalizeId(value);
  /* KOHUSTUSLIK, mitte valikuline. Ilma räsita ei ole auditirida ühegi
     tekstiversiooniga seotud — täpselt see seis, mille SOL-CW-16 maha võttis.
     Vaikselt lubatud puuduv väärtus tähendaks, et vana klient taastab vea. */
  if (!hash || !CONTENT_HASH_SHAPE.test(hash.toLowerCase())) {
    throw badRequest("casework.errors.transfer_content_hash_invalid");
  }
...
export async function buildStar2Block({
  ...
  return {
    ...
    /* SISU SÕRMEJÄLG (SOL-CW-16). Ta käib ploki ja auditi vahel kaasa ning seob
       auditirea SELLE tekstiversiooniga, mis päriselt lõikelauale läks. */
    contentHash: star2ContentHash(fields),
 ...
 *
 * `READ_ONLY` JUHTUM EI BLOKEERI SEDA ja see on teadlik, kitsas erand
 ...
 * mitte-`ACTIVE` juhtumitel. `ARCHIVED` on lõplik säilituseis ja sinna uusi
 * ülekandetõendeid enam ei lisata.
 *
 * KORDUS ANNAB 200, MITTE 409 (L22) — AGA AINULT SAMA TEO KORDUS. Kasutaja tegi
 * ühe teo ja peab nägema ühte tulemust; 409 sunniks liidese seletama viga, mida
 * ei ole. Sama võti TEISE payload'iga ei ole aga kordus, vaid uus tegu vana
 * võtme all (SOL-CW-06): vaikne 200 tagastaks eelmise kopeerimise auditirea ja
 * teine tegu jääks tõendita. Selline vastuolu annab 409.
 *
...
export async function recordCopyEvent({
  ...
  const copiedHash = normalizeContentHash(contentHash);

  const caseWork = await requireVisibleCase({ db, ownerUserId: owner, caseWorkAssistId: caseId });
  /* READ_ONLY jääb teadlikult lubatuks: kopeerimine on lugemistegu ja audit
     kirjeldab juba toimunud lõikelauategevust. ARCHIVED on seevastu lõplik
     säilituseis — sinna ei tohi enam uut tõendit lisada. */
  if (caseWork.retentionState === RETENTION_STATE.ARCHIVED) {
    throw conflict("casework.errors.not_active");
  }
  const draft = await requireDraftInCase({ db, caseWorkAssistId: caseId, draftId: id });
  ...
  /* PLOKK PEAB OLEMA VEEL VÄRSKE (SOL-CW-16).
     Väljanimede olemasolu kontroll ütles ainult, et samanimelised väljad
     eksisteerivad PRAEGU — mitte seda, et nende sisu on sama, mis lõikelauale
     läks. Negatiivkontrollis kopeeriti „VERSIOON A", andmebaasis asendati see
     „VERSIOON B"-ga ja audit võeti vastu.
     409, MITTE 400: mustand on kasutajale nähtav ja takistus on SEISUND —
     tema plokk on aegunud. Õige tee edasi on kopeerida uuesti, mitte parandada
     päringut. */
  if (star2ContentHash(existing) !== copiedHash) {
    throw conflict("casework.errors.transfer_block_stale");
  }
  ...
  const data = {
    ...
    /* SÕRMEJÄLG JÄÄB REA KÜLGE (SOL-CW-16). Ilma temata tõendaks rida ainult
       aega ja väljanimesid; temaga saab hiljem küsida „kas STAR-i läks SEE
       tekst" ja vastata jah või ei. Väärtusi ta ei kanna (L8). */
  ...
  try {
    ...
    /* SAMA VÕTI PEAB TÄHENDAMA SAMA TEGU (SOL-CW-06). Ilma selle kontrollita
       annaks sama võti teiste väljadega näiliselt eduka 200 ja tagastaks
       eelmise kopeerimise auditirea — teine tegu jääks jäädavalt tõendita. */
    if (!isSameCopyAction(event, data)) {
      throw conflict("casework.errors.transfer_action_key_conflict");
    }
...
/**
 * Kas olemasolev auditirida kirjeldab TÄPSELT sama kopeerimist?
 *
 * Võrreldakse teo identiteeti, mitte kogu rida: `id` ja `createdAt` on esimese
 * kirjutuse omad ka õigustatud korduse korral. `fieldKeys` järjekord loeb —
 * teine järjekord tähendab, et kasutaja valis väljad teisiti, ja auditirida
 * peab kirjeldama seda, mis päriselt juhtus.
 */
function isSameCopyAction(existing, data) {
  const sameKeys =
    Array.isArray(existing.fieldKeys) &&
    existing.fieldKeys.length === data.fieldKeys.length &&
    existing.fieldKeys.every((key, index) => key === data.fieldKeys[index]);

  return (
    sameKeys &&
    existing.kind === data.kind &&
    existing.draftType === data.draftType &&
    existing.transferStateAtEvent === data.transferStateAtEvent &&
    existing.caseWorkAssistId === data.caseWorkAssistId &&
    existing.actorUserId === data.actorUserId &&
    /* SISU VERSIOON ON OSA TEO IDENTITEEDIST (SOL-CW-16): sama võti teise
       tekstiversiooniga ei ole kordus, vaid uus tegu vana võtme all. */
 ...
 *
 * NELI ASJA ÜHES TEHINGUS: tingimuslik siire (`expectedFrom` → 409, kui keegi
 * jõudis ette), `transferredAt`, auditirida ja U1 outbox-sündmus.
 * `withActiveCaseLock()` on väljaspool neid, sest mustandi seisu muutmine ON
 * juhtumi lapse kirjutus (L21) — ja tema jõustaja peab olema kirjutusega samas
 * atomaarses piiris.
 *
 * SÜNDMUS ON TEHINGU SEES (SOL-CW-04), MITTE PÄRAST COMMIT'i.
 *
 * Varem emiteeriti ta pärast commit'i, põhjendusega „tehingu sees emiteeritud
 * sündmus jõuaks välja ka siis, kui tehing tagasi veereb". See põhjendus kehtiks
 * välise teenuse väljakutse kohta, aga `emitDomainEvent` EI KUTSU midagi välja —
 * ta kirjutab `DomainEvent` rea SAMASSE andmebaasi (outbox). Outbox-rida veereb
 * tehinguga koos tagasi, seega „sündmus ülekande kohta, mida ei toimunud" ei ole
 * võimalik. Pärast commit'i emiteerimine tähendas aga vastupidist ja päris riski:
 * teine tehing võis ebaõnnestuda ja viga neelati alla — API vastas eduga, seis
 * muutus, aga ajajoone-/teavitussündmus kadus jäädavalt ilma ühegi taastajata.
 *
 * Sama muster, mis platvormil juba mujal: `lib/journey/service.js` ja
 * `lib/preInquiries.js` emiteerivad samuti tehingu sees.
 */
export async function markTransferred({
  ...
  const result = await withActiveCaseLock({ ownerUserId: owner, caseWorkAssistId: caseId, db }, async (tx) => {
    ...
    /* SÜNDMUS SIIN, mitte pärast commit'i: kui outbox-rea kirjutus ebaõnnestub,
       peab kogu ülekanne tagasi veerema. Vaikne osaline edu (seis muutus,
       sündmus kadus) on halvem kui aus 500, mille peale kasutaja kordab —
       kordus on ohutu, sest tingimuslik siire annab teisel korral 409. */
