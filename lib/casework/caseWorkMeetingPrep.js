...
import { RETENTION_STATE, caseDisplayLabel, resolveClientNames, withActiveCaseLock } from "./caseWorkAssist.js";
...
const PREP_SELECT = Object.freeze({
  ...
  meetingAt: true,
  /* O-JTA-6: purge'itud ettevalmistus peab olema pinnal ERISTATAV tühjast.
     Ilma nende väljadeta näeks „töötaja arhiveeris töömaterjali" välja täpselt
     nagu „ettevalmistust ei ole veel alustatud". */
...
/**
 * PURGE'ITUD ETTEVALMISTUSSE EI KIRJUTATA UUT SISU (O-JTA-6).
 *
 * MIKS: `contentPurgedAt` on avaldus selle kohta, et selle ettevalmistuse
 * kliendi sisu on kustutatud. Kui sinna saaks kohe uue välja kirjutada, oleks
 * see avaldus VALE ja ekraanil seisaks korraga „sisu on arhiveeritud" ja sisu.
 * Uue kohtumise ettevalmistus on uus rida — neid tohib olla mitu (O-JTA-3).
 *
 * 409, MITTE 404: ettevalmistus on kasutajale nähtav ja takistus on SEISUND,
 * mitte ligipääs. Sama vahe mis mustandi terminaalses seisus.
 */
function assertPrepWritable(prep) {
  if (prep?.contentPurgedAt) throw conflict("casework.errors.prep_content_purged");
}
...
export async function updateMeetingPrep({ ownerUserId, caseWorkAssistId, meetingPrepId, meetingAt, db = prismaClient } = {}) {
  ...
  return withActiveCaseLock({ ownerUserId: owner, caseWorkAssistId: caseId, db }, async (tx) => {
    const prep = await requirePrepInCase({ db: tx, caseWorkAssistId: caseId, meetingPrepId: prepId });
    assertPrepWritable(prep);
...
/**
 * Laua ettevalmistuste väljavalik (SOL-CW-13).
 *
 * OMA SELECT, MITTE `PREP_SELECT`: silt tuleb JUHTUMILT ja seda pesastatud
 * valikut ei tohi olla juhtumi enda prep-vastustes. Kui laiendaksin
 * `PREP_SELECT`-i, ilmuks kliendi identiteet ka `getMeetingPrep()` väljundisse,
 * kus tal ei ole asja (L20 valge nimekiri).
 */
const OWNER_PREP_SELECT = Object.freeze({
  id: true,
  caseWorkAssistId: true,
  meetingAt: true,
  updatedAt: true,
...
/**
 * Laua aktiivsed ettevalmistused — OMANIKU SKOOBIS, üle kõigi juhtumite.
 *
 * MIKS OMA LUGEJA (SOL-CW-13). Laud kuvas siin `listCaseWorkAssists()` tulemust:
 * ta nimetas IGA aktiivse juhtumi ettevalmistustööks ega eristanud päriselt
 * alustatud ettevalmistust läheneva kohtumise omast. Töötaja prioriteediloend
 * oli sisuliselt vale. `listMeetingPreps()` on juhtumipõhine ja lauale ei kõlba
 * — laud vajab ridu ÜLE juhtumite, aga sama omanikupiiriga.
 *
 * MIS ON „AKTIIVNE". Kolm tingimust, igaühel oma põhjus:
 *
 *   1. **juhtum on `ACTIVE`** — `READ_ONLY` ja `ARCHIVED` juhtum on laualt
 *      läinud (sama reegel mis mustanditel, SOL-CW-03). Ettevalmistus, mida
 *      keegi enam muuta ei tohi, ei ole tööjärg.
 *   2. **sisu ei ole purge'itud** — `contentPurgedAt` tähendab, et töötaja
 *      arhiveeris töömaterjali (O-JTA-6). Alles on marker, mitte tööjärg.
 *      Juhtumi enda pinnal jääb ta nähtavaks ja tühjast eristatavaks; laual ei
 *      ole tal midagi öelda.
 *   3. **kohtumine ei ole möödas** — piir tuleb KUTSUJALT (`from`), mitte
 *      siit. Laud annab Eesti kalendripäeva alguse, sest kell 09:00 algav
 *      kohtumine ei tohi laualt kaduda 09:01, kui töötaja alles istub selles.
 *
 * AJATA ETTEVALMISTUS JÄÄB ALLES. `meetingAt: null` on plaan ilma kuupäevata —
 * pooleliolev töö, mille kadumine oleks vaikne andmekadu. Ta seisab loendi
 * lõpus, sest kella tal ei ole.
 *
 * **Teadlik otsus, mille omanik võib ümber pöörata:** möödunud kohtumise
 * ettevalmistus KAOB laualt. Alternatiiv oleks „üle tähtaja" märgis, mille L3
 * sõnaselgelt keelab (laud ei loenda töötajat). Juhtum ise jääb alles ja tema
 * ettevalmistus on juhtumi pinnal.
 */
export async function listActiveMeetingPrepsForOwner({
  ownerUserId,
  from = null,
  limit = LIST_LIMIT_DEFAULT,
  db = prismaClient
  ...
  const take = normalizeLimit(limit, { fallback: LIST_LIMIT_DEFAULT, max: LIST_LIMIT_MAX });
  const since = from instanceof Date && !Number.isNaN(from.getTime()) ? from : null;

  const rows = await db.caseWorkMeetingPrep.findMany({
    where: {
      caseWorkAssist: { ownerUserId: owner, retentionState: RETENTION_STATE.ACTIVE },
      contentPurgedAt: null,
      ...(since ? { OR: [{ meetingAt: null }, { meetingAt: { gte: since } }] } : {})
    },
    select: OWNER_PREP_SELECT,
    /* LÄHIM KOHTUMINE EES — vastupidi `listMeetingPreps()`-ile, kus uusim ees
       on juhtumi AJALUGU. Laud on tööjärg, mitte ajalugu. `nulls: "last"`
       hoiab ajata plaanid lõpus; ilma temata upuksid nad Postgresis `ASC`
       sortimisel ette ja tõrjuksid homse kohtumise silma alt. */
    orderBy: [{ meetingAt: { sort: "asc", nulls: "last" } }, { updatedAt: "desc" }, { id: "desc" }],
    take
  });

  /* Silt lahendatakse SIIN, ühe hulgipäringuga. Kuvanime reegel (L10) elab
     `caseDisplayLabel`-is ja teine koopia läheks temast lahku. */
  const cases = rows.map((row) => row.caseWorkAssist).filter(Boolean);
  const names = await resolveClientNames(cases, { db });
  ...
  return {
    items: rows.map((row) => ({
      ...
      label: caseDisplayLabel(row.caseWorkAssist || {}, names.get(row.caseWorkAssist?.clientUserId) || null)
    }))
  };
}
 ...
 *
 * AKTIIVNE ETTEVALMISTUS ON TULEVIKUPLAAN, MITTE TÕEND: ta kirjeldab
 * kohtumist, mida veel ei ole olnud. Selle kustutus on kõva kustutus ja
 * kaskaad viib väljad ja küsimused kaasa. Purge'itud konteiner on aga
 * arhiveerimise marker ning peab koos seostega alles jääma (O-JTA-6).
 */
...
export async function setPrepField({
  ...
  return withActiveCaseLock({ ownerUserId: owner, caseWorkAssistId: caseId, db }, async (tx) => {
    const prep = await requirePrepInCase({
      db: tx,
      caseWorkAssistId: caseId,
      meetingPrepId: prepId,
      select: { id: true, contentPurgedAt: true }
    });
    assertPrepWritable(prep);
