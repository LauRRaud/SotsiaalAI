 ...
 *
 * Leping: docs/platvormi arendus/juhtum-v1-arendusleping.md (v7), etapp E2.
 *
...
import { recipientVisiblePreInquiryWhere } from "@/lib/preInquiries";
...
import { decodeListCursor, encodeListCursor, listCursorWhere, normalizeLimit } from "./paging.js";
...
/** UUID-kuju, sama mis kopeerimisauditil (L22). Puuduv võti on lubatud. */
const ACTION_KEY_SHAPE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/;

function normalizeActionKey(value) {
  const key = normalizeId(value);
  if (!key) return null;
  /* OMA VÕTI, mitte kopeerimisauditi oma: sõnum „Kopeerimise tunnus on vigase
     kujuga" räägiks juhtumi loomisel toimingust, mida kasutaja ei teinud. */
  if (!ACTION_KEY_SHAPE.test(key.toLowerCase())) throw badRequest("casework.errors.create_action_key_invalid");
  return key.toLowerCase();
}

/**
 * Säilitusseisu filter või `null`. TUNDMATU VÄÄRTUS ON KLIENDI VIGA (SOL-CW-08).
 *
 * Sama põhjendus mis `normalizeDate`-il allpool: valideerimata väärtus jõuab
 * vaikselt Prisma enum-filtrisse, kes viskab oma vea, ja kasutaja saab 400
 * asemel 500. Piir on siin, kus väärtus veel tähendust kannab.
 */
function normalizeRetentionStateFilter(value) {
  if (value === null || value === undefined) return null;
  const text = String(value).trim();
  if (!text) return null;
  /* Sama tõlkevõti mis `transitionRetention`-il: „tundmatu seis" on kasutaja
     jaoks üks ja seesama asi, olgu ta filtris või siirdes. */
  if (!Object.hasOwn(RETENTION_STATE, text)) throw badRequest("casework.errors.retention_unknown");
  return RETENTION_STATE[text];
}

/**
 * Ajahetk või `null`. VIGANE SISEND EI LÄHE EDASI.
 *
 * `new Date("eile")` annab `Invalid Date` ja ta jõuab vaikselt Prismani, kes
 * viskab oma valideerimisvea — kasutaja saab 400 asemel sisemise vea ja logi
 * täitub päringutega, mille põhjus on tegelikult trükiviga. Piir on siin, kus
 * väärtus veel tähendust kannab.
 */
function normalizeDateTime(value, messageKey) {
  if (value === null || value === undefined || value === "") return null;
  const date = value instanceof Date ? value : new Date(value);
  if (Number.isNaN(date.getTime())) throw badRequest(messageKey);
  return date;
}
...
/**
 * Kiire abi pöördumise seisud, millest juhtumit EI SAA luua (SOL-CW-11).
 *
 * `RECALLED` on ainus, mis on siin NÕUTUD: saatja võttis pöördumise tagasi ja
 * tema sisust ei tohi enam uut tööobjekti sündida. `EXPIRED`, `RESOLVED` ja
 * `DECLINED` on TEADLIKULT LUBATUD — laud näeb neid endiselt oma järjekorras
 * (`lib/urgent/deskQueue.js` toob kõik laua read), ja kui juhtumi loomine neist
 * keelduks, ütleksid järjekord ja juhtumipind eri tõde. Kui omanik soovib
 * kitsamat reeglit, on ta täpselt siin, ühes konstandis.
 */
const ORIGIN_BLOCKED_URGENT_STATUSES = Object.freeze(["RECALLED"]);
 ...
 *
 * ELUTSÜKKEL ON OSA NÄHTAVUSEST (SOL-CW-11). Varem kontrolliti eelpöördumisel
 * ainult `id + recipientOwnerId`: tagasivõetud (`recalledAt`) või saatmata
 * (`DRAFT`) pöördumisest sai otse-API kaudu juhtumi teha. Filter tuleb nüüd
 * eelpöördumise enda töövoost (`recipientVisiblePreInquiryWhere`), mitte
 * teisest koopiast — kaks koopiat tähendaks, et üks jääb muutmisel maha.
 *
...
async function resolveOrigin({ db, ownerUserId, preInquiryId, urgentRequestId }) {
  ...
  if (preId) {
    const preInquiry = await db.preInquiry.findFirst({
      where: { id: preId, ...recipientVisiblePreInquiryWhere(ownerUserId) },
  ...
  const request = await db.urgentRequest.findFirst({
    where: {
      id: urgentId,
      status: { notIn: ORIGIN_BLOCKED_URGENT_STATUSES },
      desk: {
 ...
 *
 * CURSOR KANNAB TÄIELIKKU SORTIMISVÕTIT (SOL-CW-20), mitte ainult ID-d. Varem
 * liikus kliendile rea ID ja järgmine leht leiti Prisma `cursor: { id }`-ga,
 * mis positsioneerib rea PRAEGUSE koha järgi. `updatedAt` on aga muutlik —
 * juhtumi iga lapse kirjutus puudutab vanemrida (`touchCase`) — nii et vahepeal
 * muudetud rida kas kordus järgmisel lehel või kadus lehtede vahele jäljetult.
 * Nüüd on lehepiir keyset-tingimus + esimese lehe **stabiilne ülempiir**, seega
 * lehitsemine on snapshot: vahepealne muudatus tuleb nähtavale värskendusel,
 * mitte poole lehitsemise pealt. Vt `paging.js`.
 *
 * `retentionState` VALIDEERITAKSE SIIN (SOL-CW-08): marsruut edastab URL-i
 * suvalise väärtuse ja valideerimata enum-filter annab Prismalt erindi, mille
 * veakaardistus muudab 500-ks. Vigane päringuparameeter on KLIENDI viga —
 * 500 rikub API lepingut ja toodab eksitavat veaseiret. Sama reegel kehtib
 * cursorile: loetamatu cursor annab 400, MITTE vaikset esimest lehte. Vaikne
 * tagasilangus kaotaks kasutaja koha loendis ja näeks välja nagu andmete kadu.
 */
export async function listCaseWorkAssists({
  ...
  const position = decodeListCursor(cursor);
  const state = normalizeRetentionStateFilter(retentionState);

  const rows = await db.caseWorkAssist.findMany({
    where: {
      ownerUserId: owner,
      ...(state ? { retentionState: state } : {}),
      ...listCursorWhere(position)
    },
  ...
  /* ÜLEMPIIR SÜNNIB ESIMESEL LEHEL ja kandub edasi muutumatuna. Kui ta tekiks
     iga lehe peal uuesti, liiguks snapshot koos lehitsemisega ja kogu kaitse
     kaoks — täpselt see viga, ainult aeglasemal kujul. */
  const boundary = position ? position.boundary : page[0]?.updatedAt || null;
  const last = page[page.length - 1];

  return {
    items: page.map((row) => ({ ...row, label: caseDisplayLabel(row, names.get(row.clientUserId) || null) })),
    nextCursor:
      hasMore && last && boundary ? encodeListCursor({ updatedAt: last.updatedAt, id: last.id, boundary }) : null
  };
}
...
export async function createCaseWorkAssist({
  ...
  const nextContact = normalizeDateTime(nextContactAt, "casework.errors.next_contact_invalid");
  const actionKey = normalizeActionKey(clientActionId);

  /* KONTROLL JA LOOMINE ÜHES TEHINGUS (SOL-CW-11). Varem olid nad eraldi
     päringud: kontrolli ja loomise vahel jõudis saatja pöördumise tagasi võtta
     ja juhtum sündis sisust, mida tal enam ei olnud õigust kasutada. */
  const row = await db.$transaction(async (tx) => {
    const origin = await resolveOrigin({ db: tx, ownerUserId: owner, preInquiryId, urgentRequestId });
    ...
    let created;
    try {
      created = await tx.caseWorkAssist.create({ data, select: ASSIST_SELECT });
    } catch (error) {
      if (!isUniqueConflict(error)) throw error;
      /* JÕUSTAJA ON INDEKS, mitte eelkontroll (SOL-CW-12). Siia jõutakse alles
         PÄRAST seda, kui andmebaas on korduse tagasi lükanud. */
      return resolveCreateConflict({ db: tx, owner, actionKey, origin, data });
    }

    /* SAMA KONTROLL UUESTI, NÜÜD INSERT'i JÄREL. PostgreSQL READ COMMITTED
       annab igale lausele värske hetktõmmise, seega vahepeal commit'itud
       tagasivõtt on siin nähtav ja kogu tehing veereb tagasi. Ilma selleta
       jääks aken kontrolli ja kirjutuse vahele lahti. */
    if (origin.preInquiryId || origin.urgentRequestId) {
      await resolveOrigin({ db: tx, ownerUserId: owner, preInquiryId, urgentRequestId });
    }
...
/**
 * Unikaalsuskonflikti tõlgendus (SOL-CW-12).
 *
 * KAKS ERI OLUKORDA, KAKS ERI VASTUST:
 *
 *   sama `clientActionId`  → SAMA TEGU. Topeltklõps või võrgu korduskatse peab
 *                            andma sama juhtumi tagasi, mitte veateate. AGA
 *                            ainult siis, kui ka sisu on sama — sama võti teise
 *                            sisuga on uus tegu vana võtme all ja annab 409
 *                            (sama reegel mis kopeerimisauditil, SOL-CW-06).
 *
 *   sama lähteobjekt       → TEADLIK TEINE KATSE. Töötaja proovib samast
 *                            pöördumisest teist juhtumit teha; see on omaniku
 *                            otsusega keelatud ja peab olema NÄHTAV, mitte
 *                            vaikselt esimeseks juhtumiks tõlgitud.
 */
async function resolveCreateConflict({ db, owner, actionKey, origin, data }) {
  if (actionKey) {
    const existing = await db.caseWorkAssist.findFirst({
      where: { ownerUserId: owner, clientActionId: actionKey },
      select: ASSIST_SELECT
    });
    if (existing) {
      if (!isSameCaseCreation(existing, data)) throw conflict("casework.errors.create_action_key_conflict");
      return existing;
    }
  }

  if (origin.preInquiryId || origin.urgentRequestId) {
    throw conflict("casework.errors.origin_already_has_case");
  }
  throw conflict();
}

/**
 * Kas olemasolev juhtum on TÄPSELT sama loomiskutse tulemus?
 *
 * Võrreldakse teo identiteeti, mitte kogu rida: `id`, `createdAt` ja
 * `retentionState` on esimese kirjutuse omad ka õigustatud korduse korral.
 * `nextContactAt` võrreldakse ajahetkena, sest sisendist tuleb `Date`.
 */
function isSameCaseCreation(existing, data) {
  ...
  return (
    existing.preInquiryId === (data.preInquiryId ?? null) &&
    existing.urgentRequestId === (data.urgentRequestId ?? null) &&
    existing.clientUserId === (data.clientUserId ?? null) &&
    existing.clientDisplayName === (data.clientDisplayName ?? null) &&
    existing.clientExternalRef === (data.clientExternalRef ?? null) &&
    existing.externalSystem === (data.externalSystem ?? null) &&
    existing.externalReference === (data.externalReference ?? null) &&
    sameTime(existing.nextContactAt, data.nextContactAt)
  );
}
...
export async function updateCaseWorkAssist({ ownerUserId, id, patch = {}, db = prismaClient } = {}) {
  ...
  if ("nextContactAt" in patch) {
    data.nextContactAt = normalizeDateTime(patch.nextContactAt, "casework.errors.next_contact_invalid");
  }
...
/* ────────────────────────────────────────────────────────────────────────────
   LASTE KIRJUTUSKAITSE (L14) — JÕUSTAJA, MITTE EELKONTROLL
   ──────────────────────────────────────────────────────────────────────────── */

/**
 * Kirjutab lapse (puuduv info, seos, …) juhtumisse, mis on kirjutuse HETKEL
 * veel `ACTIVE`.
 *
 * MIKS SEE OLEMAS ON. Lapsemoodulid tegid varem `loe → kontrolli → kirjuta`:
 * eraldi päring küsis, kas vanem on `ACTIVE`, ja alles siis läks kirjutus
 * teele. Kahe päringu vahele mahub `transitionRetention()`:
 *
 ...
 *
 * See on TÄPSELT sama võistlus, mille vastu `updateCaseWorkAssist()` end juba
 * tingimusliku update'iga kaitseb — ainult et lapse peal jäi ta lahti. Vanema
 * seisu kontroll ei tohi olla lapse kirjutuse EEL, ta peab olema tema SEES.
 *
 * KUIDAS. Tingimuslik `updateMany` vanema real võtab Postgresis reataseme
 * LUKU, mis kestab tehingu lõpuni. Samaaegne `transitionRetention()` jääb
 * ootama; kui tema jõuab enne, ei leia meie `WHERE … retentionState = ACTIVE`
 * enam ühtegi rida ja kirjutus KUKUB, mitte ei võida. Kaks samaaegset lapse
 * kirjutust ei sega teineteist — nad ootavad järjekorras ja mõlemad näevad
 * kehtivat seisu.
 *
 * KÕRVALMÕJU ON TEADLIK: lukustav update puudutab `@updatedAt`-i, seega lapse
 * lisamine tõstab juhtumi juhtumiloendis ettepoole. See on aus — juhtum
 * MUUTUS. Säilituskella see EI liiguta: L7 järgi käib kell päris üleminekust
 * `ARCHIVED`-isse, mitte `updatedAt`-ist, ja just seepärast on see ohutu.
 *
 * VEAD JÄÄVAD ERISTATAVAKS: võõras või olematu juhtum annab 404, oma aga
 * kirjutuskaitstud juhtum 409. Lisapäring tehakse AINULT ebaõnnestumisel.
 *
 * @param {object} scope
 * @param {(tx: object) => Promise<any>} write kirjutus, mis saab TEHINGU kliendi
 */
export async function withActiveCaseLock({ ownerUserId, caseWorkAssistId, db = prismaClient } = {}, write) {
  const owner = normalizeId(ownerUserId);
  const caseId = normalizeId(caseWorkAssistId);
  if (!owner || !caseId) throw notFound();
  if (typeof write !== "function") throw new TypeError("withActiveCaseLock: write is not a function");

  return db.$transaction(async (tx) => {
    const locked = await tx.caseWorkAssist.updateMany({
      where: { id: caseId, ownerUserId: owner, retentionState: RETENTION_STATE.ACTIVE },
      /* Väärtus on sama, mis ta juba on — meid huvitab LUKK, mitte muutus.
         Nulliga `data` jätaks Prisma teostuse otsustada; siin on ta välja
         öeldud, sest sellest sõltub kogu garantii. */
      data: { retentionState: RETENTION_STATE.ACTIVE }
    });

    if (!locked?.count) {
      const visible = await tx.caseWorkAssist.findFirst({
        where: { id: caseId, ownerUserId: owner },
        select: { id: true }
      });
      throw visible ? conflict("casework.errors.not_active") : notFound();
    }
...
export async function eraseCaseClientReference({
  ...
  return eraseCaseClientReferenceInternal({
    caseId,
    ownerUserId: owner,
    actorUserId,
    actorKind,
...
/** Privileged account-deletion path. Never export this owner-free primitive. */
async function eraseCaseClientReferenceInternal({
  caseId,
  ownerUserId,
  actorUserId,
  actorKind,
  ...
  const current = await db.caseWorkAssist.findFirst({
    where: { id: caseId, ...(ownerUserId ? { ownerUserId } : {}) },
  ...
  const changed = await db.$transaction(async (tx) => {
    const result = await tx.caseWorkAssist.updateMany({
      ...
      where: { id: caseId, ...(ownerUserId ? { ownerUserId } : {}), clientErasedAt: null },
...
export async function eraseCaseWorkClientReferences({ userId, db = prismaClient } = {}) {
  ...
  for (const row of rows) {
    const result = await eraseCaseClientReferenceInternal({
      caseId: row.id,
      ownerUserId: null,
      actorUserId: null,
      actorKind: "SYSTEM",
