 ...
 *
 * Leping: docs/platvormi arendus/juhtum-v1-arendusleping.md (v7), etapp E3.
 *
 ...
 *
 * AINULT LUGEMISRAJALE. Kirjutus ei tohi seisu kontrollida eraldi päringuga —
 * `transitionRetention()` mahub kontrolli ja kirjutuse vahele. Kirjutusrada
 * käib `withActiveCaseLock()` kaudu, kus tingimus on kirjutuse enda sees.
 */
async function requireOwnedCase({ db, ownerUserId, caseWorkAssistId }) {
...
export async function linkCaseWorkItem({
  ...
  /* L4: kontrollime sihtobjekti OMANDIT, mitte olemasolu. Võõras objekt annab
     404 — sama vastus mis olematu, muidu lekiks ID olemasolu. Siht ei ole
     juhtumi laps ja tema kontroll ei kuulu luku sisse. */
  ...
  try {
    const row = await withActiveCaseLock({ ownerUserId: owner, caseWorkAssistId: caseId, db }, (tx) =>
      tx.caseWorkItem.create({
...
export async function unlinkCaseWorkItem({ ownerUserId, caseWorkAssistId, itemId, db = prismaClient } = {}) {
  ...
  return withActiveCaseLock({ ownerUserId: owner, caseWorkAssistId: caseId, db }, async (tx) => {
    ...
    const result = await tx.caseWorkItem.deleteMany({
 ...
 *
 * JA SAMA OMANIKUPIIRI (v5 parandus). Enne seda kandis loendur AINULT
 * `visibleItemWhere()`-i — see piirab SIHTOBJEKTE, mitte juhtumit ennast.
 * Võõra `caseWorkAssistId`-ga kutse ei kukkunud, vaid arvutas: „mitu SINU
 * objekti on seotud TEMA juhtumiga". Tavaandmetel on vastus 0 ja seepärast
 * ei olnud see kasutatav IDOR — aga:
 *
 *   1. **garantii ei tohi sõltuda andmete kujust.** Jagatav või üleantav siht,
 *      omanikuvahetus või erandlik rida teeb sellest mitte-nulli, ja siis
 *      kinnitab loendur võõra juhtumi olemasolu;
 *   2. **kaks sama mooduli lugejat kandsid ERI garantiid** — `listCaseWorkItems()`
 *      kutsub `requireOwnedCase()`, loendur ei kutsunud. Sama marsruut
 *      (`app/api/casework/cases/[caseId]/route.js`) kutsub mõlemat ja
 *      `countOpenMissingInfo()` kannab oma kontrolli juba täna. Asümmeetria
 *      lugemisradade vahel on täpselt see koht, kust järgmine leke tuleb.
 *
 * Marsruut oli päästetud ainult sellest, et `getCaseWorkAssist()` jookseb tema
 * ees ja viskab võõra juhtumi peal 404. **Kutsuja kaudu saadud kaitse ei ole
 * lugeja garantii** — järgmine kutsuja ei pea seda järjekorda teadma.
 */
