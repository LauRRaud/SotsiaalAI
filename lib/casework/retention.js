...
import { createNotificationEvent, notificationDedupeKey, NOTIFICATION_EVENT_TYPES } from "@/lib/notifications";
 ...
 */
async function readArchivedTransitionPage({ db, cutoffFrom, cutoffUntil, limit, skip = 0 }) {
  ...
  return { alive, rawCount: rows.length };
}

async function findArchivedTransitions({ db, cutoffFrom, cutoffUntil, limit }) {
  const { alive } = await readArchivedTransitionPage({ db, cutoffFrom, cutoffUntil, limit });
  return alive;
}

/**
 * Mitu lehte tohib üks käivitus juba hoiatatud ridade tagant läbi vaadata.
 * Ülempiir on olemas selleks, et üks käivitus ei skaneeriks kogu hoiatusakent
 * lõputult; edasi jõuab järgmine käivitus.
 */
const WARNING_SCAN_PAGES = 20;

/**
 * Juba hoiatatud juhtumid välja (SOL-CW-07).
 *
 * Küsitakse TÄPSELT sama `dedupeKey` järgi, mille `sendRetentionWarning()`
 * hiljem kirjutab — võtme kuju tuleb teavituskihist, mitte siit.
 */
async function withoutAlreadyWarned({ db, rows }) {
  if (!rows.length) return rows;

  const keys = rows.map((row) =>
    notificationDedupeKey({
      type: NOTIFICATION_EVENT_TYPES.CASE_WORK_RETENTION_WARNING,
      sourceId: row.caseWorkAssistId,
  ...
  const warned = new Set(existing.map((row) => row.dedupeKey));
  return rows.filter((row, index) => !warned.has(keys[index]));
}
 ...
 *
 * JUBA HOIATATUD READ EI TOHI BATCH'I TÄITA (SOL-CW-07).
 *
 * Varem rakendati `take` piir ENNE, kui keegi küsis, kas hoiatus on juba
 * saadetud — deduplikatsioon toimus alles `sendRetentionWarning()` sees. Kui
 * hoiatusaknas oli rohkem juhtumeid kui batch'i suurus, tõid kõik käivitused
 * järjest samad vanimad read ja uuemad juhtumid ei saanud hoiatust enne, kui
 * vanad kustutusaknasse liikusid. Ükski veateade ei tekkinud — töö raporteeris
 * ausalt „0 hoiatust", sest kõik olid duplikaadid.
 *
 * Nüüd loetakse lehthaaval edasi, kuni `limit` HOIATAMATA rida on koos või
 * allikas saab otsa.
 */
export async function findCasesDueForWarning({ now = new Date(), limit = DEFAULT_BATCH, db = prismaClient } = {}) {
  ...
  const target = normalizeBatch(limit);
  const collected = [];

  for (let page = 0; page < WARNING_SCAN_PAGES && collected.length < target; page += 1) {
    const { alive, rawCount } = await readArchivedTransitionPage({
      db,
      cutoffFrom: deletionCutoff,
      cutoffUntil: warningCutoff,
      limit: target,
      skip: page * target
    });

    for (const row of await withoutAlreadyWarned({ db, rows: alive })) {
      collected.push(row);
      if (collected.length >= target) break;
    }

    /* ALLIKAS ON OTSAS, kui leht tuli täiest väiksem. `alive` pikkus ei kõlba
       selleks otsuseks: temast on juba kustutatud juhtumid välja filtreeritud
       ja lühem `alive` tähendaks ekslikult „rohkem ei ole". */
    if (rawCount < target) break;
  }
 ...
 *
 * ULATUS ON KOGU ETTEVALMISTAV TÖÖMATERJAL (O-JTA-6, omaniku otsus 08.08):
 * kandmata mustandid JA kohtumise ettevalmistused. Esimene kuju katsid ainult
 * mustandid — see oli lepingu sõnastus tähttäheline, aga jättis katmata otsuse
 * enda motiveeriva näite („kaks aastat vana kohtumise ettevalmistus"). Tegu,
 * mis ei kata seda, mida ta lubab, on halvem kui puuduv tegu.
 *
 * KUMMASKI EI KUSTU RIDA, KUSTUB SISU. Mustandil jääb rida koos ülekande
 * tõendiga; ettevalmistusel jääb konteiner koos oma seostega — märge, mis sellele
 * ettevalmistusele viitab, ei tohi kaotada infot, et ettevalmistus üldse toimus.
 * Terve rea kustutamine annaks kahele objektile vastuolulise elutsükli.
 *
 * MÄRGE ON VÄLJAS ja see ei ole väljajätt: E4 ütleb, et märget ei kustutata —
 * ettevalmistus on tulevikuplaan, märge kirjeldab seda, mis JUBA juhtus.
 *
...
export async function archiveWorkingMaterial({
  ...
  return withActiveCaseLock({ ownerUserId: owner, caseWorkAssistId: caseId, db }, async (tx) => {
    const [drafts, preps] = await Promise.all([
      tx.caseWorkDraft.findMany({
        where: {
      ...
      tx.caseWorkMeetingPrep.findMany({
        where: { caseWorkAssistId: caseId, contentPurgedAt: null },
        select: { id: true }
      })
    ]);

    /* TÜHI TEGU ANNAB 409, MITTE VAIKSE „ok". Nupp, mis vajutamise peale midagi
       ei tee ja seda ei ütle, õpetab kasutajat arvama, et ta juba vajutas. */
    if (!drafts.length && !preps.length) throw conflict("casework.errors.working_material_empty");

    const purgeMark = {
      contentPurgedAt: now,
      contentPurgeReason: PURGE_REASON.WORKER_ARCHIVED_WORKING_MATERIAL
    };
    ...
    for (const draft of drafts) {
      ...
      await tx.caseWorkDraft.updateMany({ where: { id: draft.id, contentPurgedAt: null }, data: purgeMark });
    }

    let prepFields = 0;
    let questions = 0;
    for (const prep of preps) {
      const removedFields = await tx.caseWorkMeetingPrepField.deleteMany({ where: { meetingPrepId: prep.id } });
      const removedQuestions = await tx.caseWorkQuestion.deleteMany({ where: { meetingPrepId: prep.id } });
      prepFields += removedFields?.count || 0;
      questions += removedQuestions?.count || 0;
      /* KONTEINER JÄÄB koos oma seostega (O-JTA-6). Märge, mis sellele
         ettevalmistusele viitab, ei tohi kaotada infot, et ettevalmistus oli. */
      await tx.caseWorkMeetingPrep.updateMany({ where: { id: prep.id, contentPurgedAt: null }, data: purgeMark });
    }

    return { drafts: drafts.length, fields, preps: preps.length, prepFields, questions };
  });
}
