 ...
 *
 * Leping: docs/platvormi arendus/juhtum-v1-arendusleping.md (v7), etapp E4.
 *
...
import { badRequest, featureDisabled, notFound } from "./errors.js";
...
/* L14 KIRJUTUSKEELD laieneb LASTELE, ja teda jõustab `withActiveCaseLock()`
   (`caseWorkAssist.js`) — KIRJUTUSE SEES, mitte enne teda. Eraldi eelkontroll
   siin oli võistlus: `transitionRetention()` mahtus kontrolli ja kirjutuse
   vahele ning `READ_ONLY` juhtumi loend muutus ikkagi. */
...
export async function setMissingInfoStatus({
  ...
  return withActiveCaseLock({ ownerUserId: owner, caseWorkAssistId: caseId, db }, async (tx) => {
    const result = await tx.caseWorkMissingInfo.updateMany({
    ...
    return tx.caseWorkMissingInfo.findFirst({ where: { id, caseWorkAssistId: caseId }, select: ITEM_SELECT });
  });
}
...
export async function removeMissingInfo({ ownerUserId, caseWorkAssistId, itemId, db = prismaClient } = {}) {
  ...
  return withActiveCaseLock({ ownerUserId: owner, caseWorkAssistId: caseId, db }, async (tx) => {
    const result = await tx.caseWorkMissingInfo.deleteMany({ where: { id, caseWorkAssistId: caseId } });
    if (!result?.count) throw notFound();
    return { ok: true };
  });
}
