...
/**
 * Eesti kalendripäeva piirid (leping L12: #2 „tänased" ja #6 „eesootavad" ei
 * kattu, ja piir on KALENDRIPÄEV, mitte UTC-hetk).
 *
 * MIKS MITTE UTC: suveajal on Eesti UTC+3. Kell 01:30 Eesti aja järgi on UTC
 * eelmine päev, seega UTC-põhine „täna" jätaks varahommikuse kontakti eilsesse
 * ja töötaja ei näeks teda oma tänases loendis. Sama kuupäevareegel, mis A4
 * lubade kehtivusel — seal õpiti see kalli hinnaga.
 */
export function estonianDayBounds(now = new Date()) {
  ...
  /* Nihe leitakse MÕÕTES, mitte konstandiga: Eesti on talvel UTC+2 ja suvel
     UTC+3, ja kõva +2 annaks pool aastat vale piiri. */
  const asUtcMidnight = new Date(`${isoDay}T00:00:00Z`);
  const offsetMs = asUtcMidnight.getTime() - new Date(`${isoDay}T00:00:00`).getTime();
  const localMidnightUtc = new Date(asUtcMidnight.getTime() - offsetMs);

  return {
    isoDay,
    start: localMidnightUtc,
    end: new Date(localMidnightUtc.getTime() + 24 * 60 * 60 * 1000)
  };
}
 ...
 */
async function withDeadline(load, deadlineMs) {
  let timer = null;
  ...
  try {
    ...
    /* Veateadet EI panda vastusesse: teenuskihi erind võib kanda päringu või
       kirje sisu. Kasutaja näeb L2 üldist teksti, põhjus läheb logisse. */
    console.error("[casework/workbench] section failed", result.error?.message || result.error);
...
export async function getCaseWorkbench({
  ...
  const loaders = {
    ...
    activePreparations: async () => {
      ...
      const rows = items.map((row) => ({ ...row, openMissingInfoCount: counts.get(row.id) || 0 }));
    ...
    networkPreparation: async () => {
      ...
      return settled(await listWorkerActionableShares({ prisma: db, workerId: owner, limit: LIST_LIMIT }));
    },

    practiceReflection: async () => settled(await listPracticeReflectionWorkspaces(owner, { db })),

    covisionPreparation: async () => settled(await listTopicSeeds(owner, { db }))
  };

  const keys = WORKBENCH_SECTIONS;
  const results = await Promise.all(keys.map((key) => withDeadline(loaders[key], deadlineMs)));
