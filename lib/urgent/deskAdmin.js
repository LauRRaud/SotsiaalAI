...
const DESK_STAFF_ROLES = new Set(["SOCIAL_WORKER", "SERVICE_PROVIDER"]);

async function requireEligibleDeskStaff(tx, desk, rawUserId) {
  const userId = cleanText(rawUserId, { max: 60 });
  if (!userId) return null;

  const user = await tx.user.findFirst({ where: { id: userId } });
  if (!user) fail("urgent_desk.member_not_a_user");
  if (!DESK_STAFF_ROLES.has(String(user.role || "").toUpperCase()) || user.accessSuspendedAt) {
    fail("urgent_desk.member_not_a_user");
  }

  if (desk.recipientType === "KOV_CONTACT") {
    const membership = await tx.organizationMembership.findFirst({
      where: {
        userId,
        status: "ACTIVE",
        organization: { status: "ACTIVE", municipalityId: desk.municipalityId }
      }
    });
    if (!membership) fail("urgent_desk.member_not_a_user");
  } else {
    const entry = desk.serviceEntryId
      ? await tx.serviceMapEntry.findFirst({ where: { id: desk.serviceEntryId } })
      : null;
    const profile = entry?.providerProfileId
      ? await tx.serviceProviderProfile.findFirst({ where: { id: entry.providerProfileId } })
      : null;
    const ownsSoloProfile = profile?.ownershipMode === "SOLO" && profile.ownerId === userId;
    const belongsToProvider = profile?.ownershipMode === "ORGANIZATION" && profile.organizationId
      ...
      : null;
    if (!ownsSoloProfile && !belongsToProvider) fail("urgent_desk.member_not_a_user");
  }
  return userId;
}
...
export async function createUrgentDesk({ prisma, municipalityId, recipientType = "KOV_CONTACT", data = {}, actorUserId, now = () => new Date() }) {
  ...
  return prisma.$transaction(async (tx) => {
    const ownerUserId = await requireEligibleDeskStaff(tx, {
...
export async function updateUrgentDesk({ prisma, deskId, data = {}, actorUserId, now = () => new Date() }) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const at = now();
    const ownerUserId = data.ownerUserId === undefined
      ? desk.ownerUserId
      : await requireEligibleDeskStaff(tx, {
          ...desk,
          serviceEntryId: data.serviceEntryId === undefined
            ? desk.serviceEntryId
            : cleanText(data.serviceEntryId, { max: 60 }) || null
        }, data.ownerUserId);
    ...
    await recordDeskAudit(tx, {
      ...
      meta: {
        ...
        changedFields: [...VERIFIED_CONDITION_FIELDS, "ownerUserId", "serviceEntryId"].filter((field) => {
          const nextValue = field === "ownerUserId"
            ? ownerUserId
            : field === "serviceEntryId"
              ? (data.serviceEntryId === undefined ? desk.serviceEntryId : cleanText(data.serviceEntryId, { max: 60 }) || null)
              : conditions?.[field];
          return String(desk?.[field] ?? "") !== String(nextValue ?? "");
        })
      }
    });
    return updated;
...
export async function addUrgentDeskMember({ prisma, deskId, userId, actorUserId }) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await requireEligibleDeskStaff(tx, desk, member);
