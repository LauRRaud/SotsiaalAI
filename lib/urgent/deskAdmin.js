...
import { adminDeskProjection, deskReadiness } from "@/lib/urgent/desk";
...
export async function addUrgentDeskMember({ prisma, deskId, userId }) {
  ...
  const existing = await prisma.urgentDeskMember.findFirst({ where: { deskId: desk.id, userId: member } });
  if (existing) {
    return prisma.urgentDeskMember.update({ where: { id: existing.id }, data: { isActive: true } });
  }
  return prisma.urgentDeskMember.create({ data: { deskId: desk.id, userId: member, isActive: true } });
}
...
export async function removeUrgentDeskMember({ prisma, deskId, userId }) {
  const existing = await prisma.urgentDeskMember.findFirst({
    where: { deskId: String(deskId || ""), userId: String(userId || "") }
  });
  if (!existing) fail("urgent_desk.member_not_found");
  return prisma.urgentDeskMember.update({ where: { id: existing.id }, data: { isActive: false } });
}
