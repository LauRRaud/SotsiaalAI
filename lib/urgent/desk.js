...
/**
 * LAUA RIDA ON VALMIDUSE MUTEX (SOL-URG-08, SOL-URG-09).
 *
 * Valmidus koosneb kahest allikast — laua enda väljadest ja aktiivsete liikmete
 * arvust — ja neid loeti varem eraldi päringutega ILMA luguta. Admin, kes sulges
 * laua või eemaldas viimase mehitaja täpselt kontrolli ja kirjutuse vahel, jättis
 * inimesele eduka pöördumise lauale, mis seda lubadust enam ei kandnud.
 *
 * `FOR UPDATE` laua rea peal on mõlema poole KOKKULEPE: nii pöördumise loomine
 * kui iga valmidust muutev adminitoiming võtavad sama luku. Ainult ühel pool
 * lukustamine ei ole lukk — teine pool sõidaks lihtsalt mööda.
 *
 * Lukk on tehingu oma ja vabaneb commit'iga; eraldi vabastamist ei ole.
 */
export async function lockDeskRow(tx, deskId) {
  const id = String(deskId || "");
  if (!id) return false;
  if (typeof tx?.$queryRaw !== "function") {
    /* Klient ilma toorpäringuta ei saa lukustada. See on VIGA, mitte haru:
       vaikne möödaminek tähendaks, et lukk kaob esimese refaktori peale ja
       keegi ei märkaks. */
    const error = new Error("Desk readiness lock requires a raw-capable client");
    error.code = "URGENT_DESK_LOCK_UNAVAILABLE";
    throw error;
  }
  await tx.$queryRaw`SELECT "id" FROM "UrgentDesk" WHERE "id" = ${id} FOR UPDATE`;
  return true;
}
...
export function adminDeskProjection(desk, { now = new Date(), activeMemberCount = null } = {}) {
  ...
  return {
    ...
    /* SOL-URG-12: admini vaates on kinnitus KOLM asja — millal, kes ja millise
       tekstiversiooni. Ainult aeg oli see, mis tegi „partner kinnitas" väiteks
       ilma tõendita. */
    lastVerifiedByUserId: desk.lastVerifiedByUserId || null,
    verifiedConditionsHash: desk.verifiedConditionsHash || null,
