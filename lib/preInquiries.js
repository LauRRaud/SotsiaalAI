...
export async function sendExternalPreInquiry(userId, inquiryId, { db = prisma, mailer = null } = {}) {
  const existing = await getVisiblePreInquiry(userId, inquiryId, { db });
  if (!existing) {
    const error = new Error("api.common.not_found");
    error.status = 404;
  ...
  if (existing.recipientEntry?.type === "SERVICE_PROVIDER" && (!existing.recipientServiceId || !existing.recipientLocationId)) {
    const error = new Error("pre_inquiries.errors.recipient_channel_changed");
    error.status = 409;
    throw error;
  }
  if (existing.recipientEntryId) {
    const currentRecipient = await resolveRecipient({
      recipientEntryId: existing.recipientEntryId,
      recipientServiceId: existing.recipientServiceId,
      recipientLocationId: existing.recipientLocationId
    }, { db, validateClientIdentity: false });
    const unchanged = currentRecipient.recipientType === existing.recipientType
      && currentRecipient.selectedRecipientName === existing.selectedRecipientName
      && currentRecipient.selectedRecipientEmail === existing.selectedRecipientEmail
      && currentRecipient.deliveryChannel === existing.deliveryChannel;
    if (!unchanged) throw preInquiryError("pre_inquiries.errors.recipient_channel_changed", 409);
  }
  if (existing.recipientServiceId || existing.recipientLocationId) {
    const currentPolicy = await loadCurrentProviderPolicy(db, existing);
    if (!currentPolicy?.emailAllowed || !currentPolicy.email || currentPolicy.email !== existing.selectedRecipientEmail) {
      const error = new Error("pre_inquiries.errors.recipient_channel_changed");
      error.status = 409;
      throw error;
    }
  ...
  await (mailer || getMailer("pre-inquiries")).sendMail({
  ...
  const now = new Date();
  const inquiry = await db.preInquiry.update({
    where: { id: existing.id },
    data: {
      status: "SENT",
      sentAt: now,
