...
/* T25 viil B: organisatsiooni vastuvõtulaud. Ainult KOLM funktsiooni — värav,
   kohaletoimetamine ja tagasivõtmine. Eelpöördumise moodul EI TEA org-kihi
   sisemisest skoobi- ega õigusloogikast midagi ja ei tohi teadagi. */
...
import {
  ...
  assertPreInquiryAssessmentLimits,
...
import { resolveSelectedServicePolicy } from "@/lib/serviceMap/contactPolicy";
import { buildPublicServiceMapContactEligibilityClause } from "@/lib/serviceMap/contactFreshnessProjection";
import { parseServiceMapEntryId } from "@/lib/serviceMap/entryIdentity";
...
// Every author-owned status mutation is declared here and enforced below.
// ARCHIVED and SENT have no ordinary PATCH exits; reopening is deliberately a
// separate operation with its own single transition.
export const PRE_INQUIRY_AUTHOR_STATUS_TRANSITIONS = Object.freeze({
  PATCH: Object.freeze({
    DRAFT: Object.freeze(["DRAFT", "READY", "SENT", "ARCHIVED"]),
    READY: Object.freeze(["DRAFT", "READY", "SENT", "ARCHIVED"]),
    DOWNLOADED: Object.freeze(["DOWNLOADED", "READY", "SENT", "ARCHIVED"]),
    SENT: Object.freeze([]),
    ARCHIVED: Object.freeze([])
  }),
  REOPEN: Object.freeze({
    ARCHIVED: Object.freeze(["READY"])
  })
});

const MAX_SHORT_TEXT_LENGTH = 1_000;
const MAX_TEXT_LENGTH = 12_000;
export const PRE_INQUIRY_ACTIVE_DRAFT_LIMIT = 250;

const INPUT_LIMITS = Object.freeze({
  topic: MAX_SHORT_TEXT_LENGTH,
  situation: MAX_TEXT_LENGTH,
  generatedDraft: MAX_TEXT_LENGTH,
  userEditedDraft: MAX_TEXT_LENGTH,
  correctionText: MAX_TEXT_LENGTH,
  assistantMessage: MAX_TEXT_LENGTH
});
const CLIENT_ACTION_ID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu;

function normalizeClientActionId(value) {
  const normalized = String(value || "").trim().toLowerCase();
  if (!normalized) return null;
  if (!CLIENT_ACTION_ID_RE.test(normalized)) throw preInquiryError("pre_inquiries.errors.invalid_action_key", 400);
  return normalized;
}

function preInquiryActionHash(value) {
...
export function assertPreInquiryInputLimits(input = {}) {
  for (const [field, maxLength] of Object.entries(INPUT_LIMITS)) {
    if (!Object.prototype.hasOwnProperty.call(input, field) || input[field] == null) continue;
    const value = String(input[field]).replace(/\r\n/g, "\n").trim();
    if (value.length > maxLength) {
      const key = field.replace(/[A-Z]/g, (letter) => `_${letter.toLowerCase()}`);
      throw preInquiryError(`pre_inquiries.errors.${key}_too_long`, 413);
    }
  }
  if (hasAssessmentState(input.assessmentState)) assertPreInquiryAssessmentLimits(input.assessmentState);
}
...
const preInquiryInclude = {
  ...
  /* T25 viil B: AINULT avalik nimi ja liik. Mitte liikmed, mitte üksused,
     mitte moodulid — saatjale ei kuulu ükski org-sisene fakt. */
...
// List endpoint projection: deliberately enumerate the fields the current
// cards/actions use. Newly added database columns (including action hashes) do
// not become public merely because they exist; the ID detail reader remains the
// authoritative expansion point.
const PRE_INQUIRY_LIST_SELECT = Object.freeze({
  id: true,
  authorId: true,
  authorErasedAt: true,
  ...
  recalledAt: true,
  supersededById: true,
  externalSendConfirmedAt: true,
  createdAt: true,
  updatedAt: true,
  recipientEntry: true,
  author: { select: { id: true, email: true, role: true } },
  recipientOwner: { select: { id: true, email: true, role: true } },
  recipientOrganization: { select: { id: true, displayName: true, legalKind: true } }
});

function normalizeText(value, _maxLength = MAX_SHORT_TEXT_LENGTH) {
...
// A provider-location marker has a UI-only id of
// `<serviceMapEntryId>:location:<locationId>`. Persist only the authoritative
// base entry, so an INTERNAL provider route cannot silently fall back to email.
export function normalizeServiceMapRecipientEntryId(value) {
  return parseServiceMapEntryId(value).baseEntryId;
}
...
function inferRecipientType(entry, inputType) {
  if (entry) return entry.type === "SERVICE_PROVIDER" ? "SERVICE_PROVIDER" : "KOV_CONTACT";
...
function scoreEntry(entry, preferredType, routingEvidence = {}) {
  let score = 0;
  if (preferredType && entry.type === preferredType) score += 2;
  if (routingEvidence.regionMatch) score += 5;
  if (routingEvidence.needMatch) score += 4;
  if (routingEvidence.channelMatch) score += 1;
  return score;
}
...
export async function listPreInquiryOrganizationRecipients({ db = prisma } = {}) {
  if (!isOrgInboxEnabled()) return [];

  const organizations = await db.organization.findMany({
    where: {
      status: "ACTIVE",
      modules: { some: { moduleKey: "KOV_INTAKE", status: "ACTIVE" } }
    },
    select: {
      id: true,
      displayName: true,
      legalKind: true,
      municipality: { select: { displayName: true, county: true } }
    },
    orderBy: [{ displayName: "asc" }, { id: "asc" }]
  });

  return organizations.map((organization) => ({
    id: `organization-inbox:${organization.id}`,
    recipientOrganizationId: organization.id,
    type: "ORGANIZATION_INBOX",
    title: organization.displayName,
    legalKind: organization.legalKind,
    municipalityName: organization.municipality?.displayName || null,
    county: organization.municipality?.county || null,
    deliveryChannel: "INTERNAL"
  }));
}

async function resolveRecipient(input = {}, { db = prisma, validateClientIdentity = true } = {}) {
  const recipientEntryId = normalizeServiceMapRecipientEntryId(input.recipientEntryId);
  ...
  if (recipientEntryId) {
    const publicContactEligibility = await buildPublicServiceMapContactEligibilityClause(db);
    const findEntry = db.serviceMapEntry.findFirst?.bind(db.serviceMapEntry)
      || db.serviceMapEntry.findUnique.bind(db.serviceMapEntry);
    recipientEntry = await findEntry({
      where: {
        id: recipientEntryId,
        status: "PUBLISHED",
        AND: [publicContactEligibility]
      },
      include: {
        providerProfile: {
          select: {
            ...
            serviceItems: {
              where: { mapVisible: true, status: "PUBLISHED" },
              select: {
                id: true,
                email: true,
                phone: true,
            ...
            serviceLocations: {
              ...
              select: {
                ...
                email: true,
                phone: true,
                serviceLinks: { select: { providerServiceId: true } }
              }
            }
          }
        }
    ...
    if (!recipientEntry) throw preInquiryError("api.common.not_found", 404);
  }

  const recipientType = inferRecipientType(recipientEntry, input.recipientType);
  const authoritativeRecipientName = recipientEntry?.title || recipientEntry?.providerProfile?.organizationName || null;
  const selectedRecipientName = recipientEntry ? authoritativeRecipientName : explicitRecipientName;
  const requestedRecipientType = normalizeEnum(input.recipientType, PRE_INQUIRY_RECIPIENT_TYPES, "");
  if (validateClientIdentity && recipientEntry && requestedRecipientType && requestedRecipientType !== recipientType) {
    throw preInquiryError("api.common.invalid_request", 400);
  }
  if (validateClientIdentity && recipientEntry && explicitRecipientName && explicitRecipientName !== authoritativeRecipientName) {
    throw preInquiryError("api.common.invalid_request", 400);
  }
  const requestedServiceId = normalizeText(input.recipientServiceId, 128);
  const requestedLocationId = normalizeText(input.recipientLocationId, 128);
  const servicePolicy = requestedServiceId || requestedLocationId
    ? resolveSelectedServicePolicy({ entry: recipientEntry, serviceId: requestedServiceId, locationId: requestedLocationId })
    : null;
  if ((requestedServiceId || requestedLocationId) && !servicePolicy) {
    const error = new Error("pre_inquiries.errors.invalid_service_selection");
    error.status = 409;
    throw error;
  }
  if (recipientEntry?.type === "SERVICE_PROVIDER" && recipientEntry.providerProfile?.serviceItems?.length && !servicePolicy) {
    const error = new Error("pre_inquiries.errors.service_selection_required");
    error.status = 409;
    throw error;
  }
  const recipientEmail = recipientEntry?.type === "SERVICE_PROVIDER"
    ? (servicePolicy?.email || null)
    : (recipientEntry ? (recipientEntry.email || null) : selectedRecipientEmail);
  if (validateClientIdentity && recipientEntry && selectedRecipientEmail && selectedRecipientEmail !== recipientEmail) {
    throw preInquiryError("api.common.invalid_request", 400);
  }
  ...
  const providerAcceptsPlatform =
    ...
    (servicePolicy ? servicePolicy.platformAllowed : recipientEntry?.providerProfile?.acceptsPlatformPreInquiries !== false);
  const matchedUserAcceptsPlatform = Boolean(
    matchedRecipientOwner?.acceptsPreInquiries &&
    (recipientEntry?.type !== "SERVICE_PROVIDER" || providerAcceptsPlatform)
  );

  /* T25 viil B — ORGANISATSIOONI VASTUVÕTULAUD adressaadina.
     Kolm väravat, kõik kohustuslikud (arenduskava §5.7, §6):
       1. organisatsioon on ACTIVE — mustandile ei saa saata;
       2. `KOV_INTAKE` moodul on aktiivne — moodulita ei ole postkasti;
       3. globaalne `ORG_INBOX_ENABLED` värav on lahti.
     Ükski neist ei ole kliendi öelda: adressaadi kõlblikkust tõendab server. */
  const requestedOrganizationId = normalizeText(input.recipientOrganizationId, 64);
  if (requestedOrganizationId && recipientEntryId) {
    throw preInquiryError("api.common.invalid_request", 400);
  }
  let recipientOrganization = null;
  if (requestedOrganizationId && isOrgInboxEnabled()) {
    const organization = await db.organization.findFirst({
      ...
      select: { id: true, displayName: true, legalKind: true }
    });
    recipientOrganization = organization || null;
  }
  if (requestedOrganizationId && !recipientOrganization) {
    throw preInquiryError("pre_inquiries.errors.recipient_channel_changed", 409);
  }
  if (validateClientIdentity && requestedOrganizationId && requestedRecipientType && requestedRecipientType !== "ORGANIZATION_INBOX") {
    throw preInquiryError("api.common.invalid_request", 400);
  }
  const recipientOrganizationId = recipientOrganization?.id || null;
  const finalSelectedRecipientName = recipientOrganization?.displayName || selectedRecipientName;

  /* Organisatsioon on INTERNAL adressaat ILMA `recipientOwnerId`-ta: tema
     postkast ei kuulu ühelegi inimesele. Just seepärast ei tekita org-saadetis
     ühtegi isiklikku vastuvõtjat ega ilmu kellegi isiklikku loendisse — teda
     loeb `lib/org/inbox.js` oma skoobi- ja projektsioonireeglitega. */
  /* NB `resolveProviderRecipientUserId` tagastab ORGANIZATION-režiimis ALATI
     null-i: seal on `ownerId` ainult päritolu (vt lib/org/profileRecipient.js).
     Varem sattus org-profiilile saadetud pöördumine profiili endise LOOJA
     isiklikku loendisse — ka siis, kui ta oli organisatsioonist lahkunud.
     Org-profiil jõuab platvormisiseselt kohale ainult siis, kui klient valis
     sõnaselgelt organisatsiooni vastuvõtulaua (`recipientOrganizationId`);
     muidu jääb alles avaliku aadressi rada, mille organisatsioon ise avaldas. */
  const providerRecipientUserId = resolveProviderRecipientUserId(recipientEntry?.providerProfile);
  const recipientOwnerId = recipientOrganizationId
    ? null
    : providerAcceptsPlatform && providerRecipientUserId
      ? providerRecipientUserId
      : matchedUserAcceptsPlatform
        ? matchedRecipientOwner.id
        : null;
  const deliveryChannel = recipientOrganizationId || recipientOwnerId ? "INTERNAL" : "EXTERNAL_EMAIL";

  return {
    recipientEntry,
    recipientType: recipientOrganizationId ? "ORGANIZATION_INBOX" : recipientType,
    selectedRecipientEmail: recipientOrganizationId ? null : recipientEmail,
    selectedRecipientName: finalSelectedRecipientName,
    recipientServiceId: servicePolicy?.providerServiceId || null,
    recipientLocationId: servicePolicy?.providerLocationId || null,
    recipientOwnerId,
...
/**
 * Kas see salvestus tuleb organisatsiooni postkasti toimetada?
 *
 * Sama kuju kui `shouldSendInternalArrival` — tahtlikult: mõlemad on
 * „üleminek SENT-i, üks kord" otsused ja neid loetakse kõrvuti.
 */
export function shouldDeliverToOrganization({ previousStatus, nextStatus, recipientOrganizationId }) {
  return Boolean(recipientOrganizationId && nextStatus === "SENT" && previousStatus !== "SENT");
}
...
export function serializePreInquiry(inquiry, { viewerId = null } = {}) {
  ...
  const serialized = {
    ...
    authorErasedAt: inquiry.authorErasedAt || null,
    recipientOwnerId: inquiry.recipientOwnerId,
    recipientEntryId: inquiry.recipientEntryId,
    recipientServiceId: inquiry.recipientServiceId || null,
    recipientLocationId: inquiry.recipientLocationId || null,
    /* T25 viil B (§5.7): pöörduja peab nägema adressaati kujul „organisatsiooni
       vastuvõtutiim" — mitte tühja välja ega inimese nime. `recipientType`
       ütleb, et tegemist on org-postkastiga, ja `recipientOrganization` kannab
       AINULT organisatsiooni avalikku nime.
       Mida siin EI OLE: liikmete arvu, koordinaatori nime, üksuse nime ega
       ühtegi org-sisest fakti. Saatja näeb ust, mitte selle taga olevat tuba. */
    recipientOrganizationId: inquiry.recipientOrganizationId || null,
    recipientOrganization: inquiry.recipientOrganization
      ? {
          id: inquiry.recipientOrganization.id,
          displayName: inquiry.recipientOrganization.displayName,
          legalKind: inquiry.recipientOrganization.legalKind
        }
      : null,
...
/**
 * Adressaadi NÄHTAVUS- JA ELUTSÜKLIFILTER — kanooniline kuju.
 *
 * ÜKS DEFINITSIOON, MITU KASUTAJAT (SOL-CW-11): sama reeglit vajab ka
 * juhtumitöö päritolukontroll (`lib/casework/caseWorkAssist.js`). Kaks koopiat
 * tähendaks, et üks neist jääb muutmisel maha — ja just see üks lubaks luua
 * juhtumi pöördumisest, mille saatja on tagasi võtnud.
 *
 * Kolm tingimust koos: adressaat olen mina · saatja EI OLE tagasi võtnud ·
 * pöördumine on päriselt saadetud (mustandit ei ole mulle antud).
 */
export function recipientVisiblePreInquiryWhere(userId) {
  return {
...
function visiblePreInquiryWhere(userId) {
  return {
    OR: [{ authorId: userId }, recipientVisiblePreInquiryWhere(userId)]
  };
}

export async function listVisiblePreInquiries(userId, { limit = 100, db = prisma } = {}) {
  return (await listVisiblePreInquiryPage(userId, { limit, db })).items;
}

export async function listVisiblePreInquiryPage(userId, { limit = 100, cursor = null, db = prisma } = {}) {
  if (!userId) return { items: [], total: 0, hasMore: false, nextCursor: null };
  const take = normalizePreInquiryPageLimit(limit);
  const decoded = decodePreInquiryCursor(cursor);
  const baseWhere = visiblePreInquiryWhere(userId);
  const cursorWhere = preInquiryCursorWhere(decoded);
  ...
  const [rows, total] = await Promise.all([
    db.preInquiry.findMany({
      ...
      select: PRE_INQUIRY_LIST_SELECT
    }),
    typeof db.preInquiry.count === "function" ? db.preInquiry.count({ where: baseWhere }) : Promise.resolve(null)
  ]);
  const hasMore = rows.length > take;
  const pageRows = hasMore ? rows.slice(0, take) : rows;
  return {
    items: pageRows.map((inquiry) => serializePreInquiry(inquiry, { viewerId: userId })),
    total: total ?? pageRows.length,
    hasMore,
...
function assertAuthorStatusTransition(operation, currentStatus, nextStatus) {
  const allowed = PRE_INQUIRY_AUTHOR_STATUS_TRANSITIONS[operation]?.[currentStatus] || [];
  if (!allowed.includes(nextStatus)) {
    throw preInquiryError("pre_inquiries.errors.invalid_status_transition", 409);
  }
}
...
export async function recallPreInquiry(
  ...
  return withPreInquiryRoomLock(existing.id, async (tx) => {
    ...
    /* T25 viil B: kui pöördumine läks organisatsiooni postkasti, tuleb ka
       postkastikirje tagasi võtta ja elavad määramised sulgeda. SAMAS
       tehingus — vastasel juhul jääks tagasivõetud pöördumine kellegi
       ülesandeks ja koordinaator näeks kirjet, mille sisu enam ei kehti. */
    if (fresh.recipientOrganizationId) {
      await recallInboxItemForSourceWithin(tx, { sourceId: fresh.id }, { now: occurredAt });
    }
...
export async function sendPreInquiryCorrection(
  ...
  assertPreInquiryInputLimits(input);
  const existing = await db.preInquiry.findFirst({
    where: { id: inquiryId, authorId: userId },
    select: { id: true }
  });
  ...
  const outcome = await withPreInquiryRoomLock(existing.id, async (tx) => {
    ...
    /* T25 viil B: adressaat võib olla kas inimene VÕI organisatsiooni
       vastuvõtulaud. Parandusring peab töötama mõlemal juhul — vastasel korral
       jääks org-postkasti saadetud pöördumine ainsaks, mida pöörduja pärast
       avamist parandada ei saa. */
    if (!fresh.recipientOwnerId && !fresh.recipientOrganizationId) {
      throw preInquiryError("pre_inquiries.errors.not_sent", 409);
    }
    if (!fresh.openedAt) {
    ...
    if (fresh.recipientEntry?.type === "SERVICE_PROVIDER") {
      if (!fresh.recipientServiceId || !fresh.recipientLocationId) {
        throw preInquiryError("pre_inquiries.errors.recipient_channel_changed", 409);
      }
      const currentPolicy = await loadCurrentProviderPolicy(tx, fresh);
      if (!currentPolicy?.platformAllowed) {
        throw preInquiryError("pre_inquiries.errors.recipient_channel_changed", 409);
      }
    }
    if (fresh.recipientOrganizationId) {
      const organization = isOrgInboxEnabled()
        ? await tx.organization.findFirst({
          where: {
            id: fresh.recipientOrganizationId,
            status: "ACTIVE",
            modules: { some: { moduleKey: "KOV_INTAKE", status: "ACTIVE" } }
          },
          select: { id: true }
    ...
    const replacement = await tx.preInquiry.create({
      data: {
        ...
        recipientOrganizationId: fresh.recipientOrganizationId,
        recipientEntryId: fresh.recipientEntryId,
        recipientServiceId: fresh.recipientServiceId,
        recipientLocationId: fresh.recipientLocationId,
    ...
    if (fresh.recipientOrganizationId) {
      await deliverPreInquiryToOrganizationWithin(tx, {
        preInquiryId: replacement.id,
        organizationId: fresh.recipientOrganizationId
      });
    }
...
export async function createPreInquiry(userId, input = {}, { db = prisma } = {}) {
  ...
  const clientActionHash = clientActionId ? preInquiryActionHash({
    ...
    recipientServiceId: recipient.recipientServiceId,
    recipientLocationId: recipient.recipientLocationId,
    sourceJourneyId,
    recipientType: recipient.recipientType,
  ...
  const deliverToOrganization = shouldDeliverToOrganization({
    previousStatus: null,
    nextStatus: status,
    recipientOrganizationId: recipient.recipientOrganizationId
  });

  /* ATOMAARSUS (parandus 01.08, kontrolli leid): pöördumise salvestus ja
     postkastikirje loomine peavad õnnestuma KOOS või mitte kumbki.
     Varem loodi kirje pärast commit'i — siis oleks postkastikirje tõrge
     jätnud `SENT` pöördumise, mida organisatsioon EI NÄE, samal ajal kui
     saatja usub, et ta on kohale toimetatud. See on halvim võimalik seis:
     vaikne kadu, mida kumbki pool ei märka.
     Tehing avatakse AINULT org-adressaadi korral — isikliku vastuvõtja rada
     jääb täpselt endiseks. */
  const create = (client) => client.preInquiry.create({
    data: {
      authorId: userId,
      clientActionId,
      clientActionHash,
  ...
  let createdNow = true;
  const createWithin = async (tx) => {
    if (typeof tx.$executeRaw === "function") {
      await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${`pre-inquiry-create:${userId}`}))`;
    }
    if (clientActionId && typeof tx.preInquiry.findFirst === "function") {
      const existingAction = await tx.preInquiry.findFirst({
        where: { authorId: userId, clientActionId },
        include: preInquiryInclude
      });
      if (existingAction) {
        if (existingAction.clientActionHash !== clientActionHash) {
          throw preInquiryError("pre_inquiries.errors.action_key_conflict", 409);
        }
        createdNow = false;
        return existingAction;
      }
    }
    if (["DRAFT", "READY", "DOWNLOADED"].includes(status) && typeof tx.preInquiry.count === "function") {
      const activeCount = await tx.preInquiry.count({
        where: { authorId: userId, status: { in: ["DRAFT", "READY", "DOWNLOADED"] } }
      });
      if (activeCount >= PRE_INQUIRY_ACTIVE_DRAFT_LIMIT) {
        throw preInquiryError("pre_inquiries.errors.active_limit_reached", 409);
      }
    }
    let created;
    try {
      created = await create(tx);
    } catch (error) {
      if (!clientActionId || !isUniqueConflict(error) || typeof tx.preInquiry.findFirst !== "function") throw error;
      const existingAction = await tx.preInquiry.findFirst({ where: { authorId: userId, clientActionId }, include: preInquiryInclude });
      if (!existingAction || existingAction.clientActionHash !== clientActionHash) {
        throw preInquiryError("pre_inquiries.errors.action_key_conflict", 409);
      }
      createdNow = false;
      return existingAction;
    }
    if (deliverToOrganization) {
        await deliverPreInquiryToOrganizationWithin(tx, {
          preInquiryId: created.id,
          organizationId: recipient.recipientOrganizationId
        });
    }
    return created;
  };
  const inquiry = typeof db.$transaction === "function"
    ? await db.$transaction(createWithin)
    : await createWithin(db);

  if (createdNow && shouldSendInternalArrival({
    previousStatus: null,
    nextStatus: status,
    deliveryChannel: recipient.deliveryChannel,
    recipientOwnerId: recipient.recipientOwnerId
...
export async function assertRecipientChangeAllowed(
  ...
  {
    inquiryId,
    previousRecipientOwnerId,
    nextRecipientOwnerId,
    previousRecipientOrganizationId,
    nextRecipientOrganizationId
  }
) {
  const sameOwner = String(nextRecipientOwnerId || "") === String(previousRecipientOwnerId || "");
  const sameOrganization = String(nextRecipientOrganizationId || "") === String(previousRecipientOrganizationId || "");
  if (sameOwner && sameOrganization) return;
...
export async function updatePreInquiry(userId, inquiryId, input = {}, { db = prisma } = {}) {
  ...
  if (existing.status === "ARCHIVED") {
    throw preInquiryError("pre_inquiries.errors.archived_cannot_be_edited", 409);
  }
  ...
  const inquiry = await withPreInquiryRoomLock(existing.id, async (tx) => {
    ...
    if (!sameUpdatedAtFingerprint(fresh.updatedAt, input.expectedUpdatedAt)) {
      throw preInquiryError("pre_inquiries.errors.edit_conflict", 409);
    }

    // Resolve the recipient from the FRESH record's defaults, using the same tx.
    const hasRecipientField = (field) => Object.prototype.hasOwnProperty.call(input, field);
    const nextRecipientEntryId = hasRecipientField("recipientEntryId")
      ? input.recipientEntryId
      : fresh.recipientEntryId;
    const nextRecipientServiceId = hasRecipientField("recipientServiceId")
      ? input.recipientServiceId
      : fresh.recipientServiceId;
    const nextRecipientLocationId = hasRecipientField("recipientLocationId")
      ? input.recipientLocationId
      : fresh.recipientLocationId;
    const nextRecipientOrganizationId = hasRecipientField("recipientOrganizationId")
      ? input.recipientOrganizationId
      : fresh.recipientOrganizationId;
    const hasServiceMapRecipient = Boolean(normalizeServiceMapRecipientEntryId(nextRecipientEntryId));
    const recipient = await resolveRecipient({
      recipientOrganizationId: nextRecipientOrganizationId,
      recipientEntryId: nextRecipientEntryId,
      recipientServiceId: nextRecipientServiceId,
      recipientLocationId: nextRecipientLocationId,
      recipientType: hasServiceMapRecipient ? input.recipientType : (input.recipientType ?? fresh.recipientType),
      selectedRecipientEmail: hasServiceMapRecipient ? input.selectedRecipientEmail : (input.selectedRecipientEmail ?? fresh.selectedRecipientEmail),
      selectedRecipientName: hasServiceMapRecipient ? input.selectedRecipientName : (input.selectedRecipientName ?? fresh.selectedRecipientName)
    }, { db: tx });
    ...
    await assertRecipientChangeAllowed(tx, {
      ...
      previousRecipientOrganizationId: fresh.recipientOrganizationId,
      nextRecipientOrganizationId: recipient.recipientOrganizationId
    });
    ...
    assertAuthorStatusTransition("PATCH", fresh.status, status);
    ...
    const result = await tx.preInquiry.updateMany({
      ...
      data: {
        ...
        userEditedDraft: finalUserEditedDraft,
        status,
        sentAt: status === "SENT" ? fresh.sentAt || occurredAt : fresh.sentAt,
        updatedAt: occurredAt
      }
    });
    if (result.count !== 1) {
    ...
    const updated = await loadPreInquiryAfterMutation(tx, existing.id);
    ...
    /* SAMAS tehingus kui salvestus (vt `createPreInquiry` selgitust):
       `SENT` pöördumine ilma postkastikirjeta oleks vaikne kadu. */
    if (shouldDeliverToOrganization(arrival)) {
      await deliverPreInquiryToOrganizationWithin(tx, {
        preInquiryId: updated.id,
        organizationId: arrival.recipientOrganizationId
      });
    }
...
export async function reopenPreInquiry(
  userId,
  inquiryId,
  { expectedUpdatedAt = null, db = prisma } = {}
) {
  ...
  return withPreInquiryRoomLock(existing.id, async (tx) => {
    const fresh = await loadPreInquiryAfterMutation(tx, existing.id);
    if (!fresh || fresh.authorId !== userId) throw preInquiryError("api.common.not_found", 404);
    if (fresh.status !== "ARCHIVED") {
      throw preInquiryError("pre_inquiries.errors.reopen_requires_archived", 409);
    }
    if (fresh.sentAt || fresh.openedAt || fresh.supersededById) {
      throw preInquiryError("pre_inquiries.errors.opened_cannot_be_edited", 409);
    }
    if (!sameUpdatedAtFingerprint(fresh.updatedAt, expectedUpdatedAt)) {
      throw preInquiryError("pre_inquiries.errors.reopen_conflict", 409);
    }
    assertAuthorStatusTransition("REOPEN", fresh.status, "READY");
    ...
    const result = await tx.preInquiry.updateMany({
      ...
      data: { status: "READY", updatedAt: occurredAt }
    });
    if (result.count !== 1) throw preInquiryError("pre_inquiries.errors.reopen_conflict", 409);
    return serializePreInquiry(await loadPreInquiryAfterMutation(tx, fresh.id), { viewerId: userId });
  }, { db });
}
...
export async function updatePreInquiryReceiverWorkflow(
  ...
  inquiryId,
  input = {},
  { db = prisma, roomLockAlreadyHeld = false } = {}
) {
  const existing = await db.preInquiry.findFirst({
    where: {
      id: inquiryId,
  ...
  // A caller that already holds the canonical room advisory lock may compose
  // this mutation into a larger transaction. This is used by FIELD handover so
  // the receiver-note append, handover state, visit stamp and audit either all
  // commit or all roll back together.
  if (roomLockAlreadyHeld) return mutate(db);
  return withPreInquiryRoomLock(existing.id, mutate, { db });
}

async function loadCurrentProviderPolicy(db, inquiry) {
  const publicContactEligibility = await buildPublicServiceMapContactEligibilityClause(db);
  const currentEntry = await db.serviceMapEntry.findFirst({
    where: {
      id: inquiry.recipientEntryId,
      status: "PUBLISHED",
      AND: [publicContactEligibility]
    },
    include: {
      providerProfile: {
        include: {
          serviceItems: { where: { mapVisible: true, status: "PUBLISHED" } },
          serviceLocations: {
            where: { mapVisible: true, status: "PUBLISHED" },
  ...
  return resolveSelectedServicePolicy({
    entry: currentEntry,
    serviceId: inquiry.recipientServiceId,
    locationId: inquiry.recipientLocationId
  });
}

export async function confirmExternalPreInquirySent(
  userId,
  inquiryId,
  { expectedUpdatedAt = null, db = prisma } = {}
) {
  ...
  return withPreInquiryRoomLock(existing.id, async (tx) => {
    const fresh = await loadPreInquiryAfterMutation(tx, existing.id);
    if (!fresh || fresh.authorId !== userId) throw preInquiryError("api.common.not_found", 404);
    if (fresh.deliveryChannel !== "EXTERNAL_EMAIL") {
      throw preInquiryError("pre_inquiries.errors.internal_cannot_email", 409);
    }
    if (fresh.status === "SENT" && fresh.externalSendConfirmedAt) {
      return serializePreInquiry(fresh, { viewerId: userId });
    }
    if (fresh.status === "ARCHIVED") {
      throw preInquiryError("pre_inquiries.errors.archived_cannot_be_edited", 409);
    ...
    if (!sameUpdatedAtFingerprint(fresh.updatedAt, expectedUpdatedAt)) {
      throw preInquiryError("pre_inquiries.errors.external_confirmation_conflict", 409);
    }
    if (fresh.recipientEntry?.type === "SERVICE_PROVIDER" && (!fresh.recipientServiceId || !fresh.recipientLocationId)) {
      throw preInquiryError("pre_inquiries.errors.recipient_channel_changed", 409);
    }
    if (fresh.recipientEntryId) {
      const currentRecipient = await resolveRecipient({
        recipientEntryId: fresh.recipientEntryId,
        recipientServiceId: fresh.recipientServiceId,
        recipientLocationId: fresh.recipientLocationId
      }, { db: tx, validateClientIdentity: false });
      const unchanged = currentRecipient.recipientType === fresh.recipientType
        && currentRecipient.selectedRecipientName === fresh.selectedRecipientName
        && currentRecipient.selectedRecipientEmail === fresh.selectedRecipientEmail
        && currentRecipient.deliveryChannel === fresh.deliveryChannel;
      if (!unchanged) throw preInquiryError("pre_inquiries.errors.recipient_channel_changed", 409);
    }
    if (fresh.recipientServiceId || fresh.recipientLocationId) {
      const currentPolicy = await loadCurrentProviderPolicy(tx, fresh);
      if (!currentPolicy?.emailAllowed || !currentPolicy.email || currentPolicy.email !== fresh.selectedRecipientEmail) {
        throw preInquiryError("pre_inquiries.errors.recipient_channel_changed", 409);
      }
    }
    ...
    const result = await tx.preInquiry.updateMany({
      ...
      data: {
        status: "SENT",
        sentAt: occurredAt,
        externalSendConfirmedAt: occurredAt,
        updatedAt: occurredAt
      }
    });
    if (result.count !== 1) {
...
export async function assistPreInquiry({
  ...
  assertPreInquiryInputLimits({ topic, situation, assistantMessage });
  ...
  const preferredMapType =
    preferredRecipientType === "SERVICE_PROVIDER"
      ...
      : preferredRecipientType === "KOV_CONTACT"
        ? "KOV_SOCIAL_CONTACT"
        : "";
  const publicContactEligibility = await buildPublicServiceMapContactEligibilityClause(
    db,
  ...
  const suggestions = entries
    .map((entry) => {
      ...
      // Sama reegel mis saatmisel: org-profiili looja EI ole soovitusloendis
      // adressaat. Muidu lubaks soovitus „platvormisisest" kohaletoimetamist
      // inimesele, kellel ei ole selle organisatsiooniga enam mingit seost.
      const providerRecipientUserId = resolveProviderRecipientUserId(entry.providerProfile);
      ...
      const score = scoreEntry(entry, preferredMapType, matchExplanation.routingEvidence);
      return {
        id: entry.id,
        type: entry.type,
        title: entry.title,
        ...
        routingEvidence: matchExplanation.routingEvidence,
        score
      };
    })
    .filter((entry) => {
      const evidence = entry.routingEvidence;
      if (!evidence?.channelMatch) return false;
      if (normalizedMunicipality && !evidence.regionMatch) return false;
      if (entry.type !== "SERVICE_PROVIDER" && evidence.regionLevel === "COUNTY") return false;
      if ((keywords.length || normalizedNeedAreas.length) && !evidence.needMatch) return false;
      return true;
    })
