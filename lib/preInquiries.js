...
async function resolveRecipient(input = {}, { db = prisma, validateClientIdentity = true } = {}) {
  ...
  let recipientOrganizationId = null;
  if (requestedOrganizationId && isOrgInboxEnabled()) {
    ...
    recipientOrganizationId = organization?.id || null;
  }
...
export async function updatePreInquiry(userId, inquiryId, input = {}, { db = prisma } = {}) {
  ...
  const inquiry = await withPreInquiryRoomLock(existing.id, async (tx) => {
    ...
    const nextRecipientEntryId = input.recipientEntryId ?? fresh.recipientEntryId;
    ...
    const recipient = await resolveRecipient({
      ...
      recipientServiceId: input.recipientServiceId ?? fresh.recipientServiceId,
      recipientLocationId: input.recipientLocationId ?? fresh.recipientLocationId,
