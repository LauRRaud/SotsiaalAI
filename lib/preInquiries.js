...
export async function recallPreInquiry(
  ...
  return withPreInquiryRoomLock(existing.id, async (tx) => {
    ...
    if (fresh.recalledAt) return serializePreInquiry(fresh);
    ...
    return serializePreInquiry(await loadPreInquiryAfterMutation(tx, fresh.id));
  }, { db });
}
...
export async function sendPreInquiryCorrection(
  ...
  const outcome = await withPreInquiryRoomLock(existing.id, async (tx) => {
    ...
    if (fresh.supersededById) {
      ...
      return { inquiry: serializePreInquiry(replacement), created: false };
    }
    if (fresh.recalledAt) {
    ...
    return { inquiry: serializePreInquiry(replacement), created: true };
  }, { db });
