...
import { deliverPreInquiryToOrganization, recallInboxItemForSource } from "@/lib/org/inbox";
...
export async function recallPreInquiry(
  ...
  return withPreInquiryRoomLock(existing.id, async (tx) => {
    ...
    if (fresh.recipientOrganizationId) {
      await recallInboxItemForSource({ sourceId: fresh.id }, { db: tx, now: occurredAt });
    }
...
export async function createPreInquiry(userId, input = {}, { db = prisma } = {}) {
  ...
  /* Kohaletoimetamine käib PÄRAST tehingut ja on idempotentne
     (`@@unique([organizationId, sourceType, sourceId])`). Nii ei hoia
     postkastikirje loomine eelpöördumise enda salvestust kinni ega saa teda
     ka tagasi keerata — eelpöördumine on saadetud sõltumata sellest, kas
     postkastirida jõudis tekkida. */
  if (shouldDeliverToOrganization({
    previousStatus: null,
    nextStatus: status,
    recipientOrganizationId: recipient.recipientOrganizationId
  })) {
    await deliverPreInquiryToOrganization(
      { preInquiryId: inquiry.id, organizationId: recipient.recipientOrganizationId },
      { db }
    );
  }
...
export async function updatePreInquiry(userId, inquiryId, input = {}, { db = prisma } = {}) {
  ...
  if (arrival && shouldDeliverToOrganization(arrival)) {
    await deliverPreInquiryToOrganization(
      { preInquiryId: inquiry.id, organizationId: arrival.recipientOrganizationId },
      { db }
    );
  }
