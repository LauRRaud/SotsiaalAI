...
import { makeChunks } from './chunking.js';
...
export function normalize({ metadata, originalMetadata = metadata, parsed, structured, tenant, documentId, versionId, pdfHash, metadataHash, config, profile, rights, ingestedAt }) {
  const { pages, source_units, spans, sections, blocks, removed, nulReplacements = 0 } = structured;
  ...
  for (const s of spans.filter(s => s.pdf_page === 1)) {
    ...
    if (month > 0) {
      ...
      if (new Date(date).toISOString().slice(0, 10) === date) {
        ...
        if (metadata.year && metadata.year !== Number(match[3])) warnings.push({ code: 'publication_year_conflict', resolution: 'Imported bibliography retained; first-page date remains a candidate.' });
        break;
      }
    }
  }
  ...
  for (const [field, raw] of Object.entries(parsed.source_metadata || {})) {
    ...
    const zonedDate = ['valid_from', 'valid_to', 'publication_date'].includes(field)
      && typeof raw === 'string' && raw.match(/^(\d{4}-\d{2}-\d{2})(?:Z|[+-]\d{2}:\d{2})$/u);
    const value = zonedDate ? zonedDate[1] : raw;
  ...
  const chunks = makeChunks({ spans, blocks, sections, source_units, metadata: { ...metadata, title: fields.title.value }, versionId, scope, config });
