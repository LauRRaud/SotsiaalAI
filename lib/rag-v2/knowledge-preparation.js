...
export function knowledgePreparationPlan(bundle, config) {
  ...
  const body = { model: config.model, store: false, reasoning: { effort: config.reasoning }, max_output_tokens: config.maxOutputTokens,
    ...
    input: JSON.stringify({ sources: sources.map(({ source_id, pdf_page, source_unit_index, source_location, text }) => ({ source_id, ...(pdf_page === undefined ? { source_unit_index, source_location } : { pdf_page }), text })) }),
