...
import { validateKnowledgeInput, validateKnowledgeBundle } from './knowledge.js';
import { unitForSpan, validateSourceLocations } from './source-locations.js';
import { validateStructuredRecord, validateStructuredRecordSources } from './structured-record.js';

export const SCHEMA_VERSION = 'rag-v2/2';
export const DEFAULT_CONFIG = Object.freeze({
  parser: 'pdfjs-dist@5.4.296', normalization: 'source-structure-v25', chunking: 'structure-blocks-v19',
  maxFileBytes: 32 * 1024 * 1024, maxMetadataBytes: 1024 * 1024,
  maxPages: 400, maxTextChars: 2000000, timeoutMs: 30000,
  maxPdfItemsPerPage: 20000, maxPdfItems: 100000, maxExpandedChars: 20000000,
  chunkMaxChars: 2200, marginFraction: 0.1, headingRatio: 1.5,
...
export function nonempty(value) { return typeof value === 'string' && value.trim().length > 0 && !value.includes('\u0000'); }
export function validateMetadata(meta) {
  ...
  for (const key of ['authors', 'tags', 'regions']) {
    if (meta[key] !== undefined && (!Array.isArray(meta[key]) || !meta[key].every(nonempty))) fail(`invalid_metadata_${key}`);
  }
  for (const key of ['docId', 'articleId', 'source_id', 'description', 'section', 'last_checked', 'journalTitle', 'issueLabel', 'authority', 'source_status', 'collection_id']) {
  ...
  if (meta.audience !== undefined && (Array.isArray(meta.audience)
    ? !meta.audience.length || meta.audience.length > 32 || meta.audience.some(value => !nonempty(value) || value.length > 100)
    : !nonempty(meta.audience) || meta.audience.length > 100)) fail('invalid_metadata_audience');
  const fieldLimits = {
    ...
    last_checked: 64, journalTitle: 300, issueLabel: 100,
  ...
  validateKnowledgeInput(meta.knowledge);
}
...
export function validateBundle(bundle) {
  if (![SCHEMA_VERSION, 'rag-v2/1'].includes(bundle.schema_version) || !nonempty(bundle.tenant_id)) fail('invalid_bundle');
  if (bundle.schema_version === SCHEMA_VERSION && !Array.isArray(bundle.source_units)) fail('invalid_source_units');
  if (!Array.isArray(bundle.knowledge_cards ?? []) || !Array.isArray(bundle.dependencies ?? []) || !Array.isArray(bundle.knowledge_gaps ?? [])) fail('invalid_knowledge_shape');
  const objects = [bundle.document, bundle.version, ...bundle.assets, ...bundle.spans, ...bundle.sections, ...bundle.blocks, ...bundle.chunks, ...bundle.relations,
    ...(bundle.knowledge_cards || []), ...(bundle.dependencies || []), ...(bundle.knowledge_gaps || [])];
  ...
  for (const block of bundle.blocks) {
    if (!['heading', 'paragraph', 'quote', 'list_item', 'table_row', 'legal_unit'].includes(block.kind) || block.span_ids.some(s => !spans.has(s))) fail('invalid_block');
  }
  for (const span of bundle.spans) {
    const page = unitForSpan(bundle, span);
    const pdf = (bundle.version.source_format ?? 'pdf') === 'pdf';
    if (!page || (pdf ? span.pdf_page !== span.parser_page_index + 1 : span.pdf_page !== null || span.parser_page_index !== null)
      || span.document_version_id !== bundle.version.id || !Number.isSafeInteger(span.start) || !Number.isSafeInteger(span.end)
      || span.start < 0 || span.end <= span.start || span.end > page.raw_text.length
      || typeof span.source_text !== 'string' || !span.source_text.isWellFormed() || page.raw_text.slice(span.start, span.end) !== span.source_text) fail('invalid_source_span');
  ...
  if (bundle.document.fields.structured_record) {
    const record = bundle.document.fields.structured_record.value;
    validateStructuredRecord(record);
    if (bundle.version.source_format !== 'json' || !Array.isArray(bundle.source_units)
      || !bundle.document.fields.regions?.value?.includes(record.region)) fail('record_source_scope_mismatch');
    validateStructuredRecordSources(record, bundle.source_units);
  }
  validateSourceLocations(bundle);
  validateKnowledgeBundle(bundle);
  return bundle;
}
