...
import { unitForSpan } from './source-locations.js';

export const KNOWLEDGE_SCHEMA = 'rag-v2/knowledge-input-2';
...
function anchors(value) {
  ...
  for (const anchor of value) {
    shape(anchor, ['pdf_page', 'source_unit_index', 'quote', 'start']); text(anchor.quote, 4000);
    if ((anchor.source_unit_index === undefined ? !Number.isSafeInteger(anchor.pdf_page) || anchor.pdf_page < 1
      : anchor.pdf_page !== undefined || !Number.isSafeInteger(anchor.source_unit_index) || anchor.source_unit_index < 0)
...
export function validateKnowledgeInput(value) {
  ...
  shape(value, ['schema_version', 'cards', 'dependencies', 'gaps']);
  if (![KNOWLEDGE_SCHEMA, 'rag-v2/knowledge-input-1'].includes(value.schema_version)) fail('unsupported_knowledge_schema');
  ...
  if (value.gaps !== undefined) {
    list(value.gaps, 0, 64);
    const gapKeys = new Set();
    for (const gap of value.gaps) {
      shape(gap, ['key', 'from', 'statement', 'reason', 'anchors']);
      key(gap.key); text(gap.statement, 2000); text(gap.reason, 1000); anchors(gap.anchors);
      if (gapKeys.has(gap.key) || gap.from !== null && !keys.has(gap.from)) fail('invalid_knowledge_gap');
      gapKeys.add(gap.key);
    }
  }
...
function resolveAnchors(values, bundle) {
  return values.map(anchor => {
    const generic = anchor.source_unit_index !== undefined;
    const page = generic ? bundle.source_units?.[anchor.source_unit_index] : bundle.pages[anchor.pdf_page - 1];
    if (!page || !generic && page.pdf_page !== anchor.pdf_page) fail('knowledge_anchor_page_missing');
    ...
    const spans = bundle.spans.filter(span => (generic ? unitForSpan(bundle, span) === page : span.pdf_page === anchor.pdf_page) && span.start < end && span.end > start).sort((a, b) => a.start - b.start);
    ...
    return { ...(generic ? { source_unit_index: anchor.source_unit_index, source_location: page.locator } : { pdf_page: anchor.pdf_page }), start, end, quote: anchor.quote, span_ids: spans.map(span => span.id) };
  });
}
...
export function prepareKnowledge(value, bundle) {
  ...
  const grounded = entry => {
    ...
    return { anchors, span_ids: [...new Set(anchors.flatMap(anchor => anchor.span_ids))],
      verification_state: KNOWLEDGE_STATE, provenance: { kind: 'metadata', asset_hash: bundle.version.metadata_hash, schema_version: value.schema_version } };
  };
  ...
  const knowledge_gaps = value.gaps?.map(gap => ({ ...scope, id: id('knowledge_gap', scope.tenant_id, bundle.version.id, gap.key), key: gap.key,
    from_card_id: gap.from === null ? null : cardId(bundle.version.id, gap.from), statement: gap.statement, reason: gap.reason, ...grounded(gap) }));
  return { knowledge_cards, dependencies, ...(knowledge_gaps === undefined ? {} : { knowledge_gaps }) };
}

export function validateKnowledgeBundle(bundle) {
  const input = bundle.document.legacy_metadata?.knowledge;
  if (input === undefined && !bundle.knowledge_cards?.length && !bundle.dependencies?.length && !bundle.knowledge_gaps?.length) return;
  const expected = prepareKnowledge(input, bundle);
  if (stable(bundle.knowledge_cards) !== stable(expected.knowledge_cards) || stable(bundle.dependencies) !== stable(expected.dependencies)
    || stable(bundle.knowledge_gaps) !== stable(expected.knowledge_gaps)) fail('knowledge_provenance_mismatch');
}
