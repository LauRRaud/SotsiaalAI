...
import { evidenceDraftEnabled, EVIDENCE_DRAFT_SCHEMA, EVIDENCE_DRAFT_PROMPT } from './evidence-draft.js';
import { dialogueEnabled, DIALOGUE_PROMPT_VERSION, DIALOGUE_SEARCH_VERSION } from './dialogue.js';
...
export async function readPilotConfig(userId, { purpose = 'execute' } = {}) {
  ...
  if (config.mode === 'real') {
    if (candidate && (config.evidenceDraftSchemaHash !== digest(EVIDENCE_DRAFT_SCHEMA) || config.evidenceDraftPromptVersion !== EVIDENCE_DRAFT_PROMPT)) reject('evidence_draft_approval_mismatch', 403);
    ...
    if (purpose === 'read' && (!(continuing ? [DIALOGUE_PROMPT_VERSION] : READABLE_PROMPT_VERSIONS).includes(config.promptVersion)
