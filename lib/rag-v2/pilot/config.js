...
import { dialogueStateContract, TYPED_DIALOGUE_STATE_VERSION } from './dialogue-state.js';
...
export async function readPilotConfig(userId, { purpose = 'execute' } = {}) {
  ...
  if (unifiedRetrievalEnabled(config) && (stateContract?.version !== TYPED_DIALOGUE_STATE_VERSION
    || config.recordCatalogue !== RECORD_RETRIEVAL_VERSION || config.mode !== 'real'
    || config.budget.embeddingAttempts < 1)) reject('invalid_unified_retrieval_plan', 403);
  if (config.recordCatalogue !== undefined && (config.recordCatalogue !== RECORD_RETRIEVAL_VERSION || !continuing)) reject('invalid_record_catalogue_plan', 403);
