...
import { dialogueStateEnabled, DIALOGUE_ANSWER_SCHEMA } from './dialogue-state.js';
...
export async function readPilotConfig(userId, { purpose = 'execute' } = {}) {
  ...
  if (dialogueStateEnabled(config) && (!continuing || config.dialogueStateSchemaHash !== digest(DIALOGUE_ANSWER_SCHEMA))) reject('invalid_dialogue_state_plan', 403);
