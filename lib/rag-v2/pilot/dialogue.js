...
import { dialogueStateEnabled, dialogueStateContract, TYPED_DIALOGUE_STATE_VERSION, STATE_INSTRUCTIONS, TYPED_PERIOD_INSTRUCTIONS } from './dialogue-state.js';
...
export const DIALOGUE_PROMPT_VERSION = 'm4-grounded-dialogue-7';
export const READABLE_DIALOGUE_PROMPT_VERSIONS = Object.freeze(['m4-grounded-dialogue-1', 'm4-grounded-dialogue-2', 'm4-grounded-dialogue-3', 'm4-grounded-dialogue-4', 'm4-grounded-dialogue-5', 'm4-grounded-dialogue-6', DIALOGUE_PROMPT_VERSION]);
export const DIALOGUE_SEARCH_VERSION = 'm4-user-scope-search-5';
export const READABLE_DIALOGUE_SEARCH_VERSIONS = Object.freeze(['m4-user-scope-search-1', 'm4-user-scope-search-2', 'm4-user-scope-search-3', 'm4-user-scope-search-4', DIALOGUE_SEARCH_VERSION]);
...
export function acceptDialogue(config, input, rows, head, turnId) {
  ...
  const selection = {
    ...
    ...(dialogueStateEnabled(config) ? { stateTurnId: published.at(-1)?.id || null } : {}),
  };
...
export function dialogueInput(accepted, assistant = null, state = null) {
  const result = { version: DIALOGUE_VERSION, scope: accepted.context, userTurns: accepted.userTurns,
    ...
    ...(state ? { previousState: state.previous?.value || null, stateContext: state.context,
...
export function dialogueRequest(config, question, evidence, language, dialogue) {
  ...
  if (dialogueStateEnabled(config)) {
    ...
    body.instructions += STATE_INSTRUCTIONS;
    if (config.dialogueStateVersion === TYPED_DIALOGUE_STATE_VERSION) body.instructions += TYPED_PERIOD_INSTRUCTIONS;
  }
