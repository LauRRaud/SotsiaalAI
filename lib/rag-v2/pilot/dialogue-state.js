...
export function validateDialogueState(value, accepted, context, previous = null, version = DIALOGUE_STATE_VERSION) {
  ...
  shape(value.region, ['id', 'status', 'support'], 'region'); support(value.region.support, value.region.id === null ? 0 : 1, 'region');
  if (!['unknown', 'tentative', 'reported', 'ambiguous'].includes(value.region.status)
    || (['unknown', 'ambiguous'].includes(value.region.status) ? value.region.id !== null
      : !context.regions.some(row => row.region === value.region.id))) invalid('region_id');
  if (version === TYPED_DIALOGUE_STATE_VERSION) list(value.periods, 0, 2, 'periods');
  for (const period of version === TYPED_DIALOGUE_STATE_VERSION ? value.periods : value.period === null ? [] : [value.period]) {
    shape(period, version === TYPED_DIALOGUE_STATE_VERSION ? ['basis', 'from', 'to', 'support'] : ['from', 'to', 'support'], 'period'); support(period.support, 1, 'period');
