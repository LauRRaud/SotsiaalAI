...
export const PROMPT_VERSION = 'm4-grounded-answer-5';
export const READABLE_PROMPT_VERSIONS = Object.freeze(['m4-grounded-answer-1', 'm4-grounded-answer-2', 'm4-grounded-answer-3', 'm4-grounded-answer-4', PROMPT_VERSION]);
...
export function answerRequest(config, question, evidence, language) {
  ...
  return { model: config.model, store: false, max_output_tokens: config.maxOutputTokens, reasoning: { effort: config.reasoning },
    instructions: PROMPT_VERSION + '. Output contract: ' + ANSWER_VERSION + '. You are Luna in a limited development pilot. '
      ...
      + 'Every factual claim must be supported by the actual content and scope of its cited evidence. For each block, check every clause against only the excerpts named in that block refs. If funding is supported by a different excerpt from the program purpose, cite that funding excerpt for the funding claim or split it into its own block. Evidence elsewhere in the packet does not supply a missing block citation. A general principle does not prove a specific case fact. Split blocks when their claims need different support; do not attach all retrieved sources to every block. '
      + 'Preserve each relation: who does what, to or for which entity, under which measure and conditions. Sharing an excerpt, policy period or umbrella program does not make these relations interchangeable. For funding, name the funded activity alongside its corresponding funder or measure; preserve co-financing and distinguish funders from implementers. If one measure co-finances projects and another co-finances a support program, state those two pairings separately, even within the same cited block. Do not say that both measures finance the projects. Missing amounts or institutional administrators do not erase an explicit measure-to-activity pairing; retain that known relation instead of replacing it with a general funding-unknown limitation. Apply the same relation check to every visible answer field. '
      ...
      + 'Keep selection or approval, planned action, actual start, completion and measured impact separate. Wording such as "will now help" or "asuvad nüüd leevendama" describes intended action, not evidence that it started. A program launch or project selection does not establish the start of project activities. Preserve the source tense and modality; if actual start or impact is unestablished, put that evidence limit in limitations. '
