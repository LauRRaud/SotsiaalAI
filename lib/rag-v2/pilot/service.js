...
import { evidenceDraftEnabled, evidenceDraftRequest, projectEvidenceDraft, EVIDENCE_DRAFT_VERSION, EVIDENCE_DRAFT_PROMPT } from './evidence-draft.js';
...
export class PilotService {
  ...
  async restore(row) {
    ...
    return { id: row.id, state: 'completed', mode: config.mode, question: row.payload.question, answer: validateAnswer(row.payload.answer, Object.keys(row.payload.packet.reference_map), row.payload.answerVersion || 'm4-text-refs-1'),
      answerVersion: row.payload.answerVersion || 'm4-text-refs-1', language: row.payload.query.language, context: publicContext(row),
    ...
    try {
      ...
      row = await this.store.save(config, row, 'claimed', { packet, answerVersion: ANSWER_VERSION,
        requestAudit: { body, bodyHash: digest(body), questionHash: query.hash, language: query.language,
          ...(candidate ? { evidenceDraftVersion: EVIDENCE_DRAFT_VERSION, evidenceDraftPromptVersion: EVIDENCE_DRAFT_PROMPT } : {}),
      ...
      const projected = candidate ? await projectEvidenceDraft(result.value, packet, config, this.adapters.canonical) : null;
  ...
  async checkEvidenceDraft(config, row) {
    ...
    if (!evidenceDraftEnabled(config) || row.payload.requestAudit.evidenceDraftVersion !== EVIDENCE_DRAFT_VERSION
      || row.payload.responseAudit?.draft?.truncated) reject('evidence_draft_unavailable', 403);
    const projected = await projectEvidenceDraft(JSON.parse(row.payload.responseAudit.draft.text), row.payload.packet, config, this.adapters.canonical);
