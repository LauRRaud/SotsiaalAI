...
import { dialogueStateEnabled, previousStateFor, projectDialogueAnswer, validateStateContext, validateStateRegion } from './dialogue-state.js';
...
export class PilotService {
  ...
  async run(userId, input) {
    ...
    try {
      ...
      const stateProjection = stateEnabled ? projectDialogueAnswer(result.value, packet, row.payload.contextAudit, stateContext, previousState, config.dialogueStateVersion) : null;
  ...
  checkDialogueState(config, row) {
    ...
    const projected = projectDialogueAnswer(JSON.parse(row.payload.responseAudit.draft.text), row.payload.packet,
      row.payload.contextAudit, row.payload.dialogueStateContext, row.payload.previousDialogueState, config.dialogueStateVersion);
