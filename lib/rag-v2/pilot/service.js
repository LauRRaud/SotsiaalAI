...
import { dialogueStateEnabled, DIALOGUE_STATE_VERSION, previousStateFor, projectDialogueAnswer, validateStateContext, validateStateRegion } from './dialogue-state.js';
...
export class PilotService {
    ...
    try {
      ...
      row = await this.store.save(config, row, 'claimed', { packet, answerVersion: ANSWER_VERSION,
        requestAudit: { body, bodyHash: digest(body), questionHash: query.hash, language: query.language,
          ...
          ...(stateEnabled ? { dialogueStateVersion: DIALOGUE_STATE_VERSION } : {}),
      ...
      const stateProjection = stateEnabled ? projectDialogueAnswer(result.value, packet, row.payload.contextAudit, stateContext, previousState) : null;
      if (stateProjection) await (this.adapters.validateDialogueStateRegion || validateStateRegion)(stateProjection.audit.value, stateContext);
      ...
      row = await this.store.save(config, row, 'needs_recovery', { answer, ...(projected ? { evidenceDraftAudit: projected.audit } : {}),
        ...(stateProjection ? { dialogueState: stateProjection.audit } : {}),
  ...
  checkDialogueState(config, row) {
    ...
    if (!dialogueStateEnabled(config) || row.payload.requestAudit?.dialogueStateVersion !== DIALOGUE_STATE_VERSION
      || !row.payload.dialogueState || !row.payload.responseAudit?.draft?.text || row.payload.responseAudit.draft.truncated) reject('dialogue_state_unavailable', 403);
    const projected = projectDialogueAnswer(JSON.parse(row.payload.responseAudit.draft.text), row.payload.packet,
      row.payload.contextAudit, row.payload.dialogueStateContext, row.payload.previousDialogueState);
    if (digest(projected.audit) !== digest(row.payload.dialogueState) || digest(projected.answer) !== digest(row.payload.answer)) reject('dialogue_state_projection_mismatch', 403);
  }
