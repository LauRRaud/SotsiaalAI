...
import { validateKnowledgePreparationConfig } from '../knowledge-preparation.js';
import { DEFAULT_RETRIEVAL_PROFILE, retrievalProfile } from '../search/profiles.js';
...
const FIELDS = ['enabled', 'id', 'tenant', 'subject', 'users', 'documentIds', 'storeRoot', 'workRoot', 'policyFile',
  ...
  'expiresAt', 'maxFileBytes', 'maxMetadataBytes', 'knowledgePreparation', 'retrievalProfileId'];
...
export function validateAdminConfig(value, now = Date.now()) {
  ...
  validateKnowledgePreparationConfig(value.knowledgePreparation);
  // The published generation must suit the retrieval profile the pilot will query it with.
  const retrievalProfileId = value.retrievalProfileId ?? DEFAULT_RETRIEVAL_PROFILE;
  try { retrievalProfile(retrievalProfileId); } catch { fail('rag_v2_admin_retrieval_profile_invalid'); }
  const config = { ...value, ...paths, users: [...new Set(value.users)].sort(), documentIds: [...new Set(value.documentIds)].sort(),
    ...
    maxFileBytes: value.maxFileBytes ?? 20 * 1024 * 1024, maxMetadataBytes: value.maxMetadataBytes ?? 1024 * 1024, retrievalProfileId };
