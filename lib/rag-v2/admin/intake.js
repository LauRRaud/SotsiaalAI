...
import { indexSnapshot, searchConfig } from '../search/indexing.js';
import { assertIndexCapacity, snapshotCapacity } from '../search/capacity.js';
import { assertProfileGeneration, retrievalProfile } from '../search/profiles.js';
...
import { knowledgePreparationPlan } from '../knowledge-preparation.js';
import { KnowledgeJobs } from './knowledge-jobs.js';
...
function summary(bundle) {
  return { document_id: bundle.document.id, version_id: bundle.version.id, pdf_sha256: bundle.version.pdf_hash,
    ...
    knowledge: { cards: bundle.knowledge_cards?.length || 0, dependencies: bundle.dependencies?.length || 0,
      verification_state: 'source_anchored_unreviewed' },
...
export class IntakeService {
  ...
  async get(jobId, asset) {
    ...
    if (!asset) {
      const result = await this.present(receipt);
      await this.receipt(jobId, await this.access());
      return result;
    }
  ...
  async present(receipt) {
    const result = publicReceipt(receipt);
    if (!this.config.knowledgePreparation) return result;
    try {
      const { bundle, plan, jobs } = await this.knowledgeContext(receipt);
      result.knowledge_preparation = { plan: plan.summary, ...await jobs.read(receipt, plan, bundle) };
    } catch (cause) {
      result.knowledge_preparation = { state: 'unavailable', error: safeCode(cause) };
    }
    return result;
  }
  async knowledgeContext(receipt) {
    if (!this.config.knowledgePreparation) throw intakeError('knowledge_preparation_disabled', 409);
    const assertSource = async () => {
      if (!(await this.access()).includes(receipt.bundle.document_id)) throw intakeError('rag_v2_document_not_allowed', 403);
      const snapshot = await loadSnapshot(this.config.storeRoot, this.config.tenant, [receipt.bundle.document_id]);
      const bundle = snapshot.bundles[0];
      if (bundle.version.id !== receipt.bundle.version_id || bundle.version.pdf_hash !== receipt.bundle.pdf_sha256
        || bundle.version.metadata_hash !== receipt.bundle.metadata_sha256) throw intakeError('rag_v2_source_changed', 409);
      return bundle;
    };
    const bundle = await assertSource(), plan = knowledgePreparationPlan(bundle, this.config.knowledgePreparation);
    const jobs = new KnowledgeJobs({ root: rootFor(this.config), config: this.config.knowledgePreparation,
      fingerprint: fingerprint(this.config), writeJson: atomicWrite, assertAccess: assertSource,
      call: this.deps.knowledgeCall, apiKey: this.deps.knowledgeApiKey ?? process.env.OPENAI_API_KEY });
    return { bundle, plan, jobs };
  }
  async prepareKnowledge(jobId, planHash) {
    const documents = await this.access(), receipt = await this.receipt(jobId, documents);
    const release = await lock(rootFor(this.config));
    try {
      const { bundle, plan, jobs } = await this.knowledgeContext(receipt);
      if (plan.hash !== planHash) throw intakeError('knowledge_preparation_plan_changed', 409);
      await jobs.run(receipt, plan, bundle);
      await this.receipt(jobId, await this.access());
      return await this.present(receipt);
    } finally { await release(); }
  }
  async applyKnowledge(jobId, draftHash, selection) {
    const documents = await this.access(), receipt = await this.receipt(jobId, documents);
    const release = await lock(rootFor(this.config));
    try {
      const { bundle, plan, jobs } = await this.knowledgeContext(receipt), prepared = await jobs.read(receipt, plan, bundle);
      if (prepared.state !== 'draft_ready' || !prepared.draft.knowledge || prepared.draft.hash !== draftHash) throw intakeError('knowledge_draft_changed', 409);
      if (!selection || Object.keys(selection).sort().join(',') !== 'cards,dependencies') throw intakeError('knowledge_selection_invalid');
      const knowledge = prepared.draft.knowledge;
      for (const [key, entries] of [['cards', knowledge.cards], ['dependencies', knowledge.dependencies]]) {
        if (!Array.isArray(selection[key]) || new Set(selection[key]).size !== selection[key].length
          || selection[key].some(value => typeof value !== 'string' || !entries.some(entry => entry.key === value))) throw intakeError('knowledge_selection_invalid');
      }
      const cards = knowledge.cards.filter(card => selection.cards.includes(card.key)), kept = new Set(cards.map(card => card.key));
      if (!cards.length) throw intakeError('knowledge_selection_empty');
      const dependencies = knowledge.dependencies.filter(edge => selection.dependencies.includes(edge.key) && kept.has(edge.from)
        && edge.targets.every(target => kept.has(target.key)));
      const gaps = prepared.draft.unresolved.filter(gap => gap.from === null || kept.has(gap.from)).map((gap, index) => ({ key: `gap-${index}`, ...gap }));
      const metadata = { ...receipt.metadata, knowledge: { ...knowledge, cards, dependencies, gaps },
        knowledge_preparation: { schema_version: prepared.draft.schema_version, plan_hash: plan.hash, draft_hash: draftHash,
          source_version_id: bundle.version.id, model: this.config.knowledgePreparation.model,
          verification_state: 'source_anchored_unreviewed', unresolved: prepared.draft.unresolved } };
      const pdfBytes = await fs.readFile(path.join(jobDir(this.config, jobId), 'source.pdf'));
      if (hash(pdfBytes) !== receipt.bundle.pdf_sha256) throw intakeError('version_integrity_failed', 409);
      return await this.#prepare({ pdfBytes, metadataBytes: jsonBytes(metadata), sourceUse: true, baseReceipt: receipt });
    } finally { await release(); }
  }
  async prepare(input) {
    await this.access();
    const release = await lock(rootFor(this.config));
    try { return await this.#prepare(input); } finally { await release(); }
  }
  async #prepare({ pdfBytes, metadataBytes, sourceUse, baseReceipt }) {
    ...
    const beforePublish = async () => {
      ...
      if (baseReceipt) {
        const snapshot = await loadSnapshot(config.storeRoot, config.tenant, [documentId]);
        if (snapshot.documents[documentId]?.version_id !== baseReceipt.bundle.version_id) throw intakeError('rag_v2_source_changed', 409);
      }
    };
  ...
  async status() {
    ...
    const knowledge = this.config.knowledgePreparation ? await new KnowledgeJobs({ root: rootFor(this.config),
      config: this.config.knowledgePreparation, fingerprint: fingerprint(this.config) }).status() : { enabled: false };
    return { configured: true, cap_usd: this.config.maxSpendUsd, reserved_usd: formatUsd(budget.reserved_nano_usd),
      ...
      knowledge_preparation: knowledge };
  }
  async publish(jobId, reviewedHash) {
    ...
    try {
      ...
      // Before any paid reservation: the index this purchase feeds must suit the pilot's retrieval
      // profile (its lexical analysis) and fit the local index capacity.
      const profile = retrievalProfile(config.retrievalProfileId), lexical = profile.generation_requirements.lexical;
      try { assertProfileGeneration(profile, { config: searchConfig(prepared.manifest.config, lexical) }); } catch { throw intakeError('rag_v2_index_profile_mismatch', 409); }
      try { assertIndexCapacity(snapshotCapacity(snapshot)); } catch { throw intakeError('rag_v2_index_capacity_exceeded', 409); }
      ...
      const indexed = await (this.deps.indexSnapshot || indexSnapshot)({ snapshot, postgres: resources.postgres, qdrant: resources.qdrant,
        embedding: vectors.embedding, lexical, hooks: { beforeActivate: () => this.snapshotFor(receipt) } });
