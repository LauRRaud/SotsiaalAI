...
export function structure(parsed, scope, config) {
  ...
  const pages = parsed.pages.map(page => {
    ...
    const layout = orderPdfItems(orderedInput, page.view), ordered = layout.items;
    for (const item of ordered) {
      let row = rows.at(-1);
      if (row && (Math.abs(row.y - item.y) >= 2 || row.reading_lane !== item.reading_lane)) row = null;
      if (!row) { row = { y: item.y, reading_lane: item.reading_lane, items: [] }; rows.push(row); }
      row.items.push(item);
    }
    let raw = '';
    ...
    return { parser_page_index: page.parser_page_index, pdf_page: page.pdf_page, view: page.view, raw_text: raw, lines, columns: layout.columns,
  ...
  for (const page of pages) {
    for (const line of page.lines) {
      ...
      const rotatedMargin = Math.abs(line.rotation) > 5 && (line.x < page.view[0] + (page.view[2] - page.view[0]) * config.marginFraction
        || line.x > page.view[2] - (page.view[2] - page.view[0]) * config.marginFraction);
      if (rotatedMargin || marginal(line, page) && counts.get(key(line)) >= Math.max(2, Math.ceil(pages.length * 0.6))) {
        removed.push({ ...base, reason: rotatedMargin ? 'rotated_margin' : 'repeated_margin' });
      } else {
        for (const range of textRanges(line.text, config.chunkMaxChars)) {
          ...
          spans.push({ ...base, id: id('span', scope.document_version_id, page.pdf_page, line.start + range.start, line.start + range.end),
            ...
            transformation: line.nul_replacements ? 'pdf_nul_to_replacement_character_then_whitespace' : 'whitespace_only' });
        }
      }
    }
  }
  ...
  for (const span of spans) {
    ...
    previousSpan = span;
    if (heading) {
      if (previousHeading && previousHeading.pdf_page === span.pdf_page && previousHeading.reading_lane === span.reading_lane && Math.abs(previousHeading.y - span.y) < span.height * 1.6) {
  ...
  return { pages, source_units: pages.map((page, index) => sourceUnit(page.raw_text, { kind: 'pdf', pdf_page: page.pdf_page }, index, scope.document_version_id)), spans, sections, blocks, removed, warnings: pages.some(page => page.columns > 1) ? [{ code: 'pdf_column_order_detected', detail: 'Persistent gutter used to order two columns within vertical bands; complex tables and irregular layouts still require review.' }] : [], nulReplacements: pages.reduce((sum, page) => sum + page.nul_replacements, 0) };
}
