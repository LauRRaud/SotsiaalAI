...
export function structure(parsed, scope, config) {
  ...
  const pages = parsed.pages.map(page => {
    ...
    return { parser_page_index: page.parser_page_index, pdf_page: page.pdf_page, view: page.view, raw_text: raw, lines,
  ...
  for (const page of pages) {
    for (const line of page.lines) {
      ...
      if (marginal(line, page) && counts.get(key(line)) >= Math.max(2, Math.ceil(pages.length * 0.6))) {
        ...
        spans.push({ ...base, retrieval_text: line.text.replace(/\s+/gu, ' ').trim(),
  ...
  return { pages, spans, sections, blocks, removed, nulReplacements: pages.reduce((sum, page) => sum + page.nul_replacements, 0) };
}
