...
export function parsePdf(bytes, config) {
  return new Promise((resolve, reject) => {
    const worker = new Worker(new URL('./pdf-worker.js', import.meta.url), {
      workerData: { bytes: new Uint8Array(bytes), config }, execArgv: [],
      resourceLimits: { maxOldGenerationSizeMb: 256 },
    });
    let result, settled = false;
    const timer = setTimeout(() => { void worker.terminate(); finish('parser_timeout'); }, config.timeoutMs);
    ...
    worker.once('exit', code => finish(code !== 0 || !result ? 'parser_worker_failed' : result.error, result));
  });
}
