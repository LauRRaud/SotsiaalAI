...
function jsonRecords(text, metadata) {
  ...
  function addRecord(object, pointer, fallback) {
    ...
    for (const [key, value] of Object.entries(object)) {
      ...
      if (NON_CONTENT.has(key) && !linkField && !mappedField || value === null || value === '') continue;
      if (linkField && Array.isArray(value)) {
        for (const [index, target] of value.entries()) {
          ...
          records.push({ text: target, kind: 'paragraph', headings: [title, key, String(index)].filter(Boolean), group: pointer || '/',
