...
function textContent(node) {
  ...
  if (node.attribs?.hidden !== undefined || node.attribs?.['aria-hidden'] === 'true') return '';
  if (['script', 'style', 'noscript', 'template', 'svg'].includes(localName(node))) return '';
  return children(node).map(child => `${BREAKS.has(localName(child)) ? '\n' : ''}${textContent(child)}${BREAKS.has(localName(child)) ? '\n' : ''}`).join('');
}
...
function xmlRecords(text) {
  ...
  function visit(node, headings = []) {
    ...
    const title = value(node, 'pealkiri');
    const nextHeadings = title ? [...headings, title] : headings;
    if (tag === 'paragrahv') {
      ...
      records.push({ text: textContent(node).replace(/\r/gu, '').replace(/[ \t]+/gu, ' ').replace(/\n{3,}/gu, '\n\n').trim(),
...
const NON_CONTENT = new Set(['id', 'canonical_item_id', 'source_id', 'document_id', 'docId', 'schemaVersion', 'metadata_schema_version', 'content_hash',
  ...
  'related_forms', 'related_contacts', 'related_documents', 'allowed_claim_types', 'disallowed_claim_types']);
...
function jsonRecords(text, metadata) {
  ...
  function addRecord(object, pointer, fallback) {
    ...
    for (const [key, value] of Object.entries(object)) {
      if (NON_CONTENT.has(key) || value === null || value === '') continue;
      const text = typeof value === 'string' ? value : JSON.stringify(value, null, 2);
      if (!text?.trim()) continue;
      records.push({ text, kind: key === 'title' || key === 'name' ? 'heading' : 'paragraph', headings: [title, key].filter(Boolean), group: pointer || '/',
  ...
  return { records, metadata: {}, warnings: [{ code: 'collected_package_text', detail: 'JSON values preserve collected content; they are not a fresh retrieval of the referenced web pages.' }] };
}
...
export function parseTextSource(bytes, format, metadata, scope, config) {
  ...
  return { parsed: { info: {}, source_format: format, source_metadata: result.metadata, metadata_basis: result.metadata_basis, metadata_paths: result.metadata_paths },
    structured: { pages: [], source_units, spans, blocks, sections, removed: [], nulReplacements: 0, warnings: result.warnings } };
}
