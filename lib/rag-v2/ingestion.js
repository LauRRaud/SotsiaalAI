...
export async function ingest(options, dependencies = {}) {
  ...
  try {
    const metadataBytes = options.metadata === undefined ? await readInput(inputRoot, metadataFile, config.maxMetadataBytes) : Buffer.from(JSON.stringify(options.metadata));
