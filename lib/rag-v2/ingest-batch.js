...
export async function prepareNextBatchItem({ queue, tenant, batchId, inputRoot, storeRoot, leaseSeconds = 120 }, dependencies = {}) {
  ...
  try {
    ...
    // Only controlled codes are stored: our own, or an OS error name (EPERM -> io_eperm), never a message.
    const code = typeof error.code !== 'string' ? 'batch_preparation_failed' : /^[a-z][a-z0-9_]+$/.test(error.code) ? error.code
      : /^E[A-Z]{2,15}$/.test(error.code) ? `io_${error.code.toLowerCase()}` : 'batch_preparation_failed';
