...
import { sourceAsset, sourceHash, SOURCE_FORMATS } from './source-locations.js';
...
export async function loadVersion(dir, versionId) {
  ...
  const original = Object.values(SOURCE_FORMATS).map(value => value.asset).filter(name => Object.hasOwn(manifest.files, name));
  if (original.length !== 1) fail('invalid_version_manifest');
  const expected = ['bundle.json', original[0], 'metadata.json', 'provenance.json', 'chunks.json', 'spans.json', 'report.html'];
...
export async function stageVersion(dir, bundle, pdfBytes, metadataBytes) {
  ...
  if (exists) return loadVersion(dir, bundle.version.id);
  const staging = path.join(dir, `staging-${randomUUID()}`);
  await fs.mkdir(staging);
  const values = {
    'bundle.json': JSON.stringify(bundle, null, 2), [sourceAsset(bundle)]: pdfBytes, 'metadata.json': metadataBytes,
  ...
  try { await renameWhenReleased(staging, destination); }
  catch (error) {
    // Concurrent preparers may finish the same content-addressed version. Never replace it.
    if (!['EEXIST', 'ENOTEMPTY', 'EPERM'].includes(error.code)) throw error;
    const existing = await loadVersion(dir, bundle.version.id);
    // Only our UUID staging directory is removed; the winning immutable version is untouched.
    if (path.dirname(path.resolve(staging)) !== path.resolve(dir) || !/^staging-[a-f0-9-]{36}$/.test(path.basename(staging))) fail('invalid_staging_path');
...
export async function publish(dir, active, bundle) {
  const documents = { ...active.documents, [bundle.document.id]: {
    ...
    source_selector: bundle.document.legacy_metadata.source_selector?.json_pointer ?? null, external_id: bundle.document.external_ids.document_id,
  } };
  ...
  return writeActive(dir, next);
}

/** Caller must hold openCatalog's writer lock. One rename exposes the entire generation. */
export async function writeActive(dir, next) {
