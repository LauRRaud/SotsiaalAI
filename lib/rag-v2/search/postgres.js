...
import { LEGACY_LEXICAL, MORPHOLOGY_LEXICAL, morphologyFields, morphologyText } from './morphology.js';
...
export class PostgresCatalog {
  ...
  async importSnapshot(snapshot, generationId, units, { partial = false } = {}) {
    return this.transaction(async client => {
      const generation = (await client.query('SELECT config,snapshot FROM rag_v2_generation WHERE tenant=$1 AND id=$2', [snapshot.tenant, generationId])).rows[0];
      if (!generation || ![LEGACY_LEXICAL, MORPHOLOGY_LEXICAL].includes(generation.config.lexical)) fail('unknown_lexical_config');
      ...
      const multilingual = generation.config.lexical === MORPHOLOGY_LEXICAL;
      ...
      for (const unit of units) {
        await client.query(`INSERT INTO rag_v2_unit(tenant,generation_id,id,document_id,version_id,chunk_id,ordinal,data,title,authors,body,search_aids,morphology)
          ...
          [snapshot.tenant, generationId, unit.id, unit.document_id, unit.version_id, unit.chunk_id, unit.ordinal, unit, unit.title, unit.authors, unit.body, unit.search_aids,
            multilingual ? morphologyFields(unit) : {}]);
      }
      ...
      if (actual.length !== units.length || actual.some(u => expected.get(u.id) !== stable(u.data)
        || stable(u.morphology) !== stable(multilingual ? morphologyFields(u.data) : {}))) fail('index_unit_integrity_failed');
    });
  }
  ...
  async units(tenant, generationId, documentIds) {
    return (await this.pool.query(`SELECT u.*,g.config->>'lexical' AS lexical FROM rag_v2_unit u JOIN rag_v2_generation g ON g.tenant=u.tenant AND g.id=u.generation_id
      WHERE u.tenant=$1 AND u.generation_id=$2 AND u.document_id=ANY($3::text[]) ORDER BY u.document_id,u.ordinal,u.id`, [tenant, generationId, documentIds])).rows.map(r => {
      ...
      if (r.lexical === MORPHOLOGY_LEXICAL && stable(r.morphology) !== stable(morphologyFields(r.data))) fail('index_morphology_integrity_failed');
      return r.data;
    });
  }
  ...
  async lexical(tenant, generationId, documentIds, text, limit, unitIds = null) {
    ...
    if (lexical === MORPHOLOGY_LEXICAL) return (await this.pool.query(`WITH q AS (
      ...
      ORDER BY score DESC,u.id COLLATE "C" ASC LIMIT $5`, [tenant, generationId, documentIds, text, limit, unitIds, morphologyText(text)])).rows;
    if (lexical !== LEGACY_LEXICAL) fail('unknown_lexical_config');
