...
import { retrievalDirectory, verifyDirectory } from './discovery.js';
...
export class PostgresCatalog {
  ...
  async importSnapshot(snapshot, generationId, units, { partial = false } = {}) {
    const generation = (await this.pool.query('SELECT config,snapshot FROM rag_v2_generation WHERE tenant=$1 AND id=$2', [snapshot.tenant, generationId])).rows[0];
    ...
    return this.transaction(async client => {
      const current = (await client.query('SELECT config,snapshot FROM rag_v2_generation WHERE tenant=$1 AND id=$2', [snapshot.tenant, generationId])).rows[0];
      ...
      for (const b of snapshot.bundles) {
        ...
        const directory = retrievalDirectory(b, generation.config.embedding), directoryHash = hash(stable(directory));
      ...
      for (const unit of units) {
        await client.query(`INSERT INTO rag_v2_unit(tenant,generation_id,id,document_id,version_id,chunk_id,ordinal,data,title,authors,body,search_aids,morphology)
          ...
          [snapshot.tenant, generationId, unit.id, unit.document_id, unit.version_id, unit.chunk_id, unit.ordinal, unit, unit.title, unit.authors, unit.body, unit.search_aids,
            morphology.get(unit.id)]);
      }
      const actual = (await client.query(`SELECT id,data,morphology FROM rag_v2_unit WHERE tenant=$1 AND generation_id=$2
  ...
  async bundles(tenant, generationId, documentIds) {
    const rows = (await this.pool.query(`SELECT v.*,d.retrieval_directory,d.retrieval_hash,g.config FROM rag_v2_generation_document d JOIN rag_v2_version v
    ...
    const bundles = rows.map(r => {
      ...
      if (r.retrieval_directory !== null && (hash(stable(r.retrieval_directory)) !== r.retrieval_hash
        || stable(retrievalDirectory(r.bundle, r.config.embedding)) !== stable(r.retrieval_directory))) fail('retrieval_directory_integrity_failed');
    ...
    const expected = new Map(bundles.flatMap(b => [b.document, b.version, ...b.assets, ...b.sections, ...b.blocks, ...b.spans, ...b.chunks,
  ...
  async retrievalDirectory(tenant, generation, documentIds) {
    const rows = (await this.pool.query(`SELECT d.document_id,d.version_id,d.retrieval_directory,d.retrieval_hash,v.bundle_hash
    ...
    if (rows.length !== documentIds.length) fail('missing_generation_source');
    // Do not hide corruption behind a legacy fallback when some documents have directories.
    const directories = rows.map(row => row.retrieval_directory === null && row.retrieval_hash === null ? null : verifyDirectory(row, generation));
    return directories.some(directory => directory === null) ? null : directories;
  }
  async units(tenant, generationId, documentIds) {
    const rows = (await this.pool.query(`SELECT u.*,g.config->>'lexical' AS lexical FROM rag_v2_unit u JOIN rag_v2_generation g ON g.tenant=u.tenant AND g.id=u.generation_id
      WHERE u.tenant=$1 AND u.generation_id=$2 AND u.document_id=ANY($3::text[]) ORDER BY u.document_id,u.ordinal,u.id`, [tenant, generationId, documentIds])).rows;
    for (const r of rows) {
      ...
      if (r.lexical !== rows[0].lexical) fail('index_morphology_integrity_failed');
    }
    const fields = rows.length ? await lexicalFields(rows.map(r => r.data), rows[0].lexical, this.analyzer) : [];
    if (rows.some((r, index) => stable(r.morphology) !== stable(fields[index]))) fail('index_morphology_integrity_failed');
  ...
  async canonicalReferences(references, { checkBundle } = {}) {
    ...
    const generation = (await this.pool.query("SELECT snapshot,config FROM rag_v2_generation WHERE tenant=$1 AND id=$2 AND state='ready'", [first.tenant, first.generation_id])).rows[0];
    ...
    const units = (await this.pool.query(`SELECT id,document_id,version_id,chunk_id,data FROM rag_v2_unit
    ...
    return references.map(reference => {
      ...
      return { tenant: reference.tenant, query_id: reference.query_id, generation_id: reference.generation_id,
        ...
        ...locationFields(chunk), pdf_pages: chunk.pdf_pages, source_text_sha256: hash(chunk.source_text) };
    });
  }
  async lexical(tenant, generationId, documentIds, text, limit, unitIds = null) {
    if (!documentIds.length || !text.trim()) return [];
    const lexical = (await this.pool.query('SELECT config FROM rag_v2_generation WHERE tenant=$1 AND id=$2', [tenant, generationId])).rows[0]?.config.lexical;
    if (!LEXICAL_CONFIGS.includes(lexical)) fail('unknown_lexical_config');
    if (lexical !== LEGACY_LEXICAL) return (await this.pool.query(`WITH q AS (
      ...
      SELECT u.id,ts_rank_cd(u.search_vector,q.exact) + 0.35 * ts_rank_cd(u.morphology_vector,q.stems) AS score
      FROM rag_v2_unit u,q WHERE u.tenant=$1 AND u.generation_id=$2 AND u.document_id=ANY($3::text[])
        AND (u.search_vector @@ q.exact OR u.morphology_vector @@ q.stems) AND ($6::text[] IS NULL OR u.id=ANY($6::text[]))
      ORDER BY score DESC,u.id COLLATE "C" ASC LIMIT $5`, [tenant, generationId, documentIds, text, limit, unitIds, await lexicalQuery(text, lexical, this.analyzer)])).rows;
