...
export class PostgresCatalog {
  ...
  async lexical(tenant, generationId, documentIds, text, limit) {
    ...
    return (await this.pool.query(`WITH q AS (SELECT replace(plainto_tsquery('pg_catalog.simple',$4)::text,' & ',' | ')::tsquery AS query)
      ...
      WHERE u.tenant=$1 AND u.generation_id=$2 AND u.document_id=ANY($3::text[]) AND u.search_vector @@ q.query
      ORDER BY score DESC,u.id COLLATE "C" ASC LIMIT $5`, [tenant, generationId, documentIds, text, limit])).rows;
  }
