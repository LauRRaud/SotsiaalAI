  ...
  try {
    ...
    if (query.text && documentIds.length && eligibleIds.length) {
      const [lexical, vector] = await Promise.allSettled([
        query.method === 'vector' ? Promise.resolve([]) : time('lexical', () => postgres.lexical(context.tenant, generation.id, documentIds,
          query.lexicalStopwords ? withoutStopwords(query.text) : query.text, query.limits.candidates, eligibleIds)),
    ...
    const mergeStart = performance.now(), ranked = rrf(channels, generation.config.rrf_constant, query.channelWeights);
