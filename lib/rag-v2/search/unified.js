...
function coverage(directories, packet, filters = {}) {
  ...
  for (const row of eligible) {
    const year = row.fields.publication_date.value?.slice(0, 4);
  ...
  return { counting_unit: 'indexed_source_document', indexed_documents: eligible.length,
    ...
    missing_publication_date_documents: directories.filter(row => !row.fields.publication_date.value).length,
