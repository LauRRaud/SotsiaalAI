...
export class StructuredRecordSource {
  ...
  async retrieve({ context, region, generationId, recordIds = [], kinds = ['service', 'benefit', 'resource'], question = null, limits: overrides = {} }) {
    ...
    // channel (same contract and analyzer as the main search) over this region's record units.
    // Function words are dropped first (ADR-024): they matched nearly every record.
    const relevance = new Map(), terms = question === null ? '' : withoutStopwords(question);
    if (terms) {
      const recordOf = new Map();
      for (const record of catalogue) for (const chunk of record.bundle.chunks) recordOf.set(indexUnit(chunk, record.bundle, generation.config.embedding).id, record);
      const rows = recordOf.size ? await this.postgres.lexical(context.tenant, generation.id, catalogue.map(record => record.bundle.document.id),
        terms, recordOf.size, [...recordOf.keys()]) : [];
      for (const row of rows) {
        const record = recordOf.get(row.id);
        if (!record) fail('record_relevance_outside_scope');
        relevance.set(record, Math.max(relevance.get(record) || 0, Number(row.score)));
      }
    }
    ...
    const project = (view, listed, summarized = new Set()) => {
      ...
      const packet = { schema_version: 'rag-v2/evidence-1', tenant: context.tenant, query_id: id('query', context.tenant, randomUUID()),
        ...
        record_context: { version: RECORD_RETRIEVAL_VERSION, region, view, entries, relations: linked,
          ...
          selection: question === null ? 'stable_id' : 'question_relevance', relevant_summaries: summarized.size,
