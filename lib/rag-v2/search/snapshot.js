...
import { sourceAsset, sourceHash } from '../source-locations.js';
...
export function verifiedBundle(bundle, tenant) {
  ...
  for (const chunk of bundle.chunks) {
    const spanPages = [...new Set(chunk.span_ids.map(s => bundle.spans.find(x => x.id === s).pdf_page).filter(Number.isInteger))];
    ...
    if (bundle.schema_version === 'rag-v2/2') for (const neighbor of [chunk.previous_id, chunk.next_id].filter(Boolean)) {
      if (chunks.get(neighbor).record_key !== chunk.record_key) fail('source_neighbor_mismatch');
    }
  }
  return bundle;
}
export async function loadSnapshot(store, tenant, allowedDocuments) {
  ...
  return loadPinnedSnapshot(store, tenant, active.generation, selected);
}

/** Read exact immutable versions from a previously validated, frozen index plan. */
export async function loadPinnedSnapshot(store, tenant, sourceGeneration, selectedDocuments) {
  tenantId(tenant);
  if (!/^generation_[a-f0-9]{64}$/.test(sourceGeneration) || !selectedDocuments || Array.isArray(selectedDocuments)) fail('invalid_source_generation');
  const dir = path.resolve(store, id('tenant', tenant));
  const bundles = [], documents = {}, assets = {};
  for (const documentId of Object.keys(selectedDocuments).sort()) {
    const selected = selectedDocuments[documentId];
    ...
    if (bundle.document.id !== documentId || sourceHash(bundle) !== (selected.source_hash ?? selected.pdf_hash)) fail('source_identity_mismatch');
    const manifest = await readJson(path.join(dir, 'versions', selected.version_id, 'manifest.json'));
    if (manifest.files[sourceAsset(bundle)] !== sourceHash(bundle) || manifest.files['metadata.json'] !== bundle.version.metadata_hash
  ...
  return { tenant, source_generation: sourceGeneration, documents, bundles, assets, snapshot_hash: hash(stable(documents)) };
}
