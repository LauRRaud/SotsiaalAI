  ...
  async verify(generation, units, vectors) {
    ...
    do {
      ...
      for (const point of page.points) {
        ...
        if (!u || p.tenant !== generation.tenant || p.generation_id !== generation.id || p.config_id !== generation.config.id || p.embedding_mode !== generation.config.embedding.embedding_mode
        ...
        validateVector(point.vector, generation.config.embedding);
        const wanted = vectorMap.get(point.id), norm = Math.hypot(...wanted), actualNorm = Math.hypot(...point.vector);
        if (point.vector.some((v, i) => Math.abs(v / actualNorm - wanted[i] / norm) > 0.00001)) fail('qdrant_vector_content_mismatch');
