...
import { assertIndexCapacity } from './capacity.js';
...
/** Directory metadata changes do not change the vector space. Import an exact cache row of
 * any earlier directory schema (newest first) into the new immutable namespace, preserving
 * its validation: a v2 row was previously skipped, so v3 re-embedded paid vectors. */
export async function cachedIndexVector({ postgres, tenant, rights, config, unit, embedding, readOnly = false }) {
  ...
  const keyFor = directory => directory && directory !== LEGACY_DISCOVERY_SCHEMA ? id('directory_embedding_cache', previousKey, directory) : previousKey;
  const key = keyFor(config.directory);
  let vector = await postgres.cacheGet(tenant, key, config.id, unit.input_hash), imported = false;
  const earlier = config.directory ? DISCOVERY_SCHEMAS.slice(0, DISCOVERY_SCHEMAS.indexOf(config.directory)).reverse() : [];
  for (const directory of readOnly ? [] : earlier) {
    if (vector) break;
    vector = await postgres.cacheGet(tenant, keyFor(directory), searchConfig(config.embedding, config.lexical, directory).id, unit.input_hash);
...
export async function indexSnapshot({ snapshot, postgres, qdrant, embedding, lexical = LEGACY_LEXICAL, directory = DISCOVERY_SCHEMA, hooks = {} }) {
  ...
  assertIndexCapacity({ documents: snapshot.bundles.length, units: units.length });
