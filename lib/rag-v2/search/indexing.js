...
import { DISCOVERY_SCHEMA, LEGACY_DISCOVERY_SCHEMA } from './discovery.js';
...
export function searchConfig(embedding, lexical = LEGACY_LEXICAL, directory = DISCOVERY_SCHEMA) {
  ...
  if (![DISCOVERY_SCHEMA, LEGACY_DISCOVERY_SCHEMA].includes(directory)) fail('unsupported_retrieval_directory');
  const config = { embedding: embeddingConfig(embedding), lexical, ranking: 'rrf-v1', rrf_constant: 60,
    ...(directory === DISCOVERY_SCHEMA ? { directory } : {}) };
