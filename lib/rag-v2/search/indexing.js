...
import { LEGACY_LEXICAL, MORPHOLOGY_LEXICAL } from './morphology.js';

export function searchConfig(embedding, lexical = LEGACY_LEXICAL) {
  if (![LEGACY_LEXICAL, MORPHOLOGY_LEXICAL].includes(lexical)) fail('unknown_lexical_config');
...
export function verifySearchConfig(config) {
  if (stable(searchConfig(config.embedding, config.lexical)) !== stable(config)) fail('search_config_mismatch');
}
export async function indexSnapshot({ snapshot, postgres, qdrant, embedding, lexical = LEGACY_LEXICAL, hooks = {} }) {
  assertSnapshot(snapshot);
  const config = searchConfig(embedding.config, lexical);
  ...
  for (const unit of units) {
    ...
    const baseKey = cacheKey(snapshot.tenant, rights, config.embedding, unit.input_text);
    // Legacy entries bind the entire search configuration. Keep them readable;
    // a new lexical generation can re-import existing persisted real vectors.
    const key = config.lexical === LEGACY_LEXICAL ? baseKey : id('lexical_embedding_cache', baseKey, config.lexical);
    let vector = await postgres.cacheGet(snapshot.tenant, key, config.id, unit.input_hash);
    if (vector && config.embedding.embedding_mode === 'real' && stable(vector) !== stable(await embedding.embed(unit.input_text))) fail('embedding_cache_integrity_failed');
    if (vector) hits++;
