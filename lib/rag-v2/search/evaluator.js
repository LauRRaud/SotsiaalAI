...
/** queryOptions carry a profile's query-time choices (channel weights, query stopwords) into the fixed methods. */
export async function evaluateRetrieval({ snapshot, questions, groups, postgres, qdrant, embedding, policy, context, queryOptions = {} }) {
  const { limits: limitOverrides = {}, ...options } = queryOptions;
  ...
  for (const question of questions.cases) for (const method of methods) {
    ...
    const packet = await retrieve({ postgres, qdrant, embedding, policy, context, allowLexicalFallback: false,
      query: { text: question.query, language: question.language, method: method.method, graph: method.graph,
        includeDocumentLabels: false, contextMode: 'compact', finalLimit: 5, ...options,
        limits: { topK: method.topK, perDocument: 5, contextTokens, graphAdditions: 2, graphSteps: 8, candidates: 40, ...limitOverrides } } });
