...
export const DISCOVERY_SCHEMA = 'rag-v2/retrieval-directory-1';
...
export function retrievalDirectory(bundle, config) {
  ...
  return { schema_version: DISCOVERY_SCHEMA, document_id: bundle.document.id, version_id: bundle.version.id,
...
export function verifyDirectory(row, generation) {
  ...
  if (!directory || directory.schema_version !== DISCOVERY_SCHEMA || hash(stable(directory)) !== row.retrieval_hash
