...
import { REVERSE_DEPENDENCIES } from './discovery.js';

const reverseTypes = REVERSE_DEPENDENCIES;
...
export async function expandDependencies({ bundles, units, selected, add, limits, loadDocument, incomingDocuments, documentVersions }) {
  const cards = new Map(), documents = new Map(), outgoing = new Map(), incoming = new Map(), bySpan = new Map();
  const gapsByCard = new Map(), unlinkedGaps = [], selectedGaps = new Map();
  const push = (map, key, value) => { if (!map.has(key)) map.set(key, []); map.get(key).push(value); };
  function register(bundle, loadedUnits) {
    ...
    for (const gap of bundle.knowledge_gaps || []) {
      const owned = { ...gap, document_id: bundle.document.id };
      if (gap.from_card_id) push(gapsByCard, gap.from_card_id, owned); else unlinkedGaps.push(owned);
    }
  ...
  for (const bundle of bundles) register(bundle, units.filter(unit => unit.document_id === bundle.document.id));
  ...
  let steps = 0, additions = 0, documentsLoaded = 0;
  async function ensureDocument(documentId, versionId) {
    if (documents.get(documentId) === versionId) return true;
    if (!loadDocument || documentVersions.get(documentId) !== versionId) return false;
    if (documentsLoaded >= limits.dependencySteps) { unresolved.push({ reason: 'dependency_document_limit' }); return false; }
    const loaded = await loadDocument(documentId, versionId);
    if (!loaded) return false;
    documentsLoaded++; register(loaded.bundle, loaded.units); return true;
  }
  ...
  function includeGap(gap) {
    if (selectedGaps.has(gap.id)) return;
    if (selectedGaps.size >= limits.dependencySteps) { unresolved.push({ reason: 'dependency_gap_limit' }); return; }
    selectedGaps.set(gap.id, gap); includeSource(gap);
  }
  for (const gap of unlinkedGaps.filter(gap => gap.span_ids.some(span => initialSpans.has(spansKey(gap.document_version_id, span))))
    .sort((a, b) => a.id.localeCompare(b.id))) includeGap(gap);
  while (queue.length && steps < limits.dependencySteps) {
    ...
    if (!card) continue;
    // An exception may live in a document with no lexical/vector hit. Consult
    // only version-pinned, policy-filtered reverse addresses before traversing.
    for (const documentId of incomingDocuments?.(card) || []) {
      if (!await ensureDocument(documentId, documentVersions.get(documentId))) {
        unresolved.push({ card_id: cardId, reason: 'dependency_context_unavailable' });
      }
    }
    includeSource(card);
    for (const gap of (gapsByCard.get(cardId) || []).sort((a, b) => a.id.localeCompare(b.id))) includeGap(gap);
    for (const edge of [...new Map([...(outgoing.get(cardId) || []), ...(incoming.get(cardId) || [])].map(edge => [edge.id, edge])).values()].sort((a, b) => a.id.localeCompare(b.id))) {
      ...
      for (const target of next) {
        if (!await ensureDocument(target.document_id, target.document_version_id) || !cards.has(target.card_id)) {
  ...
  return { cards, edges: [...edges.values()], gaps: [...selectedGaps.values()], relevant, unresolved, steps, additions };
}
...
export function dependencyContext(plan, evidence, allowedDocuments) {
  ...
  for (const gap of plan.gaps || []) {
    if (!allowed.has(gap.document_id)) continue;
    const refs = refsFor(gap), from = keys.get(gap.from_card_id);
    unresolved.push({ ...(from ? { from } : {}), reason: refs.length ? 'source_dependency_unresolved' : 'source_dependency_context_missing',
      ...(refs.length ? { statement: gap.statement, detail: gap.reason, refs, verification_state: gap.verification_state } : {}) });
  }
