...
export function retrievalProfile(profileId = DEFAULT_RETRIEVAL_PROFILE) {
  ...
  const dependencies = multilingual || profileId.includes('-source-dependencies-');
  return {
    schema_version: 'rag-v2/retrieval-profile-1', id: profileId, selection_policy: RANKED_FIRST_POLICY,
    generation_requirements: { ranking: 'rrf-v1', rrf_constant: 60, lexical: multilingual ? MORPHOLOGY_LEXICAL : LEGACY_LEXICAL },
    ...
    query: {
      ...
      limits: { topK: 5, perDocument: dependencies ? 9 : expanded ? 7 : 5, candidates: 40, contextTokens: 6000, graphSteps: 8, graphAdditions: 2,
