...
import { searchConfig, verifySearchConfig } from './indexing.js';
import { cacheKey, indexUnit, validateVector } from './embedding.js';
import { LEGACY_LEXICAL } from './morphology.js';
...
export function validateIndexPlan(plan) {
  ...
  if (!/^generation_[a-f0-9]{64}$/.test(plan.source?.source_generation) || !plan.source.documents || Array.isArray(plan.source.documents)
    ...
    || !Array.isArray(plan.items) || !plan.items.length || plan.items.length > 1000) fail('invalid_index_plan');
  ...
  for (const item of plan.items) {
    if (!item || !/^document_[a-f0-9]{64}$/.test(item.document_id) || !/^version_[a-f0-9]{64}$/.test(item.version_id)
      ...
      || !digest(item.bundle_sha256) || !digest(item.units_sha256) || !integer(item.units, 5000)
      || !integer(item.embedding_input_tokens, 5000 * 8191)) fail('invalid_index_plan');
    seen.add(item.document_id); count += item.units;
  }
  if (count > 5000) fail('local_index_limit');
...
export async function planIndexJob({ storeRoot, tenant, documents: allowed, embedding, lexical = LEGACY_LEXICAL }) {
  tenantId(tenant);
  if (!Array.isArray(allowed) || !allowed.length || allowed.length > 1000) fail('allowed_documents_required');
...
async function vectorsFor(units, bundle, plan, postgres, embedding, counters, readOnly = false) {
  ...
  for (const unit of units) {
    const baseKey = cacheKey(plan.tenant, bundle.document.rights, plan.config.embedding, unit.input_text);
    const key = plan.config.lexical === LEGACY_LEXICAL ? baseKey : id('lexical_embedding_cache', baseKey, plan.config.lexical);
    let vector = await postgres.cacheGet(plan.tenant, key, plan.config.id, unit.input_hash);
    if (vector && plan.config.embedding.embedding_mode === 'real' && stable(vector) !== stable(await embedding.embed(unit.input_text))) fail('embedding_cache_integrity_failed');
