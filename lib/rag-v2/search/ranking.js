...
export function validateChannelWeights(weights) {
  if (!weights || typeof weights !== 'object' || Array.isArray(weights)
    || Object.entries(weights).some(([channel, value]) => !['lexical', 'vector'].includes(channel) || !Number.isFinite(value) || value <= 0 || value > 4)) fail('invalid_channel_weights');
}
...
export function validateQuery(query) {
  ...
  return { ...query, text: query.text.trim(), filters, limits, graph: query.graph ?? false, semanticGraph: query.semanticGraph ?? false,
