...
export async function runPilot({ prepared, approval, price, policy, context, root, execute = false, apiKey, transport, onProgress }) {
  ...
  const send = transport || openAITransport(apiKey); // Missing credentials fail before any reservation.
  ...
  try {
    ...
    try { ledger = await readJson(file); }
      ...
      transport: transport ? 'test_transport' : 'openai_https', state: 'running', reserved_attempts: 0, reserved_tokens: 0, reserved_nano_usd: '0', entries: [] }; }
    if (ledger.manifest_sha256 !== prepared.manifest_sha256 || hash(stable(ledger.manifest)) !== prepared.manifest_sha256 || ledger.approval_hash !== hash(stable(approval)) || ledger.price_hash !== hash(stable(price))
      || ledger.tenant !== context.tenant || stable(ledger.config) !== stable(prepared.manifest.config) || ledger.transport !== (transport ? 'test_transport' : 'openai_https')) fail('pilot_ledger_scope_mismatch');
