...
export function modelProjection(entries, scope) {
  ...
  for (const [index, entry] of entries.entries()) {
    ...
    const ref = `S${index + 1}`;
    evidence.push({ ref, source, pdf_pages: entry.pdf_pages, ...(entry.pdf_pages.length ? {} : { source_locations: entry.source_locations?.map(({ kind, path, act_reference, record_id }) => ({ kind, path, ...(act_reference ? { act_reference } : {}), ...(record_id ? { record_id } : {}) })) }), text: entry.source_text });
  ...
  const context = entries.length ? { schema_version: MODEL_SERIALIZER, sources, evidence,
    ...(scope.dependency_context ? { dependencies: scope.dependency_context } : {}),
    ...(scope.record_context ? { records: scope.record_context } : {}) } : scope.record_context ? { schema_version: MODEL_SERIALIZER, sources, evidence, records: scope.record_context } : null;
