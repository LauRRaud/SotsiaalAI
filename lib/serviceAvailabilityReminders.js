...
async function writeAudit(db, service, action, reason, extra = {}) {
  await db.dataAuditLog.create({
    data: {
      // Org-profiili toiming ei ole ühegi eraisiku kohta käiv auditikirje —
      // `targetUserId` on nullable just selleks. Organisatsiooni jälg elab
      // `meta.organizationId`-s, mitte inimese all.
      targetUserId: resolveProviderAuditTargetUserId(service.providerProfile),
      action,
      ...
      meta: {
        ...
        organizationId: resolveProviderRecipientOrganizationId(service.providerProfile),
        ...extra
      }
    }
  });
...
export async function dispatchServiceAvailabilityReminders(options = {}) {
  ...
  for (const service of services) {
    ...
    /* ORGANIZATION-profiilil kasutatakse AINULT profiili enda avalikku
       aadressi. Looja isiklik e-post ei ole varuvariant — meeldetuletus
       „kinnita oma teenuseprofiilis" saadetuna inimesele, kes on
       organisatsioonist lahkunud, on korraga vale adressaat ja vale ülesanne
       (tal ei ole enam õigust seda profiili toimetada). */
    const recipient = cleanEmail(resolveProviderRecipientEmail(service.providerProfile));
    let missingReason = "";
...
export async function listServiceAvailabilityAdminRows(options = {}) {
  ...
  return services.map((service) => {
    ...
    return {
      ...
      /* Eelvaade peab näitama SEDA, mida saatja päriselt teeb. Kui siin oleks
         endiselt toores `ownerId`/omaniku e-post, näeks admin org-profiili real
         profiili LOOJA isiklikku aadressi — korraga vale ootus ja eraisiku
         aadressi leke organisatsiooni konteksti. */
      ownerId: resolveProviderRecipientUserId(service.providerProfile) || "",
      ownerEmail: resolveProviderRecipientEmail(service.providerProfile),
      organizationId: resolveProviderRecipientOrganizationId(service.providerProfile),
      availability,
