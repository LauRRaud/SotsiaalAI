...
export async function upsertServiceProviderProfileForOwner(ownerId, input) {
  ...
  const serviceLocations = normalizeServiceProviderLocationsInput(input, normalizedBase, suggestionOptions);
  ...
  try {
    if (savedRagJobId) {
      await processServiceProviderProfileRagJobs({ db: prisma, jobId: savedRagJobId, limit: 1 });
      return prisma.serviceProviderProfile.findUnique({
