...
import { processServiceProviderProfileRagJobs, queueServiceProviderProfileRagJob } from "@/lib/serviceProviderProfileRagJobs";
...
export async function reconcileServiceProviderProfileRagJobs() {
  return { checked: 0, consistent: 0, drifted: 0, queued: 0, reason: "rag_retired" };
}
...
async function syncServiceProviderProfileToRag(profile = {}) {
  ...
  if (!RAG_AVAILABLE) return profile;
...
const LICENCE_RELEVANT_SERVICE_FIELDS = [
  "name",
  "description",
  "longDescription",
  "includesText",
...
function licenceRelevantServiceChanged(previous, next) {
  if (!previous) return false;
  return LICENCE_RELEVANT_SERVICE_FIELDS.some((field) => stableJson(previous[field]) !== stableJson(next[field]));
}
  ...
  for (let attempt = 1; attempt <= MAX_PROFILE_SAVE_ATTEMPTS; attempt += 1) {
    try {
      const transactionResult = await db.$transaction(async (tx) => {
        ...
        const licenceIdentityChanged = Boolean(existing) && (
          existing.registryCode !== normalized.registryCode ||
          existing.organizationName !== normalized.organizationName
        );
    ...
    for (const item of serviceItems) {
      ...
      /* Loahinnang tõendab konkreetset registriidentiteeti ja teenuse
         tähendust. Rea ID ning admini seotus võivad säilida, kuid vana avalik
         tõend ei tohi liikuda uue registrikoodi, organisatsiooni või ümber
         kirjutatud teenuse külge. Kontakt- ja kättesaadavusparandused seda
         tõendit ei muuda ning jätavad hinnangu alles. */
      if (previous?.id && (licenceIdentityChanged || licenceRelevantServiceChanged(previous, serviceData))) {
        await tx.serviceLicenceAssessment.deleteMany({ where: { providerServiceId: previous.id } });
      }
    ...
    await tx.serviceMapEntry.upsert({
      ...
      update: {
        ...
        let ragJobId = null;
        if (RAG_AVAILABLE && finalProfile.status === "PUBLISHED" && finalProfile.assistantRecommendationAllowed === true) {
...
async function buildPublishedServiceMapFilters({
  keyword = "",
  municipalityId = "",
  municipalityName = "",
  county = "",
  ...
  if (!includeNeedsReview) {
    filters.AND = [await buildPublicServiceMapContactEligibilityClause(db)];
  }
  if (entryId) filters.id = entryId;

  if (!includeUnlocated || mapOnly) {
    filters.AND = [
      ...(filters.AND || []),
      {
        OR: [
          ...(!mapOnly ? [{
            type: { in: ["KOV_SOCIAL_CONTACT", "KOV_GENERAL_CONTACT"] }
          }] : []),
          {
...
export async function listPublishedServiceMapEntries({
  ...
  const filters = await buildPublishedServiceMapFilters({
    keyword,
    municipalityId,
    municipalityName,
    county,
...
function serializeServiceMapMarkerEntry(entry) {
  return {
    id: entry.id,
    type: entry.type,
    title: entry.title,
    ...
    longitude: entry.longitude,
    adsObjectId: entry.adsObjectId,
    isLightweightMarker: true,
    serviceActions: [],
    canStartPreInquiry: false
  };
}

/**
 * Avaliku kaardi markerite kerge lehekülg. Filtrid on ühised täisloendiga,
 * kuid päring ei lae profiili tekstimassiivi, loahinnangut ega muid detailvaate välju.
 */
export async function listPublishedServiceMapMarkers(query = {}, db = prisma) {
  const markerQuery = {
    ...query,
    includeUnlocated: false,
    mapOnly: true
  };
  const filters = await buildPublishedServiceMapFilters(markerQuery, db);
  const take = Math.max(1, Math.min(Number(markerQuery.limit) || 200, 2000));
  const rows = await db.serviceMapEntry.findMany({
    where: filters,
    take: take + 1,
    orderBy: [{ title: "asc" }, { id: "asc" }],
    select: serviceMapMarkerSelect
  });
  const hasMore = rows.length > take;
  const entries = hasMore ? rows.slice(0, take) : rows;
  const projected = entries.flatMap((entry) => splitServiceLocationMapEntries({
    ...serializeServiceMapMarkerEntry(entry),
    providerProfile: entry.providerProfile || null
  }));
  const last = entries.at(-1);
  ...
  return {
    ...
    page: {
      ...
      nextCursor: hasMore && last
        ? encodeServiceMapCursor({ kind: "service", title: last.title, id: last.id }, markerQuery)
        : null,
      returnedBaseCount: entries.length,
      returnedCount: projected.length,
      truncated: hasMore
