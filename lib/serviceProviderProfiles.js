    ...
    for (const item of serviceItems) {
      ...
      const service = await tx.serviceProviderService.create({
        data: {
          ...
          ...serviceData,
          /* A4: profiili salvestamine teeb teenustele delete + create. Ilma
             selle reata kaoks `serviceKey` IGA salvestusega ära ja koos
             teenuse reaga kustuks kaskaadis ka kogu loahinnang — osutaja
             kaotaks märgise iga kord, kui ta kirjavea parandab.
             `serviceKey` EI OLE profiilivormi väli: teda muudetakse ainult
             eraldi sidumisoperatsiooniga, seega siin ta ainult SÄILIB. */
          serviceKey: previous?.serviceKey ?? null,
          providerProfileId: profile.id,
