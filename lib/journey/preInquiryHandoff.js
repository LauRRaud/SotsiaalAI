...
const MAX_JOURNEY_SHARE_KEY_LENGTH = 64;

function invalidJourneyShareKeys(message) {
...
export function partitionJourneyShareKeys(value) {
  if (!Array.isArray(value)) {
    throw invalidJourneyShareKeys("shareKeys must be an array");
  }
  if (value.length > JOURNEY_SHARE_KEYS.length) {
    throw invalidJourneyShareKeys("shareKeys contains too many entries");
  }
  const confirmedKeys = [];
  const ignoredKeys = [];
  ...
  for (const item of value) {
    ...
    if (rawKey.length > MAX_JOURNEY_SHARE_KEY_LENGTH) {
      throw invalidJourneyShareKeys("shareKeys contains an oversized entry");
    }
    const key = rawKey.trim();
    if (!key) continue;
    const isConfirmed = JOURNEY_SHARE_KEY_SET.has(key);
    const target = isConfirmed ? confirmedKeys : ignoredKeys;
    const seen = isConfirmed ? confirmedKeySet : ignoredKeySet;
    if (!seen.has(key)) {
      seen.add(key);
      target.push(key);
    }
