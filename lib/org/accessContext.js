...
const WRITABLE_ORG_STATUSES = new Set([OrganizationStatus.ACTIVE]);
 ...
 */
const VISIBLE_ORG_STATUSES = new Set([
...
async function resolvePayerSource(userId, { db, now }) {
  ...
  if (!subscription) return PayerSource.SELF;
  return subscription.billingSource === "SPONSORED_BY_HOST"
    ? PayerSource.INDIVIDUAL_SPONSOR
    : PayerSource.SELF;
}
...
export async function resolveOrgAccessContext(
  ...
  const payerSource = await resolvePayerSource(trimmedUserId, { db, now });
  ...
  return Object.freeze({
    ...
    userId: trimmedUserId,
    effectiveProductRole,
    /* Viilus A saab maksjaks olla ainult kasutaja ise või individuaalne sponsor.
       `ORGANIZATION` tuleb viilus B koos päris kohaga — org-kontekstis viibimine
       ei tähenda, et organisatsioon maksab. */
    payerSource,
    writable,
