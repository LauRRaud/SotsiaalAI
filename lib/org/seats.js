...
/**
 * PLAANIREA LUKK — ÜKS PROTOKOLL KOLMELE KIRJUTAJALE (SOL-ORG-05).
 *
 * Kohaandmine, limiidi muutmine ja plaani lõpetamine on kolm eri toimingut, aga
 * nad võistlevad SAMA invariandi pärast: `usedSeats <= seatLimit` ja „lõpetatud
 * plaanil ei ole aktiivseid kohti". Invariant, mille kolm kirjutajat hoiavad
 * kolme eri protokolliga, ei ole invariant.
 *
 * KAKS SAMMU JA NENDE JÄRJEKORD ON KOGU MÕTE:
 *   1. lukusta plaanirida (`FOR UPDATE`) — see serialiseerib kõik kolm rada;
 *   2. loe plaani seis ALLES SIIS.
 *
 * Vana kood tegi vastupidi: `assignSeat` luges `status`-e ja `seatLimit`-i ENNE
 * lukku ja kasutas pärast luku saamist sama, juba VANANENUD objekti. Vahepeal
 * commit'unud limiidi langetus või plaani lõpetamine oli talle nähtamatu —
 * klassikaline TOCTOU, kus lukk ise oli õigel real, aga otsus tehti luku-eelse
 * hetktõe pealt. `updateSeatLimit` ja `endSeatPlan` ei võtnud lukku üldse ja
 * Postgres andis neile rea luku alles `UPDATE` hetkel, ehk PÄRAST loendust.
 *
 * READ COMMITTED tähendab, et luku all tehtud päring näeb värsket seisu — luku
 * ootamise ajal commit'unu kaasa arvatud. Just seda me siin vajame.
 */
async function lockSeatPlan(tx, organizationId, seatPlanId) {
...
export async function updateSeatLimit(
  ...
  return db.$transaction(async (tx) => {
    /* Sama lukk, mis kohaandmisel. Ilma selleta sai paralleelne `assignSeat`
       lisada koha vana limiidi järgi ja lõppseis oli `used > seatLimit`:
       loendus käis siin ENNE `UPDATE`-i, mille peal Postgres luku alles andis. */
    const plan = await lockSeatPlan(tx, organizationId, seatPlanId);
...
export async function endSeatPlan(organizationId, seatPlanId, { actorUserId, reason }, { db = prisma, now = new Date() } = {}) {
  return db.$transaction(async (tx) => {
    /* Lukk ENNE kohtade lõpetamist. Ilma selleta jõudis paralleelne
       `assignSeat` oma koha sisse kirjutada pärast siinset `updateMany`-t ja
       enne plaani `UPDATE`-i — tulemuseks aktiivne koht lõpetatud plaani all,
       ehk täpselt see rippuv koht, mida allolev kommentaar välistada lubab. */
    const plan = await lockSeatPlan(tx, organizationId, seatPlanId);
 ...
 *
 * SOL-ORG-05: lukk oli õigel real, aga OTSUS tehti luku-eelse hetktõe pealt —
 * `status` ja `seatLimit` loeti enne `FOR UPDATE`-i. Nüüd loeb neid
 * `lockSeatPlan` alles luku all, seega vahepeal commit'unud limiidi langetus või
 * plaani lõpetamine on nähtav.
 */
export async function assignSeat(
  ...
  return db.$transaction(async (tx) => {
    ...
    /* SAAJA ELUSUS LOETAKSE LUKU ALL (SOL-ORG-10). Plaanirea lukk ütleb, mitu
       kohta on vaba; liikmesuse lukk ütleb, kas see inimene on veel majas.
       Ilma teiseta jäi lahkunule organisatsiooni makstav koht. */
