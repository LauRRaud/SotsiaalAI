...
/**
 * Minimaalne liikmevaliku projektsioon töö ja koha määramiseks.
 *
 * See ei ole liikmehalduse vaade: e-post, üksused, liikmesuse kuupäevad ja
 * capability'd ei tohi jõuda nõrgema WORK_ASSIGNER/BILLING_MANAGER värava taha.
 */
export async function listMemberOptions(organizationId, { db = prisma } = {}) {
  const rows = await db.organizationMembership.findMany({
    where: { organizationId, status: OrganizationMembershipStatus.ACTIVE },
    select: {
      id: true,
      status: true,
      seatRole: true,
