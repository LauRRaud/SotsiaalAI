...
export async function deliverPreInquiryToOrganizationWithin(
  ...
  return (async () => {
    ...
    const existing = await tx.organizationInboxItem.findFirst({ where, select: { id: true } });
    if (existing) return existing;

    /* IDEMPOTENTSUS ON `create` TULEMUS, mitte eelnev `findFirst`.
       `findFirst → create` on võistlusaken: kaks samaaegset kohaletoimetamist
       (nt kordussaatmine või kaks serveriprotsessi) näeksid mõlemad tühjust ja
       mõlemad kirjutaksid. Unikaalindeks päästab andmed, aga kasutaja saaks
       P2002 → 500. Siin loeme kokkupõrke ÕNNESTUMISEKS: teine kirjutaja luges
       lihtsalt kaotuse ja tagastab võitja rea. */
    let item;
    try {
      item = await tx.organizationInboxItem.create({
        data: {
          ...where,
          unitId,
      ...
      const winner = await tx.organizationInboxItem.findFirst({ where, select: { id: true } });
...
export async function getInboxItem(context, inboxItemId, { db = prisma } = {}) {
  return db.$transaction(async (tx) => {
    ...
    /* AVAMISE AJATEMPEL on pöörduja õiguste alus, mitte statistika:
       ...
       `updateMany` tingimusega `openedAt: null` on idempotentne: teine avaja
       ei nihuta esimese ajatemplit. */
    if (inquiry && !inquiry.openedAt) {
      const openedAt = new Date();
      const marked = await tx.preInquiry.updateMany({
        where: { id: item.sourceId, openedAt: null },
        data: { openedAt }
      });
      if (marked.count === 1) inquiry.openedAt = openedAt;
    }
