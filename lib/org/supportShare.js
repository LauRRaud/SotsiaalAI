...
/**
 * Loendivaade ei avalda avamata avalduse sisu. Sisu saab saaja alles
 * `openSupportShare` kaudu, mis märgib avalduse avatuks ja kirjutab auditi.
 * Juba avatud avalduste sisu võib loendis kuvada, sest avamise jälg on olemas.
 */
export function toRecipientListView(share) {
  const view = toRecipientView(share);
  if (!view || view.openedAt) return view;
  const { snapshot: _snapshot, ...metadata } = view;
  return metadata;
}
...
export async function listReceivedSupportSharePage(
  ...
  const page = toCursorPage(rows, pageSize, (row) => ({ sentAt: row.sentAt, id: row.id }));
  return { ...page, items: page.items.map(toRecipientListView) };
}
