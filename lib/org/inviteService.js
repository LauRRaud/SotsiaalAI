...
export async function listInvitePage(
  organizationId,
  { db = prisma, includeClosed = false, take = 100, cursor, status } = {}
) {
  const normalizedStatus = String(status || "").trim();
  if (normalizedStatus && !Object.values(OrganizationInviteStatus).includes(normalizedStatus)) {
  ...
  const rows = await db.organizationInvite.findMany({
    where: decoded ? { AND: [baseWhere, descendingCursorWhere(decoded, ["createdAt", "id"])] } : baseWhere,
    select: {
      id: true,
      email: true,
      status: true,
...
export async function listInvites(organizationId, options = {}) {
  return (await listInvitePage(organizationId, options)).items;
}
