...
export async function revokeInvite(organizationId, inviteId, { actorUserId }, { db = prisma, now = new Date() } = {}) {
  return db.$transaction(async (tx) => {
    ...
    if (invite.status !== OrganizationInviteStatus.PENDING) throw conflict("org.errors.invite_not_pending");

    const updated = await tx.organizationInvite.update({
      where: { id: inviteId },
      data: { status: OrganizationInviteStatus.REVOKED, revokedAt: now, revokedByUserId: actorUserId }
    });
...
export async function declineInvite(rawToken, { userId, userEmail }, { db = prisma, now = new Date() } = {}) {
  ...
  return db.$transaction(async (tx) => {
    ...
    const updated = await tx.organizationInvite.update({
      where: { id: invite.id },
      data: { status: OrganizationInviteStatus.DECLINED, declinedAt: now }
    });
