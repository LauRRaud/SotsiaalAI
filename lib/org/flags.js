...
/**
 * UI VÄRAV töölaua kaardile (`NEXT_PUBLIC_ORG_WORKSPACE_ENABLED`).
 *
 * KAKS LIPPU, ERI ELUIGA — sama muster mis teenuspäevikul
 * (`lib/serviceLog/flags.js`): serveri `ORG_WORKSPACE_ENABLED` loetakse päringu
 * ajal, aga `NEXT_PUBLIC_*` KÜPSETATAKSE BUILD'i. Tema muutmine serveris EI MÕJU
 * enne uut build'i.
 *
 * LOETAKSE LITERAALSELT, mitte `env[võti]` kaudu: Next inline'ib väärtuse ainult
 * siis, kui ligipääs on tekstiliselt `process.env.NEXT_PUBLIC_...`. Dünaamiline
 * indekseerimine jääks kliendipaketis `undefined`-iks ja kaart ei ilmuks kunagi —
 * vaikne viga, mis serveris töötaks ja brauseris mitte.
 *
 * TA AINULT PEIDAB KAARDI. Tõde on serveri lipp: kui see UI lipp on ekslikult
 * sees, näeb kasutaja kaarti, aga `/org` vastab `notFound()`-iga
 * (`app/org/_serverContext.js`).
 */
export function isOrgWorkspaceUiEnabled() {
  const raw = String(process.env.NEXT_PUBLIC_ORG_WORKSPACE_ENABLED ?? "").trim().toLowerCase();
