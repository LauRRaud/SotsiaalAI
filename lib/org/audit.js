...
import { decodePageCursor, encodePageCursor, normalizePageSize } from "./pagination.js";
...
export const OrgAuditAction = Object.freeze({
  ...
  MEMBER_IDENTITY_ERASED: "org.member_identity_erased",
  ...
  /* TEENUSPÄEVIK: kuuaruande jagamine juhile. Eraldi tegevused seepärast, et
     „saatis", „juht avas" ja „võttis tagasi" vastavad eri küsimustele. */
  /* TEENUSPÄEVIK E10: juht paneb töö töötaja päeva / annab ta teisele. Kaks eri
     tegevust, sest „see töö on sinu" ja „see töö läks kelleltki teiselt sinule"
     vastavad eri küsimustele. */
  SERVICE_VISIT_ASSIGNED: "org.service_visit_assigned",
  SERVICE_VISIT_REASSIGNED: "org.service_visit_reassigned",
  SERVICE_REPORT_SHARE_SENT: "org.service_report_share_sent",
  SERVICE_REPORT_SHARE_ACCESS_ATTEMPTED: "org.service_report_share_access_attempted",
  SERVICE_REPORT_SHARE_OPENED: "org.service_report_share_opened",
  SERVICE_REPORT_SHARE_RECALLED: "org.service_report_share_recalled",
 ...
 */
function orgAuditWhere(organizationId) {
  return {
...
/**
 * Stabiilne auditi leht. `createdAt` üksi ei ole cursor: ühe tehingu mitu
 * auditirida võivad saada sama ajatempli. ID on kohustuslik viigimurdja.
 */
export async function listOrgAuditEventPage(
  organizationId,
  { take = 100, cursor = null, db = prisma } = {}
) {
  const pageSize = normalizePageSize(take, 100, 200);
  const decoded = decodePageCursor(cursor, { dateKeys: ["createdAt"], stringKeys: ["id"] });
  const baseWhere = orgAuditWhere(organizationId);
  const where = decoded
    ? {
        AND: [
          baseWhere,
  ...
  const [total, rows] = await Promise.all([
    db.dataAuditLog.count({ where: baseWhere }),
  ...
  return {
    items: pageRows.map(toAuditView),
    total,
    hasMore,
    nextCursor:
      hasMore && last
        ? encodePageCursor({ createdAt: new Date(last.createdAt).toISOString(), id: last.id })
        : null
  };
}

/** Vana sisemine kuju jääb alles väikestele kutsujatele. Uued vaated kasutavad lehte. */
export async function listOrgAuditEvents(organizationId, options = {}) {
  const page = await listOrgAuditEventPage(organizationId, options);
  return page.items;
}

/**
 * Ekspordirada ei tohi kärpida. Kui lehtede ahel ei anna esimese päringu
 * koguarvu täpselt kätte, katkeb eksport veaga — poolik manifest on hullem kui
 * aus tõrge.
 */
export async function listAllOrgAuditEvents(
  organizationId,
  { db = prisma, pageSize = 200 } = {}
) {
  const events = [];
  const ids = new Set();
  let cursor = null;
  ...
  do {
    const page = await listOrgAuditEventPage(organizationId, {
      db,
      take: pageSize,
      cursor
    });
    ...
    for (const event of page.items) {
      if (ids.has(event.id)) {
        const error = new Error("Organisation audit export cursor repeated an event");
        error.code = "ORG_EXPORT_INCOMPLETE";
        error.status = 500;
        throw error;
      }
      ids.add(event.id);
    ...
    if (!page.nextCursor || page.items.length === 0) {
      const error = new Error("Organisation audit export cursor made no progress");
      error.code = "ORG_EXPORT_INCOMPLETE";
      error.status = 500;
      throw error;
    }
  ...
  if (events.length !== expectedTotal) {
    const error = new Error(
      `Organisation audit export is incomplete: expected ${expectedTotal}, received ${events.length}`
    );
    error.code = "ORG_EXPORT_INCOMPLETE";
    error.status = 500;
    throw error;
