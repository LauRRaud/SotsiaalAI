...
export async function runLicenceCheck({
  ...
  let leaseToken = null;
  if (trigger === CHECK_TRIGGER.MANUAL) {
    leaseToken = randomUUID();
    const leaseUntil = new Date(now.getTime() + MTR_CHECK_POLICY.manualLeaseMs);
    const claimed = await prisma.serviceProviderProfile.updateMany({
      where: {
        id: profile.id,
        OR: [{ licenceCheckLeaseUntil: null }, { licenceCheckLeaseUntil: { lte: now } }]
      },
      data: { licenceCheckLeaseToken: leaseToken, licenceCheckLeaseUntil: leaseUntil }
    });
    if (claimed.count !== 1) {
      return { completed: false, succeeded: false, skipped: CHECK_SKIPPED.IN_PROGRESS };
    }
  }
  ...
  return {
    ...
    if (leaseToken) {
      await prisma.serviceProviderProfile.updateMany({
        where: { id: profile.id, licenceCheckLeaseToken: leaseToken },
        data: { licenceCheckLeaseToken: null, licenceCheckLeaseUntil: null }
      });
    }
  }
}
