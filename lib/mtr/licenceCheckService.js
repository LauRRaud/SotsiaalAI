...
import { ASSESSMENT_REASON, assessServiceLicence, publicClaimIsCurrent } from "./assessment.js";
...
export async function runLicenceCheck({
  ...
  const profileName = String(profile.organizationName || "").trim();
  const nameMismatch = entityResolved && entityName
    ? entityName.toLocaleLowerCase("et") !== profileName.toLocaleLowerCase("et")
    : false;
  const entityNameMatches = entityResolved && entityName && profileName && !nameMismatch;
  const succeeded = licenceOk && Boolean(entityNameMatches);
  ...
  const checkPayload = {
    ...
    entityReason:
      entity.reason ||
      (entityOk && !entity.found ? "ENTITY_NOT_FOUND" : null) ||
      (entityResolved && !entityNameMatches ? ASSESSMENT_REASON.ENTITY_NAME_MISMATCH : null),
    entityResolved,
  ...
  const written = await prisma.$transaction(async (tx) => {
    ...
    const assessments = await writeAssessments({
      ...
      check: {
        ...
        reason:
          licences.reason ||
          (entityResolved && !entityNameMatches ? ASSESSMENT_REASON.ENTITY_NAME_MISMATCH : entity.reason || null),
        entityResolved,
  ...
  return {
    ...
    entityResolved,
    entityName: entity.name || null,
    /* Alarm säilib ka siis, kui identiteedivärav avaliku väite blokeerib. */
    nameMismatch,
    missingOrderedColumns: licences.missingOrderedColumns || [],
    unknownColumns: licences.unknownColumns || [],
