...
export function assessServiceLicence({ serviceKey = null, check = null, previous = null, now = new Date() } = {}) {
  ...
  const technicalFailure = (reason, { preservePrevious = true } = {}) => {
    if (preservePrevious && positiveEvidenceStillValid(previous, now)) {
      return { ...keepPrevious(reason), confirmedMissCount: 0 };
    }
    return {
      ...snapshot,
  ...
  /* Registrikood võib lahenduda teisele juriidilisele isikule. Sellisel juhul
     ei tohi ka varasemat positiivset väidet alles hoida. */
  if (check.reason === ASSESSMENT_REASON.ENTITY_NAME_MISMATCH) {
    return technicalFailure(check.reason, { preservePrevious: false });
  }
