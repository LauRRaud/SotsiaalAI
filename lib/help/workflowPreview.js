...
export function createHelpWorkflowPreview({ normalizeDraft, collapseWhitespace, formatLabelledLine }) {
  ...
  function buildMapPrivacyLines(state, draft, replyLang = "et") {
    ...
    const visibleRegion = locationLabel
      ? locationLabel
      : helpWorkflowT(replyLang, "preview.mapRegionUnspecified", undefined, "piirkond täpsustub kuulutuse asukoha põhjal");
    return [
      formatLabelledLine(
        helpWorkflowLabel(replyLang, "mapVisibility", "Kaardil kuvamine"),
        helpWorkflowT(replyLang, "preview.mapVisibleValue", {
          region: visibleRegion
        }, `Kuulutus avaldatakse Teenusekaardil üldistatud piirkonnana: ${visibleRegion}.`)
      ),
      formatLabelledLine(
        helpWorkflowLabel(replyLang, "mapPrivacy", "Privaatsus"),
        helpWorkflowT(replyLang, "preview.mapPrivacyValue", undefined, "Täpset koduaadressi ega avalikke kontaktandmeid ei kuvata; ühendus toimub platvormis.")
      )
    ];
  }
