...
function rateLimitKey(operation, userId) {
...
export async function consumeHelpRateLimit({
  ...
  if (typeof prismaClient?.$queryRawUnsafe !== "function" || typeof prismaClient?.$executeRawUnsafe !== "function") {
    throw new Error("HELP_RATE_LIMIT_STORAGE_UNAVAILABLE");
  }

  const current = now instanceof Date ? now : new Date(now);
  const nextResetAt = new Date(current.getTime() + policy.windowMs);
  await prismaClient.$executeRawUnsafe(
    `DELETE FROM "HelpRateLimitBucket" WHERE "resetAt" <= $1`,
    current
  );
  const [bucket] = await prismaClient.$queryRawUnsafe(
    ...
    rateLimitKey(operation, normalizedUserId),
    nextResetAt,
    current
  );
