...
export async function createHelpOffer(input = {}, prismaClient = prisma) {
  ...
  const created = await prismaClient.helpOffer.create({
    data: {
      userId,
      municipalityId,
  ...
  await syncHelpOfferMapEntry(created, mapEntryFieldsFromInput(input), prismaClient);
  return getHelpOfferById(created.id, prismaClient);
}
...
export async function updateHelpOffer(helpOfferId, input = {}, prismaClient = prisma) {
  ...
  const updated = await prismaClient.helpOffer.update({
    where: { id },
    data: {
      ...data,
  ...
  await syncHelpOfferMapEntry(updated, mapEntryFieldsFromInput(input), prismaClient);
  return getHelpOfferById(id, prismaClient);
}
