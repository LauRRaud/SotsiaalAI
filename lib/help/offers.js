...
import { mapEntryFieldsFromInput, syncHelpOfferMapEntry } from "./mapEntries.js";
...
const HELP_OFFER_STATUS_TRANSITIONS = Object.freeze({
...
function helpOfferError(code, current = undefined) {
...
function normalizeExpectedUpdatedAt(value) {
  if (value === undefined || value === null || value === "") {
    throw helpOfferError("HELP_OFFER_EXPECTED_UPDATED_AT_REQUIRED");
  }
  const date = value instanceof Date ? new Date(value) : new Date(String(value));
  if (!Number.isFinite(date.getTime())) {
    throw helpOfferError("HELP_OFFER_EXPECTED_UPDATED_AT_INVALID");
  }
  return date;
}
...
function normalizeTransition(input = {}) {
  if (hasOwn(input, "status")) throw helpOfferError("HELP_OFFER_STATUS_PATCH_INVALID");
  const action = String(input?.action || "").trim().toUpperCase();
  const transition = HELP_OFFER_STATUS_TRANSITIONS[action];
  if (!transition) throw helpOfferError("HELP_OFFER_TRANSITION_ACTION_INVALID");
  const reason = String(input?.reason || "").replace(/\s+/g, " ").trim().slice(0, 500);
  if (!reason) throw helpOfferError("HELP_OFFER_TRANSITION_REASON_REQUIRED");
  return { action, reason, transition };
}

function requireTransaction(prismaClient) {
  if (typeof prismaClient?.$transaction !== "function") {
    throw helpOfferError("HELP_OFFER_TRANSACTION_REQUIRED");
  }
...
function normalizeExpiresAt(value) {
  ...
  if (!Number.isFinite(date.getTime())) {
    const error = new Error("HELP_OFFER_EXPIRES_AT_INVALID");
    error.code = "HELP_OFFER_EXPIRES_AT_INVALID";
    throw error;
  }
  return date;
}
...
export async function createHelpOffer(input = {}, prismaClient = prisma) {
  ...
  return transaction(async (tx) => {
    const created = await tx.helpOffer.create({
      ...
      providerScopeOrConditions: normalizeOptionalText(input?.providerScopeOrConditions, "providerScopeOrConditions"),
    ...
    await syncHelpOfferMapEntry(created, mapEntryFieldsFromInput(input), tx);
    return getHelpOfferById(created.id, tx);
  });
}
...
async function listHelpOffers(filters = {}, prismaClient = prisma) {
  ...
  // FAIL-CLOSED nähtavuspõrand — peegel abisoovi loogikale. Ainult 'mine' skoop
  // näeb mitteavalikke staatuseid; globaalne/skoobita kutsuja saab OPEN-põranda.
...
export async function updateHelpOffer(helpOfferId, input = {}, prismaClient = prisma) {
  ...
  if (Object.prototype.hasOwnProperty.call(input, "providerScopeOrConditions")) data.providerScopeOrConditions = normalizeOptionalText(input?.providerScopeOrConditions, "providerScopeOrConditions");
  ...
  const transaction = requireTransaction(prismaClient);
  return transaction(async (tx) => {
    const claimed = await tx.helpOffer.updateMany({
      where: { id, updatedAt: expectedUpdatedAt },
      data: { ...data, updatedAt: nextUpdatedAt(expectedUpdatedAt) }
    });
    if (claimed.count !== 1) {
      const current = await getHelpOfferById(id, tx);
      if (!current) throw helpOfferError("HELP_OFFER_NOT_FOUND");
      throw helpOfferError("HELP_OFFER_CONFLICT", current);
    }
    ...
    const updated = await getHelpOfferById(id, tx);
    await syncHelpOfferMapEntry(updated, mapEntryFieldsFromInput(input), tx);
    return getHelpOfferById(id, tx);
  });
}

export async function transitionHelpOfferStatus(helpOfferId, input = {}, prismaClient = prisma) {
  const id = String(helpOfferId || "").trim();
  if (!id) throw helpOfferError("HELP_OFFER_ID_REQUIRED");
  const expectedUpdatedAt = normalizeExpectedUpdatedAt(input?.expectedUpdatedAt);
  const { action, reason, transition } = normalizeTransition(input);
  const transaction = requireTransaction(prismaClient);

  return transaction(async (tx) => {
    const before = await getHelpOfferById(id, tx);
    if (!before) throw helpOfferError("HELP_OFFER_NOT_FOUND");
    if (!transition.from.includes(before.status)) {
      throw helpOfferError("HELP_OFFER_TRANSITION_CONFLICT", before);
    }
    ...
    const updated = await getHelpOfferById(id, tx);
    await syncHelpOfferMapEntry(updated, { syncLifecycleStatus: true }, tx);
    await tx.dataAuditLog.create({
      data: {
        actorUserId: updated.userId,
        targetUserId: updated.userId,
        action: `HELP_OFFER_${action}`,
...
export async function deleteHelpOffer(helpOfferId, optionsOrPrisma = {}, prismaClient = prisma) {
  const id = String(helpOfferId || "").trim();
  if (!id) return null;
  ...
  return deleteHelpListingWithAcceptedMatchGuard({ kind: "offer", id, ...options }, legacyPrismaClient || prismaClient);
}
...
export async function listHelpOfferListingViews(filters = {}, options = {}, prismaClient = prisma) {
  ...
  return records.map((record) => toListingView(record, { kind: "offer", locale }));
}
