...
function buildKeywordSummary({
  ...
  // `rawPlace` on omaniku privaatne toorasukoht ega tohi sattuda avalikku
  // võtmesõna-kokkuvõttesse (seda vaadet kasutavad nii globaalne loend, avalik
  // detail kui kaart). Üldistatud `municipalityLabel` katab asukohavajaduse.
  // Fail-closed: eemaldatud allikast, mitte pärast serialiseerimist.
...
const PUBLIC_HELP_TYPES = new Set(["VOLUNTARY", "PAID", "MIXED"]);
const PUBLIC_TIME_TYPES = new Set(["ONE_TIME", "RECURRING", "FLEXIBLE"]);
const PUBLIC_LISTING_STATUSES = new Set(["OPEN"]);
const PUBLIC_DELIVERY_MODES = new Set(["ON_SITE", "AT_HOME", "ONLINE", "PHONE", "REGIONAL"]);
const PUBLIC_CONTACT_MODES = new Set(["PLATFORM", "PHONE", "EMAIL", "OTHER"]);

function normalizePublicEnum(value, allowedValues) {
  const normalized = String(value || "").trim().toUpperCase();
  return allowedValues.has(normalized) ? normalized : "";
}

function resolvePublicCategory(record = {}, locale = "et") {
  const requestedCode = record?.primaryCategory?.code
    || record?.primaryCategoryCode
    || record?.mapEntry?.categoryCode
    || "";
  const taxonomy = enrichHelpTaxonomy({ categoryCode: requestedCode });
  const relationCode = String(record?.primaryCategory?.code || "").trim().toUpperCase();
  const relationLabel = relationCode === taxonomy.categoryCode
    ? pickLocalizedField(record?.primaryCategory, locale)
    : "";
  return {
    code: taxonomy.categoryCode,
    label: relationLabel || fallbackEnumLabel(taxonomy.categoryCode),
    taxonomy
  };
}

function resolvePublicMunicipality(record = {}) {
  return normalizeText(record?.municipality?.displayName || "");
}

function resolvePublicTargetGroupLabels(record = {}, locale = "et") {
  return resolveTargetGroupLabels({
    targetGroupLinks: Array.isArray(record?.targetGroupLinks) ? record.targetGroupLinks : [],
    targetGroups: Array.isArray(record?.targetGroups) ? record.targetGroups : []
  }, locale);
}

function buildPublicTitle({ kind = "request", locale = "et", categoryLabel = "", municipalityLabel = "" } = {}) {
  const kindLabel = formatListingKindLabel(kind, locale);
  if (categoryLabel && municipalityLabel) return `${kindLabel}: ${categoryLabel} - ${municipalityLabel}`;
  if (categoryLabel) return `${kindLabel}: ${categoryLabel}`;
  if (municipalityLabel) return `${kindLabel} - ${municipalityLabel}`;
  return kindLabel;
}

function buildPublicPrivacyNote(mapEntry = {}) {
  if (mapEntry?.mapMode === "PHYSICAL" && mapEntry?.address) {
    return "Kaardil kuvatakse eraldi avaldamiseks kinnitatud asukoht.";
  }
  return "Kaardil kasutatakse üldistatud piirkonda; täpset koduaadressi ei avaldata.";
}
...
// Ainus avalik help-listing'u sisuprojektsioon. See ei loe salvestatud title,
// description, structuredSummary, roleLabel, rawPlace, tingimuse ega muu
// vabateksti väärtust. Avalik tekst sünteesitakse ainult kontrollitud
// taksonoomiast ja KOV-i nimest, nii et ka legacy-kirjes teise välja alla
// kopeeritud privaatväärtus jääb fail-closed väljundist välja.
export function toPublicHelpListingProjection(record = {}, options = {}) {
  const locale = pickLocale(options?.locale);
  const kind = String(options?.kind || record?.kind || "request").trim().toLowerCase() === "offer" ? "offer" : "request";
  const { code: primaryCategoryCode, label: categoryLabel, taxonomy } = resolvePublicCategory(record, locale);
  const municipalityLabel = resolvePublicMunicipality(record);
  const helpType = normalizePublicEnum(record?.helpType, PUBLIC_HELP_TYPES);
  const timeType = normalizePublicEnum(record?.timeType, PUBLIC_TIME_TYPES);
  const helpTypeLabel = formatHelpTypeLabel(helpType, locale);
  const timeTypeLabel = formatTimeTypeLabel(timeType, locale);
  const targetGroupLabels = resolvePublicTargetGroupLabels(record, locale);
  const status = normalizePublicEnum(record?.status, PUBLIC_LISTING_STATUSES);
  const title = buildPublicTitle({ kind, locale, categoryLabel, municipalityLabel });
  const summary = buildKeywordSummary({
    categoryLabel,
    municipalityLabel,
    helpTypeLabel,
  ...
  const mapEntry = record?.mapEntry || null;
  ...
  return {
    ...
    roleLabel: "",
    status,
    statusLabel: formatListingStatusLabel(status, locale),
    targetGroupLabels,
    score: Number.isFinite(Number(record?.score)) ? Number(record.score) : null,
    isOwn: false,
    expiresAt: record?.expiresAt || mapEntry?.expiresAt || null,
    availabilityOrStart: "",
    compensationDetails: "",
    conditions: "",
    primaryCategoryCode,
    region: {
      county: normalizeText(record?.municipality?.county || ""),
      municipalityLabel
    },
    deliveryModes: Array.isArray(mapEntry?.deliveryModes)
      ? mapEntry.deliveryModes
          .map((value) => normalizePublicEnum(value, PUBLIC_DELIVERY_MODES))
          .filter(Boolean)
      : [],
    contactMode: normalizePublicEnum(mapEntry?.contactMode, PUBLIC_CONTACT_MODES),
    privacyNote: buildPublicPrivacyNote(mapEntry),
...
export function toHelpListingDetailView(record = {}, options = {}) {
  ...
  const taxonomy = enrichHelpTaxonomy({
    categoryCode: record?.primaryCategory?.code || record?.primaryCategoryCode || record?.mapEntry?.categoryCode,
    needTags: record?.mapEntry?.needTags || []
  });
  return {
    ...listingView,
...
// FAIL-CLOSED avaliku/võõra detailprojektsioon. Erinevalt `toHelpListingDetailView`-st
// (omanikuprojektsioon) ei kanna see EI `rawPlace`-i, täpset aadressi/koordinaati,
// `beneficiaryLabel`/`urgency`/`providerScopeOrConditions`/`skillsOrBackground`-i,
// `editable*`-välju ega ühtki muud omaniku- või mustandivälja. Projektsioon on
// koostatud allowlist-põhimõttel (ehitatud turvalisest kokkuvõttevaatest üles),
// nii et ükski hiljem lisatud omanikuväli EI leki võõrale ilma teadliku lisamiseta.
// Väljastab ainult kontrollitud kategooria-, piirkonna-, abi tüübi-, ajavormi-,
// sihtrühma- ja taksonoomiasildid; vabatekstiväljad jäävad tühjaks.
export function toPublicHelpListingDetailView(record = {}, options = {}) {
  return toPublicHelpListingProjection(record, options);
}
