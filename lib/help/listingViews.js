...
// Väljastab ainult selle, mida avalik kaart niikuinii kuvab (üldistatud piirkond,
// abi liik, aeg, tasu/tingimus, sihtrühm, taksonoomia, privaatsusmärge).
export function toPublicHelpListingDetailView(record = {}, options = {}) {
  ...
  return {
    ...
    primaryCategoryCode: normalizeText(record?.primaryCategory?.code || record?.primaryCategoryCode || ""),
    region: {
      county: normalizeText(mapEntry?.county || ""),
      serviceArea: normalizeText(mapEntry?.serviceArea || ""),
      municipalityIds: Array.isArray(mapEntry?.municipalityIds) ? mapEntry.municipalityIds : []
    },
    deliveryModes: Array.isArray(mapEntry?.deliveryModes) ? mapEntry.deliveryModes : [],
    contactMode: normalizeText(mapEntry?.contactMode || ""),
    privacyNote: normalizeText(mapEntry?.privacyNote || ""),
    serviceRecommendation: {
      relatedServiceCategories: taxonomy.relatedServiceCategories,
      needTags: taxonomy.needTags,
      lifeDomains: taxonomy.lifeDomains,
