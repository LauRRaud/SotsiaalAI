...
function evaluateHardFilters(request, offer) {
  ...
  if (isHelpRecordExpired(request)) failures.push("request_expired");
  if (isHelpRecordExpired(offer)) failures.push("offer_expired");
...
export async function decideHelpMatch(input = {}, prismaClient = prisma) {
  ...
  return prismaClient.$transaction(async (tx) => {
    ...
    if (match.status === "ACCEPTED" || match.status === "DECLINED" || match.status === "CLOSED") return match;
...
export async function listIncomingHelpMatches(userId, prismaClient = prisma) {
