...
export function createHelpWorkflowActions({
  ...
  function decorateBrowseResults(results = [], sourceDraft = {}, replyLang = "et") {
    return results.map((item) => {
      const listingView = toPublicHelpListingProjection(item, { kind: item.kind, locale: replyLang });
      const compatibilityWarnings = normalizeCompatibilityWarnings(item?.compatibilityWarnings);
      return {
        id: listingView.id,
        kind: listingView.kind,
        title: listingView.title,
        description: listingView.description,
        municipalityName: listingView.municipalityLabel,
        score: listingView.score,
        helpType: listingView.helpType,
        timeType: listingView.timeType,
        compatibilityWarnings,
        compatibilitySummary: compatibilityWarnings.length
          ? buildCompatibilitySummary(sourceDraft, item, compatibilityWarnings, replyLang)
          : "",
  ...
  async function createMatchesForSelections(selections = [], prismaClient, options = {}) {
    ...
    const initiatedByUserId = String(options?.initiatedByUserId || "").trim();
    const matches = [];
    let failureCount = 0;
  ...
  function buildBrowseReply(intent, results, replyLang) {
    ...
    const lines = results.map((item, index) => {
      const listingView = item.listingView;
  ...
  async function handleConnectTurn({ state, message, replyLang, prismaClient, userId }) {
