...
function sortClosures(items, sort) {
  return [...items].sort((left, right) => {
    if (sort === "title") return left.generalizedTitle.localeCompare(right.generalizedTitle, "et");
    if (sort === "oldest") return new Date(left.closedAt) - new Date(right.closedAt);
    if (sort === "newest") return new Date(right.closedAt) - new Date(left.closedAt);
    if (sort === "follow_up") {
  ...
  await withLock(db, `covisionClosure:${normalizeId(closureId)}`, async (tx) => {
    ...
    if (request.decision === "close") {
      ...
      if (closure.sourceTopicSeedId) {
        await tx.topicSeed.update({ where: { id: closure.sourceTopicSeedId }, data: { status: "CLOSED" } });
      }
      return;
    }
