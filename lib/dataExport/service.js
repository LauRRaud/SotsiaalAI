...
function storageDirectory() {
  ...
  return path.resolve(configured || path.join("tmp", "data-exports"));
}
...
async function collectExportEntries(job, { db, now }) {
  const entries = [];
  const surfaces = [];
  for (const surface of DATA_EXPORT_REGISTRY) {
    const collected = await surface.collect({ db, userId: job.userId });
    const files = collected.map(item => ({ name: item.name, content: item.content }));
    entries.push(...files);
    surfaces.push({
      name: surface.name,
...
export async function runNextDataExport({ db = defaultPrisma, now = new Date(), audit = logDataAudit, notify = createNotificationEvent } = {}) {
  ...
  try {
    const { entries, manifest } = await collectExportEntries(job, { db, now });
