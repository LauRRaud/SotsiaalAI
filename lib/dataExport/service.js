...
export async function runNextDataExport({
  ...
  try {
    ...
    outputPath = path.join(directory, storageFileName(job.id));
    await fs.writeFile(outputPath, archive, { flag: "wx" });
    const expiresAt = new Date(now.getTime() + DATA_EXPORT_TTL_MS);
    const published = await db.dataExportJob.updateMany({ where: { id: job.id, status: DATA_EXPORT_STATUS.RUNNING }, data: {
    ...
    const updated = await db.dataExportJob.findUnique({ where: { id: job.id } });
    ...
    if (outputPath) {
      await fs.unlink(outputPath).catch(unlinkError => { if (unlinkError?.code !== "ENOENT") throw unlinkError; });
    }
...
/** Remove every archive while its owner-scoped metadata still exists.
 *
 * The worker also unlinks an archive when its RUNNING -> READY CAS loses, which
 * closes the complementary race where the User cascade happens during export.
 */
export async function removeDataExportsForAccountDeletion(userId, { db = defaultPrisma } = {}) {
  const ownerId = String(userId || "").trim();
  if (!ownerId) return { ok: true, removed: 0 };
  const rows = await db.dataExportJob.findMany({
  ...
  for (const row of rows) {
    // A RUNNING worker may have written the deterministic path but not yet
    // persisted outputPath. Removing both representations is idempotent.
    const paths = new Set([row.outputPath, path.join(storageDirectory(), storageFileName(row.id))].filter(Boolean));
    for (const archivePath of paths) {
      try {
        await fs.unlink(archivePath);
        removed += 1;
      } catch (error) {
        if (error?.code !== "ENOENT") failures.push({ jobId: row.id, error: error?.code || "unlink_failed" });
      }
    }
  }
  return failures.length ? { ok: false, failures, removed } : { ok: true, removed };
}
