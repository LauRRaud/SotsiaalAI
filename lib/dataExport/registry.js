...
export const DATA_EXPORT_REGISTRY = Object.freeze([
  {
    name: "covision_private_states", version: "1.0", thirdPartyExcluded: true,
    async collect({ db, userId }) {
      const rows = await db.covisionPrivateState.findMany({
        where: { userId },
        orderBy: [{ createdAt: "asc" }, { id: "asc" }],
        select: { id: true, sessionId: true, stage: true, kind: true, content: true, version: true, createdAt: true, updatedAt: true }
      });
      return [{ name: "covision-private-states.ndjson", content: ndjson(rows), count: rows.length }];
    }
  },
  {
    name: "supervision_private_items", version: "1.0", thirdPartyExcluded: true,
    async collect({ db, userId }) {
      const rows = await db.supervisionPrivateItem.findMany({
        where: { ownerUserId: userId },
        orderBy: [{ createdAt: "asc" }, { id: "asc" }],
        select: {
          id: true, processId: true, kind: true, title: true, body: true, sharedTopicId: true,
          sourceKind: true, sourceWellbeingDraftId: true, version: true, createdAt: true, updatedAt: true
        }
      });
      return [{ name: "supervision-private-items.ndjson", content: ndjson(rows), count: rows.length }];
    }
  },
  {
    name: "supervision_personal_outcomes", version: "1.0", thirdPartyExcluded: true,
    async collect({ db, userId }) {
      const rows = await db.supervisionPersonalOutcome.findMany({
        where: { ownerUserId: userId },
        orderBy: [{ createdAt: "asc" }, { id: "asc" }],
        select: { id: true, processId: true, processTitleGeneralized: true, contentJson: true, createdAt: true }
      });
      return [{ name: "supervision-personal-outcomes.ndjson", content: ndjson(rows), count: rows.length }];
    }
  },
  {
