...
export async function deleteTopicSeed(
  ...
  return withTransaction(db, async (tx) => {
    ...
    const closureLink = await tx.covisionClosure.findFirst({
      where: {
        OR: [
          { sourceTopicSeedId: existing.id },
          { continuationTopicSeedId: existing.id }
        ]
      },
      select: { id: true }
    });
    if (closureLink) throw fail("topic_seeds.errors.lifecycle_conflict", 409);
    ...
    const deleted = await tx.topicSeed.deleteMany({
      where: {
        ...
        status: "DRAFT",
        covisionCaseId: null,
        sourceForClosures: { none: {} },
        continuationForClosure: { is: null },
        version
      }
    });
