 ...
 *
 * A TopicSeed is a SEPARATE model from CovisionCase. This service implements only
 * the DRAFT -> WAITING slice: an owner privately drafts a seed and, through one
 * deliberate version-safe action, freezes a shareable generalized card and moves
 * the seed to WAITING. WAITING does NOT publish the seed to anyone else — there is
 * no group scope yet. Every read/write is owner-scoped; a foreign or missing seed
 * yields the same generic 404 (no existence leak). db is injected for tests.
 */
