  ...
  for (const row of rows) {
    ...
    if (!event?.user?.email) {
      await db.notificationEvent.updateMany({
        ...
        data: { emailStatus: "SKIPPED_NO_RECIPIENT", emailNextAttemptAt: null, emailLastErrorCode: "NO_RECIPIENT" }
      });
      counters.skippedRecipient += 1;
      continue;
    }
    ...
    try {
      ...
      const retry = error?.retryable !== false && attempts < attemptLimit;
      await db.notificationEvent.updateMany({
        where: { id: row.id, emailStatus: "SENDING", emailClaimedAt: now },
        data: {
          emailStatus: retry ? "RETRY" : "FAILED",
          emailNextAttemptAt: retry ? new Date(now.getTime() + backoffMs(attempts)) : null,
          emailLastErrorCode: safeCode(error)
        }
      });
