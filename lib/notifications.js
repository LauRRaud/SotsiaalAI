...
export async function assertNotificationRecipient(db, { type, userId, sourceId, targetId }) {
  ...
  if (type === NOTIFICATION_EVENT_TYPES.PRE_INQUIRY_ARRIVED) {
    ...
    /* Sama värav nagu E6 ligipääsul: salvestisest teavitatakse ainult kohal
       olnud ja nõustunuks jäänud kasutajat (withdraw → WITHDRAWN → teavitust ei
       teki). Praegust ruumiliikmesust EI nõuta — nõusoleku andnul on artefakti
       ligipääs ka pärast ruumist lahkumist ja tema õigus sellest teada on osa
       samast nõusolekust. */
    allowed = Boolean(await db.callRecordingConsent.findFirst({
      where: {
        recordingRequestId: sourceId,
        userId,
        status: "CONSENTED",
        recordingRequest: { callSession: { roomId: targetId } }
      },
      select: { id: true }
    }));
