...
import { hostSponsorOrigin } from "@/lib/payments/subscriptionOrigin";
...
export async function acceptInviteWithinTx({
  ...
  if (!userActive) {
    ...
    if (invite.paymentMode === "SPONSORED_BY_HOST") {
      ...
      /* SOL-PAY-04: päritolu kirjutatakse TERVIKUNA. Vana kuju seadis ainult
         `sponsorUserId` + `inviteId` ja jättis varasema organisatsioonisponsorluse
         väljad rea külge rippuma — `lib/org/accessContext.js` valib sponsori just
         nende järgi. */
      const origin = hostSponsorOrigin({ sponsorUserId, inviteId: invite.id });
