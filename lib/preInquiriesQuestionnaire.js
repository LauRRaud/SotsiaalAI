...
function assessmentLimitError(message) {
  const error = new Error(message);
  error.status = 413;
  return error;
}

function assertAssessmentString(value, maxLength, message) {
  if (value == null) return;
  if (String(value).replace(/\r\n/g, "\n").trim().length > maxLength) throw assessmentLimitError(message);
}

function assertAssessmentList(values, itemLimit, itemLength) {
  if (values == null) return;
  if (!Array.isArray(values) || values.length > itemLimit) {
    throw assessmentLimitError("pre_inquiries.errors.assessment_too_large");
  }
  for (const value of values) {
    assertAssessmentString(value, itemLength, "pre_inquiries.errors.assessment_too_large");
  }
}

function assertFollowUps(value) {
  if (value == null) return;
  ...
  for (const [question, answer] of Object.entries(value)) {
    assertAssessmentString(question, 120, "pre_inquiries.errors.assessment_too_large");
    assertAssessmentString(answer, 2_000, "pre_inquiries.errors.assessment_too_large");
  }
}

export function assertPreInquiryAssessmentLimits(input = {}) {
  const subject = input?.subject || {};
  assertAssessmentString(subject.concernsAbout, 120, "pre_inquiries.errors.assessment_too_large");
  assertAssessmentString(subject.municipalityText, 180, "pre_inquiries.errors.assessment_municipality_too_long");
  assertAssessmentString(subject.consentStatus, 120, "pre_inquiries.errors.assessment_too_large");
  assertAssessmentString(subject.urgency, 120, "pre_inquiries.errors.assessment_too_large");
  assertAssessmentList(input?.riskGate?.reasons, 24, 1_000);
  assertAssessmentString(input?.riskGate?.userVisibleMessage, 4_000, "pre_inquiries.errors.assessment_too_large");
  const domains = input?.domains;
  if (domains != null && (!Array.isArray(domains) || domains.length > PRE_INQUIRY_DOMAIN_DEFINITIONS.length)) {
    throw assessmentLimitError("pre_inquiries.errors.assessment_too_large");
  }
  for (const domain of domains || []) {
    for (const key of ["strengths", "concerns", "missingInfo"]) assertAssessmentList(domain?.[key], 24, 1_000);
    assertFollowUps(domain?.followUpAnswers);
    const primary = domain?.primaryAnswers;
    if (primary != null && (!Array.isArray(primary) || primary.length > 24)) {
      throw assessmentLimitError("pre_inquiries.errors.assessment_too_large");
    }
    for (const answer of primary || []) assertFollowUps(answer?.followUpAnswers);
  }
  assertAssessmentString(input?.supportContext?.existingSupport, 4_000, "pre_inquiries.errors.assessment_too_large");
  assertAssessmentString(input?.supportContext?.supportAdequacy, 180, "pre_inquiries.errors.assessment_too_large");
  assertAssessmentString(input?.supportContext?.personWish, 4_000, "pre_inquiries.errors.assessment_too_large");
}

function normalizeString(value, _maxLength = 4_000) {
...
/**
 * THE single canonical downloadable text for a pre-inquiry snapshot — exactly the
 * text that lands in the .txt file. Shared by the editor, the saved-records list
 * and the server-side content-change check, so "what the author downloaded" and
 * "did the downloadable content change" can never drift apart. Every field that
 * is visible in the file (topic, recipient name, situation, draft, assessment
 * answers) participates, so e.g. changing a DOWNLOADED record's recipient makes
 * the offline copy stale.
 */
export function buildPreInquiryDownloadContent(inquiry = {}) {
  return buildPreInquiryAssessmentExportText(inquiry.assessmentState || {}, {
    topic: inquiry.topic || "",
