...
// Maksja riik — määrab MakeCommerce makselehe vaikimisi makseviisid ja
// `paymentCountry` parameetri. Ilma selleta valis provider vale riigi
// (nähtud 23.07: redirect-URL `paymentCountry=se`, Rootsi pangad ees).
const MAKSEKESKUS_DEFAULT_COUNTRY = String(
  process.env.MAKSEKESKUS_DEFAULT_COUNTRY || "EE"
)
  .trim()
  .toUpperCase();
...
// MakeCommerce redirect-URL kannab `paymentCountry` parameetrit, mis otsustab,
// millise riigi makseviisid vaikimisi ees on. Sunnime selle soovitud riigiks
// (EE), et maksja ei satuks vale riigi (Rootsi) pankade lehele. `trx` on
// allkirjastamata otsinguvõti, seega parameetri muutmine on ohutu.
function withPaymentCountry(url, country) {
  const target = String(url || "").trim();
  const iso = String(country || "").trim().toLowerCase();
  if (!target || !iso) return target;
  try {
    const parsed = new URL(target);
    parsed.searchParams.set("paymentCountry", iso);
...
function buildRequestHeaders() {
  // Maksekeskus/MakeCommerce API kasutab HTTP Basic auth'i: Shop ID = kasutajanimi,
  // Secret key = parool. (Varem oli ekslikult `Bearer <secret>` → 401 Unauthorized;
  // tõestatud test-sandbox'is 2026-07-22.)
  const basic = Buffer.from(`${MAKSEKESKUS_SHOP_ID}:${MAKSEKESKUS_API_KEY}`).toString("base64");
  return {
...
function buildTransactionPayload({
  ...
  // MakeCommerce API ootab `customer`-it tipptasemel (transaction'i KÕRVAL, mitte
  // sees). Varem oli see transaction'i sees → 400 {"missing","customer"};
  // tõestatud test-sandbox'is 2026-07-22.
  const resolvedCountry = String(customerCountry || MAKSEKESKUS_DEFAULT_COUNTRY || "")
    .trim()
    .toUpperCase();
  const customer = {
    ip: normalizeIp(ip),
...
/**
 * Kliendipoolne iframe-konfiguratsioon. SOL-PAY-03: taaskasutatud checkout
 * tagastatakse ILMA uue provideritransaktsioonita, seega need kaks välja peavad
 * olema saadaval ka siis, kui `createMaksekeskusRecurringSetup`-i ei kutsutud.
 */
export function getMaksekeskusCheckoutClientConfig() {
  return {
...
export function mapProviderPaymentStatus(input) {
  ...
  /* SOL-PAY-06: provideri OSALINE tagastus on tema oma seis ja tema kokkusurumine
     `REFUNDED`-iks tegi 0,01 € tagastusest täismahus clawback'i. Osaline kontroll
     peab olema ENNE üldist refund-haru, muidu neelab see ta alla. */
  if (["part_refunded", "partially_refunded", "part_refund"].includes(raw)) {
    return PaymentStatus.PART_REFUNDED;
  }
  ...
  if (raw.includes("part") && raw.includes("refund")) return PaymentStatus.PART_REFUNDED;
...
export function verifyMaksekeskusMac(jsonText, receivedMac, secret = MAKSEKESKUS_API_KEY) {
  ...
  // Fail-closed (L-02): seadistamata/tühi jagatud saladus ei tohi KUNAGI
  // verifitseerimist vaikimisi läbida — muidu saaks võltssõnumit vastu võtta.
...
export async function createMaksekeskusCheckout({
  ...
  const checkoutUrl = withPaymentCountry(
    getRedirectCheckoutUrl(transaction.raw),
    customerCountry || MAKSEKESKUS_DEFAULT_COUNTRY
  );
  if (!checkoutUrl) {
    throw new Error("api.subscription.checkout_create_failed");
  }
...
// Reconciliation-worker'i verifitseeritud provideri-seisu päring (L-05, O-M2).
// Kutsutakse AINULT operaatori selge loaga; tagastab null (= tundmatu, jäta
// INITIATED) igasuguse tõrke/puuduva konfiguratsiooni korral.
export async function fetchMaksekeskusTransactionStatusByReference(reference) {
  const ref = String(reference || "").trim();
  if (!ref) return null;
  try {
    const data = await callMaksekeskusApi(`/v1/transactions/${encodeURIComponent(ref)}`, {
      method: "GET",
    });
    const status = mapProviderPaymentStatus(
      data?.status || data?.transaction?.status || data?.payment_status || data?.transaction_status || ""
    );
    return { status: status || "PENDING", payload: data };
  } catch {
    return null;
  }
}
