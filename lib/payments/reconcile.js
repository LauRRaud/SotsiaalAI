...
const VERIFIED_TERMINALS = new Map([
  ...
  // SOL-PAY-06: osaline tagastus on oma seis. Ilma temata jääks selline rida
  // igaveseks „pending" seisu ja blokeeriks kordusmakse valikut.
  ["PART_REFUNDED", PaymentStatus.PART_REFUNDED]
]);
...
async function applyReconciledStatus({ db, paymentId, verifiedStatus, payload, now }) {
  ...
  return db.$transaction(async (tx) => {
    ...
    if (nextStatus === PaymentStatus.PAID) {
      ...
      if (!canActivateHere) {
        await writePaymentAudit(tx, { action: "reconcile_paid_needs_webhook", result: "skipped", paymentId, inviteId: current.inviteId, subscriptionId: current.subscriptionId });
      ...
      await writePaymentAudit(tx, { action: "reconcile_activate", result: "paid", paymentId, subscriptionId: updated.subscriptionId });
      return "activated";
    }

    // FAILED / CANCELED / REFUNDED: märgi makse terminaliks; kutse revoke'itakse.
    // SOL-PAY-06: OSALINE tagastus ei ole clawback — ta märgitakse ära, aga kutset
    // ta ei revoke'i ega ligipääsu ei lõpeta.
    const partialRefund = nextStatus === PaymentStatus.PART_REFUNDED;
    await tx.payment.update({
      where: { id: paymentId },
      data: {
        status: nextStatus,
        ...(nextStatus === PaymentStatus.FAILED || nextStatus === PaymentStatus.CANCELED ? { failedAt: now } : {}),
        ...(nextStatus === PaymentStatus.REFUNDED || partialRefund ? { refundedAt: now } : {}),
        raw: buildPaymentRawRecord(baseRaw, payload)
      }
    });
    if (
      ...
      // A terminal provider decision releases the RECONCILE_PENDING blocker. Move
      // the subscription to the next attempt at the same time, otherwise the
      // worker derives the same provider reference and skips it forever as a
      // duplicate payment.
    ...
    if (current.inviteId && !partialRefund) {
      await tx.invite.updateMany({
        where: { id: current.inviteId, status: { in: ["PENDING_PAYMENT", "SENT"] } },
        data: { status: "REVOKED" }
      });
    }
    await writePaymentAudit(tx, { action: "reconcile_terminal", result: String(nextStatus).toLowerCase(), paymentId, inviteId: current.inviteId, subscriptionId: current.subscriptionId });
    if (nextStatus === PaymentStatus.FAILED) return "failed";
    if (nextStatus === PaymentStatus.CANCELED) return "canceled";
    return partialRefund ? "part_refunded" : "refunded";
  });
}
