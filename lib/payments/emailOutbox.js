...
/**
 * Repo-hallatav, VAIKIMISI MITTEAKTIIVNE worker (route-värav). Claim CAS-iga,
 * backoff, terminalne olek. Kordus ei korda makset ega õiguse andmist — ainult
 * saadab e-kirja uuesti.
 */
export async function runPaymentEmailDelivery({
  ...
  for (const { id } of rows) {
    ...
    try {
      ...
      await db.paymentEmailOutbox.updateMany({
        where: { id, status: "SENDING", claimedAt: now },
        data: { status: "SENT", sentAt: new Date(), nextAttemptAt: null, lastErrorCode: null }
      });
      counters.sent += 1;
    } catch (error) {
