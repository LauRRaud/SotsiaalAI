 ...
 *
 * MIS OLI VALESTI. `Promise.race()` piiras ootamist, aga ei katkestanud algset
 * `sendMail()` tööd — SMTP võis kirja pärast timeout'i ikkagi vastu võtta. Sama
 * kehtis lease-taastel: `SENDING` rida tõsteti `RETRY`-ks teadmata, mis juhtus.
 * Uus katse saatis sama kirja UUESTI ja ilma püsiva sõnumitunnuseta oli see
 * adressaadi jaoks TEINE kiri: kaks maksekinnitust, kaks clawback-teadet, kaks
 * kutselinki.
 *
 * MIS SIIN ON. Kaks asja, mille peale leping saab toetuda:
 *
 *   · **Püsiv Message-ID** (`PaymentEmailOutbox.messageId`) — korduskatse kannab
 *     sama tunnust, seega duplikaat on RFC mõttes sama sõnum ja postiklient
 *     tunneb ta ära.
 *   · **`AMBIGUOUS` on oma seis**, mitte `RETRY`. Ja tema kohtlemine sõltub
 *     kirjast:
 *       – **kandja-kirjad** (kutselink, sisemine omaniku teade) korratakse:
 *         nende sisu on identne ja saamata jäänud link on suurem kahju kui
 *         topeltkiri;
 *       – **inimesele suunatud UUDISED** (maksekinnitus, sponsorluse
 *         tagasivõtmine) EI lähe pimedale kordusele. Nad jäävad `AMBIGUOUS`
 *         seisu ilma järgmise katseta — otsuse teeb inimene. Just seda nõuab
 *         kriteerium: „ebamäärane tulemus peab olema eraldi review/reconcile
 *         olek, mitte pime retry tundliku kirja puhul".
 */
const AMBIGUOUS_RETRY_SAFE_TEMPLATES = new Set([
  "invite_sponsored",
  "invite_create",
  "invite_resend",
  "owner_webhook"
...
export function isAmbiguousRetrySafe(template) {
  return AMBIGUOUS_RETRY_SAFE_TEMPLATES.has(String(template || ""));
}
...
/**
 * SOL-PAY-11: püsiv sõnumitunnus sünnib KOOS reaga, mitte saatmise hetkel —
 * korduskatse peab kandma sama ID-d, muidu on ta adressaadi jaoks teine kiri.
 */
export function mintOutboxMessageId(seed = "") {
  const unique = String(seed || "").trim() || crypto.randomUUID();
  return `<${crypto.createHash("sha256").update(unique).digest("hex").slice(0, 32)}@sotsiaal.ai>`;
}
 ...
 *
 * NB (SOL-PAY-07 õppetund): TEHINGU SEES ei kõlba erindipõhine duplikaadi-haru —
 * unikaalsuse rikkumine mürgitab tehingu. Kutsuja peab siis olemasolu ise ENNE
 * kontrollima.
 */
  ...
  try {
    await db.paymentEmailOutbox.create({
      data: {
        ...
        messageId: mintOutboxMessageId(String(dedupeKey)),
        payload,
        paymentId,
        inviteId,
        status: "PENDING",
  ...
  switch (row.template) {
    ...
    case "sponsored_revoked": {
      ...
      return {
        ...
        html: serverT(locale, "email.invite.sponsored_revoked.html", {
          roomTitle: escapeHtml(values.roomTitle),
    ...
    case "field_safety_escalation":
    case "field_safety_resolved":
      return {
        subject: String(payload.subject || "SotsiaalAI välitöö teavitus"),
...
export async function runPaymentEmailDelivery({
  ...
  const templateFilter = Array.isArray(templates)
    ? [...new Set(templates.map(value => String(value || "").trim()).filter(Boolean))]
    : [];
  const counters = {
    eligible: 0,
    claimed: 0,
  ...
  if (!dryRun) {
    /* SOL-PAY-11: lease-taaste EI TEA, kas SMTP kirja vastu võttis. Vana kood
       tõstis rea `RETRY`-ks (= pime kordus); nüüd läheb ta `AMBIGUOUS` seisu ja
       järgmise katse saab ainult kandja-kiri. Tundlik kiri jääb inimesele. */
    const stale = await db.paymentEmailOutbox.findMany({
      where: {
        status: "SENDING",
        claimedAt: { lt: leaseCutoff },
        ...(templateFilter.length ? { template: { in: templateFilter } } : {})
      },
      select: { id: true, template: true, attempts: true }
    });
    for (const row of stale) {
      const retrySafe = isAmbiguousRetrySafe(row.template);
      await db.paymentEmailOutbox.updateMany({
        where: { id: row.id, status: "SENDING" },
        data: {
          status: "AMBIGUOUS",
          lastErrorCode: "AMBIGUOUS_AFTER_LEASE",
          nextAttemptAt: retrySafe ? new Date(now.getTime() + backoffMs(Number(row.attempts || 0))) : null
        }
      });
      counters.ambiguous += 1;
      if (!retrySafe) counters.review += 1;
    }
  }

  /* `AMBIGUOUS` kandja-kiri tuleb tagasi valikusse (tal on `nextAttemptAt`);
     ülevaatust ootaval real on `nextAttemptAt = null`, mille `lte` välistab —
     poliitika elab andmetes, mitte teises päringus. */
  const rows = await db.paymentEmailOutbox.findMany({
    where: {
      status: { in: ["PENDING", "RETRY", "AMBIGUOUS"] },
  ...
  for (const { id } of rows) {
    ...
    try {
      ...
      /* SOL-PAY-11: sõnumitunnus on PÜSIV ja tuleb realt. Kui rida on vanem kui
         see parandus (`messageId` NULL), mindime ta esimesel katsel ja salvestame
         — järgmine katse kannab juba sama tunnust. */
      const messageId = row?.messageId || mintOutboxMessageId(row?.dedupeKey || id);
      if (!row?.messageId) {
        await db.paymentEmailOutbox.updateMany({ where: { id }, data: { messageId } });
      }
      await Promise.race([
        transport.sendMail({
          to: recipient,
      ...
      /* SOL-PAY-11: timeout EI ole tõrge — ta on TEADMATUS. `Promise.race` ei
         katkesta SMTP tööd, seega kiri võis kohale jõuda. Vana kood pani ta
         `RETRY`-ks (pime kordus); nüüd on ta oma seis ja tundlik kiri jääb
         inimesele otsustada. */
      const ambiguous = code === "EMAIL_TIMEOUT";
      const retrySafe = isAmbiguousRetrySafe(row?.template);
      const retry = attempts < attemptLimit && (!ambiguous || retrySafe);

      await db.paymentEmailOutbox.updateMany({
        where: { id, status: "SENDING", claimedAt: now },
        data: {
          status: ambiguous ? "AMBIGUOUS" : retry ? "RETRY" : "FAILED",
      ...
      logPaymentEvent("payment_email_delivery_error", {
        outboxId: id,
