...
import { RENEWAL_BLOCKING_STATUSES } from "@/lib/payments/providerOutcome";
...
export function getDueRecurringSubscriptionWhere(now = new Date()) {
  return {
    ...
    /* SOL-PAY-02/-05: lahendamata katsega tellimust EI laadita uuesti. Teadmata
       tulemusega kutse võis raha juba võtta; teine katse oleks sama kuu eest
       teine laadimine. Rea lahendab webhook, reconciliation-worker või (ülevaatuse
       korral) inimene, mille järel tellimus tuleb valikusse tagasi. */
    payments: { none: { status: { in: RENEWAL_BLOCKING_STATUSES } } },
  };
}
...
export function getUnresolvedRenewalSubscriptionWhere(now = new Date()) {
  return {
    ...getDueRecurringSubscriptionBaseWhere(now),
    payments: { some: { status: { in: RENEWAL_BLOCKING_STATUSES } } },
  };
}
