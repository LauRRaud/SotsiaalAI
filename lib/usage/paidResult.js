 ...
 *
 *  1. viga ENNE commit'i vabastab vaikimisi reservatsiooni — kasutamata tööst ei
 *     jää kunagi arvestatud ühikut. Marsruut võib kasutaja kontrollitava
 *     püsistustõrke erandina commit'ida, et juba tehtud mudelitööd ei saaks tasuta
 *     korrata;
 ...
 * @param commitOnPersistError `(error) => boolean` — kitsas erand juhuks, kus
 *                 tasuline töö on tehtud, kuid kasutaja kontrollitav püsistustõrge
 *                 ei tohi muuta mudelikutset tasuta korratavaks.
 *
...
export async function runPaidResult({
  ...
  try {
    ...
    if (typeof commit === "function" && commitOnPersistError?.(error)) {
      try {
        await commit(handle, null)
      } catch (commitError) {
        throw markStage(commitError, PAID_RESULT_STAGES.COMMIT)
      }
      throw markStage(error, PAID_RESULT_STAGES.PERSIST)
    }
