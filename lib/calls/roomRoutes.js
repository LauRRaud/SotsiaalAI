...
import { ROOM_READ, ROOM_WIND_DOWN, ROOM_WRITE, resolveRoomAccess } from "@/lib/rooms/accessGuard";

// Lepingunimed tulevad kutsujatele siit: kõnemarsruut impordib juba sellest moodulist.
export { ROOM_READ, ROOM_WIND_DOWN, ROOM_WRITE };
...
/**
 * Kõne- ja salvestusmarsruutide ainus värav.
 *
 * `intent` vaikeväärtus on `ROOM_WRITE` ja see on tahtlik (SOL-ROOM-01): uus kõnemarsruut,
 * mis lepingut ei nimeta, on arhiveeritud ruumis KINNI. Nimelised erandid on need rajad, mis
 * ainult LÕPETAVAD juba avatud asja — neid ei tohi sulgeda, muidu jääks arhiveerimise hetkel
 * käimasoleva kõne osaleja lukku (`ROOM_WIND_DOWN`).
 */
export async function requireRoomCallAccess(roomId, { intent = ROOM_WRITE } = {}) {
  ...
  const { room, member: membership } = access;
