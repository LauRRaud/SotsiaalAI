...
function recordingPurposeLabel(purpose, purposeText = "") {
...
function buildRecordingConsentText({ requesterName, purpose, purposeText }) {
...
export function createCallService({
  ...
  // egress'i KOHE ja kõrvaldab artefakti (fail-closed). Jagatud helikõne = üks
  // segatud OGG — tagasivõtja hääl on juba failis, seega ei tehta seda kättesaadavaks:
  // fail märgitakse DELETED (+ retention möödas kohe), taotlus STOPPED (mitte
  // COMPLETED), dokumenti ei looda. Egress-tõrge ei tohi jätta taotlust ACTIVE-ks.
  async function discardActiveRecording({ callSessionId, recordingRequestId, userId }) {
    ...
    const stoppedAt = now();
    const file = await findRecordingFile(prisma, { recordingRequestId, callSessionId });
    if (file?.egressId && resolvedEgress.stopRecording) {
      await resolvedEgress.stopRecording({ egressId: file.egressId, callSessionId, recordingRequestId }).catch(() => null);
    }
    if (file?.filePath && resolvedRecordingStorage.discardEgressArtifact) {
      await resolvedRecordingStorage.discardEgressArtifact({ fileName: file.filePath }).catch(() => null);
    }
    if (file) {
      await prisma.callRecordingFile.update({
        where: { id: file.id },
        data: { status: "DELETED", retentionUntil: stoppedAt }
      }).catch(() => null);
    }
    const updated = await prisma.callRecordingRequest.update({
      where: { id: request.id },
  ...
  async function deleteRecordingFile({ callSessionId, recordingRequestId, userId, canModerate = false }) {
    ...
    const file = await findRecordingFile(prisma, { recordingRequestId, callSessionId });
    if (!file) throw new Error("call.recording_file_not_found");
    await purgeRecordingFile({ db: prisma, file, storage: resolvedRecordingStorage });
    await writeRecordingAudit(prisma, {
  ...
  return {
    ...
    respondToRecordingConsent: async ({ callSessionId, recordingRequestId, userId, decision, ipAddress, userAgent }) => {
