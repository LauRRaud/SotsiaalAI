...
const OPEN_RECORDING_STATUSES = ["REQUESTED", "READY_TO_RECORD", "ACTIVE", "STOPPING", "STOP_FAILED"];
...
const VISIBLE_RECORDING_STATUSES = ["REQUESTED", "READY_TO_RECORD", "DECLINED", "ACTIVE", "STOPPING", "STOP_FAILED", "STOPPED", "COMPLETED", "FAILED"];
...
export function createCallService({
  ...
  async function stopActiveRecordingForCall({ callSessionId, userId }) {
    ...
    return stopRecording({
      ...
      /* SOL-CALL-01 — kinnitamata stopi EI TOHI siin üle kirjutada. `stopRecording`
         on juba kirjutanud `STOP_FAILED` ja pannud taasproovi järjekorda; pime
         `FAILED` kustutaks selle info ära ja teeks salvestusest, mis võib veel käia,
         tavalise ebaõnnestunud salvestise. */
