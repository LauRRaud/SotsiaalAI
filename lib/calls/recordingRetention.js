 ...
 *
 * SOL-CALL-09 — `finalize` on valikuline hook, mis jookseb SAMAS tehingus viimase
 * astmega (`DELETED`). Kustutuse audit on ainus elutsükli jälg, mille juures
 * tagasipööramine oleks halvem kui tõrge: artefakt on selleks hetkeks juba kettalt
 * kadunud, seega „ennista rida" tähendaks rida olematu faili kohta (sama piir, mille
 * SOL-FIELD-03 nimetas). Siin ei ole seda valikut vaja: kui jälg ei õnnestu, jääb rida
 * `DELETE_PENDING`-iks ja sweep proovib uuesti — puuduv fail on siis ENOENT ehk
 * õnnestumine ja jälg sünnib teisel katsel. Tõrge ja taasproov, mitte vaikne null.
 */
export async function purgeRecordingFile({ db, file, storage = null, finalize = null }) {
  ...
  if (db.callRecordingFile?.update) {
    const markDeleted = client => client.callRecordingFile.update({
      where: { id: file.id },
      data: { status: "DELETED", filePath: null, createdDocumentId: null }
    });
    // `finalize` puudumisel ei avata tehingut — üks kirjutus on niigi atomaarne.
    const commit = finalize
      ? db.$transaction(async tx => {
          await markDeleted(tx);
          await finalize(tx);
        })
      : markDeleted(db);
    const finished = await commit.catch(error => error);
