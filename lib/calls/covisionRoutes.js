...
import { findCovisionParticipantForActor, isCovisionCaseTerminal } from "@/lib/covisionAccessShared";
...
import { withCovisionCallMutationLock } from "@/lib/calls/covisionLifecycle";
...
export async function requireCovisionCallAccess(covisionCaseId, { allowTerminal = false } = {}) {
  try {
    ...
    const terminal = isCovisionCaseTerminal(covisionCase);
    if (!allowTerminal && terminal) {
      return { ok: false, status: 409, message: "covision.errors.case_read_only" };
    }
...
export async function requireCallInCovision(callSessionId, covisionCaseId, { db = prisma } = {}) {
...
export function createCovisionCallService({ db = prisma } = {}) {
...
export async function withCovisionCallMutation(
  covisionCaseId,
  access,
  callback,
  { onTerminal = null } = {}
  ...
  return withCovisionCallMutationLock({
    ...
    covisionCaseId,
    access,
    createService: (tx) => createCovisionCallService({ db: tx }),
    callback,
    onTerminal
  });
}
