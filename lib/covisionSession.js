...
import { queueCovisionInviteDelivery } from "./covisionInviteDelivery.js";
...
const ACTION_PAYLOAD_KEYS = Object.freeze({
  ...
  RESEND_INVITATION: new Set(["participantId"]),
  REVOKE_PARTICIPANT: new Set(["participantId"]),
  DECLINE_INVITATION: new Set(),
...
function normalizeTopicSeedExpectedVersion(value) {
  if (!Number.isInteger(value) || value < 1) throw conflict("TOPIC_SEED_VERSION_CONFLICT");
  return value;
}
...
export function normalizeCovisionStartRequest(input) {
  ...
  return { expectedVersion: normalizeTopicSeedExpectedVersion(input.expectedVersion) };
}
...
export function normalizeCovisionSessionActionRequest(input) {
  ...
  if (["RESEND_INVITATION", "REVOKE_PARTICIPANT"].includes(action)) {
    const participantId = normalizeId(payload.participantId);
    if (!participantId) throw invalid("PARTICIPANT_ID_REQUIRED");
    return { action, expectedVersion, payload: { participantId } };
  }
...
function resolveAccess(covisionCase, userId, email = "") {
  const participant = (covisionCase.participants || []).find((item) =>
    ...
    && !item.identityErasedAt
    && !(
      item.inviteStatus === "INVITED"
      && item.inviteExpiresAt
      && new Date(item.inviteExpiresAt).getTime() <= Date.now()
    )
    && (
      item.userId === userId
...
function serializeParticipant(participant) {
  ...
  return {
    ...
    role: participant.role,
    inviteStatus: participant.inviteStatus,
    deliveryStatus: participant.inviteDelivery?.status || null,
    state: state ? {
...
function serializeSessionCase(covisionCase, access) {
  ...
  const ready = access.role === "OWNER" || Boolean(access.participant?.sessionState?.readyAt);
  const restricted = access.inviteStatus === "INVITED" || !ready;
  if (restricted) {
    return {
...
async function createOwnerSession(tx, covisionCase, expectedVersion) {
  ...
  return { session, participant, now };
}

function sessionAuditMetadata(action, payload, session) {
  const base = { stage: session?.stage || 1, phase: session?.phase || "waiting_room" };
  if (action === "INVITE_PARTICIPANT") return { ...base, invitedRole: payload.role };
  if (["RESEND_INVITATION", "REVOKE_PARTICIPANT"].includes(action)) {
    return { ...base, participantId: payload.participantId };
  }
  if (action === "CONFIRM_PARTICIPANT") {
    return {
      ...base,
      present: Boolean(payload.present),
      roleConfirmed: Boolean(payload.roleConfirmed),
      agreementConfirmed: Boolean(payload.agreementConfirmed),
      ready: Boolean(payload.ready)
    };
  }
  if (["SUBMIT_WORK_ITEM", "UPDATE_WORK_ITEM"].includes(action)) {
    return { ...base, kind: payload.kind || null, status: payload.status || null, workStage: payload.stage || session?.stage };
  }
  if (action === "SAVE_PRIVATE_STATE") return { ...base, kind: payload.kind, privateStage: payload.stage || session?.stage };
  if (action === "SET_PHASE") return { ...base, requestedPhase: payload.phase };
  return base;
}
...
async function inviteParticipant(tx, covisionCase, session, payload) {
  ...
  const inviteExpiresAt = new Date(invitedAt.getTime() + 14 * 24 * 60 * 60 * 1000);
  if (participant) {
  ...
  await queueCovisionInviteDelivery(tx, {
...
async function applyLockedAction(tx, covisionCase, access, request) {
  ...
  if (!session) {
    ...
    const created = await createOwnerSession(tx, covisionCase, expectedVersion);
    await appendCovisionAuditEvent(tx, {
      covisionCaseId: covisionCase.id,
      participantId: created.participant.id,
      actorUserId: access.userId,
      actorRoleSnapshot: access.role,
      action,
      entityType: "SESSION_ACTION",
      entityId: created.session.id,
      idempotencyKey: `${covisionCase.id}:session:${expectedVersion}:${action}`,
      metadata: { stage: 1, phase: created.session.phase },
  ...
  const participantLifecycleAction = ["CONFIRM_PARTICIPANT", "DECLINE_INVITATION"].includes(action);
  if (!MUTATING_ROLES.has(access.role) && !participantLifecycleAction) {
    throw forbidden("ROLE_FORBIDDEN");
  }
  if (access.inviteStatus !== "ACCEPTED" && !participantLifecycleAction) {
    throw forbidden("INVITATION_NOT_ACCEPTED");
  }
  if (
    access.role !== "OWNER"
    && !participantLifecycleAction
    && !access.participant?.sessionState?.readyAt
  ) {
    throw forbidden("PARTICIPANT_NOT_READY");
  }
  ...
  const sessionPatch = {};
  let postCommit = null;
  let auditParticipantId = access.participant?.id || null;
  ...
  if (action === "START_SESSION") {
    ...
    if (confirmation.agreementConfirmed && !roleConfirmedAt) {
      throw invalid("ROLE_CONFIRMATION_REQUIRED");
    }
    ...
    if (
      ...
      await tx.covisionParticipant.update({
        ...
        data: {
          ...
          ...(confirmation.ready && participant.inviteStatus === "INVITED"
            ? { inviteStatus: "ACCEPTED", decisionAt: now }
            : {})
        }
      });
    }
    ...
    assertRole(access, LEADER_ROLES);
    const invited = await inviteParticipant(tx, covisionCase, session, payload);
    auditParticipantId = invited.id;
  } else if (action === "RESEND_INVITATION") {
    assertRole(access, LEADER_ROLES);
    const target = (covisionCase.participants || []).find((item) => item.id === payload.participantId);
    if (
      !target
      || target.inviteStatus !== "INVITED"
      || !target.email
      || (target.inviteExpiresAt && new Date(target.inviteExpiresAt).getTime() <= now.getTime())
    ) {
      throw notFound();
    }
    await queueCovisionInviteDelivery(tx, {
    ...
    auditParticipantId = target.id;
  } else if (action === "REVOKE_PARTICIPANT") {
    assertRole(access, LEADER_ROLES);
    const target = (covisionCase.participants || []).find((item) => item.id === payload.participantId);
    if (!target || target.role === "OWNER" || !LIVE_INVITE_STATUSES.has(target.inviteStatus)) {
      throw notFound();
    }
    await tx.covisionParticipant.update({
      where: { id: target.id },
    ...
    await tx.covisionInviteDelivery.updateMany({
      where: { participantId: target.id, status: { in: ["PENDING", "RETRY"] } },
      data: { status: "CANCELLED", claimedAt: null }
    });
    auditParticipantId = target.id;
  } else if (action === "DECLINE_INVITATION") {
    const participant = assertParticipant(access);
    if (participant.inviteStatus !== "INVITED") throw conflict("INVITATION_NOT_PENDING");
    await tx.covisionParticipant.update({
      where: { id: participant.id },
      data: { inviteStatus: "DECLINED", decisionAt: now }
    });
    await tx.covisionInviteDelivery.updateMany({
      where: { participantId: participant.id, status: { in: ["PENDING", "RETRY"] } },
      data: { status: "CANCELLED", claimedAt: null }
    });
    postCommit = { accessEnded: true };
  ...
  await appendCovisionAuditEvent(tx, {
    covisionCaseId: covisionCase.id,
    participantId: auditParticipantId,
    actorUserId: access.userId,
    actorRoleSnapshot: access.role,
    action,
    entityType: "SESSION_ACTION",
    entityId: session.id,
    idempotencyKey: `${covisionCase.id}:session:${expectedVersion}:${action}`,
    metadata: sessionAuditMetadata(action, payload, session),
    occurredAt: now
  });
  return postCommit;
}
...
export async function applyCovisionSessionAction(
  ...
  if (postCommit?.accessEnded) return { ok: true, accessEnded: true };
