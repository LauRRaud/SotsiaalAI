...
// Case creation is narrower than participant access: a service provider may
// join an explicitly invited case but cannot become its owner through this API.
export function assertCovisionCreator(auth) {
  const role = String(auth?.role || "").trim().toUpperCase();
  if (auth?.isAdmin === true || role === "ADMIN" || role === "SOCIAL_WORKER") return auth;
