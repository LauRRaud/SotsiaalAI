...
function cancellationAcknowledged(payload) {
  return Boolean(payload && typeof payload === "object" && payload.cancelAcknowledgedAt);
}
...
/**
 * @param fence `true` = tulemuse kirjutus (done/error): tohib ainult praegune omanik.
 *              `false` = juhttoiming (cancel): tohib ka keegi teine protsess, sest peatamine ei ole
 *              tulemus. Ilma selle vaheteta kukuks omaniku enda Stop worker-režiimis alati läbi.
 */
    ...
    await prisma.researchJob.deleteMany({
      where: {
        OR: [
          ...
          { status: "cancelled", payload: { path: ["cancelAcknowledgedAt"], not: Prisma.DbNull } },
        ],
        endedAt: { lt: cutoff },
      },
    });
...
export async function cancelResearchJob(job, message = "research.error.cancelled") {
  ...
  // Inline-jooksja abortController on siin päris töö oma ja abort on sünkroonselt nähtav. Teise
  // protsessi snapshot ei tohi kinnitust teeselda: tema rida peab jääma workerile signaaliks.
  const acknowledgedAt = jobs.get(job.id) === job && job.ownedByThisProcess !== false ? endedAt : null;
  const persisted = await transitionResearchJobToTerminal(job, {
    status: "cancelled",
    error,
    workerId: null,
    leaseUntil: null,
    endedAt: new Date(endedAt),
    ...(acknowledgedAt ? { payload: { ...(job.payload || {}), cancelAcknowledgedAt: acknowledgedAt } } : {}),
  }, { fence: false });
  if (!persisted) return;
...
export async function deleteResearchJobForOwner({ jobId, userId }) {
  ...
  if (status === "cancelled" && !cancellationAcknowledged(record?.payload || local?.payload)) {
    return "active";
  }
...
export async function syncResearchCancellation(job) {
  ...
  try {
    const record = await prisma.researchJob.findUnique({
      where: { id: job.id },
      select: { status: true, error: true, workerId: true, payload: true },
    });
    if (record?.status === "cancelled") {
      ...
      // Kinnitus avaldatakse alles pärast lokaalse runtime'i aborti. DELETE võib markerit nähes rea
      // eemaldada, seega vastupidine järjekord looks sama võistluse ühe protsessi sees uuesti.
      await prisma.researchJob.updateMany({
        where: { id: job.id, status: "cancelled" },
        data: { payload: { ...payload, cancelAcknowledgedAt: acknowledgedAt } },
      });
      return true;
    }
...
export async function getActiveResearchJobCount(userId) {
  ...
  return prisma.researchJob.count({
    where: {
      ...
      OR: [
        { status: { in: ACTIVE_STATUSES } },
        { status: "cancelled", payload: { path: ["cancelAcknowledgedAt"], equals: Prisma.DbNull } },
      ],
    },
  });
}
