...
import { DEFAULT_MODEL, RAG_BASE } from "@/lib/chat/settings";
...
function renderReportText(report, locale, targetStyle) {
  ...
  const evidenceIndexById = new Map(
    (Array.isArray(report.evidence) ? report.evidence : [])
      .map((ev, index) => [String(ev?.id || "").trim(), String(index + 1)])
      .filter(([id]) => id)
  );
  ...
  if (!report.findings.length) {
    ...
    for (const finding of report.findings) {
      const refs = Array.isArray(finding.evidence_refs)
        ? finding.evidence_refs
            .map(ref => evidenceIndexById.get(String(ref || "").trim()))
            .filter(Boolean)
        : [];
      const refPart = refs.length ? ` [${Array.from(new Set(refs)).join(", ")}]` : "";
      lines.push(`- ${finding.claim}${refPart} (${i18n.confidence}: ${formatConfidence(finding.confidence, locale)})`);
    }
  }
...
function toConversationSources(evidence) {
  ...
  return evidence.map(item => {
    ...
    return {
      ...
      journalTitle: item.journalTitle || undefined,
...
export async function runDeepResearchJob(job) {
  ...
  try {
    ...
    /* SOL-RES-05: vestlusse salvestamine EI OLE kõrvaltoiming. Varem neelasid persistInit/Append/
       Done kõik DB-vead ja pipeline ei vaadanud tagastusväärtust — uuring märgiti `done`, kasutus
       commit'iti, aga pärast navigeerimist avanes vestlus ILMA raportita. Nüüd on lõpp seotud
       kinnitatud püsikoopiaga: kui vestlussõnumit ei õnnestu kirjutada, EI märgita tööd
       lõppenuks ja kasutust ei arvestata — töö jääb aktiivseks ja on korratav.

       Kirjutus on job-idempotentne (`persistKey`), seega korduskatse ega teine worker ei tekita
       vestlusse teist raportit (SOL-RES-04 kriteeriumi viimane lause). */
    let conversationCopyConfirmed = true;
    if (payload.persist && payload.convId && payload.userId) {
      await persistAppend({
        convId: payload.convId,
        userId: payload.userId,
      ...
      const persistKey = `research:${job.id}`;
      let persisted = await persistDone({
        convId: payload.convId,
        userId: payload.userId,
        status: "COMPLETED",
        ...
        isCrisis: false,
        persistKey,
        metadataExtra: { researchJobId: job.id },
      });
      if (!persisted) {
        // Üks kordus: ajutine DB-tõrge ei tohi tasulist tulemust ära visata.
        persisted = await persistDone({
          convId: payload.convId,
          userId: payload.userId,
          status: "COMPLETED",
          finalText: reportText,
      ...
      conversationCopyConfirmed = Boolean(persisted?.assistantMessageId);
    }

    if (!conversationCopyConfirmed) {
      /* Töö jääb aktiivseks: tulemus on olemas, aga kasutajale taasavatavat koopiat ei ole veel
         kinnitatud. Kasutust EI commit'ita — `done` tähendab siin lubadust, mida me veel täita ei
         saa. Aegunud aktiivne töö läheb hiljem ausalt `error`-iks ja reservatsioon vabaneb. */
      await publishResearchProgress(job, { stage: "finalizing", persistPending: true });
      try {
        console.error("[research][pipeline] conversation copy not confirmed, job stays active", {
          jobId: job.id,
        });
      } catch {}
      return;
