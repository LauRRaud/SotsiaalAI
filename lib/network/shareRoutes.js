...
import { createNetworkShareOutbox } from "@/lib/network/shareOutbox";
import { createRoomForNetworkShare } from "@/lib/network/shareRoom";
import { guardShareRequest as runShareRequestGuard } from "@/lib/network/shareRequestGuard";
 ...
 */
export async function hasFrameworkAcceptance(userId, { db = prisma } = {}) {
...
export function guardShareRequest(request, auth, actionCode, { mutation = false, resourceId = null } = {}) {
  return runShareRequestGuard({
    db: prisma,
    request,
    userId: auth?.userId,
    actionCode,
...
export function createShareOutboxPort() {
  return ({ share, db, now }) => createNetworkShareOutbox({ share, db, now });
}
...
const STATUS_BY_CODE = new Map([
  ...
  ["network_share.participation_ended", 410],
  ...
  ["network_share.source_inactive", 409],
  ["network_share.idempotency_key_required", 428]
]);
