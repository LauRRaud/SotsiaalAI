...
export async function createNetworkShare({
  ...
  const isExternalClient = !clientUserId;
  if (isExternalClient && !displayName) fail("network_share.client_required");
  if (clientUserId && clientUserId === recipientUserId) fail("network_share.client_cannot_be_recipient");
  if (clientUserId && clientUserId === workerId) fail("network_share.worker_cannot_be_client");
