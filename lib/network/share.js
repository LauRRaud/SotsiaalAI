...
const CLIENT_VISIBLE_STATUSES = Object.freeze([
  NetworkShareStatus.AWAITING_CLIENT,
  NetworkShareStatus.CONFIRMED,
  NetworkShareStatus.DECLINED,
  NetworkShareStatus.SENT,
  NetworkShareStatus.OPENED,
  NetworkShareStatus.RESPONDED,
  NetworkShareStatus.RECALLED,
  NetworkShareStatus.ENDED
]);
const RECIPIENT_LIST_STATUSES = Object.freeze([
  NetworkShareStatus.SENT,
  NetworkShareStatus.OPENED,
  NetworkShareStatus.RESPONDED
]);
...
const RECIPIENT_LIST_SELECT = Object.freeze({
  id: true,
...
function encodeShareCursor(row) {
  if (!row?.id || !row?.updatedAt) return null;
  return Buffer.from(JSON.stringify([new Date(row.updatedAt).toISOString(), row.id]), "utf8").toString("base64url");
}

function decodeShareCursor(value) {
  if (!value) return null;
  try {
    const [timestamp, id] = JSON.parse(Buffer.from(String(value), "base64url").toString("utf8"));
    const updatedAt = new Date(timestamp);
    if (!id || !Number.isFinite(updatedAt.getTime())) fail("network_share.invalid_cursor");
    return { updatedAt, id: String(id) };
  } catch (error) {
    if (error instanceof NetworkShareError) throw error;
    fail("network_share.invalid_cursor");
  }
}

export async function listNetworkShares({
  prisma,
  viewerUserId,
  role,
  ...
  if (!["worker", "client", "recipient"].includes(normalizedRole)) fail("network_share.invalid_role");
  const allowedStatuses = normalizedRole === "client"
    ? CLIENT_VISIBLE_STATUSES
    : normalizedRole === "recipient"
      ? RECIPIENT_LIST_STATUSES
      : Object.values(NetworkShareStatus);
  const requestedStatus = String(status || "").trim().toUpperCase();
  if (requestedStatus && !allowedStatuses.includes(requestedStatus)) fail("network_share.invalid_status_filter");
  const pageSize = Math.max(1, Math.min(Number(limit) || 50, 100));
  const after = decodeShareCursor(cursor);
  const today = toDateOnly(now);
  const ownerWhere = normalizedRole === "client"
    ? { clientUserId: viewerUserId }
    : normalizedRole === "recipient"
      ? { recipientUserId: viewerUserId, participationEndsOn: { gte: today } }
      : { workerId: viewerUserId };
  const where = {
    ...ownerWhere,
    ...(sourcePreInquiryId ? { sourcePreInquiryId: String(sourcePreInquiryId) } : {}),
    status: requestedStatus ? requestedStatus : { in: allowedStatuses },
    ...(after ? {
      OR: [
  ...
  const rows = await prisma.networkShare.findMany({
    where,
    select: normalizedRole === "client"
      ? CLIENT_LIST_SELECT
      : normalizedRole === "recipient"
        ? RECIPIENT_LIST_SELECT
        : LIST_SELECT,
    orderBy: [{ updatedAt: "desc" }, { id: "desc" }],
    take: pageSize + 1
  });
  ...
  return { rows: page, nextCursor: hasMore ? encodeShareCursor(page.at(-1)) : null };
}
...
export function isActiveNetworkShareSource(source, { workerId } = {}) {
  return Boolean(
    source
    && source.recipientOwnerId === workerId
    && source.status === "SENT"
    && source.sentAt
    && !source.recalledAt
...
function assertActiveNetworkShareSource(source, workerId) {
  if (!source) fail("network_share.source_not_found");
  if (source.recipientOwnerId !== workerId) fail("network_share.source_forbidden");
  if (!isActiveNetworkShareSource(source, { workerId })) fail("network_share.source_inactive");
}
 ...
 */
async function commitOnce(db, { id, where, data, conflictCode, lifecycle = null }) {
  const run = async (tx) => {
    const result = await tx.networkShare.updateMany({ where: { id, ...where }, data });
    if (!result || result.count !== 1) fail(conflictCode);
    const changed = await tx.networkShare.findFirst({ where: { id } });
    if (lifecycle && tx.domainEvent?.create && tx.dataAuditLog?.create) {
      await recordNetworkShareLifecycle({ db: tx, share: changed, ...lifecycle });
    }
    return changed;
  };
  return lifecycle && typeof db.$transaction === "function" ? db.$transaction(run) : run(db);
}
...
export async function createNetworkShare({
  ...
  assertActiveNetworkShareSource(source, workerId);
  ...
  const run = async (tx) => {
    const currentSource = await tx.preInquiry.findFirst({ where: { id: sourcePreInquiryId } });
    assertActiveNetworkShareSource(currentSource, workerId);
    const created = await tx.networkShare.create({ data: {
      sourcePreInquiryId,
      workerId,
    ...
    if (tx.domainEvent?.create && tx.dataAuditLog?.create) {
      await recordNetworkShareLifecycle({
        db: tx, share: created, actorUserId: workerId, actionCode: "CREATE", mutationKey, now: created.createdAt
      });
    }
    return created;
  };
...
export async function updateNetworkShareDraft({
  ...
  return commitOnce(prisma, {
    ...
    lifecycle: { actorUserId: workerId, actionCode: "UPDATE", fromStatus: share.status, mutationKey, now: now() }
  });
}

/** Töötaja saadab jagamise kliendile ülevaatamiseks. */
export async function submitToClient({ prisma, shareId, workerId, now = () => new Date(), mutationKey = null }) {
  const share = await loadForWorker(prisma, { shareId, workerId });
  ...
  return commitOnce(prisma, {
    ...
    conflictCode: "network_share.concurrent_change",
    lifecycle: { actorUserId: workerId, actionCode: "SUBMIT", fromStatus: share.status, mutationKey, now: now() }
  });
}
...
export async function clientRespondToShare({
  ...
  return commitOnce(prisma, {
    ...
    lifecycle: { actorUserId: clientUserId, actionCode: "DECIDE", fromStatus: share.status, mutationKey, now: stamp }
  });
}
...
export async function attestClientDecision({
  ...
  const run = async (tx) => {
    ...
    return commitOnce(tx, {
      ...
      conflictCode: "network_share.content_changed",
      lifecycle: { actorUserId: workerId, actionCode: "ATTEST", fromStatus: share.status, mutationKey, now: stamp }
    });
  };
  try {
...
export async function sendNetworkShare({
  ...
  const run = async (tx) => {
    ...
    await assertExternalFrameworkAcceptances({ share, hasFrameworkAcceptance, db: tx });
    const source = await tx.preInquiry.findFirst({ where: { id: share.sourcePreInquiryId } });
    assertActiveNetworkShareSource(source, workerId);
    assertParticipationActive(share, stamp);
    ...
    if (tx.domainEvent?.create && tx.dataAuditLog?.create) {
      await recordNetworkShareLifecycle({
        db: tx, share: sent, actorUserId: workerId, actionCode: "SEND", fromStatus: share.status, mutationKey, now: stamp
      });
    }
    return sent;
  };

  /* Kergem klient ei pruugi `$transaction`-it pakkuda. ATOMAARSUS ise on päris
     andmebaasi omadus ja vajab eraldi käsitsi runtime-kontrolli. */
  try {
    return typeof prisma.$transaction === "function"
      ? await prisma.$transaction(run, { isolationLevel: "Serializable" })
      : await run(prisma);
...
export async function markShareOpened({ prisma, shareId, recipientUserId, now = () => new Date(), mutationKey = null }) {
  const share = await prisma.networkShare.findFirst({ where: { id: shareId } });
  if (!share) fail("network_share.not_found");
  ...
  return commitOnce(prisma, {
    ...
    lifecycle: { actorUserId: recipientUserId, actionCode: "OPEN", fromStatus: share.status, mutationKey, now: stamp }
  });
}

/** Tagasivõtmine on võimalik ainult enne avamist. */
export async function recallNetworkShare({ prisma, shareId, workerId, now = () => new Date(), mutationKey = null }) {
  const share = await loadForWorker(prisma, { shareId, workerId });
  ...
  const run = async (tx) => {
    const recalled = await commitOnce(tx, {
      ...
      lifecycle: { actorUserId: workerId, actionCode: "RECALL", fromStatus: share.status, mutationKey, now: stamp }
    });
...
/** Saaja esimene päris ruumivastus viib kandva jagamise idempotentselt RESPONDED-olekusse. */
export async function markNetworkShareRespondedByRoom({
  db,
  roomId,
  authorUserId,
  messageId,
  ...
  const share = await db.networkShare.findFirst({ where: { roomId } });
  if (!share || share.recipientUserId !== authorUserId) return null;
  assertParticipationActive(share, now);
  if (share.status === NetworkShareStatus.RESPONDED) return share;
  if (![NetworkShareStatus.SENT, NetworkShareStatus.OPENED].includes(share.status)) return null;
  return commitOnce(db, {
    id: share.id,
    where: { status: { in: [NetworkShareStatus.SENT, NetworkShareStatus.OPENED] } },
    data: { status: NetworkShareStatus.RESPONDED, respondedAt: now, updatedAt: now },
    conflictCode: "network_share.concurrent_change",
    lifecycle: {
      actorUserId: authorUserId,
      actionCode: "RESPOND",
      fromStatus: share.status,
      mutationKey: `room-message:${messageId}`,
      now
    }
  });
}

export function clientProjection(share, { viewerUserId } = {}) {
  if (!share || !share.clientUserId || share.clientUserId !== viewerUserId) return null;
  if (!CLIENT_VISIBLE_STATUSES.includes(share.status)) return null;
  return {
    id: share.id,
