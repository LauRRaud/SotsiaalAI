...
import { readAllWellbeingRecordsForUser } from "./records.js";
...
function formatWellbeingFactor(value) {
  const quickCheckLabel = formatQuickCheckFactor(value);
  return quickCheckLabel === value ? wellbeingLabel(value) : quickCheckLabel;
}
...
function topCounts(map, limit = 5) {
  return [...map.entries()]
    ...
    .map(([key, count]) => ({
      key,
      label: formatWellbeingFactor(key),
      count
    }));
}
...
function buildManagerMemo({ recordCount, signalCounts, topLoadFactors, topResourceFactors, riskMarkers, workflowCounts: workflows }) {
  const signal = wellbeingLabel(periodSignal(signalCounts, recordCount));
  const signalLine = `Perioodi signaal: ${signal}. Punaseid signaale: ${signalCounts.red}, kollaseid: ${signalCounts.yellow}, rohelisi: ${signalCounts.green}.`;
...
/* SOL-WB-10: „Kõik" tähendas 100 uusimat kirjet. Pikaajalisel kasutajal kadusid
   vanemad kirjed trendist ja juhiga jagatavast memost ilma ühegi hoiatuseta —
   ja kuna ainus punane signaal võib olla just vanim, oli memo vaikselt vale.
   Ülevaade loeb nüüd kogu valitud perioodi stabiilses järjestuses; kaitsepiir on
   alles, aga tema tabamine on vastuses näha. Piir on kasutajapõhine ja seetõttu
   tunduvalt madalam kui koondil. */
const OVERVIEW_MAXIMUM_RECORDS = 20000;
...
export async function buildWellbeingOverviewForUser(userId, filters = {}, options = {}) {
  ...
  const { records, truncated } = await readAllWellbeingRecordsForUser(userId, {
    workflowType: filters.workflowType,
    periodStart: period.periodStart,
    periodEnd: period.periodEnd,
    /* Piir tuleb serveri koodist, mitte päringustringist. */
    maxRecords: Number(options.maxRecords) > 0 ? Math.trunc(options.maxRecords) : OVERVIEW_MAXIMUM_RECORDS
  }, options);
  ...
  return {
    ...
    /* Kui piir sai täis, ütleb vastus seda välja — „Kõik" ei tohi vaikselt
       tähendada „nii palju kui mahtus". */
    truncated,
