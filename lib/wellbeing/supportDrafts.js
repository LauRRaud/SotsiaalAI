...
export async function listWellbeingOutputDraftsForUser(userId, filters = {}, options = {}) {
  ...
  /* SOL-WB-15: sama vaikne lõpp mustanditel, ainult 50 rea juures — ja siin on
     kaotus suurem, sest mustand on kasutaja enda kirjutatud tekst. Sama
     stabiilne järjestus ja kursor mis kirjetel. */
  const take = normalizeTake(filters.take);
  const cursor = String(filters.cursor || "").trim();
  const rows = await prisma.wellbeingOutputDraft.findMany({
    where: {
      userId: normalizedUserId,
  ...
  const drafts = hasMore ? rows.slice(0, take) : rows;
  return { drafts, hasMore, nextCursor: hasMore ? drafts.at(-1)?.id || null : null };
}

/**
 * SOL-WB-16: mustandi AVAMINE. Ilma temata nägi kasutaja pärast lehelt lahkumist
 * ainult seda, et mustand on olemas — tundlikku teksti ei saanud tagasi ega
 * kustutada.
 *
 * Omanik-skoop `findFirst`-iga, sama muster mis kirjel: võõra mustandi ID
 * tagastab `null` (marsruut → 404), nii et olemasolu ei leki üle omanikupiiri.
 */
export async function getWellbeingOutputDraftForUser(userId, draftId, options = {}) {
  const normalizedUserId = requireUserId(userId);
...
/**
 * SOL-WB-16: mustandi KUSTUTAMINE, idempotentselt.
 *
 * ÜLEANTUD MUSTANDI POLIITIKA, kirja pandult. Kustutamine on lubatud ka siis,
 * kui mustand on kovisiooni üle antud — see on kasutaja ENDA privaatne tuletis
 * ja tema õigus oma andmeid kustutada ei sõltu sellest, et ta kunagi midagi
 * jagas. AGA kustutus EI VÕTA jagatut tagasi: kovisiooni juhtum on eraldi
 * jagatud objekt oma elutsükliga. Vastus ütleb selle välja (`handedOff`), et
 * liides saaks kasutajale ausalt öelda, mis kaob ja mis jääb.
 *
 * `deleteMany` omanik-skoobiga: võõras või olematu ID annab `count: 0` ilma
 * viskamiseta ja marsruut teeb sellest 404.
 */
export async function deleteWellbeingOutputDraftForUser(userId, draftId, options = {}) {
  const normalizedUserId = requireUserId(userId);
  const normalizedDraftId = String(draftId || "").trim();
  if (!normalizedDraftId) {
    const error = new Error("wellbeing.errors.output_draft_missing");
  ...
  return withWellbeingOutputDraftLock(prisma, normalizedDraftId, async (tx) => {
    const existing = await tx.wellbeingOutputDraft.findFirst({
      where: { id: normalizedDraftId, userId: normalizedUserId },
      select: { id: true, covisionCaseId: true, handedOffAt: true }
    });
    if (!existing) return { deleted: false, handedOff: false };

    const result = await tx.wellbeingOutputDraft.deleteMany({
      where: { id: normalizedDraftId, userId: normalizedUserId }
    });
    return {
      deleted: Number(result?.count) === 1,
      handedOff: Boolean(existing.covisionCaseId || existing.handedOffAt)
    };
  });
}

/**
 * SOL-WB-16: lähtekirje kustutamisel jäid mustandid alles katkenud
 * naasmispunktiga — `sourceRecordId` on FK-ta tekstiväli, seega kaskaad neid ei
 * puuduta. Valik on nüüd TEADLIK ja kutsuja peab ta tegema; vaikimisi säilivad
 * mustandid, sest nad on eraldi kirjutatud tekst ja vaikne kustutus oleks
 * andmekadu.
 */
export async function deleteWellbeingOutputDraftsForRecord(userId, sourceRecordId, options = {}) {
  const normalizedUserId = requireUserId(userId);
  const recordId = String(sourceRecordId || "").trim();
  if (!recordId) return { deleted: 0 };
  const prisma = options.prisma || defaultPrisma;
  const result = await prisma.wellbeingOutputDraft.deleteMany({
    where: { userId: normalizedUserId, sourceRecordId: recordId }
  });
...
/* E1: ühe kirjega seotud mustandid (kirje↔mustandi side + handoff-ajalugu).
   Omanik-skoop; `sourceRecordId` seob mustandi lähtekirjega. Handoff-ajalugu
   loeb `covisionCaseId`/`handedOffAt` väljadelt, mis mustandil juba on. */
export async function listWellbeingOutputDraftsForRecord(userId, sourceRecordId, options = {}) {
  const normalizedUserId = requireUserId(userId);
  const normalizedRecordId = String(sourceRecordId || "").trim();
  if (!normalizedRecordId) return [];
  const prisma = options.prisma || defaultPrisma;
  return prisma.wellbeingOutputDraft.findMany({
    where: { userId: normalizedUserId, sourceRecordId: normalizedRecordId },
