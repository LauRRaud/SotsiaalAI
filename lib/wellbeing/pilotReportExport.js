...
/* SOL-WB-06 saba: avaldamata signaalilahter EI ole null. Nii Excel kui
   prindivaade lõikavad arvu tema kontekstist lahti — seal on „0" lõplik väide
   ja just seda viga summutus muidu ise tekitaks. */
export function signalCell(value) {
  return value === null || value === undefined ? "avaldamata" : value;
}

/* SOL-WB-01: piir, mille all koond arvutati, käib aruande päises kaasa — muidu
   näeb platvormiülene valim välja kohaliku asutuse tulemusena. */
function filterSummary(filters = {}) {
  return [
    filters.organizationId ? `Organisatsioon: ${filters.organizationId}` : null,
    filters.municipalityId ? `KOV: ${filters.municipalityId}` : null,
...
<body>
  <main>
    ...
    <p class="muted"><strong>Analüüsiühik: ${escapeHtml(report.analysisUnitLabel || "-")}.</strong> ${escapeHtml(report.analysisUnitNotice || "")}</p>
    ${report.truncated ? `<section class="decision"><div class="muted">Andmete täielikkus</div><p>${escapeHtml(report.completenessNotice || "")}</p></section>` : ""}
    ${report.cellSuppressionNotice ? `<section class="decision"><div class="muted">Avaldamata lahtrid</div><p>${escapeHtml(report.cellSuppressionNotice)}</p></section>` : ""}
    ...
    <div class="grid">
      ...
      <section class="card"><div class="muted">Arvestatud ühikuid</div><div class="kpi">${escapeHtml(report.countedRecordCount ?? report.recordCount ?? "-")}</div></section>
      <section class="card"><div class="muted">Punased</div><div class="kpi">${escapeHtml(signalCell(report.signal?.redCount))}</div></section>
      <section class="card"><div class="muted">Kollased</div><div class="kpi">${escapeHtml(signalCell(report.signal?.yellowCount))}</div></section>
    </div>
    <h2>Töökorralduslikud prioriteedid</h2>
    <table>
      <thead><tr><th>Kategooria</th><th>Prioriteet</th><th>Esinemus</th><th>Osakaal</th></tr></thead>
      <tbody>${priorities.map((item) => `<tr><td>${escapeHtml(item.categoryLabel)}</td><td>${escapeHtml(item.label)}</td><td>${escapeHtml(item.count)}/${escapeHtml(item.denominator ?? item.sampleSize)}</td><td>${escapeHtml(percent(item.count, item.denominator ?? item.sampleSize))}</td></tr>`).join("")}</tbody>
    </table>
  ...
  const entries = [
    ...
    { name: "xl/_rels/workbook.xml.rels", data: workbookRelsXml() },
    {
      ...
      data: worksheet([
        ...
        ["Kirjeid kokku", report.recordCount],
        /* SOL-WB-04: ühik ja tema nimetaja peavad olema tabelis endas. Excelis
           lõigatakse arv oma kontekstist lahti kõige kiiremini — kui ühik jääb
           ainult JSON-i, loeb lugeja iga rida inimeste osakaaluna. */
        ["Arvestatud ühikuid", report.countedRecordCount ?? report.recordCount],
        ["Analüüsiühik", report.analysisUnitLabel || report.analysisUnit || ""],
        ["Ühiku tähendus", report.analysisUnitNotice || ""],
        ["Miinimumgrupp", report.minimumGroupSize],
        /* SOL-WB-06 saba: `|| 0` oleks siin sama vaikne null, mille pärast
           summutus üldse käib — Excelis on „0 punast" lõplik ja kontekstita
           väide. Avaldamata lahter ütleb ennast sõnadega välja. */
        ["Punased", signalCell(report.signal?.redCount)],
        ["Kollased", signalCell(report.signal?.yellowCount)],
        ["Rohelised", signalCell(report.signal?.greenCount)],
        ["Privaatsus", report.privacyNotice || ""],
        /* Kärbe ja lahtrisummutus käivad tabeliga kaasa: Excelis eraldub arv oma
           kontekstist kõige kiiremini ja just seal loetakse teda täielikuks. */
        ...(report.truncated ? [["Andmete täielikkus", report.completenessNotice || ""]] : []),
        ...(report.cellSuppressionNotice ? [["Avaldamata lahtrid", report.cellSuppressionNotice]] : [])
      ])
    },
    {
      name: "xl/worksheets/sheet2.xml",
      data: worksheet([
        /* Veerus on `denominator`, mitte valim — vana pealkiri „Valim" ütles
           sama vale, mille SOL-WB-04 osakaaludest välja võttis. */
        ["Kategooria", "Prioriteet", "Esinemus", "Nimetaja"],
        ...priorities.map((item) => [item.categoryLabel, item.label, item.count, item.denominator ?? item.sampleSize])
      ])
    },
    {
