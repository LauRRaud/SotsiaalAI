...
import { buildHardCaseRecord } from "./hardCase.js";
import { validateWellbeingStandardizedFields } from "./fieldSchemas.js";
import { withWellbeingAdvisoryLock } from "./outputDraftLock.js";
import { readWellbeingRecordsPaged } from "./pagedRecords.js";
import { deleteWellbeingOutputDraftsForRecord } from "./supportDrafts.js";
import {
  inheritWellbeingParticipation,
  participationCreateInput,
  resolveWellbeingParticipation
} from "./participation.js";
import { buildInterruptionsRecord } from "./interruptions.js";
import { buildQuickCheckRecord } from "./quickCheck.js";
import { buildRecoveryRecord } from "./recovery.js";
import { buildRoleBoundariesRecord } from "./roleBoundaries.js";
import { buildStarterSupportRecord } from "./starterSupport.js";
import { buildWorkplaceViolenceRecord } from "./workplaceViolence.js";
import { buildWorkProcessesRecord } from "./workProcesses.js";
import { buildWorkBoundariesRecord } from "./workBoundaries.js";
...
/* SOL-WB-03: üheksa validaatorit küsisid täpselt üht asja — kas võti on
   objektis. Väärtust ei vaadanud keegi, ja kuna skoorijad annavad tundmatule
   väärtusele `0`, hinnati TEADMATUS ohutuks (`dangerStatus: "ONGOING"` vale
   kirjapildiga andis `no_immediate_danger`). Kontroll elab nüüd ühes
   deklaratiivses skeemis (`fieldSchemas.js`) ja on fail-closed: tundmatu enum,
   vale tüüp, tundmatu võti või liiga pikk tekst annab 400 ja kirjet ei sünni.
   Veavõti ja `details.missing` jäid samaks — liidese tõlge ei murdu. */
const validateQuickCheckFields = (fields = {}) => validateWellbeingStandardizedFields("quick-check", fields);
const validateHardCaseFields = (fields = {}) => validateWellbeingStandardizedFields("hard-case", fields);
const validateInterruptionsFields = (fields = {}) => validateWellbeingStandardizedFields("interruptions", fields);
const validateWorkplaceViolenceFields = (fields = {}) => validateWellbeingStandardizedFields("workplace-violence", fields);
const validateRecoveryFields = (fields = {}) => validateWellbeingStandardizedFields("recovery", fields);
const validateRoleBoundariesFields = (fields = {}) => validateWellbeingStandardizedFields("role-boundaries", fields);
const validateStarterSupportFields = (fields = {}) => validateWellbeingStandardizedFields("starter-support", fields);
const validateWorkBoundariesFields = (fields = {}) => validateWellbeingStandardizedFields("work-boundaries", fields);
const validateWorkProcessesFields = (fields = {}) => validateWellbeingStandardizedFields("work-processes", fields);
...
/* SOL-WB-01/-02: osalusprojektsioon sünnib SIIN, ühes kohas kõigi üheksa
   töövoo eest. Ta ei tule payload'ist ja teda ei anna kutsuja: kutsujad
   erinevad ainult töövoo builderi poolest ja üheksa koopiat sellest otsusest
   lahkneksid. Tuletus käib luku EES — ta loeb liikmesust, mitte kirjeid, ja
   tema hoidmine luku sees pikendaks serialiseeritud lõiku ilma kasuta.
   Osalus luuakse kirjega SAMAS `create`-s: kirje ilma osaluseta oleks
   koondist vaikselt väljas ja seda ei parandaks ükski hilisem samm. */
async function createWellbeingRecordDeduped(prisma, data) {
  const participation = await resolveWellbeingParticipation(data.ownerUserId, { prisma });
  ...
  return withWellbeingAdvisoryLock(prisma, lockKey, async (tx) => {
    ...
    const record = await tx.wellbeingRecord.create({
      data: { ...data, participation: participationCreateInput(participation) }
    });
...
export async function listWellbeingRecordsForUser(userId, filters = {}, options = {}) {
  ...
  /* SOL-WB-15: loend lõppes vaikselt 100 rea juures — ei kursorit, ei
     `hasMore`-i, ei „laadi veel"-i. Pikaajalisel kasutajal kadusid vanemad
     kirjed UI-st ilma ühegi märgita, kuigi nad on andmebaasis alles.

     Järjestus on `(createdAt desc, id desc)`: `createdAt` üksi ei ole
     unikaalne ja sama sekundi read hüppaksid lehtede vahel edasi-tagasi.
     Kursor on `skip: 1` mustriga, mitte `offset` — offset triivib, kui
     vahepeal kirje lisandub või kustub. */
  const take = normalizeTake(filters.take);
  const cursor = String(filters.cursor || "").trim();
  const rows = await prisma.wellbeingRecord.findMany({
    where: {
      ownerUserId,
    ...
    /* Üks rida üle: nii saab „kas on veel" vastuse ilma teise päringuta. */
    take: take + 1,
    ...(cursor ? { cursor: { id: cursor }, skip: 1 } : {})
  });

  const hasMore = rows.length > take;
  const records = hasMore ? rows.slice(0, take) : rows;
  return { records, hasMore, nextCursor: hasMore ? records.at(-1)?.id || null : null };
}

/* SOL-WB-10: ülevaade vajab KOGU perioodi, mitte lehekülge. `listWellbeingRecordsForUser`
   jääb liidese loendiks (tema 1..100 piir on ekraani, mitte analüüsi piir) ja
   koondav lugemine käib siit — stabiilses järjestuses ja ausa `truncated`-iga. */
export async function readAllWellbeingRecordsForUser(userId, filters = {}, options = {}) {
  const ownerUserId = requireUserId(userId);
  const prisma = options.prisma || defaultPrisma;
  const workflowType = String(filters.workflowType || "").trim();
  const periodStart = parseDate(filters.periodStart);
  ...
  return readWellbeingRecordsPaged(prisma, {
    where: {
      ownerUserId,
      ...(VALID_WORKFLOW_TYPES.has(workflowType) ? { workflowType } : {}),
      ...(Object.keys(createdAt).length > 0 ? { createdAt } : {})
...
export async function deleteWellbeingRecordForUser(userId, recordId, options = {}) {
  ...
  const deleted = Number(result?.count) === 1;

  /* SOL-WB-16: `WellbeingOutputDraft.sourceRecordId` on FK-ta tekstiväli, seega
     kaskaad mustandeid ei puuduta ja nad jäid katkenud naasmispunktiga alles.
     Valik on nüüd TEADLIK: `deleteDrafts: true` viib nad kaasa, vaikimisi
     jäävad. Vaikne kustutus oleks andmekadu — mustand on eraldi kirjutatud
     tekst, mitte kirje tuletis, mille saab uuesti genereerida. */
  let draftsDeleted = 0;
  if (deleted && options.deleteDrafts) {
    ({ deleted: draftsDeleted } = await deleteWellbeingOutputDraftsForRecord(ownerUserId, id, { prisma }));
  }

  return { deleted, count: Number(result?.count) || 0, draftsDeleted };
}
...
export async function createWellbeingRecordCorrectionForUser(userId, recordId, payload = {}, options = {}) {
  ...
  return withWellbeingAdvisoryLock(prisma, `wellbeingRecord:correct:${id}`, async (tx) => {
    ...
    const original = await tx.wellbeingRecord.findFirst({
      where: { id, ownerUserId },
      /* Osalus tuleb kaasa, sest parandus pärib ta (vt allpool) — teist
         päringut selleks ei tehta ja uuesti tuletada teda ei tohi. */
      include: { supersededBy: { select: { id: true } }, participation: true }
    });
    if (!original) {
      const error = new Error("wellbeing.errors.record_missing");
      error.status = 404;
    ...
    /* SOL-WB-08: kontrollpunkt LIIGUB, mitte ei kopeeru. Vana rada jättis
       kokkuleppe ka asendatud kirje külge: taimer nägi mõlemat, sourceId erines
       ja kasutaja sai sama kokkuleppe kohta kaks badge'i ning sai vastata kahele
       iseseisvalt lahknevale kontrollpunktile. Omand on üheselt ahela kehtiva
       tipu peal — seal, kus kasutaja teda ka näeb. */
    ...
    const record = await tx.wellbeingRecord.create({
      data: {
        ...
        /* Kontrollpunkt tuleb kaasa: parandus kirjeldab sama hetke, seega ka
           ...
           peale ära ja badge vaikiks. Koos temaga liigub ka vastuse aeg — muidu
           ärkaks juba vastatud kokkulepe paranduse peale taimeris uuesti ellu. */
        checkpointDueOn: original.checkpointDueOn ?? null,
        checkpointAnsweredAt: original.checkpointAnsweredAt ?? null,
        checkpoint: original.checkpoint ?? null,
        /* SOL-WB-01/-02: osalus PÄRITAKSE, mitte tuletata uuesti. Parandus
           kirjeldab sama hetke; vahepealne töökohavahetus koliks muidu mullu
           tekkinud koormuse uue tööandja raportisse. */
        participation: participationCreateInput(inheritWellbeingParticipation(original.participation))
      }
    });
