...
function moderatedProfileChanged(existing, input) {
  return Object.entries(input).some(([field, value]) => {
    const current = existing[field] ?? (Array.isArray(value) ? [] : null);
...
export async function upsertOwnMentorProfile(actor, payload = {}, options = {}) {
  ...
  return withMentoringProfileLock(db, actor.userId, async (tx) => {
    ...
    const returnsToReview = moderatedProfileChanged(existing, input)
      && (existing.status === MENTOR_PROFILE_STATUS.ACTIVE
        || existing.status === MENTOR_PROFILE_STATUS.PAUSED);
    const updated = await tx.mentorProfile.updateMany({
      where: { id: existing.id, userId: actor.userId, version: expectedVersion },
      data: {
        ...input,
        ...(returnsToReview ? {
          status: MENTOR_PROFILE_STATUS.PENDING_REVIEW,
          approvedSnapshotVisible: existing.status === MENTOR_PROFILE_STATUS.ACTIVE,
...
export async function pauseOwnMentorProfile(actor, options = {}) {
  return transitionOwnProfile(actor, {
    ...
    data: { approvedSnapshotVisible: false }
  }, options);
}
...
export async function resumeOwnMentorProfile(actor, options = {}) {
  return transitionOwnProfile(actor, {
    ...
    data: { approvedSnapshotVisible: true }
  }, options);
}
