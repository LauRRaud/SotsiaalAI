...
export async function createMentoringRequest(actor, payload = {}, options = {}) {
  ...
  return withMentoringProfileLock(db, `request:${actor.userId}`, async (tx) => {
    ...
    return serializeRequestForMentee(request, publicProfile);
  });
}
...
export async function listMyMentoringRequests(actor, options = {}) {
    ...
    include: {
      mentorProfile: {
        select: {
          ...
          consentStatus: true,
          consentEvidenceType: true,
          consentEvidenceRef: true,
          checkedAt: true,
  ...
  return requests.map((request) => serializeRequestForMentee(
    request,
    publicMentorProfile(request.mentorProfile, now)
  ));
}
