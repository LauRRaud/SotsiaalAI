...
import { parseOffsetDateTime } from "./time.js";

function parseOccurredAt(value) {
  const date = parseOffsetDateTime(value);
  if (!date) throw invalid("INVALID_MEETING_TIME");
  return date;
}

async function resolveRoomReference(tx, relation, roomId) {
  const id = String(roomId || "").trim();
  if (!id) throw invalid("MEETING_ROOM_REQUIRED");
  const members = [relation.mentorUserId, relation.menteeUserId].filter(Boolean);
  if (members.length !== 2) throw conflict("RELATION_MEMBER_MISSING");
  const room = await tx.room.findFirst({ where: { id, archivedAt: null }, select: { id: true } });
  if (!room) throw notFound();
  const activeMemberCount = await tx.roomMember.count({
    where: { roomId: id, userId: { in: members }, leftAt: null }
  });
  if (activeMemberCount !== 2) throw notFound();
  return id;
}
...
export async function createMentoringMeeting(actor, relationId, payload = {}, options = {}) {
  ...
  return withMentoringRelationLock(db, relationId, async (tx) => {
    ...
    const roomId = mode === MENTORING_MEETING_MODE.PLATFORM_ROOM
      ? await resolveRoomReference(tx, relation, payload.roomId)
      : null;
      ...
      data = {
        ...
        roomId: mode === MENTORING_MEETING_MODE.PLATFORM_ROOM
          ? (payload.roomId !== undefined
            ? await resolveRoomReference(tx, relation, payload.roomId)
            : await resolveRoomReference(tx, relation, meeting.roomId))
          : null,
