 ...
 * - `fist`: sõrmed peopessa kõverdatud (ots randmele lähemal kui sõrme
 *   alus; nimetissõrm rangemalt) ja pöial sõrmede peal, mitte väljas;
...
export function handShape(landmarks, aspect = 4 / 3) {
  ...
  const curled = curls[0] < INDEX_CURLED && curls.slice(1).every((c) => c < FINGER_CURLED);
  return {
    fist: curled && thumb < THUMB_ON_FINGERS,
    ratio: thumb < THUMB_TUCKED ? Number.POSITIVE_INFINITY : pinch,
    metrics: { curls, thumb, pinch },
  };
}
