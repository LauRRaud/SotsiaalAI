...
/**
 * Salvestab jagamise fakti + sisu-snapshot'i (U10 voog).
 *
 * VISKAB (SOL-ROOM-06). Varem neelas ta `upsert` vea ja tagastas `{recorded:false}`, mida
 * marsruut ei vaadanud: kõik nägid ruumis kinnitatud kokkuvõtet, aga ruumi lõppedes ei
 * saanud sellest keegi privaatkoopiat, sest üleandmine loeb ainult `RoomSharedSummary`
 * ridu. Nüüd kutsutakse teda sõnumi loomisega SAMAS tehingus — kas mõlemad või mitte
 * kumbki, ja kasutaja näeb ausat viga, mitte kadunud kandjat.
 */
...
async function ensureCopyForUser(db, { summary, userId, now }) {
  const existing = await db.roomSummaryCopy.findFirst({
    ...
    select: { id: true, savedAnalysisId: true, completedAt: true }
  });
  // `savedAnalysisId` muutub kasutaja kustutuse järel SetNull tõttu tühjaks.
  // Püsiv lõpetamismärge eristab seda tõrke järel pooleli jäänud kandereast.
  if (existing?.completedAt || existing?.savedAnalysisId) return "existing";
  ...
  if (!ledgerId) {
    try {
      ...
      if (raced?.completedAt || raced?.savedAnalysisId) return "existing";
  ...
  await db.roomSummaryCopy.update({
    where: { id: ledgerId },
    data: { savedAnalysisId: analysis.id, copiedAt: now, completedAt: now }
  });
  return "created";
}
...
export async function copyRoomSummariesToParticipants({
  ...
  const activeNow = await db.roomMember.findMany({
    where: { roomId, leftAt: null },
    select: { userId: true }
  });

  /**
   * SAAJATE RING ON JAGAMISE HETK, MITTE RUUMI LÕPP (SOL-ROOM-07).
   *
   * Varem valiti ainult lõpetamise hetkel aktiivsed liikmed, seega inimene, kes lahkus
   * PÄRAST kokkuvõtte jagamist, kaotas koopia — ja hiljem lahkunu sai selle. Kättesaadavus
   * sõltus ruumi sulgemise juhuslikust ajast, mitte sellest, kellele kokkuvõte jagati.
   *
   * Ring on nüüd ÜHEND: praegused liikmed JA need, kes olid liikmed jagamise hetkel. Ühend,
   * mitte asendus — hiljem liitunu näeb kokkuvõtet ruumi ajaloos ja tema koopiat ei võeta
   * ära. Ajalugu on tuletatav `RoomMember.joinedAt`/`leftAt` väljadest, seega uut veergu ja
   * migratsiooni ei ole vaja.
   */
  async function recipientsFor(summary) {
    const sharedAt = summary.sharedAt instanceof Date ? summary.sharedAt : new Date(summary.sharedAt || 0);
    const atShareTime = await db.roomMember.findMany({
      where: {
        roomId,
        joinedAt: { lte: sharedAt },
        OR: [{ leftAt: null }, { leftAt: { gt: sharedAt } }]
      },
      select: { userId: true }
    });
    return [
      ...new Set(
        [...activeNow, ...atShareTime]
  ...
  for (const summary of summaries) {
    ...
    const recipientIds = await recipientsFor(summary);
