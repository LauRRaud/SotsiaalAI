...
export async function putFieldVisitAttachment(
  ...
  const recoveryAllowed = Boolean(
    ...
    capturedAt.getTime() <= new Date(visit.closedAt).getTime()
  );
  if (!OPEN_STATUSES.has(visit.status) && !recoveryAllowed) {
  ...
  try {
    ...
    const pending = await quota(
      ...
      if (recoveryAllowed) {
        await writeDataAudit({
          db: tx,
          actorUserId: ownerUserId,
          action: "field.attachment_recovery_imported",
          resourceType: "FIELD_VISIT",
          resourceId: visit.id
        });
      }
    ...
    return { created: true, recovered: recoveryAllowed, attachment: serializeAttachment(attachment) };
  } catch (error) {
