...
import { preInquiryRoomLockKey } from "@/lib/rooms/preInquiryRoom";
import {
...
const CLIENT_ACTION_ID_RE = /^[A-Za-z0-9_-]{12,100}$/u;
...
const VISIT_SELECT = Object.freeze({
  ...
  safetyResolvedNoticeStatus: true,
...
export function serializeFieldNote(row) {
  return {
    ...
    recoveryImportedAt: iso(row.recoveryImportedAt),
    createdAt: iso(row.createdAt),
    updatedAt: iso(row.updatedAt)
  };
}
...
function serializeAttachment(row) {
  ...
  return {
    ...
    documentGone: active && !row.documentId,
    ...
    documentRequestAt: iso(row.documentRequestAt),
...
function encodeFieldCursor(row) {
  if (!row?.id || !row?.createdAt) return null;
  return Buffer.from(JSON.stringify([iso(row.createdAt), row.id]), "utf8").toString("base64url");
}

function decodeFieldCursor(value) {
  if (!value) return null;
  try {
    const [createdAt, id] = JSON.parse(Buffer.from(String(value), "base64url").toString("utf8"));
    const date = new Date(createdAt);
    if (!id || !Number.isFinite(date.getTime())) throw new Error("invalid");
    return { createdAt: date, id: String(id) };
  } catch {
    throw fieldError("field.errors.invalid_cursor", 400);
  }
}

export async function listFieldVisits(userId, { db = prisma, cursor = null, limit = 50 } = {}) {
  const ownerUserId = requireUserId(userId);
  const pageSize = Math.min(100, Math.max(1, Number(limit) || 50));
  const after = decodeFieldCursor(cursor);
  const rows = await db.fieldVisit.findMany({
    ...
    // Immutable order keeps cursor traversal complete even when another device edits updatedAt.
    orderBy: [{ createdAt: "desc" }, { id: "asc" }],
    take: pageSize + 1
  });
  const page = rows.slice(0, pageSize);
  const visits = page.map((row) => ({
    ...serializeFieldVisit(row),
    noteCount: row._count?.notes ?? 0,
    attachmentCount: row._count?.attachments ?? 0
  }));
  const [openCount, closedCount] = await Promise.all([
    db.fieldVisit.count({ where: { ownerUserId, status: { in: OPEN_STATUSES } } }),
    db.fieldVisit.count({ where: { ownerUserId, status: { in: [FIELD_VISIT_STATUS.CLOSED, FIELD_VISIT_STATUS.CANCELLED] } } })
  ]);
  return {
    visits,
    nextCursor: rows.length > pageSize ? encodeFieldCursor(page.at(-1)) : null,
...
export async function getFieldVisitDetail(userId, visitId, { db = prisma } = {}) {
  ...
  const [notes, attachments, inquiry] = await Promise.all([
    db.fieldVisitNote.findMany({
      ...
      // Owner detail is complete: never silently truncate notes that can be handed over.
    }),
    db.fieldVisitAttachment.findMany({
      ...
      orderBy: [{ createdAt: "asc" }, { id: "asc" }],
      // Owner detail is complete: never silently hide attachment carriers.
    }),
    visit.preInquiryId
...
export async function performFieldVisitAction(
  ...
  if (kind === "take_pack") {
    ...
    data.safetyResolvedNoticeStatus = null;
...
export async function putFieldVisitNote(
  ...
  const noteForRecovery = !input.resolve && !input.withdrawConsent ? normalizeNoteInput(input) : null;
  const recoveryImport = Boolean(input.recoveryImport);
  const recoveryAllowed = Boolean(
    recoveryImport &&
    visit.status === FIELD_VISIT_STATUS.CLOSED &&
    visit.closedAt &&
    noteForRecovery?.deviceCreatedAt &&
    noteForRecovery.deviceCreatedAt.getTime() <= new Date(visit.closedAt).getTime()
  );
  if (!OPEN_STATUSES.includes(visit.status) && !recoveryAllowed) assertVisitOpen(visit);
  ...
  const note = noteForRecovery || normalizeNoteInput(input);
  const sha = contentFingerprint(note);
  if (recoveryAllowed && existing) {
    if (existing.contentSha256 === sha && existing.revision === note.revision) {
      return {
        created: false,
        existing: true,
        recovered: Boolean(existing.recoveryImportedAt),
        note: serializeFieldNote(existing)
      };
    }
    throw fieldError("field.errors.recovery_conflict", 409);
  }
  ...
  if (!existing) {
    try {
      const created = await db.$transaction(async (tx) => {
        const row = await tx.fieldVisitNote.create({
          data: {
            ...
            recoveryImportedAt: recoveryAllowed ? now : null,
            createdAt: now,
            updatedAt: now
          },
          select: NOTE_SELECT
        ...
        if (recoveryAllowed) {
          await writeDataAudit({
            db: tx,
            actorUserId: ownerUserId,
            action: "field.note_recovery_imported",
            resourceType: "FIELD_VISIT",
            resourceId: visit.id
          });
        }
      ...
      return { created: true, recovered: recoveryAllowed, note: serializeFieldNote(created) };
    } catch (error) {
...
export async function deleteFieldVisitNote(
  ...
  if (!existing) throw fieldError("api.common.not_found", 404);
  if (existing.kind === FIELD_NOTE_KIND.CONSENT) {
    throw fieldError("field.errors.consent_must_be_withdrawn", 409);
  }
  await db.$transaction(async (tx) => {
    const result = await tx.fieldVisitNote.deleteMany({
      where: { id: existing.id, visitId: visit.id }
    });
    if (!result.count) throw fieldError("api.common.not_found", 404);
    await writeDataAudit({
      db: tx,
      actorUserId: ownerUserId,
      action: "field.note_deleted",
      resourceType: "FIELD_VISIT",
      resourceId: visit.id
    });
  });
...
function normalizeHandoverRequest(input) {
  const clientActionId = String(input?.clientActionId || "").trim();
  if (!CLIENT_ACTION_ID_RE.test(clientActionId)) {
    throw fieldError("field.errors.invalid_client_action_id", 400);
  }
  const toArtifact = Boolean(input?.toArtifact);
  const toPreInquiry = Boolean(input?.toPreInquiry);
  if (!toArtifact && !toPreInquiry) throw fieldError("field.errors.no_handover_target", 400);
  const noteClientItemIds = Array.isArray(input?.noteClientItemIds)
    ? [...new Set(input.noteClientItemIds.map((id) => String(id || "").trim()).filter(Boolean))].sort()
    : [];
  return {
    clientActionId,
    toArtifact,
    toPreInquiry,
    noteClientItemIds,
    artifactTitle: toArtifact ? normalizeText(input.artifactTitle, MAX_TEXT.title) : null,
    preInquiryNote: toPreInquiry
      ? normalizeText(input.preInquiryNote, MAX_TEXT.handoverNote, { required: true, field: "handover_note" })
      : null,
    nextContactOn: toPreInquiry && input.nextContactOn != null ? String(input.nextContactOn) : null
  };
}

function handoverRequestSha256(payload) {
  return crypto.createHash("sha256").update(JSON.stringify(payload)).digest("hex");
}

async function lockHandover(tx, handoverId) {
  if (typeof tx.$executeRaw !== "function") return;
  await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${`fieldHandover:${handoverId}`})::bigint)`;
}

function targetFailure(error) {
  const status = Number(error?.status);
  const code = String(error?.message || error?.code || "handover_target_failed");
  return { status: "FAILED", errorCode: code.slice(0, 160), httpStatus: Number.isFinite(status) ? status : 500 };
}

async function persistHandoverFailure(db, actionId, target, error, now) {
  try {
    await db.$transaction(async (tx) => {
      await lockHandover(tx, actionId);
      const fresh = await tx.fieldVisitHandover.findUnique({ where: { id: actionId } });
      if (!fresh || fresh.targetStates?.[target]?.status === "DONE") return;
      await tx.fieldVisitHandover.update({
        where: { id: actionId },
        data: {
          targetStates: { ...fresh.targetStates, [target]: targetFailure(error) },
          updatedAt: now
        }
      });
    });
...
/** Durable, per-target handover saga. Every target owns one atomic transaction. */
export async function handoverFieldVisit(
  ...
  const payload = normalizeHandoverRequest(input);
  const requestSha256 = handoverRequestSha256(payload);
  const initialStates = {
    artifact: payload.toArtifact ? { status: "PENDING" } : { status: "SKIPPED" },
    preInquiry: payload.toPreInquiry ? { status: "PENDING" } : { status: "SKIPPED" }
  };
  let action;
  try {
    action = await db.fieldVisitHandover.create({
      data: {
        visitId: visit.id,
        ownerUserId,
    ...
    if (error?.code !== "P2002") throw error;
    action = await db.fieldVisitHandover.findFirst({
      where: { visitId: visit.id, clientActionId: payload.clientActionId, ownerUserId }
    });
  }
  if (!action) throw fieldError("field.errors.handover_failed", 500);
  if (action.requestSha256 !== requestSha256) {
    throw fieldError("field.errors.handover_idempotency_conflict", 409);
  }

  if (payload.toArtifact && action.targetStates?.artifact?.status !== "DONE") {
    try {
      await db.$transaction(async (tx) => {
        await lockHandover(tx, action.id);
        const current = await tx.fieldVisitHandover.findUnique({ where: { id: action.id } });
        if (current?.targetStates?.artifact?.status === "DONE") return;
        const notes = await tx.fieldVisitNote.findMany({
          where: {
            visitId: visit.id,
            ...(payload.noteClientItemIds.length
              ? { clientItemId: { in: payload.noteClientItemIds } }
              : {})
          },
          select: NOTE_SELECT,
          orderBy: [{ createdAt: "asc" }, { id: "asc" }]
        ...
        const created = await tx.agentArtifact.create({
          data: {
            ...
            type: "CASE_SUMMARY",
            status: "DRAFT",
            title: payload.artifactTitle || "Külastuse kokkuvõte",
            content: buildHandoverArtifactContent(visit, notes, new Date(current.createdAt)),
            metadata: { source: "FIELD_VISIT", fieldVisitId: visit.id, handoverId: current.id },
            idempotencyKey: `field-handover:${current.id}:artifact`
          },
        ...
        await tx.fieldVisitHandover.update({
          ...
          data: {
            targetStates: {
              ...current.targetStates,
              artifact: { status: "DONE", id: created.id, result: created }
            },
            updatedAt: now
          }
        });
      ...
      await persistHandoverFailure(db, action.id, "artifact", error, now);
    }
  }

  action = await db.fieldVisitHandover.findUnique({ where: { id: action.id } });
  if (payload.toPreInquiry && action.targetStates?.preInquiry?.status !== "DONE") {
    try {
      await db.$transaction(async (tx) => {
        await lockHandover(tx, action.id);
        const current = await tx.fieldVisitHandover.findUnique({ where: { id: action.id } });
        if (current?.targetStates?.preInquiry?.status === "DONE") return;
        if (!visit.preInquiryId) throw fieldError("field.errors.no_pre_inquiry", 409);
        if (typeof tx.$executeRaw === "function") {
          await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${preInquiryRoomLockKey(visit.preInquiryId)}))`;
        }
        const inquiry = await tx.preInquiry.findFirst({
        ...
        const appended = [
          ...
          `--- Välitöö üleandmine ${new Date(current.createdAt).toISOString().slice(0, 10)} ---`,
          payload.preInquiryNote
        ].filter(Boolean).join("\n\n");
        const updated = await workflow(
          ...
          inquiry.id,
          {
            ...
            ...(payload.nextContactOn == null ? {} : { nextContactOn: payload.nextContactOn }),
            expectedUpdatedAt: inquiry.updatedAt
          },
          { db: tx, roomLockAlreadyHeld: true }
        );
        await tx.fieldVisit.updateMany({
          where: { id: visit.id, ownerUserId },
        ...
        await tx.fieldVisitHandover.update({
          ...
          data: {
            targetStates: {
              ...current.targetStates,
              preInquiry: { status: "DONE", id: inquiry.id, updated: Boolean(updated) }
            },
            updatedAt: now
          }
        });
      ...
      await persistHandoverFailure(db, action.id, "preInquiry", error, now);
    }
  }
  ...
  const fresh = await findOwnedVisit(db, ownerUserId, visit.id);
  return {
    handover: { id: action.id, clientActionId: action.clientActionId, targets: action.targetStates },
    artifact: action.targetStates?.artifact?.result || null,
    preInquiry: action.targetStates?.preInquiry?.status === "DONE" ? action.targetStates.preInquiry : null,
    visit: serializeFieldVisit(fresh)
  };
}
...
export const fieldServiceInternals = Object.freeze({
  ...
  normalizeHandoverRequest,
  handoverRequestSha256,
