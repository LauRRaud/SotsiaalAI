...
export async function performFieldVisitAction(
  ...
  if (kind === "arm_safety" || kind === "cancel_safety" || kind === "close" || kind === "cancel_visit") {
    await logDataAudit({
...
export async function putFieldVisitNote(
  ...
  if (!existing) {
    try {
      const created = await db.fieldVisitNote.create({
        data: {
          visitId: visit.id,
          clientItemId: itemId,
  ...
  if (note.revision === existing.revision + 1 && !existing.conflictState) {
    const result = await db.fieldVisitNote.updateMany({
