...
export async function putFieldVisitNote(
  ...
  const noteInput = !input.resolve && !input.withdrawConsent ? normalizeNoteInput(input) : null;
  if (!OPEN_STATUSES.includes(visit.status)) assertVisitOpen(visit);
  ...
  const note = noteInput || normalizeNoteInput(input);
  ...
  if (!existing) {
    try {
      ...
      return { created: true, recovered: false, note: serializeFieldNote(created) };
    } catch (error) {
