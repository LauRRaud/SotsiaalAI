...
export async function sweepFieldRetention({
  ...
  for (const link of links) {
    ...
    if (deleted.count) {
      /* SOL-FIELD-03: sama süstitud klient, aga TEADLIKULT best-effort. Fail on
         kettalt ja RAG-ist juba läinud — tagasipööramine jätaks alles rea, mis
         viitab olematule failile, ja see oleks halvem kui puuduv logirida. */
      await logDataAudit({
        db,
        targetUserId: document.ownerId,
