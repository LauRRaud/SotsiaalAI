...
import { PROVENANCE, PROVENANCES, isProvenance } from "../workspaces/provenance.js";
...
/**
 * Kas külastus on LÕPETATUD? Küsi seisumasinalt, ära loetle olekuid.
 *
 * SOL-ORG-08 õppetund kordub siin vastupidises suunas: „lõpetatud" on tuletatud
 * omadus (masin ei anna ühtegi väljapääsu), mitte teine nimekiri, mis unustatakse
 * uuendada, kui olekuid lisandub. Tundmatu olek EI ole lõpetatud — säilituse
 * poolel tähendab see „hoia alles", ja alles jäänud paketil on niikuinii
 * ajaline ülempiir (4.5).
 */
export function isFieldVisitClosed(status) {
  const outgoing = FIELD_VISIT_TRANSITIONS[String(status || "")];
...
/**
 * K2 provenance dictionary. The eight canonical values now live in
 * lib/workspaces/provenance.js (CASEWORK-P0 consolidation) — FIELD re-exports
 * them so there is ONE dictionary, not two. The string values are byte-identical
 * to what FIELD already stores in production; `isFieldProvenance` stays the
 * FIELD-facing name for existing call sites.
 */
export const FIELD_PROVENANCE = PROVENANCE;
export const FIELD_PROVENANCES = PROVENANCES;
