...
export function fieldJson(payload, status = 200, extraHeaders = {}) {
  return NextResponse.json(payload, { status, headers: { ...FIELD_NO_STORE_HEADERS, ...extraHeaders } });
}

export async function requireFieldUser(opts = {}) {
  const getSession = opts.getSession || (() => getServerSession(authConfig));
  const subscriptionGate = opts.requireSubscription || requireSubscription;
  ...
  const gate = await subscriptionGate(session, roleFromSession(session));
...
export function fieldErrorResponse(error, fallbackKey = "field.errors.request_failed") {
  ...
  return fieldJson(payload, status, error?.retryAfter ? { "Retry-After": String(error.retryAfter) } : {});
}
