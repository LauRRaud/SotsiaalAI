...
export async function runFieldSafetySweep({
  ...
  for (const visit of dueEscalations) {
    ...
    try {
      ...
      /* SOL-FIELD-03: sama süstitud klient. Tagasipööramist siin EI OLE ja see
         on teadlik — kiri on juba välja läinud, seega rollback tähendaks järgmisel
         käigul TEIST kirja samale inimesele. Puuduv logirida on väiksem kahju. */
      await logDataAudit({
        db,
