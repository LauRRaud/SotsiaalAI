...
async function markPersistedMeetingSummaryJobInterrupted(job, { usage = usageService } = {}) {
  if (!job?.id) return null;
 ...
 */
export async function sweepMeetingSummarySnapshots({ now = Date.now(), usage = usageService } = {}) {
  ...
  for (const entry of entries) {
    ...
    if (shouldInterruptStaleActiveJob(record, now)) {
      try {
        record = await markPersistedMeetingSummaryJobInterrupted(record, { usage });
