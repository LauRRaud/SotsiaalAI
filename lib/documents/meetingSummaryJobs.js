...
async function settleMeetingSummaryUsage(job, key, action, reason = null) {
  ...
  try {
    if (action === "commit") {
      await usageService.commit({ userId: job.userId, idempotencyKey });
      entry.state = "committed";
    } else {
      await usageService.release({
        userId: job.userId,
        idempotencyKey,
...
async function releaseIncompleteMeetingSummaryUsage(job, reason) {
  await Promise.all([
    settleMeetingSummaryUsage(job, "stt", "release", reason),
    settleMeetingSummaryUsage(job, "document", "release", reason)
  ]);
}
...
export async function createMeetingSummaryJob({ userId, payload, usage = null }) {
...
export async function runMeetingSummaryJob(job) {
  ...
  if (!process.env.OPENAI_API_KEY) {
    await releaseIncompleteMeetingSummaryUsage(job, "meeting_summary_not_configured");
  ...
  try {
    ...
    await settleMeetingSummaryUsage(job, "stt", "commit");
    await persistMeetingSummaryJob(job);
    ...
    if (job.usage?.document) job.usage.document.workCompleted = true;
    await settleMeetingSummaryUsage(job, "document", "commit");
    await persistMeetingSummaryJob(job);
    ...
    await releaseIncompleteMeetingSummaryUsage(job, "meeting_summary_failed");
