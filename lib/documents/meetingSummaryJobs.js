 ...
 * teine POST võiks ta ELUSALT üle võtta — täpselt see kahju, mille vastu see leid on. Südamelöök
 * käib jooksu loomulikes punktides; eraldi taimerit ei ole, sest sellel tööl ei ole vahepealseid
 * ootepunkte peale nende kahe.
 */
...
async function markMeetingSummaryFailed(job, errorMessage, { db = prisma } = {}) {
  const endedAt = nowIso();
  job.status = "error";
  job.updatedAt = endedAt;
  job.endedAt = endedAt;
...
export async function runMeetingSummaryJob(
  ...
  try {
    if (!process.env.OPENAI_API_KEY) {
      ...
      await markMeetingSummaryFailed(job, translated("api.stt.not_configured"), { db });
      return;
    }
