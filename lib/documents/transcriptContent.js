 ...
 *
 * `db` ja `stage` on süstitavad, et andmebaasi- ja kettasammud jääksid selgelt eraldatuks.
 */
...
export async function updateDocumentWithStagedText(
  ...
  try {
    const row = await db.$transaction(async (tx) => {
      ...
      let updated
      if (readWhere) {
        const result = await tx.userDocument.updateMany({ where, data: writeData })
        if (result.count !== 1) {
          const current = await tx.userDocument.findFirst({ where: readWhere, select })
          if (onConflict) onConflict(current)
          throw new Error("documents.errors.version_conflict")
        }
        updated = await tx.userDocument.findFirst({ where: readWhere, select })
        if (!updated) throw new Error("documents.errors.not_found")
      } else {
        updated = await tx.userDocument.update({ where, data: writeData, select })
      }
