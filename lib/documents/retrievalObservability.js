...
export function createRetrievalDebugMeta({
  ...
  return {
    ...
    evidence_chunks: (Array.isArray(evidenceChunks) ? evidenceChunks : []).map((chunk) => ({
      sourceDocumentId: chunk?.originalDocumentId || null,
      ragDocumentId: chunk?.docId || null,
      chunkId: chunk?.chunkId || null,
      chunkIndex: Number.isFinite(Number(chunk?.chunkIndex)) ? Number(chunk.chunkIndex) : null,
      textSha256: crypto.createHash("sha256").update(String(chunk?.text || "")).digest("hex")
    }))
  }
}
