...
function provenanceRetentionLabel(retention, t) {
  ...
  /* KOV-ile esitatud aruanne kannab oma tähtaega kaasas (RPS §12). Ilma nende
     kaheta oleks tema kehtivuse sildiks „kuni kustutamiseni" — mis on vale
     kahtpidi: teda EI TOHI enne tähtaega kustutada ega pärast igavesti hoida. */
  if (retention.key === "retention_until") {
    return t(
      "documents.provenance.retention.until",
      { date: String(retention.until).slice(0, 10) },
      `kuni ${String(retention.until).slice(0, 10)}`
    );
  }
  if (retention.key === "retention_years") {
    return t("documents.provenance.retention.years", { years: retention.years }, `${retention.years} a`);
  }
