...
export function readBoundedDocxZipEntries(buffer) {
  if (!Buffer.isBuffer(buffer) || buffer.length < 22) {
    throw new Error("Invalid ZIP: package is truncated")
  }
  const eocdOffset = findEndOfCentralDirectory(buffer)
  if (buffer.readUInt16LE(eocdOffset + 4) !== 0 || buffer.readUInt16LE(eocdOffset + 6) !== 0) {
    throw new Error("Invalid ZIP: multi-disk packages are unsupported")
  }
  const totalEntries = buffer.readUInt16LE(eocdOffset + 10)
  if (!totalEntries || totalEntries > MAX_DOCX_TEMPLATE_ENTRIES) {
  ...
  for (let index = 0; index < totalEntries; index += 1) {
    if (cursor + 46 > eocdOffset) throw new Error("Invalid ZIP: central directory is truncated")
    ...
    const centralEntryEnd = cursor + 46 + fileNameLength + extraLength + commentLength
    if (centralEntryEnd > eocdOffset) throw new Error("Invalid ZIP: entry metadata is out of bounds")
    const name = buffer.subarray(cursor + 46, cursor + 46 + fileNameLength).toString("utf8")
    const pathParts = name.split("/")
    if (
      !name || name.includes("\0") || name.includes("\\") || name.startsWith("/") ||
      pathParts.includes("..") || names.has(name)
    ) {
      throw new Error("Invalid ZIP: unsafe or duplicate entry name")
    }
    names.add(name)
    if (flags & 0x1) throw new Error("Invalid ZIP: encrypted entries are unsupported")

    if (localHeaderOffset + 30 > centralDirectoryOffset) {
      throw new Error("Invalid ZIP: local file header is out of bounds")
    }
    ...
    if (dataStart > centralDirectoryOffset || dataEnd > centralDirectoryOffset) {
      throw new Error("Invalid ZIP: compressed data is out of bounds")
    }
    const compressed = buffer.subarray(dataStart, dataStart + compressedSize)
    if (uncompressedSize > 1024 * 1024 && uncompressedSize > Math.max(1, compressedSize) * 200) {
      throw new Error("Template ZIP compression ratio is unsafe")
    }
    ...
    if (compressionMethod === 0) {
      ...
      data = zlib.inflateRawSync(compressed, { maxOutputLength: Math.max(1, uncompressedSize + 1) })
    } else {
      throw new Error(`Unsupported ZIP compression method: ${compressionMethod}`)
    }

    if (data.length !== uncompressedSize || crc32(data) !== expectedCrc) {
      throw new Error("Invalid ZIP: entry size or checksum mismatch")
    }
...
function buildTemplateBasedDocx(templateBuffer, context) {
  const entries = readBoundedDocxZipEntries(templateBuffer)
  let replaced = false
...
/* ---------------------------------------------------------------------------
   ÜLDINE DOCX (TEENUSPÄEVIK E6 ja edaspidi)

   Miks siia, mitte oma failina: zip-kirjutaja, CRC32 ja OOXML-i karkass on
   juba siin. Teine koopia tähendaks kahte kohta, kus zip võib katki minna — ja
   katkist zip-i ei ole näha enne, kui keegi proovib faili avada.

   MIKS DOCX ÜLDSE, kui leping ütleb „CSV + PDF": platvormi PDF-kirjutaja
   kodeerib WinAnsi-sse. Eesti täpitähed mahuvad sinna ära, KIRILLITSA MITTE —
   ja kliendi nimi „Мария" muutuks arve lisas küsimärkideks. DOCX on UTF-8 ja
   sellel piirangul ei ole tema üle võimu. PDF jääb alles, aga tema kõrval peab
   olema väljund, mis iga päris nime ära kannab.
   --------------------------------------------------------------------------- */

function buildTableRowXml(cells, { bold = false } = {}) {
  const runProps = bold ? "<w:rPr><w:b/></w:rPr>" : ""
  const tc = cells
    .map(
      (cell) =>
        `<w:tc><w:tcPr><w:tcW w:w="0" w:type="auto"/></w:tcPr><w:p><w:r>${runProps}<w:t xml:space="preserve">${escapeXml(
          String(cell ?? "")
        )}</w:t></w:r></w:p></w:tc>`
    )
    .join("")
...
function buildTableXml({ head = [], rows = [] }) {
  const headXml = head.length ? buildTableRowXml(head, { bold: true }) : ""
  const bodyXml = rows.map((row) => buildTableRowXml(row)).join("")
  return [
...
function buildBlocksXml(blocks = []) {
  return blocks
    .map((block) => {
      if (!block) return ""
      if (block.kind === "heading") return toWordParagraphs(block.text, { styleId: "Heading2" })
      if (block.kind === "title") return toWordParagraphs(block.text, { styleId: "Title" })
      if (block.kind === "table") return buildTableXml(block)
      return toWordParagraphs(block.text)
    })
    .join("")
}

function buildBlocksDocumentXml(blocks) {
  return `${DOCX_XML_HEADER}
<w:document xmlns:wpc="http://schemas.microsoft.com/office/word/2010/wordprocessingCanvas" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math" xmlns:v="urn:schemas-microsoft-com:vml" xmlns:wp14="http://schemas.microsoft.com/office/word/2010/wordprocessingDrawing" xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" xmlns:w10="urn:schemas-microsoft-com:office:word" xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:w14="http://schemas.microsoft.com/office/word/2010/wordml" xmlns:w15="http://schemas.microsoft.com/office/word/2012/wordml" mc:Ignorable="w14 w15">
  <w:body>
    ${buildBlocksXml(blocks)}
    <w:sectPr>
...
/**
 * @param blocks `{kind:"title"|"heading"|"paragraph", text}` või
 *   `{kind:"table", head:[...], rows:[[...]]}`
 */
export function createSimpleDocxBuffer({ title = "", blocks = [] } = {}) {
  const entries = [
    { name: "[Content_Types].xml", data: buildContentTypesXml() },
    { name: "_rels/.rels", data: buildRootRelationshipsXml() },
    { name: "docProps/app.xml", data: buildAppXml() },
    {
      ...
      data: buildCoreXml({ title, approvedAtLabel: new Date().toISOString().slice(0, 10) })
    },
    { name: "word/document.xml", data: buildBlocksDocumentXml(blocks) },
    { name: "word/_rels/document.xml.rels", data: buildDocumentRelationshipsXml() },
    { name: "word/styles.xml", data: buildStylesXml() }
  ]
