...
const AUDIT_EVENT_TO_ACTION = {
  ...
  // T20 COLLAB-P1: U10 jagamine ruumi (RUUM-A0 8 K2 auditiauk suletud).
  "artifact.shared": "ARTIFACT_SHARE",
  /* TEENUSPÄEVIK: KOV-ile esitatud kuuaruande arhiveerimine ja kordusväljastus.
     Kaks eri sündmust, sest nad vastavad eri küsimustele: „millal see aruanne
     tekkis" ja „mitu korda ta teele läks". */
  "document.service_log_archived": "UPLOAD",
  "document.service_log_reissued": "DOWNLOAD",
  "chat.exported": "DOWNLOAD",
  /* SOL-DOC-09: privaatse AI-analüüsi salvestus ja kustutus. Need kutsed OLID koodis olemas, aga
     kaardis mitte — tundmatu sündmus andis `null` ja logifunktsioon lõpetas kirjutamata. Analüüs
     paistis auditeerituna, kuid ühtki rida ei jäänud, ja hiljem ei saanud kasutaja kustutust
     eristada retentionist ega puuduvast objektist. */
  "analysis.saved": "ANALYSIS_SAVE",
  "analysis.deleted": "ANALYSIS_DELETE"
}

/** Kas see sündmus ON auditikaardis? Kohustuslik jälg peab seda küsida SAAMA. */
export function isMappedAuditEvent(event) {
  return Boolean(AUDIT_EVENT_TO_ACTION[String(event || "").trim()])
}
...
function mapAuditEventToAction(event) {
