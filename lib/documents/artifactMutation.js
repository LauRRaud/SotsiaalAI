...
async function assertNoActiveRefinement(db, artifactId, ownerId, now = new Date()) {
  // Fake-DB unit tests cover the conditional update itself; the real PostgreSQL
  // race is covered by the compose probe. A transaction client supplies both APIs.
  if (typeof db?.$executeRaw !== "function" || !db?.agentArtifactRefinement?.findFirst) return
  await db.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${`artifactRefinement:${artifactId}`}))`
  const active = await db.agentArtifactRefinement.findFirst({
    where: {
      artifactId,
      ownerId,
      status: "RUNNING",
  ...
  if (active) throw conflict("documents.artifacts.errors.refinement_in_progress")
}
...
export async function updateDraftArtifact(
  ...
  await assertNoActiveRefinement(db, artifactId, ownerId)
  const data = {}
...
export async function approveArtifact(
  ...
  await assertNoActiveRefinement(db, artifactId, ownerId, now)
