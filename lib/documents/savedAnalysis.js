...
export async function createSavedAnalysis({ userId, role, title, content, sourceDocumentIds }) {
  ...
  // SOL-DOC-09: auditijälg on KOHUSTUSLIK ja käib samas tehingus objektiga, mida ta kirjeldab.
  // Varem kutsuti siin best-effort logijat sündmusega, mida auditikaardis polnud — kutse nägi
  // välja nagu audit, aga ühtki rida ei tekkinud kunagi.
  const analysis = await withStorageQuota(
    { userId, role, addBytes: getUtf8ByteLength(normalizedContent) },
    {},
    async (tx) => {
      const created = await tx.savedAnalysis.create({
        data: {
          ownerId: userId,
      ...
      await writeDocumentAudit(
        "analysis.saved",
        { userId, analysisId: created.id, title: created.title, sourceCount: requestedIds.length },
        { db: tx }
      )
      return created
    }
...
export async function listSavedAnalysesForOwner({ userId, limit, offset, search = "" }) {
  const query = String(search || "").trim().slice(0, MAX_TITLE_LENGTH)
  const where = {
    ownerId: userId,
    ...(query
      ? {
...
export async function deleteSavedAnalysisForOwner({ userId, id }) {
  // Kustutus ja tema jälg on üks toiming: kui jälge ei õnnestu kirjutada, ei tohi ka kustutus
  // kehtida — muidu ei saaks hiljem eristada kasutaja kustutust retentionist ega puuduvast reast.
  return prisma.$transaction(async (tx) => {
    const result = await tx.savedAnalysis.deleteMany({ where: { id, ownerId: userId } })
    if (result.count === 0) return false
    await writeDocumentAudit("analysis.deleted", { userId, analysisId: id }, { db: tx })
    return true
  })
}
