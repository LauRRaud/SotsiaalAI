...
const ARTIFACT_REFINE_PROVIDER_TIMEOUT_MS = Math.max(
  5_000,
  Math.min(Number(process.env.ARTIFACT_REFINE_PROVIDER_TIMEOUT_MS) || 90_000, 180_000)
)
const ARTIFACT_GENERATION_PROMPT_VERSION = "artifact-generate-v1"
const ARTIFACT_REFINEMENT_PROMPT_VERSION = "artifact-refine-v1"
...
export async function generateArtifactDraftContent({
  ...
  return {
    ...
    debugMeta: {
      ...material.debugMeta,
      model: DEFAULT_MODEL,
      prompt_version: ARTIFACT_GENERATION_PROMPT_VERSION,
      evidence_chunks: material.debugMeta?.evidence_chunks || []
    }
  }
}
...
export async function refineArtifactDraftContent({
  ...
  artifactId = null,
  researchJobId = null,
  requestSignal = AbortSignal.timeout(ARTIFACT_REFINE_PROVIDER_TIMEOUT_MS)
}) {
  ...
  return {
    ...
    debugMeta: {
      ...material.debugMeta,
      model: DEFAULT_MODEL,
      prompt_version: ARTIFACT_REFINEMENT_PROMPT_VERSION,
      evidence_chunks: material.debugMeta?.evidence_chunks || []
    }
  }
}
