...
import { usageErrorDescriptor } from "@/lib/usage/routeAdapter"
...
export function errorJson(messageKey, status = 400, locale = "en", extras = {}, extraHeaders = {}) {
...
export function usageErrorJson(error, scope, locale = "en") {
...
/* opts.allowWithoutSubscription: KÕVA REEGEL (SotsiaalAI.md, omanik 28.07) —
   oma failide/tulemuste lugemine, allalaadimine ja kustutamine (GET/DELETE
   marsruudid annavad { allowWithoutSubscription: true }) ei sõltu tellimusest;
   üleslaadimine, muutmine ja kõik AI-kulu (POST/PATCH) jäävad värava taha. */
export async function requireDocumentUser(opts = {}) {
  try {
...
export async function promoteStoredDocument(stagingStoragePath, finalStoragePath) {
  const stagingPath = resolveAbsoluteDocumentPath(stagingStoragePath)
  const finalPath = resolveAbsoluteDocumentPath(finalStoragePath)
  await fs.rename(/*turbopackIgnore: true*/ stagingPath, finalPath)
}
...
/**
 * Mime -> laiend. AINULT nende jaoks, mida me ise toodame; tundmatu mime jätab
 * nime puutumata.
 */
...
/**
 * LAIENDITA FAIL EI AVANE. `buildDownloadHeaders` võtab nime `document.title ||
 * originalName` järgi ja üleslaaditud failide puhul on pealkiri tavaliselt
 * failinimi koos laiendiga — seega see auk oli aastaid nähtamatu. Teenuspäeviku
 * arhiveeritud aruande pealkiri on aga INIMESELE loetav („Teenuspäevik 2026-08
 * · Harku vald · mall A_TIMESHEET") ja tema laiendit ei ole. Windows ei tea,
 * millega selline fail avada — omanik ütles otse, et ta ei saanud aruannet
 * avada.
 *
 * Lisame laiendi AINULT siis, kui nimel ei ole ühtegi. Juba laiendiga nimi
 * jääb puutumata, seega ükski olemasolev allalaadimine ei muutu.
 */
function ensureExtension(name, mime) {
  if (getFileExtension(name)) return name
  const extension = MIME_EXTENSION[String(mime || "").split(";")[0].trim()]
  return extension ? `${name}${extension}` : name
}

export function buildDownloadHeaders(fileName, mime) {
  const safeName = ensureExtension(sanitizeTextFilename(fileName), mime)
