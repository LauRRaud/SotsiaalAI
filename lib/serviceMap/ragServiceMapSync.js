...
function contactMaterialChanged(existing, entry) {
  if (!existing) return true;
  return CONTACT_VERIFICATION_FIELDS.some(field => clean(existing[field]) !== clean(entry[field]));
}
...
function legacyKovRagDocIdCandidates(row = {}) {
  const primary = resolveKovRagDocId(row);
  const slug = clean(row?.municipality?.slug)?.toLowerCase();
  const candidates = [primary];
  if (slug?.includes("-")) candidates.push(buildKovRagDocId(slug.replace(/-/gu, "_")));
...
function stableKovRagContactEntryId(metadata = {}, chunk = {}) {
  ...
  return `rag-kov-contact-${normalizeSlug(docId)}-${normalizeSlug(itemId)}`.slice(0, 180);
}
...
export function createRagServiceMapClient({
  ...
  return {
    async listDocumentChunks(docId, { itemType = "", sourceType = "" } = {}) {
      ...
      if (!Array.isArray(payload?.chunks)) throw new Error("RAG chunk response has invalid shape");
    ...
    async listDocuments({ pageSize = 100, maxPages = 100 } = {}) {
      ...
      for (let page = 0; page < safeMaxPages; page += 1) {
        ...
        const payload = await response.json();
        const items = Array.isArray(payload) ? payload : Array.isArray(payload?.documents) ? payload.documents : null;
        if (!items) throw new Error("RAG documents response has invalid shape");
      ...
      if (!complete) throw new Error("RAG documents pagination limit reached before completion");
      return documents;
    }
  };
}
...
async function upsertEntry(prisma, entry) {
  ...
  const nextStatus = existing?.tombstonedAt || existing?.status === "HIDDEN"
    ? "NEEDS_REVIEW"
    : canPreserveGeocode ? existing.status || "PUBLISHED" : entry.status;
  const nextGeocodingStatus = canPreserveGeocode ? existing.geocodingStatus : entry.geocodingStatus;
  const isPersonContact = [
    SERVICE_MAP_SOURCE.KOV_FILE_CONTACT,
    SERVICE_MAP_SOURCE.RAG_KOV_CONTACT
  ].includes(entry.sourceNamespace);
  const contactReappeared = Boolean(existing?.tombstonedAt || existing?.status === "HIDDEN");
  const materialChanged = isPersonContact && contactMaterialChanged(existing, entry);
  const nextCheckedAt = isPersonContact
    ? (contactReappeared || materialChanged ? null : existing?.checkedAt || null)
    : entry.checkedAt;
  const revisionChanged = Boolean(existing && (
    !isPersonContact || contactReappeared || materialChanged ||
    existing.status !== nextStatus || existing.geocodingStatus !== nextGeocodingStatus
  ));
  ...
  return prisma.serviceMapEntry.upsert({
    ...
    update: {
      ...
      ...(revisionChanged ? { revision: { increment: 1 } } : {})
    }
  });
}

async function syncKovContactsFromRagToServiceMapWithin({
  ...
  for (const row of rows) {
    const docIds = legacyKovRagDocIdCandidates(row);
    ...
    let chunks = [];
    let lastError = null;
    for (const candidateDocId of docIds) {
      try {
        chunks = await ragClient.listDocumentChunks(candidateDocId, {
          itemType: "contact"
        });
        lastError = null;
        break;
    ...
    if (lastError) {
      ...
      result.errors.push({
        docId,
        message: String(lastError?.message || "RAG chunk read failed").slice(0, 300)
      });
      continue;
    }
    ...
    for (const chunk of chunks) {
      ...
      const entry = {
        ...mapKovRagContactChunkToServiceMapEntry(chunk, {
        municipality: row.municipality
        }),
        ...sourceLifecycleFields(SERVICE_MAP_SOURCE.RAG_KOV_CONTACT, generation, now)
      };
  ...
  result.hidden = !dryRun && result.failedDocuments === 0
    ? await reconcileCompleteServiceMapSource({
        db: prisma,
        namespace: SERVICE_MAP_SOURCE.RAG_KOV_CONTACT,
        generation,
        now
      })
    : 0;
...
export async function syncKovContactsFromRagToServiceMap(options = {}) {
  const db = options.prisma || defaultPrisma;
  return withServiceMapSourceLock(db, SERVICE_MAP_SOURCE.RAG_KOV_CONTACT, (tx) =>
    syncKovContactsFromRagToServiceMapWithin({ ...options, prisma: tx })
  );
}

async function syncServiceProviderDocumentsFromRagToServiceMapWithin({
  prisma = defaultPrisma,
  ragClient = createRagServiceMapClient(),
  dryRun = false,
  generation = createSourceGeneration(),
  ...
  for (const document of documents) {
    ...
    const entry = {
      ...mapServiceProviderRagDocumentToServiceMapEntry(document),
      ...sourceLifecycleFields(SERVICE_MAP_SOURCE.RAG_SERVICE_PROVIDER, generation, now)
    };
...
export async function syncServiceProviderDocumentsFromRagToServiceMap(options = {}) {
  const db = options.prisma || defaultPrisma;
  return withServiceMapSourceLock(db, SERVICE_MAP_SOURCE.RAG_SERVICE_PROVIDER, (tx) =>
    syncServiceProviderDocumentsFromRagToServiceMapWithin({ ...options, prisma: tx })
  );
}
