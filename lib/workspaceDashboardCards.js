  ...
  const caseWorkCards = isCaseWorkUiEnabled() && (isProviderView || role === "SOCIAL_WORKER")
    ? [
        makeCard({
          ...
          route: "/juhtumid"
          /* EI OLE `requiresPaid` (SOL-CW-01, omanik 09.08.2026): juhtumite
             lugemine on tasuta, tasulised on tööriistatoimingud. Enne rääkisid
             kaart ja server eri tõde — kaart oli lukus, server lubas kõik läbi.
             Piir jõustub nüüd serveris (`caseWorkRequiresSubscription`,
             `lib/casework/routes.js`), mis vastab tasulisel toimingul 402-ga.
             Lukus kaart tähendaks, et tellimuseta töötaja ei näe oma juhtumeid
             — see rikuks lubadust „ligipääs oma andmetele ei aegu kunagi". */
        })
      ]
    : [];
  ...
  const caseWorkbenchCards = isCaseWorkUiEnabled() && (isProviderView || role === "SOCIAL_WORKER")
    ? [
        makeCard({
          ...
          /* Sama põhjus mis „Minu juhtumite" kaardil (SOL-CW-01): laua
             lugemine on tasuta, tasulised toimingud jõustab server. */
        })
      ]
    : [];
