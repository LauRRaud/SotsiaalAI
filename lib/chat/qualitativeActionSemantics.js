...
const ACTION_FAMILY_PATTERNS = Object.freeze([
  ...
  ["provide", /^(?:pak(?:u|k)|taga(?:da|ma|n|d|b|me|te|vad|s|sin|sid|sime|site|ks|ksin|ksid|ksime|ksite|ta|takse|des|mata)?$|voimalda|provide|offer|ensure|enable|обеспеч|предостав)/u, "enable"],
...
export function qualitativeActionSignature(value = "") {
  ...
  if (/\b(?:(?:peab|peaks|peavad|peaksid|tuleb)\s+olema|(?:should|must|needs?\s+to)\s+be|(?:долж\p{L}*|нужно)\s+быть)\b/u.test(normalized)) {
...
// A recommendation needs a directive, not merely an action-shaped word or a
// positive adjective. In particular, a finite description of an adverse
// effect and "may be necessary" do not establish a proposed intervention.
// This is an evidence-role gate; it does not infer the recommendation itself.
export function hasQualitativeRecommendationIntent(value = "") {
  const normalized = normalizeActionText(value);
  const signature = qualitativeActionSignature(normalized);
  const actions = signature.entries.filter(entry => !entry.family.endsWith("_state"));
  const explicitRecommendation = /\b(?:soovit\p{L}*|ettepanek\p{L}*|recommend\p{L}*|propos\p{L}*)\b/u.test(normalized);
  const obligation = /\b(?:tuleb|peab|peaks|peavad|peaksid|vaja|should|must|needs?\s+to)\b/u.test(normalized);
  const contingent = /\b(?:voib|voivad|voiks|voiksid|may|might|could)\b/u.test(normalized);
  const infinitives = actions.filter(entry => /(?:da|ta|ma)$|^luua$/u.test(entry.term));
  if (explicitRecommendation && actions.length) return true;
  if (obligation && (infinitives.length || signature.families.includes("require_state") ||
    (actions.length && /\b(?:should|must|needs?\s+to)\b/u.test(normalized)))) return true;
  if (contingent) return false;
  const opening = actionTokens(normalized.replace(/^.*?:\s*/u, ""))[0];
  // Bare imperative/infinitive list items can follow a recommendation heading
  // outside the chunk. Do not require that heading to be repeated in each item.
  if (actions.some(entry => entry.term === opening &&
    (infinitives.includes(entry) || /^(?:soovita|maara|nimeta|korralda|muuda|paku|taga|voimalda|vii|toimeta|lihtsusta|holbusta|kasuta|rakenda|toeta|paranda|arenda|tugevda|suurenda|jatka|valti|takista|vahenda|keela|lopeta|sulge|eemalda|keeldu|piira)(?:ge|gem)?$/u.test(entry.term)))) return true;
  // English/Russian directives are admitted only at the clause opening or
  // under an explicit recommendation/modality cue, not by positive sentiment.
  return actions.some(entry => entry.term === opening &&
    /^(?:assign|appoint|create|establish|provide|offer|ensure|enable|deliver|bring|simplify|use|implement|apply|support|improve|develop|strengthen|expand|continue|avoid|prevent|reduce|decrease|ban|prohibit|stop|remove|назначить|создать|обеспечить|предоставить|упростить|использовать|поддержать|развивать|избегать)$/u.test(entry.term));
}
