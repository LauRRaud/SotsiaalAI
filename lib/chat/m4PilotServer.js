...
export async function pilotHandler(fn) {
  try { return await fn(); }
    ...
    const messageKey = ['context_window_full', 'context_search_budget_exceeded', 'context_dialogue_budget_exceeded', 'conversation_context_limit'].includes(code)
    ...
    return pilotJson({ ok: false, code, ...(messageKey ? { messageKey } : {}) }, Number.isInteger(error.status) ? error.status : 500);
  }
}
export async function pilotPost(req) {
  return pilotHandler(async () => {
    ...
    // Fail-closed: a crisis sentence always reaches the crisis contacts, whatever happens to
    // the search or the answer (rejected plan question, provider error, unfinished turn).
    const crisisNotice = result => pilotJson({ ...result, ok: true, isCrisis: true, messageKey: 'chat.crisis.notice', sources: result.sources || [] });
    const crisis = chat && typeof input?.question === 'string' && detectCrisis(input.question);
    let turn;
    try { turn = await service.run(auth.userId, input); }
    catch (error) {
      // Read back this exact key only; a failed request never causes a replacement search/call.
      const row = chat && error.pilotTurnId ? await store.existing(await service.access(), auth.userId, input).catch(() => null) : null;
      if (row && ['answer_rejected', 'stopped'].includes(row.state)) turn = await service.restore(row);
      else if (crisis) return crisisNotice({ completionStatus: 'FAILED', pilotState: row?.state || 'stopped' });
      else throw error;
    }
    if (chat) {
  ...
  return pilotHandler(async () => {
    ...
    if (ref) {
      ...
      return pilotJson({ title: evidence.bibliography.title, version: canonical.document_version_id, pages: canonical.pdf_pages,
        ...(canonical.source_locations === undefined ? {} : { source_locations: canonical.source_locations }), text: evidence.source_text, ref });
    }
    ...
    if (url.searchParams.get('format') === 'chat') {
      ...
      return pilotJson({ ok: true, convId, messages, text: messages.at(-1)?.text || '', sources: messages.at(-1)?.sources || [], isCrisis: !!messages.at(-1)?.isCrisis });
    }
