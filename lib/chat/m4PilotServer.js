...
// Tests replace only the authentication and RAG session boundaries.
export async function pilotPost(req, { authenticate = pilotAuth, session = pilotSession } = {}) {
  return pilotHandler(async () => {
    ...
    try {
      const { config, store, service } = await session(auth);
      if (chat && config.questionPolicy?.mode === 'locked') {
      ...
      if (crisis) return crisisNotice({ completionStatus: 'FAILED', pilotState: 'stopped' });
      throw error;
    }
  });
}
