...
const DIRECTED_RELATION_SEQUENCE_HINT_RE = /^(?:jarjekorr\p{L}*|order|sequence|enne|parast|seejarel)$/u;

export function prioritizeRequestedRelationTopicHints(questionPlan = {}, lexicalHints = [], maxHints = 8) {
  const slots = Array.isArray(questionPlan?.semantic_candidates?.requested_fact_slots?.slots)
    ? questionPlan.semantic_candidates.requested_fact_slots.slots
    : [];
  const directedSlots = slots.filter(slot =>
    slot?.payload_kind === "directed_event_relation_set"
  );
  if (!directedSlots.length) return (Array.isArray(lexicalHints) ? lexicalHints : []).slice(0, maxHints);

  const eventHints = directedSlots.flatMap(slot =>
    Array.isArray(slot?.requested_event_keys) ? slot.requested_event_keys : []
  );
  const sequenceHints = directedSlots.flatMap(slot =>
    Array.isArray(slot?.relation_terms) ? slot.relation_terms : []
  ).filter(term => DIRECTED_RELATION_SEQUENCE_HINT_RE.test(normalizePlanningText(term)));
  const ordered = [...eventHints, ...sequenceHints, ...(Array.isArray(lexicalHints) ? lexicalHints : [])];
  const unique = [];
  const seen = new Set();
  for (const value of ordered) {
    const hint = normalizePlanningText(value);
