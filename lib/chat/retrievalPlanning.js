...
export function buildTemporalRetrievalPlan({
  ...
  const years = (
    ...
    (periodRole !== "evidence_episode" && hasExplicitPeriodCue(combined, normalized, candidateYears.length)) ||
    hasTemporalBreakdownCue(normalized) ||
    hasTemporalComparisonCue(normalized, candidateYears.length)
  )
    ? candidateYears
    : [];
  const breakdownYears = periodRole === "evidence_episode" ? [] : years;
  const comparisonRequested = hasTemporalComparisonCue(normalized, candidateYears.length);
...
export function buildTemporalDevelopmentQueries({
  documentIds = [],
  topicTerms = []
} = {}) {
  const scopedDocumentIds = Array.from(new Set((Array.isArray(documentIds) ? documentIds : [])
    .map(documentId => String(documentId || "").trim())
    .filter(Boolean))).slice(0, 2);
  if (!scopedDocumentIds.length) return [];

  const topicText = Array.from(new Set((Array.isArray(topicTerms) ? topicTerms : [])
    .map(term => String(term || "").trim())
    .filter(Boolean))).slice(0, 6).join(" ");
  if (!topicText) return [];
  const qualitativeDevelopmentFocus = [
    topicText,
    "kuidas tegevus tulemuste järel edasi arenes, jätkus või laienes",
    "uus periood järgmine etapp praeguseks liitunud plaanis süsteemne arendamine"
  ].filter(Boolean).join(" ").trim();
  return scopedDocumentIds.map(documentId => ({
    query: qualitativeDevelopmentFocus,
