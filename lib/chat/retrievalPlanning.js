...
export function extractExplicitSourceYears(text = "") {
  ...
  const allYears = extractTemporalBreakdownYears(source);
  return allYears.length === 1 ? allYears : [];
}
...
export function buildTemporalRetrievalPlan({ message = "", history = [], baseQuery = "" } = {}) {
  ...
  const preferredYears = extractExplicitSourceYears(current);
  const years = (
    acceptedAssistantOffer ||
    hasExplicitPeriodCue(combined, normalized, candidateYears.length) ||
