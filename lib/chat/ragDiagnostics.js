...
import { projectQuestionRequirementsShadow } from "./questionRequirementsContract.js";
import {
  hasValidatedPublicationRecord as hasValidatedPublication,
...
} from "./responsePolicyContract.js";
...
import { projectSourceSelectionTrace } from "./sourceSelectionContract.js";
...
export function projectRagDiagnosticEvidence(input) {
  ...
  const evidence = {
    ...
    source_selection: projectSourceSelectionTrace(trace.source_selection),
    sources: {},
    omissions
  };
  ...
  evidence.qualitative_contract = qualitativeContract ? {
    ...
    ...pick(qualitativeContract, ["requested_slot_count", "mapped_slot_count", "unbound_candidate_count", "ignored_duplicate_candidate_count", "conflicting_unbound_candidate_count"], boundedCount),
    ...pick(qualitativeContract, ["reason"], member([...GROUP_CONTRACT_REASONS, ...DIRECTED_RELATION_CONTRACT_REASONS, "requested_slots_incomplete", "document_identity_not_high", "qualitative_evidence_mapping_incomplete", "qualitative_evidence_conflict", "all_qualitative_slots_bound_to_rendered_evidence"])),
    ...
    slots: list(qualitativeContract.slots, "qualitative_contract.slots", omissions, slot => ({
      ...
      ...pick(slot, ["payload_kind"], member(["known_value_interpretation", "referenced_study_period", "group_distribution", "group_membership", "directed_event_relation_set"])),
  ...
  evidence.validation = {
    ...
    directed_relation_evidence_locators: projectDirectedRelationEvidenceLocators(validation.directed_relation_evidence_locators),
  ...
  evidence.context = list(trace.selected_context_details, "selected_context_details", omissions, context => {
    ...
    return {
      ...
      ...pick(context, [
        "rendered_evidence_chars", "rendered_body_count", "original_body_count",
        "rendered_evidence_body_chars", "body_span_count", "body_span_origin_bound_count",
        "body_span_provenance_bound_count", "rendered_body_span_covered_chars",
        "rendered_body_external_gap_chars", "rendered_body_synthetic_marker_chars",
        "rendered_body_uncovered_chars"
      ], finite),
...
export function buildRagDiagnostics({ trace, turnId = null, userMessageId = null, attempt = null, completionStatus = "COMPLETED", runtime = null } = {}) {
  ...
  const stages = [
    ...(evidence.source_selection ? [stage("source_selection",
      ["changed", "expired", "unavailable"].includes(evidence.source_selection.status) ? "BLOCKED" : "OBSERVED",
      `source_selection_${evidence.source_selection.status || "recorded"}`, ["source_selection"]),
      ...evidence.source_selection.parts.map((part, index) => stage(`source_selection_document_${index + 1}`,
        part.semantic_outcome === "PARTIAL" ? "PARTIAL" : part.fact_validation_passed === false ? "BLOCKED" : part.fact_validation_passed === true ? "PASSED" : "NOT_PROVEN",
        part.fact_validation_reason || "per_document_validation_not_recorded", [`source_selection.parts.${index}`]))] : []),
  ...
  return {
    ...
    runtime_missing_fields: ["configured_model", "reasoning_effort", "build_id", "release_sha", "prompt_hash", "index_generation"].filter(key => !token(runtime?.[key])),
    runtime: {
      ...pick(runtime, ["configured_model", "actual_model", "reasoning_effort", "build_id", "release_sha", "prompt_hash", "model_settings_hash", "index_generation", "registry_generation", "validator_version", "question_contract_version", "question_contract_hash", "query_plan_hash", "rendered_context_hash"]),
...
export function projectRagTraceForLog(payload = {}) {
  ...
  return {
    ...
    ...(evidence.source_selection ? { source_selection: evidence.source_selection } : {}),
