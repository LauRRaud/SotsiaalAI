...
const identityReason = value => /^(?:source_year|evidence_period):(?:19|20)\d{2}$/u.test(value || "") ? value : member(["exact_title_anchor", "ambiguous_identity", "no_identity_candidate", "document_identity_not_lock_eligible", "trusted_document_id_mismatch", "current_turn_document_identity", "current_turn_author_confirmation", "previous_source_exact_filter", "current_turn_document_lock_mismatch", "current_turn_document_lock_mismatch_after_recovery", "decisive_exact_title_anchor", "decisive_canonical_title_family_anchor", "decisive_document_anchor_lead", "decisive_strong_anchor_lead", "decisive_ranked_title_lead", "decisive_numeric_fact_lead", "decisive_fact_fingerprint_lead", "decisive_bounded_episode_lead", "requested_metric_shape_observed", "bounded_episode_evidence_colocated"])(value);
...
export function projectIdentityDecision(value, omissions = []) {
  ...
  return {
    ...
    checks: pick(value.checks, ["exact_title_confirmed", "canonical_title_confirmed", "all_authors_confirmed", "all_source_years_confirmed", "author_confirmation_trusted", "candidate_matches_locked_document"], flag)
  };
}
...
export function projectRagDiagnosticEvidence(input) {
  ...
  evidence.identity = {
    ...
    candidates: list(trace.document_identity?.candidates, "document_identity.candidates", omissions, candidate => ({
      ...
      ...pick(candidate, ["identity_matched", "author_matched", "source_compatible", "source_year_compatible"], flag),
  ...
  evidence.qualitative_contract = qualitativeContract ? {
    ...
    ...pick(qualitativeContract, ["reason"], member(["requested_slots_incomplete", "document_identity_not_high", "qualitative_evidence_mapping_incomplete", "qualitative_evidence_conflict", "all_qualitative_slots_bound_to_rendered_evidence"])),
    ...
    slots: list(qualitativeContract.slots, "qualitative_contract.slots", omissions, slot => ({
      ...
      ...pick(slot, ["value_type"], member(["category", "date", "entity_list", "location", "method", "month", "organization", "person_role", "recommendation", "season", "text_relation", "timepoint"])),
      ...pick(slot, ["payload_kind"], member(["known_value_interpretation", "referenced_study_period"])),
