...
export function projectRagDiagnosticEvidence(input) {
  ...
  const yearMentions = list(mentionsObserved ? yearField.value : [], "answer_validation_contract_shadow.planner.fields.year_role_mentions", omissions, mention => year(mention?.value) ? {
    ...
    ...pick(mention, ["method"], member(["forward_explicit_year_cue", "nearest_explicit_year_cue", "conflicting_explicit_year_cues", "no_explicit_year_cue", "bounded_episode_period", "explicit_year_range"]))
  } : null);
...
export function projectRagTraceForLog(payload = {}) {
  ...
  return {
    ...
    ...pick(payload, ["role"]),
    ...evidence.counts,
    query_plan: { ...pick(evidence.plan, ["mode", "selection_strategy"]), years: evidence.plan.years },
    document_identity: evidence.identity,
    history_selection: evidence.history,
    fact_validation: evidence.validation,
    requested_fact_slot_contract: evidence.metric_contract,
