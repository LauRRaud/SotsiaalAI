...
function isAssistantSourceUiIssue(message = "") {
  ...
  const explicitSourceUiSurface = /\b(vastuste allikad|allikarea|allikapaneel|allikate nupp|viidete rida|source panel|sources panel)\b/.test(text);
  const describesPresentationProblem = /\b(kaasa|tarbet|uleliig|ebavajal|naeb|naitab|kuvab|tekib|tekkis|valtida|imelik|valesti|katki|ilma otsese)\b/.test(text);
  const hasInterfaceContext = /\b(vastus|vastuses|liides|nupp|rida|paneel|kuva|ekraan|vestlus|chat)\b/.test(text);
  return explicitSourceUiSurface || (describesPresentationProblem && hasInterfaceContext);
}
