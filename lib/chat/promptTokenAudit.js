...
//
// Lipp sees:
//   - mõõdetakse ainult mahud, tokeniarvud ja hash;
...
export async function measurePromptComponents({ components, model }) {
  ...
  for (const part of parts) {
    out[`${part.name}_chars`] = part.chars;
    out[`${part.name}_sha256_12`] = part.chars ? shortHash(part.text) : null;
    let tokens = null;
    ...
    out[`${part.name}_tokens_estimated`] = tokens;
