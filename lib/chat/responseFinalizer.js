...
export async function finalizeAssistantReply({
  ...
  /* SOL-CHAT-01: tasu arveldatakse SIIN, terminalmarkeriga ühes tehingus. Kutsuja ei tohi enam
     ise enne finaliseerimist commit'ida — kadunud vastus koos arvestatud ühikuga oli täpselt see
     viga. `null` tähendab „selle raja arveldust ei ole" (nt persist=false rada). */
  settleUsage = null,
  ...
  const appendPersistence = deps.persistAppend || persistAppend;
  const completePersistence = deps.persistDone || persistDone;
  const onPersistFailure = deps.onPersistFailure || null;
  let finalAttachments = Array.isArray(attachments) ? attachments : [];
  let persistOutcome = { required: false, durable: true, failure: null };
  if (persist && convId && userId) {
    persistOutcome = { required: true, durable: false, failure: null };
    const persistDeps = {
      onFailure: (reason, error) => {
        persistOutcome.failure = persistOutcome.failure || reason;
        if (typeof onPersistFailure === "function") onPersistFailure(reason, error);
      }
    };
    if (!persistInitialized) {
      ...
      if (initialized === false) {
        return { attachments: finalAttachments, persisted: persistOutcome };
      }
    }
    if (!ragAttempt) await appendPersistence({
      convId,
      userId,
      fullText: reply
    });
    const persistResult = await completePersistence({
      convId,
      userId,
      status: "COMPLETED",
      finalText: reply,
    ...
    persistOutcome.durable = !!persistResult;
    persistOutcome.diagnosticRef = persistResult?.assistantMessageId ? `message:${persistResult.assistantMessageId}` : null;
    if (!persistResult) {
      return { attachments: finalAttachments, persisted: persistOutcome };
    }
