...
function bindLocator(group, block, bodySpan, candidate, blockIndex) {
  ...
  if (at < 0 || original.lastIndexOf(fragment) !== at) {
    return { reason: "directed_relation_fragment_origin_ambiguous", locator: null };
  }
  if (!Number.isInteger(bodySpan.literal_original_start) ||
  ...
  if (reparsed.status !== "ADMITTED" || !reparsed.candidates.some(item =>
    item.start === at && item.end === at + fragment.length && directedRelationPayloadMatches(item, candidate))) {
  ...
  for (const [blockIndex, group] of groups.entries()) {
    ...
    for (const span of block.bodySpans || []) {
      ...
      for (const candidate of parsed.candidates) {
        ...
        if (!bound.locator) return fail(bound.reason || "directed_relation_locator_missing");
