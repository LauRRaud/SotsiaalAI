...
const REQUESTED_QUALITATIVE_SLOT_CUE_RE = /(?<![\p{L}\p{N}])(?:mida|mis|millis\p{L}*|kuidas|millal|kelle|kes|kus|what|which|how|when|whose|who|where|какой|какая|какие|как|когда|чей|чья|чьи|кто|где)(?![\p{L}\p{N}])/gu;
...
const REQUESTED_COUNT_RELATION_RE = /(?:^|[^\p{L}\p{N}])(?:inimes\p{L}*|laps\p{L}*|noor\p{L}*|osalej\p{L}*|vastaj\p{L}*|tootaj\p{L}*|spetsialist\p{L}*|kohtum\p{L}*|intervju\p{L}*|vestlus\p{L}*|juhtum\p{L}*|otsus\p{L}*|riik\p{L}*|maakon\p{L}*|omavalits\p{L}*|people|persons?|children|participants?|respondents?|workers?|specialists?|meetings?|interviews?|cases?|decisions?|countries|counties|municipalities|человек\p{L}*|ребен\p{L}*|участник\p{L}*|респондент\p{L}*|работник\p{L}*|специалист\p{L}*|встреч\p{L}*|интервью|случа\p{L}*|решени\p{L}*|стран\p{L}*|муниципал\p{L}*)(?=$|[^\p{L}\p{N}])/u;
...
function extractRequestedNumericSlots(normalized = "", inputForm = "original", rawInput = "") {
  ...
  for (let index = 0; index < cueMatches.length; index += 1) {
    ...
    const followingQuestionCue = candidateSegment.search(
      /[,;]\s*(?:ning\s+|ja\s+|and\s+|и\s+)?(?:millis\p{L}*|kuidas|millal|kelle|kes|kus|what|which|how|when|whose|who|where|какой|какая|какие|как|когда|чей|чья|чьи|кто|где)\b/u
    );
...
function requestedFactValueType(normalized = "") {
  ...
  if (/\b(?:loetle|nimeta|millis\p{L}*|mis olid)\b/u.test(normalized)) return "entity_list";
...
function extractRequestedFactSlots(normalized = "", inputForm = "original", rawInput = "", numericSlots = null, morphology = null) {
...
function buildQuestionSemanticCandidates(normalized = "", inputForm = "original", rawInput = "", morphology = null) {
  ...
  return {
    ...
    requested_fact_slots: extractRequestedFactSlots(normalized, inputForm, rawInput, requestedNumericSlots, morphology),
...
export function buildQuestionPlan({
  ...
  const currentTurnResearchAuthor = semanticCandidates?.current_turn_document_identity?.author?.confidence === "high"
    ? semanticCandidates.current_turn_document_identity.author.value
    : null;
  const researchFactPersonName = rawSpecificResearchFact?.personName || currentTurnResearchAuthor;
  const specificResearchFact = rawSpecificResearchFact && researchFactPersonName
    ? {
        ...rawSpecificResearchFact,
        personName: canonicalMorphologyPersonCandidate(researchFactPersonName, morphology)
      }
    : rawSpecificResearchFact;
