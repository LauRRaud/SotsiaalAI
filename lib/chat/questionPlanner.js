...
function namedEnumeratedResearchFactShape(normalized = "", sourceMatch = null) {
  if (!sourceMatch || !/\b(?:nimeta|loetle|millis\p{L}*)\b/u.test(normalized)) return false;
  const countWords = normalized.match(
    /\b(?:\d+|uhe\p{L}*|kahe\p{L}*|kolm\p{L}*|kolme\p{L}*|neli\p{L}*|nelja\p{L}*|viis\p{L}*|viie\p{L}*|kuus\p{L}*|kuue\p{L}*|seitse\p{L}*|seitsme\p{L}*|kaheksa\p{L}*|uheksa\p{L}*|kumn\p{L}*)\b/gu
  ) || [];
  return countWords.length >= 2 && /\b(?:ja|ning)\b/u.test(normalized);
}
...
function extractSpecificResearchFactIntent(message = "") {
  ...
  const enumeratedListShape = namedEnumeratedResearchFactShape(normalized, sourceMatch);
  if (
    (!sourceMatch && !compactShape && !implicitNumericShape) ||
    (!RESEARCH_FACT_SHAPE_RE.test(normalized) && !enumeratedListShape)
  ) return null;
  ...
  const sourceKind = /uuring/u.test(sourceToken)
    ...
    : /uurimus/u.test(sourceToken)
      ? "uurimus"
      : sourceToken || "uuring";
  return { personName, subjectTerms, factTerms, sourceKind, compactShape, implicitNumericShape, enumeratedListShape };
}
