...
const REQUESTED_NUMERIC_SLOT_CUE_RE = /(?<![\p{L}\p{N}])(?:kui\s+mitu\s+protsenti|mitu\s+protsenti|kui\s+suur\s+osa|mis\s+osakaal|kui\s+suured?|kui\s+palju|kui\s+kaua|mitu|what\s+(?:percentages?|proportions?|shares?)|how\s+(?:many|much|large|big|long)|сколько\s+процент\p{L}*|какой\s+процент\p{L}*|какая\s+доля|как\s+долго|сколько)(?![\p{L}\p{N}])/gu;
...
function extractRequestedNumericSlots(normalized = "", inputForm = "original", rawInput = "") {
  ...
  for (let index = 0; index < cueMatches.length; index += 1) {
    ...
    const alternativeRelations = directValueType === "proportion"
      ...
      : null;
    const coordinatedRelations = alternativeRelations || coordinatedRequestedSlotRelations(segment);
...
function extractRequestedFactSlots(normalized = "", inputForm = "original", rawInput = "", numericSlots = null) {
  ...
  return {
    version: "requested_fact_slots_v1",
...
function extractSpecificResearchFactIntent(message = "") {
  ...
  const implicitNamedFactShape = !sourceMatch && !compactShape && !implicitNumericShape && !boundedEpisodeShape &&
  ...
  if (
    (!sourceMatch && !compactShape && !implicitNumericShape && !boundedEpisodeShape && !implicitNamedFactShape && !enumeratedListShape) ||
    (!RESEARCH_FACT_SHAPE_RE.test(normalized) && !boundedEpisodeShape && !implicitNamedFactShape && !enumeratedListShape)
  ) return null;
  ...
  const sourceKind = /uuring/u.test(sourceToken)
    ...
    : /uurimus/u.test(sourceToken)
      ? "uurimus"
      : !sourceMatch && (boundedEpisodeShape || implicitNamedFactShape || enumeratedListShape)
        ? ""
        : sourceToken || "uuring";
  return {
    personName,
...
export function extractNamedPersonIntent(message = "", morphology = null) {
  ...
  const fallbackCandidates = [
    ...surfacePersonCandidates(text),
    ...morphologyPersonCandidates(morphology)
  ];
...
export function buildQuestionPlan({
  ...
  if (isKovServiceAndLocalRuleQuestion(normalized, resolvedMunicipalities)) {
    const resolvedMunicipality = Array.isArray(resolvedMunicipalities) ? resolvedMunicipalities[0] : null;
    return makeQuestionPlan({
  ...
  if (isKovServiceOrBenefitQuestion(normalized, resolvedMunicipalities)) {
    const resolvedMunicipality = Array.isArray(resolvedMunicipalities)
      ? resolvedMunicipalities[0]
      : null;
    return makeQuestionPlan({
