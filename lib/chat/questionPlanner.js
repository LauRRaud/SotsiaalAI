...
const RESEARCH_FACT_SOURCE_RE = /\b(uuring|uuringu|uuringus|uuringust|uurimus|uurimuse|uurimuses|uurimusest|artikkel|artikli|artiklis|artiklist|aruanne|aruande|aruandes|aruandest|raport|raporti|raportis|raportist|analuus|analuusi|analuusis|analuusist|kaardistus|kaardistuse|kaardistuses|kusitlus|kusitluse|kusitluses)\b/;
...
const RESEARCH_FACT_STOP_PREFIXES = Object.freeze([
  ...
  "jareld", "kaardist", "kirjeld", "kui", "kusitlus", "meetod", "millal",
  "milline", "millised", "mis", "mitu", "oli", "olid", "osakaal", "osalej", "palju", "pohjal", "protsent", "raport",
  "tehti", "uuring", "uurimus", "valim", "vastaj"
]);
...
function extractSpecificResearchFactIntent(message = "") {
  ...
  if (!RESEARCH_FACT_SOURCE_RE.test(normalized) || !RESEARCH_FACT_SHAPE_RE.test(normalized)) return null;
  ...
  const subjectTerms = Array.from(new Set(tokens.filter(token =>
    token.length >= 4 &&
    !personTokens.has(token) &&
    !RESEARCH_FACT_STOP_PREFIXES.some(prefix => token.startsWith(prefix))
  ))).slice(0, 10);
  const factTerms = Array.from(new Set(tokens.filter(token =>
    RESEARCH_FACT_STOP_PREFIXES.some(prefix => token.startsWith(prefix)) &&
    /^(?:aasta|analuus|arv|intervju|jareld|meetod|osakaal|osalej|protsent|valim|vastaj)/u.test(token)
  ))).slice(0, 8);
  const sourceKind = normalized.match(RESEARCH_FACT_SOURCE_RE)?.[1] || "uuring";
  return { personName, subjectTerms, factTerms, sourceKind };
}
...
export function buildQuestionPlan({ message = "", role = "" } = {}) {
  ...
  if (specificResearchFact) {
    return makePlan({
      ...
      planner_reason: "singular_research_source_and_fact_shape"
    });
  }
