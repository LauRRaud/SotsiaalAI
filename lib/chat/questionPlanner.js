...
function requestedYearRole(normalized = "", yearRoleMentions = []) {
  const yearQuestion = /\b(?:mis\s+aasta(?:l|st)?|millisel\s+aastal|mis\s+ajast|millal|when|what\s+year|which\s+year)\b/u.exec(
    String(normalized || "")
  );
  ...
  const roles = new Set(
    (Array.isArray(yearRoleMentions) ? yearRoleMentions : [])
      ...
      .filter(role => role === "document_source_year" || role === "evidence_year")
  );
  if (roles.size === 1) return roles.has("document_source_year") ? "publication_year" : "evidence_year";
  return "ambiguous";
}

function currentEvidenceScope(normalized = "", mode = "default") {
  if (/(?:^|[^\p{L}\p{N}])(?:praegu|praeg\p{L}*|hetkel|hetkeseis\p{L}*|tana|tanapaev\p{L}*|kehtiv\p{L}*|ajakoh\p{L}*|varsk\p{L}*|current\p{L}*|latest|fresh|recent\p{L}*|now|today|valid|up\s+to\s+date|as\s+of|most\s+recent|сейчас|сегодня|текущ\p{L}*|актуальн\p{L}*|свеж\p{L}*|последн\p{L}*)(?=$|[^\p{L}\p{N}])/u.test(normalized)) {
    return "current";
  }
  if (["specific_research_fact", "specific_document_question", "specific_document_summary", "person_source_lookup", "legal_exact"].includes(mode)) {
