...
function isSpecificDocumentGuidanceQuestion(text = "", morphology = null) {
  ...
  const documentSignal = /\b(artikkel|artikli|artiklis|artiklist|dokument|dokumendi|dokumendis|dokumendist|fail|faili|failis|failist|pdf|uuring|uuringu|uuringus|uuringust|aruanne|aruande|aruandes|aruandest|raport|raporti|raportis|raportist|juhend|juhendi|juhendis|juhendist|juhendmaterjal|juhendmaterjali|tooleht|toolehe|toolehel|tooleht)\b/.test(searchable);
...
const REQUESTED_NUMERIC_SLOT_CUE_RE = /(?<![\p{L}\p{N}])(?:kui\s+mitu\s+protsenti|mitu\s+protsenti|kui\s+suur\s+osa|mis\s+osakaal|kui\s+suured?|kui\s+palju|kui\s+kaua|mitu|mitmes|what\s+(?:percentages?|proportions?|shares?)|how\s+(?:many|much|large|big|long)|сколько\s+процент\p{L}*|какой\s+процент\p{L}*|какая\s+доля|как\s+долго|сколько)(?![\p{L}\p{N}])/gu;
const REQUESTED_QUALITATIVE_SLOT_CUE_RE = /(?<![\p{L}\p{N}])(?:milline|millised|millist|kuidas|millal|kelle|kes|kus|what|which|how|when|whose|who|where|какой|какая|какие|как|когда|чей|чья|чьи|кто|где)(?![\p{L}\p{N}])/gu;
...
function currentTurnDocumentIdentityAuthor(message = "", documentCues = [], inputForm = "original") {
  const text = String(message || "");
  if (!text || !documentCues.length) return null;
  ...
  const { distance: _distance, ...author } = candidate;
  return author;
}
...
function currentTurnDocumentIdentityCandidate(message = "", inputForm = "original", yearRoleMentions = []) {
  const documentCues = originalTextPlannerMatches(message, CURRENT_TURN_DOCUMENT_IDENTITY_SOURCE_CUES)
    ...
    .sort((left, right) => left.index - right.index || left.end - right.end)
    .slice(0, 8);
  const author = currentTurnDocumentIdentityAuthor(message, documentCues, inputForm);
...
function currentEvidenceScope(normalized = "", mode = "default") {
  if (currentStatusEvidenceRequested(normalized)) {
    return "current";
  }
...
function requestedNumericSlotValueType(cue = "", segment = "", inheritedType = null) {
  if (/(?:kui\s+kaua|how\s+long|как\s+долго)/u.test(cue)) {
...
function extractRequestedNumericSlots(normalized = "", inputForm = "original", rawInput = "") {
  ...
  for (let index = 0; index < cueMatches.length; index += 1) {
    ...
    const followingQuestionCue = candidateSegment.search(
      /[,;]\s*(?:ning\s+|ja\s+|and\s+|и\s+)?(?:milline|millised|millist|kuidas|millal|kelle|kes|kus|what|which|how|when|whose|who|where|какой|какая|какие|как|когда|чей|чья|чьи|кто|где)\b/u
    );
...
function requestedFactValueType(normalized = "") {
  ...
  if (/\b(?:summa|euro|maksumus|hind|tasu)\p{L}*\b/u.test(normalized)) return "amount";
  ...
  if (/\b(?:mis aastal|millisel aastal|aasta)\p{L}*\b/u.test(normalized)) return "calendar_year";
  if (/\b(?:millal|mis ajal|ajapunkt)\p{L}*\b/u.test(normalized)) return "timepoint";
  if (/\b(?:kui kaua|kestus|paev|nadal|kuu)\p{L}*\b/u.test(normalized)) return "duration";
  ...
  if (/\b(?:meetod|metoodika|kuidas uuriti|andmeid koguti)\p{L}*\b/u.test(normalized)) return "method";
  if (/\b(?:mitu|kui palju|arv|loendus)\p{L}*\b/u.test(normalized)) return "count";
  if (/\b(?:loetle|nimeta|millised|mis olid)\b/u.test(normalized)) return "entity_list";
...
function extractRequestedQualitativeSlots(normalized = "", inputForm = "original") {
  ...
  return qualitativeCues.flatMap(match => {
    ...
    const segment = normalized.slice(cueEnd, segmentEnd)
      ...
      .replace(/[\s,;:.!?]+$/gu, "")
      .trim();
    const relationTerms = requestedSlotRelationTerms(`${cue} ${segment}`);
    if (!relationTerms.length) return [];
    const distributionMatch = `${cue} ${segment}`.match(
    ...
    return [{
      index: 0,
      span_start: cueStart,
      value_type: requestedFactValueType(`${cue} ${segment}`),
      relation_terms: relationTerms,
      derivation: "qualitative_clause",
...
function extractRequestedFactSlots(normalized = "", inputForm = "original", rawInput = "", numericSlots = null) {
  ...
  const explicitValueSlots = explicitValueRelation
    ? Array.from(String(rawInput || normalized).matchAll(/(?<![\p{L}\d])\d+(?:[.,]\d+)?\s*%?/gu)).map(match => {
        const spanStart = Number(match.index);
        const value = String(match[0] || "").replace(/\s+/gu, "").replace(",", ".");
        const localRelation = String(rawInput || normalized).slice(
          Math.max(0, spanStart - 48),
          Math.min(String(rawInput || normalized).length, spanStart + String(match[0] || "").length + 48)
        );
        return {
          index: 0,
    ...
    : [];
  const qualitativeSlots = explicitValueRelation
    ? []
  ...
  return {
    ...
    complete: inherited.length ? numericSlots.complete === true : relationTerms.length > 0,
    expected_cardinality: expectedCardinality,
    slots: slots.slice(0, 12)
  };
}

function buildQuestionSemanticCandidates(normalized = "", inputForm = "original", rawInput = "") {
  ...
  const yearRoleMentions = semanticYearMentions(normalized, episodeYearSpans, inputForm);
  const requestedNumericSlots = extractRequestedNumericSlots(normalized, inputForm, rawInput);
  return {
    ...
    requested_fact_slots: extractRequestedFactSlots(normalized, inputForm, rawInput, requestedNumericSlots),
...
function extractSpecificResearchFactIntent(message = "") {
  ...
  if (
    (!sourceMatch && !compactShape && !implicitNumericShape && !explicitValueRelationShape && !boundedEpisodeShape && !implicitNamedFactShape && !enumeratedListShape) ||
    (!RESEARCH_FACT_SHAPE_RE.test(normalized) && !explicitValueRelationShape && !boundedEpisodeShape && !implicitNamedFactShape && !enumeratedListShape)
  ) return null;
  ...
  const sourceKind = /uuring/u.test(sourceToken)
    ...
    : /uurimus/u.test(sourceToken)
      ? "uurimus"
      : !sourceMatch && (explicitValueRelationShape || boundedEpisodeShape || implicitNamedFactShape || enumeratedListShape)
        ? ""
        : sourceToken || "uuring";
  return {
    personName,
...
function isKovServiceOrBenefitQuestion(text = "", resolvedMunicipalities = []) {
...
function isKovServiceAndLocalRuleQuestion(text = "", resolvedMunicipalities = []) {
  return isKovServiceOrBenefitQuestion(text, resolvedMunicipalities) &&
...
function isExplicitJournalSynthesisQuestion(text = "") {
  ...
  const pluralArticleSignal = /\b(?:artiklid|artiklites|artiklite|kirjutised|lood|allikad)\b/u.test(text);
  const crossSourceSignal = /\b(eri|mitm\w*|ulevaad\w*|vordl\w*|kirjeldavad|kasitlevad|toovad|raagivad)\b/.test(text);
  return sourceSignal && crossSourceSignal && (journalSignal || pluralArticleSignal);
}
...
export function buildQuestionPlan({
  ...
  const normalized = normalizePlannerText(message);
  const lexicalSocialScope = inferLexicalSocialScope(normalized);
  ...
  const boundedSemanticInputForm = ["original", "canonical_fallback"].includes(semanticInputForm)
    ? semanticInputForm
    : null;
  const semanticCandidates = buildQuestionSemanticCandidates(normalized, boundedSemanticInputForm, message);
  ...
  if (isKovServiceAndLocalRuleQuestion(normalized, resolvedMunicipalities)) {
  ...
  if (isKovServiceOrBenefitQuestion(normalized, resolvedMunicipalities)) {
    const municipalityMatches = Array.isArray(resolvedMunicipalities) ? resolvedMunicipalities : [];
    const resolvedMunicipality = municipalityMatches.length === 1 ? municipalityMatches[0] : null;
    return makeQuestionPlan({
  ...
  const specificResearchFact = rawSpecificResearchFact?.personName
    ? {
        ...rawSpecificResearchFact,
        personName: canonicalMorphologyPersonCandidate(rawSpecificResearchFact.personName, morphology)
      }
    : rawSpecificResearchFact;
  ...
  if (isSpecificDocumentGuidanceQuestion(normalized, morphology)) {
    return makeQuestionPlan({
