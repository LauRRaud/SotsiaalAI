...
function extractSpecificResearchFactIntent(message = "") {
  ...
  if ((!sourceMatch && !compactShape) || !RESEARCH_FACT_SHAPE_RE.test(normalized)) return null;
  ...
  const sourceKind = sourceMatch?.[1] || "uuring";
  return { personName, subjectTerms, factTerms, sourceKind, compactShape };
}
