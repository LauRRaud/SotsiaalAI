...
const REQUESTED_NUMERIC_SLOT_CUE_RE = /(?<![\p{L}\p{N}])(?:kui\s+mitu\s+protsenti|mitu\s+protsenti|kui\s+suur\s+osa|mis\s+osakaal|kui\s+suured?|kui\s+palju|mitu|what\s+(?:percentage|proportion|share)|how\s+(?:many|much|large|big)|сколько\s+процент\p{L}*|какой\s+процент\p{L}*|какая\s+доля|сколько)(?![\p{L}\p{N}])/gu;
...
function extractRequestedNumericSlots(normalized = "", inputForm = "original") {
  ...
  for (let index = 0; index < cueMatches.length; index += 1) {
    ...
    recognizedClauseCount += relationParts.length;
