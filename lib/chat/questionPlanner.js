...
import { namedMetricAgeScopes } from "./namedMetricScope.js";
import { groupRequirementForClause } from "./groupFactSemantics.js";
import { directedRelationRequirementForClause } from "./directedRelationSemantics.js";
import { isBibliographicSourceModifierCue } from "./questionClauseRoles.js";
import {
...
const REQUESTED_QUALITATIVE_SLOT_CUE_RE = /(?<![\p{L}\p{N}])(?:mida|mis|millis\p{L}*|millin\p{L}*|kuidas|miks|millal|kelle|kes|kus|what|which|how|why|when|whose|who|where|какой|какая|какие|как|почему|когда|чей|чья|чьи|кто|где)(?![\p{L}\p{N}])/gu;
...
const REQUESTED_SLOT_RELATION_NOISE_RE = /^(?:aasta\p{L}*|year\p{L}*|год\p{L}*|artik(?:kel|l)\p{L}*|kirjutis\p{L}*|aruand\p{L}*|raport\p{L}*|dokumend\p{L}*|uuring\p{L}*|uurimus\p{L}*|study|research|report|article|document|исследован\p{L}*|отч[её]т\p{L}*|стат\p{L}*|kirjeld\p{L}*|vahenda\p{L}*|esita\p{L}*|maini\p{L}*|mida|mis|millis\p{L}*|millin\p{L}*|kuidas|miks|millal|kelle|kes|kus|kohta|what|which|how|why|when|whose|who|where|ja|ning|voi|and|or|и|et|that|что|почему|jargi|pohjal|according|oli|olid|on|oeldi|is|are|was|were|nad|they|need|neid|these|those)$/u;
const REQUESTED_ENUMERATION_COUNT_WORDS = new Map([
  ["uks", 1], ["uhe", 1], ["uht", 1], ["uhte", 1],
  ["kaks", 2], ["kahe", 2], ["kaht", 2], ["kahte", 2],
  ["kolm", 3], ["kolme", 3], ["neli", 4], ["nelja", 4],
  ["viis", 5], ["viie", 5], ["kuus", 6], ["kuue", 6]
]);

function requestedQualitativeSlotRelationTerms(value = "", limit = 8) {
  return requestedSlotRelationTerms(value, limit)
    .filter(token => !REQUESTED_ENUMERATION_COUNT_WORDS.has(token));
}
...
function semanticYearMentions(normalized = "", episodeYearSpans = [], inputForm = "original", rawInput = normalized) {
  ...
  return Array.from(String(normalized || "").matchAll(/(?<!\d)(?:19\d{2}|20\d{2}|2100)(?!\d)/gu), (yearMatch, mentionIndex) => {
    ...
    // A locally scoped observation year must not borrow the publication noun
    // beyond a different year ("osakaalud 2017. aasta kohta ... 2019. aasta artikkel").
    const yearEnd = Number(yearMatch.index) + value.length;
    const observationPrefix = normalized.slice(Math.max(0, Number(yearMatch.index) - 72), Number(yearMatch.index));
    const observationSuffix = normalized.slice(yearEnd, yearEnd + 40);
    if (/\b(?:osakaal\p{L}*|naitaj\p{L}*|statistik\p{L}*|andme\p{L}*)\s*$/u.test(observationPrefix) &&
      /^\.?\s*aasta\s+kohta\b/u.test(observationSuffix)) {
      return { ...mentionIdentity, value, role: "evidence_year", cue_family: "data", method: "explicit_observation_year", input_form: inputForm };
    }
...
function requestedSlotRelationTerms(value = "", limit = 8) {
  // This interrogative asks for the value's shape, not a subject/category
  // called "range" that every answer must repeat as a content anchor.
  const relationText = normalizePlannerText(value).replace(/\bmillis\p{L}*\s+vahemik\p{L}*\b/gu, " ");
  return Array.from(new Set(
    (relationText.match(/[\p{L}\p{N}-]+/gu) || [])
      .filter(token =>
        token.length >= 3 &&
        !/^\d+(?:[.,]\d+)?$/u.test(token) &&
...
function requestedNamedScopeProportions(rawInput = "", inputForm = "original", yearRoleMentions = []) {
  // Preserve case until the two explicitly named comparison scopes are found.
  // Lowercase coordinated subjects and joint totals are not separate metrics.
  const match = String(rawInput || "").match(/^(\s*(?:[Mm]illis\p{L}*|[Mm]is|[Kk]ui\s+suur\p{L}*)\s+)([\p{Lu}][\p{L}-]+(?:\s+[\p{Lu}][\p{L}-]+){0,2})\s+(?:ja|ning)\s+([\p{Lu}][\p{L}-]+(?:\s+[\p{Lu}][\p{L}-]+){0,2})\s+([^.;?!]*?\bosakaal\p{L}*)/u);
  if (!match || /\b(?:kokku|uhine|koond)\p{L}*\b/u.test(normalizePlannerText(rawInput))) return null;
  if ([...normalizePlannerText(rawInput.slice(match[0].length)).matchAll(REQUESTED_NUMERIC_SLOT_CUE_RE)].length) return null;
  const scopes = [match[2], match[3]].map(value => requestedSlotRelationTerms(value));
  if (scopes.some(terms => !terms.length) || scopes[0].join(" ") === scopes[1].join(" ")) return null;
  const shared = requestedSlotRelationTerms(match[4], 5);
  const ages = namedMetricAgeScopes(match[4]);
  const observationYears = [...new Set(yearRoleMentions.filter(mention => mention.role === "evidence_year").map(mention => mention.value))];
  if (ages.length > 1 || observationYears.length > 1) return null;
  const scopeConstraints = { ...(ages[0] ? { age: ages[0] } : {}),
    ...(observationYears[0] ? { observation_year: observationYears[0] } : {}) };
  const slots = scopes.map((terms, index) => ({
    index: index + 1,
    span_start: 0,
    value_type: "proportion",
    relation_terms: [...new Set([...terms, ...shared])].slice(0, 8),
    named_scope_terms: terms,
    named_scope_constraints: scopeConstraints,
    scope_values: requestedSlotScopeValues(match[4]),
    expected_cardinality: null,
    derivation: "coordinated_named_scope",
    coordination_group: 1,
    value_type_source: "explicit_cue",
    input_form: inputForm
  }));
  return { complete: true, recognized_clause_count: 1, emitted_slot_count: 2,
    unresolved_clause_count: 0, truncated: false, slots };
}

function extractRequestedNumericSlots(normalized = "", inputForm = "original", rawInput = "", yearRoleMentions = []) {
  const namedScopes = requestedNamedScopeProportions(rawInput, inputForm, yearRoleMentions);
  if (namedScopes) return namedScopes;
...
function requestedFactExpectedCardinality(normalized = "") {
  const numeric = normalized.match(/\b(\d{1,2})\s+(?:nait\p{L}*|ulesan\p{L}*|osakaal\p{L}*|protsent\p{L}*|teema\p{L}*|punkt\p{L}*|tulemus\p{L}*|komponent\p{L}*|tingimus\p{L}*|element\p{L}*|osa\p{L}*|samm\p{L}*|tegevus\p{L}*|pohimot\p{L}*|soovitus\p{L}*|aspekt\p{L}*|tegur\p{L}*|meetod\p{L}*)\b/u);
  if (numeric) return Number(numeric[1]);
  const word = normalized.match(/\b(uks|uhe|kaks|kahe|kolm|kolme|neli|nelja|viis|viie|kuus|kuue|seitse|seitsme|kaheksa|uheksa|kumme|kumnet)\s+(?:nait\p{L}*|ulesan\p{L}*|osakaal\p{L}*|protsent\p{L}*|teema\p{L}*|punkt\p{L}*|tulemus\p{L}*|komponent\p{L}*|tingimus\p{L}*|element\p{L}*|osa\p{L}*|samm\p{L}*|tegevus\p{L}*|pohimot\p{L}*|soovitus\p{L}*|aspekt\p{L}*|tegur\p{L}*|meetod\p{L}*)\b/u);
  return word ? REQUESTED_FACT_CARDINALITY_WORDS[word[1]] || null : null;
}

function requestedFactValueType(normalized = "") {
  if (knownProportionInterpretation(normalized)) return "text_relation";
  if (referencedStudyPeriodRequested(normalized)) return "timepoint";
  ...
  const asksProcessStart = /\b(?:kus\p{L}*|where|откуда|где)\b/u.test(normalized) &&
    /\b(?:alg\p{L}*|starts?|begins?|начин\p{L}*)\b/u.test(normalized) &&
    /\b(?:[\p{L}-]*teekon\p{L}*|protsess\p{L}*|menetlus\p{L}*|etapp\p{L}*|journey|pathway|process|procedure|процесс\p{L}*|путь\p{L}*)\b/u.test(normalized);
  if (asksProcessStart) return "text_relation";
  ...
  if (/\b(?:kuidas\s+(?:jagun|jaota)\p{L}*|jaotus|jagunesid)\p{L}*\b/u.test(normalized)) return "distribution";
...
function knownProportionInterpretation(text = "") {
  const values = Array.from(text.matchAll(/(?<![\p{L}\d])(\d+(?:[.,]\d+)?)\s*%/gu));
  if (values.length !== 1 || !/\b(?:tahenda\p{L}*|meaning|means?|interpret\p{L}*)\b/u.test(text)) return null;
  return { value: values[0][1].replace(",", "."), unit: "percent" };
}

function referencedStudyPeriodRequested(text = "") {
  return /\b(?:mis\s+ajast|millisest\s+ajast|millal|mis\s+periood\p{L}*)\b/u.test(text) &&
    /\b(?:uuring\p{L}*|uurimus\p{L}*)\b/u.test(text);
}
...
function extractRequestedQualitativeSlots(normalized = "", inputForm = "original") {
  ...
  const qualitativeCues = Array.from(
    ...
    .filter(match => !isBibliographicSourceModifierCue(normalized, match));
  ...
  const emittedByCue = qualitativeCues.map(match => {
    ...
    const relationTerms = requestedQualitativeSlotRelationTerms(requestedClause);
    ...
    if (ellipsisDistributionMatch) {
      ...
      return branches.map((branch, index) => ({
        ...
        value_type: "count",
        relation_terms: requestedQualitativeSlotRelationTerms(branch),
    ...
    return relationParts.flatMap((relationPart, index) => {
      const partTerms = requestedQualitativeSlotRelationTerms(relationPart);
      if (!partTerms.length) return [];
      const directedRequirement = directedRelationRequirementForClause(`${cue} ${relationPart}`);
      const valueType = directedRequirement.value_type || requestedFactValueType(`${cue} ${relationPart}`);
      const knownAnchor = knownProportionInterpretation(relationPart);
      const studyPeriod = referencedStudyPeriodRequested(`${cue} ${relationPart}`);
      return [{
        index: 0,
        span_start: cueStart + index,
        clause_span_start: cueStart,
        clause_span_end: segmentEnd,
        value_type: valueType,
        ...groupRequirementForClause(cue + " " + relationPart, valueType),
        ...directedRequirement,
        ...(knownAnchor ? { payload_kind: "known_value_interpretation", known_anchor: knownAnchor,
          requested_dimensions: ["population", "measured_phenomenon", "frequency_or_condition"] } : {}),
        ...(studyPeriod ? { payload_kind: "referenced_study_period", temporal_role: "referenced_study_period" } : {}),
        relation_terms: partTerms,
        minimum_answer_items: directedRequirement.minimum_answer_items ||
          requestedQualitativeMinimumAnswerItems(cue, relationPart, valueType),
...
function extractRequestedFactSlots(
  ...
  const qualitativeSlots = qualitative.slots.filter(slot => !inherited.some(numeric =>
    numeric.derivation === "coordinated_named_scope" && numeric.span_start === slot.span_start
  ));
  ...
  const fallbackSlots = extractedSlots.length ? extractedSlots : [{
    ...
    ...directedRelationRequirementForClause(normalized),
    relation_terms: relationTerms,
    input_form: inputForm
  }];
  const slots = fallbackSlots.map((slot, index) => {
    const slotExpectedCardinality = slot.expected_cardinality ?? expectedCardinality;
    const minimumAnswerItems = slot.value_type === "entity_list" && Number.isInteger(slotExpectedCardinality)
      ? Math.max(1, Number(slot.minimum_answer_items || 1), slotExpectedCardinality)
      : slot.minimum_answer_items;
    return {
      ...slot,
      index: index + 1,
      expected_cardinality: slotExpectedCardinality,
      ...(minimumAnswerItems ? { minimum_answer_items: minimumAnswerItems } : {}),
      category: slot.category || null,
      explicit_values: Array.isArray(slot.explicit_values) ? slot.explicit_values
        : Number.isInteger(slot.clause_span_start) && Number.isInteger(slot.clause_span_end)
          ? Array.from(normalized.slice(slot.clause_span_start, slot.clause_span_end)
            .matchAll(/(?<![\p{L}\d])\d+(?:[.,]\d+)?\s*%?/gu), match => match[0].replace(/\s+/gu, "").replace(",", "."))
  ...
  const interpretations = slots.filter(slot => slot.payload_kind === "known_value_interpretation");
  if (interpretations.length === 1) {
    for (const slot of slots.filter(slot => slot.payload_kind === "referenced_study_period")) {
      // An evidence-search dependency, NOT a numeric value required in the time answer.
      slot.reference_known_anchor = interpretations[0].known_anchor;
      slot.reference_slot_index = interpretations[0].index;
    }
  }
  ...
  return enrichRequestedFactSlotsWithMorphology({
    ...
    qualitative_clause_count: Number(qualitative.recognized_clause_count || 0) -
      (qualitative.slots.length > qualitativeSlots.length ? 1 : 0),
...
function buildQuestionSemanticCandidates(normalized = "", inputForm = "original", rawInput = "", morphology = null) {
  ...
  const documentIdentity = currentTurnDocumentIdentityCandidate(rawInput, inputForm, yearRoleMentions);
  let requestedRawInput = maskQuotedDocumentTitles(rawInput);
  for (const author of documentIdentity.authors.filter(author => author.confidence === "high")) {
    // Identity spans refer to the original input; masking preserves their offsets.
    if (!Number.isInteger(author.span_start) || !Number.isInteger(author.span_end)) continue;
    requestedRawInput = requestedRawInput.slice(0, author.span_start) +
      " ".repeat(author.span_end - author.span_start) + requestedRawInput.slice(author.span_end);
  }
  ...
  const requestedNumericSlots = extractRequestedNumericSlots(requestedNormalized, inputForm, requestedRawInput, yearRoleMentions);
  return {
    ...
    current_turn_document_identity: documentIdentity
  };
}
...
function extractSpecificResearchFactIntent(message = "") {
  ...
  const explicitEvidencePeriodYears = Array.from(new Set(typedYearRoleMentions
    .filter(mention => mention.role === "evidence_year" && ["explicit_year_range", "explicit_observation_year"].includes(mention.method))
