...
const BOUNDED_EPISODE_QUANTITY_CUE_RE = /(?<![\p{L}\p{N}])(?:kui\s+palju|kui\s+suured?|mitu|mitmesse|mitmest|how\s+(?:many|much|large|big)|number\s+of|count\s+of|сколько|число|количество)(?![\p{L}\p{N}])/gu;
const BOUNDED_EPISODE_MAGNITUDE_CUE_RE = /^(?:kui\s+suured?|how\s+(?:large|big))$/u;
