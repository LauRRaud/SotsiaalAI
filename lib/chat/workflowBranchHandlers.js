...
export async function handleDocumentWorkflowBranch({
  ...
  try {
    ...
    if (!generationCompleted && typeof onGenerationFailure === "function") {
      try {
        await onGenerationFailure("chat_document_generation_failed");
      } catch (usageError) {
        if (typeof logError === "function") {
          logError("document_task.usage_release_failed", {
            err: usageError?.message || String(usageError),
            userId
          });
        }
      }
    ...
    if (String(error?.code || "").startsWith("USAGE_")) {
      error.usageWorkCompleted = generationCompleted;
      throw error;
    }
    if (typeof logError === "function") {
