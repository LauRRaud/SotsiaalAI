...
const GEOCODING_PLACE_CANDIDATE_STOPWORDS = new Set([
  "on", "oli", "olid", "oleks", "asub", "asus", "toimub", "toimus", "toimusid",
  "tehti", "korraldati", "hakati", "mis", "mida", "mitu", "palju", "kas", "kui", "kuidas"
]);
...
function cleanGeocodingPlaceCandidate(value = "") {
  ...
  const normalizedWords = normalizeMunicipalitySearchText(cleaned).split(/\s+/u).filter(Boolean);
  if (normalizedWords.length && normalizedWords.every(word => GEOCODING_PLACE_CANDIDATE_STOPWORDS.has(word))) return "";
  return cleaned;
}
