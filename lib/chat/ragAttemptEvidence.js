...
const FAILURE_CODES = new Set(["usage_reservation_failed", "retrieval_failed", "model_failed", "model_reasoning_effort_unsupported", "persistence_failed", "request_cancelled", "lease_expired", "attempt_superseded", "unhandled_failure"]);
...
export function projectAttemptRuntime(input = {}) {
  const result = {};
  for (const key of ["release_sha", "build_id", "configured_model", "actual_model", "reasoning_effort", "validator_version", "question_contract_version", "index_generation", "registry_generation"]) {
