...
function normalizeToken(token = "") {
  ...
  const service = value.match(/^([\p{Letter}\p{Number}]*?teenus)(?:e|t|el|ele|elt|es|est|ega|eks|eni|esse|ed|eid|te|tes|test|tele|tel|telt)?$/u);
  if (service?.[1]) return service[1];
  const benefit = value.match(/^([\p{Letter}\p{Number}]*?toetus)(?:e|t|el|ele|elt|es|est|ega|eks|eni|esse|ed|eid|te|tes|test|tele|tel|telt)?$/u);
  if (benefit?.[1]) return benefit[1];
  return value;
}
...
function queryLexicalTokens(query = "") {
  const ignored = new Set([
    "teenus", "sotsiaalteenus", "toetus", "abi", "vald", "valla", "vallas",
    "linn", "linna", "linnas", "kuidas", "milline", "millised", "seotud",
    "saada", "saab", "taotlema", "taotleda", "taotlemine", "kohta", "asjus",
    "poole", "poorduda", "olemas", "pakutakse", "kas"
  ]);
  return [...new Set(normalizeText(query)
    .split(" ")
    .map(normalizeToken)
...
function requestedPackageSections(query = "") {
  const normalized = normalizeText(query);
  const sections = new Set(["description"]);
  const overview = isServiceOverviewQuestion(query);
  const conditions = isServiceConditionQuestion(query);
  const asksApplication = /\b(taotle|taotleda|taotlemine|taotl|avaldus|avalduse|kuidas saada|kuidas kasutada)\b/u.test(normalized);

  if (overview) {
    for (const section of ["eligibility", "application", "forms", "contacts", "legal_basis"]) {
      sections.add(section);
    }
  }
  if (conditions) {
    for (const section of ["eligibility", "application", "legal_basis"]) sections.add(section);
  }
  if (asksApplication) {
    for (const section of ["application", "forms", "contacts"]) sections.add(section);
  }
  for (const [section, pattern] of Object.entries(QUERY_SECTION_PATTERNS)) {
    if (pattern.test(normalized)) sections.add(section);
  }
  return [...sections].filter(section => Object.prototype.hasOwnProperty.call(SECTION_LABELS_ET, section));
}
...
function packageIdentityTokens(pkg = {}) {
  return new Set(packageIdentityText(pkg)
    .split(" ")
    .map(normalizeToken)
    .filter(Boolean));
}
...
function packageRelevanceScore(pkg = {}, options = {}) {
  ...
  const tokens = packageIdentityTokens(pkg);
  let score = 0;
  for (const anchor of anchors) {
    if (tokens.has(anchor)) score += 100;
  }
...
function choosePackages(sourcePackages = [], options = {}) {
  ...
  const anchors = queryAnchorTokens(options.query);
  const lexicalTokens = queryLexicalTokens(options.query);
  ...
  const scored = packages.map((pkg, index) => ({
    ...
    anchorMatches: anchors.filter(anchor => packageIdentityTokens(pkg).has(anchor)),
    lexicalMatches: lexicalTokens.filter(token => packageIdentityTokens(pkg).has(token))
  }));
  if (!packages.length) {
    return { packages: [], status: "no_packages", anchors };
  }
  if (!clean(options.query)) {
    return {
      packages: scored
        .sort((a, b) => b.score - a.score || a.index - b.index)
        .slice(0, maxPackages)
        .map(item => item.pkg),
      status: "legacy_no_query",
      anchors
    };
  }
  if (!anchors.length) {
    const lexical = scored.filter(item => item.lexicalMatches.length >= 2);
    if (!lexical.length) {
      return { packages: [], status: "insufficient_service_match", anchors };
    }
    return {
      packages: lexical
        .sort((a, b) => b.lexicalMatches.length - a.lexicalMatches.length || b.score - a.score || a.index - b.index)
        .slice(0, 1)
        .map(item => item.pkg),
      status: "strong_lexical_match",
      anchors
    };
  }
  const relevant = scored.filter(item => item.anchorMatches.length > 0);
  if (!relevant.length) {
    const lexical = scored.filter(item => item.lexicalMatches.length >= 2);
    if (!lexical.length) {
      return { packages: [], status: "insufficient_service_match", anchors };
...
export function buildPackageAwareContext(sourcePackages = [], options = {}) {
  const selection = choosePackages(sourcePackages, options);
  ...
  if (!packages.length) {
    return {
      ...
      missingSectionsUsed: [],
      requestedSections: [],
      packageSectionSourceIds: {},
      packageCandidateSourceIds: [],
      packageDisplayedSourceIds: [],
      packageAnswerFlags: selection.status === "insufficient_service_match"
        ? ["insufficient_service_match"]
        : [],
      packageSelectionStatus: selection.status,
      serviceAnchors: selection.anchors,
      insufficientPreciseSupport: selection.status === "insufficient_service_match",
  ...
  const requestedSections = requestedPackageSections(options.query);
  const sectionSourceIds = Object.fromEntries(requestedSections.map(section => [section, new Set()]));
  ...
  const professionalRole = String(options.role || options.normalizedRole || "").trim().toUpperCase() === "SOCIAL_WORKER";
  ...
  packages.forEach((pkg, index) => {
    ...
    for (const section of requestedSections) {
      const sectionSources = Array.isArray(sections[section]) ? sections[section] : [];
      for (const source of sectionSources) {
        const sourceId = clean(source?.source_id);
        if (!sourceId) continue;
        sectionSourceIds[section].add(sourceId);
  ...
  const guardrails = [
    ...
    "- Täpse tasu, tähtaja, vormi või kontakti tohib esitada ainult sama municipality_id ja sama canonical_item_id package'i vastava kinnitatud sektsiooni põhjal.",
    "- Kui küsimuse jaoks vajalik fees, deadlines, forms või contacts sektsioon on märgitud missing, ära kasuta selle detaili täitmiseks teise teenuse package'it ega üldteadmisi; ütle, et kasutatud allikad ei kinnita detaili piisavalt.",
    ...
    "- Tallinna linna puhul: kui teenuse taotlemine käib elukohajärgse linnaosa sotsiaalhoolekande osakonna kaudu ja kasutaja ei ole linnaosa/asumit täpsustanud, ütle, et konkreetse kontakti valimiseks on vaja linnaosa, ning küsi linnaosa üle. Ära jäta sellepärast teenuse tingimusi, taotlemise põhisuunda või õiguslikku alust vastamata.",
    "- Kui kasutaja on Tallinnas nimetanud linnaosa, asumit või aadressi ning kontaktiallikad on olemas, paku vastava linnaosa kontakt või kontaktileht; kui täpne linnaosa kontakt puudub, paku Tallinna teenuse ametlik leht ja palu linnaosa täpsustada.",
    "- journal_article võib olla taust, aga mitte current teenuse, vormi, kontakti, tasu, tähtaja või õigusliku aluse kinnitus.",
    "- Kasutajale kuvatavad allikad peavad tulema küsimuse jaoks kasutatud package'i sektsioonide kinnitatud source_id väärtustest; pelk samasse package'isse kuulumine ei ole kuvamiseks piisav."
  ].join("\n");
  ...
  const professionalLegalGuardrail = professionalRole
    ? [
        "SPETSIALISTI ÕIGUSVIITE REEGEL:",
        "- Kui vastad sotsiaalvaldkonna spetsialistile ja legal_basis sektsioon on olemas, nimeta paragrahvinumber ja paragrahvi pealkiri, kui need on allika pealkirjas või metaandmetes nähtavad.",
        "- Erista õigusakti tasand: KOV kord/määrus, riiklik seadus või muu õigusakt.",
        "- Kui allikas kinnitab ainult õigusakti nimetuse, aga mitte täpset paragrahvi, ütle seda lühidalt; ära mõtle paragrahve välja."
      ].join("\n")
    : "";

  return {
    used: true,
    contextText: [guardrails, conditionGuardrail, professionalLegalGuardrail, ...blocks].filter(Boolean).join("\n\n"),
    usedPackageIds,
    missingSectionsUsed: [...missingSections].sort(),
    requestedSections,
    packageSectionSourceIds: Object.fromEntries(
      Object.entries(sectionSourceIds).map(([section, ids]) => [section, [...ids].sort()])
    ),
    packageCandidateSourceIds: [...candidateSourceIds].sort(),
    // Legacy compatibility only. These are candidates from requested sections;
    // the final displayed set is decided after claim attribution.
    packageDisplayedSourceIds: [...candidateSourceIds].sort(),
    packageAnswerFlags: [...answerFlags].sort(),
    packageSelectionStatus: selection.status,
    serviceAnchors: selection.anchors,
    insufficientPreciseSupport: missingSections.size > 0,
    requiredEvidenceSections: [...missingSections].sort()
  };
}
