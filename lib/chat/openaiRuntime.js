...
function buildPromptCacheKey({ userId, effectiveRole, replyLang, isCrisis }) {
  const userPartition = promptCacheKeyPart(userId, "anonymous").slice(-24);
  const role = promptCacheKeyPart(effectiveRole, "client").slice(0, 16);
  const lang = promptCacheKeyPart(replyLang, "et").slice(0, 4);
  return `sotsiaalai:chat:v1:${userPartition}:${role}:${lang}:${isCrisis ? "c" : "n"}`.slice(0, 64);
}
...
export async function callOpenAI({
  ...
  const payload = buildResponsesPayload(input, {
    stream: false,
    effectiveRole,
    promptCacheKey: buildPromptCacheKey({ userId, effectiveRole, replyLang, isCrisis })
  });
