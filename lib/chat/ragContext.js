...
function stripSyntheticRagPrefix(text = "") {
  ...
  body = body.replace(/^\s*\[PAGES\]\s*[\d,\s\-–—]+/i, "");
  body = body.trim();
...
function resolveContextMaxChars(options = {}) {
  ...
  const narrowFactTarget = Math.max(
    Math.ceil(RAG_CTX_MAX_CHARS * 1.5),
    RAG_GROUP_BODY_MAX_CHARS * 8
  );
...
export function renderOneContextBlock(entry, index, options = {}) {
  ...
  const bodyLimit = expandedBodyBudget
    ? Math.min(contextMaxChars, Math.max(RAG_GROUP_BODY_MAX_CHARS, RAG_GROUP_BODY_MAX_CHARS * 6))
  ...
  const maxBodies = Number.isFinite(requestedMaxBodies)
    ? Math.max(1, Math.min(6, Math.trunc(requestedMaxBodies)))
    : 2;
  ...
  if (bodies.length === 1) {
    bodyText = bodies[0].length > bodyMaxChars
    ...
    const clipBody = (body, cap) => body.length > cap
...
function buildCompactContextWithBudget(groups, budget, maxGroups = CONTEXT_GROUPS_MAX, options = {}) {
  ...
  let high = options?.allowExpandedBodyBudget === true
    ? Math.min(resolveContextMaxChars(options), RAG_GROUP_BODY_MAX_CHARS * 6)
    : RAG_GROUP_BODY_MAX_CHARS;
  let bestText = "";
