...
export function renderOneContextBlock(entry, index, options = {}) {
  ...
  if (bodies.length === 1) {
    ...
    const selectedBodies = bodies.slice(0, 2);
    const available = Math.max(2, bodyMaxChars - separator.length);
    let firstCap = Math.ceil(available * 0.55);
    let secondCap = available - firstCap;
    if (selectedBodies[0].length < firstCap) {
      secondCap += firstCap - selectedBodies[0].length;
      firstCap = selectedBodies[0].length;
    }
    if (selectedBodies[1].length < secondCap) {
      firstCap += secondCap - selectedBodies[1].length;
      secondCap = selectedBodies[1].length;
    }
    const clipBody = (body, cap) => body.length > cap
      ? `${body.slice(0, Math.max(1, cap - 3)).trimEnd()}...`
      : body;
    bodyText = [
      clipBody(selectedBodies[0], firstCap),
      clipBody(selectedBodies[1], secondCap)
    ].join(separator);
  }
