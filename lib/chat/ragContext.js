...
function renderContextBlock(entry, index, options = {}) {
  ...
  if (bodies.length === 1) {
    renderedBodyCount = 1;
    truncated = bodies[0].length > bodyMaxChars;
    const rawRenderedBody = truncated
      ? bodies[0].slice(0, Math.max(1, bodyMaxChars - 3)).trimEnd()
      : bodies[0];
    bodyText = truncated ? `${rawRenderedBody}...` : rawRenderedBody;
    ...
    bodySpans = [{
      ...
      start_offset: 0,
      end_offset: rawRenderedBody.length,
      truncated
    }];
  } else if (bodies.length > 1) {
    const selectedBodies = bodies.slice(0, maxBodies);
    ...
    for (let bodyIndex = 0; bodyIndex < selectedBodies.length; bodyIndex += 1) {
      const bodiesLeft = selectedBodies.length - bodyIndex;
      const evenShare = Math.max(1, Math.floor(remaining / bodiesLeft));
      const cap = Math.min(selectedBodies[bodyIndex].length, evenShare);
      caps.push(cap);
      remaining -= cap;
    }
    const clipBody = (body, cap, bodyIndex) => {
      const bodyTruncated = body.length > cap;
      const rawRenderedBody = bodyTruncated
        ? body.slice(0, Math.max(1, cap - 3)).trimEnd()
        : body;
      if (bodyTruncated) truncated = true;
      const renderedBody = bodyTruncated ? `${rawRenderedBody}...` : rawRenderedBody;
      const displayedBody = displayUrlsInText(renderedBody);
      bodySpans.push({
        original_body_index: bodyIndex,
        original_body_hash: hashContextText(body),
        rendered_body_hash: hashContextText(displayedBody),
    ...
    bodyText = selectedBodies.map((body, bodyIndex) => clipBody(body, caps[bodyIndex], bodyIndex)).join(separator);
  }
