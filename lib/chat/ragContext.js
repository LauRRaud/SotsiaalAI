...
import { hasUnresolvedPassageOpening, isJournalFrontMatter } from "./evidenceContent.js";
import { isOrdinalTimeBoundary } from "./qualitativeTimeSemantics.js";
import { canonicalAuthorKey } from "./authorIdentity.js";
...
function normalizeMatch(m, idx) {
  ...
  const documentVersion = md.document_version || m?.document_version || null;
  const rawChunk = String(m?.text || m?.chunk || "");
  const bodyOffset = bodyRaw ? rawChunk.indexOf(bodyRaw) : -1;
  const bodyEvidence = bodyRaw && bodyOffset >= 0 && rawChunk.lastIndexOf(bodyRaw) === bodyOffset
    ? { document_id: docId, source_id: sourceId, chunk_id: chunkId,
        document_version: documentVersion, chunk_hash: hashContextText(rawChunk),
        chunk_char_count: rawChunk.length,
        author_identity_hashes: authors.map(author => hashContextText(canonicalAuthorKey(author))),
        normalized_body_hash: hashContextText(bodyRaw), chunk_body_offset: bodyOffset }
    : null;
  ...
  if (bodyEvidence) bodyEvidence.source_status = sourceStatus;
...
export function groupMatches(matches) {
  ...
  matches.forEach((raw, idx) => {
    ...
    if (!m.body) return;
    const nonContentBody = isJournalFrontMatter(m.body, m);
    // A shared opening is not identical evidence: later claims or source
    // versions or author metadata may disagree. Preserve conflicting provenance
    // for downstream gates even when the literal body is identical.
    const snippetKey = [m.docId || m.title, m.page ?? "", m.bodyEvidence?.document_version || "",
      m.sourceId || "", m.sourceStatus || "", m.sourceType || "", m.collectionId || "",
      [...new Set(m.bodyEvidence?.author_identity_hashes || [])].sort().join(","), hashContextText(m.body)].join("|");
    ...
    // Keep author/title registry identity even when the only retrieved chunk
    // is front matter; it contributes no body to a substantive answer.
    if (!nonContentBody) {
  ...
  return order.map(key => {
    ...
    return {
      ...safeEntry,
      bodyEvidence: (Array.isArray(entry.bodyRecords) ? entry.bodyRecords : [])
        .map(record => record.evidence).filter(Boolean),
      authors,
      pages,
      pageRanges,
      tags,
...
function topicTokenRecords(value = "") {
  const normalized = String(value || "")
    ...
    .replace(/\p{Diacritic}+/gu, "")
    .toLowerCase();
  return Array.from(normalized.matchAll(/[a-z0-9]+/gi), match => ({
    token: match[0],
    index: match.index
  }));
}

function findTopicHintPositions(text = "", hint = "") {
  const textRecords = topicTokenRecords(text);
  const hintTokenList = topicTokens(hint);
  if (!textRecords.length || !hintTokenList.length) return [];
  const positions = [];
  ...
  for (let start = 0; start < textRecords.length; start += 1) {
    ...
    let hintIndex = 1;
    while (
      hintIndex < hintTokenList.length &&
      textIndex < textRecords.length &&
      textIndex <= start + hintTokenList.length + 3
    ) {
      if (topicTokenMatches(textRecords[textIndex].token, hintTokenList[hintIndex])) hintIndex += 1;
      textIndex += 1;
    }
    if (hintIndex === hintTokenList.length) positions.push(textRecords[start].index);
    if (positions.length >= 32) break;
  }
  return positions;
}

function topicPassageAroundPosition(source = "", matchPosition = -1, cap = RAG_GROUP_BODY_MAX_CHARS) {
  if (!Number.isSafeInteger(matchPosition) || matchPosition < 220) return null;
  const lookbackStart = Math.max(0, matchPosition - 320);
  const beforeMatch = source.slice(lookbackStart, matchPosition);
  const lastBoundary = Array.from(beforeMatch.matchAll(/[.!?]\s+|\n+/g)).at(-1);
  let passageStart = lastBoundary
    ? lookbackStart + lastBoundary.index + lastBoundary[0].length
    : Math.max(0, matchPosition - 80);
  const maxRawChars = Math.max(1, cap - 6);
  if (matchPosition >= passageStart + maxRawChars) passageStart = Math.max(0, matchPosition - 80);
  const passageLimit = Math.min(source.length, passageStart + maxRawChars);
  const afterMatch = source.slice(matchPosition, Math.min(passageLimit, matchPosition + 1100));
  const endBoundaries = Array.from(afterMatch.matchAll(/[.!?](?=\s|$)/g));
  const preferredEndBoundary = endBoundaries[Math.min(1, endBoundaries.length - 1)];
  const passageEnd = preferredEndBoundary
    ? matchPosition + preferredEndBoundary.index + preferredEndBoundary[0].length
    : passageLimit;
  const rawPassage = source.slice(passageStart, passageEnd).trim();
  if (!rawPassage) return null;
  passageStart = source.indexOf(rawPassage, passageStart);
  const boundedEnd = passageStart + rawPassage.length;
  return passageStart >= 0 && passageStart <= matchPosition && boundedEnd > matchPosition
    ? { start: passageStart, end: boundedEnd }
    : null;
}

function preferredTopicPassageSpan(text = "", topicHints = [], cap = RAG_GROUP_BODY_MAX_CHARS) {
  const source = String(text || "");
  if (source.length <= cap || !Array.isArray(topicHints) || !topicHints.length) return null;
  const normalizedHints = Array.from(new Set(topicHints
    .map(hint => normalizeTopicValue(hint))
    .filter(Boolean)));
  const positions = Array.from(new Set(normalizedHints
    .flatMap(hint => findTopicHintPositions(source, hint))))
    .sort((a, b) => a - b)
    .slice(0, 96);
  const sequenceRequested = normalizedHints.some(hint =>
    /^(?:jarjekord|järjekord|order|sequence|enne|parast|pärast|seejarel|seejärel)$/u.test(hint)
  );
  const candidates = positions
    .map(position => topicPassageAroundPosition(source, position, cap))
    .filter(Boolean)
    .map(span => {
      const passage = source.slice(span.start, span.end);
      const hintCoverage = normalizedHints.filter(hint => textMatchesTopicHint(passage, hint)).length;
      const sequenceCueCount = sequenceRequested
        ? (normalizeTopicValue(passage).match(/\b(?:koigepealt|kõigepealt|esmalt|enne|seejarel|seejärel|alles|siis|parast|pärast|samal ajal)\b/gu) || []).length
        : 0;
      return { ...span, hintCoverage, sequenceCueCount };
    })
    .sort((left, right) =>
      right.hintCoverage - left.hintCoverage ||
      right.sequenceCueCount - left.sequenceCueCount ||
...
function rankBodiesWithTopicHints(group, topicHints = []) {
  ...
  return bodies
    ...
    // Keep every retrieved body byte-for-byte intact. Rewriting a body to put
    // a focused passage first invalidates the immutable chunk hash and offset
    // proof attached during normalization. The renderer already receives the
    // same topic terms and can select a bounded passage without changing the
    // source body's identity.
    .map(item => item.body);
}
...
export function selectProfessionalMethodGuidanceGroups(groups, k = 4, lambda, options = {}) {
  ...
  const populationScope = value => {
    const text = normalizeTopicValue(value);
    return {
      child: /\b(?:laps|lapse\w*|laste\w*|lastel|lastele|lastest|child\w*)\b/u.test(text),
      adult: /\b(?:taiskasvan\w*|adult\w*)\b/u.test(text)
    };
  };
  const requestedPopulation = populationScope(options.question || (options.topicHints || []).join(" "));
  ...
  const eligible = (Array.isArray(groups) ? groups : []).filter(group => {
    ...
    const sourcePopulation = populationScope(group.title || "");
    // A shared assessment term cannot override an explicit, conflicting
    // population in the source title. Mixed-population sources remain eligible.
    if (requestedPopulation.child && !requestedPopulation.adult && sourcePopulation.adult && !sourcePopulation.child) return false;
    if (requestedPopulation.adult && !requestedPopulation.child && sourcePopulation.child && !sourcePopulation.adult) return false;
...
function clipContextBody(body = "", cap = RAG_GROUP_BODY_MAX_CHARS, preferredYears = [], options = {}) {
  ...
  const topicSpan = options?.preferTopicPassage === true
    ? preferredTopicPassageSpan(text, options?.preferredTopicTerms, cap)
    : null;
  if (topicSpan) {
    const rawRenderedBody = text.slice(topicSpan.start, topicSpan.end);
    return {
      renderedBody: `${topicSpan.start > 0 ? "..." : ""}${rawRenderedBody}${topicSpan.end < text.length ? "..." : ""}`,
      rawRenderedBody,
      startOffset: topicSpan.start,
      endOffset: topicSpan.end,
      truncated: true
    };
  }

  let rawRenderedBody = text.slice(0, Math.max(1, cap - 3)).trimEnd();
  if (options?.requireSelfContainedPassages === true) {
    const sentenceEnds = Array.from(rawRenderedBody.matchAll(/[.!?][”"'’»]?(?=\s|$)/gu))
      .filter(match => !isOrdinalTimeBoundary(text.slice(0, match.index + match[0].length), text.slice(match.index + match[0].length)));
    const lastEnd = sentenceEnds.at(-1);
    // No complete sentence fits: keep no partial claim, rather than inventing
    // a syntactic continuation from a separately ranked excerpt.
    rawRenderedBody = lastEnd ? rawRenderedBody.slice(0, lastEnd.index + lastEnd[0].length) : "";
  }
  return {
...
function renderContextBlock(entry, index, options = {}) {
  ...
  if (bodies.length === 1) {
    ...
    const clippedBody = clipContextBody(bodies[0], bodyMaxChars, preferredYears, {
      requireSelfContainedPassages: options?.requireSelfContainedPassages,
    ...
    rawRenderedParts = [clippedBody.renderedBody];
    ...
    rawRenderedParts = selectedBodies.map((body, bodyIndex) => clipBody(body, caps[bodyIndex]));
    bodyText = rawRenderedParts.join(separator);
  }
  const renderedParts = rawRenderedParts.length
    ? rawRenderedParts.map(part => displayUrlsInText(part))
    : [displayUrlsInText(bodyText)];
  const renderedEvidenceText = renderedParts.join(separator);
  // URL rewriting and clipping can change lengths. Consumers must bind a
  // literal rendered span to the original body before claiming a chunk offset.
  let renderedOffset = 0;
  bodySpans = bodySpans.map((span, spanIndex) => {
    const part = renderedParts[spanIndex] || "";
    const original = String(bodies[span.original_body_index] || "");
    const boundedStart = Number.isSafeInteger(span.start_offset) && span.start_offset >= 0
      ? span.start_offset
      : -1;
    const boundedEnd = Number.isSafeInteger(span.end_offset) && span.end_offset > boundedStart &&
      span.end_offset <= original.length ? span.end_offset : -1;
    const expectedLiteral = boundedStart >= 0 && boundedEnd > boundedStart
      ? original.slice(boundedStart, boundedEnd)
      : "";
    const leadingMarker = boundedStart > 0 ? "..." : "";
    const trailingMarker = boundedEnd >= 0 && boundedEnd < original.length ? "..." : "";
    const expectedPart = `${leadingMarker}${expectedLiteral}${trailingMarker}`;
    const exactAtBoundedStart = !!expectedLiteral && part === expectedPart;
    const renderedLiteralStart = exactAtBoundedStart ? leadingMarker.length : 0;
    const renderedLiteralEnd = exactAtBoundedStart
      ? renderedLiteralStart + expectedLiteral.length
      : part.length;
    const renderedLiteral = part.slice(renderedLiteralStart, renderedLiteralEnd);
    const provenance = (Array.isArray(entry.bodyEvidence) ? entry.bodyEvidence : [])
      .filter(record => record.normalized_body_hash === span.original_body_hash);
    const result = { ...span,
      rendered_body_hash: hashContextText(renderedLiteral),
      rendered_body_chars: renderedLiteral.length,
      synthetic_marker_chars: leadingMarker.length + trailingMarker.length,
      rendered_start_offset: renderedOffset + renderedLiteralStart,
      rendered_end_offset: renderedOffset + renderedLiteralEnd,
      literal_original_start: exactAtBoundedStart
        ? boundedStart
        : null,
      provenance: renderedParts.length === bodySpans.length ? provenance : [] };
    renderedOffset += part.length + separator.length;
    return result;
  });
  return {
    ...
    syntheticSeparatorChars: Math.max(0, renderedParts.length - 1) * separator.length,
    bodySpans
  };
}
...
function buildCompactContextWithBudget(groups, budget, maxGroups = CONTEXT_GROUPS_MAX, options = {}) {
  ...
  if (bestText) {
    return finalizeRenderedContext(usableGroups, bestRenderedBlocks, options);
  }
  ...
  return finalizeRenderedContext(used, renderedBlocks, options);
}

function finalizeRenderedContext(groups, blocks, options = {}) {
  const pairs = blocks.map((block, index) => ({ block, group: groups[index] }))
    .filter(({ block }) => options?.requireSelfContainedPassages !== true || /[\p{L}\p{N}]/u.test(block.evidenceText));
  const renderedBlocks = pairs.map(({ block }, index) => ({ ...block, text: block.text.replace(/^\(\d+\)/u, `(${index + 1})`) }));
  return { text: renderedBlocks.map(block => block.text).join("\n\n"),
    used: pairs.map(({ group }) => group), renderedBlocks };
}
...
export function buildContextWithBudget(groups, options = {}) {
  ...
  const usableGroups = groups.map(group => ({ ...group,
    bodies: (Array.isArray(group.bodies) ? group.bodies : []).filter(body =>
      !isJournalFrontMatter(body, group) &&
      !(options?.requireSelfContainedPassages === true && hasUnresolvedPassageOpening(body, group)))
  })).filter(group => group.bodies.length).slice(0, maxGroups);
