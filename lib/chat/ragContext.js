...
function renderContextBlock(entry, index, options = {}) {
  ...
  bodySpans = bodySpans.map((span, spanIndex) => {
    ...
    const literalAt = part ? original.indexOf(part) : -1;
    const boundedStart = Number.isSafeInteger(span.start_offset) && span.start_offset >= 0
      ? span.start_offset
      : -1;
    const exactAtBoundedStart = boundedStart >= 0 &&
      original.slice(boundedStart, boundedStart + part.length) === part;
    ...
    const result = { ...span, rendered_start_offset: renderedOffset,
      rendered_end_offset: renderedOffset + part.length,
      literal_original_start: exactAtBoundedStart
        ? boundedStart
        : literalAt >= 0 && original.lastIndexOf(part) === literalAt ? literalAt : null,
