...
export function buildSourceAnchoredRagQueries(message = "", history = [], baseQuery = "") {
  ...
  if (broadMultiSource) {
    if (isThematicSynthesisRagQuestion(message)) {
      ...
      return [
        ...selectThematicBroadQueries(query, Math.max(1, 3 - focusedLimited.length)),
        ...focusedLimited
      ];
    }
    return [
...
export async function searchRagQueries({
  ...
  /* Chroma päringud kasutavad sama kohalikku indeksit. Kolm samaaegset laia
     päringut muudavad iga alampäringu tootmises mitu korda aeglasemaks kui
     nende järjestikune täitmine, seega hoia see rada ühe töötaja peal. */
  const settled = await runWithConcurrency(uniqueQueries, 1, (entry, index) =>
    searchRagDirect({
      query: entry.query,
      topK: perQueryTopK,
