...
// B0b: jõudeoleku-timeout'i deterministlikuks testimiseks. Tootmises jäävad
// need vaikeväärtused muutumatuks — fetchImpl on globaalne fetch ja timeoutMs
// on sama 12000, mis varem.
...
async function searchRagDirect({
  ...
  const effectiveTimeoutMs = Math.max(1000, Number(timeoutMs) || 12000);
...
export async function searchRagQueries({
  ...
  const settled = await runWithConcurrency(uniqueQueries, 3, (entry, index) =>
    searchRagDirect({
      ...
      conversationId,
      requestId: logicalRequestId,
      timeoutMs: multiQueryTimeoutMs === null ? 18000 : multiQueryTimeoutMs,
      fetchImpl,
      onTiming,
      profileConsentDb
    })
