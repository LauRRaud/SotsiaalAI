...
export function isContextDependentRetrievalTurn(message = "") {
  ...
  if (/\b(see|seda|sellest|selle|selles|sellel|seal|siin|sealt|sinna|sama|samast|eelmine|eelmise|jah|jep|okei|ok)\b/.test(normalized)) {
    return true;
  }
...
export function buildSourceAnchoredRagQueries(message = "", history = [], baseQuery = "") {
  ...
  const focused = useSourceAnchors ? extractRecentAssistantSourceFocus(history, 2)
    .map((item) => ({
