...
export function isPluralSourceSetFollowup(message = "") {
  ...
  const pluralReference = /\b(nende|neid|nendest|nendega|these|those|their|them)\b/.test(normalized) ||
    /(?:^|\s)(?:их|эти|этих)(?:\s|$)/u.test(normalized) ||
    /\bneed\s+(?:artikl|allik|dokument|materjal|pealkirj|nimi)/.test(normalized);
  const sourceSet = /\b(artikl|allik|dokument|materjal|pealkirj|nimed|works|articles|sources|documents|titles)\w*/.test(normalized) ||
    /(?:стат|источник|документ|назван)/u.test(normalized);
  return pluralReference && sourceSet;
}
