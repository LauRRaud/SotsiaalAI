...
async function searchRagDirect({
  ...
  const emitTiming = (extra = {}) => {
    ...
    try {
      onTiming({
        retrieval_total_ms: Date.now() - startedAt,
        retrieval_timeout_ms: effectiveTimeoutMs,
        aborted_stage: abortedStage,
        time_since_previous_rag_request_ms: sincePrevious,
        // Embeddingu ja retrieverite kestust ei saa kliendipoolelt eristada —
        // need toimuvad rag-service'i sees. Väljad on olemas, väärtus null,
        // kuni rag-service neid vastuses ei raporteeri.
