...
function isContextDependentRetrievalTurn(message = "") {
...
export async function searchRagQueries({
  ...
  const settled = await runWithConcurrency(
    uniqueQueries,
    Math.max(1, Math.min(6, Number(multiQueryConcurrency) || 3)),
    (entry, index) => searchRagDirect({
      query: entry.query,
