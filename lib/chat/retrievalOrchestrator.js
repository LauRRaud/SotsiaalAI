...
function capSupplementalQueryScore(match = {}, ceiling) {
  if (!Number.isFinite(ceiling)) return match;
  const capped = { ...match, multi_query_supplemental: true };
  const hybridScore = finiteRagNumber(capped.hybrid_score);
  const camelHybridScore = finiteRagNumber(capped.hybridScore);
  const distance = finiteRagNumber(capped.distance);
  if (hybridScore !== null) {
    capped.hybrid_score = Math.min(hybridScore, ceiling);
  }
  if (camelHybridScore !== null) {
    capped.hybridScore = Math.min(camelHybridScore, ceiling);
  }
  if (hybridScore === null && camelHybridScore === null && distance !== null) {
    capped.distance = Math.max(distance, 1 - ceiling);
  }
  return capped;
}
...
export async function searchRagQueries({
  ...
  const fulfilledByIndex = settled
    .map((item, index) => ({ item, index }))
    .filter(entry => entry.item.status === "fulfilled" && Array.isArray(entry.item.value));
  const primaryResultIndex = fulfilledByIndex[0]?.index ?? -1;
  const primaryResults = primaryResultIndex >= 0
    ? settled[primaryResultIndex].value
    : [];
  const primaryScores = primaryResults
    .map(comparableRagScore)
    .filter(Number.isFinite);
  const primaryFloor = primaryScores.length ? Math.min(...primaryScores) : null;
  const merged = [];
  let firstError = null;
  for (let queryIndex = 0; queryIndex < settled.length; queryIndex += 1) {
    ...
    if (item.status === "fulfilled") {
      ...
      for (let resultIndex = 0; resultIndex < values.length; resultIndex += 1) {
        const value = values[resultIndex];
        if (queryIndex === primaryResultIndex || !Number.isFinite(primaryFloor)) {
          merged.push({
            ...value,
            multi_query_index: queryIndex,
            multi_query_primary: queryIndex === primaryResultIndex
          });
          continue;
        }
        const ceiling = Math.max(0, primaryFloor - 0.0001 * (queryIndex + resultIndex + 1));
        merged.push({
          ...capSupplementalQueryScore(value, ceiling),
          multi_query_index: queryIndex,
  ...
  return dedupeRagMatches(merged, { preserveFirstScores: true });
}
