...
function isPureEvidenceLimitReply(reply = "") {
  const normalized = normalizeSemanticText(reply).trim();
  if (!normalized) return true;
  return /^(?:kasutatud materjalid ei kinnita|valitud allikad ei kinnita|ma ei leidnud kasutatud materjalidest|praegune allikakontekst ei kinnita|the (?:used|selected) (?:materials|sources) do not confirm|i could not find (?:this|that) in the (?:used|selected) (?:materials|sources)|использованные материалы не подтверждают|выбранные источники не подтверждают)/u.test(normalized);
}
