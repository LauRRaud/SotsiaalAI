...
function isEvidenceLimitSentence(value = "") {
  ...
  return /^(?:ma\s+)?(?:ei\s+(?:saa|suuda|oska)\b.*\b(?:kinnita\w*|vastata|oelda|kontrollida)|ei\s+(?:leia|leidnud)\b)/u.test(text) ||
    /^(?:(?:kasutatud|valitud|olemasolev\w*|praegun\w*|minu)\s+)?(?:allik\w*|materjal\w*|andm\w*|teab\w*|kontekst\w*|korpus\w*)\b.*\b(?:ei\s+(?:kinnita|sisalda|anna|voimalda|ole)|pole|puudub|puuduvad)/u.test(text) ||
...
function suppressSubsumedClaimSources(displayItems = [], decisions = []) {
  ...
  for (const item of Array.isArray(displayItems) ? displayItems : []) {
    ...
    if (!completeClaimIndexSet || !supportedClaimIndices.length || addsUncoveredClaim) {
      kept.push(item);
