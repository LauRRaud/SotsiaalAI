...
function sourceClaimSupport(claims = [], source = {}, allowBibliographicMetadata = false) {
  ...
  const rawSource = sourceText(frontMatter ? { ...source, evidenceText: "", text: "", chunk: "" } : source);
...
function buildClaimAttributionContract(reply = "", list = [], queryPlan = {}) {
  ...
  const sourceSupport = (Array.isArray(list) ? list : []).map((source, index) => ({
    ...
    ...sourceClaimSupport(claims, source, allowBibliographicMetadata)
  }));
