...
function sourceClaimSupport(claims = [], source = {}, allowBibliographicMetadata = false, requireJournalBody = false) {
  ...
  for (const claim of claims) {
    ...
    const numbersSupported = !claim.numbers.length || claim.numbers.every(number =>
      ...
      (journalBodyOnly && titleMention && /^(?:19|20)\d{2}$/u.test(number) && String(source.year || source.metadata?.year || "") === number)
    );
  ...
  return {
    ...
    // Keep the complete bounded claim set for coverage decisions. Only the
