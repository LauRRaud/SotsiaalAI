...
import { authorNamesCompatible, sourceDocumentId } from "./authorIdentity.js";
import { isJournalFrontMatter } from "./evidenceContent.js";
import { QUOTED_OBJECT_PATTERN, joinQuotedInitial } from "./sourceAttributionLanguage.js";
...
const ATTRIBUTION_DECISION_REASONS = Object.freeze({
  ...
  CLAIM_SUPPORT_SUBSUMED: "claim_support_subsumed",
...
function isEvidenceLimitSentence(value = "") {
  const text = normalizeSemanticText(value).trim();
  // A contrast may contain a real supported answer after the limitation.
  const contrasts = [...text.matchAll(/\b(?:kuid|aga|but|however|но)\s+/gu)];
  if (contrasts.some(match => !/^(?:ei\s+(?:sisalda|kinnita|anna|voimalda)|puudub|puuduvad|pole)\b/u.test(
    text.slice(Number(match.index) + match[0].length)
  ))) return false;
  return /^(?:[^.!?]{0,140}\s)?ei\s+(?:saa|suuda|oska)\b.*\b(?:kinnita\w*|vastata|oelda|kontrollida)/u.test(text) ||
    /^(?:ma\s+)?ei\s+(?:leia|leidnud)\b/u.test(text) ||
    /^(?:vastamiseks|tapseks\s+vastuseks|selle\s+kinnitamiseks)\s+(?:on\s+)?(?:vaja|vajalik)/u.test(text) ||
    /^(?:(?:kasutatud|valitud|olemasolev\w*|praegun\w*|siinsed|need|minu)\s+)?(?:allik\w*|materjal\w*|valjavot\w*|andm\w*|teab\w*|kontekst\w*|korpus\w*)\b.*\b(?:ei\s+(?:kinnita|sisalda|anna|voimalda|ole)|pole|puudub|puuduvad)/u.test(text) ||
    /^(?:mul|selle\s+kohta)\b.*\b(?:ei\s+ole|pole|puudub)\b.*\b(?:andm\w*|info\w*|allik\w*|teab\w*)/u.test(text) ||
    /^(?:i\s+(?:(?:can'?t|cannot|could\s+not)\s+(?:confirm|verify|find|answer)|do\s+not\s+have)|the\s+(?:used|selected|available)\s+(?:materials|sources)\s+do\s+not\s+(?:confirm|contain)|я\s+не\s+могу\s+(?:подтвердить|найти)|(?:использованные\s+материалы|выбранные\s+источники)\s+не\s+подтверждают)/u.test(text);
}

function isPureEvidenceLimitReply(reply = "") {
  if (!normalizeSemanticText(reply).trim()) return true;
  // A limitation is not a source-backed claim. Keep independently supported
  // substantive sentences when an answer is only partly limited.
  return String(reply).split(/\n+|(?<=[.!?])\s+/u).some(isEvidenceLimitSentence) &&
    replyClaims(reply).length === 0;
}
...
function tokenize(value = "") {
  ...
  return normalized
    ...
    .filter(token => !STOPWORDS.has(token) && !isQuestionDeterminer(token));
}

function isQuestionDeterminer(token = "") {
  // Inflections of "milline" express a question, not its evidence topic.
  // Counting "millised" + a generic verb as two distinctive words can
  // manufacture the only supposedly unique claim of a background source.
  return /^(?:milline|millist|millis(?:e|te)(?:d|sse|s|st|le|l|lt|ks|ni|na|ta|ga)?|milliseid)$/u.test(token);
}
...
function isProfessionalMethodGuidanceQueryPlan(queryPlan = {}) {
  const mode = String(queryPlan?.mode || queryPlan?.queryPlanMode || "").trim();
  const nestedMode = String(queryPlan?.question_planner?.mode || "").trim();
  return mode === "professional_method_guidance" ||
    ((!mode || mode === "default") && nestedMode === "professional_method_guidance");
}

function isSynthesisQueryPlan(queryPlan = {}) {
  // This route may retrieve several guides, but its answer is one method,
  // not a deliberate multi-source synthesis or source inventory.
  if (isProfessionalMethodGuidanceQueryPlan(queryPlan)) return false;
  const mode = String(queryPlan?.mode || queryPlan?.queryPlanMode || "").trim();
...
function sourceHasExactAuthor(source = {}, personName = "") {
  if (!String(personName || "").trim()) return false;
  return sourceAuthorValues(source).some(author => authorNamesCompatible(personName, author));
}

function sourceHasPersonReference(source = {}, personName = "") {
  const personTokens = normalizeText(personName).split(/\s+/u).filter(Boolean);
  if (personTokens.length < 2) return false;
  const sourceTokens = uniqueTokens(sourceText(source));
  const firstName = personTokens[0];
  const surname = personTokens.at(-1);
  const firstNamePresent = sourceTokens.some(token =>
    token === firstName || (Math.min(token.length, firstName.length) >= 4 && (
      token.startsWith(firstName) || firstName.startsWith(token)
    ))
  );
  return firstNamePresent && sourceTokens.some(token =>
    authorNamesCompatible(`${firstName} ${surname}`, `${firstName} ${token}`)
  );
}
...
function shouldApplySingleTopicClaimCover(query = "", options = {}) {
  const queryPlan = options?.queryPlan && typeof options.queryPlan === "object"
    ? options.queryPlan
    : {};
  const mode = String(queryPlan?.mode || queryPlan?.queryPlanMode || "").trim();
  const nestedMode = String(queryPlan?.question_planner?.mode || "").trim();
  const methodGuidance = isProfessionalMethodGuidanceQueryPlan(queryPlan);
  const deliberateMultiSource =
    (!methodGuidance && (
      queryPlan?.needs_multiple_sources === true ||
      queryPlan?.question_planner?.needs_multiple_sources === true
    )) ||
    queryPlan?.source_set_listing === true ||
    queryPlan?.sourceSetListing === true ||
    isSynthesisQueryPlan(queryPlan) ||
    isLifeSituationGuidanceQueryPlan(queryPlan) ||
    isComparisonQueryPlan(queryPlan) ||
    isPersonSourceLookupQueryPlan(queryPlan) ||
    isMunicipalityContactQueryPlan(queryPlan) ||
    isPluralSourceSetListingQuery(query);
  const protectedRoute =
    options?.factValidation?.passed === true ||
    options?.legalLookupPlan?.enabled === true ||
    options?.packageAwareAnsweringUsed === true ||
    (Array.isArray(options?.contactInventoryValidatedSourceIds) && options.contactInventoryValidatedSourceIds.length > 0) ||
    mode === "temporal" ||
    mode === "legal_exact" ||
    mode === "source_set_listing" ||
    mode === "source_listing" ||
    mode.startsWith("kov_") ||
    nestedMode.startsWith("kov_");
  return !deliberateMultiSource && !protectedRoute;
}
...
const NAMED_OBJECT_TYPE = /^(?:[\p{Letter}-]*(?:mudel|meetod|raamistik|tooriist|programm|teenus)[\p{Letter}-]*|(?:model|method|framework|tool|program|service)[\p{Letter}-]*)$/u;

function namedObjectTokens(value = "") {
  return normalizeSemanticText(value).split(/\s+/u).filter(Boolean);
}

function quotedObjectAnchors(value = "") {
  return Array.from(String(value).matchAll(QUOTED_OBJECT_PATTERN)).flatMap(match => {
    const before = namedObjectTokens(value.slice(0, match.index)).at(-1) || "";
    const after = namedObjectTokens(value.slice(match.index + match[0].length))[0] || "";
    // A copular description may put the type after modifiers/year, not next
    // to the name. Stay inside this clause and a bounded eight-token window.
    const description = value.slice(match.index + match[0].length).split(/[,;!?]/u)[0];
    const describedType = /^(?:on|oli|is|was)\s/u.test(normalizeSemanticText(description)) &&
      namedObjectTokens(description).slice(0, 8).some(token => NAMED_OBJECT_TYPE.test(token));
    if (!NAMED_OBJECT_TYPE.test(before) && !NAMED_OBJECT_TYPE.test(after) && !describedType) return [];
    const tokens = namedObjectTokens(joinQuotedInitial(match[1]));
    return tokens.length && tokens.length <= 10 ? [tokens] : [];
  });
}

function namedObjectEvidenceSegments(value = "") {
  return String(value)
    .replace(/^\s*\(\d+\)[^\r\n]*\r?\n/u, "")
    .replace(QUOTED_OBJECT_PATTERN, (_match, phrase) => joinQuotedInitial(phrase))
    .split(/\n---\n|[.!?;](?:\s|$)/u)
    .map(namedObjectTokens)
    .filter(tokens => tokens.length);
}

function hasNamedObjectAnchor(segments, anchor, language = {}) {
  return segments.some(tokens => tokens.some((_token, start) =>
    anchor.every((token, offset) => {
      const other = tokens[start + offset];
      if (tokensCompatible(token, other)) return true;
      // Whole ordered multiword identity only; no edit-distance guesses or
      // metadata-only alias can manufacture rendered body evidence.
      return anchor.length >= 2 && (language.lemmas?.[token] || []).some(lemma =>
        language.lemmas?.[other]?.includes(lemma));
    })
  ));
}

function attributionSentences(value = "") {
  return String(value).split(/\n+/u).flatMap(line => {
    const sentences = [];
    let start = 0;
    for (const boundary of line.matchAll(/(?<=[.!?])\s+/gu)) {
      const before = line.slice(Math.max(0, boundary.index - 6), boundary.index);
      const after = line.slice(boundary.index + boundary[0].length);
      // Estonian ordinal years belong to the following time phrase, whether
      // it names a publication year or an event year. Keep the number bound
      // to the assertion; treating it as a sentence loses that constraint.
      if (/(?:^|\D)\d{4}\.$/u.test(before) && /^aasta\p{Letter}*(?:\s|$)/u.test(after)) continue;
      sentences.push(line.slice(start, boundary.index));
...
function replyClaims(reply = "", language = {}) {
  const normalizedLines = attributionSentences(String(reply || "")
    .replace(/```[\s\S]*?```/gu, " ")
    .split(/\r?\n/u).filter(line => !language.headings?.includes(line.trim())).join("\n")
    // Strip actual line-leading list markers before sentence segmentation.
    // A four-digit ordinal may be a year, never silently discard it.
    .replace(/^\s*(?:[-*•]|(?!\d{4}\.\s)\d+[.)])\s+/gmu, ""));
  const claims = [];
  for (const line of normalizedLines) {
    if (isEvidenceLimitSentence(line)) continue;
    ...
    claims.push({
      ...
      tokens,
      named_object_anchors: quotedObjectAnchors(line),
...
const PUBLICATION_WORD = /^(?:(?:ajakirja|teadusajakirja)\s+)?(?:ajakirj|artikl|uuring|aruand|raport|kasitlus|teemakasitlus|valjaand|juhend|article|journal|study|report|publication)\p{Letter}*(?:\s|$)/u;

function sourceSupportsPublicationYear(claim, source, number) {
  if (!/^\d{4}$/u.test(number) || String(source.year || source.metadata?.year || "") !== number) return false;
  const occurrences = Array.from(claim.text.matchAll(/(?<![\p{Letter}\p{Number}])\d{4}(?![\p{Letter}\p{Number}])/gu))
    .filter(match => match[0] === number);
  const title = normalizeText(source.title || "");
  // Every occurrence must have a bibliographic role. Mentioning a report's
  // year once cannot license an unsupported event dated to that same year.
  return occurrences.length > 0 && occurrences.every(match => {
    const before = claim.text.slice(0, match.index);
    const after = claim.text.slice(match.index + number.length);
    const normalizedAfter = normalizeText(after);
    // "aasta uuringus" dates the publication; "aastal uuringus osalenud"
    // dates an event. Only an explicit publication verb licenses the latter.
    const locativeYear = /^aastal(?:\s|$)/u.test(normalizedAfter);
    const following = normalizedAfter.replace(/^aasta(?:l)?\s+/u, "");
    // Describing a named method in this publication is bibliographic too,
    // even without repeating the full article title. The caller has already
    // required its complete name in rendered body evidence; metadata alone
    // cannot license this exception or an implementation/outcome event.
    if (locativeYear && claim.named_object_anchors.length > 0 &&
      /^(?:kirjeldatud|kirjeldati|kasitletud|kasitleti)(?:\s|$)/u.test(following)) return true;
    if (locativeYear && !/^(?:ilmunud|avaldatud|avaldati|ilmus)(?:\s|$)/u.test(following)) return false;
    if (PUBLICATION_WORD.test(following.replace(/^(?:ilmunud|avaldatud)\s+/u, ""))) return true;
    if (title && claim.normalized.includes(title) &&
      /^(?:kirjeldatud|kirjeldati|kasitletud|kasitleti|avaldatud|avaldati|ilmunud|ilmus)(?:\s|$)/u.test(following)) return true;
    // A title-bound parenthesis is bibliographic; a title elsewhere in the
    // sentence is not permission to use metadata for an event's date.
    const citationStart = before.lastIndexOf("(");
    if (!title || citationStart < 0 || !/^\s*\)/u.test(after)) return false;
    const citationPrefix = before.slice(citationStart + 1);
    if (citationPrefix.includes(")")) return false;
    return normalizeText(citationPrefix).endsWith(title) ||
      (!citationPrefix.trim() && normalizeText(before.slice(0, citationStart)).endsWith(title));
  });
}

function sourceClaimSupport(claims = [], source = {}, allowBibliographicMetadata = false, requireJournalBody = false, language = {}) {
  const frontMatter = isJournalFrontMatter(source.evidenceText || source.text || source.chunk, source);
  if (frontMatter && !allowBibliographicMetadata) {
    return { supported: false, supported_claim_indices: [], supported_claim_count: 0, score: 0 };
  }
  const journalBodyOnly = requireJournalBody && !allowBibliographicMetadata &&
    /journal/iu.test(String(source.source_type || source.sourceType || source.metadata?.source_type || ""));
  const rawSource = journalBodyOnly
    ? String(source.evidenceText || source.text || source.chunk || "").replace(/^\s*\(\d+\)[^\r\n]*\r?\n/u, "")
    : sourceText(frontMatter ? { ...source, evidenceText: "", text: "", chunk: "" } : source);
  const normalizedSource = normalizeText(rawSource);
  const sourceTokens = uniqueTokens(normalizedSource);
  const sourceBigrams = bigrams(sourceTokens);
  const sourceSentenceTokens = attributionSentences(rawSource).map(uniqueTokens);
  const namedObjectSegments = requireJournalBody && claims.some(claim => claim.named_object_anchors.length)
    ? namedObjectEvidenceSegments(source.evidenceText || source.text || source.chunk || "")
    : [];
  const supportedClaimIndices = [];
  let score = 0;
  for (const claim of claims) {
    // A named method/service needs its own rendered-body anchor; generic
    // vocabulary in a broad guide cannot subsume that distinct source.
    if (requireJournalBody && !claim.named_object_anchors.every(anchor => hasNamedObjectAnchor(namedObjectSegments, anchor, language))) continue;
    if (frontMatter) {
      const title = normalizeText(source.title || "");
      if (!title || !claim.normalized.includes(title)) continue;
      let residual = claim.normalized.split(title).join(" ");
      for (const author of sourceAuthorValues(source)) residual = residual.split(normalizeText(author)).join(" ");
      residual = residual.replace(/\b(?:19|20)\d{2}\b/gu, " ");
      if (uniqueTokens(residual).some(token => !/^(?:kirjutas|kirjutanud|autor|autori|artikkel|artikli|artiklid|teos|teose|ilmus|avaldas|avaldatud|wrote|written|author|article|published)$/u.test(token))) continue;
    }
    ...
    const numbersSupported = !claim.numbers.length || claim.numbers.every(number =>
      ...
      (journalBodyOnly && sourceSupportsPublicationYear(claim, source, number))
    );
    const coverage = common.length / Math.max(1, claim.tokens.length);
    // The weakest overlap route must have a local evidence relation. Two
    // long but generic words in unrelated sentences do not establish a
    // claim. Stronger multi-sentence synthesis and phrase matches remain.
    const weakOverlapHasLocalSupport = distinctive.length !== 2 || matchingBigrams.length > 0 ||
      sourceSentenceTokens.some(tokens => distinctive.every(token => tokenListHasCompatible(tokens, token)));
    const supported = numbersSupported && (
      (!journalBodyOnly && titleMention) ||
      (distinctive.length >= 2 && coverage >= 0.18 && weakOverlapHasLocalSupport) ||
  ...
  return {
    ...
    // Keep the complete claim set for coverage decisions. Only the
    // serialized decision trace may truncate it; a trace cap is not evidence.
    supported_claim_indices: supportedClaimIndices,
...
function buildClaimAttributionContract(reply = "", list = [], queryPlan = {}, language = {}) {
  const claims = replyClaims(reply, language);
  const allowBibliographicMetadata = isPersonSourceLookupQueryPlan(queryPlan) &&
    ["authored_works", "authored_works_count"].includes(queryPlan?.question_planner?.person_source_intent || queryPlan?.person_source_intent);
  const requireJournalBody = [queryPlan?.mode, queryPlan?.question_planner?.mode].some(mode =>
    ["overview_synthesis", "thematic_synthesis", "professional_method_guidance"].includes(mode));
  const sourceSupport = (Array.isArray(list) ? list : []).map((source, index) => ({
    source_id: getSourceAttributionId(source, index),
    ...sourceClaimSupport(claims, source, allowBibliographicMetadata, requireJournalBody, language)
  }));
...
function suppressSubsumedClaimSources(displayItems = [], decisions = [], explicitlyReferencedIndexes = new Set()) {
  const coveredClaimIndices = new Set();
  const decisionsByIndex = new Map(
    (Array.isArray(decisions) ? decisions : []).map(decision => [decision.source_index, decision])
  );
  const kept = [];

  for (const item of Array.isArray(displayItems) ? displayItems : []) {
    const supportedClaimIndices = Array.from(new Set(
      (Array.isArray(item?.supportedClaimIndices) ? item.supportedClaimIndices : [])
        .map(value => Number(value))
        .filter(Number.isInteger)
    ));
    const supportedClaimCount = Number(item?.supportedClaimCount);
    const completeClaimIndexSet = Number.isInteger(supportedClaimCount) &&
      supportedClaimCount === supportedClaimIndices.length;
    const addsUncoveredClaim = supportedClaimIndices.some(index => !coveredClaimIndices.has(index));

    // Incomplete external support remains fail-open for display, but internal
    // coverage uses all reply claims, independently of trace serialization.
    if (explicitlyReferencedIndexes.has(item.index) || !completeClaimIndexSet || !supportedClaimIndices.length || addsUncoveredClaim) {
      kept.push(item);
      // A navigation-only registry cannot subsume substantive evidence and
      // then leave the final registry-only boundary with no answer source.
      if (evidenceRoleFor(item.source) !== "registry_reference") {
        for (const index of supportedClaimIndices) coveredClaimIndices.add(index);
      }
      continue;
    }

    const decision = decisionsByIndex.get(item.index);
    if (!decision || decision.decision !== "display") {
      kept.push(item);
      continue;
    }
    decision.decision = "hide";
    decision.reason = ATTRIBUTION_DECISION_REASONS.CLAIM_SUPPORT_SUBSUMED;
  }
...
function validatedSupportingSourceIds(options = {}) {
  const validation = options?.factValidation && typeof options.factValidation === "object"
    ...
    : null;
  if (options.validatedGroupPublication === true && hasValidatedPublication(validation)) {
    return uniqueAttributionIds([validation.supporting_source_id, ...(validation.supporting_source_ids || [])]);
  }
  if (validation?.response_decision || validation?.passed !== true) return [];
...
function identifiedPublicationFactEvidence(source = {}, index = 0, options = {}) {
  ...
  const validatedSourceBoundedFact =
    ...
    (factValidation?.passed === true || options.validatedGroupPublication === true) &&
    !!supportingSourceId &&
...
export function buildSourceAttribution(reply = "", sources = [], options = {}) {
  options = { ...options, validatedGroupPublication: hasValidatedPublication(options?.factValidation, reply) };
  const list = Array.isArray(sources) ? sources : [];
  const query = String(options?.query || "");
  const claimAttributionContract = buildClaimAttributionContract(reply, list, options?.queryPlan, options?.attributionLanguage);
  ...
  if (
    String(options?.nonAnswerSourceSuppressionReason || "").trim() ||
    isPureEvidenceLimitReply(reply)
  ) {
  ...
  if ((options?.factValidation?.passed === false || options?.factValidation?.response_decision) && !options.validatedGroupPublication) {
    const decisions = list.map((source, index) =>
  ...
  if (hasValidatedPublication(options?.factValidation, reply)) {
    const displayItems = [];
    const decisions = list.map((source, index) => {
      const evidence = identifiedPublicationFactEvidence(source, index, options);
      if (!evidence) return buildDecision(source, index, "hide", ATTRIBUTION_DECISION_REASONS.QUERY_ANCHOR_MISMATCH, 0);
      displayItems.push({ source, index, score: 10 });
      return buildDecision(source, index, "display", ATTRIBUTION_DECISION_REASONS.FACT_CONTRACT_VALIDATED, 10, evidence);
    });
  ...
  const validatorSupportedSourceIds = new Set(validatedSupportingSourceIds(options));
  if (validatorSupportedSourceIds.size && options?.queryPlan?.mode !== "temporal") {
    const validatorDocumentIdentity = options?.documentIdentityEvidence &&
      typeof options.documentIdentityEvidence === "object"
      ? options.documentIdentityEvidence
      : {};
    const validatorLockedDocumentId =
      options?.queryPlan?.mode === "specific_research_fact" &&
      validatorDocumentIdentity?.matched === true &&
      validatorDocumentIdentity?.confidence === "high"
        ? String(validatorDocumentIdentity?.selectedDocumentId || "").trim()
        : "";
    const displayItems = [];
    const decisions = list.map((source, index) => {
      const sourceId = getSourceAttributionId(source, index);
      if (
        !validatorSupportedSourceIds.has(sourceId) ||
        (validatorLockedDocumentId && sourceDocumentId(source) !== validatorLockedDocumentId)
      ) {
        return buildDecision(source, index, "hide", ATTRIBUTION_DECISION_REASONS.QUERY_ANCHOR_MISMATCH, 0);
      }
      const score = Math.max(10, scoreSourceForReply(reply, source));
      displayItems.push({ source, index, score });
      return buildDecision(source, index, "display", ATTRIBUTION_DECISION_REASONS.FACT_CONTRACT_VALIDATED, score, {
  ...
  if (legalLookupPlan?.enabled) {
    ...
    const compositeKovQuestion = options?.queryPlan?.question_planner?.mode === "kov_service_and_local_rule";
    const displayItems = [];
    const decisions = list.map((source, index) => {
      const sourceType = String(source?.sourceType || source?.source_type || source?.type || "").toLowerCase();
      const compositeServiceSource = compositeKovQuestion && [
        "kov_service", "kov_service_info", "municipality_service"
      ].includes(sourceType);
      const contractReason = compositeServiceSource
        ? null
        : sourceMatchesLegalContract(source, legalLookupPlan, allowedParagraphRefs);
      if (contractReason) {
  ...
  if (isPersonSourceLookupQueryPlan(options?.queryPlan)) {
    ...
    const authoredWorksRequest = ["authored_works", "authored_works_count"].includes(
      options?.queryPlan?.question_planner?.person_source_intent || options?.queryPlan?.person_source_intent
    );
    const riskPolicy = options?.riskPolicy || null;
    const authorInventoryDocumentIds = new Set(
      (Array.isArray(options?.authorCorpusEvidence?.documentIds)
        ? options.authorCorpusEvidence.documentIds
        : [])
        .map(value => String(value || "").trim())
        .filter(Boolean)
    );
    const displayItems = [];
    const decisions = list.map((source, index) => {
      const inventoryAuthorMatch = authorInventoryDocumentIds.has(sourceDocumentId(source));
      if (
        !inventoryAuthorMatch &&
        !sourceHasExactAuthor(source, personName) &&
        (authoredWorksRequest || !sourceHasPersonReference(source, personName))
      ) {
        return buildDecision(source, index, "hide", ATTRIBUTION_DECISION_REASONS.QUERY_ANCHOR_MISMATCH, 0);
      }
  ...
  if (isSynthesisQueryPlan(options?.queryPlan)) {
    ...
    displayItems.sort((a, b) => b.score - a.score || a.index - b.index);
    // A narrated synthesis can name its actual sources while weak lexical
    // overlap adds unrelated articles to its introduction. Keep every named
    // source and every additional claim, not redundant unnamed background.
    const normalizedReply = normalizeText(reply);
    const explicitlyReferencedIndexes = new Set(displayItems.filter(item => {
      const title = normalizeText(item.source?.title || "");
      return title.length >= 12 && normalizedReply.includes(title);
    }).map(item => item.index));
    const useNarrativeClaimCover = !resourceDiscoveryPlan && !lifeSituationPlan && !comparisonPlan &&
      !isPluralSourceSetListingQuery(query) && explicitlyReferencedIndexes.size > 0;
    const finalItems = useNarrativeClaimCover
      ? suppressSubsumedClaimSources(displayItems.map(item => {
          const support = claimSupportFor(item.source, item.index);
          return { ...item, supportedClaimIndices: support.supported_claim_indices, supportedClaimCount: support.supported_claim_count };
        }), decisions, explicitlyReferencedIndexes)
      : displayItems;
    return finishAttribution(decisions, finalItems);
  }
  if (list.length === 1) {
  ...
  const decisions = list.map((source, index) => {
    ...
    displayItems.push({
      ...
      index,
      score,
      supportedClaimIndices: claimSupport.supported_claim_indices,
      supportedClaimCount: claimSupport.supported_claim_count
    });
    return buildDecision(source, index, "display", ATTRIBUTION_DECISION_REASONS.CLAIM_SUPPORT_VALIDATED, score, {
      ...evidence,
      ...claimSupport
  ...
  displayItems.sort((a, b) => b.score - a.score || a.index - b.index);
  const finalDisplayItems = shouldApplySingleTopicClaimCover(query, options)
    ? suppressSubsumedClaimSources(displayItems, decisions)
    : displayItems;
  return finishAttribution(decisions, finalDisplayItems);
}
