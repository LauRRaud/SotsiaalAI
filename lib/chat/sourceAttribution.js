...
const QUOTED_OBJECT_PATTERN = /[„“«"]([^„“”«»"\n]{1,120})[“”»"]/gu;
const NAMED_OBJECT_TYPE = /^(?:mudel|meetod|raamistik|tooriist|programm|teenus|model|method|framework|tool|program|service)[\p{Letter}-]*$/u;

function namedObjectTokens(value = "") {
  return normalizeSemanticText(value).split(/\s+/u).filter(Boolean);
}

function joinQuotedInitial(value = "") {
  // Compare PDF drop capitals without changing stored or rendered evidence.
...
function quotedObjectAnchors(value = "") {
  return Array.from(String(value).matchAll(QUOTED_OBJECT_PATTERN)).flatMap(match => {
    ...
    if (!NAMED_OBJECT_TYPE.test(before) && !NAMED_OBJECT_TYPE.test(after)) return [];
...
function hasNamedObjectAnchor(segments, anchor) {
  return segments.some(tokens => tokens.some((_token, start) =>
    anchor.every((token, offset) => tokensCompatible(token, tokens[start + offset]))
  ));
}
...
function sourceClaimSupport(claims = [], source = {}, allowBibliographicMetadata = false, requireJournalBody = false) {
  ...
  for (const claim of claims) {
    ...
    if (requireJournalBody && !claim.named_object_anchors.every(anchor => hasNamedObjectAnchor(namedObjectSegments, anchor))) continue;
    if (frontMatter) {
      const title = normalizeText(source.title || "");
...
function buildClaimAttributionContract(reply = "", list = [], queryPlan = {}) {
  ...
  const sourceSupport = (Array.isArray(list) ? list : []).map((source, index) => ({
    ...
    ...sourceClaimSupport(claims, source, allowBibliographicMetadata, requireJournalBody)
  }));
...
export function buildSourceAttribution(reply = "", sources = [], options = {}) {
  ...
  const claimAttributionContract = buildClaimAttributionContract(reply, list, options?.queryPlan);
