...
// Kriisituvastus on determinstlik fail-safe (VEST-P0/P0a). ET põhikomplekt jääb muutmata;
...
export function detectCrisis(text = "") {
  ...
  const hits = [
    ...
    /appi!?(\s+appi!?)*$/,
    // --- Eesti laiendus (24.09): käändevormid ja sõnastused, mida ülemine komplekt ei tabanud
    // (nt „enesetapumõtted" ühe p-ga, „tahan end tappa", „lõpetada oma elu"). Liitsõna osa ei ole
    // tabamus: „elukindlustus", „tööelu", „endale leiba" jäävad välja (piirid (?:^|[^…]) ja (?!…); lookbehind'i ei kasutata, sest vanem Safari ei parsi seda). ---
    /enesetap|suitsii?d/,
    /(?:^|[^a-zõäöüšž])(end|ennast|enda) ([äa]ra )?tappa|taha(n|ks|ksin) ([äa]ra )?surra/,
    /(?:^|[^a-zõäöüšž])elu ([äa]ra )?l[õo]petada|l[õo]petada (oma )?elu(?![a-zõäöüšž])|(endale|enesele) l[õo]pp(?![a-zõäöüšž])/,
    /vigasta(n|sin|ma|mas) (end|ennast)(?![a-zõäöüšž])|(?:^|[^a-zõäöüšž])(end|ennast) vigasta(n|sin|ma|mas)|l[õo]i(kun|gun|kasin) (end|ennast)(?![a-zõäöüšž])/,
