...
function researchIdentityRoots(value = "") {
  ...
  for (const suffix of [
    ...
    "mist", "mise", "mine", "usega", "usest", "uses", "used", "ust", "use", "sid", "s"
  ]) {
...
function researchIdentityTermMatches(term = "", candidateText = "") {
  const candidateTokens = normalizeIntentText(candidateText).match(/[a-z0-9]+/gu) || [];
  const termRoots = researchIdentityRoots(term);
  return candidateTokens.some(candidateToken => {
    const candidateRoots = researchIdentityRoots(candidateToken);
    return termRoots.some(termRoot => candidateRoots.some(candidateRoot => {
      ...
      return (
        ...
        (candidateRoot.length >= 5 && String(term).length >= 8 && normalizeIntentText(term).includes(candidateRoot))
      );
    }));
  });
}
...
export function selectSpecificResearchFactGroups(_message = "", groups = [], questionPlan = {}) {
  ...
  const ranked = candidates.map((group, index) => {
    ...
    const subjectMatches = subjectTerms.filter(term => researchIdentityTermMatches(term, identityText));
    const titleSubjectMatches = subjectTerms.filter(term =>
      researchIdentityTermMatches(term, String(group?.title || ""))
    );
    ...
    const authorMatched = !!requestedAuthor && authors.some(author => researchPersonMatchesAuthor(requestedAuthor, author));
    ...
    const identityMatched = sourceCompatible && (requestedAuthor
      ? authorMatched && subjectMatches.length >= Math.min(1, subjectTerms.length)
      : subjectTerms.length >= 2 && subjectMatches.length >= 2) || !!(
        sourceCompatible && implicitNumericIdentity && subjectMatches.length >= 1 && numericFactMatches.length >= 2
      );
  ...
  const trustedDocumentMatch = !!(
    trustedDocumentId && best?.documentId === trustedDocumentId && best.sourceCompatible
  );
  ...
  if (best && !best.identityMatched && (decisiveSubjectLead || decisiveStrongAnchorLead || decisiveRankedTitleLead)) {
...
async function loadServiceMapKovContactsForMunicipalities(municipalities = []) {
  ...
  const entries = await prisma.serviceMapEntry.findMany({
    where: {
      ...
      sourceUrl: { not: null },
      AND: [
        verificationProjection.whereIdentity,
        { OR: or }
      ]
    },
    orderBy: [
  ...
  return entries.map(entry => ({
    ...entry,
    contactVerifiedAt: verificationProjection.verifiedAtById[entry.id] || null
  }));
}
...
function serviceMapKovContactSummary(decorated = []) {
  ...
  return [
    "KOV_CONTACT_TOTAL (varskuskontrolli labinud avalike kontaktikirjete arv, mitte kogu personali arv):",
...
function supportsDeterministicContactReply(message = "") {
  ...
  return Object.values(intent).some(Boolean) ||
    /(?:^|[^\p{L}\p{N}])(?:kontaktid|contacts|контакты)(?=$|[^\p{L}\p{N}])/u.test(normalizeIntentText(message));
}
...
function hasIndependentContactSourceCue(message = "") {
  ...
  if (/(?:^|[^\p{L}\p{N}])(?:varem|ajalool\p{L}*|tootas\p{L}*|kirjuta\p{L}*|artikkel\p{L}*|uuring\p{L}*|aruanne\p{L}*|autor\p{L}*|allikas\p{L}*|dokument\p{L}*|fail\p{L}*|pdf|formerly|historical\p{L}*|worked|wrote|writes|article\p{L}*|research\p{L}*|study|report\p{L}*|author\p{L}*|source\p{L}*|document\p{L}*|истор\p{L}*|раньше|работал\p{L}*|статья\p{L}*|исследован\p{L}*|отчет\p{L}*|автор\p{L}*|источник\p{L}*|документ\p{L}*)(?=$|[^\p{L}\p{N}])/u.test(normalized)) return true;
