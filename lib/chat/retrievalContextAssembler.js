...
export function selectPersonSourceGroups(
  ...
  for (const group of candidates) {
    const normalizedAuthors = (Array.isArray(group?.authors) ? group.authors : [])
      .map(normalizePersonName)
      .filter(Boolean);
    const exactAuthor = normalizedAuthors.includes(target)
      && requiredCoauthors.every(coauthor => normalizedAuthors.includes(coauthor))
...
function buildAuthorCorpusEvidence(matches = [], questionPlan = {}) {
  ...
  const summaries = (Array.isArray(matches) ? matches : [])
    ...
    .filter(summary => normalizePersonName(summary.requested_author) === target)
    .map(summary => ({
...
function researchPersonMatchesAuthor(personName = "", author = "") {
  const requested = normalizeIntentText(personName).split(/\s+/u).filter(Boolean);
  const candidate = normalizeIntentText(author).split(/\s+/u).filter(Boolean);
  if (requested.length < 2 || candidate.length < 2 || requested[0] !== candidate[0]) return false;
  const requestedSurname = requested[requested.length - 1];
  const candidateSurname = candidate[candidate.length - 1];
  let common = 0;
  while (
    common < requestedSurname.length &&
    common < candidateSurname.length &&
    requestedSurname[common] === candidateSurname[common]
  ) common += 1;
  return common >= Math.max(3, Math.min(requestedSurname.length, candidateSurname.length) - 1) &&
    Math.abs(requestedSurname.length - candidateSurname.length) <= 2;
}
...
export function selectSpecificResearchFactGroups(_message = "", groups = [], questionPlan = {}) {
  ...
  const explicitDocumentAnchorTerms = acronymAnchorTerms.length
    ? Array.from(new Set([
        ...acronymAnchorTerms,
    ...
    : [];
  const requestedAuthor = String(questionPlan?.person_name || "").trim();
  const numericFactTerms = (Array.isArray(questionPlan?.document_fact_terms)
    ? questionPlan.document_fact_terms
    : []).map(value => String(value || "").trim()).filter(value => /^\d+(?:[.,]\d+)?%?$/u.test(value));
  const implicitNumericIdentity = questionPlan?.planner_reason === "implicit_multi_cohort_numeric_fact_shape";
  const ranked = candidates.map((group, index) => {
    ...
    const authorMatched = !!requestedAuthor && authors.some(author =>
      researchPersonMatchesAuthor(requestedAuthor, author)
    );
    ...
    const sourceYearMatches = boundedEpisodeIdentity
      ? requestedSourceYears.filter(year => (
    ...
    score += implicitNumericIdentity ? numericFactMatches.length * 2 : 0;
    ...
    const identityMatched = sourceCompatible && (boundedEpisodeIdentity
      ? boundedEpisodeCoLocated && (!requestedAuthor || authorMatched)
      : requestedAuthor ? authoredIdentityMatched : anonymousIdentityMatched);
    const strongAnchorCandidate = sourceCompatible && !requestedAuthor &&
      subjectMatches.length >= 1 &&
    ...
    (requestedAuthor ? Number(right.authorMatched) - Number(left.authorMatched) : 0) ||
    ...
    (implicitNumericIdentity ? right.numericFactMatches.length - left.numericFactMatches.length : 0) ||
  ...
  if (
    trustedIndex > 0 &&
    (!requestedAuthor || ranked[trustedIndex]?.authorMatched) &&
  ...
  const trustedDocumentMatch = !!(
    ...
    (!requestedAuthor || best.authorMatched) &&
    (!boundedEpisodeIdentity || best.boundedEpisodeCoLocated)
  );
  if (trustedDocumentMatch) best.identityMatched = true;
  const runnerUp = ranked[1] || null;
  const decisiveSubjectLead = !!(
    best?.titleSubjectMatches.length &&
    (!runnerUp || best.subjectMatches.length > runnerUp.subjectMatches.length)
  );
  ...
  const decisiveDocumentAnchorLead = !!(
    ...
    best.documentAnchorMatches.length >= 2 &&
  ...
  if (
    ...
    !requestedAuthor &&
    !boundedEpisodeIdentity &&
    (decisiveDocumentAnchorLead || decisiveSubjectLead || decisiveStrongAnchorLead || decisiveRankedTitleLead)
  ) {
  ...
  const ambiguous = !trustedDocumentMatch && !!(
  ...
  return {
    ...
    reasons: best ? [
      ...(best.authorMatched ? ["author_match"] : []),
      ...
      ...(trustedDocumentMatch ? ["previous_source_exact_filter"] : []),
      ...(decisiveDocumentAnchorLead ? ["decisive_document_anchor_lead"] : []),
      ...(decisiveSubjectLead ? ["decisive_subject_lead"] : []),
...
const REQUESTED_METRIC_CONTRACT_VERSION = "requested_metric_contract_v1";
const REQUESTED_METRIC_CONTRACT_MAPPING_METHOD = "bounded_inherited_scope_peer_alignment_v4";
const REQUESTED_METRIC_CONTRACT_MAX_SLOTS = 6;
const REQUESTED_METRIC_CONTRACT_MAX_EVIDENCE_CANDIDATES = 512;
const REQUESTED_METRIC_CONTRACT_MAX_FRAGMENT_SPAN = 8;
const REQUESTED_METRIC_SUPPORTED_VALUE_TYPES = new Set(["proportion"]);
...
function requestedMetricTermMatchQuality(left = "", right = "") {
  const a = requestedMetricToken(left);
  const b = requestedMetricToken(right);
  if (!a || !b) return 0;
  if (a === b) return 1;
...
function requestedMetricEvidenceTokens(value = "") {
  return Array.from(String(value || "").matchAll(/[\p{L}\p{N}-]+/gu), match => ({
    value: requestedMetricToken(match[0]),
...
function requestedMetricEvidenceFragments(value = "") {
  return String(value || "")
    .split(/(?<=[.!?])\s+|[\r\n]+|�+/u)
    .map(fragment => fragment.replace(/\s+/gu, " ").trim())
    .filter(Boolean);
}
...
function requestedMetricMentionIsQualified(fragment = "", start = 0, end = 0) {
  ...
  const qualifiedPrefix = /(?:^|\s)(?:alla|ule|vahemalt|kuni|ligi|umbes|ligikaudu|keskmiselt|peaaegu|ca\.?|u\.?|enam\s+kui|rohkem\s+kui|vahem\s+kui|approximately|about|around|nearly|over|under|at\s+least|at\s+most|up\s+to|more\s+than|less\s+than|около|примерно|более|менее|не\s+менее|не\s+более|до|свыше)\s*$/u.test(prefix) ||
    /(?:[~≈<>≤≥]|\d{1,3}(?:[.,]\d+)?\s*%?\s*(?:[–—-]|kuni|to))\s*$/u.test(prefix);
  const qualifiedSuffix = /^\s*(?:[–—-]\s*\d{1,3}(?:[.,]\d+)?\s*%?|(?:voi|or|или)\s+(?:vahem|rohkem|less|more|меньше|больше))/u.test(suffix);
  return qualifiedPrefix || qualifiedSuffix;
}

function requestedMetricEvidenceCandidates(renderedText = "", valueType = "proportion") {
  ...
  fragments.forEach((fragment, fragmentIndex) => {
    const tokens = requestedMetricEvidenceTokens(fragment);
    const mentions = Array.from(fragment.matchAll(/(?<![\p{L}\p{N}])(\d{1,3}(?:[.,]\d+)?)\s*%(?![\p{L}\p{N}])/gu));
    mentions.forEach((mention, mentionIndex) => {
      const start = Number(mention.index);
      const end = start + String(mention[0] || "").length;
      if (requestedMetricMentionIsQualified(fragment, start, end)) return;
...
function requestedMetricCandidateScore(slot = {}, candidate = {}, inheritedScopeTerm = "") {
  const relationTerms = Array.isArray(slot?.relation_terms)
    ? slot.relation_terms.map(requestedMetricToken).filter(Boolean).slice(0, 8)
    : [];
  ...
  for (const relationTerm of relationTerms) {
    ...
    for (const token of candidate.tokens) {
      const quality = requestedMetricTermMatchQuality(relationTerm, token.value);
      if (!quality) continue;
  ...
  const minimumMatches = Math.max(2, Math.ceil(relationTerms.length * 0.45));
  if (matchedTermCount < minimumMatches || nearTermCount < 1) return null;
  const immediateFollowingToken = candidate.tokens.find(token =>
    token.start >= candidate.end && token.start - candidate.end <= 40
  ) || null;
  const localRelationHeadMatched = !!immediateFollowingToken && relationTerms.some(relationTerm =>
    requestedMetricTermMatchQuality(relationTerm, immediateFollowingToken.value) >= 0.72
  );
  const normalizedInheritedScopeTerm = requestedMetricToken(inheritedScopeTerm);
  const inheritedScopeHeadMatched = !!normalizedInheritedScopeTerm && !!immediateFollowingToken &&
    requestedMetricTermMatchQuality(normalizedInheritedScopeTerm, immediateFollowingToken.value) >= 0.86;
  if (
    slot?.derivation === "coordinated_shared_head" &&
    normalizedInheritedScopeTerm &&
    !inheritedScopeHeadMatched
  ) return null;
  if (localRelationHeadMatched) score += 2;
  if (inheritedScopeHeadMatched) score += 2;
  return {
    candidate,
    score,
    matchedTermCount,
...
function bestRequestedMetricAssignment(slots = [], candidates = []) {
  const emptyResult = { assignment: null, ambiguous: false, diagnostics: [] };
  if (!slots.length || !candidates.length) return emptyResult;
  const inheritedScopeTerm = requestedMetricToken(slots[0]?.relation_terms?.[0] || "");
  const scoredOptionsBySlot = slots.map(slot => candidates
    .map(candidate => requestedMetricCandidateScore(slot, candidate, inheritedScopeTerm))
    .filter(Boolean)
  ...
  const diagnostics = scoredOptionsBySlot.map((options, index) => ({
    ...
    top_candidates: options.slice(0, 3).map(option => ({
      ...
      inherited_scope_head_matched: option.inheritedScopeHeadMatched
    }))
  }));
  const optionsBySlot = scoredOptionsBySlot.map(options => options.slice(0, 6));
  if (optionsBySlot.some(options => !options.length)) {
    return { assignment: null, ambiguous: false, diagnostics };
  }
  const solutions = [];
  const visit = (slotIndex, usedCandidateKeys, assignment, totalScore) => {
    if (slotIndex >= slots.length) {
      const firstFragmentIndex = assignment[0]?.candidate?.fragmentIndex;
      const lastFragmentIndex = assignment.at(-1)?.candidate?.fragmentIndex;
      const fragmentSpan = Number.isInteger(firstFragmentIndex) && Number.isInteger(lastFragmentIndex)
        ? lastFragmentIndex - firstFragmentIndex
        : Number.POSITIVE_INFINITY;
      if (fragmentSpan > REQUESTED_METRIC_CONTRACT_MAX_FRAGMENT_SPAN) return;
      solutions.push({
    ...
    for (const option of optionsBySlot[slotIndex]) {
      ...
      if (!requestedMetricCandidateComesAfter(option.candidate, previousCandidate)) continue;
      ...
      if (coordinationGroup) {
        ...
        if (
          peerCandidate &&
          (
            option.candidate.fragmentIndex !== peerCandidate.fragmentIndex ||
  ...
  visit(0, new Set(), [], 0);
  if (!solutions.length) return { assignment: null, ambiguous: false, diagnostics };
  solutions.sort((left, right) => right.rankingScore - left.rankingScore);
  const best = solutions[0];
  const second = solutions[1];
  const assignmentsConflict = second && best.assignment.some((option, index) =>
    option.candidate.value !== second.assignment[index]?.candidate?.value
  );
  const ambiguous = !!(assignmentsConflict && best.rankingScore - second.rankingScore < 0.9);
  return { assignment: ambiguous ? null : best.assignment, ambiguous, diagnostics };
}
...
function requestedMetricContractInstruction(trace = null, replyLang = "et") {
  ...
  const slotLines = trace.slots.map(slot =>
    `slot_${slot.slot_index}: ${slot.value_type}=${slot.evidence_value}%`
  );
  if (replyLang === "en") {
    return [
      "REQUESTED_METRIC_CONTRACT_V1:",
...
function buildRequestedMetricContract({
  questionPlan = null,
  renderedGroups = [],
  replyLang = "et",
  ...
  if (
    ...
    Number(requested?.unresolved_clause_count || 0) !== 0 ||
    Number(requested?.emitted_slot_count) !== requestedSlots.length
  ) {
  ...
  if (replyLang !== "et" || requestedSlots.some(slot => slot?.input_form !== "original")) {
  ...
  for (const group of (Array.isArray(renderedGroups) ? renderedGroups : []).slice(0, 8)) {
    ...
    const renderedText = String(group?.bodies?.[0] || "");
    if (!sourceId || !documentId || !renderedText) continue;
    if (selectedDocumentId && documentId !== selectedDocumentId) continue;
    const evidenceCandidates = requestedMetricEvidenceCandidates(renderedText, "proportion");
    const candidates = evidenceCandidates.candidates;
    const { assignment, ambiguous, diagnostics } = bestRequestedMetricAssignment(requestedSlots, candidates);
...
function buildUniformParticipantRelationContract(message = "", groups = [], replyLang = "et") {
  ...
  for (const group of (Array.isArray(groups) ? groups : []).slice(0, 1)) {
    for (const body of Array.isArray(group?.bodies) ? group.bodies : []) {
      ...
      if (!relation.perGroupValue || !relation.totalValue) continue;
      ...
      const instruction = replyLang === "en"
        ? [
            ...relationLines,
            "These values come from one explicit uniform-group relation in the selected source. Answer the participant-group sizes and the same-sample people total only. When the question names participant groups, state only those requested groups; when it asks generally for all groups without naming them, state every group governed by this same evidence relation. State each included group exactly once with its group size and state the total exactly once; do not repeat the per-group number in an additional generic sentence. Preserve the source-based group or role name needed to identify each group, but omit unrequested duties, activities and descriptive relative clauses that are not needed to distinguish it. The supporting evidence can contain organization, jurisdiction or location coverage counts; those counts are outside this answer scope and must be omitted unless the question independently asks for coverage. If coverage is requested, state it separately with its own unit."
          ].join("\n")
        : replyLang === "ru"
          ? [
              ...relationLines,
              "Эти значения получены из одной явной связи одинакового размера групп в выбранном источнике. Отвечай только размерами групп участников и общим числом людей в той же выборке. Если в вопросе названы группы участников, укажи только эти группы; если вопрос просит назвать все группы, не перечисляя их, укажи все группы, относящиеся к этой же связи в доказательстве. Каждую включённую группу укажи ровно один раз с размером группы, а общий итог — ровно один раз; не повторяй число на группу в отдельной общей фразе. Сохраняй название группы или роли из источника, необходимое для её идентификации, но опускай не запрошенные обязанности, действия и описательные придаточные, которые не нужны для различения группы. В подтверждающем фрагменте могут быть числа организаций, административных единиц или мест; они не входят в этот ответ и должны быть опущены, если вопрос отдельно не запрашивает охват. Запрошенный охват указывай отдельно со своей единицей."
            ].join("\n")
          : [
              ...relationLines,
              "Need väärtused tulevad valitud põhiallika ühest selgest ühtlase rühmasuuruse seosest. Vasta ainult osalejarühmade suuruste ja sama valimi inimeste koguarvuga. Kui küsimus nimetab osalejarühmad, esita ainult need küsitud rühmad; kui küsimus küsib kõiki rühmi neid ette nimetamata, esita kõik sama tõendiseosega hõlmatud rühmad. Esita iga kaasatud rühm täpselt ühe korra koos rühma suurusega ning koguarv täpselt ühe korra; ära korda rühma kohta käivat arvu eraldi üldlauses. Säilita iga rühma eristamiseks vajalik allikapõhine rühma- või rollinimi, kuid jäta välja küsimata ülesanded, tegevused ja kirjeldavad kõrvallaused, mida rühma eristamiseks vaja ei ole. Toetavas tõendis võivad esineda organisatsioonide, haldusüksuste või asukohtade katvusarvud; need ei kuulu sellesse vastuseulatusse ja tuleb välja jätta, kui küsimus katvust eraldi ei küsi. Küsitud katvus esita eraldi ning oma ühikuga."
            ].join("\n");
      return {
        instruction,
        trace: {
          version: "uniform_participant_relation_v3",
          enabled: true,
...
function hasMunicipalStaffTerm(normalized = "") {
  return /(?:^|[^\p{L}\p{N}])(?:tootaj\p{L}*|sotsiaaltootaj\p{L}*|spetsialist\p{L}*|ametnik\p{L}*|sotsiaalosakon\p{L}*|social\s+worker\p{L}*|employee\p{L}*|staff|worker\p{L}*|specialist\p{L}*|социальн\p{L}*\s+(?:работник\p{L}*|сотрудник\p{L}*|специалист\p{L}*)|работник\p{L}*|сотрудник\p{L}*|специалист\p{L}*)(?=$|[^\p{L}\p{N}])/u.test(normalized);
}
...
function isCurrentMunicipalStaffingPresenceRequest(normalized = "") {
  ...
  return /(?:^|[^\p{L}\p{N}])(?:kas|on|tootab|tootavad|is|are|work|works|working|есть|работает|работают)(?=$|[^\p{L}\p{N}])/u.test(normalized) &&
    hasMunicipalStaffTerm(normalized);
}
...
function hasMunicipalityProviderRequestTerm(normalized = "") {
  return /\b(kes (?:neid |seda |neid teenuseid |teenuseid |toetusi )?(?:pakub|pakuvad|osutab|osutavad|korraldab|korraldavad|menetleb|menetlevad))\b/u.test(normalized) ||
    /\b(who (?:provides|provide|offers|offer|delivers|deliver|administers|administer)(?: them)?)\b/u.test(normalized) ||
    /\bкто\s+(?:их\s+)?(?:оказывает|предоставляет|организует|оформляет)\b/u.test(normalized);
}

function isMunicipalityContactInventoryRequest(message = "", history = []) {
  const normalized = normalizeIntentText(message);
  if (!normalized) return false;
  if (hasIndependentContactSourceCue(normalized)) return false;
  ...
  if (hasMunicipalityProviderRequestTerm(normalized)) {
    if (hasServiceTerm(normalized) || hasBenefitTerm(normalized)) return true;
    const recent = extractRecentUserText(history, 4).join("\n");
    return isMunicipalityServiceBenefitListRequest(recent);
  }
  return /(?:^|[^\p{L}\p{N}])(?:koik|loetle|nimeta|kes|millised|mitu|kui\s+palju|all|list|name|who|which|how\s+many|все|перечисл\p{L}*|назов\p{L}*|кто|какие|сколько)(?=$|[^\p{L}\p{N}])/u.test(normalized) &&
    (/(?:^|[^\p{L}\p{N}])(?:kontakt\p{L}*|contact\p{L}*|контакт\p{L}*)(?=$|[^\p{L}\p{N}])/u.test(normalized) || hasMunicipalStaffTerm(normalized));
}
...
function hasContactRequestTerm(normalized = "") {
  ...
  if (/\b(mitu|mitme|arv\w*)\b/.test(normalized) && /\b(?:19|20)\d{2}\b/.test(normalized)) return false;
  if (/(?:^|[^\p{L}\p{N}])(?:kontakt\p{L}*|telefoninumb\p{L}*|telefon\p{L}*|e-post\p{L}*|epost\p{L}*|email\p{L}*|meiliaadress\p{L}*|spetsialist\p{L}*|ametnik\p{L}*|sotsiaaltootaja\p{L}*|lastekaitsetootaja\p{L}*|juhtumikorraldaja\p{L}*|contact\p{L}*|phone\p{L}*|e-mail\p{L}*|social\s+worker\p{L}*|specialist\p{L}*|official\s+contact\p{L}*|municipal\s+official\p{L}*|контакт\p{L}*|телефон\p{L}*|почт\p{L}*|социальн\p{L}*\s+(?:работник\p{L}*|сотрудник\p{L}*|специалист\p{L}*)|сотрудник\p{L}*|специалист\p{L}*)(?=$|[^\p{L}\p{N}])/u.test(normalized)) {
    return true;
  }
  if (hasMunicipalityProviderRequestTerm(normalized)) return true;
...
export async function assembleRetrievalContext({
  ...
  const questionPlan = resolvedSemanticCandidates
    ? {
  ...
  const contactFollowupAnchorText = contextualCurrentMunicipalityStaffingFollowup || contextualMunicipalityContactFollowup
    ? lastAssistantServiceMapMunicipalityText(rawHistory) || extractRecentUserText(rawHistory, 1).join("\n")
    : "";
  ...
  const currentMunicipalityContactEvidenceRequested =
    ...
    (
      contextualCurrentMunicipalityStaffingFollowup ||
      contextualMunicipalityContactFollowup ||
      isMunicipalityContactInventoryRequest(effectiveMessage, rawHistory) ||
  ...
  const activeContactScope = currentMunicipalityContactEvidenceRequested
    ? resolveActiveContactScope(serviceMapKovContacts, effectiveMessage, rawHistory, {
        contextual: contextualCurrentMunicipalityStaffingFollowup || contextualMunicipalityContactFollowup
      })
    : null;
  const serviceMapKovContactRows = municipalityContactRequest || hasContactRequestTerm(normalizedMunicipalityQuestion)
  ...
  const municipalityServiceBenefitListRequest =
    allowMunicipalityScopedRag && !municipalityContactRequest && (
      ...
      isContextualServiceBenefitListFollowup(effectiveMessage, rawHistory)
    );
  ...
  const baseRagQueryText = sourceLookupRequest
    ...
    : buildRagSearchQuery(
        retrievalMessage,
        canonicalRetrievalActive ? [] : rawHistory
      );
  const temporalRetrievalPlan = sourceLookupRequest
    ...
    : buildTemporalRetrievalPlan({
        ...
        history: canonicalRetrievalActive ? [] : rawHistory,
  ...
  const {
    ...
    rawHistory: canonicalRetrievalActive ? [] : rawHistory,
    sourceLookupRequest,
    sourceLookupParagraphRefs,
    temporalRetrievalPlan,
    overviewSynthesisQuestion,
  ...
  const trustedSpecificResearchDocumentId = specificResearchFactQuestion
    ...
    : null;
  const specificResearchQuestionPlan = trustedSpecificResearchDocumentId
    ? { ...questionPlan, trusted_document_id: trustedSpecificResearchDocumentId }
    : questionPlan;
  ...
  if (shouldRunRag) {
    ...
    if (journalChunksPerDocument > 3) {
      ...
      factSegmentSearchDurationMs = monotonicDurationMs(factSegmentSearchStartedAt);
      if (numericFactEvidence.enabled) {
        if (
          !numericFactEvidence.sufficient &&
          specificResearchFactQuestion &&
          documentIdentityEvidence.matched &&
          documentIdentityEvidence.selectedDocumentId
        ) {
          const scopedQueries = buildDocumentScopedResearchFactQueries(questionPlan);
          if (scopedQueries.length) {
            try {
              ...
              const scopedMatches = await runRagSearch({
                ...
                filters: {
                  ...(searchFilters || {}),
                  doc_id: documentIdentityEvidence.selectedDocumentId
                },
              ...
              numericFactEvidence = selectSingleSourceNumericFactGroups(
                effectiveMessage,
                documentIdentityEvidence.groups
              );
            } catch (err) {
    ...
    const percentCountSemanticsInstruction = buildPercentCountSemanticsInstruction(chosen);
    if (percentCountSemanticsInstruction) extraSystemInstructions.push(percentCountSemanticsInstruction);
    const numericScopeEvidenceSummary = buildNumericScopeEvidenceSummary(effectiveMessage, chosen);
    ...
    renderedEvidenceGroups = budgeted.used.map((group, index) => ({
      ...group,
      bodies: [String(budgeted.renderedBlocks?.[index]?.text || "")]
    }));
  ...
  if (usedRagContext) {
    const requestedMetricContractResult = buildRequestedMetricContract({
      questionPlan,
    ...
    requestedMetricContract = requestedMetricContractResult.trace;
