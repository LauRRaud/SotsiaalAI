...
function bestRequestedMetricAssignment(slots = [], candidates = []) {
  if (!slots.length || !candidates.length) return { assignment: null, ambiguous: false };
  const sharedSubjectTerm = requestedMetricToken(slots[0]?.relation_terms?.[0] || "");
  const optionsBySlot = slots.map(slot => candidates
    .map(candidate => requestedMetricCandidateScore(slot, candidate, sharedSubjectTerm))
    .filter(Boolean)
    .sort((left, right) => right.score - left.score)
    .slice(0, 4));
  if (optionsBySlot.some(options => !options.length)) return { assignment: null, ambiguous: false };
  const solutions = [];
  ...
  visit(0, new Set(), [], 0);
  if (!solutions.length) return { assignment: null, ambiguous: false };
  ...
  return { assignment: ambiguous ? null : best.assignment, ambiguous };
}
...
function buildRequestedMetricContract({
  ...
  for (const group of (Array.isArray(renderedGroups) ? renderedGroups : []).slice(0, 8)) {
    ...
    const { assignment, ambiguous } = bestRequestedMetricAssignment(requestedSlots, candidates);
