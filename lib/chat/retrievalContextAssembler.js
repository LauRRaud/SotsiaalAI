...
function requestedFactBodyAlignment(slots = [], body = "") {
  const candidateEntries = researchIdentityCandidateRootEntries(body);
  ...
  slots.forEach((slot, slotIndex) => {
    ...
    const matchedTermCount = relationTerms.filter(term =>
      requestedFactRelationAlternatives(slot, term).some(variant =>
        researchIdentityTermMatchesEntries(variant, candidateEntries)
      )
    ).length;
    const minimumMatches = relationTerms.length <= 2 ? 1 : Math.min(2, Math.ceil(relationTerms.length * 0.35));
...
function requestedMetricCandidateScore(slot = {}, candidate = {}) {
  ...
  const localRelationHeadMatched = !!immediateFollowingToken && relationTerms.some(relationTerm =>
    requestedFactRelationAlternatives(slot, relationTerm).some(variant =>
      requestedMetricTermMatchQuality(variant, immediateFollowingToken.value) >= 0.72
    )
  );
