  ...
  for (const group of (Array.isArray(groups) ? groups : []).slice(0, 1)) {
    for (const body of Array.isArray(group?.bodies) ? group.bodies : []) {
      ...
      const instruction = replyLang === "en"
        ? [
            ...relationLines,
            "These values come from one explicit uniform-group relation in the selected source. participant_count_per_group applies only to participant groups governed by this same relation; total_participant_count is the people total for that same sample. Keep all remaining numbers attached to their explicit units in the evidence; when coverage is requested, state it separately."
          ].join("\n")
        : replyLang === "ru"
          ? [
              ...relationLines,
              "Эти значения получены из одной явной связи одинакового размера групп в выбранном источнике. participant_count_per_group относится только к группам участников в этой же связи; total_participant_count — общее число людей в той же выборке. Все остальные числа сохраняй с явно указанными в доказательстве единицами; запрошенный охват указывай отдельно."
            ].join("\n")
          : [
              ...relationLines,
              "Need väärtused tulevad valitud põhiallika ühest selgest ühtlase rühmasuuruse seosest. participant_count_per_group kehtib ainult sama seosega hõlmatud osalejarühmadele; total_participant_count on sama valimi inimeste koguarv. Seo kõik ülejäänud arvud tõendis selgelt nimetatud ühikutega; kui kasutaja küsib katvust, esita see eraldi."
            ].join("\n");
      return {
        instruction,
        trace: {
          version: "uniform_participant_relation_v1",
          enabled: true,
...
export function shouldUseNumericScopeInstruction(message = "") {
  const normalized = normalizeIntentText(message);
  if (!normalized) return false;
  return /\b(?:arv(?:u|ud)?|kokku|kui\s+palju|kui\s+suur\s+osa|mitu|osakaal\p{L}*|protsent\p{L}*|how\s+(?:many|much)|what\s+(?:percentages?|proportions?|shares?)|сколько|какая\s+доля|какой\s+процент\p{L}*)\b/u.test(normalized);
}
