...
export function selectSpecificResearchFactGroups(_message = "", groups = [], questionPlan = {}) {
  ...
  const ranked = candidates.map((group, index) => {
    ...
    const factFingerprintMatched = subjectMatches.length >= 1 && (
    ...
    if (requestedMetricShape.complete) score += 4;
    ...
    const strongAnchorCandidate = sourceCompatible && sourceYearCompatible && !requestedAuthor &&
      subjectMatches.length >= 1 &&
    ...
    Number(right.exactTitleAnchorMatched && right.sourceYearCompatible) -
      Number(left.exactTitleAnchorMatched && left.sourceYearCompatible) ||
