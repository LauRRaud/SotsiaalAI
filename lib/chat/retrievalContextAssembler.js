...
export function buildPinnedDevelopmentContextUnit({
  ...
  const span = {
    ...
    literal_original_start: literalAt === startOffset && originalBody.lastIndexOf(renderedUnit) === literalAt
      ? literalAt
      : null,
    provenance
  };
  if (append && normalizeIntentText(baseBlock?.evidenceText || "").includes(normalizeIntentText(normalizedUnit))) {
    ...
    const hasBoundExistingSpan = existingSpans.some(bodySpan => {
      ...
      const literalAt = sourceBody.indexOf(renderedBody);
      return existingAt >= bodySpan.rendered_start_offset && existingEnd <= bodySpan.rendered_end_offset &&
        literalAt === bodySpan.literal_original_start && sourceBody.lastIndexOf(renderedBody) === literalAt &&
