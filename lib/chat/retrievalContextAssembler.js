...
export function selectSpecificResearchFactGroups(_message = "", groups = [], questionPlan = {}) {
  ...
  if (best && !best.identityMatched && (decisiveSubjectLead || decisiveStrongAnchorLead)) {
...
export function buildPercentCountSemanticsInstruction(groups = []) {
  ...
  return `PERCENT_COUNT_SEMANTICS: tõendis olevad protsendi ja loenduse paarid on ${pairs}. Kuju X% (n=Y) tähendab, et allikas esitab selle näitaja juures loenduse Y. Ära nimeta Y-d valimi suuruseks, ära arvuta X% × Y põhjal uut inimeste arvu ja vasta paariga X% ehk Y inimest.`;
}
...
export function shouldCarryMunicipalityFromHistory(message = "") {
  ...
  if (serviceOrBenefit && (continuationLead || normalized.length <= 60)) return true;
  ...
  if (socialHelpTerm && (continuationLead || normalized.length <= 60)) return true;
  ...
  return normalized.length <= 60 && /\b(kontakt|kontaktid|telefon|e-post|email|taotlus|taotlema)\b/.test(normalized);
}
...
export async function assembleRetrievalContext({
  ...
  const plannerDurationMs = Math.max(0, Date.now() - plannerStartedAt);
  ...
  if (shouldRunRag) {
    ...
    if (legalLookupPlan?.enabled && legalLookupPlan.mode === "explicit_paragraph") {
      ...
      const documentIdentityStartedAt = Date.now();
      documentIdentityEvidence = selectSpecificResearchFactGroups(
        effectiveMessage,
        groupedMatches,
        questionPlan
      );
      documentIdentityDurationMs = Math.max(0, Date.now() - documentIdentityStartedAt);
    ...
    if (journalChunksPerDocument > 3) {
      ...
      const numericCandidateGroups = specificResearchFactQuestion
        ...
        : groupedMatches;
      const factSegmentSearchStartedAt = Date.now();
      numericFactEvidence = selectSingleSourceNumericFactGroups(effectiveMessage, numericCandidateGroups);
      factSegmentSearchDurationMs = Math.max(0, Date.now() - factSegmentSearchStartedAt);
        ...
        if (
          ...
          if (scopedQueries.length) {
            try {
              ...
              factSegmentSearchDurationMs += Math.max(0, Date.now() - scopedStartedAt);
              matches = dedupeRagMatches([...matches, ...scopedMatches]);
              groupedMatches = rankGroupsWithTopicHints(groupMatches(matches), topicHints, {
                ragRiskPolicy
              });
              const refreshedIdentityStartedAt = Date.now();
              documentIdentityEvidence = selectSpecificResearchFactGroups(
                effectiveMessage,
                groupedMatches,
                questionPlan
              );
              documentIdentityDurationMs += Math.max(0, Date.now() - refreshedIdentityStartedAt);
    ...
    const contextRenderStartedAt = Date.now();
    ...
    contextRenderDurationMs = Math.max(0, Date.now() - contextRenderStartedAt);
  ...
  return {
    ...
    retrievalMeta: {
      ...
      performanceTimings: {
        ...
        retrieval_context_total_ms: Math.max(0, Date.now() - assemblyStartedAt)
      },
      overviewSynthesis: overviewSelection?.metadata || null,
      evidencePackage,
      queryPlan,
