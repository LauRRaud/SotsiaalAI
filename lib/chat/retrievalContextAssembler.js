...
function requestedMetricEvidenceCandidates(renderedText = "", valueType = "proportion") {
  if (valueType !== "proportion") return [];
  const candidates = [];
...
function requestedMetricCandidateScore(slot = {}, candidate = {}, sharedSubjectTerm = "") {
  ...
  const normalizedSharedSubject = requestedMetricToken(sharedSubjectTerm);
  const immediateFollowingTokens = candidate.tokens.filter(token =>
    token.start >= candidate.end && token.start - candidate.end <= 40
  );
  const sharedSubjectHeadMatched = !!normalizedSharedSubject && immediateFollowingTokens.some(token =>
    requestedMetricTermMatchQuality(normalizedSharedSubject, token.value) >= 0.86
  );
  if (sharedSubjectHeadMatched) score += 4;
  return {
    candidate,
    score,
    matchedTermCount,
    relationTermCount: relationTerms.length,
    sharedSubjectHeadMatched
  };
}
...
function bestRequestedMetricAssignment(slots = [], candidates = []) {
  ...
  const sharedSubjectTerm = requestedMetricToken(slots[0]?.relation_terms?.[0] || "");
  const scoredOptionsBySlot = slots.map(slot => candidates
    .map(candidate => requestedMetricCandidateScore(slot, candidate, sharedSubjectTerm))
    .filter(Boolean)
  ...
  const optionsBySlot = scoredOptionsBySlot.map(options => options.slice(0, 4));
  ...
  const visit = (slotIndex, usedCandidateKeys, assignment, totalScore) => {
    if (slotIndex >= slots.length) {
      solutions.push({ assignment: [...assignment], totalScore });
      return;
    }
  ...
  solutions.sort((left, right) => right.totalScore - left.totalScore);
  ...
  const ambiguous = !!(assignmentsConflict && best.totalScore - second.totalScore < 0.9);
...
function buildRequestedMetricContract({
  ...
  const baseTrace = {
    ...
    mapping_method: "bounded_lexical_relation_alignment_v1",
  ...
  for (const group of (Array.isArray(renderedGroups) ? renderedGroups : []).slice(0, 8)) {
    ...
    const candidates = requestedMetricEvidenceCandidates(renderedText, "proportion");
    const { assignment, ambiguous, diagnostics } = bestRequestedMetricAssignment(requestedSlots, candidates);
    const currentDiagnostics = {
      evidence_candidate_count: candidates.length,
      ambiguous,
      slots: diagnostics
    };
    if (
