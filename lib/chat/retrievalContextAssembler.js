...
export async function assembleRetrievalContext({
  ...
  const selectedContextDetails = budgeted.used.map((entry, idx) => {
    ...
    const renderedBodyExternalGapChars = Math.max(0, renderedBodySpans.length - 1) * 5;
    return {
      source_id: displaySourceIdForContextEntry(entry, idx),
      raw_source_id: entry.sourceId || undefined,
      ...
      rendered_body_uncovered_chars: Math.max(
        0,
        renderedEvidenceBodyChars - renderedBodySpanCoveredChars - renderedBodyExternalGapChars
      ),
