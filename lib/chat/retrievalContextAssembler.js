...
import { factRelationTermMatchQuality, factRelationTokens, recommendationRelationMatches } from "@/lib/chat/factRelationSemantics";
import { genericPerformedTimeRelation, hasPerformedStudyEvent, isOrdinalTimeBoundary, qualitativeTimePayload } from "@/lib/chat/qualitativeTimeSemantics";
import { buildQuestionRequirementsShadow, projectQuestionRequirementsShadow } from "./questionRequirements.js";
import { typedKnownValueCandidate } from "./knownValueSemantics.js";
import { extractGroupFacts, groupPayloadForSlot, isGroupFactSlot } from "./groupFactSemantics.js";
import { buildGroupFactContract } from "./groupFactContract.js";
import { buildDirectedRelationContract } from "./directedRelationContract.js";
import {
  hasQualitativeRecommendationIntent,
...
import {
  ...
  requestedNamedScopeBoundToClaim,
...
function canonicalResearchTitleFamilyMatch(requestedTitle = "", candidateTitle = "") {
  const rawRequested = String(requestedTitle || "").trim();
  const terminalEllipsis = /(?:\u2026|\.{3})\s*$/u.test(rawRequested);
  const requested = normalizeIntentText(
    terminalEllipsis ? rawRequested.replace(/(?:\u2026|\.{3})\s*$/u, "") : rawRequested
  );
  ...
  if (
    terminalEllipsis &&
    requestedTokens.length >= 4 &&
    requested.length >= 18 &&
    candidate.startsWith(requested) &&
    candidate.length > requested.length &&
    /[^\p{L}\p{N}]/u.test(candidate[requested.length])
  ) {
    return { matched: true, kind: "truncated_prefix", priority: 2.5 };
  }
...
export function describeSpecificResearchDocumentLock(questionPlan = {}, identityEvidence = null) {
  const decision = {
    stage: "initial_lock",
    candidate_document_id: identityEvidence?.selectedDocumentId || null,
    candidate_matched: identityEvidence?.matched === true,
    candidate_confidence: identityEvidence?.confidence || null,
    selected_source_year: extractMatchGroupYear(identityEvidence?.groups?.[0] || {}),
    required_source_years: [],
    confirmed_source_years: [],
    unconfirmed_source_years: [],
    checks: {}
  };
  if (
  ...
  ) return { ...decision, eligible: false, reason: "candidate_not_confirmed" };
  ...
  ) return { ...decision, eligible: true, reason: "previous_source_exact_filter" };
  ...
  const topicConfirmation = buildCurrentTurnAuthorConfirmation(currentIdentity, identityEvidence.groups, null, questionPlan);
  const topicProof = topicConfirmation.topic_evidence;
  const recordedProof = identityEvidence.authorTopicEvidence;
  const sameBodyProof = topicProof?.candidates?.some(fresh => fresh.confirmed && fresh.document_id === selectedDocumentId &&
    fresh.document_version === questionPlan.trusted_document_version && recordedProof?.candidates?.some(recorded =>
      recorded.confirmed && recorded.document_id === selectedDocumentId &&
      ["source_id", "document_version", "chunk_id", "chunk_hash", "body_hash", "fragment_hash", "offset_basis", "start", "end"]
        .every(key => fresh[key] === recorded[key])));
  const authorTopicTrusted = trustedDocumentId === selectedDocumentId &&
    trustedSource === "current_turn_author_topic_confirmation" &&
    topicConfirmation.status === "confirmed_exact" && topicConfirmation.promotion_eligible === true &&
    topicProof?.confirmed_document_count === 1 && recordedProof?.confirmed_document_count === 1 &&
    sameBodyProof === true;
  const eligible = exactTitleConfirmed ||
  ...
  return {
    ...decision,
    eligible,
    reason: eligible ? "document_lock_confirmed" : allAuthorsConfirmed && requestedYears.length && !explicitYearConfirmed
      ? "source_years_unconfirmed" : "document_anchor_not_confirmed",
    required_source_years: requestedYears,
    confirmed_source_years: requestedYears.filter(year => reasons.includes(`source_year:${year}`)),
    unconfirmed_source_years: requestedYears.filter(year => !reasons.includes(`source_year:${year}`)),
    checks: {
      exact_title_confirmed: exactTitleConfirmed,
      canonical_title_confirmed: canonicalTitleFamilyConfirmed,
      all_authors_confirmed: allAuthorsConfirmed,
      all_source_years_confirmed: explicitYearConfirmed,
      author_confirmation_trusted: authorConfirmationTrusted,
      author_body_topic_confirmed: authorTopicTrusted
    }
  };
}

export function describeSpecificResearchDocumentRecheck(candidate, lockedId, stage) {
  const matched = candidate?.matched === true;
  const sameDocument = candidate?.selectedDocumentId === lockedId;
  return {
    stage,
    candidate_document_id: candidate?.selectedDocumentId || null,
    candidate_matched: matched,
    candidate_confidence: candidate?.confidence || null,
    selected_source_year: extractMatchGroupYear(candidate?.groups?.[0] || {}),
    locked_document_id: lockedId,
    eligible: matched && sameDocument,
    reason: matched && sameDocument ? "document_lock_confirmed" : stage === "recovery_recheck" ? "current_turn_document_lock_mismatch_after_recovery" : "current_turn_document_lock_mismatch",
    checks: { candidate_matches_locked_document: sameDocument }
  };
}

export function selectSpecificResearchFactGroups(_message = "", groups = [], questionPlan = {}) {
  const candidates = Array.isArray(groups) ? groups.filter(Boolean) : [];
  const authorConfirmation = buildCurrentTurnAuthorConfirmation(
    questionPlan?.semantic_candidates?.current_turn_document_identity, candidates, null, questionPlan
  );
  const authorTopicEvidence = authorConfirmation.topic_evidence;
  const requiresAuthorTopic = authorTopicEvidence.required &&
    questionPlan.trusted_document_id_source !== "previous_source_exact_filter";
  const bodyTopicDocumentIds = new Set(authorTopicEvidence.confirmed_document_ids);
  ...
  const ranked = candidates.map((group, index) => {
    ...
    const titleFamilyMatch = canonicalResearchTitleFamilyMatch(rawTitleHint, normalizedTitle);
    ...
    else if (canonicalTitleFamilyVariant === "truncated_prefix") score += 95;
    ...
    const authorBodyTopicConfirmed = bodyTopicDocumentIds.has(researchGroupDocumentId(group, index));
    const authoredIdentityMatched = authorMatched && (requiresAuthorTopic ? authorBodyTopicConfirmed : (
    ...
    (requiresAuthorTopic ? Number(right.authorBodyTopicConfirmed) - Number(left.authorBodyTopicConfirmed) : 0) ||
  ...
  const trustedDocumentIdSource = [
    ...
    "current_turn_author_topic_confirmation",
  ...
  if (
    trustedIndex > 0 &&
    (!requiresAuthorTopic || ranked[trustedIndex]?.authorBodyTopicConfirmed) &&
  ...
  const requiredTopicBodyHashes = Array.isArray(questionPlan.trusted_author_topic_body_hashes)
    ? questionPlan.trusted_author_topic_body_hashes : [];
  const topicProofCompatible = !questionPlan.trusted_document_version && !requiredTopicBodyHashes.length ||
    authorTopicEvidence.candidates.some(candidate => candidate.confirmed && candidate.document_id === best?.documentId &&
      (!questionPlan.trusted_document_version || candidate.document_version === questionPlan.trusted_document_version) &&
      (!requiredTopicBodyHashes.length || requiredTopicBodyHashes.includes(candidate.body_hash)));
  const trustedDocumentMatch = !!(
    ...
    (!requiresAuthorTopic || best.authorBodyTopicConfirmed) &&
    topicProofCompatible &&
  ...
  const competingAuthorTopicSources = requiresAuthorTopic && bodyTopicDocumentIds.size > 1;
  const ambiguous = competingAuthorTopicSources || competingCanonicalTitleFamilySources || (!trustedDocumentMatch && !!(
  ...
  const matched = !!best?.identityMatched && !ambiguous && topicProofCompatible;
  ...
  return {
    ...
    reasons: best ? [
      ...(requiresAuthorTopic ? [authorTopicEvidence.status] : []),
      ...(!topicProofCompatible ? ["author_topic_locked_evidence_changed"] : []),
      ...
      ...(best.canonicalTitleFamilyVariant === "truncated_prefix"
        ? ["canonical_title_family_anchor:truncated_prefix"]
    ...
    candidates: ranked.slice(0, 5).map((item, index) => ({
      ...
      authorBodyTopicConfirmed: item.authorBodyTopicConfirmed,
      ...
      resolvedSourceYear: extractMatchGroupYear(item.group),
...
function requestedFactRelationMatchDetails(slot = {}, value = "") {
  ...
  const relationTerms = Array.isArray(slot?.relation_terms)
    ...
    : [];
  const matchedTerms = slot.value_type === "recommendation" ? recommendationRelationMatches(slot, value) : relationTerms.filter(term =>
  ...
  const minimumMatches = relationTerms.length <= 2
    ? (slot.value_type === "recommendation" ? relationTerms.length : 1)
...
function requestedFactPrimaryRecommendation(value = "", slot = {}) {
  const text = String(value || "");
  const relative = /,\s*(?:mis|mille\p{L}*|which|that|котор\p{L}*)\b/iu.exec(text);
  if (!relative) return text;
  const primary = text.slice(0, relative.index);
  const explanation = text.slice(relative.index + relative[0].length);
  const fullRelation = requestedFactRelationMatchDetails(slot, text);
  const primaryRelation = requestedFactRelationMatchDetails(slot, primary);
  const preservesRequestedObjects = fullRelation.matchedTerms.every(term => primaryRelation.matchedTerms.includes(term));
  // A trailing relative clause describes the result of the recommendation.
  // Its modal/support verbs are not additional requested actions. Ambiguous
  // comma-nested clauses remain intact; coordinated main actions stay intact.
  return preservesRequestedObjects && !explanation.includes(",") && qualitativeActionSignature(primary).entries.length
    ? primary : text;
}

function requestedFactRecommendationSignature(value = "") {
  const signature = qualitativeActionSignature(value);
  const resultModals = new Set(Array.from(normalizeIntentText(value).matchAll(
    /(?:^|,)\s*(?:mis|mille\p{L}*)\s+(voimalda(?:b|vad))\b/gu
  ), match => match[1]));
  const entries = signature.entries.filter(entry => !resultModals.has(entry.term));
  return { ...signature, entries, terms: [...new Set(entries.map(entry => entry.term))],
    families: [...new Set(entries.map(entry => entry.family))], categories: [...new Set(entries.map(entry => entry.category))] };
}

function requestedRecommendationObjectDescriptions(slot = {}, value = "") {
  const signature = qualitativeActionSignature(value);
  return Array.from(new Set(requestedMetricEvidenceTokens(value).map(token => token.value).filter(token =>
    token.length >= 4 && !REQUESTED_FACT_ANSWER_ANCHOR_STOPWORDS.has(token) &&
    !/^(?:sellest|tulenevalt|teeme|ettepanek\p{L}*|pohimott\p{L}*|toimiv\p{L}*|inime\p{L}*|teenus\p{L}*|lahendus\p{L}*|susteem\p{L}*|protsess\p{L}*|voimalus\p{L}*|praktika\p{L}*|saama\p{L}*)$/u.test(token) &&
    !signature.terms.some(term => requestedMetricTermMatchQuality(term, token) >= 0.72) &&
    !recommendationRelationMatches(slot, token).length
  ))).slice(0, 8);
}
...
function requestedFactActionObjectBindings(slot = {}, value = "") {
  ...
  const issuanceIndex = terms.findIndex(term => /^(?:anti|andma|given)$/u.test(normalizeIntentText(term)));
  const recipient = issuanceIndex >= 0 && /le$/u.test(normalizeIntentText(terms[issuanceIndex + 1] || ""))
    ? terms[issuanceIndex + 1] : null;
  for (const clause of qualitativeActionClauses(requestedFactPrimaryRecommendation(value, slot))) {
    const signature = requestedFactRecommendationSignature(clause);
    if (!signature.entries.length) continue;
    const relation = requestedFactRelationMatchDetails(slot, clause);
    // Bind only actions about a requested object, not every consequence and
    // administrative detail in the selected paragraph.
    if (!relation.matchedTerms.length) continue;
    const objectAnchors = Array.from(new Set(relation.matchedTerms.filter(term => term !== recipient &&
      !/^(?:soovitus\p{L}*|anti|andma|recommendation\p{L}*|given)$/u.test(normalizeIntentText(term))))).slice(0, 8);
    ...
    for (const entry of signature.entries) {
      ...
      bindings.push({
        ...
        object_description_terms: requestedRecommendationObjectDescriptions(slot,
          value.startsWith(clause) ? value : clause),
        object_ellipsis_terms: relation.relationTerms.length === 2 ? requestedMetricEvidenceTokens(clause)
          .map(token => token.value).filter(token => /^(?:lahendus|susteem|meetod|alternatiiv)\p{L}*$/u.test(token)).slice(0, 4) : [],
        minimum_object_matches: relation.relationTerms.length <= 2
          ? Math.min(relation.minimumMatches, objectAnchors.length) : 1,
...
function requestedFactMethodAnchors(slot = {}, value = "") {
  ...
  tokens.forEach((token, index) => {
    ...
    if (!/^\s+$/u.test(String(value).slice(preceding.end, token.start))) return;
    if (/^(?:kasuta|teh|tege|labi|intervju|andme|uuring|uurimus|aruan|artikl)/u.test(preceding.value)) return;
...
function requestedFactQualitativeEvidenceBinding(slot = {}, body = "", peerSlots = []) {
  const valueType = String(slot?.value_type || "");
  if (!REQUESTED_QUALITATIVE_SLOT_VALUE_TYPES.has(valueType)) return null;
  if (slot.payload_kind === "referenced_study_period" && slot.reference_known_anchor) {
    const reference = peerSlots.find(peer => peer.index === slot.reference_slot_index && peer.payload_kind === "known_value_interpretation");
    const referenceBinding = reference ? requestedFactQualitativeEvidenceBinding(reference, body) : null;
    if (referenceBinding?.evidence_conflict) return referenceBinding;
    if (!referenceBinding?.admitted_payload) return null;
    // The known number alone is not an identity: two studies can report the
    // same percentage for different populations. Bind to the admitted meaning.
    slot = { ...slot, reference_payload: referenceBinding.admitted_payload };
  }
  ...
  const adjacentFallbackEnabled = phoneMode ||
    ageMode ||
    slot.payload_kind === "referenced_study_period" ||
  ...
  if (["known_value_interpretation", "referenced_study_period"].includes(slot.payload_kind)) {
    const admitted = candidates.map(candidate => ({ candidate, ...typedKnownValueCandidate(slot, candidate.text) }))
      .filter(item => item.status === "ADMITTED");
    // Every independently admitted proposition must agree; score cannot erase a conflict.
    if (!admitted.length) return null;
    if (new Set(admitted.map(item => JSON.stringify(item.payload))).size !== 1) return { evidence_conflict: true };
    const { candidate, payload } = admitted.sort((a, b) => a.candidate.text.length - b.candidate.text.length)[0];
    return { score: 20, matched_relation_terms: [], minimum_relation_matches: 0,
      minimum_answer_items: 1, minimum_evidence_anchor_count: 0, minimum_anchor_matches: 0,
      evidence_anchor_terms: [], temporal_binding: null, performed_study_time: false,
      required_numeric_values: [], evidence_action_terms: [], evidence_action_categories: [],
      evidence_negated: false, minimum_action_matches: 0, action_object_bindings: [],
      payload_kind: slot.payload_kind, admitted_payload: payload,
      evidence_fragment_hash: hashRenderedText(candidate.text), evidence_fragment_length: candidate.text.length,
  ...
  for (const candidate of candidates) {
    ...
    const timeMode = ["date", "month", "season", "timepoint"].includes(valueType);
    const timePayload = timeMode ? qualitativeTimePayload(candidate.text, smallCardinalNumberValue) : null;
    if (timeMode && !timePayload) continue;
    const performedTime = timeMode && genericPerformedTimeRelation(relation.relationTerms);
    if (performedTime) {
      if (!hasPerformedStudyEvent(candidate.text)) continue;
      relation.matchedTerms = [...relation.relationTerms];
    }
    ...
    if (valueType === "recommendation" && !hasQualitativeRecommendationIntent(
      requestedFactPrimaryRecommendation(candidate.text, slot)
    )) continue;
    const actionSignature = valueType === "recommendation"
      ? requestedFactRecommendationSignature(requestedFactPrimaryRecommendation(candidate.text, slot))
    ...
    const evidenceAnchors = roleComparisonMode
      ? roleAnchors
      : timeMode ? timePayload.anchor_terms
      : valueType === "method" ? methodAnchors
      : valueType === "recommendation"
        ? []
        : contentAnchors;
    const minimumEvidenceAnchorCount = phoneMode || ageMode || timeMode
      ? 0
      : roleComparisonMode
        ? minimumAnswerItems
        : valueType === "recommendation"
    ...
    const minimumAnchorMatches = phoneMode || ageMode || timeMode
      ? 0
      : valueType === "recommendation"
        ? 0
        : valueType === "text_relation"
    ...
    const requiredNumericValues = phoneMode ? phoneValues : ageMode ? ageValues : timeMode ? timePayload.numeric_values : [];
    ...
    const score = relation.matchedTerms.length * 5 + evidenceAnchors.length +
      requiredNumericValues.length * 3 + (actionObjectBindings.length ? 2 : 0);
    if (!candidate.adjacentFallback) standaloneBindingFound = true;
    if (!best || score > best.score || (score === best.score && candidate.text.length < best.evidence_fragment_length)) {
      best = {
        score,
        ...
        temporal_binding: timePayload,
        performed_study_time: performedTime,
        ...
        evidence_fragment_length: candidate.text.length,
        evidence_fragment_index: candidate.relationScopeIndex
      };
    }
  }
...
function requestedFactBodyAlignment(slots = [], body = "") {
  ...
  slots.forEach((slot, slotIndex) => {
    if (isGroupFactSlot(slot)) {
      if (groupPayloadForSlot(extractGroupFacts(body), slot)) {
    ...
    const qualitativeBinding = REQUESTED_QUALITATIVE_SLOT_VALUE_TYPES.has(slotValueType)
      ? requestedFactQualitativeEvidenceBinding(slot, body, slots)
      : null;
    if (qualitativeBinding?.evidence_conflict) return;
...
const REQUESTED_METRIC_CONTRACT_MAPPING_METHOD = "bounded_rendered_sentence_peer_alignment_v10";
...
function requestedMetricEvidenceFragments(value = "") {
  ...
  for (const body of text.split(bodySeparator)) {
    ...
    for (const match of body.matchAll(boundaries)) {
      ...
      if (isOrdinalTimeBoundary(body.slice(0, Number(match.index)), body.slice(Number(match.index) + match[0].length))) continue;
...
function requestedMetricMentionQualifier(fragment = "", start = 0, end = 0) {
  ...
  if (
    /(?:[~≈<>≤≥]|\d{1,3}(?:[.,]\d+)?\s*%?\s*(?:[–—−-]|kuni|to|до))\s*$/u.test(prefix) ||
    /^\s*(?:(?:[–—−-]|kuni|to|до)\s*\d{1,3}(?:[.,]\d+)?\s*%?|(?:voi|or|или)\s+(?:vahem|rohkem|less|more|меньше|больше))/u.test(suffix)
  ) return "range";
...
function requestedMetricEvidenceCandidates(renderedText = "", valueTypes = ["proportion"]) {
  ...
  fragments.forEach((fragmentEntry, fragmentIndex) => {
    ...
    const occupiedSpans = [];
    if (requestedValueTypes.has("proportion")) {
      percentageRanges(fragment).forEach((range, mentionIndex) => {
        // Reserve both endpoints even when an unsupported external qualifier
        // prevents mapping; neither endpoint may fall back to a scalar fact.
        occupiedSpans.push([range.start, range.end]);
        if (requestedMetricMentionQualifier(fragment, range.start, range.end)) return;
        candidates.push({
          key: `${fragmentIndex}:percentage-range:${mentionIndex}`,
          fragment, fragmentIndex, mentionIndex,
          bodyIndex: fragmentEntry.bodyIndex,
          relationScopeIndex: fragmentEntry.relationScopeIndex,
          start: range.start, end: range.end,
          absoluteStart: fragmentEntry.startOffset + range.start,
          absoluteEnd: fragmentEntry.startOffset + range.end,
          value: range.value, rangeEnd: range.endValue,
          unit: "percent", qualifier: "range", valueType: "proportion",
          parenthesisDepth: requestedMetricParenthesisDepth(fragment, range.start),
          tokens
        });
      });
    }
...
function requestedMetricCandidateScore(
  ...
  if (["coordinated_shared_head", "coordinated_named_scope"].includes(slot?.derivation) && candidate.parenthesisDepth > 0) return null;
  let score = 0;
...
function requestedMetricCoordinationAssignment(slots = [], assignment = []) {
  ...
  for (const slotIndexes of coordinationGroups.values()) {
    ...
    if (slotIndexes.every(index => slots[index]?.derivation === "coordinated_named_scope")) {
      const scopedSlots = slots.map((slot, index) => ({ ...slot, evidence_value: qualified[index]?.candidate?.value }));
      if (!slotIndexes.every(index => {
        const candidate = qualified[index].candidate;
        return requestedNamedScopeBoundToClaim(candidate.fragment, {
          index: candidate.start, raw: candidate.fragment.slice(candidate.start, candidate.end),
          value: candidate.value, percentage: candidate.valueType === "proportion"
        }, scopedSlots[index], scopedSlots).passed;
      })) return null;
      continue;
    }
...
function bestRequestedMetricAssignment(slots = [], candidates = []) {
  ...
  const diagnostics = scoredOptionsBySlot.map((options, index) => ({
    ...
    top_candidates: options.slice(0, 3).map(option => ({
      ...
      ...(option.candidate.rangeEnd ? { evidence_range_end: option.candidate.rangeEnd, qualifier: "range" } : {}),
  ...
  const visit = (slotIndex, usedCandidateKeys, assignment, totalScore) => {
    ...
    for (const option of optionsBySlot[slotIndex]) {
      ...
      if (
        !allExplicitSlots &&
        !(slots[slotIndex]?.derivation === "coordinated_named_scope" &&
          slots[slotIndex - 1]?.derivation === "coordinated_named_scope" &&
          slots[slotIndex]?.coordination_group === slots[slotIndex - 1]?.coordination_group) &&
      ...
      if (coordinationGroup) {
        ...
        if (
          ...
          (
            ...
            (slots[slotIndex]?.derivation === "coordinated_named_scope"
              ? Math.max(option.candidate.absoluteEnd, peerCandidate.absoluteEnd) - Math.min(option.candidate.absoluteStart, peerCandidate.absoluteStart)
              : option.candidate.absoluteStart - peerCandidate.absoluteEnd) >
              REQUESTED_METRIC_CONTRACT_MAX_COORDINATION_PEER_CHAR_SPAN
          )
        ) continue;
  ...
  const conflictingSolution = solutions.find(solution => best.assignment.some((option, index) => {
    ...
    return option.candidate.value !== competingCandidate?.value ||
      option.candidate.rangeEnd !== competingCandidate?.rangeEnd;
  }));
...
function requestedMetricContractInstruction(trace = null, replyLang = "et") {
  ...
  const slotLines = trace.slots.map(slot => {
    ...
    const cardinality = Number.isInteger(Number(slot?.expected_cardinality)) &&
      ...
      : "";
    const rangeEnd = slot.qualifier === "range" ? `–${slot.evidence_range_end}` : "";
    const namedScope = slot.named_scope_constraints;
    const typedScope = namedScope ?
      `${namedScope.age ? `; age=${namedScope.age.negated ? "not:" : ""}${namedScope.age.operator}:${namedScope.age.value}` : ""}` +
      `${namedScope.observation_year ? `; observation_year=${namedScope.observation_year}` : ""}` : "";
    return `slot_${slot.slot_index}: ${slot.value_type}=${slot.evidence_value}${rangeEnd}${unit}${scope}${typedScope}${qualifier}${cardinality}${relation}`;
  });
  if (replyLang === "en") {
    return [
...
function requestedQualitativeSlotContractInstruction(trace = null, replyLang = "et") {
  ...
  const slotLines = trace.slots.map(slot => {
    ...
    return `slot_${slot.slot_index}: ${slot.value_type}` +
      ...
      `${slot.admitted_payload ? `; admitted_proposition=${JSON.stringify(slot.admitted_payload)}` : ""}`;
  });
  if (replyLang === "en") {
    return [
...
export function buildRequestedQualitativeSlotContract({
  ...
  const slots = (Array.isArray(requested?.slots) ? requested.slots : [])
    .filter(slot => isGroupFactSlot(slot) || REQUESTED_QUALITATIVE_SLOT_VALUE_TYPES.has(String(slot?.value_type || "")))
    .slice(0, 12);
  ...
  if (slots.some(slot => slot?.payload_kind === "directed_event_relation_set")) {
    return buildDirectedRelationContract({ slots: requested.slots, groups: renderedGroups, blocks: renderedBlocks,
      documentId: selectedDocumentId, replyLang, baseTrace });
  }
  if (slots.some(isGroupFactSlot)) {
    return buildGroupFactContract({ slots: requested.slots, groups: renderedGroups, blocks: renderedBlocks,
      documentId: selectedDocumentId, replyLang, baseTrace });
  }
  const eligibleBodies = (Array.isArray(renderedGroups) ? renderedGroups : [])
    .map((group, index) => renderedBlocks[index]
      ? { ...group, bodies: [renderedBlocks[index].evidenceText] } : group)
  ...
  for (const slot of slots) {
    const bindings = eligibleBodies
      .map(body => requestedFactQualitativeEvidenceBinding(slot, body, slots))
      .filter(Boolean)
      .sort((left, right) => right.score - left.score || left.evidence_fragment_length - right.evidence_fragment_length);
    if (bindings.some(binding => binding.evidence_conflict) ||
      (bindings.some(binding => binding.admitted_payload) && new Set(bindings.map(binding => JSON.stringify(binding.admitted_payload))).size !== 1)) {
      conflictingSlotIndexes.push(Number(slot.index));
      continue;
    }
    ...
    mappedSlots.push({
      ...
      ...(binding.admitted_payload ? { payload_kind: binding.payload_kind, admitted_payload: binding.admitted_payload,
        reference_slot_index: slot.reference_slot_index ?? null } : {}),
      ...
      temporal_binding: binding.temporal_binding,
      performed_study_time: binding.performed_study_time,
      ...
      action_object_bindings: binding.action_object_bindings.map(actionBinding => ({
        ...
        object_anchor_terms: Array.isArray(actionBinding.object_anchor_terms)
          ...
          : [],
        object_description_terms: Array.isArray(actionBinding.object_description_terms)
          ? actionBinding.object_description_terms.slice(0, 8) : [],
        object_ellipsis_terms: Array.isArray(actionBinding.object_ellipsis_terms)
          ? actionBinding.object_ellipsis_terms.slice(0, 4) : [],
  ...
  const trace = {
    ...
    used_for_validation: false,
    reason: complete ? "all_qualitative_slots_bound_to_rendered_evidence"
      : conflictingSlotIndexes.length ? "qualitative_evidence_conflict" : "qualitative_evidence_mapping_incomplete",
    conflicting_slot_indexes: conflictingSlotIndexes,
...
export function buildRequestedFactSlotContract({
  ...
  for (const [groupIndex, group] of (Array.isArray(renderedGroups) ? renderedGroups : []).slice(0, 8).entries()) {
    ...
    const slots = assignment.map((option, index) => {
      ...
      evidence_value: option.candidate.value,
      ...(option.candidate.rangeEnd ? { evidence_range_end: option.candidate.rangeEnd } : {}),
      unit: option.candidate.unit,
      qualifier: option.candidate.qualifier || null,
      relation_terms: relationTerms,
      ...(Array.isArray(requestedSlots[index]?.named_scope_terms)
        ? { named_scope_terms: requestedSlots[index].named_scope_terms.slice(0, 3),
          named_scope_constraints: requestedSlots[index].named_scope_constraints } : {}),
...
export function buildPinnedDevelopmentContextUnit({
  candidate,
  baseEntry,
  baseBlock = null,
  contextIndex,
  ...
  let startOffset = candidate?.developmentSpan?.start;
  let endOffset = candidate?.developmentSpan?.end;
  if (!Number.isSafeInteger(startOffset) || !Number.isSafeInteger(endOffset) ||
    startOffset < 0 || endOffset <= startOffset || endOffset > originalBody.length) return null;
  ...
  const originalBodyHash = hashRenderedText(originalBody);
  const sourceDocumentId = String(baseEntry?.docId || "");
  const sourceId = String(baseEntry?.sourceId || baseEntry?.key || "");
  const candidateEvidence = (Array.isArray(candidate?.bodyEvidence) ? candidate.bodyEvidence : []).filter(record =>
    record?.normalized_body_hash === originalBodyHash &&
    String(record?.document_id || "") === sourceDocumentId &&
    String(record?.source_id || "") === sourceId
  );
  const bodyEvidence = [...new Map([
    ...(Array.isArray(baseEntry?.bodyEvidence) ? baseEntry.bodyEvidence : []),
    ...candidateEvidence
    ...
    record?.chunk_hash,
    record?.chunk_body_offset
  ]), record])).values()];
  const pinnedEntry = { ...baseEntry, bodies, bodyEvidence };
  ...
  const renderedUnit = String(renderedUnitBlock.evidenceText);
  const literalAt = originalBody.indexOf(renderedUnit);
  const provenance = [...new Map(bodyEvidence
    .filter(record => record?.normalized_body_hash === originalBodyHash)
    .map(record => [JSON.stringify([
      record.document_id,
      record.source_id,
      record.chunk_id,
  ...
  const span = {
    ...
    rendered_body_hash: hashRenderedText(renderedUnit),
    original_body_chars: originalBody.length,
    rendered_body_chars: renderedUnit.length,
    start_offset: startOffset,
    end_offset: endOffset,
    rendered_start_offset: 0,
    rendered_end_offset: renderedUnit.length,
    truncated: startOffset > 0 || endOffset < originalBody.length,
    literal_original_start: originalBody.slice(startOffset, startOffset + renderedUnit.length) === renderedUnit
      ? startOffset
      : literalAt >= 0 && originalBody.lastIndexOf(renderedUnit) === literalAt ? literalAt : null,
    provenance
  };
  if (append && normalizeIntentText(baseBlock?.evidenceText || "").includes(normalizeIntentText(normalizedUnit))) {
    const evidenceText = String(baseBlock?.evidenceText || "");
    const existingAt = evidenceText.indexOf(renderedUnit);
    if (existingAt < 0 || evidenceText.lastIndexOf(renderedUnit) !== existingAt) return null;
    const existingEnd = existingAt + renderedUnit.length;
    const existingSpans = Array.isArray(baseBlock?.bodySpans) ? baseBlock.bodySpans : [];
    const hasBoundExistingSpan = existingSpans.some(bodySpan => {
      if (!Number.isSafeInteger(bodySpan?.original_body_index) ||
        bodySpan.original_body_index < 0 || bodySpan.original_body_index >= bodies.length ||
        !Number.isSafeInteger(bodySpan?.rendered_start_offset) ||
        !Number.isSafeInteger(bodySpan?.rendered_end_offset) ||
        bodySpan.rendered_start_offset < 0 ||
        bodySpan.rendered_end_offset <= bodySpan.rendered_start_offset ||
        bodySpan.rendered_end_offset > evidenceText.length ||
        !Number.isSafeInteger(bodySpan?.literal_original_start)) return false;
      const renderedBody = evidenceText.slice(bodySpan.rendered_start_offset, bodySpan.rendered_end_offset);
      const sourceBody = String(bodies[bodySpan.original_body_index] || "");
      return existingAt >= bodySpan.rendered_start_offset && existingEnd <= bodySpan.rendered_end_offset &&
        sourceBody.slice(bodySpan.literal_original_start,
          bodySpan.literal_original_start + renderedBody.length) === renderedBody &&
        hashRenderedText(sourceBody) === bodySpan.original_body_hash &&
        hashRenderedText(renderedBody) === bodySpan.rendered_body_hash &&
        (bodySpan.provenance || []).some(record =>
          record?.normalized_body_hash === hashRenderedText(sourceBody) &&
          String(record?.document_id || "") === sourceDocumentId &&
          String(record?.source_id || "") === sourceId && record?.source_status === "active"
        );
    });
    if (hasBoundExistingSpan) return { entry: pinnedEntry, block: baseBlock };
    const unrelatedSpans = existingSpans.filter(bodySpan => {
      const sameOriginalBody = bodySpan?.original_body_index === originalBodyIndex ||
        bodySpan?.original_body_hash === originalBodyHash;
      const overlapsRenderedUnit = Number.isSafeInteger(bodySpan?.rendered_start_offset) &&
        Number.isSafeInteger(bodySpan?.rendered_end_offset) &&
        bodySpan.rendered_start_offset < existingEnd && bodySpan.rendered_end_offset > existingAt;
      return !sameOriginalBody && !overlapsRenderedUnit;
    });
    return {
      entry: pinnedEntry,
      block: {
        ...baseBlock,
        bodySpans: [...unrelatedSpans, {
          ...span,
          rendered_start_offset: existingAt,
          rendered_end_offset: existingEnd
        }]
      }
    };
  }
  if (!append) {
    return {
      ...
      block: {
        ...renderedUnitBlock,
        text: renderOneContextBlock(renderedUnitEntry, contextIndex, { maxBodies: 1, includeAuthors }),
        originalBodyCount: bodies.length,
        renderedBodyCount: 1,
        originalBodyHash: hashRenderedText(bodies.join(separator)),
        renderedBodyHash: hashRenderedText(renderedUnit),
        truncated: span.truncated || bodies.length > 1,
        bodySpans: [span]
      }
    };
  }
  const evidenceText = [baseBlock?.evidenceText, renderedUnit].filter(Boolean).join(separator);
  const renderedBodyCount = Number(baseBlock?.renderedBodyCount || 0) + 1;
  return {
    entry: pinnedEntry,
    block: {
      ...baseBlock,
      text: [baseBlock?.text, renderedUnit].filter(Boolean).join(separator),
      evidenceText,
      originalBodyCount: bodies.length,
      renderedBodyCount,
      originalBodyHash: hashRenderedText(bodies.join(separator)),
      ...
      bodySpans: [
        ...
        {
          ...span,
          rendered_start_offset: baseBlock?.evidenceText
            ? String(baseBlock.evidenceText).length + separator.length
            : 0,
          rendered_end_offset: evidenceText.length
        }
      ]
    }
  };
...
function buildThematicSynthesisInstruction(replyLang = "et") {
  if (replyLang === "en") {
    return [
      ...
      "Keep every claim attached to its own named service, population and time. Separate passages are not consecutive prose: never infer the referent of 'the service' from another passage.",
  ...
  return [
    ...
    "Säilita iga väite enda teenus, sihtrühm ja aeg. Eraldi katkendid ei ole järjestikune tekst: ära tuleta üldnimetuse „teenus” referenti teise katkendi teenusenimest.",
...
// Identity anchors must be independent of the author, date and requested
// quantity/operation. This deliberately bounded lexical route is not a fact
// validator: every retained topic term must occur in one versioned body window.
const AUTHOR_TOPIC_NOISE = /^(?:alusel|jargi|jargne\p{L}*|kuidas|elas|elab|elavad|elanu\p{L}*|jagun\p{L}*|koosseis\p{L}*|inimes\p{L}*|isiku\p{L}*|leibkon\p{L}*|arv\p{L}*|aasta\p{L}*|jaanuar\p{L}*|veebruar\p{L}*|marts\p{L}*|aprill\p{L}*|mai|juuni\p{L}*|juuli\p{L}*|august\p{L}*|septembr\p{L}*|oktoobr\p{L}*|novembr\p{L}*|detsembr\p{L}*)$/u;
const authorTopicHash = value => createHash("sha256").update(String(value || "")).digest("hex");
const authorTopicIdentifier = value => typeof value === "string" &&
  /^[\p{L}\p{N}_.:/+-]{1,180}$/u.test(value) && !/^sk-/iu.test(value);

function independentAuthorTopicTerms(questionPlan = {}, authors = []) {
  const authorEntries = researchIdentityCandidateRootEntries(authors.join(" "));
  const terms = [];
  for (const value of Array.isArray(questionPlan.document_subject_terms) ? questionPlan.document_subject_terms : []) {
    const term = normalizeIntentText(value);
    if (!/^\p{L}{4,}$/u.test(term) || AUTHOR_TOPIC_NOISE.test(term) ||
      researchIdentityTermMatchesEntries(term, authorEntries) ||
      terms.some(existing => researchIdentityTermMatchesEntries(term, researchIdentityCandidateRootEntries(existing)))) continue;
    terms.push(term);
  }
  return terms;
}

function independentTopicHitCount(hits, termCount) {
  const occupied = new Map();
  const assign = (termIndex, visited) => {
    for (let index = 0; index < hits.length; index++) {
      if (visited.has(index) || !hits[index].indexes.includes(termIndex)) continue;
      visited.add(index);
      if (!occupied.has(index) || assign(occupied.get(index), visited)) {
        occupied.set(index, termIndex);
        return true;
      }
    }
    return false;
  };
  return Array.from({ length: termCount }, (_, index) => assign(index, new Set())).filter(Boolean).length;
}

function authorBodyTopicEvidence(group, terms, authors) {
  const fail = reason => ({ confirmed: false, reason, matched_term_count: 0 });
  if (terms.length < 2) return fail("independent_topic_anchors_missing");
  if (!authorTopicIdentifier(group.docId) || !authorTopicIdentifier(group.sourceId) ||
    group.sourceStatus !== "active" || isLegalSource(group) || !isResearchOrJournalSource(group)) {
    return fail("source_not_active_research");
  }
  const records = Array.isArray(group.bodyEvidence) ? group.bodyEvidence : [];
  const versions = new Set(records.map(item => item.document_version).filter(Boolean));
  if (versions.size > 1) return fail("document_version_conflict");
  let matchedTermCount = 0;
  let unboundBody = false;
  let unboundAuthor = false;
  for (const body of Array.isArray(group.bodies) ? group.bodies : []) {
    const text = String(body || "");
    const bodyHash = authorTopicHash(text);
    const provenance = records.filter(item => item.normalized_body_hash === bodyHash);
    const bound = provenance.length && provenance.every(item =>
      item.document_id === group.docId && item.source_id === group.sourceId && item.source_status === "active" &&
      authorTopicIdentifier(item.chunk_id) && authorTopicIdentifier(item.document_version) &&
      ...
      Number.isSafeInteger(item.chunk_body_offset + text.length) &&
      item.chunk_body_offset + text.length <= item.chunk_char_count);
    if (!bound) { unboundBody = true; continue; }
    if (!provenance.every(item => authors.every(author => Array.isArray(item.author_identity_hashes) &&
      item.author_identity_hashes.includes(authorTopicHash(canonicalAuthorKey(author)))))) {
      unboundAuthor = true;
      continue;
    }
    // Work on original text offsets, not a title/tag concatenation or score.
    const entries = researchIdentityCandidateRootEntries(text);
    const hits = entries.flatMap(entry => {
      const indexes = terms.flatMap((term, index) => researchIdentityTermMatchesEntries(term, [entry]) ? [index] : []);
      return indexes.length ? [{ entry, indexes }] : [];
    });
    for (let startIndex = 0; startIndex < hits.length; startIndex++) {
      const start = hits[startIndex].entry.start;
      for (let endIndex = startIndex; endIndex < hits.length; endIndex++) {
        const end = hits[endIndex].entry.end;
        if (end - start > 1200) break;
        const hitCount = independentTopicHitCount(hits.slice(startIndex, endIndex + 1), terms.length);
        matchedTermCount = Math.max(matchedTermCount, hitCount);
        if (hitCount !== terms.length) continue;
        const record = provenance[0];
        return { confirmed: true, reason: "body_topic_confirmed", matched_term_count: hitCount,
          source_id: record.source_id,
          document_version: record.document_version, chunk_id: record.chunk_id, chunk_hash: record.chunk_hash,
          body_hash: bodyHash, fragment_hash: authorTopicHash(text.slice(start, end)),
          offset_basis: "retrieved_chunk_text_utf16", start: record.chunk_body_offset + start,
          end: record.chunk_body_offset + end };
      }
    }
  }
  return { ...fail(unboundAuthor ? "body_author_metadata_unconfirmed" : unboundBody ? "body_provenance_missing" : "body_topic_not_confirmed"), matched_term_count: matchedTermCount };
}
...
function groupMatchesCurrentTurnDocumentAnchors(group = {}, identityCandidate = null) {
  ...
  const explicitTitleHint = identityCandidate?.title_hint?.provenance === "explicit_current_turn"
    ? String(identityCandidate?.title_hint?.value || "").trim()
    : "";
  if (
    explicitTitleHint &&
...
export function buildCurrentTurnAuthorConfirmation(
  ...
  const bodyTopicRequired = candidateProvenance === "explicit_current_turn" &&
    candidateConfidence === "high" && !candidateDocumentSourceYears.length &&
    !(identityCandidate?.title_hint?.provenance === "explicit_current_turn" && identityCandidate.title_hint.value);
  const topicTerms = independentAuthorTopicTerms(questionPlan, candidateValues);
  const topicBase = { version: "author_body_topic_v1", required: bodyTopicRequired,
    status: "author_not_confirmed", topic_term_count: topicTerms.length,
    confirmed_document_count: 0, confirmed_document_ids: [], candidates: [] };
  const base = {
  ...
  const topicCandidates = bodyTopicRequired ? matched.map(entry => ({
    document_id: entry.documentId, ...authorBodyTopicEvidence(entry.group, topicTerms, candidateValues)
  })) : [];
  // A document can be retrieved in several groups. Conflicting versions must
  // not become a unique match just because the best-scoring group was first.
  for (const candidate of topicCandidates) {
    const siblings = retrievedGroups.filter((group, index) => researchGroupDocumentId(group, index) === candidate.document_id);
    const versions = new Set(siblings.flatMap(group => (group.bodyEvidence || []).map(item => item.document_version)).filter(Boolean));
    if (versions.size > 1) Object.assign(candidate, { confirmed: false, reason: "document_version_conflict" });
  }
  const topicDocumentIds = Array.from(new Set(topicCandidates.filter(item => item.confirmed).map(item => item.document_id)));
  const topicEvidence = { ...topicBase,
    status: !bodyTopicRequired ? "not_required" : topicTerms.length < 2 ? "independent_topic_anchors_missing"
      : topicDocumentIds.length > 1 ? "multiple_author_topic_documents" : topicDocumentIds.length === 1 ? "unique_author_topic_document" : "body_topic_not_confirmed",
    confirmed_document_count: topicDocumentIds.length, confirmed_document_ids: topicDocumentIds,
  ...
  return {
    ...
    promotion_source: bodyTopicRequired ? "current_turn_author_topic_confirmation" : "current_turn_author_confirmation",
    promotion_document_ids: bodyTopicRequired ? topicDocumentIds : matchedDocumentIds,
    promotion_eligible: candidateProvenance === "explicit_current_turn" &&
      candidateConfidence === "high" &&
      status === "confirmed_exact" &&
      (bodyTopicRequired ? topicDocumentIds.length === 1 : matchedDocumentIds.length === 1)
  };
}
...
import { scopeSelectionSearch, selectionMatchAllowed, sourceSelectionCandidates } from "./sourceSelectionEvidence.js";
...
export async function assembleRetrievalContext({
  ...
  trustedSourceSelectionDocument = null,
  effectiveMessage,
  ...
  const runRagSearch = async (options = {}) => {
    ...
    try {
      const results = await runRagSearchImpl({
        ...scopeSelectionSearch(options, trustedSourceSelectionDocument),
      ...
      return trustedSourceSelectionDocument
        ? results.filter(match => selectionMatchAllowed(match, trustedSourceSelectionDocument, normalizedRole)) : results;
    } finally {
  ...
  const topicHints = prioritizeRequestedRelationTopicHints(
    questionPlan,
    extractTopicHints(temporalRetrievalPlan.focusText || retrievalMessage)
  );
  ...
  let confirmedSourceSelectionCandidates = [];
  ...
  if (shouldRunRag) {
    ...
    if (legalLookupPlan?.enabled && legalLookupPlan.mode === "explicit_paragraph") {
      ...
      const confirmedCurrentTurnDocumentIds = Array.isArray(currentTurnAuthorConfirmation?.promotion_document_ids)
        ? currentTurnAuthorConfirmation.promotion_document_ids
          .map(value => String(value || "").trim())
          .filter(Boolean)
        : [];
      confirmedSourceSelectionCandidates = sourceSelectionCandidates(currentTurnAuthorConfirmation, groupedMatches);
      if (
        ...
        specificResearchQuestionPlan = {
          ...
          trusted_document_id_source: currentTurnAuthorConfirmation.promotion_source,
          ...(currentTurnAuthorConfirmation.topic_evidence.required ? {
            trusted_document_version: currentTurnAuthorConfirmation.topic_evidence.candidates.find(candidate =>
              candidate.confirmed && candidate.document_id === confirmedCurrentTurnDocumentIds[0])?.document_version,
            trusted_author_topic_body_hashes: currentTurnAuthorConfirmation.topic_evidence.candidates.filter(candidate =>
              candidate.confirmed && candidate.document_id === confirmedCurrentTurnDocumentIds[0]).map(candidate => candidate.body_hash)
          } : {})
        };
      }
      ...
      const initialLockDecision = describeSpecificResearchDocumentLock(specificResearchQuestionPlan, documentIdentityEvidence);
      documentIdentityEvidence.decision = initialLockDecision;
      ...
      if (
        ...
        documentIdentityEvidence = {
          ...documentIdentityEvidence,
          decision: { ...initialLockDecision, eligible: false, reason: "trusted_document_id_mismatch" },
          matched: false,
          confidence: "low",
          selectedDocumentId: null,
          selectedTitle: null,
      ...
      const lockedSpecificResearchDocumentId = documentIdentityEvidence.decision.eligible
        ? String(documentIdentityEvidence.selectedDocumentId || "").trim()
        : "";
      ...
      if (
        ...
        if (scopedQueries.length) {
          ...
          try {
            ...
            documentIdentityEvidence.decision = describeSpecificResearchDocumentRecheck(refreshedDocumentIdentityEvidence, lockedSpecificResearchDocumentId, "scoped_search_recheck");
            chosen = documentIdentityEvidence.groups;
            ...
            if (recoveryQueries.length) {
              ...
              documentIdentityEvidence.decision = describeSpecificResearchDocumentRecheck(recoveredIdentityEvidence, lockedSpecificResearchDocumentId, "recovery_recheck");
              chosen = documentIdentityEvidence.groups;
    ...
    const plannedContextBudgetOptions = buildRagContextBudgetOptions({
        temporalRetrievalPlan,
        municipalityContactRequest,
        municipalityServiceBenefitListRequest,
    ...
    const baseContextBudgetOptions = {
      ...plannedContextBudgetOptions,
      preferredTopicTerms: Array.isArray(plannedContextBudgetOptions.preferredTopicTerms) &&
        plannedContextBudgetOptions.preferredTopicTerms.length
        ? plannedContextBudgetOptions.preferredTopicTerms
        : topicHints,
      ...
      requireSelfContainedPassages: !specificResearchFactQuestion && (thematicSynthesisQuestion || overviewSynthesisQuestion),
      ...
      if (renderedAggregateRows.length >= 2 && renderedAnnualCoverageIncomplete) {
        ...
        const primaryDevelopmentCandidate = (primaryDevelopmentCandidateGroup.bodies || [])
          .map((body, originalBodyIndex) => ({
            body,
            originalBodyIndex,
            bodyEvidence: primaryDevelopmentCandidateGroup.bodyEvidence,
          ...
          .map((group, groupIndex) => {
            ...
            const developmentCandidate = (rawCandidateGroup.bodies || [])
              .map((body, originalBodyIndex) => ({
                body,
                originalBodyIndex,
                bodyEvidence: rawCandidateGroup.bodyEvidence,
                developmentSpan: preferredTemporalQualitativeDevelopmentSpan(body, {
        ...
        if (
          ...
          const pinDevelopmentUnit = args => buildPinnedDevelopmentContextUnit({
            ...args,
  ...
  const ragSources = budgeted.used.map((entry, idx) => {
    ...
    return {
      ...
      rendered_body_hash: renderedBlock?.renderedBodyHash,
      rendered_body_spans: renderedBlock?.bodySpans,
      rendered_block_index: idx
    };
  });
  ...
  const selectedContextDetails = budgeted.used.map((entry, idx) => {
    const renderedBlock = renderedRagBlocks[idx];
    const renderedBodySpans = Array.isArray(renderedBlock?.bodySpans)
      ? renderedBlock.bodySpans
      : [];
    const renderedEvidenceBodyChars = String(renderedBlock?.evidenceText || "").length;
    const renderedBodySpanCoveredChars = renderedBodySpans.reduce((sum, span) => {
      const start = Number(span?.rendered_start_offset);
      const end = Number(span?.rendered_end_offset);
      return Number.isSafeInteger(start) && Number.isSafeInteger(end) && start >= 0 && end > start
        ? sum + end - start
        : sum;
    }, 0);
    const renderedBodyExternalGapChars = Number.isSafeInteger(renderedBlock?.syntheticSeparatorChars) &&
      renderedBlock.syntheticSeparatorChars >= 0
      ? renderedBlock.syntheticSeparatorChars
      : 0;
    const renderedBodySyntheticMarkerChars = renderedBodySpans.reduce((sum, span) =>
      Number.isSafeInteger(span?.synthetic_marker_chars) && span.synthetic_marker_chars >= 0
        ? sum + span.synthetic_marker_chars
        : sum, 0);
    return {
      source_id: displaySourceIdForContextEntry(entry, idx),
      raw_source_id: entry.sourceId || undefined,
      ...
      original_body_count: Number.isFinite(Number(renderedBlock?.originalBodyCount))
        ...
        : 0,
      rendered_evidence_body_chars: renderedEvidenceBodyChars,
      body_span_count: renderedBodySpans.length,
      body_span_origin_bound_count: renderedBodySpans.filter(span =>
        Number.isSafeInteger(span?.literal_original_start) && span.literal_original_start >= 0
      ).length,
      body_span_provenance_bound_count: renderedBodySpans.filter(span =>
        Array.isArray(span?.provenance) && span.provenance.length > 0
      ).length,
      rendered_body_span_covered_chars: renderedBodySpanCoveredChars,
      rendered_body_external_gap_chars: renderedBodyExternalGapChars,
      rendered_body_synthetic_marker_chars: renderedBodySyntheticMarkerChars,
      rendered_body_uncovered_chars: Math.max(
        0,
        renderedEvidenceBodyChars - renderedBodySpanCoveredChars -
          renderedBodyExternalGapChars - renderedBodySyntheticMarkerChars
      ),
      rendered_body_spans: renderedBodySpans,
      source_type: entry.sourceType || undefined,
      collection_id: entry.collectionId || undefined,
      canonical_item_id: entry.canonicalItemId || undefined,
      item_type: entry.itemType || undefined,
  ...
  return {
    ...
    sources,
    retrievalMeta: {
      questionRequirementsShadow: projectQuestionRequirementsShadow(buildQuestionRequirementsShadow({ originalMessage: requirementOriginalMessage, resolvedQuestionPlan: questionPlan })),
      diagnosticHistory: {
        retrieval_input_count: Array.isArray(rawHistory) ? rawHistory.length : 0,
        retrieval_selected_count: Array.isArray(retrievalHistory) ? retrievalHistory.length : 0,
        retrieval_exclusion_reasons: [
          ...(canonicalRetrievalActive ? ["canonical_retrieval"] : []),
          ...(suppressHistoryForExplicitCurrentDocument ? ["explicit_current_document"] : []),
          ...(suppressHistoryForSelfContainedTemporalBreakdown ? ["self_contained_temporal"] : [])
        ]
      },
      ...
      sourceSelectionCandidates: confirmedSourceSelectionCandidates,
      ...
      documentIdentityEvidence: {
        decision: documentIdentityEvidence.decision || null,
        authorTopicEvidence: documentIdentityEvidence.authorTopicEvidence || null,
