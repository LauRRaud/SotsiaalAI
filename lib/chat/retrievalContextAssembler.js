...
function isMunicipalStaffingCountFact(normalized = "") {
  return /\b(mitu|mitme|arv\w*|kui\s+palju)\b/.test(normalized) &&
    /\b(tootaj\w*|töötaj\w*|spetsialist\w*|ametnik\w*|sotsiaalosakon\w*)\b/u.test(normalized);
}

function isCurrentMunicipalStaffingCountFact(normalized = "") {
  return isMunicipalStaffingCountFact(normalized) &&
    /\b(praegu|hetkel|tana|täna|tanase\w*|tänase\w*|ajakohas\w*|kehtiv\w*)\b/u.test(normalized);
}

function isCurrentMunicipalStaffingPresenceRequest(normalized = "") {
  if (/\b(?:19|20)\d{2}\b|\b(oli|tootas|töötas|varem|ajalool\w*)\b/u.test(normalized)) return false;
  return /\b(kas|on|tootab|töötab|tootavad|töötavad)\b/u.test(normalized) &&
    /\b(tootaj\w*|töötaj\w*|spetsialist\w*|ametnik\w*|sotsiaalosakon\w*)\b/u.test(normalized);
}
...
function isMunicipalityContactInventoryRequest(message = "", history = []) {
  ...
  return /\b(koik|kõik|loetle|nimeta|kes|millised|mitu|kui palju)\b/u.test(normalized) &&
    /\b(kontakt\w*|tootaj\w*|töötaj\w*|spetsialist\w*|ametnik\w*)\b/u.test(normalized);
}
...
export async function assembleRetrievalContext({
  ...
  const municipalityStaffingCountRequest = isMunicipalStaffingCountFact(normalizedMunicipalityQuestion);
  const currentMunicipalityStaffingCountRequest = isCurrentMunicipalStaffingCountFact(normalizedMunicipalityQuestion);
  ...
  const municipalityContactRequest =
    allowMunicipalityScopedRag && isMunicipalityContactInventoryRequest(effectiveMessage, rawHistory);
  ...
  const serviceMapKovContactSources = usedServiceMapKovContactContext
    ? buildServiceMapKovContactSources(serviceMapKovContacts)
    : [];
