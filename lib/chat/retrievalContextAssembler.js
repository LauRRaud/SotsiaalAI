...
function researchIdentityCandidateRootEntries(normalizedCandidateText = "") {
  const candidateTokens = String(normalizedCandidateText || "").match(/[a-z0-9]+/gu) || [];
...
export function selectSpecificResearchFactGroups(_message = "", groups = [], questionPlan = {}) {
  ...
  const ranked = candidates.map((group, index) => {
    ...
    const identityEntries = researchIdentityCandidateRootEntries(normalizeIntentText(identityText));
    const titleEntries = researchIdentityCandidateRootEntries(normalizeIntentText(group?.title || ""));
    ...
    const bodyEntries = authorMatched
      ? researchIdentityCandidateRootEntries(normalizedBodyText)
      : [];
    ...
    const identityMatched = sourceCompatible && (
  ...
  const trustedIndex = trustedDocumentId
    ...
    : -1;
  if (trustedIndex > 0 && (!requestedAuthor || ranked[trustedIndex]?.authorMatched)) {
