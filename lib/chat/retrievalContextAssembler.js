...
export function selectSpecificResearchFactGroups(_message = "", groups = [], questionPlan = {}) {
  ...
  const ranked = candidates.map((group, index) => {
    ...
    const exactTitleAnchorMatched = !!titleHint && normalizedTitle === titleHint;
    ...
    Number(right.exactTitleAnchorMatched) - Number(left.exactTitleAnchorMatched) ||
  ...
  const decisiveExactTitleLead = !!(
    best?.exactTitleAnchorMatched && !runnerUp?.exactTitleAnchorMatched
  );
...
function requestedAnswerShapeInstruction(questionPlan = {}, replyLang = "et") {
  ...
  if (replyLang === "en") {
    return [
      ...
      ...slotLines,
      "Answer every requested slot in order. Ground each answer in the selected evidence; if one slot is unsupported, identify that slot instead of silently omitting it or inventing a value."
    ].join("\n");
  }
  if (replyLang === "ru") {
    return [
      "REQUESTED_ANSWER_SHAPE_V2:",
      ...slotLines,
      "Ответь на каждый запрошенный слот по порядку. Каждый ответ обоснуй выбранным источником; если один слот не подтверждён, назови его вместо молчаливого пропуска или выдуманного значения."
    ].join("\n");
  }
  return [
    "REQUESTED_ANSWER_SHAPE_V2:",
    ...slotLines,
    "Vasta igale küsitud slotile järjekorras. Seo iga vastus valitud tõendiga; kui mõni slot pole tõendatud, nimeta see eraldi, selle asemel et see vaikides välja jätta või väärtus välja mõelda."
  ].join("\n");
}
