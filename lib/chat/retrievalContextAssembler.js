...
import {
  ...
  isSingularPreviousSourceFollowup,
...
export function selectSpecificResearchFactGroups(_message = "", groups = [], questionPlan = {}) {
  ...
  const trustedDocumentIdSource = questionPlan?.trusted_document_id_source === "current_turn_author_confirmation"
...
export async function assembleRetrievalContext({
  ...
  const currentTurnRefersToPreviousSource =
    isPluralSourceSetFollowup(retrievalMessage) ||
    isSingularPreviousSourceFollowup(retrievalMessage);
  const suppressHistoryForExplicitCurrentDocument =
    questionPlan?.mode === "specific_research_fact" &&
    !currentTurnRefersToPreviousSource &&
    (explicitCurrentAuthorDocumentAnchor || explicitCurrentKindYearDocumentAnchor);
  ...
  if (shouldRunRag) {
    ...
    if (
      ...
      if (renderedAggregateRows.length >= 2 && renderedAnnualCoverageIncomplete) {
        let developmentChosen = chosen;
        const primaryTemporalGroup = budgeted.used[0];
        ...
        const latestTargetYear = temporalRetrievalPlan.years.length
          ...
          : null;
        // Keep room for both primary-document spans and one exact development
        // unit from each of up to four additional source groups.
        const developmentMaxGroups = Math.min(
          Math.max(1, Number(baseContextBudgetOptions.maxGroups) || CONTEXT_GROUPS_MAX),
          5
        );
        const companionGroup = groupedMatches
          .map((group, groupIndex) => {
            ...
            const developmentSpan = (group.bodies || [])
              .map(body => preferredTemporalQualitativeDevelopmentSpan(body, {
            ...
            return developmentSpan ? { group, groupIndex, developmentSpan } : null;
          })
          .filter(Boolean)
          .sort((left, right) =>
        ...
        const companionChosenIndex = companionGroup
          ? chosen.findIndex(group => group.docId === companionGroup.docId)
          : -1;
        const companionAlreadyRendered = companionGroup
          ? chosen.slice(0, developmentMaxGroups).some(group => group.docId === companionGroup.docId)
          : false;
        if (companionGroup && !companionAlreadyRendered && budgeted.used.length > 1) {
          ...
          let replaceIndex = -1;
          const renderedChoiceCount = Math.min(chosen.length, budgeted.used.length, developmentMaxGroups);
          for (let index = renderedChoiceCount - 1; index > 0; index -= 1) {
            if (protectedAnnualSourceIds.has(chosen[index].sourceId || chosen[index].key)) continue;
            replaceIndex = index;
            break;
          }
          if (replaceIndex >= 0) {
            const nextChosen = [...chosen];
            if (companionChosenIndex >= 0) {
              [nextChosen[replaceIndex], nextChosen[companionChosenIndex]] = [
                nextChosen[companionChosenIndex],
                nextChosen[replaceIndex]
              ];
            } else {
              nextChosen[replaceIndex] = companionGroup;
            }
            developmentChosen = nextChosen;
          }
        }
        const developmentBudget = buildContextWithBudget(developmentChosen, {
          ...baseContextBudgetOptions,
          maxGroups: developmentMaxGroups,
          secondaryMaxBodies: journalChunksPerDocument > 3 ? 2 : 1,
          secondaryBodyMaxChars: journalChunksPerDocument > 3 ? 1100 : 490,
          preferTemporalDevelopment: true,
          secondaryTemporalDevelopmentRequiresTitleTopicMatch: true,
          primaryTemporalDevelopmentMinimumEvidenceYearExclusive: Number.isInteger(latestTargetYear)
            ? latestTargetYear
            : null
        });
        const developmentPrimarySource = [{
          source_id: developmentBudget.used[0]?.sourceId || developmentBudget.used[0]?.key,
          evidenceText: String(developmentBudget.renderedBlocks?.[0]?.text || "")
        }];
        const developmentAggregateRows = buildTemporalAggregatePeriodRows({
          sources: developmentPrimarySource,
          targetYears: temporalRetrievalPlan.years
        });
        const aggregateRowKey = row => JSON.stringify([
          ...
          (Array.isArray(row?.metric_tokens) ? row.metric_tokens : [])
            ...
            .filter(Boolean)
            .sort(),
          String(row?.evidence_unit || "")
            .replace(/\s+/gu, " ")
            .trim()
            .toLocaleLowerCase("et")
        ]);
        const renderedAggregateRowKeys = new Set(renderedAggregateRows.map(aggregateRowKey));
        const developmentAggregateRowKeys = new Set(developmentAggregateRows.map(aggregateRowKey));
        ...
        if (developmentAggregateRows.length >= 2 && preservesRenderedAggregateRows) {
          chosen = developmentChosen;
          budgeted = developmentBudget;
        }
      }
    }
