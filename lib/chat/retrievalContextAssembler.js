  ...
  if (shouldRunRag) {
    ...
    if (
      ...
      if (renderedAggregateRows.length >= 2 && renderedAnnualCoverageIncomplete) {
        ...
        const companionGroup = groupedMatches.find(group =>
          group.docId &&
          ...
          (group.bodies || []).some(body => preferredTemporalQualitativeDevelopmentSpan(body, {
        ...
        if (developmentAggregateRows.length >= 2) {
