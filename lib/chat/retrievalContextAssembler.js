...
const REQUESTED_METRIC_CONTRACT_MAPPING_METHOD = "bounded_rendered_sentence_peer_alignment_v9";
...
function requestedMetricContractInstruction(trace = null, replyLang = "et") {
  ...
  const slotLines = trace.slots.map(slot => {
    ...
    return `slot_${slot.slot_index}: ${slot.value_type}=${slot.evidence_value}${rangeEnd}${unit}${scope}${qualifier}${cardinality}${relation}`;
  });
  if (replyLang === "en") {
    return [
