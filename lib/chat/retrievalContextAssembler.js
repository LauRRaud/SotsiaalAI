...
function requestedMetricBodyAlignment(slots = [], body = "") {
  ...
  const { assignment, ambiguous } = bestRequestedMetricAssignment(slots, evidence.candidates);
  ...
  return { coveredSlotCount: 0, alignmentScore: 0 };
}
...
const REQUESTED_METRIC_CONTRACT_MAPPING_METHOD = "bounded_rendered_sentence_peer_alignment_v6";
...
function requestedMetricCandidateScore(slot = {}, candidate = {}) {
  ...
  return {
    ...
    subjectHead: immediateFollowingToken?.value || ""
  };
}
  ...
  for (const slotIndexes of coordinationGroups.values()) {
    ...
    const subjectAppearsInEarlierSlot = peersShareSubjectHead && qualified
      ...
      .some((option, earlierSlotIndex) => {
        ...
        if (Number.isInteger(earlierCoordinationGroup) && earlierCoordinationGroup > 0) return false;
        return requestedMetricTermMatchQuality(firstSubjectHead, option?.subjectHead || "") >= 0.86;
      });
...
function bestRequestedMetricAssignment(slots = [], candidates = []) {
  ...
  const visit = (slotIndex, usedCandidateKeys, assignment, totalScore) => {
    if (slotIndex >= slots.length) {
      ...
      const firstFragmentIndex = qualifiedAssignment[0]?.candidate?.fragmentIndex;
      const lastFragmentIndex = qualifiedAssignment.at(-1)?.candidate?.fragmentIndex;
      ...
      const evidenceCharSpan = Number.isFinite(qualifiedAssignment[0]?.candidate?.absoluteStart) &&
        Number.isFinite(qualifiedAssignment.at(-1)?.candidate?.absoluteEnd)
        ? qualifiedAssignment.at(-1).candidate.absoluteEnd - qualifiedAssignment[0].candidate.absoluteStart
        : Number.POSITIVE_INFINITY;
      ...
      if (coordinationGroup) {
        ...
        if (
          ...
          (
            ...
            option.candidate.relationScopeIndex !== peerCandidate.relationScopeIndex ||
  ...
  const conflictingSolution = solutions.find(solution => best.assignment.some((option, index) =>
    option.candidate.value !== solution.assignment[index]?.candidate?.value
  ));
  const ambiguous = !!(
    conflictingSolution &&
...
function buildCurrentTurnAuthorConfirmation(
  ...
  const candidate = identityCandidate?.author && typeof identityCandidate.author === "object"
    ? identityCandidate.author
    : null;
  const candidateValue = String(candidate?.value || "").trim() || null;
  const candidateProvenance = candidate?.provenance === "explicit_current_turn"
    ? "explicit_current_turn"
    : null;
  const candidateConfidence = ["high", "medium"].includes(candidate?.confidence)
  ...
  const base = {
    ...
    status: candidateValue ? "no_author_metadata" : "no_candidate",
  ...
  const entries = (Array.isArray(retrievedGroups) ? retrievedGroups : [])
    ...
    .filter(entry => entry.authors.length);
  ...
  for (const entry of entries) {
    ...
    for (const author of entry.authors) {
      status = authorMetadataMatchesCandidate(candidateValue, author);
      if (status) {
        matchedAuthor = author;
        break;
      }
      if (transliterationCandidate && authorMetadataMatchesCandidate(transliterationCandidate, author)) {
        status = "confirmed_existing_transliteration";
        matchedAuthor = author;
        break;
      }
    }
    if (status && matchedAuthor) matched.push({
  ...
  const matchedStatuses = new Set(matched.map(entry => entry.status));
  const status = matchedStatuses.has("confirmed_exact")
    ? "confirmed_exact"
  ...
  const matchedAuthorValues = Array.from(new Set(
    matched.map(entry => String(entry.author || "").trim()).filter(Boolean)
  )).slice(0, 8);
  return {
    ...base,
    status,
    ...
    promotion_eligible: candidateProvenance === "explicit_current_turn" && matchedDocumentIds.length > 0
  };
}
...
export async function assembleRetrievalContext({
  ...
  const evidencePackage = shouldBuildEvidencePackage({
    ...
    : null;
  const currentTurnAuthorConfirmation = buildCurrentTurnAuthorConfirmation(
