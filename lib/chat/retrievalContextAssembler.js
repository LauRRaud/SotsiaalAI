...
export function selectSpecificResearchFactGroups(_message = "", groups = [], questionPlan = {}) {
  ...
  const ranked = candidates.map((group, index) => {
    ...
    const documentAnchorMatches = explicitDocumentAnchorTerms.filter(term =>
      researchIdentityTermMatches(term, identityText)
    );
    ...
    const identityMatched = sourceCompatible && (requestedAuthor
      ...
      : subjectTerms.length >= 2 && subjectMatches.length >= 2) || !!(
        sourceCompatible && explicitDocumentAnchorTerms.length >= 2 && documentAnchorMatches.length >= 2
      ) || !!(
  ...
  if (best && !best.identityMatched && (decisiveDocumentAnchorLead || decisiveSubjectLead || decisiveStrongAnchorLead || decisiveRankedTitleLead)) {
