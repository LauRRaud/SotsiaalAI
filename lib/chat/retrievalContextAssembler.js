...
function buildServiceMapKovContactInstruction(replyLang = "et", options = {}) {
  ...
  if (replyLang === "en") {
    return [
      ...
      municipalityOverview
        ? "For a general municipal services or department overview, explain that the right specialist depends on the topic. Do not arbitrarily present one or two people as the default contacts: show the available roles and a diverse, role-labelled contact selection, and state clearly if the list is not exhaustive."
  ...
  return [
    ...
    municipalityOverview
      ? "KOV teenuste voi osakonna uldulevaates utle, et sobiv spetsialist soltub teemast. Ara esita meelevaldselt ainult uht-kaht inimest vaikimisi kontaktidena: kuva olemasolevad rollid ja mitmekesine rollidega margistatud kontaktivalik ning utle selgelt, kui loetelu ei ole ammendav."
