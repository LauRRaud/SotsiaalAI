...
export function selectSpecificResearchFactGroups(_message = "", groups = [], questionPlan = {}) {
  ...
  const ranked = candidates.map((group, index) => {
    ...
    const identityEntries = researchIdentityCandidateRootEntries(normalizedIdentityText);
    ...
    const evidenceMetricMatches = boundedEpisodeIdentity
      ? effectiveEvidenceMetricSlots.filter(slot =>
          slot.terms.some(term => researchIdentityTermMatchesEntries(term, bodyEntries))
        ).map(slot => slot.category)
      : [];
    const sourceYearMatches = boundedEpisodeIdentity
      ? requestedSourceYears.filter(year => new RegExp(`(?<![a-z0-9])${year}(?![a-z0-9])`, "u").test(normalizedIdentityText))
      : [];
    ...
    const boundedEpisodeCoLocated = !!(
      ...
      evidencePeriodYearMatches.length === evidencePeriodYears.length &&
      effectiveEvidenceMetricSlots.length >= 3 &&
      evidenceMetricMatches.length >= Math.min(3, effectiveEvidenceMetricSlots.length)
    );
