...
function requestedProportionSlotPlan(questionPlan = {}, {
  ...
  if (
    ...
    Number(requested?.emitted_slot_count) !== slots.length ||
    slots.length < 2 ||
    slots.length > REQUESTED_METRIC_CONTRACT_MAX_SLOTS ||
    slots.some(slot => slot?.value_type !== "proportion" || slot?.input_form !== "original")
  ) return [];
  return slots;
}

function requestedMetricBodyAlignment(slots = [], body = "") {
  const evidence = requestedMetricEvidenceCandidates(body, "proportion");
  const inheritedScopeTerm = requestedMetricToken(slots[0]?.relation_terms?.[0] || "");
  const scoredOptions = slots.flatMap((slot, slotIndex) => evidence.candidates
    .map(candidate => requestedMetricCandidateScore(slot, candidate, inheritedScopeTerm))
    .filter(Boolean)
    .map(option => ({ ...option, slotIndex })))
    .sort((left, right) => right.score - left.score);
  const assignedSlots = new Set();
  const usedCandidates = new Set();
  let alignmentScore = 0;
  for (const option of scoredOptions) {
    if (assignedSlots.has(option.slotIndex) || usedCandidates.has(option.candidate.key)) continue;
    assignedSlots.add(option.slotIndex);
    usedCandidates.add(option.candidate.key);
    alignmentScore += option.score;
  }
  return { coveredSlotCount: assignedSlots.size, alignmentScore };
}

export function prioritizeRequestedMetricSlotEvidence(questionPlan = {}, groups = [], options = {}) {
  const slots = requestedProportionSlotPlan(questionPlan, options);
  const candidates = Array.isArray(groups) ? groups : [];
...
export async function assembleRetrievalContext({
  ...
  if (shouldRunRag) {
    ...
    const requestedMetricSlotEvidenceActive = requestedProportionSlotPlan(
      questionPlan,
