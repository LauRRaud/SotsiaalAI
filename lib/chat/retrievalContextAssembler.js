...
function buildEvidencePreservationInstruction(replyLang = "et") {
  if (replyLang === "en") {
    return [
      ...
      "Attribute the example to its source and time. If its current operational status is not verified, state that limitation separately instead of erasing the source-reported evidence.",
  ...
  if (replyLang === "ru") {
    return [
      ...
      "Укажи источник и время примера. Если его текущий рабочий статус не подтверждён, сообщи об этом отдельно, не стирая сведения, приведённые источником.",
  ...
  return [
    ...
    "Seo näide allika ja ajaga. Kui selle praegune kasutusstaatus ei ole kinnitatud, ütle see piirang eraldi, mitte ära jäta allikas kirjeldatud näidet välja.",
