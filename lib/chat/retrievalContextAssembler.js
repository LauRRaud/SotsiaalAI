...
function specificResearchDocumentLockEligible(questionPlan = {}, identityEvidence = null) {
  ...
  return exactTitleConfirmed || authorConfirmationTrusted || (allAuthorsConfirmed && explicitYearConfirmed);
}
...
export function selectSpecificResearchFactGroups(_message = "", groups = [], questionPlan = {}) {
  ...
  const trustedCandidateMatchesCurrentTitle = !explicitCurrentTitleHint ||
    ranked[trustedIndex]?.exactTitleAnchorMatched === true;
  if (
    trustedIndex > 0 &&
  ...
  const trustedDocumentMatch = !!(
    ...
    (!explicitCurrentTitleHint || best.exactTitleAnchorMatched) &&
...
function requestedFactQualitativeEvidenceBinding(slot = {}, body = "") {
  ...
  for (let index = 0; (phoneMode || ageMode || valueType === "entity_list") && index < fragments.length - 1; index += 1) {
...
function requestedFactBodyAlignment(slots = [], body = "") {
  ...
  slots.forEach((slot, slotIndex) => {
    ...
    if (
      ...
      Number(extractUniformParticipantBreakdown(body)?.groupCardinality || 0) < expectedCardinality
    ) return;
...
function requestedMetricMentionIsStructural(fragment = "", start = 0, end = 0) {
  ...
  return bracketedReference || leadingListMarker;
}
...
function requestedMetricContractInstruction(trace = null, replyLang = "et") {
  ...
  const slotLines = trace.slots.map(slot => {
    ...
    const scope = Array.isArray(slot?.scope_values) && slot.scope_values.length
      ...
      : "";
    return `slot_${slot.slot_index}: ${slot.value_type}=${slot.evidence_value}${unit}${scope}`;
  });
  if (replyLang === "en") {
    return [
...
function buildRequestedFactSlotContract({
  ...
  for (const group of (Array.isArray(renderedGroups) ? renderedGroups : []).slice(0, 8)) {
    ...
    const slots = assignment.map((option, index) => ({
      ...
      relation_terms: Array.isArray(requestedSlots[index]?.relation_terms)
        ? requestedSlots[index].relation_terms.slice(0, 8)
        : [],
      relation_term_variants: Array.isArray(requestedSlots[index]?.relation_term_variants)
        ? requestedSlots[index].relation_term_variants.slice(0, 8)
        : [],
...
function groupMatchesCurrentTurnDocumentAnchors(group = {}, identityCandidate = null) {
  ...
  const explicitTitleHint = identityCandidate?.title_hint?.provenance === "explicit_current_turn"
    ...
    : "";
  if (explicitTitleHint && normalizeIntentText(group?.title || "") !== explicitTitleHint) return false;
