...
function buildServiceMapKovContactInstruction(replyLang = "et", options = {}) {
  const municipalityOverview = options.municipalityOverview === true;
  if (replyLang === "en") {
    return [
      ...
      municipalityOverview
        ? "For a general municipal services or department overview, explain that the right specialist depends on the topic and describe the different available professional roles. Do not name one or two people, phone numbers, or email addresses as default contacts. If the user explicitly asks for all contacts, list every available contact grouped by role; otherwise ask for the topic before naming a person."
        : "When answering about a specific municipal service or benefit, add only a contact confirmed for that service when it helps the user take the next step."
    ].join("\n");
  }
  return [
    "SERVICE_MAP_CONTACT_MODE:",
    ...
    municipalityOverview
      ? "KOV teenuste voi osakonna uldulevaates utle, et sobiv spetsialist soltub teemast, ja kirjelda olemasolevate ametiroollide mitmekesisust. Ara nimeta uht-kaht inimest, telefoninumbrit ega e-posti vaikimisi kontaktina. Kui kasutaja kusib selgelt koiki kontakte, loetle koik olemasolevad kontaktid rollide kaupa; muul juhul kusi enne inimese nimetamist tema teemat."
      : "Konkreetse KOV teenuse voi toetuse vastuses lisa ainult selle teenuse jaoks kinnitatud kontakt, kui see aitab kasutajal edasi poorduda."
  ].join("\n");
}
...
export async function assembleRetrievalContext({
  ...
  const GRAPH_CHANNEL_EXCLUDED_MODES = new Set([
    ...
    "national_source_lookup", "source_lookup", "temporal"
  ]);
  ...
  if (useServiceMapKovContactContext) {
    extraSystemInstructions.push(buildServiceMapKovContactInstruction(replyLang, {
      municipalityOverview: municipalityServiceBenefitListRequest
    }));
