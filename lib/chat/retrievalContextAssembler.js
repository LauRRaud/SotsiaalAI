...
const REQUESTED_METRIC_CONTRACT_MAPPING_METHOD = "bounded_ordered_peer_scope_alignment_v3";
...
function requestedMetricCandidateScore(slot = {}, candidate = {}) {
  ...
  const immediateFollowingTokens = candidate.tokens.filter(token =>
    token.start >= candidate.end && token.start - candidate.end <= 40
  );
  const localRelationHeadMatched = immediateFollowingTokens.some(token =>
    relationTerms.some(relationTerm =>
      requestedMetricTermMatchQuality(relationTerm, token.value) >= 0.72
    )
  );
...
function bestRequestedMetricAssignment(slots = [], candidates = []) {
  ...
  const scoredOptionsBySlot = slots.map(slot => candidates
    .map(candidate => requestedMetricCandidateScore(slot, candidate))
    .filter(Boolean)
