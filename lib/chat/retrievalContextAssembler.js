...
export async function assembleRetrievalContext({
  ...
  const suppressHistoryForExplicitCurrentDocument = specificResearchFactQuestion &&
    currentTurnDocumentIdentity?.author?.provenance === "explicit_current_turn" &&
    currentTurnDocumentIdentity?.author?.confidence === "high" &&
    (
      currentTurnIdentitySourceYears(currentTurnDocumentIdentity).length > 0 ||
      String(currentTurnDocumentIdentity?.title_hint?.value || "").trim().length > 0
    );
  const retrievalHistory = canonicalRetrievalActive || suppressHistoryForExplicitCurrentDocument
    ? []
    : rawHistory;
