...
export async function assembleRetrievalContext({
  ...
  const thematicSynthesisQuestion = !sourceLookupRequest && questionPlan.mode !== "professional_method_guidance" && isThematicSynthesisRagQuestion(effectiveMessage);
  const broadMultiSourceQuestion = !sourceLookupRequest && questionPlan.mode !== "professional_method_guidance" && isBroadMultiSourceRagQuestion(effectiveMessage);
  ...
  if (shouldRunRag) {
    ...
    if (legalLookupPlan?.enabled && legalLookupPlan.mode === "explicit_paragraph") {
      ...
      const methodSelection = selectProfessionalMethodGuidanceGroups(
        ...
        DIVERSIFY_LAMBDA,
        { focus: questionPlan.method_guidance_focus, topicHints }
      );
