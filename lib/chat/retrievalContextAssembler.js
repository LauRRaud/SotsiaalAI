...
export function selectSpecificResearchFactGroups(_message = "", groups = [], questionPlan = {}) {
  ...
  const requestedMetricSlots = Array.isArray(
    questionPlan?.semantic_candidates?.requested_numeric_slots?.slots
  )
    ? questionPlan.semantic_candidates.requested_numeric_slots.slots
  ...
  const ranked = candidates.map((group, index) => {
    ...
    if (requestedMetricShape.complete) score += 6 + Math.min(4, requestedMetricShape.score / 4);
    ...
    const anonymousIdentityMatched = (
      ...
      requestedMetricShape.complete && subjectMatches.length >= 1 && titleSubjectMatches.length >= 1
    );
    ...
    Number(right.requestedMetricShape.complete) - Number(left.requestedMetricShape.complete) ||
    right.requestedMetricShape.score - left.requestedMetricShape.score ||
  ...
  const decisiveRequestedMetricShapeLead = !!(
    best?.sourceCompatible &&
    best.requestedMetricShape.complete &&
    best.subjectMatches.length >= 1 &&
    best.titleSubjectMatches.length >= 1 &&
    (!runnerUp?.requestedMetricShape.complete || best.requestedMetricShape.score >= runnerUp.requestedMetricShape.score + 2)
  );
  if (
    best &&
    !best.identityMatched &&
    ...
    (
      ...
      decisiveRequestedMetricShapeLead
    )
  ) {
  ...
  return {
    ...
    reasons: best ? [
      ...
      ...(best.requestedMetricShape.complete ? ["requested_metric_shape_complete"] : []),
      ...(decisiveRequestedMetricShapeLead ? ["decisive_requested_metric_shape_lead"] : []),
...
function requestedFactSlotsFromQuestionPlan(questionPlan = {}) {
  ...
  const slots = Array.isArray(requested?.slots)
    ? requested.slots.map(slot => ({ ...slot, slot_source: "requested_numeric_slots" }))
    : [];
  return {
    complete: requested?.complete === true,
    truncated: requested?.truncated === true,
    unresolvedClauseCount: Number(requested?.unresolved_clause_count || 0),
    emittedSlotCount: Number(requested?.emitted_slot_count || 0),
    slotSource: "requested_numeric_slots",
    slots
  };
}
...
const REQUESTED_METRIC_CONTRACT_MAPPING_METHOD = "bounded_rendered_sentence_peer_alignment_v7";
  ...
  fragments.forEach((fragmentEntry, fragmentIndex) => {
    ...
    if (nonProportionTypes.length) {
      const numericMentions = Array.from(fragment.matchAll(
        /(?<![\p{L}\p{N}%§])(\d+(?:[.,]\d+)?)(?:\s*(eurot?|eur|€|paeva|päeva|nadala|nädala|kuud?|aastat?))?(?![\p{L}\p{N}%])/gu
      ));
      numericMentions.forEach((mention, mentionIndex) => {
        ...
        if (Number.isInteger(numericValue) && numericValue >= 1900 && numericValue <= 2100) return;
        const unitText = String(mention[2] || "").trim();
        const inferredType = /^(?:eurot?|eur|€)$/iu.test(unitText)
          ? "amount"
          : /^(?:paeva|päeva|nadala|nädala|kuud?|aastat?)$/iu.test(unitText)
            ? "duration"
...
function requestedMetricCandidateScore(slot = {}, candidate = {}) {
  ...
  const minimumMatches = slot?.slot_source === "evidence_metric_slots"
    ? 1
    : Math.max(2, Math.ceil(relationTerms.length * 0.45));
...
function requestedMetricContractInstruction(trace = null, replyLang = "et") {
  ...
  const slotLines = trace.slots.map(slot => {
    ...
    return `slot_${slot.slot_index}: ${slot.value_type}=${slot.evidence_value}${unit}`;
  });
  if (replyLang === "en") {
    return [
...
function buildRequestedFactSlotContract({
  questionPlan = null,
  renderedGroups = [],
  replyLang = "et",
  specificResearchFactQuestion = false,
