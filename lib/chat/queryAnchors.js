...
function isPlainSentenceInitialToken(text = "", index = 0, token = "") {
  if (/[\p{Ll}][\p{Lu}]|^[\p{Lu}0-9]{2,}$/u.test(token)) return false;
...
export function extractExactQueryAnchors(message = "", options = {}) {
  ...
  for (const match of text.matchAll(/[\p{Letter}\p{Number}_-]+/gu)) {
    ...
    // Ordinary title case at the start of any sentence is grammatical, not a
    // reliable proper-name signal. Multiword names still retain their later
    // capitalized tokens, while acronyms and camel-case identifiers stay exact.
    if (isPlainSentenceInitialToken(text, index, token)) continue;
    if (looksLikeExactName(token)) add(token);
