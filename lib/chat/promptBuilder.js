...
export function langStrings(lang = "et", role = "CLIENT") {
  ...
  if (lang === "ru") {
    return {
      ...
      retrievalFailed: serverT(
        ...
        retrievalFailedKey,
        undefined,
        "Поиск по материалам в этот раз не завершился, поэтому я не смог использовать источники. Пожалуйста, отправьте тот же вопрос ещё раз."
      ),
  ...
  if (lang === "en") {
    return {
      ...
      retrievalFailed: serverT(
        ...
        retrievalFailedKey,
        undefined,
        "The search over the materials did not finish this time, so I could not use any sources. Please send the same question again."
      ),
  ...
  return {
    ...
    retrievalFailed: serverT(
      ...
      retrievalFailedKey,
      undefined,
      "Materjalide otsing ei jõudnud seekord lõpule, seega ma ei saanud allikaid kasutada. Palun saada sama küsimus kohe uuesti."
    ),
