...
export async function createRagAttempt(tx, turn, now = new Date(), leaseMs = RAG_ATTEMPT_LEASE_MS, sourceSelectionBinding = null) {
  return tx.ragAttempt.create({ data: {
    ...
    evidence: projectAttemptEvidence({ stages: [{ stage: "claimed", elapsed_ms: 0 }], source_selection_binding: sourceSelectionBinding })
  } });
}
