...
function asksForNumericFact(message = "") {
  ...
  return /%|(?:^|[^\p{L}\p{N}])(?:arv(?:u|ud|ust|uga)?|koguarv\p{L}*|uldarv\p{L}*|kokku|kui\s+palju|kui\s+suur\s+osa|kui\s+kaua|kestus\p{L}*|mitu|nait(?:aja|ude?)\p{L}*|osakaal\p{L}*|protsent\p{L}*|mis\s+aastal|millisel\s+aastal|how\s+many|how\s+long|duration|number\s+of|total|percentage|percent|what\s+year|which\s+year|сколько|как\s+долго|длительность|число|итого|всего|процент\p{L}*|каком\s+году)(?=$|[^\p{L}\p{N}])/u.test(normalized);
}
...
function requestedMetricClaimAssignment(message = "", reply = "", contract = null) {
  ...
  const unexpectedClaimValues = claims
    .filter((claim, index) =>
      ...
      !userScopeValues.has(claim.value)
    )
    .map(claim => claim.value);
  return {
