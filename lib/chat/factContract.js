...
function requestedMetricClaimAssignment(message = "", reply = "", contract = null) {
  ...
  const entries = slots.map((slot, contractIndex) => {
    ...
    const expectedQualifier = ["over", "under", "at_least", "at_most", "about"].includes(slot?.qualifier)
      ? slot.qualifier
      : null;
    const expectedCardinality = Number.isInteger(Number(slot?.expected_cardinality)) &&
      ...
      : null;
    const valueMatches = claims.flatMap((claim, claimIndex) =>
  ...
  const visit = (position, usedClaimIndexes, selected) => {
    ...
    for (const edge of entry.scopedMatches) {
      if (usedClaimIndexes.has(edge.claimIndex)) continue;
      usedClaimIndexes.add(edge.claimIndex);
      selected.push({ ...edge, entry });
      visit(position + 1, usedClaimIndexes, selected);
      selected.pop();
      usedClaimIndexes.delete(edge.claimIndex);
    }
  };
  visit(0, new Set(), []);
  if (!solutions.length) {
  ...
  const assignedClaimIndexes = new Set(selected.map(edge => edge.claimIndex));
