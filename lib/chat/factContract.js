...
function requestedMetricRelationTermDistance(
  ...
  const text = normalizeText(stripStructuralNumbers(localScope?.text || ""));
...
function requestedMetricUniqueRelationBoundToClaim(localScope = null, claim = null, slot = {}, slots = []) {
  ...
  const peerTermsFor = peer => categoryModifiers.length
    ? requestedMetricRequiredPeerModifiers(peer, slots)
    : requestedMetricDiscriminatingTerms(peer, slots);
