...
function requestedFactSlotsFromRetrievalMeta(retrievalMeta = null) {
  const requested =
    retrievalMeta?.queryPlan?.question_planner?.semantic_candidates?.requested_fact_slots ||
    retrievalMeta?.queryPlan?.semantic_candidates?.requested_fact_slots ||
    null;
...
function requestedQualitativeUnitBinding(slot = {}, unit = null) {
  ...
  if (matchedRelationTerms.length < minimumRelationMatches) return null;
