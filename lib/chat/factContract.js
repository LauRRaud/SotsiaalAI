...
function temporalUnitStatesEvidenceUnavailable(value = "") {
  ...
  return /(?:ei\s+(?:ole|leidu|sisaldu|kajastu|saa|voimalda)|pole|puudu\p{L}*|kinnitamata|toendamata|not\s+(?:available|reported|provided|shown|confirmed|proven)|unavailable|cannot\s+(?:confirm|establish|infer)|no\s+year-specific|insufficient\s+evidence|нет|не\s+(?:указан\p{L}*|приведен\p{L}*|подтвержден\p{L}*|доказан\p{L}*)|недостаточно\s+(?:данных|доказательств))/u.test(normalized);
}

function validateTemporalClaimBindings({ reply = "", claims = [], contract = null, eligibleSources = [] } = {}) {
    ...
    return {
      ...
      trace: {
        ...
        temporal_missing_years: Array.isArray(temporalValidation.missingTargetYears)
          ? temporalValidation.missingTargetYears
          : [],
        year_mode: "temporal_year_value_source_rows",
        whole_scope_checked: true
      }
    };
  }
