...
function splitEvidence(source = {}, index = 0) {
  ...
  return {
    ...
    header,
    body,
    allNumbers: new Set(numericClaims(evidenceText).map(claim => claim.value)),
...
function extractWholeScopeNumbers(body = "") {
  ...
  for (const sentence of sentences) {
    const claims = numericClaims(sentence).filter(claim => !claim.year);
...
export function validateExactFactAnswer({
  ...
  const supportingSources = evidenceSources.filter(source =>
  ...
  if (!supportingSources.length) {
    const individuallySupported = claims.every(claim =>
      evidenceSources.some(source => source.allNumbers.has(claim.value))
    );
    return {
      passed: false,
