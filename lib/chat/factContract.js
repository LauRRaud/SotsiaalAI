...
export function asksForParticipantGroupNumericRelation(message = "") {
  const normalized = normalizeText(message);
  const countOrProportionCue = /%|(?:^|[^\p{L}])(?:arv\p{L}*|kokku|kui\s+palju|mitu|osakaal\p{L}*|protsent\p{L}*|count\p{L}*|how\s+many|number\p{L}*|total|share|percent\p{L}*|proportion\p{L}*|сколько|числ\p{L}*|всего|итого|дол\p{L}*|процент\p{L}*)(?=[^\p{L}]|$)/u.test(normalized);
  return countOrProportionCue &&
    /(?:^|[^\p{L}])(?:categor\p{L}*|group\p{L}*|participant\p{L}*|role\p{L}*|kategoor\p{L}*|osal\p{L}*|\p{L}*ruhm\p{L}*|roll\p{L}*|категор\p{L}*|групп\p{L}*|рол\p{L}*|участ\p{L}*)(?=[^\p{L}]|$)/u.test(normalized);
}
  ...
  return {
    ...
    structured_observations: {
      ...
      evidence_period_years: Array.isArray(shadowFieldValue(contract, "evidence_period_years"))
        ...
        : null,
      evidence_phase_ordinal: typeof phaseOrdinal === "string"
        ? phaseOrdinal.slice(0, 40)
        : null,
