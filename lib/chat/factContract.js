...
function requestedQualitativeActionObjectAssignment(slot = {}, unitText = "") {
  ...
  const clauseActions = qualitativeActionClauses(unitText)
    .map((text, clauseIndex) => {
      ...
      const firstAction = qualitativeActionSignature(answerText).terms[0];
      const actionIndex = answerTokens.indexOf(firstAction);
      const localPronoun = actionIndex >= 0 && answerTokens.slice(Math.max(0, actionIndex - 1), actionIndex + 3)
        .some(token => /^(?:seda|selle|neid|nende|it|this|them)$/u.test(token));
