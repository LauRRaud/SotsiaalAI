...
export function buildTemporalSupplementalSourceScopes({
  ...
  for (const [sourceIndex, rawSource] of (Array.isArray(sources) ? sources : []).entries()) {
    ...
    if (trendRequested) {
      ...
      if (typedScopes.has(relation)) continue;
      if (
        !samePrimaryDocument &&
  ...
  if (trendRequested) {
    return [
      typedScopes.get("companion_source_context"),
      typedScopes.get("same_document_later_development")
    ].filter(Boolean);
  }
  return scopes;
}
