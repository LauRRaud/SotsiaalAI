...
const CATEGORY_ENTITY_FAMILY_PATTERNS = [
  ...
  ["county", /^(?:maakond\p{L}*|counties|county|уезд\p{L}*)$/u],
...
function isEntityCoverageCategoryLabel(value = "") {
  ...
  const estonianPluralRoleHead = /^(?:inimesed|isikud|kohtunikud|osalejad|praktikud|sotsiaaltootajad|tootajad|vastajad)$/u.test(finalToken);
  const hasEntityRoleSeparator = /(?:\s[-–—]\s|:)/u.test(normalized);
  if (hasLeadingEntityScope && (hasEntityRoleSeparator || estonianPluralRoleHead)) {
    return true;
  }
...
function asksForCategoricalNumericBreakdown(message = "") {
  const normalized = normalizeText(message);
  const explicitCategoryCue = /(?:^|[^\p{L}])(?:categor\p{L}*|group\p{L}*|participant\p{L}*|role\p{L}*|kategoor\p{L}*|osal\p{L}*|\p{L}*ruhm\p{L}*|roll\p{L}*|категор\p{L}*|групп\p{L}*|рол\p{L}*|участ\p{L}*)(?=[^\p{L}]|$)/u.test(normalized);
