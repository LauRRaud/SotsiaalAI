...
export function smallCardinalNumberValue(rawWord = "", followingText = "") {
...
function normalizedNumericClaims(value = "", metricSlots = []) {
  ...
  for (const match of normalized.matchAll(/(?<![\p{L}\d])[\p{L}]+(?![\p{L}\d])/gu)) {
    ...
    const mappedValue = smallCardinalNumberValue(word, followingText);
...
function requestedAnswerTermMatchQuality(left = "", right = "") {
  const a = requestedAnswerTermToken(left);
  const b = requestedAnswerTermToken(right);
  if (!a || !b) return 0;
  if (a === b) return 1;
...
function requestedAnswerTokens(value = "") {
  return Array.from(normalizeText(stripStructuralNumbers(value)).matchAll(/[\p{L}\p{N}-]+/gu), match =>
    requestedAnswerTermToken(match[0])
  ).filter(Boolean);
}
...
const REQUESTED_METRIC_RELATION_GENERIC_RE = /^(?:aasta\p{L}*|alla|arv\p{L}*|hulgas|inime\p{L}*|ja|kui|kokku|mitu|ning|oli|on|osakaal\p{L}*|palju|protsent\p{L}*|seas|suur\p{L}*|ule)$/u;
...
function requestedMetricClauseCommaBoundaries(reply = "", slots = []) {
  const text = String(reply || "");
  ...
  for (const match of text.matchAll(/,/gu)) {
    ...
    const nextComma = text.indexOf(",", commaIndex + 1);
    if (nextComma >= 0) nextHardBoundaries.push(nextComma);
    ...
    const rightSegment = text.slice(commaIndex + 1, hardEnd);
    ...
    const startsNewMetricRelation = (Array.isArray(slots) ? slots : [])
      .some(slot => requestedMetricSegmentMatchesRelation(rightSegment, slot));
    if (hasClaimBefore && hasContractEvidenceValue && startsNewMetricRelation) boundaries.push(commaIndex);
  }
  return boundaries;
}
...
function requestedMetricClaimLocalScope(reply = "", claim = null, slots = []) {
  ...
  const nextBoundaries = ["\n", ";", ".", "!", "?"]
    ...
    .filter(index => index >= 0);
  const commaBoundaries = requestedMetricClauseCommaBoundaries(text, slots);
  previousBoundaries.push(...commaBoundaries.filter(index => index < claimIndex));
  nextBoundaries.push(...commaBoundaries.filter(index => index > claimIndex));
...
function requestedMetricDiscriminatingMatchScore(left = "", right = "") {
  ...
  if (a === b) return 1;
  const shorterLength = Math.min(a.length, b.length);
  if (shorterLength >= 5 && (a.includes(b) || b.includes(a))) return 0.95;
  let prefixLength = 0;
  while (prefixLength < shorterLength && a[prefixLength] === b[prefixLength]) prefixLength += 1;
  return prefixLength >= 4 ? prefixLength / shorterLength : 0;
}
...
function requestedMetricRelationTermDistance(
  ...
  for (const match of text.matchAll(/[\p{L}\p{N}-]+/gu)) {
    const token = requestedAnswerTermToken(match[0]);
    ...
    const tokenCenter = Number(match.index || 0) + String(match[0] || "").length / 2;
...
function requestedMetricWordClaimIsLocallyNumeric(reply = "", claim = null, slots = []) {
  ...
  const beforeTokens = requestedAnswerTokens(text.slice(Math.max(0, start - 72), start)).slice(-2);
  const afterTokens = requestedAnswerTokens(text.slice(end, Math.min(text.length, end + 72))).slice(0, 2);
...
function uniformGroupCardinality(value = "") {
  ...
  if (!cue) return null;
  const cardinality = normalizedNumericClaims(cue[0])
    .map(claim => Number(claim.numeric))
    .find(number => Number.isInteger(number) && number >= 1 && number <= 20);
  return Number.isInteger(cardinality) ? cardinality : null;
}
...
export function extractUniformParticipantBreakdown(value = "") {
  ...
  if (!perGroupValues.size) {
    ...
    inferredParticipantCategoryCount = new Set(
      participantCategories.map(category => normalizeText(category.label)).filter(Boolean)
    ).size;
    const categoryValues = new Set(participantCategories.map(category => category.value));
    if (participantCategories.length >= 2 && categoryValues.size === 1) {
...
function uniformParticipantRelation(source = {}, reply = "", categoryLabels = [], message = "") {
  ...
  const sourceGroupCardinality = Number(sourceBreakdown.groupCardinality || 0) || null;
  const replyGroupCue = uniformGroupCue(normalizeText(reply));
  const replyGroupCardinality = uniformGroupCardinality(reply);
  const distinctParticipantCategoryCount = new Set(
    participantCategories.map(category => normalizeText(category.label)).filter(Boolean)
  ).size;
