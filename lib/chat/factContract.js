...
function temporalAggregateCanonicalReply(primaryReply = "", supplementalBindings = [], originalReply = "", replyLang = "") {
  ...
  const heading = typedQualitative
    ? language === "ru"
      ? "Содержательное направление развития в источниках (годы обозначают время публикации статей):"
      : language === "en"
        ? "How the sources describe the development (years indicate article publication dates):"
        : "Allikates kirjeldatud sisuline arengusuund (aastad tähistavad artiklite ilmumisaega):"
    : language === "ru"
  ...
  const lines = bindings.map(binding => {
    const relationLabel = binding.relation === "companion_source_context"
      ? language === "ru"
        ? "Статья, опубликованная в запрошенный период"
        : language === "en"
          ? "Article published during the requested period"
          : "Küsitud perioodil ilmunud artikkel"
      : binding.relation === "same_document_later_development"
        ? language === "ru"
          ? "Статья, описывающая следующий этап"
          : language === "en"
            ? "Article describing the following stage"
            : "Järgmist etappi kirjeldav artikkel"
        : "";
    return relationLabel
      ? `- ${relationLabel} ${binding.publication_year} — ${binding.title}: ${binding.evidence_unit}`
