...
function requestedMetricRelationTermDistance(
  ...
  const claimCenter = Math.max(0, Number(claim?.index || 0) - Number(localScope?.start || 0)) +
    String(claim?.raw || claim?.value || "").length / 2;
  ...
  for (const match of factRelationTokens(text)) {
    ...
    const tokenCenter = (match.start + match.end) / 2;
    minimumDistance = Math.min(minimumDistance, Math.abs(tokenCenter - claimCenter));
  }
