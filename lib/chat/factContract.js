...
function requestedMetricDiscriminatingTerms(slot = {}) {
  ...
  const discriminatingTerms = relationTerms.filter(term => {
    ...
    return token && !REQUESTED_METRIC_RELATION_GENERIC_RE.test(token);
  });
...
function requestedMetricSegmentMatchesRelation(segment = "", slot = {}) {
  const requiredTerms = requestedMetricDiscriminatingTerms(slot);
...
function requestedMetricClauseBoundaries(reply = "", slots = []) {
  ...
  for (const match of separators) {
    ...
    const startsNewMetricRelation = (Array.isArray(slots) ? slots : [])
      .some(slot => requestedMetricSegmentMatchesRelation(relationSegment, slot));
...
function requestedMetricRelationFingerprint(slot = {}) {
  return requestedMetricDiscriminatingTerms(slot)
    .map(requestedAnswerTermToken)
    .filter(Boolean)
    .sort()
    .join("\u0000");
...
function requestedMetricUniqueRelationBoundToClaim(localScope = null, claim = null, slot = {}, slots = []) {
  const currentFingerprint = requestedMetricRelationFingerprint(slot);
  const peers = (Array.isArray(slots) ? slots : []).filter(peer =>
    peer !== slot &&
    requestedMetricRelationFingerprint(peer) &&
    requestedMetricRelationFingerprint(peer) !== currentFingerprint
  );
  if (!peers.length) return { passed: true, matchedUniqueTermCount: 0 };
  const currentTerms = requestedMetricDiscriminatingTerms(slot);
  const currentUniqueTerms = currentTerms.filter(term =>
    !peers.some(peer => requestedMetricDiscriminatingTerms(peer).some(peerTerm =>
  ...
  const competingTerms = peers.flatMap(peer =>
    requestedMetricDiscriminatingTerms(peer)
