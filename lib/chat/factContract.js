...
import { validateDirectedRelationReply } from "./directedRelationContract.js";
import { contactRoleTextMatches, isInstitutionalApplicationGuidance } from "@/lib/chat/contactRoleSemantics";
import { factRelationTermMatchQuality, factRelationTokens, recommendationRelationMatches, recommendationRelationMatchQuality } from "@/lib/chat/factRelationSemantics";
import { hasPerformedStudyEvent, isOrdinalTimeBoundary, qualitativeTimePayload, qualitativeTimePayloadMatches } from "@/lib/chat/qualitativeTimeSemantics";
import {
  qualitativeActionClauses,
  qualitativeActionFamiliesCompatible,
...
import { percentageRanges } from "@/lib/chat/percentageRange";
import { namedMetricScopeMatches } from "@/lib/chat/namedMetricScope";
import { typedKnownValueCheck, typedKnownValueMatches, typedKnownValueMentions, splitKnownValueAnswerClaims } from "./knownValueSemantics.js";

const YEAR_MIN = 1900;
export const FACT_VALIDATOR_VERSION = "exact_numeric_fact_v7";
const YEAR_MAX = 2100;
...
function stripStructuralNumbers(value = "") {
  return String(value || "")
    .replace(/^\s*\d{1,3}[.)]\s+/gmu, match => " ".repeat(match.length))
    .replace(/\[\d{1,3}\]/gu, match => " ".repeat(match.length));
}
...
export function smallCardinalNumberValue(rawWord = "", followingText = "", precedingText = "") {
  const word = normalizeText(rawWord).replace(/[^\p{L}]+/gu, "");
  if (!word) return null;
  const normalizedFollowingText = normalizeText(followingText);
  if (word === "kuus" && /\bkord(?:a)?\s*$/u.test(normalizeText(precedingText))) return null;
  // „Viis läbi/ellu/edasi” is a verb, not the cardinal number five.
  if (word === "viis" && /^(?:labi|ellu|edasi|sisse|valja)\b/u.test(normalizedFollowingText)) {
    return null;
  }
  const estonianMapped = ESTONIAN_SMALL_NUMBER_FORMS.find(([, pattern]) => pattern.test(word));
  const russianMapped = RUSSIAN_SMALL_NUMBER_FORMS.find(([, pattern]) => pattern.test(word));
  const mappedValue = estonianMapped?.[0] ??
    russianMapped?.[0] ??
    Number(NON_EST_SMALL_NUMBER_VALUES.get(word));
  return Number.isFinite(mappedValue) ? mappedValue : null;
}

function normalizedNumericClaims(value = "", metricSlots = []) {
  const claims = numericClaims(value);
  const normalized = stripStructuralNumbers(value)
    .normalize("NFD")
    .replace(/\p{Diacritic}+/gu, "")
    .toLowerCase();
  for (const match of normalized.matchAll(/(?<![\p{L}\d])[\p{L}]+(?![\p{L}\d])/gu)) {
    ...
    const mappedValue = smallCardinalNumberValue(word, followingText, normalized.slice(0, match.index || 0));
    ...
    if (!requestedMetricWordClaimIsLocallyNumeric(value, wordClaim, metricSlots)) continue;
    claims.push(wordClaim);
  }
...
function asksForNumericFact(message = "") {
  ...
  return /%|(?:^|[^\p{L}\p{N}])(?:arv(?:u|ud|ust|uga)?|koguarv\p{L}*|uldarv\p{L}*|telefoninumb\p{L}*|kokku|kui\s+palju|kui\s+suur\s+osa|kui\s+kaua|kestus\p{L}*|mitu|nait(?:aja|ude?)\p{L}*|osakaal\p{L}*|protsent\p{L}*|mis\s+aastal|millisel\s+aastal|how\s+many|how\s+long|duration|phone\s+numbers?|number\s+of|total|percentage|percent|what\s+year|which\s+year|сколько|как\s+долго|длительность|номер\p{L}*\s+телефон\p{L}*|число|итого|всего|процент\p{L}*|каком\s+году)(?=$|[^\p{L}\p{N}])/u.test(normalized);
}

function asksForYear(message = "") {
  const normalized = normalizeText(message);
  return /\b(?:mis\s+aasta(?:l|st)?|millisel\s+aastal|what\s+year|which\s+year|каком\s+году)\b/u.test(normalized);
}

function requestedMetricContractFromRetrievalMeta(retrievalMeta = null) {
  const contract = retrievalMeta?.requestedFactSlotContract || retrievalMeta?.requestedMetricContract;
  if (
    !contract ||
    !new Set(["requested_fact_slot_contract_v1", "requested_metric_contract_v2"]).has(contract.version) ||
    contract.enabled !== true ||
    contract.complete !== true ||
    !Array.isArray(contract.slots) ||
    !contract.slots.length
  ) return null;
  return contract;
}

function requestedQualitativeContractFromRetrievalMeta(retrievalMeta = null) {
  const contract = retrievalMeta?.requestedQualitativeSlotContract;
  if (
    !contract ||
    contract.version !== "requested_qualitative_slot_contract_v1" ||
    contract.enabled !== true ||
    contract.complete !== true ||
    !Array.isArray(contract.slots) ||
    !contract.slots.length
...
function questionScopeValuesFromRetrievalMeta(retrievalMeta = null) {
  const values = [
    ...(Array.isArray(retrievalMeta?.requestedFactSlotContract?.question_scope_values)
      ? retrievalMeta.requestedFactSlotContract.question_scope_values
      : []),
    ...(Array.isArray(retrievalMeta?.requestedMetricContract?.question_scope_values)
      ? retrievalMeta.requestedMetricContract.question_scope_values
      : []),
    ...(Array.isArray(retrievalMeta?.queryPlan?.semantic_candidates?.requested_fact_slots?.question_scope_values)
      ? retrievalMeta.queryPlan.semantic_candidates.requested_fact_slots.question_scope_values
      : []),
    ...(Array.isArray(retrievalMeta?.queryPlan?.question_planner?.semantic_candidates?.requested_fact_slots?.question_scope_values)
      ? retrievalMeta.queryPlan.question_planner.semantic_candidates.requested_fact_slots.question_scope_values
      : [])
  ];
  return new Set(values.map(value => normalizeNumber(value)).filter(Boolean));
}

function numericClaimHasQuestionScopeRole(reply = "", claim = null, scopeValues = new Set()) {
  if (
    !claim ||
    claim.percentage ||
    claim.year ||
    !scopeValues.has(String(claim.value || ""))
  ) return false;
  const text = String(reply || "");
  const start = Math.max(0, Number(claim.index) || 0);
  const end = start + String(claim.value || "").length;
  const prefix = normalizeText(text.slice(Math.max(0, start - 40), start));
  const suffix = normalizeText(text.slice(end, Math.min(text.length, end + 40)));
  const adjectivalAgeUnit = /^\s*[-–—]\s*(?:aastan\p{L}*|aastas\p{L}*|year\p{L}*[-\s]+old|летн\p{L}*)\b/u.test(suffix);
  const explicitAgeScope = /(?:^|\s)(?:vanuses|alla|ule|kuni|vahemalt|enam\s+kui|rohkem\s+kui|vahem\s+kui)\s*$/u.test(prefix);
  return adjectivalAgeUnit || explicitAgeScope;
}

function requestedScopeValuesBoundToClaim(reply = "", claim = null, requiredValues = [], allScopeValues = new Set()) {
  const required = new Set((Array.isArray(requiredValues) ? requiredValues : [])
    .map(value => normalizeNumber(value))
    .filter(Boolean));
  if (!required.size) return { passed: true, boundValues: [] };
  const claimIndex = Number(claim?.index || 0);
  const text = String(reply || "");
  const previousBoundaries = ["\n", ";", ".", "!", "?"]
    .map(separator => text.lastIndexOf(separator, Math.max(0, claimIndex - 1)))
    .filter(index => index >= 0);
  const nextBoundaries = ["\n", ";", ".", "!", "?"]
    .map(separator => text.indexOf(separator, claimIndex + 1))
    .filter(index => index >= 0);
  const clauseStart = previousBoundaries.length ? Math.max(...previousBoundaries) + 1 : 0;
  const clauseEnd = nextBoundaries.length ? Math.min(...nextBoundaries) : text.length;
  const scopeClaims = normalizedNumericClaims(reply)
    .filter(candidate => numericClaimHasQuestionScopeRole(reply, candidate, allScopeValues))
    .map(candidate => ({
      ...candidate,
      distance: Math.abs(Number(candidate.index || 0) - claimIndex),
      preceding: Number(candidate.index || 0) <= claimIndex,
      sameClause: Number(candidate.index || 0) >= clauseStart && Number(candidate.index || 0) <= clauseEnd
    }));
  const localScopeClaims = scopeClaims.some(candidate => candidate.sameClause)
    ? scopeClaims.filter(candidate => candidate.sameClause)
    : scopeClaims;
  const precedingScopeClaims = localScopeClaims
    .filter(candidate => candidate.preceding)
    .sort((left, right) => right.index - left.index);
  const followingScopeClaims = localScopeClaims
    .filter(candidate => !candidate.preceding)
    .sort((left, right) => left.index - right.index);
  const nearest = precedingScopeClaims[0] || followingScopeClaims[0] || null;
  if (!nearest || nearest.distance > 180 || !required.has(nearest.value)) {
    return { passed: false, boundValues: nearest ? [nearest.value] : [] };
  }
  if (required.size === 1) return { passed: true, boundValues: [nearest.value] };
  const localRequiredValues = new Set(localScopeClaims
    .filter(candidate => candidate.distance <= 180)
    .map(candidate => candidate.value)
    .filter(value => required.has(value)));
  return {
    passed: [...required].every(value => localRequiredValues.has(value)),
    boundValues: [...localRequiredValues]
  };
}

function numericClaimHasGroupCardinalityRole(reply = "", claim = null) {
  if (!claim || claim.percentage || claim.year) return false;
  const text = String(reply || "");
  const start = Math.max(0, Number(claim.index) || 0);
  const end = start + String(claim.value || "").length;
  const prefix = normalizeText(text.slice(Math.max(0, start - 32), start));
  const suffix = normalizeText(text.slice(end, Math.min(text.length, end + 64)));
  return /(?:^|\s)(?:iga\p{L}*|koig\p{L}*|each|every|кажд\p{L}*)\s*$/u.test(prefix) &&
    /^(?:\s*[\p{L}-]+){0,3}\s+(?:ruhm\p{L}*|groups?|групп\p{L}*)\b/u.test(suffix);
}

function requestedFactPlanFromRetrievalMeta(retrievalMeta = null) {
  const semanticTurn = retrievalMeta?.queryPlan?.semantic_turn_contract;
  // The final query plan carries the canonical contract, not the planner's
  // transient semantic_candidates. Never replace an incomplete canonical plan
  // with a stale, complete legacy plan.
  if (semanticTurn?.requested_fact_contract) {
    return {
      ...semanticTurn.requested_fact_contract,
      slots: Array.isArray(semanticTurn.requested_facts) ? semanticTurn.requested_facts : []
    };
  }
  return retrievalMeta?.queryPlan?.question_planner?.semantic_candidates?.requested_fact_slots ||
    retrievalMeta?.queryPlan?.semantic_candidates?.requested_fact_slots || null;
}

function requestedFactSlotsFromRetrievalMeta(retrievalMeta = null) {
  const requested = requestedFactPlanFromRetrievalMeta(retrievalMeta);
  return requested?.complete === true && Array.isArray(requested?.slots)
    ? requested.slots
    : [];
}

function plannedMetricClaimMatchesType(claim = null, valueType = "") {
  if (!claim) return false;
  if (valueType === "proportion") return claim.percentage === true;
  if (valueType === "calendar_year") return claim.year === true;
  return ["count", "amount", "duration", "magnitude"].includes(valueType) &&
    claim.percentage !== true &&
    claim.year !== true;
}

function requestedMetricPlanClaimAssignment(reply = "", retrievalMeta = null, scopeValues = new Set()) {
  const slots = requestedFactSlotsFromRetrievalMeta(retrievalMeta)
    .filter(slot => ["count", "amount", "duration", "magnitude", "proportion", "calendar_year"].includes(
      String(slot?.value_type || "")
    ));
  if (!slots.length) return null;
  const claims = normalizedNumericClaims(reply).filter(claim =>
    !numericClaimHasQuestionScopeRole(reply, claim, scopeValues) &&
    !numericClaimHasGroupCardinalityRole(reply, claim)
  );
  const assignments = [];
  let minimumClaimIndex = -1;
  for (const slot of slots) {
    const valueType = String(slot?.value_type || "");
    const claimIndex = claims.findIndex((claim, index) =>
      index > minimumClaimIndex && plannedMetricClaimMatchesType(claim, valueType)
    );
    if (claimIndex < 0) {
      return {
        passed: false,
        reason: "requested_fact_metric_slot_missing",
        missingSlotIndex: Number(slot?.index) || assignments.length + 1,
        assignments
      };
    }
    const requiredScopeValues = Array.isArray(slot?.scope_values) ? slot.scope_values : [];
    const scopeBinding = requestedScopeValuesBoundToClaim(
      reply,
      claims[claimIndex],
      requiredScopeValues,
      scopeValues
    );
    if (!scopeBinding.passed) {
      return {
        passed: false,
        reason: "requested_metric_scope_mismatch",
        missingSlotIndex: Number(slot?.index) || assignments.length + 1,
        assignments
      };
    }
    minimumClaimIndex = claimIndex;
    assignments.push({
      slot_index: Number(slot?.index) || assignments.length + 1,
      value_type: valueType,
      claim_index: claimIndex,
      claim_value: claims[claimIndex].value,
      scope_values: requiredScopeValues.map(value => normalizeNumber(value)).filter(Boolean),
      bound_scope_values: scopeBinding.boundValues
    });
  }
  return { passed: true, reason: "requested_fact_metric_shape_validated", assignments };
}

function requestedAnswerTermToken(value = "") {
  return normalizeText(value).replace(/[^\p{L}\p{N}-]+/gu, "").trim();
}

function requestedAnswerTermMatchQuality(left = "", right = "") {
  return factRelationTermMatchQuality(left, right);
}

function requestedAnswerTokens(value = "") {
  return factRelationTokens(stripStructuralNumbers(value)).map(token => token.value);
}

const REQUESTED_ANSWER_VALUE_STOPWORDS = new Set([
  "ja", "ning", "voi", "ka", "on", "oli", "olid", "olla", "olema", "selle", "nende", "kohta",
  "the", "and", "or", "is", "was", "were", "about", "with", "и", "или", "это", "был", "были"
]);

function requestedAnswerRelationAlternatives(slot = {}, relationTerm = "") {
  const normalizedTerm = requestedAnswerTermToken(relationTerm);
  const variantEntry = (Array.isArray(slot?.relation_term_variants) ? slot.relation_term_variants : [])
    .find(item => requestedAnswerTermToken(item?.term || "") === normalizedTerm);
  return Array.from(new Set([
    relationTerm,
    ...(Array.isArray(variantEntry?.variants) ? variantEntry.variants : [])
  ].map(requestedAnswerTermToken).filter(Boolean)));
}

function requestedAnswerUnits(reply = "") {
  return String(reply || "")
    .split(/(?:\r?\n)+|(?<=[.!?][”"'’»]?)\s+/u)
    .reduce((units, text) => {
      // "2025. aasta novembrist" is one value, not two answer units.
      if (isOrdinalTimeBoundary(units.at(-1) || "", text)) {
        units[units.length - 1] += ` ${text}`;
      } else units.push(text);
      return units;
    }, [])
    .map((text, index) => ({ index, text: String(text || "").trim() }))
    .filter(unit => unit.text)
    .filter(unit => !/^(?:ma\s+ei\s+saa|ei\s+ole\s+voimalik|i\s+(?:can'?t|cannot)|я\s+не\s+могу)\b/iu.test(normalizeText(unit.text)));
}

function requestedRecommendationAnswerItems(reply = "") {
  const items = [];
  for (const line of String(reply || "").split(/\r?\n/u)) {
    const trimmed = line.trim();
    if (!trimmed) {
      if (items.length && !items.at(-1).listed) items.at(-1).closed = true;
      continue;
    }
    const newItem = /^(?:\d{1,3}[.)]|[-*•])\s+/u.test(trimmed) || /^\*\*[^*]+\*\*\s*:?/u.test(trimmed);
    if (!items.length || newItem || items.at(-1).closed) items.push({ text: trimmed, closed: false, listed: newItem });
    else items.at(-1).text += ` ${trimmed}`;
  }
  const units = [];
  let sentenceOffset = 0;
  for (const item of items) {
    const sentences = requestedAnswerUnits(item.text).filter(unit => !/^\d{1,3}[.)]$/u.test(unit.text));
    for (let start = 0; start < sentences.length; start += 1) {
      // A contract contains at most eight action bindings. Keep every sentence,
      // plus local windows, without borrowing across separately numbered items.
      for (let end = start; end < Math.min(sentences.length, start + 8); end += 1) {
        const window = sentences.slice(start, end + 1);
        units.push({ index: units.length, text: window.map(unit => unit.text).join(" "),
          sentence_indexes: window.map((_, index) => sentenceOffset + start + index) });
      }
    }
...
function requestedStandaloneObjectAnaphora(tokens = [], signature = null) {
  const actionIndexes = (signature?.terms || []).map(term => tokens.indexOf(term)).filter(index => index >= 0);
  const auxiliaries = /^(?:tuleb|tule|peab|peaks|voib|voiks|saab|saaks|ei|mitte|should|must|can|could|not|be)$/u;
  return tokens.some((token, pronounIndex) => {
    if (!/^(?:seda|selle|neid|nende|it|this|them)$/u.test(token)) return false;
    return actionIndexes.some(actionIndex => {
      const from = Math.min(actionIndex, pronounIndex);
      const to = Math.max(actionIndex, pronounIndex);
      if (to - from > 4 || !tokens.slice(from + 1, to).every(word => auxiliaries.test(word))) return false;
      const following = tokens[pronounIndex + 1];
      // A pronoun before a predicate may be separated by auxiliary verbs.
      // After a predicate it must stand alone, not determine a new nominal
      // object ("selle transpordisüsteemi", "seda uut süsteemi"). The explicit
      // predicative role "alternatiivina" is not another direct object.
      return actionIndex > pronounIndex || !following ||
        (following === "alternatiivina" && pronounIndex + 2 === tokens.length);
    });
  });
}

function requestedQualitativeActionObjectAssignment(slot = {}, unitText = "") {
  const expectedBindings = (Array.isArray(slot?.action_object_bindings) ? slot.action_object_bindings : [])
    .filter(binding => String(binding?.action_family || "").trim())
    .slice(0, 8);
  if (!expectedBindings.length) return null;
  const clauseActions = qualitativeActionClauses(unitText)
    .map((text, clauseIndex) => {
      const labelEnd = text.indexOf(":");
      const answerText = labelEnd >= 0 ? text.slice(labelEnd + 1) : text;
      const answerTokens = requestedAnswerTokens(answerText);
      const localPronoun = requestedStandaloneObjectAnaphora(answerTokens, qualitativeActionSignature(answerText));
      const suffix = unitText.slice(unitText.indexOf(text) + text.length);
      const relativeDescription = /^\s*,\s*(?:mis|mille\p{L}*|which|that|котор\p{L}*)\b/iu.test(suffix)
        ? suffix.split(/(?<=[.!?])\s+|\r?\n/u)[0] : "";
      return {
        clauseIndex,
        // A repeated question label identifies a slot but is not the object of
        // an unrelated predicate in its answer. Explicit local anaphora is safe.
        tokens: labelEnd >= 0 && !localPronoun ? answerTokens : requestedAnswerTokens(text),
        descriptionTokens: requestedAnswerTokens(`${answerText} ${relativeDescription}`),
        signature: qualitativeActionSignature(text)
      };
    })
    .filter(clause => clause.tokens.length && clause.signature.entries.length);
  const usedActionKeys = new Set();
  const assignments = [];
  for (const expected of expectedBindings) {
    const expectedFamily = String(expected?.action_family || "").trim();
    const expectedCategory = String(expected?.action_category || "").trim();
    const expectedNegated = expected?.evidence_negated === true;
    const objectAnchors = (Array.isArray(expected?.object_anchor_terms) ? expected.object_anchor_terms : [])
      .map(requestedAnswerTermToken)
      .filter(Boolean);
    const minimumObjectMatches = Math.max(1, Number(expected?.minimum_object_matches || 1));
    const options = [];
    for (const clause of clauseActions) {
      const matchedObjectCount = objectAnchors.filter(anchor =>
        clause.tokens.some(token => recommendationRelationMatchQuality(anchor, token,
          requestedAnswerRelationAlternatives(slot, anchor)) >= 0.72)
      ).length;
      const descriptionMatches = (expected.object_description_terms || []).filter(anchor =>
        clause.descriptionTokens.some(token => recommendationRelationMatchQuality(anchor, token) >= 0.72)
      ).length;
      const objectHead = objectAnchors.at(-1);
      const headMatched = objectHead && clause.tokens.some(token => recommendationRelationMatchQuality(objectHead, token,
        requestedAnswerRelationAlternatives(slot, objectHead)) >= 0.72);
      const ellipsisHeadMatched = (expected.object_ellipsis_terms || []).some(anchor =>
        clause.tokens.some(token => recommendationRelationMatchQuality(anchor, token) >= 0.72));
      const describedObject = (ellipsisHeadMatched && descriptionMatches >= 2) ||
        (headMatched && matchedObjectCount + descriptionMatches >= minimumObjectMatches);
      if (matchedObjectCount < minimumObjectMatches && !describedObject) continue;
      clause.signature.entries.forEach((entry, entryIndex) => {
        const actionKey = `${clause.clauseIndex}:${entryIndex}`;
        if (usedActionKeys.has(actionKey)) return;
        if (!qualitativeActionFamiliesCompatible(expectedFamily, entry.family)) return;
        if (expectedCategory && entry.category !== expectedCategory) return;
        if (clause.signature.negated !== expectedNegated) return;
        options.push({
          actionKey,
          clause_index: clause.clauseIndex,
          expected_action_family: expectedFamily,
          answer_action_family: entry.family,
          action_category: entry.category,
          answer_negated: clause.signature.negated,
          matched_object_count: matchedObjectCount,
          required_object_count: minimumObjectMatches,
          score: matchedObjectCount * 4 + (entry.family === expectedFamily ? 2 : 1)
        });
      });
    }
    options.sort((left, right) => right.score - left.score || left.clause_index - right.clause_index);
    const best = options[0];
    if (!best) return null;
    usedActionKeys.add(best.actionKey);
    assignments.push(best);
  }
  return assignments;
}

function requestedQualitativeUnitBinding(slot = {}, unit = null, onReject = null) {
  // Observe the first rejecting gate without retaining any model draft text.
  // This callback must not change the binding predicates or their order.
  const reject = reason => { onReject?.(reason); return null; };
  if (!unit?.text) return reject("empty_unit");
  if (["known_value_interpretation", "referenced_study_period"].includes(slot.payload_kind)) {
    const check = typedKnownValueCheck(slot.admitted_payload, unit.text);
    if (!check.passed) return reject(check.reason);
    return { unit_index: unit.index, sentence_indexes: unit.sentence_indexes || [unit.index],
      slot_index: Number(slot.slot_index || slot.index), matched_relation_terms: [], matched_evidence_anchors: [],
      action_object_bindings: [], substantive_answer_tokens: [], required_numeric_values: [], score: 20 };
  }
  const tokens = requestedAnswerTokens(unit.text);
  if (!tokens.length) return reject("empty_unit");
  const relationTerms = Array.isArray(slot?.relation_terms)
    ? slot.relation_terms.map(value => String(value || "").trim()).filter(Boolean)
    : [];
  const matchedRelationTerms = slot.value_type === "recommendation" ? recommendationRelationMatches(slot, unit.text) : relationTerms.filter(term =>
    requestedAnswerRelationAlternatives(slot, term).some(variant =>
      tokens.some(token => requestedAnswerTermMatchQuality(variant, token) >= 0.72)
    )
  );
  const minimumRelationMatches = Math.max(
    1,
    Number(slot?.minimum_relation_matches || (relationTerms.length <= 2 ? 1 : 2))
  );
  const performedTime = slot?.performed_study_time === true && hasPerformedStudyEvent(unit.text);
  if (matchedRelationTerms.length < minimumRelationMatches && !performedTime) return reject("relation_terms_missing");
  if (slot?.temporal_binding && !qualitativeTimePayloadMatches(
    slot.temporal_binding, qualitativeTimePayload(unit.text, smallCardinalNumberValue), requestedAnswerTermMatchQuality
  )) return reject("temporal_payload_mismatch");
  const requiredNumericValues = (Array.isArray(slot?.required_numeric_values) ? slot.required_numeric_values : [])
    .map(normalizeNumber)
    .filter(Boolean);
  const unitNumericValues = new Set(normalizedNumericClaims(unit.text).map(claim => claim.value));
  if (!requiredNumericValues.every(value => unitNumericValues.has(value))) return reject("required_numbers_missing");
  const evidenceAnchors = (Array.isArray(slot?.evidence_anchor_terms) ? slot.evidence_anchor_terms : [])
    .map(requestedAnswerTermToken)
    .filter(Boolean);
  const matchedEvidenceAnchors = evidenceAnchors.filter(anchor =>
    tokens.some(token => requestedAnswerTermMatchQuality(anchor, token) >= 0.72)
  );
  const minimumAnchorMatches = Math.max(0, Number(slot?.minimum_anchor_matches || 0));
  if (matchedEvidenceAnchors.length < minimumAnchorMatches) return reject("evidence_anchors_missing");
  const valueType = String(slot?.value_type || "");
  const actionObjectAssignments = valueType === "recommendation"
    ? requestedQualitativeActionObjectAssignment(slot, unit.text)
    : [];
  if (valueType === "recommendation" && !actionObjectAssignments) return reject("action_object_mismatch");
  const hasPayloadGate = requiredNumericValues.length > 0 ||
    !!slot?.temporal_binding ||
    (evidenceAnchors.length > 0 && minimumAnchorMatches > 0) ||
    (Array.isArray(actionObjectAssignments) && actionObjectAssignments.length > 0);
  if (!hasPayloadGate) return reject("evidence_payload_missing");
  const substantiveTokens = Array.from(new Set(tokens.filter(token =>
    token.length >= 3 &&
    !REQUESTED_ANSWER_VALUE_STOPWORDS.has(token) &&
    !relationTerms.some(term =>
      requestedAnswerRelationAlternatives(slot, term).some(variant =>
        requestedAnswerTermMatchQuality(variant, token) >= 0.72
      )
    )
  )));
  const minimumAnswerItems = Math.max(1, Number(slot?.minimum_answer_items || 1));
  if (substantiveTokens.length + requiredNumericValues.length < minimumAnswerItems) return reject("answer_items_missing");
  return {
    unit_index: unit.index,
    sentence_indexes: unit.sentence_indexes || [unit.index],
    slot_index: Number(slot?.slot_index || slot?.index),
    matched_relation_terms: matchedRelationTerms,
    matched_evidence_anchors: matchedEvidenceAnchors,
    action_object_bindings: actionObjectAssignments,
    substantive_answer_tokens: substantiveTokens.slice(0, 12),
    required_numeric_values: requiredNumericValues,
    score: matchedRelationTerms.length * 5 + matchedEvidenceAnchors.length * 2 +
      requiredNumericValues.length * 3 + (actionObjectAssignments?.length || 0) * 4
  };
}

function requestedQualitativeReplyAssignment(reply = "", contract = null) {
  if (!contract?.slots?.length) return null;
  const recommendationsOnly = contract.slots.every(slot => slot.value_type === "recommendation");
  let units = recommendationsOnly
    ? requestedRecommendationAnswerItems(reply) : requestedAnswerUnits(reply);
  if (contract.slots.some(slot => slot.payload_kind === "known_value_interpretation") &&
    contract.slots.some(slot => slot.payload_kind === "referenced_study_period")) {
    units = units.flatMap(unit => splitKnownValueAnswerClaims(unit.text).map(text => ({
      text, sentence_indexes: [unit.index]
    }))).map((unit, index) => ({ ...unit, index }));
  }
  const slotEntries = contract.slots.map((slot, contractIndex) => {
    const rejected = {};
    const options = units
      .map(unit => requestedQualitativeUnitBinding(slot, unit, reason => {
        rejected[reason] = (rejected[reason] || 0) + 1;
      }))
      .filter(Boolean)
      .sort((left, right) => left.sentence_indexes.length - right.sentence_indexes.length || right.score - left.score || left.unit_index - right.unit_index);
    if (slot.admitted_payload) {
      // Binding one correct sentence must not hide another contradictory claim
      // about the same known value/reference elsewhere in the answer.
      const peers = contract.slots.filter(peer => peer.payload_kind === slot.payload_kind &&
        (slot.payload_kind !== "known_value_interpretation" || peer.admitted_payload?.value === slot.admitted_payload.value));
      const conflicting = units.filter(unit => typedKnownValueMentions(slot.admitted_payload, unit.text) &&
        !peers.some(peer => typedKnownValueMatches(peer.admitted_payload, unit.text)));
      if (conflicting.length) {
        options.length = 0;
        rejected.typed_claim_conflict = conflicting.length;
      }
    }
    return { contractIndex, slot, options, rejected };
  });
  const unitToSlot = new Map();
  const slotToBinding = new Map();
  function augment(slotIndex, visitedUnits = new Set(), visitedSlots = new Set()) {
    if (visitedSlots.has(slotIndex)) return false;
    visitedSlots.add(slotIndex);
    for (const option of slotEntries[slotIndex].options) {
      if (visitedUnits.has(option.unit_index)) continue;
      visitedUnits.add(option.unit_index);
      const currentSlotIndex = unitToSlot.get(option.unit_index);
      if (
        typeof currentSlotIndex !== "number" ||
        augment(currentSlotIndex, visitedUnits, visitedSlots)
      ) {
        unitToSlot.set(option.unit_index, slotIndex);
        slotToBinding.set(slotIndex, option);
        return true;
      }
    }
    return false;
  }
  const orderedEntries = [...slotEntries]
    .sort((left, right) => left.options.length - right.options.length || left.contractIndex - right.contractIndex);
  if (recommendationsOnly) {
    let best = new Map();
    let visits = 0;
    function assignDisjoint(position, selected, usedSentences) {
      if (++visits > 10000) return false; // Exhaustion remains fail-closed.
      if (selected.size > best.size) best = new Map(selected);
      if (best.size === slotEntries.length) return true;
      if (position >= orderedEntries.length || selected.size + orderedEntries.length - position <= best.size) return false;
      const entry = orderedEntries[position];
      for (const option of entry.options) {
        if (option.sentence_indexes.some(index => usedSentences.has(index))) continue;
        const next = new Map(selected).set(entry.contractIndex, option);
        if (assignDisjoint(position + 1, next, new Set([...usedSentences, ...option.sentence_indexes]))) return true;
      }
      return assignDisjoint(position + 1, selected, usedSentences);
    }
    assignDisjoint(0, new Map(), new Set());
    for (const [index, binding] of best) slotToBinding.set(index, binding);
  } else orderedEntries.forEach(entry => augment(entry.contractIndex));
  const assignments = slotEntries
    .map(entry => slotToBinding.get(entry.contractIndex))
    .filter(Boolean);
  const missingSlotIndexes = slotEntries
    .filter(entry => !slotToBinding.has(entry.contractIndex))
    .map(entry => Number(entry.slot?.slot_index || entry.slot?.index));
  return {
    passed: missingSlotIndexes.length === 0 && assignments.length === contract.slots.length,
    reason: missingSlotIndexes.length ? "requested_fact_answer_incomplete" : "requested_qualitative_slots_validated",
    assignments,
    missingSlotIndexes,
    answerUnitCount: units.length,
    gateChecks: slotEntries.map(entry => ({
      slot_index: Number(entry.slot?.slot_index || entry.slot?.index),
      evaluated_unit_count: units.length,
      candidate_unit_count: entry.options.length,
      assigned: slotToBinding.has(entry.contractIndex),
      assignment_conflict: !slotToBinding.has(entry.contractIndex) && entry.options.length > 0,
      rejection_counts: entry.rejected
    }))
  };
}

const REQUESTED_METRIC_RELATION_GENERIC_RE = /^(?:aasta\p{L}*|alla|arv\p{L}*|hulgas|inime\p{L}*|ja|kui|kokku|mitu|ning|oli|olid|on|tehti|kasutati|osakaal\p{L}*|palju|protsent\p{L}*|seas|suur\p{L}*|ule)$/u;
const REQUESTED_METRIC_CATEGORY_MODIFIER_RE = /^(?:palju|vahe|suur\p{L}*|vaik\p{L}*|keskm\p{L}*|korge\p{L}*|madal\p{L}*|tugev\p{L}*|nork\p{L}*|raske\p{L}*|kerge\p{L}*|mooduka\p{L}*)$/u;

function requestedMetricSlotsShareHead(slot = {}, peer = {}) {
  const terms = Array.isArray(slot?.relation_terms) ? slot.relation_terms : [];
  const peerTerms = Array.isArray(peer?.relation_terms) ? peer.relation_terms : [];
  return terms.some(head => !REQUESTED_METRIC_RELATION_GENERIC_RE.test(requestedAnswerTermToken(head)) &&
    !REQUESTED_METRIC_CATEGORY_MODIFIER_RE.test(requestedAnswerTermToken(head)) &&
    peerTerms.some(peerTerm => requestedMetricRelationTermsEquivalent(slot, head, peer, peerTerm)));
}

function requestedMetricRequiredPeerModifiers(slot = {}, slots = []) {
  const terms = Array.isArray(slot?.relation_terms) ? slot.relation_terms : [];
  const modifier = term => REQUESTED_METRIC_CATEGORY_MODIFIER_RE.test(requestedAnswerTermToken(term));
  return terms.filter(term => modifier(term) && slots.some(peer => {
    if (peer === slot) return false;
    const peerTerms = Array.isArray(peer?.relation_terms) ? peer.relation_terms : [];
    return requestedMetricSlotsShareHead(slot, peer) && !peerTerms.some(peerTerm => requestedMetricRelationTermsEquivalent(slot, term, peer, peerTerm));
  }));
}

function requestedMetricDiscriminatingTerms(slot = {}, slots = []) {
  const relationTerms = (Array.isArray(slot?.relation_terms) ? slot.relation_terms : [])
    .map(value => String(value || "").trim())
    .filter(Boolean);
  const discriminatingTerms = relationTerms.filter(term => {
    const token = requestedAnswerTermToken(term);
    return token && (!REQUESTED_METRIC_RELATION_GENERIC_RE.test(token) || requestedMetricRequiredPeerModifiers(slot, slots).includes(term));
  });
  return discriminatingTerms.length ? discriminatingTerms : relationTerms;
}

function requestedMetricMinimumRelationMatches(slot = {}, requiredTerms = []) {
  const configured = Number(slot?.minimum_relation_matches);
  if (Number.isInteger(configured) && configured > 0) {
    return Math.min(requiredTerms.length, configured);
  }
  return Math.min(2, requiredTerms.length);
}

function requestedMetricSegmentMatchesRelation(segment = "", slot = {}, slots = []) {
  const requiredTerms = requestedMetricDiscriminatingTerms(slot, slots);
  if (!requiredTerms.length) return false;
  const segmentTokens = requestedAnswerTokens(segment);
  const matchedCount = requiredTerms.filter(term =>
    requestedAnswerRelationAlternatives(slot, term).some(variant =>
      segmentTokens.some(token => requestedAnswerTermMatchQuality(variant, token) >= 0.72)
    )
  ).length;
  return matchedCount >= requestedMetricMinimumRelationMatches(slot, requiredTerms);
}

function requestedMetricHardBoundaries(text = "") {
  return [...text.matchAll(/[\n;.!?]/gu)].filter(match => {
    ...
    return !(/\d$/u.test(before) && /^\d/u.test(after)) &&
      !(/^(?:19\d{2}|20\d{2}|2100)$/u.test(before) && /^\s*aasta\p{L}*\b/u.test(after));
  }).map(match => match.index);
}

function requestedMetricClauseBoundaries(reply = "", slots = []) {
  const text = String(reply || "");
  const claims = normalizedNumericClaims(text, slots);
  const boundaries = [];
  const separators = [...text.matchAll(/,|(?<!\p{L})(?:ja|ning|and|и)(?!\p{L})/gu)]
    .filter(match => !(match[0] === "," && /\d/u.test(text[match.index - 1] || "") && /\d/u.test(text[match.index + 1] || "")));
  for (const match of separators) {
    const commaIndex = Number(match.index);
    const separatorEnd = commaIndex + match[0].length;
    if (!Number.isInteger(commaIndex)) continue;
    if (/\d/u.test(text[commaIndex - 1] || "") && /\d/u.test(text[commaIndex + 1] || "")) continue;
    const hardBoundaries = requestedMetricHardBoundaries(text);
    const previousHardBoundaries = hardBoundaries.filter(index => index < commaIndex);
    const nextHardBoundaries = hardBoundaries.filter(index => index > commaIndex);
    const hardStart = previousHardBoundaries.length ? Math.max(...previousHardBoundaries) + 1 : 0;
    const nextSeparator = separators.find(separator => separator.index >= separatorEnd);
    if (nextSeparator) nextHardBoundaries.push(nextSeparator.index);
    const hardEnd = nextHardBoundaries.length ? Math.min(...nextHardBoundaries) : text.length;
    const hasClaimBefore = claims.some(claim => claim.index >= hardStart && claim.index < commaIndex);
    const rightSegment = text.slice(separatorEnd, hardEnd);
    const rightClaims = normalizedNumericClaims(rightSegment, slots);
    const hasContractEvidenceValue = (Array.isArray(slots) ? slots : []).some(slot => {
      const expectedValue = normalizeNumber(slot?.evidence_value);
      const expectedPercentage = slot?.value_type === "proportion" || slot?.unit === "percent";
      return rightClaims.some(claim =>
        claim.value === expectedValue && claim.percentage === expectedPercentage
      );
    });
    // A conjunction is a boundary only for a new label-before-number clause.
    // Do not split the units of "30 spetsialisti ja 12 riiki" indiscriminately.
    const relationSegment = match[0] === "," ? rightSegment : rightSegment.slice(0, rightClaims[0]?.index || 0);
    const startsNewMetricRelation = (Array.isArray(slots) ? slots : [])
      .some(slot => requestedMetricSegmentMatchesRelation(relationSegment, slot, slots));
    if (hasClaimBefore && hasContractEvidenceValue && startsNewMetricRelation) boundaries.push({ start: commaIndex, end: separatorEnd });
  }
  return boundaries;
}

function requestedMetricClaimLocalScope(reply = "", claim = null, slots = []) {
  const text = String(reply || "");
  const claimIndex = Math.max(0, Number(claim?.index) || 0);
  const hardBoundaries = requestedMetricHardBoundaries(text);
  const previousBoundaries = hardBoundaries.filter(index => index < claimIndex);
  const nextBoundaries = hardBoundaries.filter(index => index >= claimIndex + String(claim?.raw || claim?.value || "").length);
  const clauseBoundaries = requestedMetricClauseBoundaries(text, slots);
  previousBoundaries.push(...clauseBoundaries.filter(boundary => boundary.end <= claimIndex).map(boundary => boundary.end - 1));
  nextBoundaries.push(...clauseBoundaries.filter(boundary => boundary.start > claimIndex).map(boundary => boundary.start));
  const start = previousBoundaries.length ? Math.max(...previousBoundaries) + 1 : 0;
  const end = nextBoundaries.length ? Math.min(...nextBoundaries) : text.length;
  return { text: text.slice(start, end), start, end };
}

function requestedMetricRelationFingerprint(slot = {}, slots = []) {
  return requestedMetricDiscriminatingTerms(slot, slots)
    .map(requestedAnswerTermToken)
    .filter(Boolean)
    .sort()
    .join("\u0000");
...
function requestedMetricRelationTermsEquivalent(leftSlot = {}, leftTerm = "", rightSlot = {}, rightTerm = "") {
  return requestedAnswerRelationAlternatives(leftSlot, leftTerm).some(leftVariant =>
    requestedAnswerRelationAlternatives(rightSlot, rightTerm).some(rightVariant =>
      requestedAnswerTermMatchQuality(leftVariant, rightVariant) >= 0.86
    )
  );
}

function requestedMetricDiscriminatingMatchScore(left = "", right = "") {
  return factRelationTermMatchQuality(left, right);
}

function requestedMetricRelationTermDistance(
  localScope = null,
  claim = null,
  slot = {},
  term = "",
  ...
  const text = stripStructuralNumbers(localScope?.text || "");
  const claimStart = Math.max(0, Number(claim?.index || 0) - Number(localScope?.start || 0));
  const claimEnd = claimStart + String(claim?.raw || claim?.value || "").length;
  const localClaims = normalizedNumericClaims(text, [slot]);
  let minimumDistance = Number.POSITIVE_INFINITY;
  for (const match of factRelationTokens(text)) {
    const adjacentOwners = localClaims.flatMap(candidate => {
      const start = candidate.index;
      const end = start + String(candidate.raw || candidate.value).length;
      if (!(end <= match.start || start >= match.end)) return [];
      const gap = text.slice(Math.min(end, match.end), Math.max(start, match.start));
      // A locally paired label belongs to its own number. Formatting and a
      // copula do not transfer it to a neighbouring metric's value.
      if (!/^[\s%:*_~–—-]*(?:(?:oli|olid|on|was|were|is|are)\s+)?[\s:*_~–—-]*$/iu.test(gap)) return [];
      return [{ start, distance: Math.max(0, match.start - end, start - match.end) }];
    }).sort((left, right) => left.distance - right.distance);
    if (adjacentOwners.length && adjacentOwners[0].start !== claimStart &&
      (adjacentOwners.length === 1 || adjacentOwners[0].distance < adjacentOwners[1].distance)) continue;
    const token = match.value;
    const currentScore = Math.max(...requestedAnswerRelationAlternatives(slot, term).map(variant =>
      requestedMetricDiscriminatingMatchScore(variant, token)
    ));
    const competingScore = competingTerms.length
      ? Math.max(...competingTerms.flatMap(item =>
          requestedAnswerRelationAlternatives(item.slot, item.term).map(variant =>
            requestedMetricDiscriminatingMatchScore(variant, token)
          )
        ))
      : 0;
    if (currentScore < 0.62 || currentScore <= competingScore + 0.05) continue;
    // Compare span edges, not word centres: the length of a compound label
    // must not make its adjacent value prefer the preceding total's short label.
    const distance = Math.max(0, match.start - claimEnd, claimStart - match.end);
    minimumDistance = Math.min(minimumDistance, distance);
  }
  return minimumDistance;
}

function requestedMetricUniqueRelationBoundToClaim(localScope = null, claim = null, slot = {}, slots = []) {
  const currentFingerprint = requestedMetricRelationFingerprint(slot, slots);
  const categoryModifiers = requestedMetricRequiredPeerModifiers(slot, slots);
  const peers = (Array.isArray(slots) ? slots : []).filter(peer =>
    peer !== slot &&
    requestedMetricSegmentMatchesRelation(localScope?.text || "", peer, slots) &&
    (!categoryModifiers.length || requestedMetricSlotsShareHead(slot, peer)) &&
    requestedMetricRelationFingerprint(peer, slots) &&
    requestedMetricRelationFingerprint(peer, slots) !== currentFingerprint
  );
  if (!peers.length) return { passed: true, matchedUniqueTermCount: 0 };
  // Population/verb words of a parent category do not compete with a child
  // modifier: "vajas" next to 26 must not beat "palju" for that same claim.
  const peerTermsFor = peer => {
    const peerModifiers = requestedMetricRequiredPeerModifiers(peer, slots);
    return peerModifiers.length || categoryModifiers.length
      ? peerModifiers : requestedMetricDiscriminatingTerms(peer, slots);
  };
  const currentTerms = categoryModifiers.length ? categoryModifiers : requestedMetricDiscriminatingTerms(slot, slots);
  const currentUniqueTerms = currentTerms.filter(term =>
    !peers.some(peer => peerTermsFor(peer).some(peerTerm =>
      requestedMetricRelationTermsEquivalent(slot, term, peer, peerTerm)
    ))
  );
  if (!currentUniqueTerms.length) return { passed: true, matchedUniqueTermCount: 0 };
  const competingTerms = peers.flatMap(peer =>
    peerTermsFor(peer)
      .filter(term => !currentTerms.some(currentTerm =>
        requestedMetricRelationTermsEquivalent(peer, term, slot, currentTerm)
      ))
      .map(term => ({ peer, term }))
  );
  const currentDistance = Math.min(...currentUniqueTerms.map(term =>
    requestedMetricRelationTermDistance(
      localScope,
      claim,
      slot,
      term,
      competingTerms.map(item => ({ slot: item.peer, term: item.term }))
    )
  ));
  if (!Number.isFinite(currentDistance)) return { passed: false, matchedUniqueTermCount: 0 };
  const currentCompetition = currentUniqueTerms.map(term => ({ slot, term }));
  const competingDistance = competingTerms.length
    ? Math.min(...competingTerms.map(({ peer, term }) =>
        requestedMetricRelationTermDistance(localScope, claim, peer, term, currentCompetition)
      ))
    : Number.POSITIVE_INFINITY;
  return {
    passed: !Number.isFinite(competingDistance) || currentDistance <= competingDistance,
    matchedUniqueTermCount: 1
  };
}

export function requestedNamedScopeBoundToClaim(reply = "", claim = null, slot = {}, slots = []) {
  if (!Array.isArray(slot?.named_scope_terms) || !slot.named_scope_terms.length) return { passed: true };
  const scoped = slots.map(peer => ({ ...peer,
    relation_terms: peer.named_scope_terms || peer.relation_terms,
    minimum_relation_matches: peer.named_scope_terms?.length || peer.minimum_relation_matches,
    named_scope_terms: null
  }));
  const index = slots.indexOf(slot);
  if (index < 0) return { passed: false };
  const scope = namedMetricScopeMatches(reply, claim?.index || 0,
    (claim?.index || 0) + String(claim?.raw || claim?.value || "").length, slot.named_scope_constraints);
  // Proper-name identity is stricter than general topic morphology. In
  // particular, Austria must not match Austraalia via a four-letter prefix.
  const caseForms = value => {
    const word = requestedAnswerTermToken(value);
    const stem = word.replace(/(?:sse|st|lt|le|ga|na|ks|ni|ta|s|l)$/u, "");
    return new Set([word, ...(stem.length >= 3 ? [stem] : [])]);
  };
  const local = requestedMetricClaimLocalScope(reply, claim, scoped);
  const localTokens = requestedAnswerTokens(local.text);
  if (!slot.named_scope_terms.every(term => {
    const names = caseForms(term);
    return localTokens.some(value => [...caseForms(value)].some(form => names.has(form)));
  })) return { ...scope, passed: false, named_scope_bound: false };
  const relation = requestedMetricRelationBoundToClaim(reply, claim, scoped[index], scoped);
  return { ...scope, passed: relation.passed && scope.passed, named_scope_bound: relation.passed };
}

function requestedMetricRelationBoundToClaim(reply = "", claim = null, slot = {}, slots = []) {
  const requiredTerms = requestedMetricDiscriminatingTerms(slot, slots);
  if (!requiredTerms.length) return { passed: true, matchedTerms: [] };
  const localScope = requestedMetricClaimLocalScope(reply, claim, slots);
  const unitTokens = requestedAnswerTokens(localScope.text);
  const matchedTerms = requiredTerms.filter(term =>
    requestedAnswerRelationAlternatives(slot, term).some(variant =>
      unitTokens.some(token => requestedAnswerTermMatchQuality(variant, token) >= 0.72)
    )
  );
  const minimumTermsMatched = matchedTerms.length >= requestedMetricMinimumRelationMatches(
    slot,
    requiredTerms
  );
  const requiredModifiersMatched = requestedMetricRequiredPeerModifiers(slot, slots)
    .every(term => matchedTerms.includes(term));
  const uniqueRelationBinding = minimumTermsMatched && requiredModifiersMatched
    ? requestedMetricUniqueRelationBoundToClaim(localScope, claim, slot, slots)
    : { passed: false, matchedUniqueTermCount: 0 };
  const namedScopeBinding = requestedNamedScopeBoundToClaim(reply, claim, slot, slots);
  return {
    passed: minimumTermsMatched && requiredModifiersMatched && uniqueRelationBinding.passed &&
      namedScopeBinding.passed,
    matchedTerms,
    requiredTermCount: requiredTerms.length,
    minimumTermsMatched,
    requiredModifiersMatched,
    uniqueRelationBound: uniqueRelationBinding.passed,
    namedScopeBound: namedScopeBinding.named_scope_bound ?? true,
    ageScopeBound: namedScopeBinding.age_bound,
    observationYearBound: namedScopeBinding.observation_year_bound,
    matchedUniqueTermCount: uniqueRelationBinding.matchedUniqueTermCount
  };
}

function requestedMetricClaimQualifier(reply = "", claim = null) {
  const claimIndex = Math.max(0, Number(claim?.index) || 0);
  const prefix = normalizeText(String(reply || "").slice(Math.max(0, claimIndex - 48), claimIndex));
...
const REQUESTED_METRIC_NEIGHBORING_UNIT_RE = /^(?:%|aasta\p{L}*|euro\p{L}*|inime\p{L}*|intervju\p{L}*|juhtum\p{L}*|kohtum\p{L}*|kokkulep\p{L}*|kord\p{L}*|kuu\p{L}*|nadala\p{L}*|osalej\p{L}*|paeva\p{L}*|protsent\p{L}*|riik\p{L}*|ruhm\p{L}*|spetsialist\p{L}*|vastaj\p{L}*)$/u;

function requestedMetricWordClaimIsLocallyNumeric(reply = "", claim = null, slots = []) {
  if (claim?.word !== true) return true;
  const text = String(reply || "");
  const start = Math.max(0, Number(claim?.index) || 0);
  const end = start + String(claim?.raw || "").length;
  // Count surface words before adding compound/phrase concepts. Otherwise a
  // synthetic "individual interview" token can displace the actual unit word.
  const beforeWords = (text.slice(Math.max(0, start - 72), start).match(/[\p{L}\p{N}-]+/gu) || []).slice(-2);
  const afterWords = (text.slice(end, Math.min(text.length, end + 72)).match(/[\p{L}\p{N}-]+/gu) || []).slice(0, 2);
  const beforeTokens = requestedAnswerTokens(beforeWords.join(" "));
  const afterTokens = requestedAnswerTokens(afterWords.join(" "));
  const neighboringTokens = [...beforeTokens, ...afterTokens];
  if (neighboringTokens.some(token => REQUESTED_METRIC_NEIGHBORING_UNIT_RE.test(token))) return true;
  return (Array.isArray(slots) ? slots : []).some(slot =>
    requestedMetricDiscriminatingTerms(slot).some(term =>
      requestedAnswerRelationAlternatives(slot, term).some(variant =>
        neighboringTokens.some(token => requestedAnswerTermMatchQuality(variant, token) >= 0.72)
      )
    )
  );
}

function requestedUniformMetricAssignment(message = "", reply = "", contract = null, sources = []) {
  const slots = Array.isArray(contract?.slots) ? contract.slots : [];
  const groupSlot = slots.find(slot => Number(slot.expected_cardinality) > 1);
  if (slots.length !== 2 || !groupSlot || slots.some(slot => slot.value_type !== "count")) return null;
  const totalSlot = slots.find(slot => slot !== groupSlot);
  if (!(totalSlot.relation_terms || []).some(term => /^(?:kokku|total|altogether|всего|итого)$/u.test(normalizeText(term)))) return null;
  const cardinality = Number(groupSlot.expected_cardinality);
  const claims = normalizedNumericClaims(reply, slots);
  const labels = extractNumericCategoryLabels(reply);
  const groupCue = uniformGroupCue(normalizeText(reply));
  const perGroupClaims = claims.filter(claim => claim.value === String(groupSlot.evidence_value) && !claim.percentage);
  const totalClaims = claims.filter(claim => claim.value === String(totalSlot.evidence_value) && !claim.percentage);
  const participantLabels = labels.filter(label => categoryRepresentsParticipant(label, String(totalSlot.evidence_value)));
  const completeGroupShape = (perGroupClaims.length === 1 && !!groupCue) ||
    (perGroupClaims.length === cardinality && distinctParticipantCategories(participantLabels).length === cardinality);
  const supportedSource = sources.find(source => {
    const evidence = extractUniformParticipantBreakdown(source.body);
    if (evidence.perGroupValue !== String(groupSlot.evidence_value) ||
      evidence.totalValue !== String(totalSlot.evidence_value) ||
      Number(evidence.groupCardinality || evidence.derivedGroupCardinality) !== cardinality) return false;
    // This branch replaces the generic metric assignment, so it must preserve
    // the same source -> contract -> answer qualifier contract explicitly.
    if (!evidence.qualifiersComplete ||
      evidence.perGroupQualifier !== (groupSlot.qualifier || null) ||
      evidence.totalQualifier !== (totalSlot.qualifier || null) ||
      perGroupClaims.some(claim => requestedMetricClaimQualifier(reply, claim) !== evidence.perGroupQualifier) ||
      totalClaims.some(claim => requestedMetricClaimQualifier(reply, claim) !== evidence.totalQualifier)) return false;
    const relation = uniformParticipantRelation(source, reply, labels, message);
    if (!relation.checked || relation.mismatch || !completeGroupShape || totalClaims.length !== 1) return false;
    if (unsupportedNumericCategoryLabels(message, reply, source).length) return false;
    return claims.every(claim => {
      if (perGroupClaims.includes(claim) || totalClaims.includes(claim)) return true;
      if (claim.year) return source.allNumbers.has(claim.value);
      // Only the explicit "in N groups" span may state the derived quotient.
      return !claim.percentage && Number(claim.value) === cardinality && groupCue &&
        uniformGroupCardinality(reply) === cardinality && claim.index >= groupCue.index &&
        claim.index < groupCue.index + groupCue[0].length;
    });
  });
  return {
    passed: !!supportedSource,
    source: supportedSource || null,
    reason: supportedSource ? "uniform_requested_metric_slots_validated" : "requested_uniform_group_relation_mismatch",
    assignments: supportedSource ? [
      { slot_index: groupSlot.slot_index, evidence_value: groupSlot.evidence_value,
        expected_cardinality: cardinality, claim_indexes: perGroupClaims.map(claim => claims.indexOf(claim)) },
      { slot_index: totalSlot.slot_index, evidence_value: totalSlot.evidence_value,
        claim_indexes: totalClaims.map(claim => claims.indexOf(claim)) }
    ] : []
  };
}

function requestedMetricClaimAssignment(message = "", reply = "", contract = null) {
  const slots = Array.isArray(contract?.slots) ? contract.slots : [];
  const claims = normalizedNumericClaims(reply, slots);
  const ranges = percentageRanges(stripStructuralNumbers(reply));
  const rangeClaimIndexes = new Set(ranges.flatMap(range => claims.flatMap((claim, index) =>
    claim.index === range.lowerIndex || claim.index === range.upperIndex ? [index] : []
  )));
  const contractScopeValues = new Set((Array.isArray(contract?.question_scope_values)
    ? contract.question_scope_values
    : []).map(normalizeNumber).filter(Boolean));
  const legacyUserScopeValues = new Set(normalizedNumericClaims(message)
    .filter(claim => !claim.percentage && !claim.year && claim.numeric >= 0 && claim.numeric <= 10)
    .map(claim => claim.value));
  const entries = slots.map((slot, contractIndex) => {
    const expectedValue = normalizeNumber(slot?.evidence_value);
    const expectedPercentage = slot?.value_type === "proportion" || slot?.unit === "percent";
    const qualifierContractPresent = Object.prototype.hasOwnProperty.call(slot || {}, "qualifier");
    const expectedQualifier = ["over", "under", "at_least", "at_most", "about", "range"].includes(slot?.qualifier)
      ? slot.qualifier
      : null;
    const expectedCardinality = Number.isInteger(Number(slot?.expected_cardinality)) &&
      Number(slot.expected_cardinality) > 1
      ? Number(slot.expected_cardinality)
      : null;
    const valueMatches = expectedQualifier === "range" ? ranges.flatMap(range => {
      if (!expectedPercentage || range.value !== expectedValue || range.endValue !== normalizeNumber(slot.evidence_range_end)) return [];
      const claimIndex = claims.findIndex(claim => claim.index === range.lowerIndex);
      const upperIndex = claims.findIndex(claim => claim.index === range.upperIndex);
      if (claimIndex < 0 || upperIndex < 0) return [];
      const claim = { ...claims[claimIndex], percentage: true };
      if (requestedMetricClaimQualifier(reply, claim)) return [];
      return [{ claim, claimIndex, claimIndexes: [claimIndex, upperIndex] }];
    }) : claims.flatMap((claim, claimIndex) =>
      !rangeClaimIndexes.has(claimIndex) &&
      claim.value === expectedValue &&
      claim.percentage === expectedPercentage &&
      (!qualifierContractPresent || requestedMetricClaimQualifier(reply, claim) === expectedQualifier)
        ? [{ claim, claimIndex, claimIndexes: [claimIndex] }]
        : []
    );
    const directRelationMatches = valueMatches.flatMap(edge => {
      const relationBinding = requestedMetricRelationBoundToClaim(reply, edge.claim, slot, slots);
      return relationBinding.passed ? [{ ...edge, relationBinding }] : [];
    });
    const relationMatches = directRelationMatches;
    const scopedMatches = relationMatches.flatMap(edge => {
      const scopeBinding = requestedScopeValuesBoundToClaim(
        reply,
        edge.claim,
        Array.isArray(slot?.scope_values) ? slot.scope_values : [],
        contractScopeValues
      );
      return scopeBinding.passed ? [{ ...edge, scopeBinding }] : [];
    });
    return {
      slot,
      contractIndex,
  ...
  const invalid = entries.find(entry =>
    !entry.valueMatches.length ||
    !entry.relationMatches.length ||
    !entry.scopedMatches.length
  );
  if (invalid) {
    const reason = !invalid.valueMatches.length
      ? "requested_metric_slot_missing"
      : !invalid.relationMatches.length
        ? "requested_metric_relation_mismatch"
        : "requested_metric_scope_mismatch";
    return {
      passed: false,
      reason,
      assignments: [],
      missingSlotIndex: Number(invalid.slot?.slot_index) || invalid.contractIndex + 1,
      relationDiagnostics: invalid.valueMatches.slice(0, 6).map(edge => {
        const binding = requestedMetricRelationBoundToClaim(reply, edge.claim, invalid.slot, slots);
        return {
          claim_index: edge.claimIndex,
          required_term_count: binding.requiredTermCount || 0,
          matched_term_count: binding.matchedTerms.length,
          minimum_terms_matched: binding.minimumTermsMatched === true,
          required_modifiers_matched: binding.requiredModifiersMatched === true,
          unique_relation_bound: binding.uniqueRelationBound === true,
          ...(Array.isArray(invalid.slot?.named_scope_terms) ? { named_scope_bound: binding.namedScopeBound === true } : {}),
          ...(invalid.slot?.named_scope_constraints?.age ? { age_scope_bound: binding.ageScopeBound === true } : {}),
          ...(invalid.slot?.named_scope_constraints?.observation_year ? { observation_year_bound: binding.observationYearBound === true } : {})
        };
      }),
      unexpectedClaimValues: invalid.relationMatches.flatMap(edge =>
        edge.scopeBinding?.boundValues || []
      )
    };
  }
  const solutions = [];
  const orderedEntries = [...entries].sort((left, right) =>
    left.scopedMatches.length - right.scopedMatches.length ||
    left.contractIndex - right.contractIndex
  );
  const visit = (position, usedClaimIndexes, selected) => {
    ...
    const entry = orderedEntries[position];
    for (const edge of entry.scopedMatches) {
      if (edge.claimIndexes.some(index => usedClaimIndexes.has(index))) continue;
      edge.claimIndexes.forEach(index => usedClaimIndexes.add(index));
      selected.push({ ...edge, entry });
      visit(position + 1, usedClaimIndexes, selected);
      selected.pop();
      edge.claimIndexes.forEach(index => usedClaimIndexes.delete(index));
    }
  };
  visit(0, new Set(), []);
  if (!solutions.length) {
    ...
    return {
      passed: false,
      reason: "requested_metric_slot_missing",
      assignments: [],
      missingSlotIndex: Number(entry?.slot?.slot_index) || 1,
      unexpectedClaimValues: []
    };
  }
  if (solutions.length > 1) {
    return {
      passed: false,
      reason: "requested_metric_assignment_ambiguous",
      assignments: [],
      missingSlotIndex: null,
      unexpectedClaimValues: []
    };
  }
  const selected = solutions[0].sort((left, right) =>
    left.entry.contractIndex - right.entry.contractIndex
  );
  const assignedClaimIndexes = new Set(selected.flatMap(edge => edge.claimIndexes));
  const assignments = selected.map(edge => ({
    slot_index: Number(edge.entry.slot?.slot_index) || edge.entry.contractIndex + 1,
    value_type: String(edge.entry.slot?.value_type || ""),
    evidence_value: edge.entry.expectedValue,
    ...(edge.entry.expectedQualifier === "range" ? { evidence_range_end: normalizeNumber(edge.entry.slot.evidence_range_end) } : {}),
    qualifier: edge.entry.expectedQualifier,
    claim_index: edge.claimIndex,
    claim_indexes: edge.claimIndexes,
    scope_values: (Array.isArray(edge.entry.slot?.scope_values)
      ? edge.entry.slot.scope_values
      : []).map(normalizeNumber).filter(Boolean),
    bound_scope_values: edge.scopeBinding.boundValues,
    matched_relation_terms: edge.relationBinding.matchedTerms,
    matched_unique_relation_term_count: edge.relationBinding.matchedUniqueTermCount,
    ...(edge.entry.slot?.named_scope_terms ? { named_scope_bound: edge.relationBinding.namedScopeBound === true } : {}),
    ...(edge.entry.slot?.named_scope_constraints?.age ? { age_scope_bound: edge.relationBinding.ageScopeBound === true } : {}),
    ...(edge.entry.slot?.named_scope_constraints?.observation_year ? { observation_year_bound: edge.relationBinding.observationYearBound === true } : {})
  }));
  const unexpectedClaimValues = claims.filter((claim, index) =>
    !claim.year &&
    !assignedClaimIndexes.has(index) &&
    requestedMetricWordClaimIsLocallyNumeric(reply, claim, slots) &&
    !legacyUserScopeValues.has(claim.value) &&
    !numericClaimHasQuestionScopeRole(reply, claim, contractScopeValues)
  ).map(claim => claim.value);
  return {
    passed: unexpectedClaimValues.length === 0,
    reason: unexpectedClaimValues.length
      ? "requested_metric_unexpected_numeric_claim"
      : "requested_metric_slots_validated",
    assignments,
    missingSlotIndex: null,
    unexpectedClaimValues
  };
...
export function temporalSupplementalTopicTermsFromQueryPlan(queryPlan = {}) {
  ...
  return [
    ...
    ...(Array.isArray(queryPlan?.temporal_query_contract?.topic_terms)
      ? queryPlan.temporal_query_contract.topic_terms
      : [])
...
export function temporalSupplementalTitleTopicCoverage(sourceTitle = "", topicTerms = []) {
  const topicTokens = temporalSupplementalTopicTokens(topicTerms);
  return topicTokens.filter(token =>
    temporalUnitMatchesSupplementalTopic(sourceTitle, [token])
  ).length;
}
...
export function buildTemporalSupplementalSourceScopes({
  ...
  const targetYearSet = new Set(normalizedTemporalTargetYears(targetYears));
  const primaryEvidenceUnitSet = new Set((Array.isArray(primaryEvidenceUnits) ? primaryEvidenceUnits : [])
    .map(unit => normalizeText(unit))
    .filter(Boolean));
  if (!primaryId || !topicTokens.length) return [];
  const scopes = [];
  const typedScopeCandidates = new Map();
  ...
  for (const [sourceIndex, rawSource] of (Array.isArray(sources) ? sources : []).entries()) {
    ...
    const samePrimaryDocument = sourceId === primaryId ||
      (primaryDocId && documentId === primaryDocId);
    const samePrimaryTitle = normalizedPrimaryTitle && normalizedTitle === normalizedPrimaryTitle;
    if (
      !source.body ||
      !sourceId ||
      !documentId ||
    ...
    if (trendRequested) {
      const relation = samePrimaryDocument
        ? "same_document_later_development"
        : "companion_source_context";
      if (
        !samePrimaryDocument &&
        (!targetYearSet.has(publicationYear) || !temporalUnitMatchesSupplementalTopic(title, topicTokens))
      ) continue;
      const latestTargetYear = Math.max(...targetYearSet);
      const developmentSpan = preferredTemporalQualitativeDevelopmentSpan(source.body, {
        sourceTitle: title,
        topicTerms,
        requireTitleTopicMatch: !samePrimaryDocument,
        minimumEvidenceYearExclusive: samePrimaryDocument && Number.isInteger(latestTargetYear)
          ? latestTargetYear
          : null
      });
      const evidenceUnit = String(developmentSpan?.text || "").replace(/\s+/gu, " ").trim();
      const evidenceYears = temporalNumericClaims(evidenceUnit)
        .filter(claim => claim.year)
        .map(claim => claim.numeric);
      if (
        !evidenceUnit ||
        evidenceUnit.length < 45 ||
        evidenceUnit.length > 480 ||
        (samePrimaryDocument && (
          !Number.isInteger(latestTargetYear) ||
          !evidenceYears.length ||
          Math.min(...evidenceYears) <= latestTargetYear
        )) ||
        primaryEvidenceUnitSet.has(normalizeText(evidenceUnit)) ||
        temporalAggregatePeriodFraming(evidenceUnit) ||
        temporalUnitStatesEvidenceUnavailable(evidenceUnit)
      ) continue;
      const candidates = typedScopeCandidates.get(relation) || [];
      candidates.push({
        sourceIndex,
        titleTopicCoverage: Number(developmentSpan?.titleTopicCoverage || 0),
        developmentScore: Number(developmentSpan?.score || 0),
        scope: {
          relation,
      ...
      typedScopeCandidates.set(relation, candidates);
      continue;
    }
    if (samePrimaryDocument || samePrimaryTitle) continue;
  ...
  if (trendRequested) {
    const strongestScope = relation => (typedScopeCandidates.get(relation) || [])
      .sort((left, right) =>
    ...
    return [
      strongestScope("companion_source_context"),
      strongestScope("same_document_later_development")
    ].filter(Boolean);
  }
  return scopes;
}
...
function temporalClaimContractFromRetrievalMeta(retrievalMeta = null, sources = null) {
  ...
  const qualitativeContextRequested = rawContract.qualitative_context_requested === true;
  if (qualitativeContextRequested !== (typedContract?.comparison_requested === true)) return null;
  ...
  const normalizedSupplementalScopes = scopes => (Array.isArray(scopes) ? scopes : [])
    .map(scope => ({
      relation: ["supplemental_context", "same_document_later_development", "companion_source_context"].includes(
        String(scope?.relation || "")
      )
        ? String(scope.relation)
...
function temporalTextUnitSpans(value = "") {
  const text = String(value || "");
  const spans = [];
  let unitStart = 0;
  const addSpan = endExclusive => {
    let start = unitStart;
    let end = Math.max(start, Math.min(text.length, endExclusive));
    while (start < end && /\s/u.test(text[start])) start += 1;
    while (end > start && /\s/u.test(text[end - 1])) end -= 1;
    if (end > start) spans.push({ text: text.slice(start, end), start, end });
    unitStart = endExclusive;
  };
  for (const boundary of temporalPunctuationBoundaries(text)) addSpan(boundary + 1);
  addSpan(text.length);
  return spans;
}

function temporalQualitativeDevelopmentCue(value = "") {
  const normalized = normalizeText(value);
  const transition = /(?:tulemuste\s+pohjal|praeguseks|uus\s+(?:tegevus)?(?:periood|etapp|faas)|jargmin\p{L}*\s+(?:periood|etapp|faas)|plaanis|susteem\p{L}*\s+arendustegevus|loomulikuks\s+osaks|based\s+on\s+(?:the\s+)?results|by\s+now|new\s+(?:period|stage|phase)|next\s+(?:period|stage|phase)|plan\p{L}*\s+to|system\p{L}*\s+development|regular\s+part|по\s+итогам|к\s+настоящему\s+времени|нов\p{L}*\s+(?:период|этап|фаз)|следующ\p{L}*\s+(?:период|этап|фаз)|планир\p{L}*|системн\p{L}*\s+развит|неотъемлем\p{L}*\s+част)/u;
  const development = /(?:alg\p{L}*|jatk\p{L}*|liitun\p{L}*|laien\p{L}*|rakenda\p{L}*|arend\p{L}*|kujunda\p{L}*|integreer\p{L}*|kinnist\p{L}*|start\p{L}*|continu\p{L}*|join\p{L}*|expand\p{L}*|implement\p{L}*|develop\p{L}*|shape\p{L}*|integrat\p{L}*|institutionali\p{L}*|нача\p{L}*|продолж\p{L}*|присоедин\p{L}*|расшир\p{L}*|внедр\p{L}*|разви\p{L}*|формир\p{L}*|интегрир\p{L}*)/u;
  return transition.test(normalized) && development.test(normalized);
}

function temporalQualitativeDevelopmentStrength(value = "") {
  const normalized = normalizeText(value);
  let score = 0;
  if (/(?:susteem\p{L}*\s+arend\p{L}*|loomulikuks\s+osaks|integreer\p{L}*|kinnist\p{L}*|system\p{L}*\s+develop\p{L}*|regular\s+part|integrat\p{L}*|institutionali\p{L}*|системн\p{L}*\s+разви\p{L}*|неотъемлем\p{L}*\s+част|интегрир\p{L}*)/u.test(normalized)) score += 8;
  if (/(?:laien\p{L}*|rakenda\p{L}*|kujunda\p{L}*|expand\p{L}*|implement\p{L}*|shape\p{L}*|расшир\p{L}*|внедр\p{L}*|формир\p{L}*)/u.test(normalized)) score += 5;
  if (/(?:liitun\p{L}*|kaasa\p{L}*|join\p{L}*|involv\p{L}*|присоедин\p{L}*|вовлеч\p{L}*)/u.test(normalized)) score += 3;
  if (/(?:alg\p{L}*|jatk\p{L}*|start\p{L}*|continu\p{L}*|нача\p{L}*|продолж\p{L}*)/u.test(normalized)) score += 1;
  return score;
}

export function preferredTemporalQualitativeDevelopmentSpan(value = "", {
  sourceTitle = "",
  topicTerms = [],
  maxChars = null,
  requireTitleTopicMatch = false,
  minimumEvidenceYearExclusive = null
} = {}) {
  const text = String(value || "");
  const boundedMaxChars = maxChars !== null &&
    maxChars !== undefined &&
    String(maxChars).trim() &&
    Number.isFinite(Number(maxChars))
    ? Math.max(1, Math.trunc(Number(maxChars)))
    : null;
  const topicTokens = temporalSupplementalTopicTokens(topicTerms);
  if (!text || !topicTokens.length) return null;
  const titleTopicCoverage = temporalSupplementalTitleTopicCoverage(sourceTitle, topicTerms);
  const titleMatches = titleTopicCoverage > 0;
  if (requireTitleTopicMatch && !titleMatches) return null;
  const minimumEvidenceYear = minimumEvidenceYearExclusive !== null &&
    minimumEvidenceYearExclusive !== undefined &&
    String(minimumEvidenceYearExclusive).trim() &&
    Number.isInteger(Number(minimumEvidenceYearExclusive))
    ? Number(minimumEvidenceYearExclusive)
    : null;
  const spans = temporalTextUnitSpans(text);
  const candidates = [];
  for (const [index, span] of spans.entries()) {
    const unit = String(span.text || "").trim();
    if (
      unit.length < 35 ||
      unit.length > 480 ||
      !temporalQualitativeDevelopmentCue(unit) ||
      temporalUnitStatesEvidenceUnavailable(unit) ||
      temporalNumericClaims(unit).some(claim => !claim.year)
    ) continue;
    let start = span.start;
    if (/^(?:suurim\s+neist|see|selle|these|this|the\s+largest\s+of\s+these|крупнейш\p{L}*\s+из\s+них)(?=$|[^\p{L}])/u.test(normalizeText(unit))) {
      const previous = spans[index - 1];
      const between = previous ? text.slice(previous.end, span.start) : "";
      const combinedLength = previous ? span.end - previous.start : unit.length;
      if (
        previous &&
        combinedLength <= 480 &&
        !/\n\s*\n/u.test(between) &&
        !temporalUnitStatesEvidenceUnavailable(previous.text) &&
        !temporalNumericClaims(previous.text).some(claim => !claim.year)
      ) start = previous.start;
    }
    for (let endIndex = index; endIndex < Math.min(spans.length, index + 4); endIndex += 1) {
      const endSpan = spans[endIndex];
      const priorEnd = spans[endIndex - 1]?.end ?? span.end;
      if (endIndex > index && /\n\s*\n/u.test(text.slice(priorEnd, endSpan.start))) break;
      const candidateText = text.slice(start, endSpan.end).replace(/\s+/gu, " ").trim();
      if (candidateText.length > 480) break;
      const markerChars = (start > 0 ? 3 : 0) + (endSpan.end < text.length ? 3 : 0);
      if (boundedMaxChars !== null && candidateText.length + markerChars > boundedMaxChars) continue;
      if (
        temporalUnitStatesEvidenceUnavailable(candidateText) ||
        temporalNumericClaims(candidateText).some(claim => !claim.year)
      ) break;
      const candidateEvidenceYears = temporalNumericClaims(candidateText)
        .filter(claim => claim.year)
        .map(claim => claim.numeric);
      if (
        minimumEvidenceYear !== null &&
        (!candidateEvidenceYears.length || Math.min(...candidateEvidenceYears) <= minimumEvidenceYear)
      ) continue;
      const unitMatches = topicTokens.filter(token =>
        temporalUnitMatchesSupplementalTopic(candidateText, [token])
      ).length;
      if (!unitMatches && !titleMatches) continue;
      candidates.push({
        text: candidateText,
        start,
        end: endSpan.end,
        titleTopicCoverage,
        score: unitMatches * 10 +
          temporalQualitativeDevelopmentStrength(candidateText) * 4 +
          (titleMatches ? 2 : 0) +
          (temporalNumericClaims(candidateText).some(claim => claim.year) ? 1 : 0)
      });
    }
  }
  return candidates.sort((left, right) => right.score - left.score || left.start - right.start)[0] || null;
}
...
function temporalAggregateCanonicalReply(primaryReply = "", supplementalBindings = [], originalReply = "", replyLang = "") {
  ...
  const language = temporalAggregateReplyLanguage(originalReply, replyLang);
  const typedQualitative = bindings.some(binding =>
    ["same_document_later_development", "companion_source_context"].includes(binding?.relation)
  );
  const heading = typedQualitative
    ? language === "ru"
      ? "Содержательное развитие:"
      : language === "en"
        ? "Development described by the sources:"
        : "Sisuline areng:"
    : language === "ru"
  ...
  const lines = bindings.map(binding => {
    const relationLabel = binding.relation === "companion_source_context"
      ? language === "ru"
        ? `По источнику за ${binding.publication_year} год`
        : language === "en"
          ? `According to the ${binding.publication_year} source`
          : `${binding.publication_year}. aasta allika järgi`
      : binding.relation === "same_document_later_development"
        ? language === "ru"
          ? `Следующий этап по источнику за ${binding.publication_year} год`
          : language === "en"
            ? `Following stage according to the ${binding.publication_year} source`
            : `Järgmine etapp ${binding.publication_year}. aasta allika järgi`
        : "";
    return relationLabel
      ? `- ${relationLabel}: ${binding.evidence_unit}`
...
function validateTemporalSupplementalSection(reply = "", scopes = []) {
  ...
  for (const line of lines) {
    ...
    bindings.push({
      relation: matchedScope.relation,
...
function validateTemporalAggregatePeriodFallback({ reply = "", contract = null, eligibleSources = [], replyLang = "et" } = {}) {
  ...
  const deterministicTypedBindings = contract?.qualitativeContextRequested === true
    ? supplementalSourceScopes
        .filter(scope => ["same_document_later_development", "companion_source_context"].includes(scope.relation))
        .map(scope => ({
          ...
          title: scope.title,
          evidence_unit: scope.evidence_units[0]
        }))
        .filter(binding => binding.evidence_unit)
    : [];
  const resolvedSupplementalBindings = deterministicTypedBindings.length
    ? deterministicTypedBindings
    : supplementalValidation.bindings;
  const resolvedSupplementalSourceIds = Array.from(new Set(
    resolvedSupplementalBindings.map(binding => binding.source_id).filter(Boolean)
  ));
  ...
  return {
    ...
    supplementalSupportingSourceIds: resolvedSupplementalSourceIds,
    supplementalBindings: resolvedSupplementalBindings,
...
function institutionalGuidanceContainsExactContact(message = "", reply = "") {
  if (!isInstitutionalApplicationGuidance(message)) return false;
  if (contactPhoneClaims(reply).length || contactEmailClaims(reply).length) return true;
  // An institutional destination does not authorize inventing a named worker.
  const text = String(reply).replace(/[*_`]/gu, " ");
  const name = "([\\p{Lu}][\\p{L}'’-]+(?:\\s+[\\p{Lu}][\\p{L}'’-]+){1,3})";
  const role = "(?:[Kk]ontaktisik|[Ss]otsiaaltöötaj|[Ss]petsialist|[Aa]metnik)\\p{L}*";
  const candidates = [
    ...text.matchAll(new RegExp(`(?:[Pp]öördu\\p{L}*|${role})\\s*[:–-]?\\s+(?:on\\s+|poole\\s+)?${name}`, "gu")),
    ...text.matchAll(new RegExp(`${name}\\s*(?:[,–-]\\s*|on\\s+)${role}`, "gu"))
  ];
  return candidates.some(match => !/(?:linna|valla)valitsus|sotsiaalosakond|sotsiaalkindlustusamet|teenusekeskus/u.test(normalizeText(match[1])));
}
...
function parsedCategoryNumberValue(value = "") {
  const normalized = normalizeText(value);
  const cardinal = smallCardinalNumberValue(normalized);
  if (cardinal !== null) return String(cardinal);
...
function categoryTokenMatches(left = "", right = "") {
  ...
  if (factRelationTermMatchQuality(left, right) >= 0.9) return true;
...
function isEntityCoverageCategoryLabel(value = "") {
  ...
  if (/^kohtunik\p{L}*$/u.test(finalToken)) return false;
...
function uniformGroupCue(value = "") {
  ...
  const patterns = [
    new RegExp(
      `(?:^|[^\\p{L}])(?:iga\\p{L}*|koig\\p{L}*)\\s+(?:${estonianGroupCount}\\s+)?(?:[\\p{L}-]+\\s+){0,2}?(?:siht\\s*[-–—]?\\s*ruhm\\p{L}*|ruhm\\p{L}*)(?=$|[^\\p{L}])`,
      "u"
    ),
    new RegExp(
      `(?:^|[^\\p{L}])(?:(?:in|from)\\s+)?(?:each|every)\\s+(?:(?:of\\s+)?(?:the\\s+)?)?(?:${CATEGORY_NUMBER_TOKEN_SOURCE}\\s+)?(?:[\\p{L}-]+\\s+){0,2}?(?:groups?|categories?)(?=$|[^\\p{L}])`,
      "u"
    ),
    /(?:^|[^\p{L}])(?:ruhm\p{L}*|kategoor\p{L}*)\s+kohta(?=$|[^\p{L}])/u,
    /(?:^|[^\p{L}])per\s+(?:group|category)(?=$|[^\p{L}])/u,
    new RegExp(
      `(?:^|[^\\p{L}])(?:(?:в|из|по)\\s+)?кажд\\p{L}*(?:\\s+из)?\\s+(?:${russianGroupCount}\\s+)?(?:[\\p{L}-]+\\s+){0,2}?(?:групп\\p{L}*|категор\\p{L}*)(?=$|[^\\p{L}])`,
      "u"
    )
  ];
  return patterns
...
function uniformGroupCardinality(value = "") {
  const cue = uniformGroupCue(normalizeText(value));
  if (!cue) return null;
  const cardinality = [...cue[0].matchAll(/\d+|\p{L}+/gu)]
    .map(match => ({
      value: /^\d+$/u.test(match[0]) ? Number(match[0]) : smallCardinalNumberValue(match[0]),
      index: match.index
    }))
    .find(claim => Number.isInteger(claim.value) && claim.value >= 1 && claim.value <= 20);
  return cardinality && !requestedMetricClaimQualifier(cue[0], cardinality) ? cardinality.value : null;
}
...
export function extractUniformParticipantBreakdown(value = "") {
  ...
  let inferredParticipantCategoryCount = 0;
  ...
  if (!perGroupValues.size) {
    const knownTotalValue = totalValues.size === 1 ? [...totalValues][0] : null;
    const participantCategories = extractNumericCategoryLabels(value)
      .filter(category => categoryRepresentsParticipant(category, knownTotalValue));
    inferredParticipantCategoryCount = distinctParticipantCategories(participantCategories).length;
    const categoryValues = new Set(participantCategories.map(category => category.value));
    if (inferredParticipantCategoryCount >= 2 && categoryValues.size === 1) {
      perGroupValues.add([...categoryValues][0]);
    }
  }
  const perGroupValue = perGroupValues.size === 1 ? [...perGroupValues][0] : null;
  const totalValue = totalValues.size === 1 ? [...totalValues][0] : null;
  const normalized = normalizeText(value);
  const numericClaims = alignedCategoryNumericClaims(normalized);
  const qualifiersFor = expectedValue => new Set(numericClaims
    .filter(claim => claim.value === expectedValue)
    .map(claim => requestedMetricClaimQualifier(normalized, claim)));
  const perGroupQualifiers = qualifiersFor(perGroupValue);
  const totalQualifiers = qualifiersFor(totalValue);
  const qualifiersComplete = perGroupQualifiers.size === 1 && totalQualifiers.size === 1;
  const perGroupQualifier = perGroupQualifiers.size === 1 ? [...perGroupQualifiers][0] : null;
  const totalQualifier = totalQualifiers.size === 1 ? [...totalQualifiers][0] : null;
  // Infer only inside a sentence that explicitly states BOTH a uniform group
  // size and the same sample total. Unrelated totals elsewhere cannot supply it.
  const hasSameSampleRelation = sentences.some(sentence => uniformGroupCue(sentence) &&
    countedParticipantValues(sentence).some(count => String(count.value) === String(perGroupValue)) &&
    extractWholeScopeNumbers(sentence, { includeFallback: false }).has(totalValue));
  const quotient = hasSameSampleRelation && qualifiersComplete && !perGroupQualifier && !totalQualifier && Number(perGroupValue) > 0
    ? Number(totalValue) / Number(perGroupValue) : null;
  const derivedCardinality = Number.isInteger(quotient) && quotient >= 2 && quotient <= 20 ? quotient : null;
  return {
    perGroupValue,
    totalValue,
    perGroupQualifier,
    totalQualifier,
    qualifiersComplete,
    groupCardinality: uniformGroupCardinality(value) || inferredParticipantCategoryCount || null,
    derivedGroupCardinality: derivedCardinality
  };
}

function distinctParticipantCategories(categories = []) {
  const distinct = [];
  for (const category of categories) {
    const tokens = significantCategoryLabelTokens(category.label);
    if (!tokens.length) continue;
    if (distinct.some(previous => {
      const previousTokens = significantCategoryLabelTokens(previous.label);
      return tokens.every(token => previousTokens.some(other => categoryTokenMatches(token, other))) &&
        previousTokens.every(token => tokens.some(other => categoryTokenMatches(token, other)));
    })) continue;
...
function uniformParticipantRelation(source = {}, reply = "", categoryLabels = [], message = "") {
  ...
  const participantCategories = categoryLabels
    .filter(category => categoryRepresentsParticipant(category, knownTotalValue));
  const expectedGroupCardinality = uniformGroupCardinality(message);
  const sourceGroupCardinality = Number(sourceBreakdown.groupCardinality || sourceBreakdown.derivedGroupCardinality || 0) || null;
  const replyGroupCue = uniformGroupCue(normalizeText(reply));
  const replyGroupCardinality = uniformGroupCardinality(reply);
  const distinctParticipantCategoryCount = distinctParticipantCategories(participantCategories).length;
  const groupCardinalitySatisfied = !expectedGroupCardinality ||
    replyGroupCardinality === expectedGroupCardinality ||
    (Boolean(replyGroupCue) && replyGroupCardinality === null) ||
    distinctParticipantCategoryCount >= expectedGroupCardinality;
  const sourceGroupCardinalitySatisfied = !expectedGroupCardinality ||
    sourceGroupCardinality === expectedGroupCardinality;
  ...
  return {
    ...
    mismatch: perGroupMismatch || totalMismatch || mismatchedCategoryValues.length > 0 ||
      !groupCardinalitySatisfied || !sourceGroupCardinalitySatisfied,
    ...
    observedTotalValue: sourceHasUniformParticipantRelation
      ...
      : null,
    expectedGroupCardinality,
    sourceGroupCardinality,
    sourceGroupCardinalitySatisfied,
    observedGroupCardinality: replyGroupCardinality,
    observedParticipantCategoryCount: distinctParticipantCategoryCount,
    groupCardinalitySatisfied
  };
}
...
function failureReply(replyLang = "et") {
  if (replyLang === "en") {
    return "I can’t confirm the requested value, scope, and year reliably enough to give an exact answer.";
  }
  if (replyLang === "ru") {
    return "Я не могу достаточно надёжно подтвердить запрошенные значение, охват и год, чтобы дать точный ответ.";
  }
  return "Ma ei saa küsitud arvu, ulatust ja aastat täpse vastuse jaoks piisavalt kindlalt kinnitada.";
}

function requestedFactCoverageFailureReply(replyLang = "et") {
  if (replyLang === "en") {
    return "I can’t confirm every requested part from the selected source reliably enough.";
  }
  if (replyLang === "ru") {
    return "Я не могу достаточно надёжно подтвердить все запрошенные части по выбранному источнику.";
  }
  return "Ma ei saa kõiki küsitud osi valitud allika põhjal piisavalt kindlalt kinnitada.";
}

function requestedFactEvidenceIncomplete(retrievalMeta = null) {
  const coverage = retrievalMeta?.requestedFactEvidenceCoverage;
  const routeMode = String(retrievalMeta?.queryPlan?.mode || "").trim();
  const plannerMode = String(retrievalMeta?.queryPlan?.question_planner?.mode || "").trim();
  return [routeMode, plannerMode].includes("specific_research_fact") &&
    coverage?.enabled === true &&
    coverage?.complete !== true;
}

function requestedFactPlanIncomplete(retrievalMeta = null) {
  const routeMode = String(retrievalMeta?.queryPlan?.mode || "").trim();
  const plannerMode = String(retrievalMeta?.queryPlan?.question_planner?.mode || "").trim();
  if (![routeMode, plannerMode].includes("specific_research_fact")) return false;
  const requested = requestedFactPlanFromRetrievalMeta(retrievalMeta);
  const recognizedClauses = Number(requested?.numeric_clause_count || 0) +
    Number(requested?.qualitative_clause_count || 0);
  const requestedSlots = Array.isArray(requested?.slots) ? requested.slots : [];
  const contractReason = String(
    retrievalMeta?.requestedFactSlotContract?.reason ||
    retrievalMeta?.requestedMetricContract?.reason ||
    retrievalMeta?.requestedQualitativeSlotContract?.reason ||
    ""
  );
  const metricValueTypes = new Set([
    "count", "amount", "duration", "magnitude", "proportion", "calendar_year",
    "explicit_value_relation"
  ]);
  const qualitativeValueTypes = new Set([
    "category", "date", "entity_list", "location", "method", "month", "organization",
    "person_role", "recommendation", "season", "text_relation", "timepoint"
  ]);
  const metricSlotCount = requestedSlots.filter(slot => metricValueTypes.has(String(slot?.value_type || ""))).length;
  const qualitativeSlotCount = requestedSlots.filter(slot => qualitativeValueTypes.has(String(slot?.value_type || ""))).length;
  const metricContract = retrievalMeta?.requestedFactSlotContract || retrievalMeta?.requestedMetricContract || null;
  const qualitativeContract = retrievalMeta?.requestedQualitativeSlotContract || null;
  const finalCoverageMeasured = retrievalMeta?.requestedFactEvidenceCoverage?.enabled === true;
  const metricContractIncomplete = metricSlotCount > 0 && (
    (metricContract && (metricContract.enabled !== true || metricContract.complete !== true)) ||
    (finalCoverageMeasured && !metricContract)
  );
  const qualitativeContractIncomplete = qualitativeSlotCount > 0 && (
    (qualitativeContract && (qualitativeContract.enabled !== true || qualitativeContract.complete !== true)) ||
    (finalCoverageMeasured && !qualitativeContract)
  );
  return contractReason === "requested_slots_incomplete" || metricContractIncomplete || qualitativeContractIncomplete || (
    requested &&
    requested.complete !== true &&
    (recognizedClauses > 0 || requestedSlots.length > 0)
  );
}

function contactFailureReply(replyLang = "et") {
  if (replyLang === "en") {
    return "I can’t confirm the requested current contact details reliably enough.";
  }
  if (replyLang === "ru") {
    return "Я не могу достаточно надёжно подтвердить запрошенные актуальные контактные данные.";
  }
  return "Ma ei saa küsitud praeguseid kontaktandmeid piisavalt kindlalt kinnitada.";
}

function answerContainsHardClaim(reply = "") {
  const text = stripStructuralNumbers(String(reply || ""));
  if (!text.trim()) return false;
  return /(?<![\p{L}\d])\d+(?:[.,]\d+)?\s*%?(?![\p{L}\d])/u.test(text) ||
    /(?:§\s*\d+[a-z]?|\b(?:\+?372\s*)?(?:\d[\s-]?){7,8}\b|\b\d{1,2}[./-]\d{1,2}[./-](?:\d{2}|\d{4})\b)/iu.test(text);
}

function generatedHardClaimValidationRoute(retrievalMeta = null) {
  const routeMode = String(retrievalMeta?.queryPlan?.mode || "").trim();
  const plannerMode = String(
    retrievalMeta?.queryPlan?.question_planner?.mode ||
    retrievalMeta?.questionPlan?.mode ||
    ""
  ).trim();
  return new Set([
...
export function shouldValidateExactFactAnswer({ message = "", reply = "", sources = [], retrievalMeta = null } = {}) {
  if (institutionalGuidanceContainsExactContact(message, reply)) return true;
  if (requestedFactPlanIncomplete(retrievalMeta)) return true;
  if (requestedFactEvidenceIncomplete(retrievalMeta)) return true;
  const requestedFactRouteModes = new Set([
    String(retrievalMeta?.queryPlan?.mode || "").trim(),
    String(retrievalMeta?.queryPlan?.question_planner?.mode || "").trim()
  ]);
  const requestedFactSlots = requestedFactSlotsFromRetrievalMeta(retrievalMeta);
  if (requestedFactRouteModes.has("specific_research_fact") && requestedFactSlots.length) return true;
  ...
  if (requestedMetricContractFromRetrievalMeta(retrievalMeta)) return true;
  if (requestedQualitativeContractFromRetrievalMeta(retrievalMeta)) return true;
  // Bibliographic years and multi-source synthesis dates are not an exact
  // numeric answer contract. Generated-claim validation is reserved for
  // KOV/legal routes where an operational number can otherwise be invented.
  if (answerContainsHardClaim(reply) && generatedHardClaimValidationRoute(retrievalMeta)) {
...
function numericClaimPattern(value = "") {
  ...
  return new RegExp(
    `(?<![\\p{L}\\d])${escapedParts.join("[.,]")}(?:\\s*%)?(?![\\p{L}\\d])`,
    "u"
  );
}

function removeUnsupportedNumericClaimSegments(reply = "", unsupportedValues = []) {
  const patterns = (Array.isArray(unsupportedValues) ? unsupportedValues : [])
    .map(numericClaimPattern)
    .filter(Boolean);
  ...
  for (const rawLine of String(reply || "").split(/\r?\n/gu)) {
    ...
    if (!line) {
      if (keptLines.length && keptLines.at(-1) !== "") keptLines.push("");
      continue;
    }
    const prefix = rawLine.match(/^\s*(?:[-*•]|\d+[.)])\s*/u)?.[0] || "";
    const body = prefix ? rawLine.slice(prefix.length) : rawLine;
    const sentences = body.split(/(?<=[.!?])\s+/u).filter(Boolean);
    const safeSentences = sentences.filter(sentence => !patterns.some(pattern => pattern.test(sentence)));
    if (!safeSentences.length) continue;
    keptLines.push(`${prefix}${safeSentences.join(" ")}`.trimEnd());
  }
  return keptLines.join("\n").replace(/\n{3,}/gu, "\n\n").trim();
}

function recoverCompleteRequestedMetricReply({ message, reply, validation, sources, retrievalMeta, replyLang }) {
  const contract = requestedMetricContractFromRetrievalMeta(retrievalMeta);
  const identity = retrievalMeta?.documentIdentityEvidence;
  if (!["requested_metric_unexpected_numeric_claim", "requested_metric_relation_mismatch"].includes(validation?.trace?.reason) ||
    !contract || contract.source !== "final_rendered_evidence" ||
    identity?.required !== true || identity?.matched !== true || identity?.confidence !== "high" ||
    !sources.length || contract.slots.some(slot =>
      !["count", "proportion"].includes(slot.value_type) ||
      slot.qualifier === "range" ||
      (slot.scope_values || []).length || Number(slot.expected_cardinality) > 1 ||
      !(slot.relation_terms || []).length
    )) return null;
  // A relation failure may be rebuilt from the source contract, never accepted
  // as-is. Keep this path closed when any requested source value is absent.
  if (validation.trace.reason === "requested_metric_relation_mismatch") {
    const claims = normalizedNumericClaims(stripStructuralNumbers(reply), contract.slots);
    if (contract.slots.some(slot => !claims.some(claim =>
      claim.value === normalizeNumber(slot.evidence_value) &&
      claim.percentage === (slot.value_type === "proportion")
    ))) return null;
  }
  const qualitative = requestedQualitativeContractFromRetrievalMeta(retrievalMeta);
  const bindings = validation.trace.requested_fact_qualitative_slot_bindings || [];
  const units = requestedAnswerUnits(reply);
  if ((qualitative?.slots || []).some(slot => !bindings.some(binding => binding.slot_index === slot.slot_index))) return null;
  const qualifiers = replyLang === "en"
    ? { over: "over", under: "under", at_least: "at least", at_most: "at most", about: "about" }
    : replyLang === "ru"
      ? { over: "более", under: "менее", at_least: "не менее", at_most: "не более", about: "около" }
      : { over: "üle", under: "alla", at_least: "vähemalt", at_most: "kuni", about: "umbes" };
  const rows = contract.slots.map(slot => {
    const label = slot.relation_terms.join(" ");
    const qualifier = qualifiers[slot.qualifier] || "";
    return { index: slot.slot_index, text: `${label[0].toUpperCase()}${label.slice(1)}: ${qualifier ? `${qualifier} ` : ""}${slot.evidence_value}${slot.value_type === "proportion" ? "%" : ""}.` };
  });
  const keptUnits = new Set();
  for (const binding of bindings) {
    if (keptUnits.has(binding.unit_index)) continue;
    const unit = units.find(candidate => candidate.index === binding.unit_index);
    if (!unit) return null;
    keptUnits.add(binding.unit_index);
    rows.push({ index: binding.slot_index, text: unit.text });
  }
  const recoveredReply = rows.sort((left, right) => left.index - right.index).map(row => row.text).join("\n");
  // Rebuild only from the already complete rendered contract, preserve only
  // validated qualitative units, then run every original gate again.
  const checked = validateExactFactAnswer({ message, reply: recoveredReply, sources, retrievalMeta, replyLang });
  if (!checked.passed) return null;
  return { ...checked, trace: { ...checked.trace, requested_metric_recovery: true,
    recovery_original_reason: validation.trace.reason } };
}

export function recoverSupportedReplyAfterNumericValidation({
  message = "",
  reply = "",
  validation = null,
  retrievalMeta = null,
  ...
  if (validation?.passed === false) {
    const recovered = recoverCompleteRequestedMetricReply({ message, reply, validation, sources, retrievalMeta, replyLang });
    if (recovered) return recovered;
  }
  const trace = validation?.trace && typeof validation.trace === "object" ? validation.trace : null;
  if (
    validation?.passed !== false ||
    !trace ||
    !new Set(["unsupported_numeric_claim", "requested_metric_unexpected_numeric_claim"]).has(trace.reason) ||
    !generatedHardClaimValidationRoute(retrievalMeta)
  ) return validation;
  const unsupportedValues = Array.isArray(trace.unsupported_claim_values)
    ? trace.unsupported_claim_values.map(value => String(value || "").trim()).filter(Boolean)
    : [];
  const recoveredReply = removeUnsupportedNumericClaimSegments(reply, unsupportedValues);
  const recoveredWordCount = recoveredReply.split(/\s+/u).filter(Boolean).length;
  const stillContainsUnsupportedValue = unsupportedValues.some(value => numericClaimPattern(value)?.test(recoveredReply));
  if (!recoveredReply || recoveredWordCount < 8 || stillContainsUnsupportedValue) return validation;
  return {
    passed: true,
    ...
    trace: {
      ...trace,
      passed: true,
      reason: "unsupported_numeric_claims_removed",
      original_reason: trace.reason,
      removed_unsupported_claim_values: unsupportedValues,
      claim_level_recovery: true
    }
  };
}
...
export function validateExactFactAnswer({
  ...
  const directedRelationValidation = validateDirectedRelationReply({ retrievalMeta, sources, reply, replyLang });
  if (directedRelationValidation) return directedRelationValidation;
  const groupValidation = validateGroupFactReply({ retrievalMeta, sources, reply, replyLang });
  ...
  const authorCorpus = retrievalMeta?.authorCorpusEvidence && typeof retrievalMeta.authorCorpusEvidence === "object"
    ...
    : null;
  const questionScopeValues = questionScopeValuesFromRetrievalMeta(retrievalMeta);
  const temporalDecision = temporalYearDecision(message, retrievalMeta);
  const requestedMetricContract = requestedMetricContractFromRetrievalMeta(retrievalMeta);
  const requestedQualitativeContract = requestedQualitativeContractFromRetrievalMeta(retrievalMeta);
  const claims = normalizedNumericClaims(reply, requestedMetricContract?.slots || []);
  const baseTrace = {
    version: FACT_VALIDATOR_VERSION,
    enabled: true,
    buffered: true,
    passed: false,
    ...
    author_corpus_document_count: Number.isInteger(Number(authorCorpus?.documentCount))
      ...
      : null,
    requested_metric_contract_checked: requestedMetricContract !== null,
    requested_fact_slot_contract_checked: requestedMetricContract !== null,
    requested_metric_slot_count: requestedMetricContract?.slots?.length || 0,
    requested_qualitative_contract_checked: requestedQualitativeContract !== null,
    requested_qualitative_slot_count: requestedQualitativeContract?.slots?.length || 0,
  ...
  if (requestedFactPlanIncomplete(retrievalMeta)) {
    return {
      passed: false,
      reply: requestedFactCoverageFailureReply(replyLang),
      trace: {
        ...baseTrace,
        reason: "requested_fact_plan_incomplete"
      }
    };
  }
  if (requestedFactEvidenceIncomplete(retrievalMeta)) {
    const coverage = retrievalMeta?.requestedFactEvidenceCoverage;
    return {
      passed: false,
      reply: requestedFactCoverageFailureReply(replyLang),
      trace: {
        ...baseTrace,
        reason: "requested_fact_evidence_incomplete",
        requested_fact_missing_slot_indexes: Array.isArray(coverage?.missing_slot_indexes)
          ? coverage.missing_slot_indexes.slice(0, 12)
          : [],
        requested_fact_covered_slot_count: Number(coverage?.covered_slot_count || 0),
        requested_fact_requested_slot_count: Number(coverage?.requested_slot_count || 0)
      }
    };
  }
  const qualitativeReplyValidation = requestedQualitativeReplyAssignment(
    reply,
    requestedQualitativeContract
  );
  if (qualitativeReplyValidation && !qualitativeReplyValidation.passed) {
    return {
      passed: false,
      reply: requestedFactCoverageFailureReply(replyLang),
      trace: {
        ...baseTrace,
        reason: qualitativeReplyValidation.reason,
        requested_fact_answer_missing_slot_indexes: qualitativeReplyValidation.missingSlotIndexes,
        requested_fact_qualitative_slot_bindings: qualitativeReplyValidation.assignments,
        requested_fact_qualitative_gate_checks: qualitativeReplyValidation.gateChecks,
        requested_fact_answer_unit_count: qualitativeReplyValidation.answerUnitCount
      }
    };
  }
  if (qualitativeReplyValidation) {
    baseTrace.requested_fact_qualitative_slot_bindings = qualitativeReplyValidation.assignments;
    baseTrace.requested_fact_qualitative_gate_checks = qualitativeReplyValidation.gateChecks;
    baseTrace.requested_fact_answer_unit_count = qualitativeReplyValidation.answerUnitCount;
  }
  const plannedMetricValidation = requestedMetricContract
    ? null
    : requestedMetricPlanClaimAssignment(reply, retrievalMeta, questionScopeValues);
  if (plannedMetricValidation && !plannedMetricValidation.passed) {
    return {
      passed: false,
      reply: failureReply(replyLang),
      trace: {
        ...baseTrace,
        reason: plannedMetricValidation.reason,
        requested_fact_metric_slot_bindings: plannedMetricValidation.assignments,
        requested_fact_metric_missing_slot_index: plannedMetricValidation.missingSlotIndex
      }
    };
  }
  if (plannedMetricValidation) {
    baseTrace.requested_fact_metric_slot_bindings = plannedMetricValidation.assignments;
  }
  ...
  const mixedStructuredContactValidation = !contactInventoryValidation &&
    ((retrievalMeta?.currentMunicipalityContactEvidenceRequested === true && contactAnswerIntentRequested(message)) ||
      institutionalGuidanceContainsExactContact(message, reply));
  ...
  if (requestedMetricContract) {
    const uniformValidation = requestedUniformMetricAssignment(message, reply, requestedMetricContract, identityEligibleSources);
    if (uniformValidation) {
      return {
        passed: uniformValidation.passed,
        reply: uniformValidation.passed ? String(reply).trim() : failureReply(replyLang),
        trace: { ...baseTrace, passed: uniformValidation.passed, reason: uniformValidation.reason,
          requested_metric_slot_bindings: uniformValidation.assignments,
          supporting_source_id: uniformValidation.source?.sourceId || null,
          category_relation_checked: true, category_relation_mode: "uniform_participant_groups" }
      };
    }
    const requestedMetricValidation = requestedMetricClaimAssignment(
      message,
      reply,
      requestedMetricContract
    );
    if (!requestedMetricValidation.passed) {
      return {
        passed: false,
        reply: failureReply(replyLang),
        trace: {
          ...baseTrace,
          reason: requestedMetricValidation.reason,
          requested_metric_slot_bindings: requestedMetricValidation.assignments,
          requested_metric_missing_slot_index: requestedMetricValidation.missingSlotIndex,
          requested_metric_relation_diagnostics: requestedMetricValidation.relationDiagnostics || [],
          unsupported_claim_values: requestedMetricValidation.unexpectedClaimValues
        }
      };
    }
    baseTrace.requested_metric_slot_bindings = requestedMetricValidation.assignments;
  }

  if (!claims.length && qualitativeReplyValidation?.passed === true) {
    return {
      passed: true,
      reply: String(reply || "").trim(),
      trace: {
        ...baseTrace,
        passed: true,
        reason: "requested_qualitative_slots_validated",
        supporting_source_id: identityEligibleSources[0]?.sourceId || null
      }
    };
  }
  ...
  if (!supportingSources.length) {
    ...
    const compositeKovEvidence = retrievalMeta?.queryPlan?.mode === "kov_service_and_local_rule" ||
      retrievalMeta?.queryPlan?.question_planner?.mode === "kov_service_and_local_rule";
    if (compositeKovEvidence && individuallySupported) {
      const compositeSupportingSources = identityEligibleSources.filter(source =>
        claims.some(claim => numericClaimSupportedBySource(reply, claim, source))
      );
      return {
        passed: true,
        reply,
        trace: {
          ...baseTrace,
          passed: true,
          reason: "composite_subgoal_claims_supported",
          supporting_source_ids: Array.from(new Set(
            compositeSupportingSources.map(source => source.attributionSourceId).filter(Boolean)
          )),
          composite_subgoals_checked: true
        }
      };
    }
  ...
  const uniformRelations = compatibleSources.map(source => ({
    source,
    relation: uniformParticipantRelation(source, reply, categoryLabels, message)
  }));
  ...
  if (uniformRelationChecked && !uniformCompatibleSources.length) {
    ...
    return {
      ...
      trace: {
        ...
        expected_group_cardinality: relation?.expectedGroupCardinality || null,
        source_group_cardinality: relation?.sourceGroupCardinality || null,
        source_group_cardinality_satisfied: relation?.sourceGroupCardinalitySatisfied !== false,
        observed_group_cardinality: relation?.observedGroupCardinality || null,
        observed_participant_category_count: relation?.observedParticipantCategoryCount || 0,
        group_cardinality_satisfied: relation?.groupCardinalitySatisfied !== false
      }
    };
  }
  const typedCategoryRelationsValidated = requestedMetricContract?.source === "final_rendered_evidence" &&
    documentIdentity?.required === true && documentIdentity.matched === true && documentIdentity.confidence === "high" &&
    requestedMetricContract.slots.every(slot => ["count", "proportion"].includes(slot.value_type)) &&
    baseTrace.requested_metric_slot_bindings?.length === requestedMetricContract.slots.length &&
    uniformCompatibleSources.some(source => source.attributionSourceId === requestedMetricContract.source_id);
  // The complete typed contract has already proved value, category, qualifier,
  // one-to-one assignment and absence of extra metrics in this same source.
  // A second lexical-only category parser must not reject its proven synonyms.
  const categoryRelationChecked = !typedCategoryRelationsValidated && asksForCategoricalNumericBreakdown(message) && (
