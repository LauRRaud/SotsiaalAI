...
const CALENDAR_MONTH_PATTERNS = Object.freeze([
  ...
  /\b(?:september\p{L}*|сентябр\p{L}*)\b/u,
  /\b(?:oktoober\p{L}*|october|октябр\p{L}*)\b/u,
  /\b(?:november\p{L}*|ноябр\p{L}*)\b/u,
  /\b(?:detsember\p{L}*|december|декабр\p{L}*)\b/u
]);
...
function calendarDateClaimSupportedByEquivalentWording(reply = "", claim = {}, source = {}) {
  ...
  const replySentence = normalizeText(paragraphAroundIndex(reply, claim.index || 0));
...
const CATEGORY_LABEL_STOP_WORDS = new Set([
  "a", "an", "and", "as", "at", "for", "from", "in", "of", "on", "or", "the", "to", "who", "with",
  "et", "iga", "ja", "kes", "kui", "mida", "mis", "ning", "oma", "voi",
  "в", "и", "из", "или", "к", "как", "которые", "на", "по", "с"
]);

const CATEGORY_LABEL_GENERIC_PREFIXES = [
  "group", "inime", "isik", "osalej", "participant", "person", "ruhm", "sihtruhm",
  "групп", "участник", "человек"
];
...
function extractNumericCategoryLabels(value = "") {
  ...
  for (const line of String(value || "").split(/[\r\n]+/u)) {
    const match = line.match(
      /^\s*(?:[-*•]\s*)?(?:\d+(?:[.,]\d+)?\s*%?|null|uks|uht|uhe|kaks|kaht|kahe|kolm|kolme|neli|nelja|viis|viit|viie|kuus|kuut|kuue|seitse|seitset|seitsme|kaheksa|uheksa|kumme|kummet|one|two|three|four|five|six|seven|eight|nine|ten|один|одна|два|две|три|четыре|пять|шесть|семь|восемь|девять|десять)(?:\s*[:=–—-]\s*|\s+)(.+?)\s*[;,.]?\s*$/u
    );
    if (!match) continue;
    const label = String(match[1] || "").trim();
    const tokens = significantCategoryLabelTokens(label);
    if (!tokens.length) continue;
    labels.push({ label, tokens });
  }
  return labels;
}
...
export function validateExactFactAnswer({
  ...
  const categoryRelationChecked = asksForCategoricalNumericBreakdown(message) && categoryLabels.length >= 2;
  const categoryCompatibleSources = categoryRelationChecked
    ? compatibleSources.filter(source => !unsupportedNumericCategoryLabels(message, reply, source).length)
    : compatibleSources;
