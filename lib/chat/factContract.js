...
function asksForWholeScope(message = "") {
  ...
  const explicitTotalCue = /\b(?:kokku|koguarv\w*|uldarv\w*)\b/u.test(normalized);
  if (asksForCategoricalNumericBreakdown(message) && !explicitTotalCue) return false;
  return /\b(?:arv|arvu|arvud|arvust|arvuga|kokku|kui\s+palju|mitu|koguarv\w*|uldarv\w*)\b/u.test(normalized) &&
