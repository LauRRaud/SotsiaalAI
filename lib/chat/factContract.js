...
function requestedQualitativeUnitBinding(slot = {}, unit = null) {
  ...
  if (matchedRelationTerms.length < minimumRelationMatches && !performedTime) return null;
  ...
  if (!requiredNumericValues.every(value => unitNumericValues.has(value))) return null;
  ...
  if (matchedEvidenceAnchors.length < minimumAnchorMatches) return null;
  const valueType = String(slot?.value_type || "");
  const actionObjectAssignments = valueType === "recommendation"
    ? requestedQualitativeActionObjectAssignment(slot, unit.text)
    : [];
  if (valueType === "recommendation" && !actionObjectAssignments) return null;
  ...
  if (substantiveTokens.length + requiredNumericValues.length < minimumAnswerItems) return null;
  return {
...
function requestedQualitativeReplyAssignment(reply = "", contract = null) {
  ...
  const units = recommendationsOnly
  ...
  const slotEntries = contract.slots.map((slot, contractIndex) => ({
    ...
    slot,
    options: units
      .map(unit => requestedQualitativeUnitBinding(slot, unit))
      .filter(Boolean)
...
function requestedMetricClauseBoundaries(reply = "", slots = []) {
  ...
  for (const match of separators) {
    ...
    const previousHardBoundaries = ["\n", ";", ".", "!", "?"]
      .map(separator => text.lastIndexOf(separator, Math.max(0, commaIndex - 1)))
      .filter(index => index >= 0);
    const nextHardBoundaries = ["\n", ";", ".", "!", "?"]
      .map(separator => text.indexOf(separator, commaIndex + 1))
      .filter(index => index >= 0);
...
function requestedMetricClaimLocalScope(reply = "", claim = null, slots = []) {
  ...
  const previousBoundaries = ["\n", ";", ".", "!", "?"]
    ...
    .filter(index => index >= 0);
  const nextBoundaries = ["\n", ";", ".", "!", "?"]
    .map(separator => text.indexOf(separator, claimIndex + String(claim?.raw || claim?.value || "").length))
    .filter(index => index >= 0);
