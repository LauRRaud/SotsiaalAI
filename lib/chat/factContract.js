...
function requestedMetricSegmentMatchesRelation(segment = "", slot = {}) {
  ...
  return matchedCount >= Math.min(2, requiredTerms.length);
}

function requestedMetricClauseCommaBoundaries(reply = "", slots = []) {
  const text = String(reply || "");
  const claims = normalizedNumericClaims(text);
  const boundaries = [];
  for (const match of text.matchAll(/,/gu)) {
    ...
    const rightClaims = normalizedNumericClaims(rightSegment);
...
function requestedMetricRelationBoundToClaim(reply = "", claim = null, slot = {}, slots = []) {
  ...
  const minimumTermsMatched = matchedTerms.length >= Math.min(2, requiredTerms.length);
...
function requestedMetricClaimAssignment(message = "", reply = "", contract = null) {
  ...
  const contractScopeValues = new Set((Array.isArray(contract?.question_scope_values)
    ...
    .map(value => normalizeNumber(value))
    .filter(Boolean));
  const legacyUserScopeValues = new Set(
    normalizedNumericClaims(message)
  ...
  const assignedClaimIndexes = new Set();
  const assignments = [];
  let minimumClaimIndex = -1;
  for (const slot of slots) {
    const expectedValue = normalizeNumber(slot?.evidence_value);
    ...
    if (claimIndex < 0) {
      return {
        ...
        assignments,
        missingSlotIndex: Number(slot?.slot_index) || assignments.length + 1,
        unexpectedClaimValues: []
      };
    }
    const relationBinding = requestedMetricRelationBoundToClaim(reply, claims[claimIndex], slot, slots);
    if (!relationBinding.passed) {
      return {
        passed: false,
        reason: "requested_metric_relation_mismatch",
        assignments,
        missingSlotIndex: Number(slot?.slot_index) || assignments.length + 1,
        unexpectedClaimValues: []
      };
    ...
    if (!scopeBinding.passed) {
      return {
        ...
        assignments,
        missingSlotIndex: Number(slot?.slot_index) || assignments.length + 1,
        unexpectedClaimValues: scopeBinding.boundValues
      };
    }
    assignedClaimIndexes.add(claimIndex);
    minimumClaimIndex = claimIndex;
    assignments.push({
      slot_index: Number(slot?.slot_index) || assignments.length + 1,
      value_type: String(slot?.value_type || ""),
      evidence_value: expectedValue,
      claim_index: claimIndex,
      scope_values: requiredScopeValues.map(value => normalizeNumber(value)).filter(Boolean),
      bound_scope_values: scopeBinding.boundValues,
      matched_relation_terms: relationBinding.matchedTerms,
      matched_unique_relation_term_count: relationBinding.matchedUniqueTermCount
    });
  }
  const unexpectedClaimValues = claims
    .filter((claim, index) =>
      !claim.year &&
...
function requestedFactPlanIncomplete(retrievalMeta = null) {
  ...
  const metricValueTypes = new Set(["count", "amount", "duration", "magnitude", "proportion", "calendar_year"]);
...
export function validateExactFactAnswer({
  ...
  const plannedMetricValidation = requestedMetricPlanClaimAssignment(
    reply,
    retrievalMeta,
    questionScopeValues
  );
