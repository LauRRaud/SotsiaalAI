...
import { sourceSelectionClaimMatches } from "./sourceSelectionStore.js";
...
export async function claimChatTurn({
  ...
  return db.$transaction(async tx => {
    ...
    if (existing) {
      ...
      if ((recordRagAttempt && !await sourceSelectionClaimMatches(tx, { conversationId, userId, userMessage,
        sourceSelectionBinding, expectedPreviousAssistantMessageId, existingTurn: existing, now })) || !await recoverySnapshotMatches(tx, {
        conversationId,
        userId,
        expectedPreviousAssistantMessageId
      })) {
      ...
      const ragAttempt = recordRagAttempt ? await createRagAttempt(tx, turn, now, leaseMs, sourceSelectionBinding) : null;
      return { outcome: CHAT_TURN_OUTCOME.CLAIMED, turn, ragAttempt };
    }

    if ((sourceSelectionBinding && !await sourceSelectionClaimMatches(tx, { conversationId, userId, userMessage,
      sourceSelectionBinding, expectedPreviousAssistantMessageId, now })) || !await recoverySnapshotMatches(tx, {
      conversationId,
      userId,
      expectedPreviousAssistantMessageId
    })) {
...
export async function initializeClaimedChatTurn(fence, { role, userMessage, expectedPreviousAssistantMessageId = null }, deps = {}) {
  const db = deps.prisma || prisma;
  return db.$transaction(async tx => {
    ...
    if (attempt.evidence?.source_selection_binding && (!expectedPreviousAssistantMessageId || !await recoverySnapshotMatches(tx, {
      conversationId: fence.conversationId, userId: fence.userId, expectedPreviousAssistantMessageId
    }))) throw staleAttemptError();
