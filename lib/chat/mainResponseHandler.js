...
import { handleSourceSelectionResponse } from "./sourceSelectionResponse.js";
import { projectDiagnosticHistory, projectIdentityDecision, projectQualitativeGateChecks, projectAuthorTopicEvidence } from "@/lib/chat/ragDiagnostics";
...
import {
  ...
  projectGroupEvidenceLocators,
  projectDirectedRelationEvidenceLocators,
  GROUP_CONTRACT_REASONS,
  DIRECTED_RELATION_CONTRACT_REASONS
...
import { classifyModelFailure } from "./modelFailure.js";
  ...
  return details.map((entry = {}) => {
    const sanitized = {
      ...
      original_body_count: typeof entry.original_body_count === "number"
        ...
        : undefined,
      rendered_evidence_body_chars: typeof entry.rendered_evidence_body_chars === "number"
        ? entry.rendered_evidence_body_chars
        : undefined,
      body_span_count: typeof entry.body_span_count === "number"
        ? entry.body_span_count
        : undefined,
      body_span_origin_bound_count: typeof entry.body_span_origin_bound_count === "number"
        ? entry.body_span_origin_bound_count
        : undefined,
      body_span_provenance_bound_count: typeof entry.body_span_provenance_bound_count === "number"
        ? entry.body_span_provenance_bound_count
        : undefined,
      rendered_body_span_covered_chars: typeof entry.rendered_body_span_covered_chars === "number"
        ? entry.rendered_body_span_covered_chars
        : undefined,
      rendered_body_external_gap_chars: typeof entry.rendered_body_external_gap_chars === "number"
        ? entry.rendered_body_external_gap_chars
        : undefined,
      rendered_body_synthetic_marker_chars: typeof entry.rendered_body_synthetic_marker_chars === "number"
        ? entry.rendered_body_synthetic_marker_chars
        : undefined,
      rendered_body_uncovered_chars: typeof entry.rendered_body_uncovered_chars === "number"
        ? entry.rendered_body_uncovered_chars
        : undefined,
      rendered_body_spans: Array.isArray(entry.rendered_body_spans)
        ? entry.rendered_body_spans.slice(0, 8).map(span => Object.fromEntries(Object.entries({
            ...
            rendered_start_offset: Number.isSafeInteger(span?.rendered_start_offset)
              ? span.rendered_start_offset
              : undefined,
            rendered_end_offset: Number.isSafeInteger(span?.rendered_end_offset)
              ? span.rendered_end_offset
              : undefined,
            literal_original_start: Number.isSafeInteger(span?.literal_original_start)
              ? span.literal_original_start
              : null,
            synthetic_marker_chars: Number.isSafeInteger(span?.synthetic_marker_chars)
              ? span.synthetic_marker_chars
              : undefined,
            provenance_count: Array.isArray(span?.provenance) ? span.provenance.length : 0,
...
function sanitizeRequestedQualitativeContractTrace(value = null) {
  ...
  const slots = inputSlots.map(slot => ({
    ...
    payload_kind: ["known_value_interpretation", "referenced_study_period", "group_distribution", "group_membership", "directed_event_relation_set"].includes(slot?.payload_kind) ? slot.payload_kind : null,
  ...
  return {
    ...
    requested_slot_count: requestedSlotCount,
    mapped_slot_count: mappedSlotCount,
    unbound_candidate_count: boundedInteger(value.unbound_candidate_count, 0, 100),
    ignored_duplicate_candidate_count: boundedInteger(value.ignored_duplicate_candidate_count, 0, 100),
    conflicting_unbound_candidate_count: boundedInteger(value.conflicting_unbound_candidate_count, 0, 100),
    sanitizer_dropped_slot: sanitizerDroppedSlot,
...
function sanitizeCurrentTurnAuthorConfirmation(value) {
  ...
  return {
    ...
    topic_evidence: projectAuthorTopicEvidence(value.topic_evidence),
...
function sanitizeFactValidationTrace(value = null) {
  ...
  return Object.fromEntries(Object.entries({
    ...
    directed_relation_evidence_locators: projectDirectedRelationEvidenceLocators(value.directed_relation_evidence_locators),
...
function sanitizeDocumentIdentityTrace(value = null) {
  ...
  return Object.fromEntries(Object.entries({
    ...
    author_topic_evidence: projectAuthorTopicEvidence(value.authorTopicEvidence),
    ...
    candidates: Array.isArray(value.candidates)
      ? value.candidates.slice(0, 5).map((candidate = {}) => ({
          ...
          author_body_topic_confirmed: candidate.authorBodyTopicConfirmed === true,
...
export async function handleMainChatResponse(input, deps = {}) {
  ...
  try {
    const response = input.sourceSelectionTurn
      ? await handleSourceSelectionResponse(input, deps, handleMainChatResponseImpl)
      : await handleMainChatResponseImpl(input, deps);
...
async function handleMainChatResponseImpl({
  req,
  sourceSelectionPartition = false,
  wantStream,
  persist,
  convId,
  userId,
  ...
  const callProvider = async input => {
    ...
    try { return await (deps.callOpenAI || callOpenAI)({ ...input, onRuntimeObservation: observation => observeModel(observation, index) }); }
      ...
      await ragAttemptController?.stage("model", { failure: { stage: "model", code: classifyModelFailure(error, {
        ...input,
        reasoningEffort: input.reasoningEffort || retrievalMeta?.diagnosticRuntime?.reasoning_effort
      }) } });
      throw error;
    }
  };
  ...
  const deterministicAuthorReply = typeof retrievalMeta?.deterministicAuthorReply === "string"
    ...
    : "";
  const directedRelationValidation = !isCrisis && !wantsDocumentDownload
    ? validateDirectedRelationReply({ retrievalMeta, sources, replyLang }) : null;
  const groupValidation = !directedRelationValidation && !isCrisis && !wantsDocumentDownload
    ? validateGroupFactReply({ retrievalMeta, sources, replyLang }) : null;
  const deterministicFactValidation = directedRelationValidation || groupValidation;
  const deterministicTypedFactTurn = hasValidatedPublication(
    deterministicFactValidation?.trace,
    deterministicFactValidation?.reply
  );
  const deterministicReply = deterministicTypedFactTurn
    ? deterministicFactValidation.reply
    : deterministicAuthorReply || deterministicContactReply;
  const deterministicAuthorTurn = retrievalMeta?.structuredAuthorCorpusTurn === true && !!deterministicAuthorReply;
  if (
    !isCrisis &&
    (
      deterministicTypedFactTurn || deterministicAuthorTurn ||
    ...
    const factValidation = deterministicTypedFactTurn ? deterministicFactValidation : deterministicAuthorTurn
      ? null
      : validateExactFactAnswer({
          message: evidenceMessage,
    ...
    const validationRecovery = deterministicAuthorTurn && !deterministicTypedFactTurn
    ...
    if (!sourceSelectionPartition) await persistSourcePackagesFromTrace(ragTrace, logError);
    ...
    if (!settled) {
      if (typeof logError === "function") {
        logError("chat.persist.not_durable", {
          branch: deterministicTypedFactTurn
            ? directedRelationValidation ? "deterministic_directed_relation" : "deterministic_group_fact"
            : deterministicAuthorTurn ? "deterministic_author_corpus" : "deterministic_contact",
          convId
        });
      }
      return makeError("chat.error.not_saved", 503);
  ...
  if (
    ...
    const noContextAssistanceMode = inferNoContextAssistanceMode({
      userMessage: evidenceMessage,
      retrievalMeta,
      recoveryContinuation: !!expectedRecoveryAssistantMessageId
    });
    ...
    const noContextRecovery = plannedSocialScopeBoundary || plannedMunicipalityClarification || buildNoContextRecovery({
      ...
      socialAcknowledgementReply,
      userMessage: evidenceMessage,
      assistanceMode: noContextAssistanceMode,
      effectiveRole: normalizedRole,
      replyLang,
      ragSearchFailed,
      isCrisis,
      target: inferNoContextRecoveryTarget(retrievalMeta, evidenceMessage),
    ...
    async start(controller) {
      ...
      try {
        ...
        if (wasAborted) {
          ...
          await ragAttemptController?.stage("model", { failure: { stage: "model", code: classifyModelFailure(err, {
            signal: req.signal,
            reasoningEffort: retrievalMeta?.diagnosticRuntime?.reasoning_effort
          }) } });
