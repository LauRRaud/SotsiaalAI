...
import { projectDiagnosticHistory, projectIdentityDecision } from "@/lib/chat/ragDiagnostics";
    ...
    .map(slot => {
      ...
      const unit = slot?.unit === "percent"
        ...
        : typeof slot?.unit === "string" && slot.unit.trim()
          ...
          : null;
      const qualifier = new Set(["over", "under", "at_least", "at_most", "about"]).has(
        slot?.qualifier
      ) ? slot.qualifier : null;
      return {
        ...
        evidence_value_valid: evidenceValue !== null,
        unit,
        qualifier,
...
export async function handleMainChatResponse({
  ...
  retrievalMeta = {
    ...
    diagnosticRuntime: {
      ...
      release_sha: process.env.RAG_BUILD_SHA || process.env.RELEASE_SHA || null,
  ...
  if (
    ...
    const factValidation = deterministicAuthorTurn
      ? null
      : validateExactFactAnswer({
          message: evidenceMessage,
    ...
    const validationRecovery = deterministicAuthorTurn
    ...
    if (!settled) {
      if (typeof logError === "function") {
        logError("chat.persist.not_durable", {
          branch: deterministicAuthorTurn ? "deterministic_author_corpus" : "deterministic_contact",
          convId
        });
      }
      return makeError("chat.error.not_saved", 503);
