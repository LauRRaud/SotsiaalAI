...
export async function handleMainChatResponse({
  ...
  if (!wantStream) {
    ...
    try {
      ...
      const firstModelCallDurationMs = Math.max(0, Date.now() - modelCallStartedAt);
      const providerReply = resolveProviderReply(aiResult?.reply, { replyLang, isCrisis });
      const factValidationStartedAt = exactFactValidationEnabled ? Date.now() : null;
      ...
      const factValidationDurationMs = factValidationStartedAt === null
        ? null
        : Math.max(0, Date.now() - factValidationStartedAt);
  ...
  const factBufferStartedAt = exactFactValidationEnabled ? Date.now() : null;
    ...
    async start(controller) {
      ...
      const finalizeStreamReply = async () => {
        ...
        if (exactFactValidationEnabled) {
          const factValidationStartedAt = Date.now();
          ...
          factValidationDurationMs = Math.max(0, Date.now() - factValidationStartedAt);
          accumulated = factValidation.reply;
          pendingDelta = accumulated;
          factValidation.trace.buffered_response_ms = Math.max(0, Date.now() - factBufferStartedAt);
      ...
      try {
        ...
        firstModelCallDurationMs = Math.max(0, Date.now() - modelCallStartedAt);
