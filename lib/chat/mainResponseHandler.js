  ...
  return Object.fromEntries(Object.entries({
    ...
    temporal_claim_bindings: Array.isArray(value.temporal_claim_bindings)
      ? value.temporal_claim_bindings.slice(0, 24).map(binding => Object.fromEntries(Object.entries({
          ...
          value: truncateTraceString(binding?.value, 40),
...
export async function handleMainChatResponse({
  ...
  if (
    ...
    const validationRecovery = resolveValidationRecovery({
      providerReply: deterministicContactReply,
      fallbackReply: factValidation?.reply || deterministicContactReply,
  ...
  if (!effectiveContext || !effectiveContext.trim()) {
    ...
    const socialAcknowledgementReply = buildSocialAcknowledgementReply(
      effectiveMessage,
      replyLang
    );
    ...
    if (!isCrisis && !ragSearchFailed && !socialAcknowledgementReply) {
    ...
    const noContextRecovery = buildNoContextRecovery({
    ...
    const attribution = buildSourceAttribution(reply, sources, {
      ...
      municipalityContext: retrievalMeta?.municipalityContext,
      documentIdentityEvidence: retrievalMeta?.documentIdentityEvidence,
      factValidation: suppressRecoverySources
        ? { passed: false, reason: recoveryAttributionReason }
        : null
    });
  ...
  const answerSystemInstructions = exactFactValidationEnabled
  ...
  if (!wantStream) {
    ...
    try {
      ...
      const domainBoundary = !isCrisis
        ...
        : null;
      const factValidationStartedAt = exactFactValidationEnabled && !domainBoundary
        ? performance.now()
        : null;
        const factValidation = exactFactValidationEnabled && !domainBoundary
          ? validateExactFactAnswer({
            message: evidenceMessage,
            reply: providerReply,
            sources,
      ...
      const validationRecovery = domainBoundary || resolveValidationRecovery({
          providerReply,
      ...
      const reply = validationRecovery.reply;
      const responseFactValidationTrace = domainBoundary
        ? { passed: false, reason: "outside_social_scope" }
        : factValidation?.trace;
      const validatedRetrievalMeta = withFactValidation(exactFactRetrievalMeta, factValidation?.trace, {
      ...
      const replySources = resolveDisplayedSources(sources, attribution, responseFactValidationTrace);
      const ragTrace = resolveRagTrace(sources, attribution, responseRetrievalMeta);
      const ragContract = buildRagContractMetadata();
      const attributionDecisions = resolveAttributionDecisions(attribution);
      await emitRagTraceEvent(logEvent, {
  ...
  const sse = new ReadableStream({
    async start(controller) {
      ...
      const finalizeStreamReply = async () => {
        ...
        } else if (exactFactValidationEnabled) {
