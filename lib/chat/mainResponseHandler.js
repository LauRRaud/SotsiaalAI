...
function sanitizeRequestedMetricContractTrace(value = null) {
    ...
    .map(slot => {
      ...
      return {
        ...
        input_form: slot?.input_form === "original" || slot?.input_form === "canonical_fallback"
          ? slot.input_form
          : null,
        fragment_index: boundedInteger(slot?.fragment_index, 0, 79),
        mention_index: boundedInteger(slot?.mention_index, 0, 79),
        ...
        shared_subject_head_matched: slot?.shared_subject_head_matched === true
      };
    })
  ...
  const mappingMethod = value.mapping_method === "bounded_lexical_relation_alignment_v1"
  ...
  const mappingDiagnostics = rawDiagnostics && typeof rawDiagnostics === "object" && !Array.isArray(rawDiagnostics)
    ? {
        evidence_candidate_count: boundedInteger(rawDiagnostics.evidence_candidate_count, 0, 512),
            ...
            top_candidates: (Array.isArray(slot?.top_candidates) ? slot.top_candidates : [])
              .map(candidate => ({
                ...
                score: typeof candidate?.score === "number" && Number.isFinite(candidate.score) &&
                  ...
                  ? candidate.score
                  : null,
                fragment_index: boundedInteger(candidate?.fragment_index, 0, 79),
                mention_index: boundedInteger(candidate?.mention_index, 0, 79),
                ...
                shared_subject_head_matched: candidate?.shared_subject_head_matched === true
              }))
