...
export async function handleMainChatResponse({
  ...
  const factRepairInstruction = replyLang === "en"
    ? "FACT_VALIDATION_REPAIR: Rewrite the answer using only numeric claims directly supported by RAG_CONTEXT. Do not add any other phone numbers, dates, counts, percentages, durations, or derived values. Answer the requested facts concisely; if a requested fact is absent, say so without inventing a number."
    : replyLang === "ru"
      ? "FACT_VALIDATION_REPAIR: Перепиши ответ, используя только числовые утверждения, прямо подтверждённые RAG_CONTEXT. Не добавляй другие телефоны, даты, количества, проценты, длительности или вычисленные значения. Кратко ответь на запрошенные факты; если факта нет, скажи об этом без выдуманного числа."
      : "FACT_VALIDATION_REPAIR: Sõnasta vastus ümber, kasutades ainult RAG_CONTEXT-is otseselt tõendatud arvulisi väiteid. Ära lisa muid telefoninumbreid, aastaid, arve, protsente, kestusi ega tuletatud väärtusi. Vasta küsitud faktidele lühidalt; puuduva fakti korral ütle seda uut arvu leiutamata.";
