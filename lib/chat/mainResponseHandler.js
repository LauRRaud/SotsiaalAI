...
export function buildRagTraceFromAttribution(sources = [], attribution, retrievalMeta = null) {
  ...
  return {
    ...
    service_anchors: Array.isArray(retrievalMeta?.packageAwareAnswering?.serviceAnchors)
      ? retrievalMeta.packageAwareAnswering.serviceAnchors
      : [],
...
async function emitRagTraceEvent(logEvent, {
  ...
  await logEvent("rag_trace", {
    ...
    service_anchors: ragTrace.service_anchors,
...
export async function handleMainChatResponse({
  ...
  if (!wantStream) {
    ...
    try {
      ...
      if (!usageSettled && persist && convId && userId) {
        const marker = await persistDone({
          convId,
          userId,
  ...
  const sse = new ReadableStream({
    async start(controller) {
      ...
      // Aus Stop (T03 E2): serveripoolne abort salvestab AINULT juba kuvatud osalise teksti
      // tähisega ABORTED (või ausa tühja katkestuse), vabastab kasutamata reservatsiooni ega
      // luba taustal valmivat täisvastust püsistada. Ei emiteeri `done`-sündmust.
      const finalizeStreamAbort = async () => {
        ...
        // SOL-CHAT-02: ABORTED marker ja vabastus ühes tehingus; kui marker ei jõua kettale,
        // vabastatakse eraldi sammuna, aga mitte kaks korda.
        let releasedWithMarker = false;
        if (persist && convId && userId) {
          const marker = await persistDone({
            convId,
            ...
            replyLang,
            settleUsage: typeof onUsageRelease === "function"
              ? tx => onUsageRelease("chat_stream_aborted", tx)
              : null
          });
          releasedWithMarker = !!marker;
        }
        if (!releasedWithMarker) {
          await settleChatUsage(onUsageRelease, logError, "usage.chat_release.error", "chat_stream_aborted");
        }
        if (typeof logEvent === "function") {
