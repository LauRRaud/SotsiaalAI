...
import { shouldValidateExactFactAnswer, validateExactFactAnswer } from "@/lib/chat/factContract";
...
function sanitizeFactValidationTrace(value = null) {
  ...
  return Object.fromEntries(Object.entries({
    ...
    buffered_response_ms: Number.isFinite(Number(value.buffered_response_ms))
      ...
      : undefined,
    repair_attempted: value.repair_attempted === true,
    repair_passed: value.repair_passed === true,
    repair_model_ms: Number.isFinite(Number(value.repair_model_ms))
      ? Number(value.repair_model_ms)
      : undefined,
    initial_reason: typeof value.initial_reason === "string" ? value.initial_reason : undefined
...
export async function handleMainChatResponse({
  ...
  const repairableFactValidationReasons = new Set([
    "unsupported_numeric_claim",
    "cross_source_numeric_mix"
  ]);
  const factRepairInstruction = replyLang === "en"
    ? "FACT_VALIDATION_REPAIR: Rewrite the answer so every numeric claim is supported together by one rendered source in RAG_CONTEXT. Do not add any other phone numbers, dates, counts, percentages, durations, or derived values. Answer the requested facts concisely; if a requested fact is absent, say so without inventing a number."
    : replyLang === "ru"
      ? "FACT_VALIDATION_REPAIR: Перепиши ответ так, чтобы все числовые утверждения вместе подтверждались одним отображённым источником из RAG_CONTEXT. Не добавляй другие телефоны, даты, количества, проценты, длительности или вычисленные значения. Кратко ответь на запрошенные факты; если факта нет, скажи об этом без выдуманного числа."
      : "FACT_VALIDATION_REPAIR: Sõnasta vastus ümber nii, et kõiki arvulisi väiteid toetaks koos üks RAG_CONTEXT-is renderdatud allikas. Ära lisa muid telefoninumbreid, aastaid, arve, protsente, kestusi ega tuletatud väärtusi. Vasta küsitud faktidele lühidalt; puuduva fakti korral ütle seda uut arvu leiutamata.";
  const attemptExactFactRepair = async ({ reply, validation }) => {
    if (
      !exactFactValidationEnabled ||
      validation?.trace?.passed === true ||
      !repairableFactValidationReasons.has(validation?.trace?.reason)
    ) {
      return { reply, validation, repairModelMs: 0 };
    }
    const unsupportedValues = Array.isArray(validation?.trace?.unsupported_claim_values)
      ? validation.trace.unsupported_claim_values.map(value => String(value || "").trim()).filter(Boolean).slice(0, 20)
      : [];
    const unsupportedValueInstruction = unsupportedValues.length
      ? `UNSUPPORTED_NUMERIC_CLAIMS_FROM_INITIAL_DRAFT: ${unsupportedValues.join(", ")}. Remove these values unless the same rendered source supports them together with every other numeric claim.`
      : "";
    const initialDraftInstruction = `INITIAL_DRAFT_TO_REPAIR:\n${String(reply || "").trim().slice(0, 5000)}`;
    ...
    const repairResult = await callProvider({
      ...
      extraSystemInstructions: [
        ...
        factRepairInstruction,
        unsupportedValueInstruction,
        initialDraftInstruction
      ],
      userId,
      role: normalizedRole,
      signal: req.signal
    });
    const repairedReply = resolveProviderReply(repairResult?.reply, { replyLang, isCrisis });
    const repairedValidation = validateExactFactAnswer({
      message: effectiveMessage,
      reply: repairedReply,
      sources,
      retrievalMeta,
    ...
    const repairModelMs = Math.max(0, Math.round(performance.now() - repairStartedAt));
    repairedValidation.trace = {
      ...repairedValidation.trace,
      repair_attempted: true,
      repair_passed: repairedValidation?.trace?.passed === true,
      repair_model_ms: repairModelMs,
      initial_reason: validation?.trace?.reason || null
    };
    return {
      reply: repairedValidation.reply,
  ...
  if (!wantStream) {
    ...
    try {
      ...
      let factValidation = exactFactValidationEnabled
        ...
        : null;
      const repairedFact = await attemptExactFactRepair({
        reply: providerReply,
        validation: factValidation
      });
      factValidation = repairedFact.validation;
      const factValidationDurationMs = factValidationStartedAt === null
        ? null
        : Math.max(0, Math.round(performance.now() - factValidationStartedAt) - repairedFact.repairModelMs);
      if (factValidation?.trace) factValidation.trace.validation_duration_ms = factValidationDurationMs;
      const reply = factValidation?.reply || repairedFact.reply || providerReply;
      const responseRetrievalMeta = withFactValidation(retrievalMeta, factValidation?.trace, {
        first_model_call_ms: firstModelCallDurationMs,
        model_ms: firstModelCallDurationMs + repairedFact.repairModelMs,
    ...
    async start(controller) {
      ...
      const finalizeStreamReply = async () => {
        ...
        if (exactFactValidationEnabled) {
          ...
          factValidation = repairedFact.validation;
          factValidationDurationMs = Math.max(
            0,
            Math.round(performance.now() - factValidationStartedAt) - repairedFact.repairModelMs
          );
          accumulated = factValidation?.reply || repairedFact.reply || accumulated;
          pendingDelta = accumulated;
          ...
          if (repairedFact.repairModelMs > 0) {
            modelDurationMs = (modelDurationMs || 0) + repairedFact.repairModelMs;
          }
          mayEmitDelta = true;
        }
        flushPendingDelta();
