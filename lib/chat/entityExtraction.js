...
const NON_ENTITY_NUMERIC_RE = /^(?:\d+(?:[.,]\d+)?%?|(?:19|20)\d{2}|\d+\s*[-–]\s*\d+|§\s*\d+[a-z]?)$/iu;
...
function morphologySpans(morphology = null) {
  return (Array.isArray(morphology?.proper_name_spans) ? morphology.proper_name_spans : [])
    .map(span => ({
      ...
      // Morphology contributes candidates, not identity. Preserve the exact
      // surface span until the registry resolves a canonical person.
      canonical: cleanEntityCandidate(typeof span === "string" ? span : span?.text),
...
export function extractSemanticEntities({ message = "", morphology = null } = {}) {
  ...
  for (const candidate of candidates) {
    ...
    if (
      !canonical ||
      seen.has(normalized) ||
      GENERIC_SINGLE_ENTITY_RE.test(normalized) ||
      NON_ENTITY_NUMERIC_RE.test(canonical)
    ) continue;
    seen.add(normalized);
