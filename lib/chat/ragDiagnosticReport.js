...
import { attemptDiagnostics } from "./ragAttemptEvidence.js";
...
export function buildDiagnosticReport({ conversationId, turns = [], messages = [], attempts = [], hasMore = false, now = new Date() }) {
  const byId = new Map(messages.map(message => [message.id, message]));
  const paired = new Set();
  const rows = attempts.map(attempt => {
    const question = byId.get(attempt.userMessageId);
    const answer = byId.get(attempt.assistantMessageId);
    ...
    const expired = attempt.status === "RUNNING" && new Date(attempt.leaseExpiresAt).getTime() <= new Date(now).getTime();
    const completionStatus = expired || ["ABANDONED", "FAILED"].includes(attempt.status) ? "ERROR"
      : attempt.status === "CANCELLED" ? "ABORTED" : attempt.status;
    return {
      reference: validAnswer ? `message:${answer.id}` : `attempt:${attempt.id}`,
      turn_id: attempt.chatTurnId,
      question_message_id: validQuestion ? question.id : null,
      assistant_message_id: validAnswer ? answer.id : null,
      pairing: validAnswer && validQuestion ? "PERSISTED_ATTEMPT" : "INCOMPLETE_PERSISTED_ATTEMPT",
      question: validQuestion ? messageText(question.content) : null,
      answer: validAnswer ? messageText(answer.content) : null,
      started_at: time(attempt.startedAt), ended_at: time(attempt.endedAt), attempt: attempt.attempt,
      recorded_status: attempt.status, effective_status: completionStatus,
      status_derivation: expired ? "lease_expired" : "recorded_status",
      diagnostics: attemptDiagnostics(expired ? { ...attempt, status: "ABANDONED" } : attempt, completionStatus)
    };
  });
  const recorded = new Set(attempts.map(attempt => `${attempt.chatTurnId}:${attempt.attempt}`));
  rows.push(...turns.filter(turn => !recorded.has(`${turn.id}:${turn.attempt}`)).map(turn => {
  ...
  return {
    ...
    source: "RagAttempt + ChatTurn + ConversationMessage; no time-based pairing",
