...
const ESTONIAN_WORD_HINT_RE = /\b(aga|aitäh|aitah|eakas|eakate|ei|ja|kas|kuidas|kus|mida|miks|millal|milline|millised|millistest|millistes|mis|olen|oled|olete|palun|seadus|soovin|tahan|teenus|tere|vajan|vald|või|voi)\b/iu;
...
function controlledTopicTermsEt(text = "") {
  ...
  if (/guardianship|опек/u.test(normalized)) terms.add("eestkoste");
  if (/caregiver|caregiving|ухаживающ|уход.*близк/u.test(normalized)) terms.add("hoolduskoormus");
  if (/social\s+transport|социальн.*транспорт/u.test(normalized)) terms.add("sotsiaaltransport");
  if (/home\s+service|домашн.*социальн.*услуг/u.test(normalized)) terms.add("koduteenus");
  if (/support\s+person|сопровождающ.*услуг|услуг.*сопровожд/u.test(normalized)) terms.add("tugiisikuteenus");
  if (/child\s+protection|защит.*дет/u.test(normalized)) terms.add("lastekaitse");
  if (/mental\s+health|психическ.*здоров/u.test(normalized)) terms.add("vaimne tervis");
  if (/social\s+(?:service|benefit)|социальн.*(?:услуг|пособ)/u.test(normalized)) terms.add("sotsiaalteenus");
  if (/\b(?:service|benefit|support|услуг|пособ|поддержк)\p{Letter}*\b/u.test(normalized)) terms.add("teenus toetus");
  if (/\b(?:law|act|statute|legal|закон|правов|юридическ)\p{Letter}*\b/u.test(normalized)) terms.add("seadus õiguslik alus");
  if (/\b(?:regulation|ordinance|procedure|постановлен|порядок|регламент)\p{Letter}*\b/u.test(normalized)) terms.add("määrus kord");
  if (/§|\b(?:paragraph|section|параграф|статья)\s*\d*/u.test(normalized)) terms.add("paragrahv");
  if (/\b(?:municipality|municipal|local government|город|муниципал|самоуправлен)\p{Letter}*\b/u.test(normalized)) terms.add("kohalik omavalitsus");
  if (/\b(?:eligibility|condition|requirement|услови|критери)\p{Letter}*\b/u.test(normalized)) terms.add("tingimused");
  if (/\b(?:apply|application|заявлен|ходатайств|обращен)\p{Letter}*\b/u.test(normalized)) terms.add("taotlemine");
  if (/\b(?:form|blank|форма|бланк)\p{Letter}*\b/u.test(normalized)) terms.add("taotlusvorm");
  if (/\b(?:fee|price|cost|amount|плат|стоимост|сумм)\p{Letter}*\b/u.test(normalized)) terms.add("tasu summa");
  if (/\b(?:deadline|duration|processing time|срок|длительност)\p{Letter}*\b/u.test(normalized)) terms.add("tähtaeg kestus");
  if (/\b(?:contact|phone|email|контакт|телефон|почт)\p{Letter}*\b/u.test(normalized)) terms.add("kontakt");
  if (/\b(?:guide|guidance|recommendation|manual|руководств|рекомендац|инструкц)\p{Letter}*\b/u.test(normalized)) terms.add("juhend soovitus");
  if (/\b(?:research|study|survey|исследован|опрос)\p{Letter}*\b/u.test(normalized)) terms.add("uuring");
  if (/\b(?:report|analysis|отч[её]т|анализ)\p{Letter}*\b/u.test(normalized)) terms.add("aruanne analüüs");
  if (/\b(?:article|publication|стать|публикац)\p{Letter}*\b/u.test(normalized)) terms.add("artikkel");
  if (/\b(?:participant|respondent|volunteer|участник|респондент|добровол)\p{Letter}*\b/u.test(normalized)) terms.add("osaleja vabatahtlik");
  if (/\b(?:county|region|municipalities|уезд|регион|муниципалитет)\p{Letter}*\b/u.test(normalized)) terms.add("maakond omavalitsus");
  return Array.from(terms).slice(0, 8);
}

function literalRetrievalAnchors(text = "") {
  const raw = String(text || "");
  const values = new Set();
  const generic = new Set([
    "and", "answer", "how", "in", "is", "please", "reply", "the", "what", "when", "where", "which", "who",
    "а", "в", "где", "и", "как", "какой", "когда", "кто", "что"
  ]);
  for (const match of raw.matchAll(/\b(?:19|20)\d{2}\b|\b\d+(?:[.,]\d+)?\s*%|§\s*\d+[a-z]?/giu)) {
    values.add(String(match[0] || "").replace(/\s+/gu, " ").trim());
  }
  for (const match of raw.matchAll(/[„“"]([^„“”"]{3,120})[”"]?/gu)) {
    values.add(String(match[1] || "").replace(/\s+/gu, " ").trim());
  }
  for (const match of raw.matchAll(/(?<![\p{Letter}])([\p{Lu}][\p{Letter}'’.-]{2,}(?:\s+[\p{Lu}][\p{Letter}'’.-]{2,}){0,3})/gu)) {
    values.add(String(match[1] || "").trim());
  }
  return Array.from(values)
    .filter(Boolean)
    .filter(value => !generic.has(value.toLowerCase()))
    .slice(0, 12);
}
...
function controlledTopicRoots(terms = []) {
  ...
  for (const term of normalized) {
    ...
    else if (term.startsWith("uurimiseet")) {
      roots.add("uurimiseet");
      roots.add("eetika");
    }
    ...
    else if (term.startsWith("eestkost")) roots.add("eestkost");
    else if (term.startsWith("hoolduskoorm")) roots.add("hoolduskoorm");
    else if (term.startsWith("sotsiaaltransport")) roots.add("sotsiaaltransport");
    else if (term.startsWith("tugiisikuteen")) roots.add("tugiisikuteen");
    else if (term.startsWith("vaimne tervis")) roots.add("vaimne tervis");
    else if (term.startsWith("sotsiaalteen")) roots.add("sotsiaalteen");
  }
...
export function matchesControlledEstonianTopic(value = "", terms = []) {
  ...
  const broadRoots = new Set(["eaka", "vanemaeal", "laps", "lastekait", "sotsiaaltoo"]);
  const distinctiveRoots = roots.filter(root => !broadRoots.has(root));
  const requiredRoots = distinctiveRoots.length ? distinctiveRoots : roots;
  return requiredRoots.some(root => text.includes(root));
}
...
function buildPersonShadowPlan({
  ...
  return {
    ...
    retrievalTermsEt: [personCanonical, coauthorCanonical, ...topicTerms].filter(Boolean),
...
function buildCrossLanguageTopicPlan(message = "", queryLanguage = null) {
  if (queryLanguage !== "ru" && queryLanguage !== "en") return null;
  const raw = stripExplicitAnswerInstruction(message);
  const topicTerms = controlledTopicTermsEt(raw);
  if (!topicTerms.length) return null;
  const literalAnchors = literalRetrievalAnchors(raw);
  const literalYears = literalAnchors.filter(value => /^(?:19|20)\d{2}$/u.test(value));
  const normalized = normalizeSemanticText(raw);
  const development = /\b(?:develop|change|trend|evolv|progress|измен|динамик|развив|эволюц|тренд)\p{Letter}*\b/u.test(normalized);
  const contact = /\b(?:contact|phone|email|specialist|контакт|телефон|почт|специалист)\p{Letter}*\b/u.test(normalized);
  const amount = /\b(?:how many|how much|number|count|сколько|число|количеств)\p{Letter}*\b/u.test(normalized);
  const canonicalLead = development
    ? "areng muutused ajas"
    : contact
      ? "kontakt spetsialist"
      : amount
        ...
        : "teave käsitlus";
  return {
    canonicalQueryEt: [topicTerms.join(" "), canonicalLead, literalAnchors.join(" ")].filter(Boolean).join(" "),
    canonicalIntent: "cross_language_topic_lookup",
    personCanonical: null,
    coauthorNames: [],
    coauthorRequested: false,
    preservedEntityTypes: [
      ...(literalYears.length ? ["year"] : []),
      ...(literalAnchors.length > literalYears.length ? ["literal_anchor"] : [])
    ],
    preservedEntityCount: literalAnchors.length,
    transliterationUsed: false,
    controlledTopicTermsEt: topicTerms,
    retrievalTermsEt: [...topicTerms, ...literalAnchors],
    controlledTopicCount: topicTerms.length
  };
}
...
export function buildChatLanguagePlan({
  ...
  const shadowPlan = extractPersonSourceShadowPlan(message, queryDetection.language)
    ...
    || buildCrossLanguageTopicPlan(message, queryDetection.language);
  ...
  return {
    ...
    controlledTopicTermsEt: Array.isArray(shadowPlan?.controlledTopicTermsEt)
      ...
      : [],
    retrievalTermsEt: Array.isArray(shadowPlan?.retrievalTermsEt)
      ? shadowPlan.retrievalTermsEt
      : []
  };
}

export function refineChatLanguagePlanWithMorphology(languagePlan = null, morphology = null) {
  if (!languagePlan || typeof languagePlan !== "object") return languagePlan;
  if (languagePlan.queryLanguage !== "unknown") return languagePlan;
  const hint = String(morphology?.language_hint || "").trim().toLowerCase();
  const confidence = roundConfidence(morphology?.language_hint_confidence);
  if (hint !== "et" || confidence < 0.8) return languagePlan;
  return {
    ...languagePlan,
    queryLanguage: "et",
    queryLanguageConfidence: confidence,
    queryLanguageReason: String(morphology?.language_hint_reason || "estonian_morphology_hint").toLowerCase().slice(0, 80),
    retrievalLanguage: "et"
  };
}
...
export function shouldActivateCanonicalSupplementalRetrieval(languagePlan = null) {
  if (!languagePlan || typeof languagePlan !== "object") return false;
  if (!languagePlan.canonicalQueryAvailable || !languagePlan.canonicalQueryEt) return false;
  if (languagePlan.canonicalIntent !== "cross_language_topic_lookup") return false;
  return languagePlan.queryLanguage === "ru" || languagePlan.queryLanguage === "en";
}
...
export function buildSafeLanguagePlanTrace(languagePlan = null, shadowQuestionPlan = null, options = {}) {
  ...
  return {
    ...
    retrieval_terms_et: Array.isArray(languagePlan.retrievalTermsEt)
      ? languagePlan.retrievalTermsEt.map(item => String(item).slice(0, 160)).slice(0, 16)
      : [],
    ...
    canonical_supplemental_retrieval_active: options?.canonicalSupplementalRetrievalActive === true,
