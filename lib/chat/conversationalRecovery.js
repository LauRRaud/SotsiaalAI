...
function recoveryPromptText(replyLang, phase) {
  ...
  if (language === "en") {
    return phase === "no_context"
      ? `CONVERSATIONAL_RECOVERY: There are no confirmed source excerpts for a factual answer. Do not answer from general model knowledge. First decide only from the user's wording whether the message has a plausible connection to the permitted social-sector scope. If it is clearly unrelated, random, or nonsensical and has no plausible social-sector intent, output exactly ${OUT_OF_SOCIAL_SCOPE_MARKER} and nothing else. If it plausibly belongs to the social sector and an ambiguity, obvious typo, or missing user-provided detail can materially help the next turn reach an answer, ask exactly one short, natural clarification question. If it plausibly belongs to the social sector but is already clear enough and no user clarification can create the missing evidence, output exactly ${NO_USEFUL_CLARIFICATION_MARKER} and nothing else. Do not mention search, RAG, a database, sources, the system, or a validator. Do not add any new fact, name, number, contact detail, or link that the user did not provide. Output only one marker or one question.`
      : "CONVERSATIONAL_RECOVERY: Use only the confirmed source excerpts for factual claims. Never fill a missing fact from external or general model knowledge. If the excerpts support a useful part of the answer, give that part immediately and state briefly what remains unproven; do not ask the user to choose a scope when that choice cannot create new evidence. Ask exactly one short, natural clarification question only when no useful factual part is supported and the user's answer can materially resolve what to answer. Do not describe the technical system or add new facts, names, numbers, contact details, or links.";
  }
  if (language === "ru") {
    return phase === "no_context"
      ? `CONVERSATIONAL_RECOVERY: Подтверждённых фрагментов источников для фактического ответа нет. Не отвечай, опираясь на общие знания модели. Сначала определи только по формулировке пользователя, есть ли у сообщения вероятная связь с разрешённой социальной сферой. Если сообщение явно не относится к ней, случайно или бессмысленно и не содержит вероятного намерения задать вопрос о социальной сфере, выведи только ${OUT_OF_SOCIAL_SCOPE_MARKER}. Если сообщение вероятно относится к социальной сфере и неоднозначность, очевидная опечатка или недостающая деталь от пользователя действительно поможет в следующем сообщении прийти к ответу, задай ровно один короткий естественный уточняющий вопрос. Если сообщение вероятно относится к социальной сфере, но уже достаточно ясно и уточнение пользователя не может создать недостающее подтверждение, выведи только ${NO_USEFUL_CLARIFICATION_MARKER}. Не упоминай поиск, RAG, базу данных, источники, систему или валидатор. Не добавляй новых фактов, имён, чисел, контактов или ссылок. Выведи только один маркер или один вопрос.`
      : "CONVERSATIONAL_RECOVERY: Для фактических утверждений используй только подтверждённые фрагменты источников. Никогда не заполняй пробелы внешними или общими знаниями модели. Если фрагменты подтверждают полезную часть ответа, сразу дай её и кратко укажи, что осталось недоказанным; не проси пользователя выбирать объём ответа, если такой выбор не создаст новых доказательств. Задай ровно один короткий естественный уточняющий вопрос только тогда, когда нельзя подтвердить никакую полезную фактическую часть и ответ пользователя действительно поможет определить ответ. Не описывай техническую систему и не добавляй новых фактов, имён, чисел, контактов или ссылок.";
  }
  return phase === "no_context"
    ? `CONVERSATIONAL_RECOVERY: Kinnitatud allikakatkendeid faktivastuseks ei ole. Ära vasta mudeli üldteadmiste põhjal. Otsusta kõigepealt ainult kasutaja sõnastuse järgi, kas sõnumil on usutav seos lubatud sotsiaalvaldkonnaga. Kui sõnum on selgelt teemaväline, juhuslik või sisutu ning selles ei ole usutavat sotsiaalvaldkonna kavatsust, väljasta täpselt ${OUT_OF_SOCIAL_SCOPE_MARKER} ja mitte midagi muud. Kui sõnum kuulub usutavalt sotsiaalvaldkonda ning ebaselgus, ilmne kirjaviga või kasutajalt puuduv detail saab järgmise pöördega päriselt vastuseni aidata, esita täpselt üks lühike loomulik täpsustusküsimus. Kui sõnum kuulub usutavalt sotsiaalvaldkonda, kuid on juba piisavalt selge ja kasutaja täpsustus ei saa puuduvat tõendit luua, väljasta täpselt ${NO_USEFUL_CLARIFICATION_MARKER} ja mitte midagi muud. Ära maini otsingut, RAG-i, andmebaasi, allikaid, süsteemi ega validatorit. Ära lisa ühtegi uut fakti, nime, numbrit, kontakti ega linki, mida kasutaja ei andnud. Väljasta ainult üks marker või üks küsimus.`
    : "CONVERSATIONAL_RECOVERY: Kasuta faktiväideteks ainult kinnitatud allikakatkendeid. Ära täida puuduvat fakti väliste ega mudeli üldteadmistega. Kui katkendid toetavad vastuse kasulikku osa, anna see kohe ja ütle lühidalt, mis jäi tõendamata; ära palu kasutajal valida vastuse ulatust, kui see valik uut tõendit ei loo. Esita täpselt üks lühike loomulik täpsustusküsimus ainult siis, kui ühtegi kasulikku faktiosa ei saa kinnitada ja kasutaja vastus saab sisuliselt määrata, millele vastata. Ära kirjelda tehnilist süsteemi ega lisa uusi fakte, nimesid, numbreid, kontakte või linke.";
}
...
export function buildNoContextRecovery({
  ...
  const domainBoundary = resolveSocialScopeBoundary(candidateReply, {
    replyLang,
  ...
  return {
    reply: String(candidateReply || "").replace(/\s+/gu, " ").trim(),
    recovery: {
      version: RECOVERY_VERSION,
      active: true,
      action: "ask_clarification",
      ...
      target,
      missing_fields: [target],
      reply_source: "single_model_call",
      question_asked: true,
      model_call_count: Math.max(0, Number(modelCallCount) || 0),
      additional_model_call_count: 0,
      external_knowledge_allowed: false,
