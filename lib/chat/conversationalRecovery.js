...
export function analyzeClarificationReply(reply = "", {
  ...
  if (!text || text.length > 420 || /[\r\n]/u.test(String(reply || ""))) return rejected;
  if ((text.match(/\?/gu) || []).length !== 1 || !/\?[)\]"'“”‘’]*$/u.test(text)) return rejected;
  if (!questionPrefixAllowed(text, language)) return rejected;
  if (
  ...
  const bodyWithoutFinalQuestionMark = text.replace(/\?[)\]"'“”‘’]*$/u, "");
  const bodyWithoutOrdinalPeriods = bodyWithoutFinalQuestionMark.replace(/\b\d+\./gu, "");
  if (/[!?]/u.test(bodyWithoutOrdinalPeriods) || /\.(?:\s|$)/u.test(bodyWithoutOrdinalPeriods)) return rejected;
...
function recoveryPromptText(replyLang, phase) {
  ...
  if (language === "en") {
    return phase === "no_context"
      ? "CONVERSATIONAL_RECOVERY: There are no confirmed source excerpts for a factual answer. Do not answer from general model knowledge. Interpret only the user's wording and obvious possible typos, then ask exactly one short, natural clarification question that would help the next turn reach an answer. Do not mention search, RAG, a database, sources, the system, or a validator. Do not add any new fact, name, number, contact detail, or link that the user did not provide. Output only the question."
      : "CONVERSATIONAL_RECOVERY: Use only the confirmed source excerpts for factual claims. Never fill a missing fact from external or general model knowledge. If the evidence cannot safely support the requested answer, do not describe the technical system: output exactly one short, natural clarification question that helps the next turn reach an answer. Do not add new facts, names, numbers, contact details, or links. Otherwise answer normally from the confirmed excerpts.";
  }
  if (language === "ru") {
    return phase === "no_context"
      ? "CONVERSATIONAL_RECOVERY: Подтверждённых фрагментов источников для фактического ответа нет. Не отвечай, опираясь на общие знания модели. Интерпретируй только формулировку пользователя и возможные очевидные опечатки, затем задай ровно один короткий естественный уточняющий вопрос, который поможет в следующем сообщении прийти к ответу. Не упоминай поиск, RAG, базу данных, источники, систему или валидатор. Не добавляй новых фактов, имён, чисел, контактов или ссылок. Выведи только вопрос."
      : "CONVERSATIONAL_RECOVERY: Для фактических утверждений используй только подтверждённые фрагменты источников. Никогда не заполняй пробелы внешними или общими знаниями модели. Если доказательств недостаточно для безопасного ответа, не описывай техническую систему: задай ровно один короткий естественный уточняющий вопрос, который поможет в следующем сообщении прийти к ответу. Не добавляй новых фактов, имён, чисел, контактов или ссылок. В остальных случаях отвечай обычно по подтверждённым фрагментам.";
  }
  return phase === "no_context"
    ? "CONVERSATIONAL_RECOVERY: Kinnitatud allikakatkendeid faktivastuseks ei ole. Ära vasta mudeli üldteadmiste põhjal. Tõlgenda ainult kasutaja sõnastust ja võimalikke ilmseid kirjavigu ning esita täpselt üks lühike loomulik täpsustusküsimus, mis aitab järgmise pöördega vastuseni jõuda. Ära maini otsingut, RAG-i, andmebaasi, allikaid, süsteemi ega validatorit. Ära lisa ühtegi uut fakti, nime, numbrit, kontakti ega linki, mida kasutaja ei andnud. Väljasta ainult küsimus."
    : "CONVERSATIONAL_RECOVERY: Kasuta faktiväideteks ainult kinnitatud allikakatkendeid. Ära täida puuduvat fakti väliste ega mudeli üldteadmistega. Kui tõend ei toeta küsitud vastust ohutult, ära kirjelda tehnilist süsteemi: esita täpselt üks lühike loomulik täpsustusküsimus, mis aitab järgmise pöördega vastuseni jõuda. Ära lisa uusi fakte, nimesid, numbreid, kontakte ega linke. Muul juhul vasta kinnitatud katkendite põhjal tavaliselt.";
}
...
function localizedClarification({ replyLang, target, years = [] }) {
  const language = normalizedLanguage(replyLang);
  ...
  if (language === "en") {
    ...
    if (target === "temporal_breakdown") return "Should I handle each year's figures separately and then summarize the trend across the period?";
  ...
  if (language === "ru") {
    ...
    if (target === "temporal_breakdown") return "Рассмотреть показатели каждого года отдельно, а затем обобщить тренд за весь период?";
  ...
  if (target === "temporal_breakdown") return "Kas käsitlen iga aasta näitajaid eraldi ja koondan seejärel kogu perioodi trendi?";
...
export function resolveValidationRecovery({
  ...
  if (validationTrace?.passed !== false) {
  ...
  const target = missingTargetForReason(reason, validationTrace);
  ...
  return {
    reply: safeProviderQuestion
      ...
      : localizedClarification({ replyLang, target, years }),
    recovery: {
...
export function buildNoContextRecovery({
  ...
  const safeCandidate = candidateQuestion.safe;
  return {
    reply: safeCandidate
      ? String(candidateReply || "").replace(/\s+/gu, " ").trim()
      : buildNoContextRecoveryFallback(replyLang, target),
    recovery: {
      version: RECOVERY_VERSION,
      active: true,
      action: "ask_clarification",
      ...
      reason: modelCallFailed
        ...
        : safeCandidate
          ? "clarification_validated"
          : "clarification_guard_fallback",
      target,
      missing_fields: [target],
      reply_source: safeCandidate ? "single_model_call" : "deterministic_fallback",
      question_asked: true,
      model_call_count: Math.max(0, Number(modelCallCount) || 0),
      additional_model_call_count: 0,
      external_knowledge_allowed: false,
      technical_status_allowed: false,
      clarification_guard: "closed_lexicon_or_typo_correction",
      correction_hints: candidateQuestion.correctionHints
    }
  };
}
...
export function buildRecoveryBoundMessage({
  ...
  const priorUserMessage = Array.isArray(trustedHistory)
    ? trustedHistory.find(item => String(item?.role || "").toLowerCase() === "user")
    : null;
  const priorText = String(priorUserMessage?.text || priorUserMessage?.content || "").trim();
  if (!priorText) return current;
  ...
  return [priorText.slice(0, priorLimit), suffix].filter(Boolean).join("\n");
}
...
export function isRagRecoveryContinuation(message = "", recoveryState = null) {
  ...
  const activeTargetMatch = recovery.target === "year_scope" || recovery.target === "temporal_breakdown"
    ? yearTargetMatch
    : recovery.target === "document_anchor"
      ? documentTargetMatch
      : recovery.target === "contact_scope"
        ? contactTargetMatch
        : recovery.target === "metric_scope"
          ? metricTargetMatch
          : false;
  ...
  if (
    startsNewQuestion ||
    ((startsStandaloneInstruction || politeStandaloneInstruction) && !activeTargetMatch)
  ) return false;
  if (isAffirmativeRecoveryReply(text)) {
    return words.length <= 8 &&
      !containsNewQuestion &&
      !changesTopicAfterAssent &&
      (!containsStandaloneInstruction || activeTargetMatch);
  }
  ...
  if (recovery.target === "contact_scope") {
