...
const NO_CONTEXT_ASSISTANCE_MODES = new Set([
  "service_navigation",
...
function evidenceBoundNoContextRequest(retrievalMeta = null) {
  const meta = retrievalMeta && typeof retrievalMeta === "object" ? retrievalMeta : {};
  const plan = meta.queryPlan && typeof meta.queryPlan === "object" ? meta.queryPlan : {};
  const mode = String(plan.mode || plan.question_planner?.mode || "").trim().toLowerCase();
  return meta.documentIdentityEvidence?.required === true ||
    meta.authorCorpusEvidence?.required === true ||
    meta.numericFactEvidence?.enabled === true ||
    meta.requestedMetricContract?.enabled === true ||
    Number(meta.questionRequirementsShadow?.requirement_count || 0) > 0 ||
    /^(?:specific_document|specific_research_fact|person_source_lookup|temporal)$/u.test(mode);
}

export function inferNoContextAssistanceMode({
  userMessage = "",
  retrievalMeta = null,
  recoveryContinuation = false
} = {}) {
  if (evidenceBoundNoContextRequest(retrievalMeta)) return null;
  const tokens = lexicalTokens(userMessage).map(normalizeLexicalToken);
  const hasPlaceQuestion = tokens.some(token => tokenStartsWithAny(token, [
    "kus", "kust", "kuhu", "millises", "asuko", "where", "municipalit", "location", "где", "куда", "мест"
  ]));
  const hasServiceNeed = tokens.some(token => tokenStartsWithAny(token, [
    "abi", "oob", "majut", "varjupa", "suuna", "teenus", "kliend", "help", "shelter", "accommodat", "refer", "service", "помощ", "ночлег", "приют", "услуг", "направ"
  ]));
  if (hasPlaceQuestion && hasServiceNeed) {
    return recoveryContinuation
      ? "service_navigation_followup"
      : "service_navigation";
  }

  const hasHowQuestion = tokens.some(token => ["kuidas", "how", "как"].includes(token));
  const hasGuidanceAction = tokens.some(token => tokenStartsWithAny(token, [
    "selgit", "toeta", "rahusta", "raag", "vestle", "planeeri", "valmista", "explain", "support", "reassur", "talk", "prepare", "plan", "объясн", "поддерж", "успоко", "поговор", "подготов", "план"
  ]));
  return hasHowQuestion && hasGuidanceAction ? "general_guidance" : null;
}
...
function localizedServiceNavigationReply(replyLang = "et", effectiveRole = "CLIENT") {
  const language = normalizedLanguage(replyLang);
  const role = String(effectiveRole || "CLIENT").trim().toUpperCase();
  if (language === "en") {
    return role === "CLIENT"
      ? "To find suitable local help, which municipality are you currently in?"
      : role === "SOCIAL_WORKER"
        ? "Which municipality is the client currently in?"
        : "Which municipality is the person who needs help currently in?";
  }
  if (language === "ru") {
    return role === "CLIENT"
      ? "Чтобы найти подходящую местную помощь, в каком муниципалитете вы сейчас находитесь?"
      : role === "SOCIAL_WORKER"
        ? "В каком муниципалитете сейчас находится клиент?"
        : "В каком муниципалитете сейчас находится человек, которому нужна помощь?";
  }
  return role === "CLIENT"
    ? "Et leida sulle sobiv kohalik abi, ütle palun, millises omavalitsuses sa praegu viibid?"
    : role === "SOCIAL_WORKER"
      ? "Millises omavalitsuses klient praegu viibib?"
      : "Millises omavalitsuses abi vajav inimene praegu viibib?";
}

function localizedGeneralGuidanceReply(replyLang = "et") {
  const language = normalizedLanguage(replyLang);
  if (language === "en") {
    return "First explain in one short sentence what the next step is and why it is needed. Then ask the person to describe how they understood it and what remains unclear, and agree together who will do what next.";
  }
  if (language === "ru") {
    return "Сначала одним коротким предложением объясните, какой следующий шаг и зачем он нужен. Затем попросите человека рассказать, как он это понял и что осталось неясным, и вместе договоритесь, кто что делает дальше.";
  }
  return "Selgita kõigepealt ühe lühikese lausega, mis on järgmine samm ja miks see vajalik on. Küsi siis inimeselt, kuidas ta sellest aru sai ja mis jäi ebaselgeks, ning leppige koos kokku, kes mida järgmisena teeb.";
}

function localizedServiceNavigationFollowup(replyLang = "et", effectiveRole = "CLIENT") {
  const language = normalizedLanguage(replyLang);
  const worker = String(effectiveRole || "CLIENT").trim().toUpperCase() === "SOCIAL_WORKER";
  if (language === "en") {
    return worker
      ? "I could not confirm a specific local service contact. I can help draft a short request to the municipality that makes clear the client needs accommodation today."
      : "I could not confirm a specific local contact. I can help draft a short request to the municipality that makes clear accommodation is needed today.";
  }
  if (language === "ru") {
    return worker
      ? "Мне не удалось подтвердить конкретный местный контакт услуги. Я могу помочь составить краткое обращение в муниципалитет с пояснением, что клиенту нужно место для ночлега сегодня."
      : "Мне не удалось подтвердить конкретный местный контакт. Я могу помочь составить краткое обращение в муниципалитет с пояснением, что место для ночлега нужно сегодня.";
  }
  return worker
    ? "Konkreetset kohalikku teenusekontakti ei saanud kinnitada. Võin aidata koostada lühikese abipalve omavalitsusele, kus on selgelt kirjas, et klient vajab ööbimiskohta täna."
    : "Konkreetset kohalikku kontakti ei saanud kinnitada. Võin aidata koostada lühikese abipalve omavalitsusele, kus on selgelt kirjas, et ööbimiskohta on vaja täna.";
}

function resolveNoContextAssistance(_candidateReply, {
  assistanceMode,
  replyLang,
  effectiveRole,
  rootUserMessageId
} = {}) {
  if (assistanceMode === "service_navigation_followup") {
    return {
      reply: localizedServiceNavigationFollowup(replyLang, effectiveRole),
      recovery: inactiveRecovery({
        action: "offer_next_step",
        trigger: "no_confirmed_context",
        reason: "service_location_unresolved_after_clarification",
        replySource: "deterministic_assistance_route",
        modelCallCount: 0,
        guard: "municipality_clarification_already_answered"
      })
    };
  }
  if (assistanceMode === "service_navigation") {
    return {
      reply: localizedServiceNavigationReply(replyLang, effectiveRole),
      recovery: {
        version: RECOVERY_VERSION,
        active: true,
        action: "ask_clarification",
        trigger: "no_confirmed_context",
        reason: "service_location_required",
        target: "municipality_scope",
        missing_fields: ["municipality_scope"],
        reply_source: "deterministic_assistance_route",
        question_asked: true,
        model_call_count: 0,
        additional_model_call_count: 0,
        external_knowledge_allowed: false,
        technical_status_allowed: false,
        clarification_guard: "deterministic_service_navigation",
        correction_hints: [],
        ...(boundedMessageId(rootUserMessageId)
          ? { root_user_message_id: boundedMessageId(rootUserMessageId) }
          : {})
  ...
  if (assistanceMode !== "general_guidance") return null;
  const reply = localizedGeneralGuidanceReply(replyLang);
  return {
    reply,
    recovery: {
      ...inactiveRecovery({
        ...
        trigger: "no_confirmed_context",
        reason: "bounded_non_factual_guidance",
        replySource: "deterministic_assistance_route",
        modelCallCount: 0,
        guard: "deterministic_general_guidance"
      }),
      question_asked: reply.includes("?"),
      ...(boundedMessageId(rootUserMessageId)
        ? { root_user_message_id: boundedMessageId(rootUserMessageId) }
        : {})
    }
...
export function inferNoContextRecoveryTarget(retrievalMeta = null, userMessage = "") {
  const meta = retrievalMeta && typeof retrievalMeta === "object" ? retrievalMeta : {};
  ...
  if (inferNoContextAssistanceMode({ userMessage, retrievalMeta: meta }) === "service_navigation") {
...
export function buildNoContextRecovery({
  ...
  const assistance = NO_CONTEXT_ASSISTANCE_MODES.has(assistanceMode)
    ? resolveNoContextAssistance(candidateReply, {
        assistanceMode,
        replyLang,
        effectiveRole,
        rootUserMessageId
