...
/**
 * SOL-NOTIF-04: kuue tunni ämber tuletatakse SÜNDMUSE ajast (viimane sõnum), mitte
 * worker'i kellast. Sama aktiivsus langeb alati samasse ämbrisse, ükskõik millal
 * job jookseb; uus sõnum järgmises ämbris annab ausalt uue teate.
 */
function roomWindow(at) {
  const size = 6 * 60 * 60 * 1000;
  return new Date(Math.floor(new Date(at).getTime() / size) * size).toISOString();
}
...
export const RECONCILE_SOURCES = Object.freeze([
  "inquiries",
  "due",
  "invites",
  "rooms",
...
/**
 * SOL-NOTIF-02 — EDENEMINE PEAB ÜLE ELAMA JOOKSU LÕPU.
 *
 * Loeb ainult need allikad, millel ON salvestatud koht. Puuduv rida tähendab
 * „alusta algusest" ja see EI ole sama mis `null` jooksu sees, mis tähendab „see
 * allikas sai selles jooksus otsa, ära teda enam küsi".
 */
export async function loadReconcileCursors(db = prisma) {
  if (!db?.notificationReconcileCursor?.findMany) return {};
  const rows = await db.notificationReconcileCursor.findMany({
    where: { source: { in: [...RECONCILE_SOURCES] } },
    select: { source: true, cursorId: true }
  });
  const cursors = {};
  for (const row of rows) {
    if (row.cursorId) cursors[row.source] = row.cursorId;
  }
  return cursors;
}

/** Allika lõpp salvestatakse NULL-ina — järgmine jooks alustab ringiga otsast. */
export async function saveReconcileCursors(db, next = {}) {
  if (!db?.notificationReconcileCursor?.upsert) return;
  for (const source of RECONCILE_SOURCES) {
    const cursorId = next[source] || null;
    await db.notificationReconcileCursor.upsert({
      where: { source },
      create: { source, cursorId },
      update: { cursorId }
    });
  }
}
...
export async function reconcileNotificationEvents({
  ...
  /* Jooksu SEES kannab kohta liitkursor (kutsuja annab ta järgmisele lehele);
     jooksude VAHEL kannab teda andmebaas. Ilma teiseta algas iga jooks uuesti
     samade vanimate ridade pealt (SOL-NOTIF-02). */
  const cursors = cursor !== null ? decodeCursor(cursor) : persistCursor ? await loadReconcileCursors(db) : null;
  ...
  /* SOL-NOTIF-03 ja -04, kaks viga ühes silmuses.
     -03: autorite välistus käis ainult SELLE lehekülje sõnumite pealt, seega sama
     ruumi teisel leheküljel kirjutanud liige sai teate aktiivsusest, mille ta ise
     tekitas. Autorid küsitakse nüüd kogu akna pealt, mitte hetkeleheküljelt.
     -04: dedupe-aken tuli WORKER'i kellast, valikuaken aga liikuvast piirist —
     sama vana sõnum sai kuue tunni piiri ümber kaks eri võtit ja seega kaks
     teadet. Aken tuleb nüüd SÜNDMUSEST: viimase sõnumi ajast. Kuni uut sõnumit
     ei tule, ei muutu ka võti — ükskõik millal job jookseb. */
  const activeRoomIds = [...new Set(roomMessages.map((message) => message.roomId))];
  for (const roomId of activeRoomIds) {
    const windowWhere = { roomId, createdAt: { gt: roomSince }, deletedAt: null };
    const [authorRows, latest] = await Promise.all([
      db.roomMessage.findMany({
        where: windowWhere,
        select: { authorId: true },
        distinct: ["authorId"]
      }),
      db.roomMessage.aggregate({ where: windowWhere, _max: { createdAt: true } })
    ]);
    const authors = [...new Set(authorRows.map((row) => row.authorId).filter(Boolean))];
    const latestActivityAt = latest?._max?.createdAt || now;
    const members = await db.roomMember.findMany({
      where: { roomId, leftAt: null, ...(authors.length ? { userId: { notIn: authors } } : {}) },
      select: { userId: true }
    });
    for (const member of members) {
      await emit({
        userId: member.userId, type: NOTIFICATION_EVENT_TYPES.ROOM_ACTIVITY,
        sourceId: roomId, targetId: roomId,
        dedupeSuffix: roomWindow(latestActivityAt), emailPolicy: "OPTIONAL"
      });
    }
  }
  ...
  /* Salvestatakse ka siis, kui mõni allikas sai otsa (NULL = ring otsast peale).
     `dryRun` ei liiguta kohta, muidu „proovijooks" varastaks ridu päris jooksult. */
  if (persistCursor && !dryRun) await saveReconcileCursors(db, next);
