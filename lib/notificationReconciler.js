...
export async function reconcileNotificationEvents({
  ...
  const [inquiries, dueInquiries, invites, roomMessages, matches, assignments, services] = await Promise.all([
    ...
    page("services", () => db.serviceProviderService.findMany({
      ...
      select: { id: true, providerProfileId: true, availabilityCheckedAt: true, providerProfile: { select: { ownerId: true } } },
      orderBy: { id: "asc" }, take
    }))
  ]);
  ...
  for (const service of services) {
    if (!service.providerProfile?.ownerId) { counters.skipped += 1; continue; }
    const period = service.availabilityCheckedAt?.toISOString() || "never";
    await emit({
      userId: service.providerProfile.ownerId, type: NOTIFICATION_EVENT_TYPES.SERVICE_AVAILABILITY_STALE,
