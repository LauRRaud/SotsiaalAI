...
async function enqueueRemoval({ receipt, actorUserId, code, db, audit, now, infected = false }) {
  await db.$transaction(async tx => {
    await tx.materialUploadQuarantine.update({
      where: { id: receipt.id },
      data: {
        scanState: infected ? "INFECTED" : (receipt.scanState === "PENDING" ? "FAILED" : receipt.scanState),
        failureCode: code,
    ...
    await audit({
      ...
      actorUserId,
      targetUserId: receipt.submittedByUserId,
      action: infected ? "MATERIAL_MALWARE_REJECTED" : "MATERIAL_QUARANTINE_REJECTED",
      resourceType: "MaterialUploadQuarantine",
      resourceId: receipt.id,
...
export async function discardMaterialQuarantine(
  { receiptId, actorUserId, code = "quarantine_rejected" } = {},
  { db = prisma, files = defaultFiles, audit = writeDataAudit, now = new Date() } = {}
) {
  const receipt = await db.materialUploadQuarantine.findUnique({ where: { id: String(receiptId || "") } })
  if (!receipt) return { deleted: true, replay: true }
  await enqueueRemoval({ receipt, actorUserId: String(actorUserId || receipt.submittedByUserId), code, db, audit, now })
  const result = await reconcileMaterialQuarantineDeletion({ receiptId: receipt.id }, { db, files })
  await db.$transaction(async tx => {
    await tx.dataDeletionJob.deleteMany({
      where: { action: QUARANTINE_DELETE_ACTION, resourceType: "MaterialUploadQuarantine", resourceId: receipt.id, status: "done" }
    })
    await tx.materialUploadQuarantine.deleteMany({ where: { id: receipt.id, storageState: "REMOVED" } })
  })
  return result
}
...
export async function quarantineMaterialUpload(
  ...
  try {
    ...
    await discardMaterialQuarantine(
      { receiptId: receipt.id, actorUserId: ownerId, code: "quarantine_write_failed" },
      { db, files, audit, now }
    )
  ...
  try {
    ...
    await discardMaterialQuarantine({ receiptId: receipt.id, actorUserId: ownerId, code }, { db, files, audit, now })
  ...
  if (result.state === "CLEAN" && (
    ...
    await discardMaterialQuarantine(
      { receiptId: receipt.id, actorUserId: ownerId, code: "scanner_protocol_error" },
      { db, files, audit, now }
    )
  ...
  if (result.state !== "CLEAN") {
    ...
    await enqueueRemoval({ receipt, actorUserId: ownerId, code, db, audit, now, infected: true })
  ...
  try {
    ...
    await discardMaterialQuarantine(
      { receiptId: receipt.id, actorUserId: ownerId, code: "file_validation_failed" },
      { db, files, audit, now }
    )
    throw error
  }
