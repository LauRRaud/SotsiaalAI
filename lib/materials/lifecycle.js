...
import { removeMaterialForWithdrawal } from "./retention.js"
...
function materialNotificationMessageId(batchId) {
...
export function buildMaterialRequestHash({ comment = "", files = [] } = {}) {
  const canonical = JSON.stringify({
    ...
    files: files.map((file) => ({
      ...
      sanitizedSha256: String(file.sanitizedSha256 || ""),
      sanitizationVersion: String(file.sanitizationVersion || "")
    }))
  })
...
const defaultFiles = Object.freeze({
  write: (storagePath, buffer) => writeMaterialBuffer(buffer, storagePath),
  publish: publishStoredMaterial,
  remove: deleteStoredMaterial,
  exists: storedMaterialExists
})

const defaultQuarantineFiles = Object.freeze({ remove: deleteQuarantinedMaterial })

async function requireCleanMaterialReceipts({ files, ownerId, db }) {
  const ids = files.map(file => String(file.quarantineReceiptId || "").trim())
  if (ids.some(id => !id) || new Set(ids).size !== ids.length) {
    throw materialError("materials_page.errors.security_receipt_required", 409)
  }
  const receipts = await db.materialUploadQuarantine.findMany({
    where: {
      id: { in: ids },
  ...
  for (const file of files) {
    const receipt = byId.get(file.quarantineReceiptId)
    if (
      !receipt || receipt.sha256 !== file.sha256 || receipt.size !== file.size ||
      receipt.declaredMime !== file.mime || !receipt.scannedAt
    ) {
      throw materialError("materials_page.errors.security_receipt_invalid", 409)
    }
  }
  return byId
}

async function removeConsumedQuarantines(files, receipts, db, quarantineFiles) {
  for (const file of files) {
    const receipt = receipts.get(file.quarantineReceiptId)
    if (receipt?.quarantinePath) await quarantineFiles.remove(receipt.quarantinePath)
    await db.materialUploadQuarantine.updateMany({
      where: { id: file.quarantineReceiptId, storageState: "CONSUMED" },
      data: { storageState: "REMOVED", quarantinePath: null }
    })
  }
}

function includeSubmitter(includeNotification = false) {
  return {
...
export async function listMaterialSubmissions(
  ...
  const [rows, total, grouped] = await Promise.all([
    db.materialSubmission.findMany({
      ...
      take: pageSize + 1,
      include: includeSubmitter(admin)
    }),
  ...
  return {
    submissions: page.items.map(row => serializeMaterialSubmission(row, { includeNotification: admin })),
...
async function claimBatch({ userId, idempotencyKey, requestHash, files, now, rateLimit, windowMs }, db) {
  return db.$transaction(async (tx) => {
    ...
    let batch = await tx.materialSubmissionBatch.create({
      data: {
    ...
    batch = await tx.materialSubmissionBatch.update({
      where: { id: batch.id },
      data: { notificationMessageId: materialNotificationMessageId(batch.id) }
    })
    const jobs = []
  ...
  for (const job of jobs) {
    try {
      if (job.action === FILE_ACTION.STAGE) {
        ...
        await files.remove(getSanitizedMaterialPath(job.storagePath))
        ...
        const sanitizedFinalPath = getSanitizedMaterialPath(finalPath)
        const sanitizedStagingPath = getSanitizedMaterialPath(job.storagePath)
        if (!(await files.exists(sanitizedFinalPath))) {
          if (!(await files.exists(sanitizedStagingPath))) throw new Error("material_sanitized_file_missing_both_paths")
          await files.publish(sanitizedStagingPath, sanitizedFinalPath)
        }
...
export async function createMaterialSubmissions(
  ...
  { db = prisma, quota = withStorageQuota, fileOps = defaultFiles, quarantineFiles = defaultQuarantineFiles, now = new Date() } = {}
) {
  const ownerId = String(userId || "").trim()
  if (!ownerId) throw materialError("api.common.unauthorized", 401)
  ...
  const securityReceipts = await requireCleanMaterialReceipts({ files, ownerId, db })
  ...
  try {
    for (const [index, file] of files.entries()) {
      ...
      if (!Buffer.isBuffer(file.sanitizedBuffer) || !file.sanitizedSha256) {
        throw materialError("material_sanitized_derivative_required", 409)
      }
      await fileOps.write(getSanitizedMaterialPath(job.storagePath), file.sanitizedBuffer)
    }
  } catch (error) {
  ...
  const totalBytes = files.reduce((sum, file) => sum
    + Number(file.size || file.buffer?.byteLength || 0)
    + Number(file.sanitizedBuffer?.byteLength || 0), 0)
  let created
  try {
    created = await quota(
        ...
        for (const [index, file] of files.entries()) {
          ...
          const row = await tx.materialSubmission.create({
            data: {
              ...
              scanEngineVersion: file.scanEngineVersion,
              scanSignatureVersion: file.scanSignatureVersion,
              scanSignatureUpdatedAt: file.scanSignatureUpdatedAt,
              scanFailureCode: null,
              quarantineReceiptId: file.quarantineReceiptId,
              ...retentionFieldsForSubmission("pending", now, retentionPolicy),
              duplicateOfId: duplicate?.id || null
            },
            include: includeSubmitter()
          })
          const consumed = await tx.materialUploadQuarantine.updateMany({
            where: {
              id: file.quarantineReceiptId,
              submittedByUserId: ownerId,
              scanState: "CLEAN",
              validationState: "VALIDATED",
              storageState: "QUARANTINED"
            },
            data: { storageState: "CONSUMED" }
          })
          if (consumed.count !== 1) throw materialError("materials_page.errors.security_receipt_invalid", 409)
          await tx.dataDeletionJob.update({
  ...
  await removeConsumedQuarantines(files, securityReceipts, db, quarantineFiles)
...
export async function requestMaterialSubmissionDeletion(
  ...
  if (!admin && !["pending", "rejected", "imported"].includes(submission.status)) {
    throw materialError("materials_page.errors.withdraw_not_allowed", 409)
  }
  const result = await removeMaterialForWithdrawal(submission, { actorUserId: actorId }, {
    db,
    files,
    audit,
    now
  })
  if (!result.ok) throw materialError("materials_page.errors.delete_pending", 503)
  return { deleted: true, replay: result.outcomes.every(outcome => outcome?.replay) }
}
