 ...
 */
async function runKovRagStateReset(slug, layer, { plan: providedPlan = null } = {}) {
  /* SOL-RAGADMIN-04: kutsuja VÕIB anda plaani ette ja siis on see plaan seesama,
     mille kinnitusvärav üle vaatas. Ilma selleta arvutaks värav ühe plaani ja
     täitmine teise — täpselt see aken, mille vastu värav on. Kuju kontrollitakse,
     sest vale slug'iga plaan oleks vaikne katastroof. */
  if (providedPlan) {
    const planSlug = String(providedPlan?.municipality?.slug || "").trim();
    const planLayer = String(providedPlan?.cleanup_layer || "");
    if (planSlug !== slug || planLayer !== layer) {
      const error = new Error("KOV RAG reset plan does not match the requested slug/layer");
      error.status = 409;
      error.code = "rag_reset_plan_mismatch";
      throw error;
    }
  }
  const plan = providedPlan || (await planKovRagStateResetBySlug(slug, { layer }));
...
export async function executeKovRagStateResetBySlug(slug, options = {}) {
  return runKovRagStateReset(slug, "all", options);
}
