...
} from "@/lib/admin/rag/ingestClaim";
...
export async function syncKovAdminIngestStatusById(kovAdminId) {
  let row = await prisma.municipalityKovAdmin.findUnique({
    where: { id: kovAdminId },
    include: KOV_SYNC_INCLUDE
  });

  if (!row) return null;

  /* SOL-RAGADMIN-03: SEE FUNKTSIOON OLI LUKU IGAVESE ELU PÕHJUS. Ta säilitas
     `INGESTING` seisu ALATI, seega katkestuse järel ei viinud ükski rada rida
     enam tagasi. Nüüd küsitakse aegunud claim'i puhul RAG-ist, mis päriselt sai
     — ja alles siis kirjutatakse seis. `unknown` ei otsusta midagi. */
  const reconciled = await Promise.all([
    reconcileStaleIngestClaim({
      delegate: prisma.municipalityKovAdmin,
      row,
      lane: INGEST_LANES.KOV_WEB,
      readPresence: readRagDocumentPresence
    }),
    reconcileStaleIngestClaim({
      delegate: prisma.municipalityKovAdmin,
      row,
      lane: INGEST_LANES.KOV_RT,
      readPresence: readRagDocumentPresence
    })
  ]);
  if (reconciled.some(result => result.reconciled)) {
    /* LEPITUS ON SELLE KUTSE TULEMUS. Allpool olev arvutus viiks iga
       mitte-INGESTING seisu tagasi READY/NOT_INGESTED peale ja kustutaks
       `lastIngestError` — ehk kirjutaks just tuvastatud tõe kohe üle
       valmisoleku-oletusega. Valmisoleku jaoks on järgmine lugemine. */
    row = await prisma.municipalityKovAdmin.findUnique({
      where: { id: kovAdminId },
      include: KOV_SYNC_INCLUDE
    });
    if (!row) return null;
    return serializeKovAdminWithRepositoryFallback(row);
  }
  ...
  const updated = await prisma.municipalityKovAdmin.update({
    ...
    data: {
      ...
      /* Mitte-INGESTING seis ei kanna lease'i. INGESTING oma EI TOHI siin
         kaduda — see oleks elusa luku vaikne vabastamine. */
      ...(nextStatus === "INGESTING" ? {} : clearedIngestClaim(INGEST_LANES.KOV_WEB)),
      ...(nextRtStatus === "INGESTING" ? {} : clearedIngestClaim(INGEST_LANES.KOV_RT))
    },
    include: KOV_SYNC_INCLUDE
  });
...
async function assertKovIngestPreconditions(entry) {
  ...
  /* SOL-RAGADMIN-03: blokeerib ainult ELUS claim. Paljas „kas seis on
     INGESTING" hoiaks surnud lukku igavesti kinni ja ei laseks claim'i varastada
     — värav ise on tingimuslik `claimIngestLease`, see kontroll on ainult
     sõbralik ja kiire 400 tavajuhu jaoks. */
  if (hasLiveIngestClaim(entry, INGEST_LANES.KOV_WEB)) {
...
async function assertKovRtIngestPreconditions(entry) {
  ...
  /* SOL-RAGADMIN-03 — vt `assertKovIngestPreconditions`. */
  if (hasLiveIngestClaim(entry, INGEST_LANES.KOV_RT)) {
...
export async function ingestKovEntryBySlug(slug, options = {}) {
  ...
  /* SOL-RAGADMIN-03: LUKU VÕTMINE ON VÄRAV, mitte märge. Kui `count !== 1`, siis
     keegi teine on juba sees ja meie ei tee mitte midagi — varem läbisid mõlemad
     paralleelsed päringud eelkontrolli ja kirjutasid sama `doc_id` peale. */
  const claim = await claimIngestLease({
    delegate: prisma.municipalityKovAdmin,
    id: entry.id,
    lane: INGEST_LANES.KOV_WEB,
    docId: ragDocId
  });
  if (!claim.ok) throw ingestClaimConflictError("KOV ingest is already in progress");

  let ragText = "";
  let datasetPayload = null;
  let metaPayload = null;
  let sourcesPayload = null;
  /* VAHETUSPUNKT: pärast õnnestunud RAG-kirjutust ei tohi ükski veaharu enam
     väita, et dokumenti ei ole. Sama mõte mis SOL-RAGADMIN-01 `committed`. */
  ...
  try {
    ...
    /* Lõppseis kirjutatakse ainult MEIE claim'i peale: aegunud lease võis
       vahepeal minna kellelegi teisele ja tema tulemust me üle ei kirjuta. */
    ...
    if (!finished.ok) {
      const error = new Error("KOV ingest claim was taken over before the result could be stored");
      error.status = 409;
      error.code = "ingest_claim_lost";
      throw error;
    }

    const updated = await prisma.municipalityKovAdmin.findUnique({
      where: { id: entry.id },
      include: {
        municipality: {
          select: {
    ...
    if (ragWriteCompleted) {
      /* RAG-is ON dokument olemas. `ERROR` oleks siin VALE VÄIDE — täpselt see
         viga, mis ütles adminile „ei õnnestunud", kuigi dokument mõjutas edasi
         otsingutulemusi. Rida jääb `INGESTING`-usse; lease aegub ja lepitus
         (`reconcileStaleIngestClaim`) küsib RAG-ist tõe ja viib rea INGESTED-isse. */
      try {
        console.error("[kov-admin] ingest wrote to RAG but the DB result was not stored", {
          slug,
          docId: ragDocId,
          error: safeError(error)
        });
      } catch {}
      error.ragWriteCompleted = true;
    } else {
      await releaseIngestClaimWithError({
        ...
        lane: INGEST_LANES.KOV_WEB,
        claimId: claim.claimId,
        message: normalizeIngestErrorMessage(error),
        docId: ragDocId
      });
    }
...
export async function ingestKovRtEntryBySlug(slug) {
  ...
  /* SOL-RAGADMIN-03 — sama värav mis veebirajal, oma veergudega. */
  const claim = await claimIngestLease({
    delegate: prisma.municipalityKovAdmin,
    id: entry.id,
    lane: INGEST_LANES.KOV_RT,
  ...
  if (!claim.ok) throw ingestClaimConflictError("RT ingest is already in progress");
  ...
  try {
    ...
    if (!finished.ok) {
      const error = new Error("RT ingest claim was taken over before the result could be stored");
      error.status = 409;
      error.code = "ingest_claim_lost";
      throw error;
    }
    ...
    if (ragWriteCompleted) {
      /* Dokument on RAG-is. Vt veebiraja kommentaari: `ERROR` oleks vale väide. */
      try {
        console.error("[kov-admin] RT ingest wrote to RAG but the DB result was not stored", {
          slug,
          docId: rtRagDocId,
          error: safeError(error)
        });
