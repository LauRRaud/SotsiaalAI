...
function assertOrganizationIngestPreconditions(entry) {
  ...
  /* SOL-RAGADMIN-03: blokeerib ainult ELUS claim — surnud lukk peab olema
     varastatav, muidu ei ole taastumist. Värav ise on `claimIngestLease`. */
  if (hasLiveIngestClaim(entry, INGEST_LANES.ORGANIZATION)) {
...
export async function ingestOrganizationEntryBySlug(slug) {
  ...
  /* SOL-RAGADMIN-03 — sama värav mis KOV-il, oma tabelis. */
  ...
  if (!claim.ok) throw ingestClaimConflictError("Organization ingest is already in progress");
  ...
  try {
    ...
    if (!finished.ok) {
      const error = new Error("Organization ingest claim was taken over before the result could be stored");
      error.status = 409;
      error.code = "ingest_claim_lost";
      throw error;
    }

    const updated = await prisma.organizationAdmin.findUnique({
      where: { id: entry.id },
      include: {
        files: true
      }
    ...
    if (ragWriteCompleted) {
      /* Dokument on RAG-is: `ERROR` oleks vale väide. Lease aegub ja lepitus
         küsib RAG-ist tõe. Vt `lib/admin/rag/ingestReconcile.js`. */
      try {
        console.error("[organization-admin] ingest wrote to RAG but the DB result was not stored", {
          slug,
          docId: ragDocId,
