...
function activeRetentionCandidate(now) {
  return {
  ...
  const candidates = await prisma.practiceReflection.findMany({
    where: {
      OR: [
        ...
        activeRetentionCandidate(now)
      ]
    },
    // Rechecking an open-ended contract touches updatedAt, so oldest-first
    // ordering prevents a full batch of null deadlines from starving later rows.
    ...
    try {
      ...
      const deleted = await prisma.practiceReflection.deleteMany({
        where: candidate.retentionState === USER_DELETED
          ...
          : { id: candidate.id, ...activeRetentionCandidate(now) }
      });
