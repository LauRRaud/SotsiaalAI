...
export const REFLECTION_UNDO_WINDOW_MS = 30_000;
const RETENTION_ACTIVE = "ACTIVE";
const RETENTION_USER_DELETED = "USER_DELETED";
...
async function resolveRetentionDeadline(prisma, ownerUserId, now) {
  // Unit-test fakes do not prove retention. Production Prisma and the dedicated
  // PostgreSQL probe always have this model; keeping the fallback avoids making
  // unrelated fake-prisma tests pretend that they exercise the contract source.
  if (typeof prisma?.subscription?.findFirst !== "function") return null;
...
export async function createPracticeReflectionForUser(userId, payload = {}, options = {}) {
  ...
  const createInTransaction = async (tx) => {
    ...
    const retentionDeadline = options.retentionDeadline !== undefined
      ? options.retentionDeadline
      : await resolveRetentionDeadline(tx, ownerUserId, options.now || new Date());
...
export async function getPracticeReflectionForUser(userId, reflectionId, options = {}) {
  ...
  const reflection = await prisma.practiceReflection.findFirst({
    where: { id, ownerUserId, retentionState: RETENTION_ACTIVE }
  });
...
export async function updatePracticeReflectionForUser(userId, reflectionId, payload = {}, options = {}) {
  ...
  const result = await prisma.practiceReflection.updateMany({
    where: { id, ownerUserId, retentionState: RETENTION_ACTIVE, updatedAt: expectedUpdatedAt },
    data
  });
  if (Number(result?.count) !== 1) {
    const current = await prisma.practiceReflection.findFirst({
      where: { id, ownerUserId, retentionState: RETENTION_ACTIVE }
    });
    if (current) {
...
export async function deletePracticeReflectionForUser(userId, reflectionId, options = {}) {
  ...
  const undoUntil = new Date(now.getTime() + REFLECTION_UNDO_WINDOW_MS);
  const result = await prisma.practiceReflection.updateMany({
    where: { id, ownerUserId, retentionState: RETENTION_ACTIVE },
    data: {
      retentionState: RETENTION_USER_DELETED,
      deletedAt: now,
      undoUntil
    }
  });
  if (Number(result?.count) === 1) {
    return { deleted: true, replayed: false, undoUntil };
  }
  const existing = await prisma.practiceReflection.findFirst({
    where: { id, ownerUserId, retentionState: RETENTION_USER_DELETED }
  });
  if (existing?.undoUntil && new Date(existing.undoUntil).getTime() > now.getTime()) {
    return { deleted: true, replayed: true, undoUntil: existing.undoUntil };
  }
  return { deleted: false, replayed: false, undoUntil: null };
}

export async function undoPracticeReflectionDeletionForUser(userId, reflectionId, options = {}) {
  const ownerUserId = requireUserId(userId);
  const id = requireReflectionId(reflectionId);
  const prisma = options.prisma || defaultPrisma;
  const now = options.now || new Date();
  ...
  return { restored: Number(result?.count) === 1 };
}
...
export async function getReflectionActivityForUser(userId, options = {}) {
  ...
  const latest = await prisma.practiceReflection.findFirst({
    where: { ownerUserId, retentionState: RETENTION_ACTIVE },
    orderBy: { updatedAt: "desc" },
