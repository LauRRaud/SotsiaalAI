...
function computeCapabilities(role, process, { hasValidAcceptance = false } = {}) {
  ...
  return {
    ...
    // Jagada saab AINULT kehtiva kontraktikinnitusega osaleja (OS, mitte OS†).
    // Superviisor on M7 puhul alati LUGEJA (Q2.2 M7) ja skeem teeb ta autorluse
    // võimatuks (M7.authorParticipationId on NOT NULL FK osalusele, SV-l osalust
    // pole) — topics.js viskab talle 403. Lipp peab seda jõustust peegeldama,
    // muidu näitaks UI superviisorile nuppu, mis alati ebaõnnestub.
    canShareTopic: isOs && hasValidAcceptance && open,
...
function buildMemberDetail(process, viewer, data) {
  ...
  const visibleSummaries = summaries.filter((summary) => {
    if (!beforeLeft(summary)) return false;
  ...
  const visibleMeetings = meetings.filter(beforeLeft);
