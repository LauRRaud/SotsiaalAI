...
function summaryCardinalityConflict(kind) {
  return kind === "MEETING"
    ? conflict("supervision.errors.conflict", "SUMMARY_EXISTS_FOR_MEETING")
    : conflict("supervision.errors.conflict", "FINAL_SUMMARY_EXISTS");
}
...
export async function createSummary({ processId, session, input }, options = {}) {
  ...
  if (kind === "MEETING") {
    ...
    const existing = await db.supervisionSummary.findFirst({ where: { meetingId, status: { not: "DISCARDED" } } });
    ...
    const existingFinal = await db.supervisionSummary.findFirst({
      where: { processId: process.id, kind: "FINAL", status: { not: "DISCARDED" } }
    });
  ...
  const summary = await withSupervisionProcessLock(db, process.id, async (tx) => {
    ...
    if (freshViewer.role !== VIEWER_ROLES.SV) throw notFound();
    if (freshProcess.status === "CLOSED") throw conflict("supervision.errors.already_closed", "ALREADY_CLOSED");
    if (kind === "MEETING") {
      const meeting = await tx.supervisionMeeting.findFirst({ where: { id: meetingId, processId: process.id } });
      if (!meeting) throw notFound();
    }
    const existing = await tx.supervisionSummary.findFirst({
      where: kind === "MEETING"
        ? { meetingId, status: { not: "DISCARDED" } }
        : { processId: process.id, kind: "FINAL", status: { not: "DISCARDED" } }
    });
    if (existing) throw summaryCardinalityConflict(kind);
    let created;
    try {
      created = await tx.supervisionSummary.create({
        data: { processId: process.id, meetingId, kind, body, status: "DRAFT", createdByUserId: userId, version: 0 }
      });
    } catch (error) {
      if (error?.code === "P2002") throw summaryCardinalityConflict(kind);
      throw error;
    }
...
export async function discardSummary({ summaryId, session }, options = {}) {
  ...
  await withSupervisionProcessLock(db, process.id, async (tx) => {
    ...
    if (fresh.status === "APPROVED") throw conflict("supervision.errors.conflict", "SUMMARY_IMMUTABLE");
    const approvalsDeleted = await tx.supervisionSummaryApproval.deleteMany({ where: { summaryId: summary.id } });
    const notificationsClosed = await tx.notificationEvent.updateMany({
      where: {
    ...
    await recordSupervisionAudit(tx, {
      action: ACTIONS.SUMMARY_DISCARDED,
      actorUserId: userId,
      processId: process.id,
      targetKind: "summary",
      targetId: summary.id,
      metadata: {
        approvalsDeleted: Number(approvalsDeleted?.count || 0),
        notificationsClosed: Number(notificationsClosed?.count || 0)
      }
    });
    await tx.supervisionProcess.update({ where: { id: process.id }, data: { lastActivityAt: now } });
  });
...
export async function approveSummary({ summaryId, session }, options = {}) {
  ...
  await withSupervisionProcessLock(db, process.id, async (tx) => {
    ...
    const freshParticipation = await tx.supervisionParticipation.findFirst({
      where: {
        id: viewer.participation.id,
        processId: process.id,
        userId,
        status: "ACCEPTED"
      }
    });
    if (!freshParticipation) throw notFound();
    const freshAcceptance = freshProcess.activeContractVersionId
      ? await tx.supervisionContractAcceptance.findFirst({
        where: {
          participationId: freshParticipation.id,
          contractVersionId: freshProcess.activeContractVersionId
        }
      })
      : null;
    if (!freshAcceptance) {
    ...
    const existing = await tx.supervisionSummaryApproval.findFirst({
      where: { summaryId: id, participationId: freshParticipation.id }
    });
    if (!existing) {
      try {
        await tx.supervisionSummaryApproval.create({
          data: { summaryId: id, participationId: freshParticipation.id, approvedAt: now }
        });
      } catch (error) {
