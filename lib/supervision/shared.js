...
/**
 * Kanooniline protsessi `lastActivityAt` leping. Siin on kõik jagatud või
 * protsessi olekut muutvad kasutajatoimingud; M6 privaatkirje create/update/
 * delete on teadlikult väljas, sest teise liikme loendijärjekord ei tohi
 * paljastada omaniku privaattegevust.
 */
export const SUPERVISION_ACTIVITY_EVENTS = Object.freeze([
...
function requireAuthenticatedSupervisionUser(session) {
...
/**
 * Detail-/skoobitud rajad lubavad ainult praeguse liikmerolliga kasutajaid.
 * Rollist eemaldatud vana protsessiseos ei anna ligipääsu; 404 säilitab võõra
 * ja olematu objekti ühetaolise vastuse (Q2.4).
 */
export function requireSupervisionUser(session) {
  const user = requireAuthenticatedSupervisionUser(session);
  if (user.isAdmin || !SUPERVISION_MEMBER_ROLES.includes(user.role)) throw notFound();
  return user;
}
...
export function requireSupervisionMember(session) {
  const { userId, role, isAdmin } = requireAuthenticatedSupervisionUser(session);
...
/**
 * Jagatud protsessiala kirjutusvärav. Kontroll käib alati protsessiluku SEES,
 * et sulgemine ei mahuks eelloetud oleku ja hilisema kirjutuse vahele.
 * Eeskambri lugemine jääb pärast sulgemist kättesaadavaks, kuid kõik kirjutused
 * kasutavad samuti seda väravat, et suletud protsessi ajalugu ei muutuks.
 */
export async function requireOpenSupervisionProcess(tx, processId) {
  const process = await tx.supervisionProcess.findUnique({ where: { id: processId } });
...
/** Vabatahtlik ISO-kuupäev; tühi → null; vigane → 400. */
export function parseOptionalDate(value, code = "INVALID_DATE") {
  if (value == null || value === "") return null;
  const d = value instanceof Date ? value : new Date(value);
  if (!Number.isFinite(d.getTime())) throw invalid(code);
  return d;
}
