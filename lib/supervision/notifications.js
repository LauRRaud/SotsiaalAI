...
export async function notifyMeetingUpcoming(tx, { meetingId, processId, userId, plannedAt }, ctx = {}) {
  return emitSupervisionNotification(tx, {
    type: NOTIFICATION_EVENT_TYPES.SUPERVISION_MEETING_UPCOMING,
    userId,
    sourceId: meetingId,
    targetId: processId,
    // Täpse ajaga dedupe: kordusjooks on idempotentne, kuid päriselt ümber
    // tõstetud kohtumine saab uue meeldetuletuse.
    dedupeSuffix: `upcoming:${new Date(plannedAt).toISOString()}`
  }, ctx);
}
...
/** Osaleja lahkus → superviisor ja allesjäänud aktiivsed osalejad. */
export async function notifyParticipantLeft(tx, { participationId, processId, userId }, ctx = {}) {
  return emitSupervisionNotification(tx, {
    type: NOTIFICATION_EVENT_TYPES.SUPERVISION_PARTICIPANT_LEFT,
    userId,
    sourceId: participationId,
    targetId: processId,
    dedupeSuffix: "v1"
...
export async function buildSupervisionContinuity(db, userId) {
  ...
  for (const participation of accepted) {
    ...
    // Ettevalmistus on kohtumisepõhine: varasema kohtumise teema ei tohi
    // järgmise ettevalmistust kustutada. Valmisoleku marker on järgmise
    // kohtumise agendaTopicIds seos kasutaja SHARED teemaga.
    const nextMeeting = await db.supervisionMeeting.findFirst({
      where: { processId: participation.processId, status: "PLANNED" },
      select: { id: true, plannedAt: true, agendaTopicIds: true }, orderBy: [{ seq: "asc" }]
    });
    if (nextMeeting) {
      const myTopics = nextMeeting.agendaTopicIds.length === 0 ? 0 : await db.supervisionSharedTopic.count({
        where: {
          id: { in: nextMeeting.agendaTopicIds },
          processId: participation.processId,
          authorParticipationId: participation.id,
          status: "SHARED"
        }
      });
      if (myTopics === 0) {
