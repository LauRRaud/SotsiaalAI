...
export async function createPrivateItem({ processId, session, input }, options = {}) {
  ...
  const item = await withSupervisionProcessLock(db, process.id, async (tx) => {
    await requireEeskamberProcess(tx, process.id, userId, { write: true });
    await requireOpenSupervisionProcess(tx, process.id);
    return tx.supervisionPrivateItem.create({
...
export async function updatePrivateItem({ itemId, session, input }, options = {}) {
  ...
  const updated = await withSupervisionProcessLock(db, item.processId, async (tx) => {
    await requireOpenSupervisionProcess(tx, item.processId);
