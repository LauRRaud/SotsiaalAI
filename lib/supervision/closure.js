...
export async function closePreview({ processId, session }, options = {}) {
  ...
  if (viewer.role === VIEWER_ROLES.KUT) throw notFound();

  const [summaries, topics, meetings, closure] = await Promise.all([
  ...
  return {
    ...
    preview: {
      alreadyClosed,
      canClose: viewer.role === VIEWER_ROLES.SV && !alreadyClosed && pending.length === 0,
      pendingSummaryIds: pending.map((s) => s.id),
      willDelete: {
        sharedTopics: topics.length,
        draftSummaries: summaries.filter((s) => ["DRAFT", "DISCARDED"].includes(s.status)).length,
        meetingNotes: meetings.filter((m) => m.note != null).length
      },
      willKeep: {
        approvedSummaries: summaries.filter((s) => s.status === "APPROVED").length,
...
export async function closeProcess({ processId, session, input }, options = {}) {
  ...
  await withSupervisionProcessLock(db, process.id, async (tx) => {
    ...
    const activeContract = fresh.activeContractVersionId
    ...
    if (!isDraft) {
      const content = {
        ...
        lastAcceptedContractBody: activeContract?.body || null,
        facts
      };
      const owners = [...new Set([fresh.supervisorId, ...accepted.map((p) => p.userId)])];
      for (const ownerUserId of owners) {
        await tx.supervisionPersonalOutcome.create({
          data: { ownerUserId, processId: process.id, processTitleGeneralized: generalizedTitle, contentJson: content }
        });
      }
    }
