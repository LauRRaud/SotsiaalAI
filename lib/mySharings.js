...
function buildSectionLoaders({ db, ownerId, now }) {
  ...
  return {
    preInquiries: async () => {
      ...
      return ready(page.items.map((row) => serializeSentPreInquiry(row, canonicalRoomIds)), page.paging);
    },
    rooms: async () => {
...
export async function loadMySharings(
  userId,
  { db = prisma, now = new Date(), sections = null, deadlineMs = MY_SHARINGS_SECTION_DEADLINE_MS } = {}
) {
  ...
  const loaders = buildSectionLoaders({ db, ownerId, now });
