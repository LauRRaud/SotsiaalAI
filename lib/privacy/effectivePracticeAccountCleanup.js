...
export async function deleteUserAfterFinalPracticeSweep(userId, db) {
  ...
  return db.$transaction(async tx => {
    ...
    /* `NetworkShare.clientUserId` uses SET NULL so that genuinely external
       clients remain supported. For an account-linked client, however, the
       frozen summary and confirmation metadata must not outlive an account
       deletion and become an apparently external share. Delete those rows
       while the client relation is still available and under the same lock. */
    const clientNetworkSharesDeleted = await tx.networkShare.deleteMany({
    ...
    return {
      ...
      privacyCounts: {
        ...
        deletedClientNetworkShares: Number(clientNetworkSharesDeleted?.count || 0),
