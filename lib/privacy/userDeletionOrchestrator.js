export async function runUserDeletionCleanup({
  ...
  removeDataExports = async () => ({ ok: true, removed: 0 }),
  ...
  // DataExportJob is deleted by the User FK cascade. Purge its sensitive ZIPs
  // first, while the owner-scoped filesystem pointers are still available.
  const exportPurge = await removeDataExports(targetUserId)
  if (!exportPurge?.ok) {
    failures.push({ stage: "data-export", resourceType: "DataExportJob", resourceId: targetUserId })
  } else {
    counts.dataExportsRemoved = Number(exportPurge?.removed || 0)
  }
