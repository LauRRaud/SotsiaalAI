...
import { removeDataExportsForAccountDeletion } from "@/lib/dataExport/service"
import {
...
async function performUserPrivacyCleanup({
  ...
  return runUserDeletionCleanup({
    ...
    deleteVerificationLinkDispatches: email => prisma.verificationLinkDispatch.deleteMany({
      where: {
        identifier: { in: [email, `email-verify:${email}`, `password-reset:${email}`] }
      }
    }),
    ...
    removeDataExports: userId => removeDataExportsForAccountDeletion(userId, { db: prisma }),
