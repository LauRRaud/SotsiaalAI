...
import { RAG_AVAILABLE, createRagRetiredError } from "../rag/retired.js";
...
export function createDeletionJobRetryService({
  ...
  return async function retry({ jobId, actorUserId, ipAddress = null, userAgent = null } = {}) {
    ...
    // Preserve both the deletion reference and retry budget until the old
    // storage has been explicitly decommissioned or a deletion provider exists.
    if (!RAG_AVAILABLE && ["RAG_DELETE", "RAG_INGEST"].includes(job.action)) {
    ...
    return db.$transaction(async tx => {
      ...
      /* SOL-DOC-J-03 — dokumendi loaeemalduse töö kannab oma seisu ka
         UserDocument.metadata-s. Seostame jobId järgi, et vana retry ei saaks
         üle kirjutada hilisema eemaldustsükli värskemat seisu. */
      if (
        job.action === "RAG_DELETE"
        && job.resourceType === "UserDocument"
        && job.resourceId
        && job.externalRef
      ) {
        const document = await tx.userDocument.findUnique({ where: { id: job.resourceId } });
        if (
          document
          && !document.agentAllowed
          && document.metadata?.ragRemoval?.jobId === job.id
        ) {
            ...
            data: {
              metadata: {
                ...
                ragRemoval: {
                  ...
                  reason: status === "done" ? "durable_deletion_job" : "retry_failed",
                  checkedAt: new Date().toISOString()
                }
              }
            }
