...
export function resolveRagProxyAction(methodInput, segmentsInput) {
  ...
  if (readMethod && segments.length === 2 && segments[0] === "lexical-index" && segments[1] === "status") {
    return action("lexical_index_status", method, segments, RAG_ADMIN_CAPABILITY.KNOWLEDGE_STEWARD);
  }
  if (method === "POST" && segments.length === 2 && segments[0] === "lexical-index" && segments[1] === "rebuild") {
    return action("lexical_index_rebuild", method, segments, RAG_ADMIN_CAPABILITY.PLATFORM_ADMIN);
  }
