...
function buildBackfilledMetadata(sourceInput = {}, context = {}, options = {}) {
  ...
  const sourceStatusSignal = String(firstValue(
    source.source_status,
    source.sourceStatus,
    source.content_status,
    source.contentStatus,
  ...
  const currentVersionSignal = source.is_current_version ?? source.isCurrentVersion;
  const inferredHistorical = currentVersionSignal === false || [
    "archived",
    "inactive",
    "stale",
    "superseded"
  ].includes(sourceStatusSignal);
