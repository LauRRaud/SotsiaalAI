const HIGH_RISK_PATTERNS = [
  ...
  /\b(summa|(?:määr|maar)(?:a|ad|ade|adel|adelt|adele|ades|adest|adega|asid|al|alt|ale|as|ast|aga|aks|ani|amine|amise|amisel|amises|amisest|amiseks)?|euro|eur|tähtaeg|tahtaeg|kuupäev|kuupaev|kehtib|kehtiv|abikõlblik|abikolblik)\b/iu,
...
export function classifyRagRisk(message = "", options = {}) {
  ...
  const semanticTerms = [
    ...(Array.isArray(options?.semanticContract?.terms?.lemmas)
      ? options.semanticContract.terms.lemmas
      : []),
    ...(Array.isArray(options?.semanticContract?.terms?.compound_roots)
      ? options.semanticContract.terms.compound_roots
      : [])
  ].map(value => String(value || "").trim()).filter(Boolean);
  const linguisticRiskText = [text, ...semanticTerms].join(" ").trim();
  const highReasons = matchingReasons(linguisticRiskText, HIGH_RISK_PATTERNS);
  const mediumReasons = matchingReasons(linguisticRiskText, MEDIUM_RISK_PATTERNS);
  const lowReasons = matchingReasons(linguisticRiskText, LOW_RISK_PATTERNS);
  const sourceBoundedDescription =
    options?.semanticContract?.risk_scope === "source_bounded_description";

  let riskLevel = "low";
  let reasons = lowReasons.length ? lowReasons : ["general_explanation"];

  if (options?.isCrisis || (highReasons.length && !sourceBoundedDescription)) {
    riskLevel = "high";
    reasons = options?.isCrisis ? ["crisis", ...highReasons] : highReasons;
  } else if (mediumReasons.length) {
    riskLevel = "medium";
    reasons = mediumReasons;
  } else if (sourceBoundedDescription && highReasons.length) {
    riskLevel = "low";
    reasons = ["source_bounded_description"];
  }
  ...
  return {
    ...
    semanticTermsUsed: semanticTerms.length > 0,
