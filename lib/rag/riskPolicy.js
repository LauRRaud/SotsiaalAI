...
export function classifyRagRisk(_message = "", options = {}) {
  ...
  } else if (intent === "professional_method_guidance") {
    riskLevel = "medium";
    stakes = "actionable";
    evidenceScope = "current_authoritative_guidance";
    reasons = ["structured_professional_method_guidance"];
  ...
  return {
    ...
    preferredSourceTypes: evidenceScope === "current_authoritative_guidance"
      ? METHOD_GUIDANCE_SOURCE_TYPES
      : policy.preferredSourceTypes,
...
export function inferSourceEvidenceStrength(source = {}, policy = null) {
  ...
  if (
    policy?.evidenceScope === "current_authoritative_guidance" &&
    status === "active" &&
    !historical &&
    ["official_guideline", "quality_guideline", "service_standard"].includes(sourceType)
  ) {
    return {
      strength: "strong",
      reason: "active_official_method_guidance"
    };
  }
