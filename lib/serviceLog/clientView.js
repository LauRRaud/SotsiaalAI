...
export async function confirmClientMonth(
  ...
  /* KINNITADA SAAB AINULT SEDA, MIDA SAI NÄHA. Kui kuus on rohkem ridu kui
     vaade kannab, ei ole kliendil olnud võimalust neid üle vaadata — ja
     kinnitus on pöördumatu. */
  const totalCount = await db.serviceEntry.count({
    where: { clientUserId: userId, status: ENTRY_STATUS.FINAL, date: { gte: from, lt: to } }
  });
  if (totalCount > CLIENT_VIEW_LIMIT) throw badRequest("service_log.errors.client_month_too_large");
