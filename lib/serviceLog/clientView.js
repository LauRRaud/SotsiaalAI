...
/** Üks kanooniline DB-projektsioon nii kliendivaatele kui kliendi andmekoopiale. */
export const SERVICE_LOG_CLIENT_ENTRY_SELECT = Object.freeze({
  id: true,
  date: true,
  unit: true,
  quantity: true,
...
export function serializeServiceLogClientEntry(row) {
  return {
    id: row.id,
...
export async function readClientMonth(
  ...
  const rows = await db.serviceEntry.findMany({
    ...
    select: SERVICE_LOG_CLIENT_ENTRY_SELECT,
    orderBy: { date: "asc" },
  ...
  const entries = rows.map(serializeServiceLogClientEntry);
  const totals = {};
