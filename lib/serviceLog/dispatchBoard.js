...
export async function resolveBoardScope(userId, organizationId, { db = prisma, now = new Date() } = {}) {
  ...
  if (!membership?.capabilityGrants?.length) return { allowed: false, unitIds: [], wholeOrg: false };

  const wholeOrg = membership.capabilityGrants.some((grant) => grant.scopeType === "ORGANIZATION");
  const unitIds = [
    ...new Set(
      membership.capabilityGrants
        .filter((grant) => grant.scopeType === "UNIT" && grant.scopeUnitId)
        .map((grant) => grant.scopeUnitId)
    )
  ];
  return { allowed: wholeOrg || unitIds.length > 0, unitIds, wholeOrg };
}
...
export async function getDispatchBoard(
  ...
  const visits = routes.length
    ? await db.serviceVisit.findMany({
        where: { routeId: { in: routes.map((row) => row.id) } },
