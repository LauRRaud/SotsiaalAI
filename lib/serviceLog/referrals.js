...
export async function listReferrals(
  ...
  const referrals = await findAllById(db.serviceReferral, {
    where: {
      providerProfileId: profile.id,
      ...(status ? { status } : {}),
...
export async function updateReferral(userId, referralId, patch = {}, { db = prisma, env = process.env } = {}) {
  ...
  /* SAAJA JA OTSUSE NUMBER ON SAMAMOODI LUKUS.
     Eksport kasutab suunamise PRAEGUST `kovName`-i, seega nime muutmine
     suunaks juba esitatud arveread teisele saajale — tagantjärele ja vaikselt.
     Otsuse number on sama: tema järgi viib KOV rea oma otsusega kokku.
     Muutmiseks lõpetatakse vana suunamine ja luuakse uus, täpselt nagu KOV
     teeb uue otsuse. */
  const nextKov = text(patch.kovName, MAX_CLIENT_NAME_LENGTH);
  const wantsKovChange = patch.kovName !== undefined && nextKov && nextKov !== existing.kovName;
  const nextNumber = text(patch.referralNumber, MAX_EXTERNAL_REF_LENGTH);
  const wantsNumberChange =
    patch.referralNumber !== undefined && nextNumber !== (existing.referralNumber || null);

  if (wantsUnitChange || wantsPeriodKindChange || wantsKovChange || wantsNumberChange) {
    const usedCount = await tx.serviceEntry.count({ where: { referralId: existing.id } });
  ...
  /* PERIOODI KITSENDAMINE EI TOHI OLEMASOLEVAID KIRJEID VÄLJA JÄTTA.
     Loomisel kontrollitakse, et kirje kuupäev on suunamise perioodis. Kui
     perioodi hiljem kitsendada, jääksid juba tehtud kirjed TAGANTJÄRELE otsuse
     kehtivusalast välja: nad on endiselt saldos ja ekspordis, aga ei vasta enam
     otsusele, mille alusel nad tehti. Vaikne vastuolu, mis avastatakse KOV-i
     kontrollis. Perioodi tohib LAIENDADA vabalt — see ei riku midagi. */
  const periodChanged =
    (patch.periodStart !== undefined && String(periodStart) !== String(existing.periodStart)) ||
    (patch.periodEnd !== undefined && String(periodEnd) !== String(existing.periodEnd));

  if (periodChanged) {
    const orphaned = await tx.serviceEntry.count({
      where: {
        referralId: existing.id,
        OR: [
          ...(periodStart ? [{ date: { lt: periodStart } }] : []),
          ...(periodEnd ? [{ date: { gt: periodEnd } }] : [])
        ]
      }
    });
    if (orphaned > 0) {
      throw conflict("service_log.errors.period_excludes_entries", { entries: orphaned });
    }
  }

  const row = await tx.serviceReferral.update({
