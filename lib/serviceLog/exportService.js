...
export async function buildServiceLogExport(
  ...
  const provider = {
    name: profileRow?.organizationName || "",
    registryCode: profileRow?.registryCode || ""
  };
  const periodLabels = {
  ...
  /* Osutaja ja periood tulevad KAASA: STAR-kuju (E9) vajab neid ümbrikus, aga
     malliehitaja paneb nad dokumendi päisereale tekstina — sealt tagasi
     parsimine oleks vaikne viga ootamas. */
  return { document, month, template, provider, period: periodLabels };
}
...
export function exportFileName({ month, template, kovName, extension = "csv" }) {
  ...
  /* „star" on VORMINGU nimi, mitte failitüüp: fail on JSON ja peab seda ka
     nimes ütlema, muidu ei ava teda ükski tööriist õigesti. */
  const requested = String(extension) === "star" ? "json" : String(extension);
  const safeExtension = /^[a-z0-9]{2,5}$/.test(requested) ? requested : "csv";
