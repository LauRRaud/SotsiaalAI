...
export function sanitizeLocationStamps(input) {
  ...
  for (const key of VISIT_STAMP_ORDER) {
    ...
    /* KASUTU TÄPSUS EI OLE ASUKOHT. Brauser filtreerib ta juba ära, aga see
       ...
       mitte see, mida meie kood saatis. Kümne kilomeetri täpsusega punkt näeks
       andmebaasis välja täpselt nagu tõend ja teda hakataks kaardil vaatama. */
    if (Number.isFinite(acc) && acc > LOCATION_MAX_USEFUL_ACCURACY_M) continue;
