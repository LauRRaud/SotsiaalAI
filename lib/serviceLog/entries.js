  ...
  try {
    return await withLockedReferral(
      ...
      async (tx, lockedReferral) => {
        ...
        return tx.serviceEntry.create(args);
      }
    );
  } catch (error) {
...
export async function createEntry(userId, input = {}, { db = prisma, env = process.env } = {}) {
  ...
  /* Mitme aktiivse suunamise korral ei ole `null` neutraalne valik: selline
     kirje jääks KOV-i ekspordist ja suunamise saldost välja. UI küsib valikut,
     kuid sama invariant peab kehtima ka API ja võrguta järjekorra jaoks. */
  if (!referralId) {
    const ambiguousReferrals = await db.serviceReferral.findMany({
      where: {
    ...
    if (ambiguousReferrals.length > 1) {
      throw badRequest("service_log.errors.referral_required");
    }
  }
...
export async function updateEntry(userId, entryId, patch = {}, { db = prisma, env = process.env } = {}) {
  ...
  if (existing.referralId) {
    ...
    assertReferralIntegrity(referral, {
      ...
      unit,
      date: nextDate,
      // Ainult auditeeritud FINAL-kirje parandus ei ole uus maht. Mustand on
      // arvestatav alles kinnitamisel ja peab seetõttu jääma aktiivse suunamise alla.
      requireActive: existing.status !== ENTRY_STATUS.FINAL
    });
  }
