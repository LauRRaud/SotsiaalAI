...
import {
  ...
  isServiceLogMeasurementEnabled
} from "./flags.js";
import { badRequest, conflict, notFound } from "./errors.js";
import { LOCATION_MAX_USEFUL_ACCURACY_M } from "./geolocation.js";
import {
  ENTRY_STATUS,
...
import { withLockedReferral } from "./referralLock.js";
import { SAMPLE_KIND, normalizeSample } from "./measurement.js";
...
function normalizeActivities(input, catalog) {
  ...
  for (const item of input) {
    if (typeof item !== "string") {
      throw badRequest("service_log.errors.activity_not_allowed");
    }
    const value = item.trim();
    if (!value || seen.has(value)) continue;
    /* TÜHI kataloog ei ole vabatekstiluba. Kui teenusel pole loendit või
       teenust ennast pole valitud, on ainus lubatud sisend tühi massiiv. */
...
function parseExpectedUpdatedAt(value) {
  ...
  if (!parsed) throw badRequest("service_log.errors.version_required");
  return parsed;
}

function versionConflict(row) {
  return conflict("service_log.errors.version_conflict", {
...
/**
 * ÜKS TINGIMUSLIK KIRJUTUS on kohtunik. Eellugemine on vajalik inimesele
 * arusaadava vea ja parandusjälje koostamiseks, kuid ta ei anna kirjutusõigust:
 * selle annab ainult sama `updatedAt` versiooni edukas CAS.
 */
async function casEntryMutation(
  db,
  { existing, data, where = {}, correction = null }
) {
  return runTransaction(db, async (tx) => {
    const changed = await tx.serviceEntry.updateMany({
      where: {
        id: existing.id,
    ...
    if (changed.count !== 1) {
      const fresh = await tx.serviceEntry.findFirst({
        where: { id: existing.id, providerProfileId: existing.providerProfileId }
      });
    ...
    if (correction) {
      await tx.serviceEntryCorrection.create({ data: correction });
    }

    const row = await tx.serviceEntry.findFirst({
      where: { id: existing.id, providerProfileId: existing.providerProfileId }
    });
 ...
 */
/**
 * ÜKS PUNKT, ILMA TEMPLISÕNASTIKUTA.
 *
 * `sanitizeLocationStamps` käib läbi `VISIT_STAMP_ORDER` — teenuskirje neli
 * templit. Päevateekonnal on OMA sõnastik (`enRouteAt`, `arrivedAt`) ja tema
 * võtmed ei kattu: `enRouteAt` kukkus sanitaatorist VAIKSELT välja ja sõidu
 * alguse punkt ei salvestunud kunagi. Vastus oli 200, andmebaasis tühjus.
 *
 * Punkti kontroll ei sõltu sellest, mis on tema nimi. Nimede loend jääb
 * `sanitizeLocationStamps`-i, valideerimine tuleb siia.
 */
export function sanitizeLocationPoint(point) {
  ...
  /* KASUTU TÄPSUS EI OLE ASUKOHT. Brauser filtreerib ta juba ära, aga see
     filter on kliendi poolel ja kliendi poolt tuleb see, mida keegi saadab. */
  if (Number.isFinite(acc) && acc > LOCATION_MAX_USEFUL_ACCURACY_M) return null;
...
export function sanitizeLocationStamps(input) {
  ...
  for (const key of VISIT_STAMP_ORDER) {
    const entry = sanitizeLocationPoint(input[key]);
...
function assertReferralIntegrity(referral, { client, serviceId, unit, date, requireActive = true }) {
  ...
  const sameClient = client.clientUserId
    ...
    : Boolean(client.clientDisplayName) &&
      text(referral.clientDisplayName, MAX_CLIENT_NAME_LENGTH) === client.clientDisplayName &&
      text(referral.clientExternalRef, MAX_EXTERNAL_REF_LENGTH) === client.clientExternalRef;
  ...
  if (requireActive && referral.status !== REFERRAL_STATUS.ACTIVE) {
    throw conflict("service_log.errors.referral_not_active");
  }
...
function requestFingerprint({
  ...
  return entryRequestHash({
    ...
    activities: fingerprintActivities(input.activities),
    moneyAmount,
...
function storedEntryFingerprint(row) {
  return entryRequestHash({
    clientUserId: row.clientUserId || null,
    clientDisplayName: row.clientDisplayName || null,
    clientExternalRef: row.clientExternalRef || null,
    date: row.date?.toISOString?.().slice(0, 10) || row.date || null,
    departedForVisitAt: row.departedForVisitAt || null,
    arrivedAt: row.arrivedAt || null,
    leftAt: row.leftAt || null,
    returnedAt: row.returnedAt || null,
    locationStamps: row.locationStamps || null,
    unit: row.unit,
    quantity: String(row.quantity),
    activities: row.activities || [],
    moneyAmount: row.moneyAmount === null || row.moneyAmount === undefined ? null : Number(row.moneyAmount),
    moneyNote: row.moneyNote || null,
    workerName: row.workerName || null,
    note: row.note || null,
    noteProvenance: row.noteProvenance || null,
...
function assertMatchingReplay(row, requestHash) {
  const storedHash = row.clientRequestHash || storedEntryFingerprint(row);
  if (storedHash !== requestHash) {
    throw conflict("service_log.errors.idempotency_payload_mismatch");
  }
}
 ...
 */
async function resolveEntryCreateConflict(db, args, error) {
  if (error?.code !== "P2002") throw error;
  const { providerProfileId, clientRequestId, clientRequestHash, sourceFieldVisitId } = args.data;
  ...
  if (clientRequestId) {
    ...
    if (existing) {
      assertMatchingReplay(existing, clientRequestHash);
...
async function createEntryRow(db, args) {
  try {
    return { row: await db.serviceEntry.create(args), replayed: false };
  } catch (error) {
    return resolveEntryCreateConflict(db, args, error);
  }
}

async function createEntryForReferral(db, args, { profile, referralId, integrity }) {
  try {
    return await withLockedReferral(
      db,
      { referralId, providerProfileId: profile.id },
      async (tx, lockedReferral) => {
        assertReferralIntegrity(lockedReferral, integrity);
        return { row: await tx.serviceEntry.create(args), replayed: false };
      }
    );
  } catch (error) {
    /* P2002 järel on tehing tagasi pööratud; alles nüüd tohib sama võtit
       järelpärida. Tehingu sees tehtud järelpäring annaks PostgreSQL 25P02. */
    return resolveEntryCreateConflict(db, args, error);
  }
}
...
export async function createEntry(userId, input = {}, { db = prisma, env = process.env } = {}) {
  ...
  /* Replay otsustatakse alles KANONISEERITUD sisu pealt. Sama võti ei ole
     luba teise kliendi või teise töö vana rea taha peitmiseks. */
  const replayed = await findByClientRequestId(db, profile.id, clientRequestId);
  if (replayed) {
    assertMatchingReplay(replayed, clientRequestHash);
    return { ...serializeEntry(replayed), overrun: null, replayed: true };
  }

  if (sourceFieldVisitId) {
    const sourceVisit = await db.serviceVisit.findFirst({
      where: {
        id: sourceFieldVisitId,
        providerProfileId: profile.id,
        ownerUserId: userId
      }
    });
    if (!sourceVisit) throw notFound();
    if (sourceVisit.status !== "COMPLETED") {
      throw conflict("service_log.errors.visit_not_completed");
    }
    if (sourceVisit.serviceEntryId) {
      throw conflict("service_log.errors.visit_already_used");
    }

    const sameClient = client.clientUserId
      ? sourceVisit.clientUserId === client.clientUserId
      : text(sourceVisit.clientDisplayName, MAX_CLIENT_NAME_LENGTH) === client.clientDisplayName &&
        text(sourceVisit.clientExternalRef, MAX_EXTERNAL_REF_LENGTH) === client.clientExternalRef;
    const sourceDate = toCalendarDate(sourceVisit.arrivedAt || sourceVisit.completedAt);
    const sameDate = sourceDate?.getTime?.() === date.getTime();
    const sameReferral = (sourceVisit.referralId || null) === referralId;
    const sameService = (sourceVisit.serviceId || null) === serviceId;
    const sameArrived = !stamps.arrivedAt || stamps.arrivedAt.getTime() === sourceVisit.arrivedAt?.getTime?.();
    const sameLeft = !stamps.leftAt || stamps.leftAt.getTime() === sourceVisit.completedAt?.getTime?.();
    if (!sameClient || !sameDate || !sameReferral || !sameService || !sameArrived || !sameLeft) {
      throw badRequest("service_log.errors.source_visit_mismatch");
    }
  }
  if (referralId) {
  ...
  const created = referralId
    ? await createEntryForReferral(db, createArgs, {
        profile,
        referralId,
        integrity: { client, serviceId, unit, date }
      })
    : await createEntryRow(db, createArgs);

  /* The duration is client-observed, but it becomes baseline evidence only
     here: after one real entry was created. Idempotent request replays cannot
     mint additional samples, and measurement failure never rolls back work. */
  if (!created.replayed && isServiceLogMeasurementEnabled(env)) {
    const sample = normalizeSample({
      kind: SAMPLE_KIND.ENTRY_INPUT,
      seconds: input.entryInputSeconds
    });
    if (sample) {
      try {
        ...
        // A measurement must never turn a successfully saved entry into an error.
      }
    }
  }
  ...
  return {
    ...serializeEntry(created.row),
    overrun: overrun?.warn ? overrun : null,
...
export async function updateEntry(userId, entryId, patch = {}, { db = prisma, env = process.env } = {}) {
  ...
  const expectedUpdatedAt = parseExpectedUpdatedAt(patch.expectedUpdatedAt);
  if (existing.updatedAt?.getTime?.() !== expectedUpdatedAt.getTime()) {
    throw versionConflict(existing);
  }
  if (patch.confirmedManually !== undefined) {
    throw badRequest("service_log.errors.manual_confirmation_lifecycle_only");
  }
  ...
  if (existing.referralId) {
    ...
    assertReferralIntegrity(referral, {
      client: {
        ...
        clientExternalRef: existing.clientExternalRef
      },
  ...
  if (existing.status === ENTRY_STATUS.FINAL) {
    ...
    if (!reason) throw badRequest("service_log.errors.reason_required");

    const row = await casEntryMutation(db, {
      existing,
      data,
      where: { status: ENTRY_STATUS.FINAL },
      correction: {
        entryId: existing.id,
        actorUserId: userId,
        reason,
...
/**
 * Välise kliendi paberkinnituse rada. Seda ei saa teha üld-PATCH-iga:
 * platvormiklient kinnitab ise kliendivaates ning iga pabermärke muutus jätab
 * parandustabelisse tegija, aja, vana väärtuse ja toimingu liigi.
 */
export async function setManualConfirmation(
  userId,
  entryId,
  { confirmed, db = prisma, env = process.env } = {}
) {
  assertServiceLogEnabled(env);
  const profile = await requireWritableProfile(userId, { db });
  const existing = await db.serviceEntry.findFirst({
  ...
  if (existing.clientUserId) {
    throw conflict("service_log.errors.manual_confirmation_external_only");
  }
  if (existing.status !== ENTRY_STATUS.FINAL) {
    throw conflict("service_log.errors.manual_confirmation_final_only");
  }

  const next = parseBoolean(confirmed);
  if (Boolean(existing.confirmedManually) === next) return serializeEntry(existing);

  const row = await casEntryMutation(db, {
    existing,
    data: { confirmedManually: next },
    where: { status: ENTRY_STATUS.FINAL, clientUserId: null },
    correction: {
      entryId: existing.id,
      actorUserId: userId,
      reason: next ? "manual_confirmation_recorded" : "manual_confirmation_removed",
      previousValues: { confirmedManually: Boolean(existing.confirmedManually) },
      changedFields: ["confirmedManually"]
    }
  });
  return serializeEntry(row);
}
...
export async function finalizeEntry(userId, entryId, { db = prisma, env = process.env, now = new Date() } = {}) {
  ...
  const existing = await db.serviceEntry.findFirst({
    where: { id: entryId, providerProfileId: profile.id },
    select: { id: true, providerProfileId: true, status: true, updatedAt: true }
  });
  if (!existing) throw notFound();
  ...
  const row = await casEntryMutation(db, {
    existing: { ...existing, providerProfileId: profile.id },
    where: { status: ENTRY_STATUS.DRAFT },
    data: {
...
export async function voidEntry(userId, entryId, { reason, db = prisma, env = process.env, now = new Date() } = {}) {
  ...
  const row = await casEntryMutation(db, {
    ...
    data: {
      status: ENTRY_STATUS.VOID,
      voidedAt: now,
      voidReason: cleanReason,
      ...(existing.status === ENTRY_STATUS.DRAFT
        ? { finalizedAt: null, recordedFiscalYear: null }
        : {})
    },
    correction: {
      entryId: existing.id,
      ...
      changedFields: ["status", "voidedAt", "voidReason"]
    }
  });
  return serializeEntry(row);
}
