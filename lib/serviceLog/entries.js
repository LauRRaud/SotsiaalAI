...
function normalizeStamps(input, { locationStampEnabled, existingLocationStamps = null }) {
  ...
  const locationStamps = locationStampEnabled
    ? sanitizeLocationStamps(input.locationStamps) ?? existingLocationStamps
 ...
 */
function assertReferralIntegrity(referral, { client, serviceId, unit, date }) {
  ...
  // 2. AKTIIVNE. Lõppenud suunamise alla ei tohi uut mahtu kirjutada.
