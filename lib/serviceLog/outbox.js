...
export function outboxItemState(item) {
  return item?.[OUTBOX_META]?.state === OUTBOX_STATE.NEEDS_ATTENTION
    ? OUTBOX_STATE.NEEDS_ATTENTION
    : OUTBOX_STATE.PENDING;
}

/** Serverile ei saadeta seadme järjekorra olekut ega veapõhjust. */
export function outboxPayload(item) {
  if (!item || typeof item !== "object") return null;
  const { [OUTBOX_META]: _meta, ...payload } = item;
  return payload;
}

export function attentionItems(store) {
  return readOutbox(store).filter(
    (item) => outboxItemState(item) === OUTBOX_STATE.NEEDS_ATTENTION
  );
}
 ...
 */
export function enqueueResult(store, item) {
  if (!item || typeof item.clientRequestId !== "string" || !item.clientRequestId) {
    return { ok: false, reason: "invalid", items: readOutbox(store) };
  }
  ...
  /* TÄIS TÄHENDAB BLOKKI, MITTE ANDMEKADU. Uus töö jääb vormile ning kasutaja
     näeb, et enne tuleb ühendus taastada või vigane rida parandada. Sama võtme
     uuendamine on lubatud, sest see ei kasvata järjekorda. */
  if (existingIndex < 0 && current.length >= OUTBOX_LIMIT) {
  ...
  const written = writeOutbox(store, next);
  const saved = written.some((queued) => queued.clientRequestId === item.clientRequestId);
  return { ok: saved, reason: saved ? null : "storage", items: written };
}

export function enqueue(store, item) {
  return enqueueResult(store, item).items;
}

/**
 * Parandatav 4xx jääb püsivalt seadmesse koos põhjusega. Reload ei tohi teda
 * muuta uuesti saatmise lõputuks tsükliks ega tehtud tööd ära kaotada.
 */
export function markNeedsAttention(store, clientRequestId, { status = 0, message = null } = {}) {
  const next = readOutbox(store).map((item) =>
    item.clientRequestId === clientRequestId
      ? {
          ...outboxPayload(item),
          [OUTBOX_META]: {
            state: OUTBOX_STATE.NEEDS_ATTENTION,
...
export function shouldRetry({ networkError = false, status = 0 } = {}) {
  ...
  /* Sessioon/õigus võib võrgu taastumise ajal olla värskendamisel; 408/425/429
     on samuti ajutine serverileping. Ükski neist ei tohi tehtud tööd kustutada. */
  if ([401, 403, 408, 425, 429].includes(Number(status))) return true;
