...
function clientRecord(type, row) {
  return {
    type,
    data: projectDates(without(row, ["providerProfileId", "ownerUserId", "clientUserId"]))
  };
}
...
export async function collectServiceLogDataExport({ db, userId }) {
  ...
  const [clientReferrals, clientEntries, clientNarratives] = await Promise.all([
    db.serviceReferral.findMany({ where: { clientUserId: userId }, orderBy: { createdAt: "asc" } }),
    db.serviceEntry.findMany({ where: { clientUserId: userId }, orderBy: { createdAt: "asc" } }),
    db.serviceMonthlyNarrative.findMany({ where: { clientUserId: userId }, orderBy: { createdAt: "asc" } })
  ]);
  const client = [
    ...clientReferrals.map((row) => clientRecord("referral", row)),
    ...clientEntries.map((row) => clientRecord("entry", row)),
    ...clientNarratives.map((row) => clientRecord("narrative", row))
  ];
