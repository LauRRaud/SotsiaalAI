...
import {
  SERVICE_LOG_CLIENT_ENTRY_SELECT,
  serializeServiceLogClientEntry
} from "./clientView.js";
...
function clientRecord(row) {
  return {
    type: "entry",
    data: serializeServiceLogClientEntry(row)
  };
}
...
export async function collectServiceLogDataExport({ db, userId }) {
  ...
  const clientEntries = await db.serviceEntry.findMany({
    where: { clientUserId: userId, status: ENTRY_STATUS.FINAL },
    orderBy: { createdAt: "asc" },
