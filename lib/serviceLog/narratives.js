...
export async function upsertNarrative(userId, input = {}, { db = prisma, env = process.env } = {}) {
  ...
  if (count === 0) {
    /* Kaotaja (või kordussalvestus) uuendab olemasolevat rida — see ongi
       upsert'i mõte: kirjutaja naaseb teksti juurde mitu korda. */
    const existing = await db.serviceMonthlyNarrative.findFirst({ where, select: { id: true } });
    if (!existing) throw conflict("service_log.errors.narrative_conflict");
    return serializeNarrative(
