...
export const EXPORT_FORMAT = Object.freeze({
  ...
  DOCX: "docx",
  PDF: "pdf",
  /* E9: riigi statistika ümbrik. Mitte inimesele lugemiseks, vaid masinale
     edastamiseks — seepärast JSON, mitte tabel. */
  STAR: "star"
});
...
export const FORMAT_MIME = Object.freeze({
  ...
  [EXPORT_FORMAT.STAR]: "application/json; charset=utf-8"
});
...
const WARNING_LABEL = Object.freeze({
  not_submittable_all_recipients:
    "Fail sisaldab mitme saaja andmeid ja ei sobi KOV-ile esitamiseks.",
  voided_excluded: "Tühistatud kirjed jäid välja.",
  drafts_excluded: "Mustandid jäid välja.",
  drafts_included: "Fail sisaldab mustandeid.",
  empty_export: "Eksport ei sisalda kirjeid.",
  narrative_missing: "Sisuline kokkuvõte puudub.",
  goals_missing: "Suunamise eesmärgid puuduvad."
});
...
function warningText(warning) {
  if (!warning || typeof warning !== "object" || Array.isArray(warning)) return cellText(warning);
  const label = WARNING_LABEL[warning.code] || cellText(warning.code);
  return Number.isFinite(warning.count) ? `${label} (${warning.count})` : label;
}
...
export function buildRenderPlan(document, { generatedAt = null } = {}) {
  ...
  return {
    ...
    totals,
    warnings: Array.isArray(document.warnings) ? document.warnings.map(warningText) : []
  };
}
