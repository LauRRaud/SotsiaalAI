...
/**
 * KLIENDI IDENTITEET, MITTE KUJU.
 *
 * Grupeerimine kuvanime järgi liidab kaks ERI inimest, kellel juhtub olema sama
 * nimi — kontrollproov andis kahe eri „Mari" peale `totalClients = 1` ja ühe
 * liidetud rea. Aruandes tähendab see, et üks inimene saab teise tunnid.
 * Platvormi kasutaja ja väline nimi on eri rajad ka siin, nagu mujal moodulis.
 */
function clientIdentity(entry) {
  if (entry?.clientUserId) return `user:${entry.clientUserId}`;
  if (entry?.clientDisplayName) {
    /* VÄLISKLIENDI IDENTITEET EI OLE AINULT NIMI. Kaks eri „Mari" eri
       suunamisotsuse viidetega on kaks eri inimest; ainult nime järgi
       grupeerides sai üks neist teise tunnid. `clientExternalRef` on täpselt
       see väli, mis nad eristab — tema jaoks ta mudelis ongi. */
    return `name:${entry.clientDisplayName}|${entry.clientExternalRef || ""}`;
  }
  return "unknown";
}
...
function sumsByClientAndService(entries) {
  ...
  for (const entry of entries) {
    /* SUUNAMISOTSUS ON OSA VÕTMEST. Sama klient võib saada sama teenust KAHE
       eri otsuse alusel (nt maht muutus keset kuud) — ilma otsuseta võtmes
       liideti nad kokku ja arve alusdokumenti läks VALE otsuse number. */
    const key = `${clientIdentity(entry)}|${entry.referralId || ""}|${entry.serviceName || entry.serviceId || ""}|${entry.unit}`;
...
export function buildTimesheet({
  ...
  if (variant === TIMESHEET_VARIANT.MONTHLY) {
    ...
    return {
      ...
      rows: summed.map((row) => ({
        client: row.client,
        // Suunamisotsuse number tuleb KAASA ka kuuvariandis: ilma temata ei
        // saa KOV rida oma otsusega kokku viia.
      ...
      warnings: buildWarnings(entries, rows, includeDrafts, recipient)
    };
  }
...
export function buildCareDiary({
  ...
  // `duration` ilma ühikuta on kahemõtteline: 1 võib olla tund, kord või ööpäev.
  const columns = ["client", "date", "arrived", "left", "duration", "unit", "activities"];
  ...
  return {
    ...
    sections: [
      { key: "goals", text: referral?.goalsText || "" },
      {
        ...
        totals: totalsByUnit(rows),
        entryCount: rows.length,
        // Mall C ptk 6a nõuab „perioodi tegevusi kokkuvõtvalt" — mitte ainult
        // mahte. Ilma tegevusteta on „kokkuvõte" tunniarv ja mitte midagi muud.
    ...
    warnings: [
      ...buildWarnings(entries, rows, includeDrafts, recipient),
 ...
 */
export function buildStatistics({ period = {}, entries = [], includeDrafts = false, recipient = {} } = {}) {
  const rows = selectExportableEntries(entries, { includeDrafts });

  const clients = new Set();
  const byService = new Map();
  for (const entry of rows) {
    const identity = clientIdentity(entry);
    if (identity !== "unknown") clients.add(identity);
    ...
    row.quantity += Number(entry.quantity) || 0;
    if (identity !== "unknown") row.clients.add(identity);
 ...
 * numbri.
 */
function buildWarnings(allEntries, exportedEntries, includeDrafts, recipient = {}) {
  const warnings = [];
  /* P0: saajata eksport EI OLE KOV-ile esitatav fail. Ilma selle hoiatuseta
     näeks osutaja korrektse välimusega dokumenti, milles on mitme KOV-i
     kliendid korraga — ja saadaks ta ühele neist. */
  if (recipient && recipient.isSingleRecipient === false) {
    warnings.push({ code: "not_submittable_all_recipients" });
  }
