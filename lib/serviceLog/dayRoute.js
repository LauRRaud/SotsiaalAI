...
export async function transitionVisit(
  ...
  if (action === "attach_location") {
    ...
    const cleaned = sanitizeLocationStamps({ [stampKey]: locationPoint });
    if (!cleaned?.[stampKey]) return serializeVisit(visit);

    const data = { locationStamps: { ...(visit.locationStamps || {}), [stampKey]: cleaned[stampKey] } };
