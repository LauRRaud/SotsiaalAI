...
import { resolveSelfAssignedOrganizationId } from "./visitOrigin.js";
import { isRoutingEnabled, routeBetween, routeDay } from "./routing.js";
...
export function serializeVisit(row) {
  ...
  return {
    ...
    /* ETTE ARVUTATUD SÕIT — nähtav siis, kui töötaja on teel. */
    plannedTravelKm: row.plannedTravelKm ?? null,
    plannedTravelMinutes: row.plannedTravelMinutes ?? null,
...
export async function createVisit(
  ...
  const data = {
    ...
    routeId: route.id,
    ownerUserId: userId,
    /* PÄRITOLU KA ENDA LISATUD TÖÖL (SOL-SLOG-17). Ilma selleta kaoks
       koduhooldaja ise lisatud töö oma juhi tahvlilt ära. Ühemõtteline vastus
       on ainult ühe aktiivse liikmesuse korral; kahe puhul jääb `null` ja töö
       ei ilmu KUMMALEGI juhile — vale juht on halvem kui puuduv rida. */
    assignedOrganizationId: await resolveSelfAssignedOrganizationId(userId, { db }),
...
/**
 * Kõik teekonna käigusolekut muutvad toimingud võtavad SAMA rea luku.
 *
 * Rakenduse eelkontroll üksi ei piisa: kaks päringut saavad mõlemad lugeda
 * vana seisu ja alles seejärel kirjutada. PostgreSQL-i realukk teeb sellest
 * ühe järjekorra nii aktiivse külastuse valimisel kui päeva sulgemisel.
 * Fake-DB testides võib tehingukiht puududa; päris invariant on all oleva
 * PostgreSQL-i sondiga eraldi mõõdetud.
 */
async function withLockedRoute(db, routeId, callback) {
...
export async function transitionVisit(
  ...
  if (action === "attach_location") {
    /* TEMPEL SÕLTUB OLEKUST, mitte kutsuja väitest: teel olles on see sõidu
       alguse punkt, kohal olles saabumise oma. Kui kutsuja saaks templi ise
       valida, saaks ta saabumispunkti kirjutada ka siis, kui ta alles sõidab. */
    const stampKey =
      visit.status === VISIT_STATUS.EN_ROUTE
        ? "enRouteAt"
        : visit.status === VISIT_STATUS.ARRIVED
          ? "arrivedAt"
          : null;
    if (!stampKey) throw conflict("service_log.errors.transition_not_allowed");
    if (!locationPoint || !isServiceLogLocationStampEnabled(env)) return serializeVisit(visit);

    /* PUNKTI SANITAATOR, MITTE TEMPLISÕNASTIKU OMA. `sanitizeLocationStamps`
       tunneb ainult teenuskirje nelja templit ja `enRouteAt` kukkus tema seest
       VAIKSELT välja — vastus oli 200 ja andmebaasis tühjus. */
    const cleaned = sanitizeLocationPoint(locationPoint);
    if (!cleaned) return serializeVisit(visit);

    const data = { locationStamps: { ...(visit.locationStamps || {}), [stampKey]: cleaned } };

    /* KAUGUS ETTE. Sõidu alguse punkt on olemas ja sihtaadress on teada —
       just see hetk on ainus, mil arv on töötajale veel kasulik. Pärast
       kohalejõudmist on ta ajalugu.

       PARIMA TAHTE KAUPA: mootori tõrge ei tohi templi salvestamist takistada. */
    if (stampKey === "enRouteAt" && isRoutingEnabled(env) && Number.isFinite(Number(visit.addressLat))) {
      const road = await routeBetween(
        cleaned,
        { lat: Number(visit.addressLat), lng: Number(visit.addressLng) },
        { env }
      );
      if (road) {
        data.plannedTravelKm = road.km;
        data.plannedTravelMinutes = road.minutes;
      }
    }

    const stamped = await db.serviceVisit.update({ where: { id: visit.id }, data });
    return serializeVisit(stamped);
  }

  return withLockedRoute(db, visit.routeId, async (tx) => {
    /* Luku järel loeme mõlemad read uuesti. Eellugemise `visit` on vaid luku
       võtme leidmiseks; otsust ei tohi teha seisu pealt, mis võis ootamise
       ajal muutuda. */
    const current = await tx.serviceVisit.findFirst({
      where: { id: visit.id, providerProfileId: profile.id }
    });
    if (!current) throw notFound();

    const route = await tx.serviceWorkRoute.findFirst({
      where: { id: current.routeId, providerProfileId: profile.id }
    });
    if (!route) throw notFound();
    if (route.status !== ROUTE_STATUS.OPEN) {
      throw conflict("service_log.errors.transition_not_allowed");
    }

    const verdict = evaluateTransition(current, action, { at: stamp, reason });
    ...
    if (
      ...
      if (cleaned?.arrivedAt) {
        data.locationStamps = { ...(current.locationStamps || {}), arrivedAt: cleaned.arrivedAt };
      }
    }

    /* JOOKSVAID KÜLASTUSI ON ÜKS. Kontroll ja kirjutus on nüüd teekonna luku
       all: kaks paralleelset alustamist ei saa enam mõlemad vana seisu näha. */
    if (isActiveVisit(verdict.status)) {
      const other = await tx.serviceVisit.findFirst({
        where: {
          providerProfileId: profile.id,
    ...
    const updated = await tx.serviceVisit.update({ where: { id: current.id }, data });
...
export async function closeRoute(userId, { now = new Date() } = {}, { db = prisma, env = process.env } = {}) {
  ...
  return withLockedRoute(db, route.id, async (tx) => {
    const current = await tx.serviceWorkRoute.findFirst({
      where: {
        id: route.id,
        providerProfileId: profile.id,
        workerUserId: userId
    ...
    if (!current) throw notFound();
    if (current.status !== ROUTE_STATUS.OPEN) {
      throw conflict("service_log.errors.transition_not_allowed");
    }

    const active = await tx.serviceVisit.findFirst({
      where: { routeId: current.id, status: { in: [VISIT_STATUS.EN_ROUTE, VISIT_STATUS.ARRIVED] } },
      select: { id: true }
    });
    if (active) throw conflict("service_log.errors.visit_still_open");

    const breakMinutes = current.breakStartedAt
      ? (current.breakMinutes || 0) + Math.max(0, Math.round((now.getTime() - current.breakStartedAt.getTime()) / 60000))
      : current.breakMinutes || 0;

    const updated = await tx.serviceWorkRoute.update({
      where: { id: current.id },
      data: { status: ROUTE_STATUS.CLOSED, endedAt: now, breakStartedAt: null, breakMinutes }
    });
    const visits = await tx.serviceVisit.findMany({ where: { routeId: current.id } });
...
export async function createEntryFromVisit(
  ...
  if (!visit) throw notFound();
  if (visit.status !== VISIT_STATUS.COMPLETED) throw conflict("service_log.errors.visit_not_completed");
  if (visit.serviceEntryId) {
    const existing = await db.serviceEntry.findFirst({
      where: { id: visit.serviceEntryId, providerProfileId: profile.id }
    });
    if (!existing) throw conflict("service_log.errors.visit_already_billed");
    return { entry: serializeEntry(existing), visit: serializeVisit(visit) };
  }
  ...
  const entry = await createEntry(
    userId,
    {
      ...
      /* Võti tuleb SERVERILT visitId-st. Kutsuja erinevad võtmed ei saa ühest
         külastusest kahte arve alusdokumenti teha. */
      clientRequestId: `visit-entry-${visit.id}`,
