...
export const SERVICE_LOG_FLAG_KEYS = Object.freeze({
  ...
  /**
   * PILOODIMÕÕTMINE (E8). AJUTINE, mitte tootefunktsioon.
   *
   * Omaniku täpsustus 02.08: mõõtmine on piloodi ajaks käivitatav vahend
   * baasjoone võtmiseks, mitte püsiv näidik. Väljas lipuga EI KOGUTA proove
   * üldse ja baasjoont ei kuvata — vaikimisi on ta väljas, seega tavaline
   * kasutaja ei anna aega, mida keegi ei ole küsinud.
   */
  MEASUREMENT: "SERVICE_LOG_MEASUREMENT",
  /** UI-pool mõõtmisele (build-time). Ei ava midagi, ainult peidab. */
  PUBLIC_MEASUREMENT: "NEXT_PUBLIC_SERVICE_LOG_MEASUREMENT",
  ...
  /**
   * PÄEVATEEKOND (E2c). Asendab nelja fikseeritud märke voo olekumasinaga,
   * kus järgmise töö EN_ROUTE→ARRIVED on eelmise lahkumise sõidulõik.
   *
   * ERALDI LIPP, sest ta muudab töötaja jaoks KÕIGE HARJUMUSPÄRASEMAT osa —
   * seda nuppu, mida ta päevas kümme korda vajutab. Väljas lipuga töötab OSA I
   * voog täpselt nagu enne, seega piloodi saab teha ühe meeskonnaga korraga.
   */
  DAY_ROUTE: "SERVICE_LOG_DAY_ROUTE",
  /** UI-pool päevateekonnale (build-time). Ei ava midagi, ainult peidab. */
  PUBLIC_DAY_ROUTE: "NEXT_PUBLIC_SERVICE_LOG_DAY_ROUTE"
});
...
export function readServiceLogFlags(env = process.env) {
  ...
  return {
    ...
    measurementEnabled: enabled && readFlag(env[SERVICE_LOG_FLAG_KEYS.MEASUREMENT]),
    dayRouteEnabled: enabled && readFlag(env[SERVICE_LOG_FLAG_KEYS.DAY_ROUTE])
  };
}
...
export function isServiceLogDayRouteEnabled(env = process.env) {
  return readServiceLogFlags(env).dayRouteEnabled;
}
...
/**
 * UI-pool päevateekonnale. LOETAKSE LITERAALSELT — vt `isServiceLogUiEnabled`
 * kommentaari: `env[võti]` jääks kliendipaketis `undefined`-iks ja voog ei
 * vahetuks kunagi.
 */
export function isServiceLogDayRouteUiEnabled() {
  const raw = String(process.env.NEXT_PUBLIC_SERVICE_LOG_DAY_ROUTE ?? "").trim().toLowerCase();
  return raw === "1" || raw === "true" || raw === "yes" || raw === "on";
}

export function isServiceLogMeasurementEnabled(env = process.env) {
  return readServiceLogFlags(env).measurementEnabled;
}

export function isServiceLogMeasurementUiEnabled() {
  const raw = String(process.env.NEXT_PUBLIC_SERVICE_LOG_MEASUREMENT ?? "").trim().toLowerCase();
  return raw === "1" || raw === "true" || raw === "yes" || raw === "on";
}
