 ...
 *
 * KOLM TINGIMUST. Capability annab õiguse üldse määrata; üksuse kattuvus ütleb,
 ...
 *
 * KOLMAS ON ORGANISATSIOONI ENDA OLEK (SOL-ORG-02). Peatatud maja jääb
 * LOETAVAKS — juht peab nägema, mis pooleli jäi —, aga kirjutamine lõpeb.
 * Vastus on **409, mitte 403**: õigus on tal alles, muutunud on maja seis, ja
 * see vahe peab teatest välja paistma, muidu otsib ta viga oma õigustest.
 */
export async function assertCanAssign(
  ...
  if (!scope.writable) throw conflict("service_log.errors.organization_not_writable");
...
export async function assignVisit(
  ...
  } = {},
  {
    ...
    env = process.env,
    now = new Date(),
    geocodeAddress = geocodeServiceMapAddress,
    /* SOL-ORG-03: auditikirjutus on SÜSTITAV PORT, et veasüstetest saaks teda
       päris tehingus kukutada. Vaikeväärtus on päris funktsioon — see ei ole
       vaikne edu, vaid seesama kirjutus, mis tehingus niikuinii toimuks. */
    writeAudit = writeOrgAudit
  } = {}
) {
...
export async function reassignVisit(
  ...
  /* `writeAudit` on siin samal põhjusel mis `assignVisit`-is: mõlemal rajal
     peab veasüstetest saama auditirea kukutada ja näha, et töö EI liikunud. */
  { db = prisma, env = process.env, now = new Date(), writeAudit = writeOrgAudit } = {}
) {
  assertServiceLogEnabled(env);
  ...
  /* Mutatsioonivastus ei ole detailivaade. Eriti samale omanikule suunatud
     no-op ei tohi muuta tahvli külastuse ID-d peidetud sisu lugemise võtmeks. */
  if (visit.ownerUserId === toWorkerUserId) return { id: visit.id };
