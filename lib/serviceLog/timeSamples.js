 ...
 */
export async function purgeExpiredSamples(
  { db = prisma, retentionDays = SAMPLE_RETENTION_DAYS, providerProfileId } = {}
) {
  const cutoff = new Date(Date.now() - retentionDays * 24 * 60 * 60 * 1000);
  const result = await db.serviceLogTimeSample.deleteMany({
    where: {
