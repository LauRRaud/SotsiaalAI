...
export async function recordSample(userId, input = {}, { db = prisma, env = process.env } = {}) {
  try {
    ...
    /* ENTRY_INPUT is authoritative contract evidence and may only be emitted
       by createEntry after a real, non-replayed entry has been persisted.
       Keep this lower-level boundary fail-closed even if a future caller
       exposes recordSample through another transport. */
    if (sample.kind === SAMPLE_KIND.ENTRY_INPUT) return false;
