...
function isStructuredFieldBoundary(input, match) {
  if (!/[\r\n]/u.test(match[0])) return false;
  const start = match.index || 0;
  const end = start + match[0].length;
  const precedingLine = input.slice(input.lastIndexOf("\n", start - 1) + 1, start);
  const nextLineBreak = input.indexOf("\n", end);
  const followingLine = input.slice(end, nextLineBreak === -1 ? input.length : nextLineBreak);
  return followingLine.trimStart().startsWith(":") ||
    (precedingLine.includes(":") && followingLine.includes(":"));
}
...
export function detectAnonymityIssues(text) {
  ...
  for (const rule of ANONYMITY_RULES) {
    ...
    for (const match of input.matchAll(pattern)) {
      // Do not join adjacent labelled template fields, but keep newline-separated names blocked.
      if (rule.type === "name" && isStructuredFieldBoundary(input, match)) continue;
...
const PRE_INQUIRY_DRAFT_MAX_TEXT = 16_000;
const PRE_INQUIRY_DRAFT_MAX_TITLE = 1_000;

export function buildCaseFromPreInquiryDraft(preInquiry) {
  const sourceText = normalizeText(preInquiry?.situation, PRE_INQUIRY_DRAFT_MAX_TEXT);
  const topics = inferCovisionTopics(preInquiry?.topic, sourceText);
  const anonymityIssues = detectAnonymityIssues(sourceText);
  return {
    title: normalizeText(preInquiry?.topic, PRE_INQUIRY_DRAFT_MAX_TITLE) || "Eelpöördumisest loodud kovisiooni mustand",
...
/**
 * Assembles the createCovisionCase() input for the pre-inquiry -> Covision
 * handoff (A4). The anonymity confirmation is NOT implied by the draft; it is
 * forwarded from the request body and must be an explicit boolean `true`, so a
 * Covision case is never created from a pre-inquiry without the specialist
 * having confirmed anonymity. `normalizeCaseInput` then rejects an unconfirmed
 * request with 400 `covision.errors.anonymityConfirmed_required`.
 */
export function buildPreInquiryCovisionCaseInput(draft = {}, body = {}) {
  return {
    ...draft,
    title: body?.title || draft?.title,
    centralQuestion: body?.centralQuestion || draft?.centralQuestion,
    anonymityConfirmed: body?.anonymityConfirmed === true,
    participants: []
  };
}
