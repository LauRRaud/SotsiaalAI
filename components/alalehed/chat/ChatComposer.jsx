...
export default function ChatComposer({
  ...
  const inputBarChildren = <>
      ...
      <div className="chat-composer__primary-actions">
        ...
        {showDictationButton && !useSimpleRoomActionButtons && recording && cancelRecording ? <button type="button" aria-label={t("chat.mic.cancel")} data-tooltip={t("chat.mic.cancel")} onClick={handleDiscardRecordingClick} onMouseDown={preserveDesktopInputFocusOnMouseDown} data-recording-cancel="true" /> : null}
        {showDictationButton && !useSimpleRoomActionButtons ? <button type="button" aria-label={recording ? t("chat.mic.stop") : micBlockedLabel || t("chat.mic.start")} data-tooltip={recording ? t("chat.mic.stop") : micBlockedLabel || t("chat.mic.start")} onClick={handleDictateClick} onMouseDown={preserveDesktopInputFocusOnMouseDown} disabled={isRoomMode && (roomBlocked || roomAuthRequired)} data-speaking={recording ? "true" : "false"} data-recording={recording ? "true" : "false"} data-recording-blocked={micBlockedLabel ? "true" : undefined} data-recording-complete={recordingPulse ? "true" : "false"} /> : null}
        {isGenerating || isStreamingAny ? <button type="submit" aria-label={t("chat.send.stop")} data-tooltip={t("chat.send.title_stop")} disabled={isRoomMode && (roomBlocked || roomAuthRequired) || !hasInput && !isGenerating && !isStreamingAny} data-loader-active="true" onPointerDown={handlePrimaryActionPointerDown} onMouseDown={preserveDesktopInputFocusOnMouseDown} /> : hasInput ? <button type="submit" aria-label={t("chat.send.send")} data-tooltip={t("chat.send.title_send")} disabled={isRoomMode && (roomBlocked || roomAuthRequired)} onPointerDown={handlePrimaryActionPointerDown} onMouseDown={preserveDesktopInputFocusOnMouseDown} /> : canOpenVoiceMode ? <button type="submit" className="conv-voice-mode-trigger" aria-label={t("chat.voice.open")} data-tooltip={t("chat.voice.open")} data-voice-mode-trigger="true" onPointerDown={handlePrimaryActionPointerDown} onMouseDown={preserveDesktopInputFocusOnMouseDown} /> : <button type="submit" aria-label={t("chat.send.send")} data-tooltip={t("chat.send.title_send")} disabled={!hasInput || isRoomMode && (roomBlocked || roomAuthRequired)} data-empty-disabled={!hasInput ? "true" : undefined} onPointerDown={handlePrimaryActionPointerDown} onMouseDown={preserveDesktopInputFocusOnMouseDown} />}
      </div>
    </>;
  return <Form ref={inputRowRef} className="chat-composer" data-room-mode={isRoomMode ? "true" : undefined} style={inputRowStyle} onSubmit={handleSubmit} autoComplete="off">
      {showSideControls ? <div className="chat-composer__side-controls">
        {hideTools ? null : <>
            ...
            {toolsMenuPanel}
            {showDocumentAttachButton ? <button type="button" aria-label={t("chat.upload.aria")} data-tooltip={t("chat.upload.tooltip")} onMouseDown={preserveDesktopInputFocusOnMouseDown} onClick={onPickDocumentFile} disabled={documentAttachDisabled}>
                    <svg aria-hidden="true" width="36" height="36" viewBox="0 0 42 42" fill="none">
