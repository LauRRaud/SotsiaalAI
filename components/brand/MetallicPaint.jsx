...
uniform float u_distort,u_contour,u_tintPulse,u_radial;
...
void main(){
  ...
  fl*=u_scale;
  fl-=an;
  // Radiaalne komponent: metallic-muster levib logo keskelt väljapoole
  // (kombineerub -an ajaliikega → rõngad laienevad sujuvalt õue)
  fl+=rd*u_radial;
  ...
  float sf=u_blur;
  // Kanalite ERINEV pehmenduslaius tegi värviservi (rohekas hõõg) ka
  // siis, kui nihe oli 0 — seome erinevused u_chroma külge: chroma 0
  // = kõik kolm kanalit identsed (puhas monokroomne metall).
  float rP=fract(fl+rO);
  float rC=mG(hi.r,lo.r,rP,sf+(.018+u_refract*cv*.025)*u_chroma,cv);
  float gP=fract(fl);
  float gC=mG(hi.g,lo.g,gP,sf+.008*u_chroma/max(.01,1.-sl),cv);
  float bP=fract(fl-bO);
  float bC=mG(hi.b,lo.b,bP,sf+.008*u_chroma,cv);
  vec3 col=vec3(rC,gC,bC);
  col=(col-.5)*u_contrast+.5;
  col=clamp(col,0.,1.);
  // Tint hingab ajas: haripunktis SELGELT šampanja, põhjas puhas
  // platina, siis uuesti šampanjasse (tellija 06.07). Varasem burn-
  // valem (1-(1-col)/tint) TUMENDAS pikslit → hall, mitte kuldne;
  // nüüd heledust säilitav toonimine + õrn kuma haripunktis.
  float tintAmt=clamp(length(u_tint-1.)*1.6,0.,.9);
  float pw=smoothstep(0.,1.,.5+.5*sin(u_time*.006));
  tintAmt*=1.-u_tintPulse*(1.-pw);
  vec3 tinted=col*u_tint/max(dot(u_tint,vec3(.299,.587,.114)),.001);
  col=mix(col,clamp(tinted,0.,1.),tintAmt);
  col*=1.+pw*u_tintPulse*.05;
  col=clamp(col,0.,1.);
  oC=vec4(col*vs,vs);
}`;
...
export default function MetallicPaint({
  ...
  const resizeCanvas = useCallback(() => {
    ...
    if (uniforms.u_ratio) {
      gl.uniform1f(uniforms.u_ratio, cssWidth / cssHeight);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    gl.uniform1f(u.u_tintPulse, tintPulse);
