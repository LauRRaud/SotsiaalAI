...
## Järgmise ploki otsus pärast analüüsi lugemist

07.09.2026, omaniku ülesanne: otsustada parandused ja edasised sammud. Kontrollitud kohalik ja kaug-main on `4aba1c3c7`; omaniku muud pooleliolevad failid säilivad. See otsus ei ole uus mudelikatse ega rakenduskoodi muudatus.

**Valik: üks väite ja tõendi seose plokk; kõigepealt kohalik kandidaat, siis piiratud võrdlus.** Praeguse `contracts.js` juhis juba keelab kõiki täheldatud laiendusi. Sama keelu lisamine veel kord ei anna piisavalt põhjendatud järgmist sammu. Ka vana tsitaadikandidaati ei aktiveerita: [varasem pärisvõrdlus](rag-v2-evidence-draft-local-2026-09-07.md#pärisvõrdlus-0709-kandidaat-jääb-välja) andis 0/7 avaldatud kandidaatvastust, sest mudel muutis kopeeritud tsitaate.

1. **Kohalik uus katsekandidaat valib etteantud algteksti lõike tunnustega.** Rakendus seob tunnused deterministlikult muutmata algteksti vahemikega ja taastab tsitaadi ise. Mudel ei kopeeri tsitaati ega vali vabalt tekstinihkeid. Nähtav väide seotakse väikese sisuliselt sidusa tõendiosaga. Kasutada olemasolevat `evidence-draft.js` rada uue versiooniga, säilitades vana kandidaadi ja v1–v4 vastuste lugemise. See kõrvaldab kopeerimistäpsuse sõltuvuse; parafraasi õigsus jääb sisuliselt hinnatavaks.
2. **F4 on peamine sisuline vastuvõtt.** Allikaplokk sisaldab üksnes valitud tõendiga toetatud väidet; tõendi puudumine jääb kasutatud katkenditega piiritletud `limitations` väljale. Üks plokk ei tohi ühendada allika fakti ja vastaja järeldust tõendi puudumisest. Piiranguvälja paigutamine ei muuda liiga laia väidet õigeks. D1/D3 lisatud otsustusosalised ja tegevuste järjekord kuuluvad sama ploki ulatuse kontrolli. Regex ei tõsta lauseid automaatselt ümber ega otsusta nende tõeväärtust.
3. **F1/F2 jaoks parandada vastuvõtuküsimusi.** Küsida otse projektide valimise, tegevuse alguse ja mõõdetud mõju eristust ning eraldi rahastaja/rahastamise ja programmi eesmärgi allikatuge. Nende vastuvõtt ei tohi sõltuda juhuslikust „teisest punktist” või sellest, kas mudel mainib rahastamist vabatahtlikult. F3 ja O1 senist teostust ei avata uuesti; nende kitsad regressioonid jäävad kontrolliks.
4. **Esimene sisuline võrdlus: kuus küsimuseperekonda, kummalgi variandil kuus vastust.** Praegune v4 baas ja uus kandidaat saavad identsed külmutatud paketid, sama mudeli ja samad piirid; uusi embedding'uid ega otsingut pole vaja. Perekonnad: ajalisus, rahastusviide, artikli/normi ulatus, faktilehe sõnaselge piir versus isikupõhine teadmatus, korralduslike sammude ulatus, uus piisava allikatoega kontrollküsimus. ET/EN/RU peavad olema esindatud; tõlkeid ei loeta uuteks perekondadeks. Täpsed küsimused, paketid, uus loa seos ja rahaline piir valmistatakse ette enne võimalikku käivitamist. Praegused kulutatud load ei laiene sellele.
5. **Kandidaat jääb alles ainult selge kasu korral.** Teadaolevad sihtvead peavad kaduma koos vajaliku sisuga; toetatud kontrollvastus peab avalduma. Võltsitud kindlus, põhjendamatu keeldumine või väljundmahu tõttu kadunud põhisisu ei ole paranemine. Mõõta avaldamist, väite tuge, väljajätteid, keelt, kulu ja viivitust; tsitaadi/ID kontrolli edu ei ole semantiline hinne. Kuus paari annavad edasiarenduse otsuse, mitte üldise kvaliteediprotsendi.
6. **Jätkuvestlusega ühendamine alles pärast seda võrdlust.** Praegune konfiguratsioon keelab kandidaadi ja dialoogi koos kasutamise; seda kaitset ei eemaldata esimeses plokis. Eduka kandidaadi jaoks tehakse eraldi sidumine olemasoleva dialoogikontekstiga ja selle muutuse kitsas kontroll. Praegu läbinud teema-, isiku- ja UI-mehhanismi ümbertegemiseks tõendit ei ole.
