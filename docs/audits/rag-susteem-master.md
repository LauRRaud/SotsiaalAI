...
### 13.10 Vestlusega seotud teosevalik — F07 teine kohalik plokk

Omaniku „tee” järel lisati `codex/repair-a` harusse F07b: sama autori ja põhiteksti teemaga kinnitatud teoste piiratud valik, järjekorranumber / täpne pealkiri / „mõlemad” ning valitud teoste eraldi vastuserajad. See on kohalik osateostus, mitte F07 loenduste ega produktsioonivalmiduse DONE. Pakkumine on kuni viie tagastatud kandidaadi loetelu, mitte autori kogu bibliograafia täielikkuse väide; „mõlemad” kehtib täpselt kahe valiku puhul.

**Usalduspiir:** `sourceSelection.js` seob pakkumise järjestuse, doc/source/version kolmiku, väljastamise aja ja 30-minutilise kehtivuse revisjoniräsiga. Operatsioon on seotud algse salvestatud USER-sõnumiga. Valik loetakse ainult omaniku sama vestluse lõpetatud assistendisõnumist; kliendi JSON, ajalugu ega ordinal ise ei määra dokumendi-ID-d. Sama allika-IDga eri dokumente ei lubata üheks valikuks. Pakkumise metaandmed elavad olemasolevas `workflow.ragRecovery.sourceSelection`-is, mitte uues tabelis.

**Katse ja konkurents:** `sourceSelectionStore.js` taastab kordusel ainult `ChatTurn.attempt` täpse `RagAttempt`-i muutumatu `source_selection_binding`-u. Muutunud sisend, uuem vestlussõnum, võõras vestlus või teine revisjon ei lähe märkamatult üldotsingusse. Binding'uta vana kavatsus ei saa omandada hilisemat pakkumist. Claim, kvoodijärgne deferred USER ning terminalne avaldamine kontrollivad vestluse hetkeolekut sama advisory-lock'i piires; vana `persistInit` kasutab nüüd samuti seda lukku. Lõpetatud pöörde kordus jääb varasesse replay-harusse ilma uue otsingu ja kasutusreservatsioonita. Pakkumine ja vastus salvestatakse koos katse ning kasutuse arveldusega; hiline vastus ei või uuema töövoo järel vana valikut taastada.

**Värske allikas:** `sourceSelectionEvidence.js` ja `sourceSelectionRetrieval.js` nõuavad siseteenuse no-store detailist aktiivset elutsüklit, sama aktiivversiooni, eksplitsiitset avaliku uurimis-/ajakirjakogu kuuluvust ning rollile lubatud audience'i. Puuduvat/tundmatut ligipääsu ei tõlgendata loana. Iga otsingu päringu- ja ühine filter lukustatakse doc+source ID-le; tagastatud chunk'i kõik olemasolevad ID/versiooni aliased peavad omavahel kattuma. Versiooni ei tuletata valikust. Kontrollis selgus konkreetne varasem katkestus: Python-teenuse shared/lexical/sibling ja dense väljund kaotas `document_version`-i. Mõlemad projektsioonid edastavad nüüd ainult päris metadata väärtuse; puuduv versioon jääb puuduvaks. Seetõttu tuleb rakendus ja RAG-teenus hiljem avaldada kooskõlalise paketina.

**Vastus:** „mõlemad” ei ühenda kahe dokumendi konteksti ühe dokumendi faktivalidaatori alla. Iga valik läbib oma assembler'i, identiteedikontrolli, olemasoleva vastusekäsitleja ja faktide/allikate kontrolli. Server liidab alles nende kontrollitud publikatsioonid pealkirjastatud osadeks ning kohandab numbriviited. Lapse-radadel on salvestus, kasutuse arveldamine ja stream keelatud; vanem salvestab ühe tulemuse. Kõik uuesti pakutavad allikad kontrollitakse enne avaldamist uuesti. Aegunud või muutunud valik nõuab värsket pakkumist; muutus mudelitöö ajal eemaldab vanad faktiväited avaldatavast vastusest. JSON/SSE saavad sama lõpliku teksti ja allikad.

**Jälg:** `source_selection` säilitab revisjoni, valitud/pakutud ID-d ning iga teose versiooni, vastuse/konteksti räsi ja eraldi faktikontrolli tulemuse. Need läbivad katse tõendiprojektsiooni ja olemasoleva diagnostika/Markdown-aruande. Osavastus, tõrge ja puuduv kontroll ei muutu üldiseks PASS-iks; toormustandit ega teksti ei lisata üldlogisse. Automaatne algpõhjus ja sisuline õigsus jäävad endiselt eraldi tõendatavaks.

Kohalik sihttõend: 110 Node'i testi UTC-s (21 valik, 19 katse elutsükkel, 22 F06/teoste eraldamine, 21 F07a, 27 diagnostika), lisaks Python-teenuse mõlema tegeliku projektsiooniväljundi kontroll. Täpsed väravad ja build on [olemasolevas kontrolliaruandes](../../eval/rag-uus75-kontroll-2026-08-31.md). Need ei tõenda päris PostgreSQL-i võistlust/rollback'i ega autenditud vestlust. Serveri chat/search/model-kutseid selles plokis 0; migratsiooni, main-integratsiooni, push'i ja deploy'd ei tehtud.
...
**Kohalik osateostus:** §13.9 katab täpse autori + sõltumatu body-teema kinnituse. §13.10 lisab repair-a harus serveriga seotud valikupakkumise, „teine/mõlemad” elutsükli, aktiivversiooni/ligipääsu kontrolli ning eraldi teoste vastused ja diagnostika. Eraldi loendusühikute vastus, integratsioon ja pärisrada jäävad allpool kirjeldatud kujul lahti.
