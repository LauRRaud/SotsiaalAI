...
V1 omaniku autenditud in-app serverikontroll `e9669a36`: kolm põhiküsimust eraldi ja järjest, kokku 3 PASS / 1 PARTIAL / 2 FAIL; automaatne koond näitas kolme kirjet, vana vastuse püsiviide säilis ja MD allalaadimissündmus kinnitati. [Täisvastused ja uuritud põhjused](../../eval/rag-uus75-kontroll-2026-08-31.md). F02 tõrge paljastas puuduva aastarolli/lõppvärava tõendi ja F01 PARTIAL tehnilise läbimise piirangu. Omaniku seejärel nõutud v2 arendus läbis 23 UTC sihttesti, sealhulgas päris planneri → canonical serializeri → projektsiooni/Markdowni raja; serveri järelkontroll on enne väljalaset `NOT_PROVEN`. Vastamise loogikat ega korpust selles plokis ei parandata.
