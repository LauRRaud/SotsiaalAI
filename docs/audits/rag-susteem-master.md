...
| M2 mitme allika järelkatse | Kohalikult ette valmistatud, pärisjooks käivitamata | 8 pärisallikat, 15 uut perekonda / 21 küsimust ja 9 regressiooniküsimust; 120-realine mock-mehaanika läbis tehnilise veata. Täpne uus embedding-manifest vajab omaniku luba. |
...
Mitme allika ettevalmistus lisas kohalikus `main`-is 4 testi ehk mõjutatud RAG-i lõplik komplekt on nüüd **61/0/0**. Lint, diff-kontroll, i18n ja tootmisbuild läbisid. Need commit'id pole veel GitHubis ega serveris; seal kehtib jätkuvalt ülal kirjeldatud lõpetatud M2.2 käitus.
