...
**Seis (13.08.2026): PARTIAL — hallatav mehhanism, alarm ning säilitustähtaja ja reatõrke päris PostgreSQL-i tõend on DONE; tootmistaimeri aktiveerimine ja kontrollitud systemd-jooks on õigus-/andmekaitsevärava taga, runtime: NOT_PROVEN.**
...
- **Negatiivtõend enne parandust:** uus runtime-sondi leping kukkus vana sondi peal **14/15**, täpselt põhjusega „sond ei käivita päris säilitustööd"; pärast sondi laiendust **15/15**. See eristab ajastuse/alarmi varasemat tõendit tähtaja ja taastumise tõendist.
...
- **Üks täpne puuduolev otsus:** omanik peab saama õigus-/andmekaitsekinnituse ja selle dokumenteeritult heaks kiitma, et `WORKER_DATA_PROCESSING` katab CaseWorkAssist/JTA töötluse ja auditmetaandmed **ning** 12-kuuline ülekantud mustandi/arhiveeritud juhtumi säilitus koos 30-päevase hoiatusega on tootmises aktiveerimiseks põhjendatud. Praegused kanoonilised lepingud märgivad Juhtum-V1 Õ2/Õ3 ja JTA Õ1/Õ3 endiselt `LEGAL_ASSUMPTION`-iks ning JTA Õ2 nõuab enne aktiveerimist õigusabi kinnitust; `lib/casework/flags.js` ütleb sama. Seetõttu ei olnud luba kustutavat taimerit sisse lülitada ega päris systemd-jooksu teha.
- **Alles (NOT_PROVEN):** pärast ülaltoodud kinnitust kontrollitud aktiveerimisrada `preflight → dry-run → alarm/smoke → enable → üks kontrollitud systemd-jooks → viimane edu / järgmine jooks / service-journal`. Kohalik visatav PostgreSQL tõendab koodi ja DB-invariante, kuid ei teeskle aktiveeritud tootmistaimerit.
- Kontroll: sihttestid **41/41**, `npm run casework:retention:probe` **28/28**, ajutise andmebaasi koristus 0 allesjäänud rida; lint ja diff-check allpool oleva commit'i väravas.
