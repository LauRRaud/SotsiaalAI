...
**Seis (13.08.2026): DONE — omaniku kinnitatud säilituspoliitika on koodis, kasutajatekstides, privaatsusteavituses ja raamlepingus vastuoludeta; hallatav taimer on tootmises aktiveeritud ning kontrollitud systemd-jooks, tervisesond ja järgmine ajastus on tõendatud.**
...
- **Tootmis-runtime 13.08, väljalase `422679c360404022aa86d2d0ce4d91ef7aff7a32`:** serveri HEAD kattus väljalaskega ja tracked tööpuu oli puhas. Mõlemad juhtumitöö lipud aktiveeriti ning rakendus ehitati uuesti. Kontrollitud kuivjooks andis 0 purge'i, 0 väljakustutust, 0 hoiatust, 0 juhtumikustutust ja 0 viga; enne esimest päris edu andis smoke nõuetekohaselt `ALARM` / `never_succeeded` ning väljumiskoodi 1.
- **Kontrollitud päris jooks ja ajastus:** `sotsiaalai-casework-retention.service` lõpetas `Result=success`, `ExecMainStatus=0`; jooks andis 0/0/0/0 ja 0 tõrget. Seejärel aktiveeriti `sotsiaalai-casework-retention.timer`: **enabled / active / waiting**, systemd'i järgmine käivitus `13.08.2026 22:00:56 EEST`. Järgmine smoke oli `OK`, viimane edukas jooks `2026-08-13T18:04:30.920Z`, viimane jooks `2026-08-13T18:04:30.755Z`, tuletatud järgmine jooks `2026-08-13T19:04:30.755Z`, intervall 60 min ja alarmi lävi 120 min. Journal sisaldas ainult koondloendureid, mitte kasutajate sisu.
...
- **Sulgemistõend:** väljalaskerada läbis `preflight → dry-run → alarm/smoke → enable → üks kontrollitud systemd-jooks → viimane edu / järgmine jooks / service-journal`. Kohalik visatav PostgreSQL tõendab mõlemat tähtaegade piiri ja reatõrkest taastumist; tootmisruntime tõendab, et sama hallatav töö päriselt käivitub ja on monitooritud.
- Kontroll: säilituse sihttestid **45/45**, registreerimise nõustumistestid **6/6**, `npm run casework:retention:probe` **28/28** päris PostgreSQL-is, ajutise andmebaasi koristus 0 allesjäänud rida; ühendatud väljalaskepuu UTC täisvärav **4751/4751**, migratsiooniahel **200/200**, i18n, lint ja diff-check läbitud.
...
**Seis (15.08.2026): DONE —** jälgitava retention-tähtaja autoritatiivne allikas on omaniku privaatmooduli tellimuse `endsAt`; aktiivse lepingu pikenemine nihutab tähtaega edasi. Worker skannib ka tähtajatu aktiivlepingu `NULL`-tähtajaga kirjeid, kontrollib lepingu iga kord uuesti ning saab kirje pärast lepingu tühistamist kustutada; vanim-enne järjekord ei lase korduskontrollidel teisi ridu näljutada. Idempotentne monitooritud worker jätab tähtajaeelsed read alles, eemaldab aegunud read, salvestab nurjunud batch'i ning võimaldab kontrollitud retry. Regressioonitest tõendas tühistatud `NULL`-tähtaja tee; päris PostgreSQL-i sond katab lisaks tähtaja mõlemad pooled, kasutaja varasema kustutuse, konto FK-kaskaadi, lepingu pikenemise ja tõrkejärgse korduskatse, kuid selle uus `NULL`-juhtum jäi kohaliku PostgreSQL-i puudumise tõttu `NOT_PROVEN`. Tootmistaimeri tegelik jooks on samuti `NOT_PROVEN`.
