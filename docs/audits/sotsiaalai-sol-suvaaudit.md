...
**Seis (09.08.2026): mehhanism DONE; taimeri LUBAMINE ootab omaniku enda lukustatud järjekorda; staging-runtime: not_run.**
...
- **Testid** (`tests/casework/retentionSchedule.test.js`, 13 uut): tervis mõlemast otsast (värske jooks → OK; üks vahelejäänud jooks **ei ole** alarm, kaks on; lävi täpselt piiril) · ükski jooks kunagi → `NEVER_RUN`, mitte vaikne OK · jooksud käivad aga ei õnnestu → ALARM · rida tekib enne tööd ja pooleli jäänud jooks ei ole `ok` · tõrgetega partii ei ole edukas jooks · erindi teadet ei salvestata · kuiv käivitus ei tee tervist roheliseks · ajastuse leping (lukk, `Persistent`, timeout, ei ühtegi `Restart=`) · deploy paigaldab aga ei luba · smoke on olemas ja alarm jõuab väljumiskoodi · käivitaja suleb jooksurea ka kukkumisel.
- **Mis on veel tegemata (NOT_DONE):** taimeri lubamine serveris ja **staging-tõend tähtaja mõlemast poolest ning nurjunud rea taastumisest**. Mõlemad nõuavad, et juhtumitöö värav oleks kuskil sees; praegu on ta tootmises väljas ja omaniku järjekord ütleb, millal ta sisse läheb. Mehhanism on valmis ootama, mitte tööle pandud.
