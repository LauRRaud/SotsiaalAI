...
- Mentorluse profiili, kataloogi, taotluste, suhte, kokkuleppe, kohtumise, kokkuvõtte, ettevalmistuse, sweep'i ja adapterite sihttestid: **24/24 läbitud**. Kogum kinnitab praegu ka seda, et supersede-link tekib juba paranduse DRAFT-i loomisel, ning loeb jagatud ettevalmistuse sisu enne avamismarkerit; seega need rohelised ootused ei tühista SOL-MENT-03/04 lepinguriske. Katmata on ACTIVE-profiili moderatsioonist mööduv muutmine, väliskirje 12 kuu stale-piir, enam kui 50 läheneva kohtumise lehekülg, brauseri ja serveri erinev ajavöönd ning PLATFORM_ROOM-i päris kahepoolne ruumivalik.
...
- **Deploy paigaldab unit-failid** (`scripts/deploy-server.mjs`: kopeerib, `daemon-reload`, teatab muutusest) **ja EI luba taimerit sisse.** See on teadlik: `SotsiaalAI.md` S1 lukustab järjekorra — Õ2/Õ3 andmekaitseanalüüs → cron paigaldatakse (sama väljalase, mis aktiveerib) → kuivjooks → aktiveerimine → päris jooks + logikontroll. Unit-failide olemasolu ei aktiveeri midagi; lubamine on üks käsk ja ta on dokumenteeritud (`deploy/systemd/README.md`).
- **Testid** (`tests/casework/retentionSchedule.test.js`, 14 uut): tervis mõlemast otsast (värske jooks → OK; üks vahelejäänud jooks **ei ole** alarm, kaks on; lävi täpselt piiril) · ükski jooks kunagi → `NEVER_RUN`, mitte vaikne OK · jooksud käivad aga ei õnnestu → ALARM · rida tekib enne tööd ja pooleli jäänud jooks ei ole `ok` · tõrgetega partii ei ole edukas jooks · erindi teadet ei salvestata · kuiv käivitus ei tee tervist roheliseks · ajastuse leping (lukk, `Persistent`, timeout, ei ühtegi `Restart=`) · deploy paigaldab aga ei luba · smoke on olemas ja alarm jõuab väljumiskoodi · käivitaja suleb jooksurea ka kukkumisel.
- **Runtime-sond tehtud 09.08 õhtul: `npm run casework:retention:probe`, 22/22 päris PostgreSQL-i vastu.** Sond loob **visatava** andmebaasi (`sotsiaal_ai_retention_probe_<ts>`), rakendab talle kogu migratsiooniahela, mõõdab ja kustutab ta ära — arendus- ega tootmisbaasi ta ei kirjuta, sest jooksulogi ridade segamine päris seisuga teeks iga järgmise mõõtmise valelikuks. Koristust **kontrollitakse** (`pg_database`), mitte ei eeldata (A4 sondi õppetund).
- **Mida sond tõendab ja `npm test` ei saa:** mõlemad `CHECK`-id on päris andmebaasis ja keelduvad päris kirjutusrajal (`ok=true` ilma lõpuajata; negatiivne loendur) · „algas ja ei lõpetanud" rida on seejuures **lubatud**, muidu ei saaks `startRetentionRun()` üldse kirjutada · lävi **täpselt piiril** on veel OK ja **üks millisekund üle** on ALARM · ainult kuiv jooks ja ainult tõrgetega jooksud on ALARM · smoke **päris protsessina**: alarm → väljumiskood **1**, korras seis → 0, väljas värav → 0, **katkine skeem → 1** (monitooring ei tohi vaikida ka siis, kui migratsioon on puudu).
...
- **Sond ise sai negatiivkontrolli:** esimene jooks andis 5 päris viga (`--import` ei võta Windowsis absoluutset teed → smoke ei käivitunudki ja nägi välja nagu alarm; piiripealne rida vananes mõõtmise enda ajal). Uus leping (`retentionSchedule.test.js`, 14.) kukub läbi, kui sondi koristuse kontroll eemaldada — 13/14, seejärel tagasi 14/14.
- **Mis on veel tegemata (NOT_DONE):** taimeri lubamine serveris ja **staging-tõend säilitustähtaja mõlemast poolest** (hoiatus ja kustutus päris kellaga) **ning nurjunud rea taastumisest**. Sond tõendab **alarmi** mõlemad otsad, mitte tähtaega — need on eri asjad ja teine nõuab, et juhtumitöö värav oleks kuskil sees. Praegu on ta tootmises väljas ja omaniku järjekord ütleb, millal ta sisse läheb. Mehhanism on valmis ootama, mitte tööle pandud.
- Kontroll: `npm test` **3225/3225**, `npm run casework:retention:probe` **22/22**, `npx eslint` puhas.
...
- **Arendusbaas oli 5 migratsiooni maas** (4 neist eelmise sessiooni omad) ja on nüüd `prisma migrate deploy`-ga järele viidud; ilma selleta oleks RAG-admin lokaalselt „veerg puudub" vigadega katki. **Server vajab seda migratsiooni deploy’l** — Prisma klient valib uusi veerge kohe.
