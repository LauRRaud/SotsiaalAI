...
**Seis.** DONE — sulgemisvärav kasutab ühtset blocker-lepingut ning blokeerib `DEVICE_ONLY`, `QUEUED`, `UPLOADING`, `FAILED`, `CONFLICT` ja auth-pargi. Turvajärelkontroll näitas, et varasem recovery-import usaldas kliendi `recoveryImport` lippu ja tagantjärele muudetavat `deviceCreatedAt` aega; erand eemaldati nii märkme kui manuse serverirajalt ning suletud külastus on nüüd alati read-only. UI ei paku enam mittetöötavat taastenupu rada. Negatiivtestid tõendavad, et võltsitud sulgemiseelne ajatempel saab 409 ning ei loo märget, manust, faili ega auditit; olemasolevad ajaloolised `recoveryImportedAt` väärtused jäävad ekspordis nähtavaks. Mitme seadme recovery vajab enne taastamist serveri tõendatavat manifesti või muud usaldatud alust. runtime: brauser not_run (turvapiir tõendati teenuse sihttestidega; pärisseadmed pole keskkonnas kättesaadavad).
