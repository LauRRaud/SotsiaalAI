...
| Meeting-summary async töö | `/api/documents/meeting-summary/jobs` | audio ainult tööprotsessi mälus; job snapshot `AGENT_STORAGE_DIR/meeting-summary-jobs/<id>.json`; lõpptulemus UserDocument (`MATERIAL`) | snapshotis `userId`, API owner-check | output UserDocument võib hiljem indekseeruda | terminal-job 30 min, **jõustatud restardist sõltumatu kataloogisweepiga** (SOL-MEET-03): rippuma jäänud `queued`/`running` katkestatakse ja vabastavad kvoodi, loetamatu `.json` ja orb `.tmp` kaovad fail-closed faili vanuse järgi; pooleli arveldusega snapshot jääb teadlikult alles kuni commit õnnestub; konto kustutus sihib kataloogi (`purgeMeetingSummarySnapshotsForUser`) |
...
| Ajutised failid | DOCX `NamedTemporaryFile`, meeting job `.tmp`, egress partial, testimarkerid | OS temp / agent / recording katalog | protsess | pole | DOCX temp contextiga; **meeting job `.tmp` on nüüd sweep'itud** (SOL-MEET-03, orvupoliitika faili vanuse järgi); egress partial endiselt sweep'imata; auditi testitemp koristati nullini |
