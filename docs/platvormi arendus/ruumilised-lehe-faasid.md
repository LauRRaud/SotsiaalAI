...
## FABLE-Q — Alumine ruumidokk, faasid ja võrdlusriiul
...
### FQ11. Täpne jätkamispunkt interaktiivse prototüübi tegemiseks

**Samm 1 — eraldiseisev klikitav makett (järgmine aken, ilma rakenduskoodita):** loo mock-andmetega interaktiivne prototüüp failina `docs/platvormi arendus/prototyybid/fable-q-dokk-riiul-proto.html` (üks fail, inline CSS/JS, platvormi tume klaas + merevaik; ilma päris API ja marsruutideta), mis simuleerib **Dokumendi koostamise** lehte (§7 faasid 1–5 mock-sisuga) + täisdokk/sang + faasiriba + kolm vaaterežiimi + võrdlusriiul (mustand v2 vs v3; mall A vs B) + kitsa ekraani A/B (aknalaius <68rem). Klaviatuur ja reduced-motion peavad töötama maketis päriselt (mitte kirjeldusena). Testi 7 stsenaariumi kasutajaga: (1) „kus sa platvormis oled / kus töös oled / mis režiimis oled" — kolm eri vastust ilma abita; (2) lisa kaks pinda riiulisse ja ava võrdlus; (3) ütle, kumb pool on muudetav ja miks; (4) sulge võrdlus back-nupuga; (5) vaheta faasi ILMA ratast kasutamata (leia kaks teed); (6) sama klaviatuuriga; (7) sama reduced-motion + kitsas aknas. Läviväärtus: 0 topeltnavigatsiooni segadust (keegi ei otsi faasinuppu dokist ega ala-linki faasiribalt); riiul 1/2 → 2/2 → split ilma juhendamiseta ≥8/10.
**Samm 2 — päris leht (alles pärast sammu 1 läbimist):** haru `codex/fable-q-dokk-proto`; ainult esituskiht Dokumendi koostamise lehel (§7 ümberlõige + dokk + riiul), ühtegi API-/skeemimuudatust ei tehta; enne ehitust vaja kasutaja otsuseid **D-FQ1 ja D-FQ3** (teised ei blokeeri); Kovisiooni sessiooni riiul tuleb alles pärast KOV-R P7. Kuvatõendid samasse pildikausta alamkausta `fable-q-proto/` + selle README jätkamispunkti uuendus.

*FABLE-Q lisatud 15.07.2026; ainult dokument — rakenduskoodi, teste ega kuvatõmmiseid ei loodud. Tõendid: [PILT …] = `fable-pildid/kovisioon-tervikvoog/` failid; [KOOD] = aktiivne main (`RoomQuickbar.jsx`, `carousel.css`, KOV-R koodiviited); [OTSUS?] = tooteomaniku otsus. Varasemaid peatükke 1–10 ei muudetud.*
