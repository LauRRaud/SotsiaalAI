...
| T12 | ROOMS-CALLS-V1 | **TÖÖS — E1–E4 harus valmis** | 19.07 alustatud, worktree `../SotsiaalAI-rooms-calls-v1`, haru `codex/rooms-calls-v1`, baas `d5d16a2e`, push'itud `origin/codex/rooms-calls-v1 @ ea41028f`. **E2** race-migratsioon (`20260721000000_rooms_calls_v1`, 2 partial-unique indeksit — 3. oli juba olemas, audit eksis) + P2002-katted (`a6811ed2`); **E1** elutsükli ristkoristus (jagatud helper, ruumi-DELETE lõpetab kõned + audit, leave vabastab osaluse+consent) (`cc02417a`); **E3** GET-püünised + rate-limit ette + IP eemaldatud + pikkuspiirid + `statusForCallError` maskimine + 4 i18n-võtit (`f20b9fc4`); **E4a** `Room.archivedAt` soft-arhiiv (additiivne migr) + DELETE voo-ruumi kaitse (409, ainult MANUAL_INVITE kustutatav) + PATCH `action=archive` (`0c35de12`); **E4b** kutse ainult-OWNER (invites GET+POST, MODERATOR→403) + ruumiloendi server-tõe lipud (canDelete/canArchive/canInvite/canTransfer + archivedAt) + omanikuvahetus (uus POST `/[roomId]/transfer`, atomaarne `$transaction`, TOCTOU-kindel) + 5 i18n-võtit (`ea41028f`). 72/72 T12-skoobis + lint 0 + i18n OK + prisma (2 payments-viga suite'is EELNEVAD, T12-välised). **JÄÄB E5+E6** (salvestuse nõusolek+egress+retention — suurim, kõrgeim risk, `RECORDING_ENABLED` värav — omanik soovis PUHAST konteksti), **E7** (kokkuvõtte kandja). main/server/merge/deploy PUUTUMATA. E1 kliendipool (14 K1 näovahetus) veel tegemata. |
