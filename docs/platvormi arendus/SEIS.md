...
| T14 | WELLBEING-V2 | **LIVE** | E1+E2 tuum liidetud kohalikku `main`-i `8eb3430b` (merge `codex/wellbeing-v2 @ 981682c0`, baas `e0a78632`, omaniku loal 19.07), **LIVE serveris 19.07 (`ac0b7d3f`)**; kirjete lugemis-/kustutusrada + „Minu kirjed" naasmisvaade + V6 continuity + `wellbeing_space` adapter (SUPPORTED); 0 migratsiooni; **1736/1736 testi + väravad rohelised**; rütmikiht P2–P5 + org-koond VÄLJAS (TO-1/TO-2/O-WB-3) — vt „T14 teostus 19.07" |
...
| T21 | CASEWORK-V1 | **LIVE** | liidetud kohalikku `main`-i `d27066c2` (merge `codex/casework-v1 @ 73c3953e`, baas `ae997dbd`, omaniku loal 19.07), haru push'itud origin'i, **LIVE serveris 19.07 (`ac0b7d3f`, smoke 10/10)**; otsustevaba tuum = provenance-konsolideerimine (FIELD_PROVENANCE → jagatud `lib/workspaces/provenance.js`, FIELD-regressioon roheline) + read-only adapterid + vastuvõtulaua ettevalmistuspaneel; 0 migratsiooni; **1743/1743 testi + väravad rohelised**; P2–P6 (STAR2/Meetodipeegel/genogramm/ökokaart) VÄLJAS, vajavad O-CW + O-CW-7 õigusanalüüsi |
| T22 | SUPERVISION-V1 | **CODE_READY (haru, main puutumata)** | **E1–E7 TERVIKUNA VALMIS 19.07** — `codex/supervision-v1 @ 3fb5ec37`, baas `ac0b7d3f`, 6 commit'i, worktree puhas, **PUSH'IMATA**; 0 uut migratsiooni (0 skeemidiffi main'i vastu); väravad: **1825/1825 testi, lint 0 viga, i18n OK, prisma validate OK, turbopack build OK**; NOT_PROVEN = brauseri-QA + mobiil — vt „T22 teostus 19.07" |
...
2. ~~T22~~ — **KOOD VALMIS 19.07 (`CODE_READY`).** E1–E7 tervikuna harus `codex/supervision-v1 @ 3fb5ec37`; väravad rohelised, worktree puhas. **Ootab omaniku otsust: push + merge + deploy** (kõik kolm tegemata). Vt „T22 teostus 19.07".
...
**Järgmine:** **T14 tuum liidetud kohalikku `main`-i 19.07 (`8eb3430b`, omaniku loal) — merge-luba antud; server/deploy jäävad eraldi otsuseks.** T14 rütmi-viil (P2–P5) ootab eraldi lühikest otsusteringi (TO-1 kirje elutsükkel, TO-2 kontrollpunkt+meeldetuletus). Järgmisena töösse võetud **T21 CASEWORK-V1 tuum** (P0+P1); vt „T21 teostus 19.07".
...
**Järgmine:** T21 P2/P3 avanevad lühikese otsusteringi järel (O-CW-2/3/4/10); P4/P5 vajavad O-CW-7 õigusabi (ei alustata koodiga). Omaniku valik järgmiseks suureks teemaks {T22, T12, T13}. **Push + merge + DEPLOY TEHTUD omaniku loal 19.07** — server `ac0b7d3f` (smoke 10/10, rollback `ae59516f`); **kohalik `main` = `origin/main` = server** (kõik sünkroonis).

### T22 teostus 19.07 (SUPERVISION-V1 — E1–E7 TERVIKUNA, `CODE_READY`)

**Haru `codex/supervision-v1 @ 3fb5ec37`, worktree `C:\Users\rauds\Desktop\SotsiaalAI-supervision-v1`, baas `ac0b7d3f`** (= `main` haru alustamise hetkel; `main` on vahepeal liikunud ühe docs-commit'i võrra `f9ca078a`-le — koodimuudatusi vahepeal ei tulnud). **6 commit'i, worktree puhas, PUSH'IMATA; `main`, server, merge ja deploy PUUTUMATA.**
...
**NOT_PROVEN:** brauseri-QA ja mobiilivaade. Põhjus on keskkondlik, mitte koodiline — worktree'l puudub `.env` ja `preview_start` osutab põhitööpuule; runtime-tõendus tuleb teha pärast merge'i või eraldi throwaway-DB peal (T09 retsept). **Keskkonnamuudatus:** worktree `node_modules` oli sümbollink põhitööpuule, mis lõhub Turbopacki (`Symlink [project]/node_modules is invalid`); link eemaldati mitte-rekursiivselt (põhitööpuu 564 kirjet enne ja pärast — puutumata) ja asendati päris `npm ci` paigaldusega, et build saaks tõendatud. Link jäi taastamata: nii saab järgmine sessioon selles worktree's build'ida.

**Järgmine:** omaniku otsus **push + merge + deploy** (kõik kolm tegemata). SUP-P11 = T27 koondvärav, jääb skoobist välja; retention on `AWAITING_POLICY` (otsus 12) ja admin-UI väljas. Koodijärjekorras avaneb **T12 ROOMS-CALLS-V1**.
