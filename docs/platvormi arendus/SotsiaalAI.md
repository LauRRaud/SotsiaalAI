...
**75 küsimuse analüüsist tulnud RAG-i semantikaparandus on koondatud üheks läbivaks lepinguks; 29.08 järgmine üldine põhjuseparandus on otse `main`-is.** `SemanticTurnContract` kannab nime-, teema-, aja-, allikajätku- ja faktiväljad plannerist otsingusse, vastuse valideerimisse ja trace'i. Dokumendiidentiteet kirjeldab nüüd eraldi autorit ja tema kanoonilist võtit, organisatsiooni, dokumendiliiki, pealkirjaankruid, aastaid, akronüüme, kindlust ja päritolu. EstNLTK/Vabamorf annab plannerile pindkuju, lemma, liitsõnaosad ja pärisnimespännid, kuid ei otsusta intenti; isikud, organisatsioonid, KOV-id, kohad, akronüümid, dokumendipealkirjad ja teemaüksused jäävad küsimuse argumendiks, mitte ei kirjuta tegelikku intenti üle. Autorirada koostab aktiivsete autorimeta dokumentide täieliku inventari enne käändevormi täppisfiltrit, kasutab sama `doc_id` loendit valikus ja allikakuvamises ning välistab pelgalt tekstis mainitud inimese autoriportfellist. RU/EN pöörded säilitavad algpäringu ja vastuse keele, kuid saavad autori-, KOV-, õigus-, juhendi- ja uuringufakti otsinguks kontrollitud eestikeelsed semantilised terminid koos algsete nimede, arvude, aastate ja paragrahvidega. KOV lahendatakse enne plannerit; teenuse taotlemise ja kohaliku paragrahvi liitküsimus jagatakse kaheks eraldi tõendatud alaeesmärgiks. Uuringufakti rada tuvastab esmalt dokumendi ning teeb alati teise, küsimuse identiteedisõnadest puhastatud faktipäringu ainult valitud `doc_id` piires. Ühine `RequestedFactSlot` kirjeldab loenduse, osakaalu, summa, suuruse, kestuse, aasta, kuupäeva, kuu, aastaaja, ajapunkti, asukoha, isikurolli, organisatsiooni, meetodi, kategooria, loendi ja tekstilise seose koos kardinaalsuse, relatsiooni, väärtuste, ajaulatuse ja modaalsusega; olemasolev range arvusloti tõendus jääb arvuliste liikide jaoks alles. Vastuse genereerimise järel tuvastatud arvud, protsendid, kuupäevad, telefonid, rahasummad, kestused ja paragrahvid käivitavad valideerimise ka siis, kui küsimuse sõnavara seda ette ei ennustanud; voog puhverdatakse kuni selle otsuseni ning validaatori toetatud `source_id` on allikakuvamisel leksikaalsest hinnangust tugevam. Allikajälg eristab retrieved-, selected-context-, model-context-, validator-supported-, claim-supported- ja displayed-source ID-d; KOV-allika nähtav nimi sisaldab omavalitsust, allikaliiki ja kehtivust. Varasema runtime-auditi allikapaneeli aritmeetika on parandatud `52/75` peale. Selle muutumatu tööpuu muudetud JS-failide ESLint, Pythoni süntaks ja `git diff --check` on rohelised; tootmisbuildi ega runtime'i ei käivitatud. Automaatteste, probe-, smoke-, benchmark- ega E2E-radu ei loodud ega käivitatud; top-k-d, fusion'it, korpust, indeksit, embeddingut ega env-i ei timmitud. Parandatud 75 küsimuse kordus ja autentitud sisuline runtime jäävad kuni eraldi mõõtmiseni **NOT_PROVEN**; 75/75 ega 10/10 ei väideta enne uut tootmisjooksu.
