...
**07.09 omaniku kinnitatud põhisiht on kogu töötav ja müüdav GraphRAG-toode, mida kasutatakse ja katsetatakse SotsiaalAI-s.** M1/M2, piiratud M4/M6 ühendus ja M3 ankurdatud sõltuvuste alus on teostatud; viimase kood on serveris, uus profiil aktiveerimata ning päris DB/UI vastuvõtt lahti. Jätkame puuduva funktsionaalsuse arendamist ja testimist olemasolevas tervikahelas: järgmisena M3 allikapõhine teadmiste ettevalmistus ning sõltuvuste kasutamine päris indeksi ja vastamisraja kaudu, seejärel suure korpuse jätkatav töötlus ja teekaardi ülejäänud võimed. Korduvkasutatavus on läbiv nõue; tehnilised koodipiirid täienevad konkreetse tootevõime arendamise käigus. Etappide seis ja tööjärjestus on S2 RAG-lõigus; [M3 andmeleping ja tõend](../rag-v2/adr-007-source-dependencies.md).
...
| M3: sisuline sõltuvuskiht | **Osaliselt teostatud; kood serveris.** Valikuline väidete/sõltuvuste metadata, täpsed PDF-ankrud, indeksiobjektid ja eraldi piiratud sõltuvusotsing on olemas. Tingimused, erandid, JA/VÕI ning puuduv kontekst säilivad; sisend on veel kontrollimata otsinguabi. Allikast rikastamine, läbivaatamise/parandamise voog, kontrollitud teadmiste seis, kasutajafaktide ja ajapiiride sidumine ning päris DB/UI vastuvõtt on lahti. [Leping ja tehniline tõend](../rag-v2/adr-007-source-dependencies.md). |
