...
**Runtime `7c91fc9d` 11 juhtumi kordus jäi `PARTIAL`: isolated 3 PASS / 8 FAIL ja sequential 2 PASS / 9 FAIL; allikapaneele PASS-juhtumites ei avatud. Põhjusepõhine järelparandus kuulub praegusesse `main`-release'i, kuid selle funktsionaalne runtime on `NOT_PROVEN`, kuni küsimused on pärast deploy'd uuesti läbi küsitud.** Parandus ehitab numbrilepingu ainult lõplikult renderdatud sama dokumendi tõendist, seob arvsõnad ja käändelised kestused kohaliku mõõdikuga, eristab allika-aasta tõendiaastast, säilitab `üle`/`alla`/`umbes` kvalifikaatorid, kontrollib explicit-arvude tõendipoolseid tähendusi, kasutab vastuse järjekorrast sõltumatut üksühest sobitust ning jätab võrdse pealkirjapere ja puuduliku tõendi fail-closed. EstNLTK/Vabamorf jääb planner'ile eelnevasse recall-kihti ja lemma-FTS shadow'ks; korpust, ingest'i, retrieval-kanaleid, indekseid ega warmup'i ei muudeta. Muudetud koodifailide ESLint, `git diff --check`, `i18n:check` ja Next 16.2.10 tootmisbuild on rohelised; automatiseeritud sihttestid jäid käivitamata, sest selle jooksu täitmisreegel ei lubanud neid. Release-ahela järel kontrollitakse kohaliku, `origin/main`-i ja serveri SHA-d, teenuseid, HTTPS-i ning RAG health'i. Seejärel korratakse 11 juhtumit esmalt isolated-, siis sequential-režiimis koos avatud allikapaneelidega, kirjutatakse vastused jooksvalt vahelogisse ja tehakse pärast sequential-faasi kokkulepitud paus. Täieliku 75-küsimuse kordus ja 37 Golden-küsimuse lisavärav ei ole veel alanud.
