...
**75 küsimuse varasema 34 FAIL/PARTIAL juhtumi esimene Luna kordus runtime'il `bc6dbace` lõpetas isolated-faasi tulemusega 12 PASS / 3 PARTIAL / 19 FAIL; sequential-faasis on kirjas 25/34 ning enne järgmist küsimuseringi kehtib omaniku nõutud paus.** Kohalikus `main`-is olev uus paranduskandidaat viib EstNLTK lemmad ja liitsõnatüved varimõõtmisest päris leksikaalse recall'i kihti kõigi sõnade jaoks, säilitades dense-otsingule algse lause ja registrile täpse identiteedi kinnitamise. Autorirada lahendab mitmeosalise käändelise nime registri vastu ning annab täieliku aktiivsete tööde inventari deterministlikult; Tartu-laadne mitme KOV-i vaste peatub enne otsingut ja küsib linna või valla täpsustust. Mudel ei tohi enam deterministlikult `in_scope` sotsiaalvaldkonna küsimust hiljem teemaväliseks ümber klassifitseerida. Täpse pealkirja, mitmeslotilise faktilepingu, lokaalse anafoori, route-põhise validaatori, toetatud tekstiosa säilitamise ja tõendipuuduse allikapeitmise varasemad parandused jäävad koodi, kuid viimase runtime'i 19 FAIL-i tõttu ei nimetata kogu parandust tehtuks. Muudetud failide ESLint, Pythoni süntaks ja `git diff --check` on rohelised; tootmisbuildi ega uut küsimuste kordust pole selle kandidaadi vastu käivitatud. Kogu 75 küsimust ja eraldi 37 Golden-küsimust on **NOT_PROVEN**; järgmine samm on ülejäänud täpse dokumendi/faktiseose parandamine, seejärel uus Luna isolated- ja sequential-kordus ning kohustuslik paus.
