...
**Täieliku püsiva leksikaalindeksi release candidate on 23.08 puhtas `codex/repair-b` tööpuus lokaalselt valmis; push ja deploy on veel tegemata.** Vana 8000-lõiguline varuskann kulutas mõõdetud üldpäringus 5,50–7,29 s: Chroma lugemine 2,49–4,32 s ning Pythoni normaliseerimine ja skoorimine kokku 2,57–2,67 s. Atomaarne SQLite FTS5 indeks katab 49 727 aktiivset lõiku ja 6073 aktiivset registridokumenti, on 459 132 928 baiti ning ehitus võttis ohutus runtime'is 37,79 s. Sama üldpäringu värske protsessi leksikaalrada oli 397 ms ja kaks sooja korda 401/394 ms; kogu `/search` oli 1309/844/844 ms, `complete=true`, `partial=false`, `degraded=false`. Release-ülevaatus sulges enne deploy'd 30-sekundilise proksi vastu leitud vea: ingest, reindex, metadata patch ja tombstone ei oota enam 37,79-sekundilist täisehitust, vaid märgivad indeksi kohe stale'iks ja ajastavad ühe koondatud taustarefresh'i; startup-ehitus jääb sünkroonseks. Autor-, pealkirja-, V04 pika faktivormi ja Kuusalu rajal säilisid võrreldud tõendi- ja source-ID-d; V04 lühikese otsepäringu varasem sõnastustundlikkus jäi enne ja pärast samaks ning autentitud vestlus/allikapaneel on uue koodi vastu **NOT_PROVEN**. Lõpliku kandidaatpuu Pythoni kompileerumine, sihitud ESLint, i18n, diff-kontroll ja tootmisbuild on rohelised; automaatteste ei loodud ega käivitatud. Järgmine tööots on nimeline commit ning omaniku release-väravas push/deploy ja autentitud `/vestlus` kontroll; 54/75 mõõtmata juhtumit ei muutu selle tööga roheliseks.
...
**Püsiv leksikaalindeks on lokaalselt DONE, release/runtime PARTIAL.** Lai `corpus_scan` varurada
...
asünkroonselt uuesti ehitada. Ohutus runtime tõendas 49 727/6073 täieliku katte,
Unicode'i säilituse, Kuusalu/collection/audience/agent-doc-ID fail-closed filtrid ja puuduva indeksi
`LEXICAL_INDEX_MISSING` varuraja. Toodangu deploy, autentitud vestlus ning uue ingest'i ja
kustutuse päris runtime on veel **NOT_PROVEN**.
...
| 33 | **Püsiva FTS5 leksikaalindeksi release-värav.** Kohalik ohutus runtime katab 49 727 aktiivset lõiku ja 6073 aktiivset dokumenti; leksikaalrada 397/401/394 ms ning kogu `/search` 1309/844/844 ms. Enne DONE seisu on vaja nimelist commit'i, push/deploy'd, toodangu health'i, autentitud vestlust ja allikapaneeli ning päris ingest/reindex/tombstone sünkroonsuse kontrolli. V04 lühikese otsepäringu pärand-sõnastustundlikkus ja 54/75 mõõtmata juhtumit jäävad eraldi avatuks | vestlus / RAG jõudlus | SABA |
