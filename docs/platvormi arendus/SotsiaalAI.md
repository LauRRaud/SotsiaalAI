...
**34 FAIL/PARTIAL juhtumi põhjusepõhine järelparandus on 30.08 kohalikus `main`-puus valmis, kuid runtime on `NOT_PROVEN`.** EstNLTK analüüs oli juba õiges, retrieval'ile eelnevas kihis; põhiviga oli planeerija `requested_fact_slots` lepingu kadumine semantiliste kandidaatide ühendamisel. Lokaalne kandidaat säilitab küsitud faktipesad otsingus, dokumendiidentiteedis ja vastusekujus ning arvulised pesad faktivalidaatorini; loeb kooloni ja `§` toorsisendist; eristab artikli aastat otsuse, juhtumi või kohtumise aastast; suunab kompaktse, lepingulise arvufakti ja organisatsioonilt adressaadile antud soovituse dokumendipõhisele rajale; otsib nimetamata allikat piiratud teema + faktisuhete fingerprint'iga ning nõuab valikul teema ja faktide koosesinemist. Täielikkuse juhis rakendub nüüd kõigile mitme pesaga RAG-vastustele, olemasolev claim-taseme toetamata arvu eemaldamine ja KOV-i kahe allika komposiitleping jäid fail-closed. 75 juhtumi manifestis on endiselt 75 unikaalset ID-d, kuid parandatud on ebamäärased või valed J03/J05/J08–J14/J18/J22/V03–V06/M01–M02 sõnastused ja K05 ootus nõuab Tartu linna/valla täpsustust; V03 on 75 sees iseseisev ning teadlik J12→V03 anaforapaar kuulub eraldi multi-turn väravasse. Ajaloolisi vastuseid ega trace'e ei kirjutatud ümber. Muudetud koodifailide ESLint ja `git diff --check` on rohelised; build'i, commit'i, push'i, deploy'd ega runtime-küsimusi ei ole tehtud. Järgmine samm omaniku loal on commit → push → deploy, seejärel Luna isolated FAIL/PARTIAL kordus, sequential-kordus ja kokkulepitud paus.

**Uus RAG-paranduskandidaat on commit'is ja serveris runtime-SHA-l `c21821c2` (29.08).** Deploy läbis tootmisbuildi, migratsioonide eelkontrolli ning teenusevärava; frontend, RAG ja research-worker on aktiivsed, `/vestlus` vastab 200 ning RAG-i indeks on valmis. Parandus lisab EstNLTK lemmasid ja liitsõnatüvesid päris leksikaalsesse recall'i kihti kõigi sõnade jaoks, säilitab dense-otsingule algse lause, parandab käändelise autorinime registrivastet ja täielikku autorinventari, peatab mitme KOV-i ebaselguse täpsustusküsimusega ning välistab sotsiaalvaldkonna küsimuse vale teemaväliseks klassifitseerimise. Uue runtime'i 34 juhtumi Luna isolated-kordus lõpetas tulemusega **18 PASS / 1 PARTIAL / 15 FAIL** ja sequential-kordus tulemusega **19 PASS / 2 PARTIAL / 13 FAIL**; vastused ja piirangud on jooksvalt [vahelogis](../../eval/luna-retest-progress.md). K05 järelkontrollis lahendas täisvastus `Tartu vald` valiku, kuid lühivastused `linn` ja `vald` kordasid täpsustusküsimust. Põhjus oli täpsustuses pakutud kandidaatide kadumine assistendi töövoo metaandmetest, mitte EstNLTK lemmakiht; GitHubi `main` commit `c827e679` säilitab nüüd suletud valiku kandidaadid ja seob linna/valla lühivastuse enne KOV-tuvastust ainsa vastava täisnimega kõigi samanimeliste paaride korral. Kood, ESLint, i18n ja tootmisbuild on rohelised, kuid seda commit'i pole veel deploy'tud ning runtime jääb **NOT_PROVEN**. Sequential-faasi järel on omaniku nõudel paus; 75 küsimuse põhinimetaja ja 37 Golden-küsimuse värav jäävad seni **NOT_PROVEN**.
