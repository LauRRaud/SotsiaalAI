...
**05.09 uue RAG-i M0–M2.2 tehniline rada ja turvajärelparandused on `main`-is ning serveris:** kinnitatud ühe artikli piloodi 16 tekstiosa ja 9 küsimust said `text-embedding-3-large` vektorid 25/25 eduka katsega, 12 420 sisendtokeni ja arvestusliku 0,001614600 USD kuluga; kuue ET/EN/RU sisuküsimuse vajalik tugi oli hübriidraja lõppkontekstis 6/6. Lõplik 57-testine komplekt läbis kohalikult ja serveris 0 vea ning 0 skip'iga; Standard Security Scani kaks keskmist ja üks madal leid on parandatud, vana RAG ja research-worker on `inactive/disabled`, uus loopback PostgreSQL/Qdrant ning frontend töötavad. Täpne ulatus, piirid ja tõendid on [M0–M2.2 auditis](../audits/rag-v2-m2-2-audit-2026-09-05.md), kuupäevastatud seosekaart [RAG masteris](../audits/rag-susteem-master.md) ning käitusjuhis [README-s](../rag-v2/README.md). Järgmine plokk laiendab kvaliteedikatset mitmele eraldi lubatud pärisartiklile ja eksitavatele allikatele, enne kui M4 ühendab serverisessiooni õigused, kanoonilised viited, allikavaate ja eraldi kinnitatud Luna vastamise; ühe artikli üldine täpsus, M3 semantiline graaf, kümne aasta korpus ning kasutaja chat jäävad `NOT_PROVEN`/`retired`.
