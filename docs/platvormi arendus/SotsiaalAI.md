...
**20.09 mobiili kaardiakende laiused ja sisu — serveris (`49ec9ca9`).** Hinnastuse kaardid paiknevad madalamal. Sisu moodustab tasakaalustatud terviku: paketinimi, hind ja kirjeldus on eraldi vahedega ning nupp ja link paiknevad eeliste järel piiratud vahega. Sama paigutus mobiilis ja desktopis, seniste seadmekohaste kaardimõõtudega. Pikk sisu kerib kaardi sees. Omaniku uuendatud juhis: nende UI-paranduste järel kohe serverisse, eraldi teste ja kohalikku build’i ei käivitata; serveri vajalik build jääb juurutusse. Visuaalne kontroll `not_run`, omanik testib ise.
