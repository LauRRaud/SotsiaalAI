...
**RAG-i semantiliste lepingute diagnostiline release `6a7f0534` on 26.08 toodangus; kogu süsteem on endiselt PARTIAL.** Shadow-v2 säilitab planneri aastarollid ja arvuslotid provenance'iga trace'is, kuid ei otsusta tootmises. Renderdatud tõendist tuletatud `requested_metric_contract` lubab J08 generaatoril kasutada ainult nelja küsitud protsendislotti ning kitsas `numeric_relation_contract` säilitab J18 osalejarühma ja koguarvu seose; validator jäi fail-closed ja FAIL-i korral on `displayed_sources=[]`. Uus release parandab dokumendi-aasta kokkuvõtte nii, et see tuletatakse typed `year_role_mentions`-ist: mixed-year kontrollis jäi `2023. aasta artikkel` dokumendi-aastaks ja `2022. aasta uuring` tõendiaastaks, mõlemad eri mainimistena. Samas autentitud vestluses olid mixed-year vastuses 61/26/11/18 ja õige Vaike Vainu 2023 hover-allikakaart; requested-slot trace oli 4/4 proportion-slotiga, kuid ühes korduses jäi requested-metric mapping ebapiisavaks ja vana generaator lisas küsimata 2%/3%. J03 2018 ajalooline vastus oli sisuliselt õige, kuid allikas peideti põhjusega `historical_source_not_current_evidence`; seda ei loeta PASS-iks. Range aktiivse SHA vaade on seetõttu **DONE 0/75 · PARTIAL 0/75 · NOT_PROVEN 75/75** kuni samal SHA-l vajalikud maatriksijuhtumid uuesti täielikult läbivad. Ajalooline koond jääb **DONE 21/75 · NOT_PROVEN 54/75**. Järgmised tööd on requested-metric mappingu üldine stabiliseerimine, author/year/history precedence, legal/KOV ja supporting→displayed atribuutika; temporal välju ei promote'ita ega nimetata release'i FINAL_SHA-ks.
