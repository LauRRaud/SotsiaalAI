...
**`0e63f11b` korduse 9 FAIL / 2 PARTIAL juhtumi teine põhjuseparandus on 30.08 koodis valmis; runtime on `NOT_PROVEN`.** Parandus säilitab nime ja küsimuse pinnakuju, kuid lisab küsitud faktisuhetele EstNLTK lemma- ja tüvealternatiivid, eristab osalejate arvu summast, toob sama valitud dokumendi eri lõikudest kõik küsitud faktislotid vastusekonteksti ning ei kasuta otsitavate arvude sobivust enam dokumendi identiteedi tõendina. Täpse pealkirjaga küsimus ei käivita enam mürast faktifingerprint'i; J03 manifesti ekslik pealkiri on viidud vastavusse tegeliku artikliga. Sihitud ESLint, `i18n:check`, `git diff --check` ja tootmisbuild on rohelised. Järgmine samm on deploy'da muutumatu koodipuu ning korrata samu 11 juhtumit esmalt isolated-, siis sequential-režiimis.
