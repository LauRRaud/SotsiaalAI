...
**Mitmekeelne RAG on kaardistatud, kuid mitte veel valmis.** Praegu määrab `uiLocale` vastuse keele enne küsimuse tegelikku keelt, planner'i intentsõnavara on peamiselt eestikeelne ning sama algne tekst läheb nii dense- kui leksikaalotsingusse. Venekeelne Lauri kontroll leidis `text-embedding-3-large` dense-rajal 36 kandidaati, kuid BM25 andis 0, planner jäi üldrežiimi, kuvatud allikaid oli 0 ja vastus tuli eestikeelse kasutajaliidese tõttu eesti keeles. Järgmine keeleplokk peab hoidma eraldi `interface_language`, `query_language` ja `answer_language`, lukustama nimed/arvud/§-viited, tegema vene ja inglise küsimusest eestikeelse retrieval-variandi, otsima dense-rajal nii originaali kui tõlkega ning kasutama FTS5-le eestikeelset varianti. Korpus ja allikad jäävad algkeelde; eraldi kolme indeksit ei tehta.
...
| 34 | **ET/RU/EN keelepõhine RAG.** Ühine eestikeelne korpus vajab eraldi `interface_language`, `query_language` ja `answer_language` lepingut. Mitte-eestikeelsest küsimusest tuleb teha entiteete muutmata eestikeelne retrieval-variant; dense otsib originaali ja tõlkega, FTS5 eestikeelse variandiga ning vastus tuleb valitud lepingukeeles. Praegune vene kontroll jäi üldplannerisse, sai ainult dense-kandidaadid, 0 BM25 ja 0 kuvatud allikat ning vastas UI keeles eesti keeles | vestlus / RAG keel | SABA |
