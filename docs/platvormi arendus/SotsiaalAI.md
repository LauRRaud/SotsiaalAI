...
**RAG-i viie runtime-veaklassi põhjusepõhine koodirelease on `3303466a8affc74fc3bdce45f4da34fca8928129` (27.08) toodangus; kogu süsteem on endiselt PARTIAL.** Lõplikul runtime-SHA-l andis J08 kõik neli küsitud osakaalu `61 / 26 / 11 / 18`, lühike J18 Erle Eenmaa ankur vastas `5 + 5 + 5 = 15`, mõlemad töötajate sisulise toe küsimused läksid RAG-i, päris Rakvere kontaktiküsimus jäi kontaktiregistri rajale ning J03 vastas `112 / 1220` koos Külli Mäe 2018 allikaga. J18 ja kontaktiregressioon korrati 28.08 samas autentitud vestluses pärast viimast shared retrieval/history muudatust: J18 trace kinnitas `specific_research_fact`, täpse current-turn autoriankru `confirmed_exact`, õige Eenmaa 2022 `document_id`, validaatori PASS-i ning ühe valitud ja kuvatud allika; Rakvere trace kinnitas `municipality_contact_list`, kahe telefonikirje valideerimise ja kaks kuvatud ametliku kontaktilehe allikat. KOV-i sisuküsimuste 27.08 järelkorduses olid mõlemad trace'id `hadRagContext=true`, `hadServiceMapKovContactContext=false`, kontaktide arv 0 ning kummalgi kaks vastust toetavat ja kuvatud ajakirjaallikat. Mitmeaastane seltsiliste küsimus ei pärinud enam 2022/Rakvere konteksti: temporal-plaan sisaldas ainult aastaid 2018/2019/2020, kuid valitud ja renderdatud tõendis puudus võrreldav kolme aasta rida, mistõttu validator jäi põhjendatult top-level `cross_source_numeric_mix` / binding `temporal_year_value_not_answered` olekus fail-closed ning aastatrend on **NOT_PROVEN**. Runtime-kontrolli hetkel kattusid kood, server ja aktiivne frontend-artifakt SHA-l `3303466a`; kolm teenust olid aktiivsed, `/vestlus` vastas 200, RAG oli `ok=true` ning originaal- ja lemma-FTS olid `ready=true` 49 727 lõigu / 6073 aktiivse dokumendiga. Top-k-d, fusion'it, korpust, indeksit, env-i, embeddingut ega lemma promotion'it ei muudetud; automaatteste ega probe-/smoke-/E2E-radu ei loodud ega käivitatud. Kogu 75 juhtumi ring jääb `NOT_PROVEN`; järgmine kitsas tootearendus on tõendipuuduse kasutajateate ja selgitusküsimuse üldine leping nii, et valideerimata fakti fail-closed piir säilib.

**Vestlusliku RAG-taaste kohalik kandidaat on 28.08 koodi-commit'il `4cc4e65b`; tootmisesse ei ole seda veel viidud.** Kui sisemisest teadmusbaasist ei saa kasutaja küsimust turvaliselt lahendada, võib sama vastusemudeli ainus selle vooru kõne küsida ühe loomuliku täpsustuse; tavalisele vastusele ei lisandu teist mudelikihti ega teist mudelikõnet. Faktivalidaatori FAIL ei käivita uut kõnet, tehniline RAG-rike ja lihtne sotsiaalne vastus kasutavad null mudelikõnet ning kasutajale ei näidata RAG-i tehnilist veateksti. Jätk seotakse serveris sama lõpetatud kasutaja- ja assistendisõnumipaariga; kliendi saadetud recovery-seisu ei usaldata. Õigekirjaparandus on kitsalt kasutaja enda sõnadega seotud, iseseisev uus küsimus ei päri vana täpsustust ning selgitusküsimuse või valideerimata vastuse allikaid ei kuvata. Assistent ei otsi veebist ega täida tõendilünka mudeli üldteadmisega: infoallikaks jääb ainult platvormi sisemine korpus ja olemasolev fail-closed tõendileping. Scoped ESLint, i18n-kontroll, `git diff --check` ja Next 16.2.10 tootmisbuild olid rohelised; automaatteste, probe-, smoke- ega E2E-radu ei loodud ega käivitatud. Aktiivne tootmisrelease on endiselt `3303466a`; push, deploy ja autentitud käsikontroll on tegemata ning kogu uus runtime-käitumine ja 75 juhtumi ring jäävad `NOT_PROVEN`.
