...
**31.08 M01/M02 ja kahe Golden-juhtumi põhjuseparandused on omaniku loal väljastatud koodirelease'iga `95104077`.** Kood on `main`-is, GitHubis ja serveris; serveri tootmisbuild läbis, kolm teenust on aktiivsed, `/vestlus` vastab 200 ning RAG-i algvormi- ja lemmaindeksid on valmis. Lapse heaolu vana vastuse täpne kordusarvutus säilitab käsiraamatu ja eraldi mudeli käsitluse ning eemaldab dubleeriva tausta. Omanik tellis nüüd kõigi nelja parandatud juhtumi in-app korduse eraldi ja järjest, mille järel paus. Jooks on **NOT_STARTED: 0/8** — brauseritööriist ei käivitu Windowsi `CryptUnprotectData` ligipääsutõrke tõttu. Välise brauseri või API kaudu küsimusi ei asendata. Tõendid ja täpne järjekord on [Luna vahelogi viimases jaotises](../../eval/luna-retest-progress.md). Varasemad FAIL/PARTIAL hinded säilivad; värsked vastused ja allikapaneel on `NOT_PROVEN`. Vana ajapiiriga automaatjätk jääb PAUSED.
