...
**75 küsimuse 34 FAIL/PARTIAL korduse analüüsist tulnud RAG-i põhjuseparandus on 29.08 tehtud omaniku loal otse kohalikus `main`-is; runtime-kordus kuulub Lunale.** EstNLTK/Vabamorf säilitab pärisnime kanoonilise väärtusena küsimuse pindkuju ning annab lemmasid ainult otsingukandidaatideks; puhtad arvud, aastad ja vahemikud ei muutu enam nimeüksusteks. Täpselt tsiteeritud pealkiri on dokumendivalikus tugev identiteediankur, vastuse mõõdikukuju ei tohi dokumenti tuvastada ning trace näitab kandidaadi tegelikku valikujärjestust. Autorirada võtab registrist kanoonilise autorinime ja täieliku aktiivsete tööde `doc_id`-, pealkirja-, aasta- ning rubriigiinventari; genereerija saab kogu kinnitatud loendi, mitte ainult valitud katkendid. `RequestedFactSlot v2` säilitab samas küsimuses eraldi arvud, kestuse, meetodi, ajapunkti, jaotuse, asukoha, isikurolli, loendi ja eksplitsiitsete arvude seosed; koordineeritud „maakonnas ja omavalitsuses” ning individuaal-/rühmavestluste harud jagatakse eraldi slot'ideks ja range arvutõendus rakendub ainult toetatud arvulistele liikidele. Sama lause lokaalse antecedendiga „selles” või „neid” ei ava vestlusajaloo päringut. Mitme KOV-i vaste, näiteks Tartu linn ja vald, läheb olemasolevasse täpsustusküsimusse. Genereeritud bibliograafilised aastad ja sünteesikuupäevad ei käivita enam üldist täppisarvu validaatorit; KOV-i ja õiguse operatiivarvud jäävad fail-closed kontrolli alla ning toetamata arvuga lause eemaldatakse võimalusel nii, et ülejäänud tõendatud juhis säilib. Puhas tõendipuuduse vastus ei kuva suvalisi kontrollitud allikaid; mitme allika sünteesi puhul otsustab kuvatavad allikad väitepõhine tugi. Muudetud JS-failide ESLint, Pythoni süntaks ja `git diff --check` on rohelised; tootmisbuildi, küsimuste kordust ega runtime'i ei käivitatud ning automaatteste, probe-, smoke-, benchmark- ega E2E-radu ei loodud. Kogu 75 küsimuse ja seejärel 37 Golden-küsimuse autentitud sisuline kordus jääb kuni Luna mõõtmiseni **NOT_PROVEN**; 75/75 ega Golden-tulemust enne seda ei väideta.
