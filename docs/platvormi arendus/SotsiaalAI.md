...
| 31 | **30 lõpetamata vaate ühine disainimigratsioon.** 18 vaates puudub või killustub terviklik sisupind ja 12 vaates on kest olemas, kuid töövoo sisekompositsioon lõpetamata. Kujundus peab kasutama olemasolevaid `PanelFrame`/`Panel`/`GlassSurface`/`SubpageHeader` ja kontrolliprimitiive; puuduv ühine sektsiooni-, loendi- või olekumuster lõpetatakse jagatud kihis, mitte 30 kohaliku CSS-süsteemina. Vastuvõtt vajab iga rolli põhivoogu, 320/600/desktop konteinerit, suurt teksti, 200% reflow'd ning puute/klaviatuuri pariteeti | UI / kujundus | SABA |
