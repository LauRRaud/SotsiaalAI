...
**05.09 vana RAG-i eemaldamine ja dokumentatsiooni koristus on omaniku loal põhikausta `main`-harusse integreeritud. Serverisse pole seda paigaldatud.** Teadmuspõhine vestlus, analüüs ja AI-dokumendiloome on ümberehituse ajaks peatatud. [RAG master](../audits/rag-susteem-master.md) sisaldab nüüd ainult kasutajafunktsioonide, lehtede ja ühenduskohtade kaarti. Vana RAG-i eraldi arendusmaterjalid, hindamispaketid ja automaattestid on eemaldatud; ajalooline kirjeldus on Git-sildi `before-rag-rebuild` juures. Algmaterjalid, ajalugu, õiguste andmed ja admini käsitsi enesetest säilivad. Järgmine samm on uue RAG-i kavandamine masteris kirjeldatud kasutajafunktsioonide ja ühenduskohtade põhjal; autenditud admini-, privaatsus- ja andmerajad on endiselt `NOT_PROVEN`.
