...
**07.09 siht on kogu töötav ja müüdav GraphRAG-toode, mida kasutatakse ja katsetatakse SotsiaalAI-s; ühine loogika peab sobima eri valdkondadele ja kliendispetsiifiline sisu on kohandatav.** Dokumendist allikakohtadega väite-/seosemustandi koostamine, haldaja valiku salvestamine uue versioonina ja lahendamata sõltuvuste säilimine otsingus on serverisse juurutatud; sihttestid ja tootmisbuild läbisid. Dokumendi vastuvõtu vorm on piiratud adminiseadistusega avatud, mudeliga koostamine veel välja lülitatud. Järgmine samm on ühe dokumendi koostamise → salvestamise → indeksi päris DB/UI/mudeli kontroll, mille ühe päringu kulupiir on ette valmistatud; seejärel jätkatava mahutöötluse ja teekaardi ülejäänud võimete arendus. Etappide seis on S2 RAG-lõigus; [koostamise leping ja tõend](../rag-v2/adr-008-source-knowledge-preparation.md).
...
| M3: sisuline sõltuvuskiht | **Osaliselt teostatud.** Ankurdatud väited/sõltuvused, indeksiobjektid ja piiratud graafiotsing on serveris. Juurutatud on ka mudeliga teadmismustandi koostamine, haldaja valiku läbivaatus/salvestus ning indekseeritud lahendamata sõltuvused; sihttestid ja lõpuvärav läbisid. Koostamine pole serveriseadistuses veel avatud ning selle päris DB/UI/mudeli tervikahela vastuvõtt on pooleli. Kandidaadid jäävad kontrollimata otsinguabiks. Mitme dokumendi automaatne seostamine, sisuline kinnitamine, kasutajafaktide/ajapiiride sidumine ja mahuka koostamise jätkamine on lahti. [Algne sõltuvusleping](../rag-v2/adr-007-source-dependencies.md), [Koostamise leping ja tõend](../rag-v2/adr-008-source-knowledge-preparation.md). |
...
1. **M3 teadmiste ettevalmistus ja töötav tervikahel:** olemasoleva ankurdatud sisendlepingu peale üldine allikast rikastamise, läbivaatamise ja parandamise voog ning sõltuvuste tegelik kasutamine indeksi, otsingu ja vastamise kaudu SotsiaalAI-s. Algtekst ja tuletatud otsinguabi jäävad eraldi, kontrollimata seos ei muutu automaatselt kinnitatud faktiks. Teostatud laiendus säilitab seose suuna ja JA/VÕI; päris DB/UI vastuvõtt ning kontrollitud sõltuvuste, kasutajafaktide ja ajapiiride sidumine on lahti.
2. **Korpuse maht ja käitamine:** päringusse ainult vajalikud indeksiüksused ja sõltuvused, mitte kogu korpuse korduv valideerimine/laadimine. Suurte importide osadeks jagamine ja jätkamine tuleb teha enne praeguse 5000 tekstiosa piiri muutmist. Versioonitud paigaldus/uuendus, õiguste eraldatus, taastamine ja kuluarvestus peavad toetama kogu toote kasutamist.
