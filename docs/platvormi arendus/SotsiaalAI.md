...
**Luna kordusprotokoll:** esmalt käib iga praegune FAIL/PARTIAL juhtum värskes eraldiseisvas vestluses (`isolated`), seejärel samad juhtumid ühes vestluses algses järjekorras (`sequential`). Juhtumipõhine võrdlus eristab küsimuse enda RAG-/valideerimisvea ajalookonteksti lekkest. Kõiki 75 küsimust ei käsitleta ühe vestlusena; sama vestlust nõudvad jätkuküsimused jäävad omaette selgelt määratud paaridesse või plokkidesse. Pärast paranduste väravat kordab Luna eraldi ka 37 `eval/golden-rag-v1.json` Golden-küsimust.
