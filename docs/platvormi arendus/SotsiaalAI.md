...
**UI responsiivsuse esimene parandusetapp on 23.08 `main`-i integreeritud, kuid runtime'is ja toodangus tõendamata.** Väikeses aknas jääb vestluse komposer koodilepingu järgi sõnumialast ette; juurfont austab brauseri baassuurust, põhinuppudel on skaleeruv puutesiht, kitsad tööpinnad reflow'vad oma konteineri järgi ning login ja modaalid kasutavad dünaamilist nähtavat kõrgust. Modaalidel on ühine klaviatuuri- ja sulgemisrada ning dokumendimustandi väljadel label'id. Lint, tõlkekontroll, diff-kontroll ja tootmisbuild on rohelised; automaatteste ei loodud ega käivitatud. Järgmine UI-plokk on käsitsi runtime-maatriks ning seejärel 30 lõpetamata vaate kujundus ühiste olemasolevate primitiivide peale, mitte lehekohaste süsteemidena.
...
**Mitmekeelse RAG-i esimene kitsas tootmisplokk on 23.08 rakendatud RAG-loogika SHA-l `c9672a05`; üldvärav jääb PARTIAL.** `interface_language`, `query_language`, `retrieval_language` ja `answer_language` on nüüd trace'is eraldi ning küsimuse ega eestikeelse retrieval-variandi teksti ei logita. Konservatiivne vene- ja ingliskeelne täisnimega autoriküsimus säilitab nime, aktiveerib eestikeelse metadata- ja hübriidotsingu ning vastab küsimuse keeles. Ingliskeelne Laur Raudsoo kontroll võttis 13,60 s, valis ja kuvas samad viis autorallikat; venekeelne eakate-teemaline kontroll võttis pärast teemakitsendust 12,45 s, valis ja kuvas samad kolm temaatilist autorallikat ning avatud paneel näitas Maardu 2019, tehisintellekti 2025 ja Padise 2017 allikat. Esimene päring pärast restarti kulutas retrieval'ile 7,93 s; varasem soe sama raja mõõtmine oli 1,51 s, seega külma/sooja hajuvus jääb jõudlusriskiks. Funktsioonilüliti `RAG_MULTILINGUAL_AUTHOR_RETRIEVAL_ENABLED=0` taastab vana raja; serveri env-i ei muudetud. Üldised vene/inglise sünteesi-, KOV-, õigus-, arvufakti-, segakeele- ja jätkuküsimused on endiselt `NOT_PROVEN`; kolme eraldi indeksit ei tehta ning 75 eestikeelse juhtumi arvestus jääb **DONE 21/75 · NOT_PROVEN 54/75**.
...
| 34 | **ET/RU/EN keelepõhine RAG.** SHA `c9672a05` eristab keelelepingu väljad ja aktiveerib konservatiivse RU/EN täisnimega autoriraja: inglise kontroll valis/kuvas 5 allikat ning vene eakate-teemaline kontroll 3 temaatilist allikat; vastused tulid küsimuse keeles ja vene vastuse allikapaneel avati. Üldine tõlkekiht ning sünteesi-, KOV-, õigus-, arvufakti-, segakeele- ja jätkuküsimuste pariteet on veel mõõtmata; feature flag võimaldab kitsa raja välja lülitada | vestlus / RAG keel | PARTIAL |
