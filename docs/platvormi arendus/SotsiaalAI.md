...
**Hinnastuse mobiilikaardid on lokaalses `main`-is joondatud ja puhastatud (26.08):** avalehel paiknevad heli- ja „Jäta vahele” nupud samal keskjoonel; hinnastuse päisest on eemaldatud registreerumise teade ning paketi täisvõrdlusest pealkiri „Kõik selle paketi võimalused”. Funktsiooniridade kiri on 0,92 rem ja hõredama tähevahega. Mobiilivaate lähtekontroll ning muudetud komponendi eslint ja `git diff --check` on rohelised; täisvõrdluse avamine selles brauseriseansis jäi **NOT_PROVEN** ning uut tootmisdeploy'd ei tehtud. Järgmine samm on omaniku korraldusel deploy-järgne mobiilikontroll.
