...
**05.09 uue RAG-i M2.1 kohalik otsingutaristu on `main`-is teostatud; tootmisse pole paigaldatud.** Haldaja M1 näidis liigub eraldatud PostgreSQL-i ja Qdranti kaudu otsingusse ning CLI tagastab algteksti, täpsed PDF-lehed, päritolu ja valiku põhjused. Õiguse tühistamine, katkestatud indekseerimine ning vana/uue põlvkonna eraldatus on kontrollitud päris kohalike teenustega; Windowsi M1 parserikrahh parandati eraldi lapsprotsessiga. M1 22 testi ja M2.1 18 testi (7 ühik-, 11 integratsioonitesti), lint, kohaliku Prisma skeemi valideerimine, tõlkekontroll ning tootmisbuild läbisid, skip oli 0 ja välismudelikutseid 0. Omaniku soovil on kuupäevastatud koondseis ja funktsioonide seosekaart [RAG masteris](../audits/rag-susteem-master.md); käitus- ja kontrollikäsud on [README-s](../rag-v2/README.md), otsinguleping [ADR-002-s](../rag-v2/adr-002-local-hybrid-search.md). Järgmine plokk on M2.2 tegeliku embedding-mudeli otsingukvaliteedi katse pärast konkreetsete materjalide väljasaatmise ja kulupiiri kinnitamist; privaatne prooviplaan on valmis, rahaline hind seadistamata. Praegused vektorid on ainult `mock`: semantiline kvaliteet, HTTP-autentimine, Luna vastamine, M3 semantiline graaf ja kümne aasta korpus on `NOT_PROVEN`; chat ning admini käsitsi enesetesti `retired` olek säilivad. Edasine töö toimub otse `main`-is.
