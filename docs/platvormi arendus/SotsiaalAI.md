...
**Hinnastuse kaartide tüpograafia on toodangus SHA-l `702a6ab6` (26.08); järgmine kohendus on lokaalses `main`-is:** paketi „Vali …” nupp kasutab taas platvormi tavalist sisupõhist laiust, mitte 92% kaardilaiust, ning avatud funktsiooniridade kiri on 18 px (1,125 rem) / 0,025 em. Avalehe heli- ja „Jäta vahele” nupud paiknevad samal keskjoonel; hinnastuse päisest on eemaldatud registreerumise teade ning paketi täisvõrdlusest pealkiri „Kõik selle paketi võimalused”. Desktopil mõõdeti nupp 158 px laiuseks 345 px kaardil ja 390 px mobiilivaates 159 px laiuseks 312 px kaardil; avatud loendi kiri oli 18 px / 0,45 px ning horisontaalset ülevoolu ei olnud. `git diff --check` on roheline; uut tootmisdeploy'd sellele kohendusele ei ole veel tehtud.

**RAG-i viimane deployitud koodirelease on `0335fafe1f16034244134a5953de2a85f5d5a87d` (26.08); kogu süsteem on endiselt PARTIAL.** See release sisaldab bounded-episode'i koordinatsiooniküsimuse morfoloogilist parandust, temporal production-closure'i ning current-turn document-identity shadow-v1 kandidaatkihti; `requested_metric_contract` ja J18 relation-contract on tootmises ning validator jäi fail-closed. Deploy-järgselt kattusid kohalik `main`, `origin/main` ja server sama SHA-ga; aktiivne frontend-artifakt oli `frontend-current-20260826T134404Z-0335fafe.tar.gz`, frontend/RAG/research-worker olid aktiivsed, `/vestlus` vastas 200 ning RAG health oli `ok=true`, FTS5 `ready=true` (49 727 lõiku, 6073 aktiivset registridokumenti). Temporal production-closure on serveri PostgreSQL run-trace'i põhjal **READY**: tavaline 2018–2020 küsimus valis `queryPlan.mode=temporal`, `period_role=breakdown`, `bounded_episode_metric_fact=not_applicable`, `breakdown_years=[2018,2019,2020]`, `temporal_query_contract=temporal_query_contract_v1`, `query_order=temporal_year_queries` ja `selection_strategy=temporal_year_coverage`. Faktivalidaator jäi eraldi `cross_source_numeric_mix` põhjusega fail-closed. Eelmise täpse J07 allikapaneeli avanemise `NOT_PROVEN` on UI/source observability piir, mitte temporal blocker; `FINAL_SHA` ega 75/75 staatust ei anta. Harul olev current-turn author metadata confirmation shadow-v1 laiendus ei ole veel deployitud ja tootmiskäitumist ei muuda; järgmine töö on selle runtime-varjutõendi kogumine enne author identity promotion'i.
