...
**RAG-i viimane deployitud koodirelease on `db119fa237447baa66a9782d2a0efa90429abcd0` (26.08); kogu süsteem on endiselt PARTIAL.** See release sisaldab bounded-episode'i koordinatsiooniküsimuse morfoloogilist parandust; `requested_metric_contract` ja J18 relation-contract on tootmises ning validator jäi fail-closed. Deploy-järgselt kattusid kohalik `main`, `origin/main` ja server sama SHA-ga; aktiivne frontend-artifakt oli `frontend-current-20260826T112937Z-db119fa2.tar.gz`, build-ID `pJ8sPbS1iDBEGfxvHiu5S`, frontend/RAG/research-worker olid aktiivsed, `/vestlus` vastas 200 ning RAG health oli `ok=true`, FTS5 `ready=true` (49 727 lõiku, 6073 aktiivset registridokumenti). Temporal production-closure on serveri PostgreSQL run-trace'i põhjal **READY**: tavaline 2018–2020 küsimus valis `queryPlan.mode=temporal`, `period_role=breakdown`, `bounded_episode_metric_fact=not_applicable`, `breakdown_years=[2018,2019,2020]`, `temporal_query_contract=temporal_query_contract_v1`, `query_order=temporal_year_queries` ja `selection_strategy=temporal_year_coverage`. Faktivalidaator jäi eraldi `cross_source_numeric_mix` põhjusega fail-closed. Eelmise täpse J07 allikapaneeli avanemise `NOT_PROVEN` on UI/source observability piir, mitte temporal blocker; `FINAL_SHA` ega 75/75 staatust ei anta.
