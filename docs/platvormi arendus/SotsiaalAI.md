...
**Hinnastuse mobiilikaardid on toodangus SHA-l `b2a0cde0` (26.08):** avalehel paiknevad heli- ja „Jäta vahele” nupud samal keskjoonel; hinnastuse päisest on eemaldatud registreerumise teade ning paketi täisvõrdlusest pealkiri „Kõik selle paketi võimalused”. Funktsiooniridade järgmine kohalik tüpograafiakohendus on 1 rem / 0,022 em ning „Vali …” nupud on 1 rem / 0,055 em ja 92% laiused. Mobiilivaate lähtekontroll ning muudetud komponendi eslint ja `git diff --check` on rohelised; täisvõrdluse avamine selles brauseriseansis jäi **NOT_PROVEN** ning uut tootmisdeploy'd sellele kohendusele ei tehtud. Järgmine samm on omaniku korraldusel deploy-järgne mobiilikontroll.
