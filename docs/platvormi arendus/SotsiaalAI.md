...
**Lapse heaolu kahe PARTIAL-vastuse allikaseose parandus on main-i integreeritud ja serveriuuendus omaniku loal töös.** Salvestatud täisvastuste taasesituses säilib nimetatud mudeli artikkel ning üldise küsimussõna tõttu lisandunud taustaallikas kaob; 24 sihttesti, lint ja tootmisbuild läbisid. [Diagnoos ja kontrollitõend](../../eval/luna-retest-progress.md). Viimane päris vestluse valim jääb **6 PASS / 2 PARTIAL / 0 FAIL (8 vastust)**, mitte kogu75/37 ega10/10 tõend. Järgmine samm on uue serveriversiooni lapse heaolu küsimus eraldi ning sama kolme eelküsimusega järjest; vastused ja paneelid salvestatakse jooksvalt. Ajastatud automaatjätk PAUSED.
