...
**07.09 dokumendi ja metadata lisamisvoog on serveris; tervikahela katse ootab kasutaja sisselogimist avatud Chrome'i vaates.** Admin saab valida PDF-i, importida või täita metadata ning vaadata läbi taastatava teksti/päritolu/hoiatuste ülevaatuse. [Kohalik päris PDF-i vastuvõtt](../audits/rag-v2-admin-intake-local-2026-09-07.md) läbis; serveri eelkontrollis parandati päris vektorikataloogi ühendusviga ja koodi `5e1d133c4` tootmisbuild läbis. Omaniku luba push'iks/deploy'ks, privaatse piloodiseadistuse kontrolliks ning ühe küsimuse embedding'u ja ühe vastuse katseks kuni 0,05 USD on antud. Dokumendivektorid peavad täielikult korduskasutusse tulema; serveri adminivoog on selleks piiratud nullise mudelikutsete eelarvega. Serveris ingest → indekseerimine → värske otsing → assistendi juhis/allikad → nähtav vastus/algallikas on veel `NOT_PROVEN`, uusi mudelikutseid pole tehtud. M4 tõendiosade kandidaat jääb katsevariandiks; T1/T5, F1/F4 ja isikupiiri semantilised lahtised otsad säilivad.
