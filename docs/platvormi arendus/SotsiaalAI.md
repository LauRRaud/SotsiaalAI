...
**Lapse heaolu kahe allikaseose vea järelparandus on integreeritud; serveri paariskordus on järgmine samm.** Jaotisesildid eristatakse sisuväidetest ning nimelise mudeli sõnajärge ja käändevormi kontrollitakse lõppvastuses piiratud EstNLTK analüüsiga. Mõlema senise vigase vastuse kohalik taasesitus läbis; runtime-hinnet sellega ei asendata. [Tõendid ja jooksvalt täienevad täisvastused](../../eval/luna-retest-progress.md). Omanik tellis pärast deploy'd ning isolated/sequential-kontrolli uue ülesande 75 täiesti uue, RAG-materjalidel põhineva küsimuse koostamiseks. Kogu75/37 ja10/10 on tõendamata; ajastatud automaatjätk jääb PAUSED.
