...
| M3: sisuline sõltuvuskiht | **Osaliselt teostatud kohalikus koodis.** Valikuline väidete/sõltuvuste metadata, täpsed PDF-ankrud, indeksiobjektid ja eraldi piiratud sõltuvusotsing on olemas. Tingimused, erandid, JA/VÕI ning puuduv kontekst säilivad; sisend on veel kontrollimata otsinguabi. Allikast rikastamine, läbivaatamise/parandamise voog, kontrollitud teadmiste seis, kasutajafaktide ja ajapiiride sidumine ning päris DB/UI vastuvõtt on lahti. [Leping ja tehniline tõend](../rag-v2/adr-007-source-dependencies.md). |
