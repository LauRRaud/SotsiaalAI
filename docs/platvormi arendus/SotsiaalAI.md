...
**P0/F04/F05 ja F06 on main'i integreeritud; arendus jätkus repair-a harus F07 esimese plokiga.** Üks täpse autori ja sõltumatu sisuteksti teemaga kinnitatud teos saab nüüd dokumendiluku pealkirja või ilmumisaastat juurde nõudmata; mitme sobiva teose puhul ei otsusta skoor kasutaja eest. [F07 teostus ja piirid](../audits/rag-susteem-master.md#139-autor-ja-sõltumatu-sisuteksti-teema--f07-esimene-kohalik-plokk), [kontrollitõendid](../../eval/rag-uus75-kontroll-2026-08-31.md). Järgmisena: vestluse/revisjoniga seotud teosevalik („teine/mõlemad”), eraldi loendusühikud ja F08 järjekorraseosed. F07 pole veel main'i integreeritud; serveripaigaldust ega migratsiooni selles töös ei tehtud. Kohaliku `RagAttempt` tabeli puudumise tõttu on autenditud katserada, serverivastus ja produktsioonivalmidus endiselt `NOT_PROVEN`.
...
**31.08 jätkuparandus seob kohalikus `main`-puus F05 teadaoleva protsendi sihtrühma, vanuse, tähenduse ja sagedusega ning eraldi viidatud uuringuperioodiga; serveris seda veel pole.** Õige arv või üks õige lause ei saa enam selle piiratud kontrolli sees peita valet nimetajat ega vastuolulist lisaväidet; jälg eristab ka tõendi puudumist ja vastuolu. Seotud sihttestid ja lõplik tootmisbuild läbivad. [Tõendid ja piirid](../../eval/rag-uus75-kontroll-2026-08-31.md), [tehniline leping](../audits/rag-susteem-master.md#137-teadaoleva-protsendi-tähendus-ja-viidatud-uuringuperiood--kohalik-f05-parandus). Varasem kohalik F04 ning katsete elutsükkel/shadow säilivad; DB-migratsioon ja autenditud runtime on `NOT_PROVEN`, väljalase vajab omaniku selget luba. Järgmine sidus plokk on F06 jaotuse/loetelu tõendileping, seejärel F07 autorivalik/loendused ja F08 järjekord. Muutmata baasi 1 PASS / 9 FAIL ei ole uue koodi tulemuseks ümber nimetatud.

**31.08 RAG-i kaks tõendatud F02 põhjust on `main`-is ja serveris parandatud: mõõtmisperiood ei tõrju enam artikli ilmumisaastat välja ning protsendivahemiku mõlemad piirid säilivad faktitoe kontrollis.** Kaks sõnastust eraldi ja samas vestluses andsid 4/4 õiget 40–60% vastust koos avatud õige allika ja mõlema piiri tõendiseosega. Automaataruanne näitab nüüd ka faktide sidumise täielikkust, põhjust ja kontrollitud vahemikupiire. [Testitõendid](../../eval/rag-uus75-kontroll-2026-08-31.md), [leping ja piirid](../audits/rag-susteem-master.md#135-vastuse-diagnostika-ja-automaatne-vestlusaruanne). Järgmine plokk on ülejäänud manifestijuhtumite kontroll; F01 varasem PARTIAL jääb alles, üldviga sellest pole tõendatud. Kogu75/37 ja alati automaatselt tuvastatav algpõhjus ei ole tõendatud.

**Lapse heaolu allikaseose järelparandus on serveris ja in-app paariskordus läbis: 2 PASS / 0 PARTIAL / 0 FAIL.** Samas järjestikuses vestluses olid ka M01, M02 ja integreeritud teenused sisuliselt PASS. Käsiraamat ja mudeliartikkel säilisid mõlemas sihtvastuses, üleliigsed taustaallikad kadusid; isolated/sequential tulemus ei erinenud. [Täisvastused, paneelid ja võrdlus](../../eval/luna-retest-progress.md). Järgmine omaniku tellitud töö on eraldi värskes ülesandes 75 täiesti uue, aktiivse RAG-korpuse materjalidel põhineva küsimuse koostamine koos allikatõendite ja hindamiskriteeriumidega; küsimusi seal veel ei käivitata. Kogu75/37 ja10/10 on tõendamata; ajastatud automaatjätk jääb PAUSED.
...
**34 FAIL/PARTIAL juhtumi põhjusepõhine järelparandus on 30.08 toodangus koodirelease'il `0e63f11b`, kuid RAG-i kvaliteet jääb PARTIAL.** Planner'i `requested_fact_slots` säilib nüüd semantiliste kandidaatide ühendamisel, nimetamata dokumendi valikul kasutatakse piiratud teema + faktisuhete fingerprint'i, artikli aasta eristatakse otsuse või juhtumi aastast ning koolon ja `§` loetakse toorsisendist. Parandatud 75-manifestis on endiselt 75 unikaalset ID-d; ajaloolisi vastuseid ega trace'e ei kirjutatud ümber. Release'i local/origin/server SHA kattus, kolm teenust olid aktiivsed, `/vestlus` vastas 200, RAG health oli `ok=true` 49 727 vektori / 6089 dokumendiga ja lemma-FTS `ready=true` EstNLTK/Vabamorfi `1.7.5-v2` indeksiga. Varasema isolated/sequential FAIL/PARTIAL ühendist korrati 16 juhtumit: isolated ja sequential andsid mõlemad **5 PASS / 2 PARTIAL / 9 FAIL**; hinde muutus oli `0/16`. PASS olid J09, J12, J22, M01 ja K01; PARTIAL J13 ja M02; FAIL J03, J05, J08, J11, J14, J18, V04, V05 ja V06. V05 näitas kõige selgemalt sõnastusvariandi riket: sama vestluse J22 vastas sama dokumendi kohta õigesti, kuid V05 ei taaskasutanud tõendatud konteksti. Täpsed vastused on [Luna vahelogis](../../eval/luna-retest-progress.md). Omaniku nõudel on nüüd paus; kogu 75 ja 37 Golden-küsimust on `NOT_STARTED`.

**Eelmine baasruntime `c21821c2` (29.08) andis 34 juhtumi isolated-korduses 18 PASS / 1 PARTIAL / 15 FAIL ja sequential-korduses 19 PASS / 2 PARTIAL / 13 FAIL.** Seega oli selles laiemas jooksus ajalooefekt olemas: sequential parandas osa juhtumeid, kuid muutis teisi PARTIAL-iks. EstNLTK lemma- ja liitsõnatüved olid juba retrieval'ile eelnevas recall-kihis; 30.08 süvadiagnoos näitas, et ülejäänud tõrgete põhjus oli pigem faktilepingu kadumine, vale dokumendiidentiteet või vastuse tõendikate, mitte küsimuse üksiku sõna kääne. K05 `linn`/`vald` lühivastuse metaandmeparandus sisaldub nüüd toodangu koodirelease'is `0e63f11b`, kuid selle konkreetne järelpäring ei kuulunud 16 juhtumi ühendisse ja jääb uuel release'il eraldi `NOT_PROVEN`.

**Luna kordusprotokoll:** esmalt käib iga praegune FAIL/PARTIAL juhtum värskes eraldiseisvas vestluses (`isolated`), seejärel samad juhtumid ühes vestluses algses järjekorras (`sequential`). Mõlema faasi iga vastus ja trace lisatakse kohe [Luna vahelogisse](../../eval/luna-retest-progress.md); tulemusi ei hoita ainult jooksva agendi mälus. Pärast `sequential`-faasi tehakse paus ja oodatakse uut jätkuluba — kogu 75 küsimust ei alustata automaatselt. Juhtumipõhine võrdlus eristab küsimuse enda RAG-/valideerimisvea ajalookonteksti lekkest. Kõiki 75 küsimust ei käsitleta ühe vestlusena; sama vestlust nõudvad jätkuküsimused jäävad omaette selgelt määratud paaridesse või plokkidesse. Pärast 75 küsimuse väravat kordab Luna eraldi ka 37 `eval/golden-rag-v1.json` Golden-küsimust.
...
**Inimlik vestluskiht on samal koodirelease'il `44c1e59b` tootmises ja käsitsi autentitud.** Tõendatud tavavastus kasutab endiselt üht mudelikõnet; iga küsimuse ette ega järele teist assistendikihti ei lisatud. Kui sotsiaalvaldkonna küsimuse tähendus jääb ebaselgeks, võib sama vooru ainus vastusemudelikõne küsida ühe loomuliku täpsustuse: kirjavigane „Kuidas saada sotsiaalvaldkonnas koxrabi?” sai küsimuse „Mida sa sõna „koxrabi” all mõtled?” ning vastus „Mõtlesin toimetulekutoetust.” viis järgmises voorus sisemise teadmusbaasi toetatud vastuseni koos ühe allikaga. Sotsiaalvaldkonnast väljas olev „Mis ilm homme Tallinnas on?” sai täpselt „Vastan ainult sotsiaalvaldkonna küsimustele.”, null mudelikõnet ja null kuvatud allikat. Recovery-jätk seotakse serveris sama lõpetatud kasutaja- ja assistendisõnumipaariga; kliendi recovery-seisu ei usaldata, iseseisev uus küsimus ei päri vana täpsustust ning validaatori FAIL ei ava korpusevälist teadmist. Assistent ei otsi veebist ega kasuta OpenAI üldteadmisi vastuse täitmiseks: infoallikaks jääb ainult platvormi sisemine korpus ja fail-closed tõendileping.

Lõppväravas kattusid kohalik kood, `origin/main`, puhas server ja aktiivne frontend-artifakt SHA-l `44c1e59b`; artefakt oli `frontend-current-20260828T133845Z-44c1e59b.tar.gz`. Frontend, RAG ja research-worker olid aktiivsed, loopback- ja avalik `/vestlus` vastasid 200, RAG health oli `ok=true` 49 727 vektori / 6089 registrikirjega ning originaal- ja lemma-FTS olid `ready=true` 49 727 lõigu / 6073 aktiivse dokumendiga ja registriga samal põlvkonnal. Lõpliku muutumatu koodipuu `i18n:check`, muudetud failide ESLint, `git diff --check` ja Next 16.2.10 tootmisbuild olid rohelised. Top-k-d, fusion'it, korpust, indeksit, env-i, embeddingut ega lemma promotion'it ei muudetud; automaatteste, probe-, smoke-, benchmark- ega E2E-radu ei loodud ega käivitatud. ET/EN/RU taastamise täismaatriks, tehnilise RAG-rikke, kriisiraja, paralleeljätku ja kogu 75 juhtumi kordus jäävad `NOT_PROVEN`; dokumentatsiooni-only HEAD liigub runtime-koodi SHA-st edasi ilma uut koodideploy'd nõudmata.
...
**Vestluse prompt cache'i variandipõhine võti on 23.08 koodi-SHA-l `dfc04a29` toodangus; cache-read/write järelmõõt on veel `NOT_PROVEN`.** Deploy-eelses 23.08 mõõdus oli cache-read 158 528, cache-write 120 097 ja uncached 309 282 tokenit ehk sama päeva read/write suhe umbes 1,32. Koodianalüüs näitas, et eksplitsiitne breakpoint piiras cache'i juba ainult stabiilse süsteemipromptiga, kuid `prompt_cache_key` jagas identse prefiksi lisaks kasutaja ID järgi. Uus `v2` võti eristab ainult süsteemiprompti päriselt muutvaid rolli, vastuse keele ja kriisirežiimi variante. Kasutaja küsimus, vestlusajalugu, värske RAG-kontekst ja allikad jäävad breakpoint'i järel cache'ist välja; RAG-otsing, planner, mudel, top-k ja vastuse leping ei muutu. Üle umbes 15 päringu minutis ühe võtme kohta tuleb liiklus jagada stabiilsetesse ämbritesse; praeguse mahu jaoks lisajaotust ei tehta. Scoped lint, diff-kontroll, i18n-kontroll ning kohalik ja serveri tootmisbuild olid rohelised. Deploy järel olid `origin/main` ja puhas server SHA-l `dfc04a29`, kolm teenust aktiivsed, `/vestlus` vastas 200, serverikoodis oli `v2` võti ja kasutajapõhine partition puudus; RAG health jäi 49 727 / 6089 ning FTS5 `ready=true`. Järgmine tõend on päris liikluse cache-read/write kordusmõõt.

**RAG-i külmkäivitus ning serveri mälu- ja kettakasutus on 23.08 põhjusepõhiselt optimeeritud; RAM-i ei suurendatud.** Sisulise RAG-i baasseis on `815f15f6` ja mõõdetud jõudlus-release `5796178f`. Tootmiskujul startup-warmup vähendas esimese lihtsa autoripäringu retrieval'i 9,94 sekundilt 2,43 sekundile ning dense-aja 7,84 sekundilt 0,263 sekundile; brauserivastus saabus 7,10 sekundiga. Deploy peatab build'i ajaks frontend'i, RAG-i ja research-workeri, jättes mõõdetult 5,3 GiB mälu vabaks. Omaniku säilitusotsuse järel eemaldati kõik aegunud frontend- ja RAG-backup'id, vana RAG-puu, Playwrighti ning build-cache'id ja tõendatud paketi-/auditijäägid; alles on üks kontrollitud RAG-snapshot ja üks värske kontrollitud andmekoopia iga muutuvat serveriandmestikku kasutava lehe kohta. Juurketta kasutus langes 96%-lt 57%-ni ja vaba ruum kasvas umbes 2 GiB-lt 21 GiB-ni; journal on piiratud 512 MiB / 14 päevaga, PM2 logid roteeruvad ning `vm.swappiness=10`. Kõik SotsiaalAI, RAIO, Avasta, Marta Raudsoo, Beyondframesi ja Kaljo Simsoni tootmisrajad vastasid pärast puhastust 200-ga. V04 kuue tõendipäringu retrieval on veel 10,49 s: suurema paralleelsuse katse aeglustas selle 17,06 sekundini ja võeti tagasi, kuid vastus säilitas 10%/640, 6%/227, 2%/100 ning avatud paneel näitas Anu Lepsi 2025 artiklit. Korpust, aktiivset indeksit ega env-i ei muudetud. Range seis on endiselt **DONE 21/75 · PARTIAL 0/75 · FAIL 0/75 · NOT_PROVEN 54/75**, mitte kogu RAG-i 10/10. Järgmine sidus töö on V04 dubleeriva fanout'i vähendamine või batch'imine ja seejärel ülejäänud 54 juhtumi värav plokkidena.

**Täielik püsiv leksikaalindeks on 23.08 runtime-release'il `d08b25a8` toodangus, kuid sisuline lõppvärav on PARTIAL.** Vana 8000-lõiguline varuskann kulutas üldpäringus 5,50–7,29 s. Uus atomaarne SQLite FTS5 indeks katab 49 727 aktiivset lõiku / 6073 aktiivset dokumenti ja on 437,9 MiB. Toodangu startup ehitas selle 46,59 sekundiga; health näitas `ready=true` ja registriga kattuvat põlvkonda. Kolme üldpäringu leksikaalajad olid 517/512/537 ms, retrieval 841/811/842 ms ning kõik vastused `complete=true`, `partial=false`, `degraded=false`. Release-ülevaatus sulges ka 30-sekundilise proksi vea: ingest, reindex, metadata patch ja tombstone märgivad indeksi kohe stale'iks ning ajastavad ühe koondatud taustarefresh'i, mitte ei oota täisehitust. Autenditud release-kontroll leidis kolm üldist regressiooni: liitküsimuse nimeparser valis asesõnafraasi, iseseisev abiküsimus päris eelmise KOV-i ja uuringudokumendi identiteet eelistas sisuteksti kattuvust pealkirjale. Need parandati RAG-loogika SHA-l `914e1452`; samas vestluses läbisid Laur Raudsoo 5 kuvatud autorallikaga, KOV-i järel omavalitsuseta üldküsimus kolme üleriigilise SHS-allikaga ning V06 `169 / 2018` Merli Lauri avatud allikaga. Järelrelease `da2c79c4` lisas määruse käändevormid: lühike Kuusalu § 6 küsimus läbis 6401 ms-ga `explicit_paragraph` raja, üks valitud ja kuvatud allikas kattusid ning avatud paneel näitas õiget „§ 6 Koduteenus” allikat. Server, `origin/main` ja kolm teenust olid samal koodirelease'il; `/vestlus` vastas 200 ning FTS5 jäi `ready=true` / 49 727 lõigu peale. Automaatteste ei loodud ega käivitatud. Päris ingest/reindex/tombstone taustasünkroonsus, Kadri Soo autorimeta vastuolu ja ülejäänud kvaliteedimaatriks jäävad **NOT_PROVEN** või avatuks.

**Mitmekeelse RAG-i kitsas autorirada on 23.08 RAG-loogika SHA-l `243da993` põhjusepõhiselt laiendatud; üldvärav jääb PARTIAL.** Keeleplaan eristab UI, küsimuse, retrieval'i ja vastuse keelt, säilitab nimed ning suunab RU/EN autoriküsimuse eestikeelsesse metadata- ja hübriidotsingusse. Jätk sulges pärisnime täpitähe järgi vale ET-tuvastuse, kirillitsas ja ladina tähtedega vene küsimuse, `ё`/`ö` nimevaste, kaasautori, lühikese jätkuküsimuse, selge vastusekeele korralduse ning liiga üldise teemaatribuudi veaklassid. Viimase SHA vastu andis vene kaasautoriküsimus 11,453 s-ga ühe õige valitud ja kuvatud 2016 allika; ingliskeelne Kadi Lubi üldküsimus 5,614 s-ga neli kattuvat toetavat allikat. Keeleleping läbis `RU UI + ET küsimus → ET vastus`, `EN UI + RU küsimus → RU vastus` ja `EN UI + RU küsimus + vasta ET → ET vastus`; konto UI taastati eesti keelde. Kõik kontrollid tehti olemasolevas autentitud vestluses ilma „Uus vestlus” workaround'ita. Mõjutatud juhtumid korrati pärast põhjuse parandust, kuid kogu 14 juhtumi ploki täiskordus viimasel SHA-l, üldised sünteesi-, KOV-, õigus- ja arvufaktirajad ning kogu mitmekeelne pariteet jäävad `NOT_PROVEN`. Funktsioonilüliti võimaldab kitsa raja välja lülitada; kolme indeksit ei tehta ning ET põhimaatriks jääb **DONE 21/75 · NOT_PROVEN 54/75**.
...
**Vestluse RAG-i süsteemne parandus ja Sotsiaaltöö puhas reindeks on 22.08 toodangus
tõendatud (`cfa26eab`, `ce56eb35`, `d984da5b`).** Korje-eelarve ei sõltu enam vastuse
`top_k`-st, lühike arvuküsimus avab õige artikli sisemised lõigud, tulemused säilitavad oma
tegelikud leidmiskanalid ning allikate sisuline võrdlusküsimus ei lähe enam ekslikult
kasutajaliidese mureks ega lülita RAG-i välja. Enne reindeksit tehti taastatav kontrollitud
varukoopia `/var/backups/sotsiaalai-rag/rag-before-journal-reindex-20260821T231641Z.tar.gz`;
puhtalt reindekseeriti 863 aktiivset ajakirjadokumenti ja 10 juba kustutatud vana aliast jäeti
teadlikult välja. Toodangu 49 727-lõiguline indeks läbis ajakirja faktivärava **11/11** ja
uuringute, juhendite ning muude materjalide värava **10/10**, ühtegi poolikut otsingut ei olnud.
Samas olemasolevas autentitud vestluses vastas 2017. aasta kaardistuse küsimus 6,7 sekundiga
õigesti 25% / 45% / 30%; viie aasta süntees valis 66 tabamusest 34 allikagruppi, kattis kõik
aastad 2021–2025 ja valmis 37 sekundiga. Lõplik UTC värav on **4931/4931**. Järgmine RAG-töö
on laiem juhuvalimiga autentitud vestluse kvaliteedimõõtmine; praegused teadaolevad otsinguaugud
on suletud, kuid ükski fikseeritud värav üksi ei tõenda kogu korpuse absoluutset veatust.
...
Vestluskiht ei ole RAG-i peale lisatud teine assistent. Tõendatud küsimus läbib tavapärase ühe
vastusemudelikõne; ebaselge, kuid sotsiaalvaldkonda kuuluva küsimuse sama kõne võib anda vastuse
asemel ühe loomuliku täpsustuse. Järgmine kasutaja vastus seotakse serveris ainult selle sama
lõpetatud küsimuse ja täpsustusega ning läbib uuesti tavalise planner'i, RAG-i, riski-, tõendi-
ja allikatee. Väljaspool sotsiaalvaldkonda olev küsimus saab vastuse „Vastan ainult
sotsiaalvaldkonna küsimustele.” ilma mudelikõne ja allikateta. Assistent võib aidata kasutaja
sõnastust mõista, kuid ei tohi otsida veebist ega täita teadmusbaasi lünka mudeli üldteadmisega.
Kui tõendatud vastust ei saa veel anda, ei jäeta vestlust tehnilise RAG-teatega pooleli:
võimaluse korral küsitakse puuduvat teemat, dokumenti, aastat, mõõdikut või kontakti; muul juhul
öeldakse inimlikult, millist täpsustust on vastamiseks vaja. Valideerimata fakt ja selle allikad
jäävad endiselt fail-closed.
