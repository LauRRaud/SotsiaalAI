#!/usr/bin/env node
/**
 * SOL-MAT-08 isolated runtime orchestrator.
 *
 ...
 * loopback-only OpenAI-compatible stub. The worker owns the material derivative journey;
...
const RAG_SERVICE_KEY = "sol-mat-08-loopback-runtime-key-01"
const sourceDatabaseUrl = String(
  process.env.MATERIAL_RAG_PROBE_DATABASE_URL
...
const worker = path.resolve("scripts/material-sanitized-rag-live-probe-worker.mjs")
const databaseName = `sol_mat_08_live_${Date.now()}_${process.pid}`
...
function expectSafeDatabaseName(value) {
  if (!/^sol_mat_08_live_[0-9_]+$/u.test(value)) {
...
async function main() {
  ...
  temporaryRoot = await mkdtemp(path.join(tmpdir(), "sol-mat-08-live-"))
  ...
  const pythonEnv = {
    ...
    RAG_COLLECTION: `sol_mat_08_${process.pid}`,
  ...
  await runCheckedAsync(process.execPath, [
    ...
    worker
  ], { env: runtimeEnv, label: "material sanitized RAG live worker" })
...
async function cleanup() {
  ...
  console.log(
    `RUNTIME_CLEANUP remote_material=absent database=${databaseCount} rag_storage=0 embedding_requests=${embeddingRequests} unexpected_requests=${unexpectedStubRequests}`
  )
...
try {
  ...
  console.error(`SOL-MAT-08 isolated runtime: FAIL — ${error?.message || "unknown error"}`)
  process.exitCode = 1
} finally {
  await cleanup().catch(error => {
    console.error(`SOL-MAT-08 cleanup: FAIL — ${error?.message || "unknown error"}`)
    process.exitCode = 1
  })
}
