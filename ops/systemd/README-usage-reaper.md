...
a grace window) and releases them through the **same atomic `usageService.release()`
path** a normal release uses, so the bucket counter is correctly restored and a
`RELEASED` usage event is recorded. It never touches `COMMITTED` rows (a reservation used
between the scan and the release is skipped, not double-freed) and never touches rows
without an `expiresAt`.
...
# Optional tuning:
# USAGE_REAPER_GRACE_MINUTES=5     # only reap rows expired longer ago than this
...
# With the key + dryRun the route reports how many rows WOULD be reaped and releases nothing:
...
Every 10 minutes (`OnCalendar=*:0/10`, +90s jitter). Leaked holds are not time-critical;
this keeps quota accurate without churn. `scripts/deploy-server.mjs` does not restart it
(it is a timer-driven oneshot), so no deploy change is needed once installed.
